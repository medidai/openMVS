...
bool Application::Initialize(size_t argc, LPCTSTR *argv) {
  ...
  if (OPT::strOutputFileName.empty())
    OPT::strOutputFileName =
        Util::getFileName(OPT::strInputFileName) + "scene" MVS_EXT;
...
void RangeToDepthMap(const Image32F &rangeMap, const Camera &camera,
                     DepthMap &depthMap) {
  ...
  for (int y = 0; y < depthMap.rows; ++y) {
    ...
    for (int x = 0; x < depthMap.cols; ++x) {
      ...
      depthRow[x] =
          (Depth)(range <= 0
                      ? 0
                      : normalized(camera.TransformPointI2C(Point2(x, y))).z *
                            range);
    }
  }
}

// parse scene stored in MVSNet format composed of undistorted images and camera
// poses for example see GigaVision benchmark (http://gigamvs.net):
// |--sceneX
//   |--images
//     |--xxx.jpg
//     |--xxx.jpg
...
//     ....
//
// where the camera parameter of one image stored in a cam.txt file contains the
// camera extrinsic E = [R|t], intrinsic K and the depth range:
//  extrinsic
//  E00 E01 E02 E03
//  E10 E11 E12 E13
//  E20 E21 E22 E23
...
bool ParseSceneMVSNet(Scene &scene, const String &strPath) {
  ...
  for (const std::filesystem::directory_entry &entry :
       std::filesystem::directory_iterator(
           (strPath + MVSNET_IMAGES_FOLDER).c_str())) {
    if (entry.path().extension() != MVSNET_IMAGES_EXT)
      continue;
    // parse camera
    const std::string strCamFileName(
        strPath + MVSNET_CAMERAS_FOLDER PATH_SEPARATOR_STR +
        entry.path().stem().string().c_str() + MVSNET_CAMERAS_NAME);
    ...
    // setup camera
    IIndex platformID;
    if (prevPlatformID == NO_ID ||
        !K.IsEqual(scene.platforms[prevPlatformID].cameras[0].K, 1e-3)) {
...
bool ParseSceneNerfstudio(Scene &scene, const String &strPath) {
  const nlohmann::json data =
      nlohmann::json::parse(std::ifstream(strPath + NERFSTUDIO_TRANSFORMS));
  if (data.empty())
    return false;
  // parse camera
  const cv::Size resolution(data["w"].get<uint32_t>(),
                            data["h"].get<uint32_t>());
  ...
  if (cameraModel == "SIMPLE_PINHOLE") {
    ...
    if (cameraModel == "PINHOLE" || cameraModel == "SIMPLE_RADIAL" ||
        cameraModel == "RADIAL" || cameraModel == "OPENCV") {
  ...
  for (const nlohmann::json &frame : frames) {
    ...
    const String strFileName(strPath +
                             frame["file_path"].get<std::string>().c_str());
    Image &imageData = scene.images.emplace_back();
    imageData.platformID = platformID;
    imageData.cameraID =
        0; // only one camera per platform supported by this format
    ...
    Platform::Pose &pose = platform.poses.emplace_back();
    const auto Ps =
        frame["transform_matrix"].get<std::vector<std::vector<double>>>();
    Eigen::Matrix4d P{{Ps[0][0], Ps[0][1], Ps[0][2], Ps[0][3]},
    ...
    DepthMap depthMap;
    {
      const String depthPath(
          strPath + String::FormatString("outputs/depth%04u.exr", imageID));
    ...
    NormalMap normalMap;
    {
      const String normalPath(
          strPath + String::FormatString("outputs/normal%04u.exr", imageID));
    ...
    const String dmapPath(strPath +
                          String::FormatString("depth%04u.dmap", imageID));
    if (!ExportDepthDataRaw(dmapPath, imageData.name, IDs, resolution, camera.K,
                            pose.R, pose.C, (float)dMin, (float)dMax, depthMap,
                            normalMap, confMap, viewsMap)) {
...
#if defined(_SUPPORT_CPP17) && (!defined(__GNUC__) || (__GNUC__ > 7))
  for (const std::filesystem::directory_entry &entry :
       std::filesystem::directory_iterator(strPath.c_str())) {
...
#pragma omp parallel for schedule(dynamic)
  ...
  FOREACH(i, strImageNames) {
    ...
    const nlohmann::json dataCamera =
        nlohmann::json::parse(std::ifstream(strFileName + RTMV_CAMERAS_EXT));
    if (dataCamera.empty())
      continue;
    const nlohmann::json &data = dataCamera["camera_data"];
    const cv::Size resolution(data["width"].get<uint32_t>(),
                              data["height"].get<uint32_t>());
    // set platform
...
#pragma omp critical
    ...
    Eigen::Matrix4d P{{Ps[0][0], Ps[1][0], Ps[2][0], Ps[3][0]},
    ...
    {
      cv::Mat imgMask =
          cv::imread(strFileName + ".seg.exr", cv::IMREAD_UNCHANGED);
      if (imgMask.empty()) {
        VERBOSE("Unable to load segmentation mask %s.",
                (strFileName + ".seg.exr").c_str());
        continue;
      }
    ...
    DepthMap depthMap;
    {
      const cv::Mat imgDepthMap =
          cv::imread(strFileName + ".depth.exr", cv::IMREAD_UNCHANGED);
      if (imgDepthMap.empty()) {
        VERBOSE("Unable to load depthmap %s.",
                (strFileName + ".depth.exr").c_str());
        continue;
      }
    ...
    const String dmapPath(strPath +
                          String::FormatString("depth%04u.dmap", imageID));
    if (!ExportDepthDataRaw(dmapPath, imageData.name, IDs, resolution, K,
                            pose.R, pose.C, (float)dMin, (float)dMax, depthMap,
                            normalMap, confMap, viewsMap)) {
      VERBOSE("Unable to save dmap: %s", dmapPath.c_str());
      continue;
...
bool ParseScene(Scene &scene) {
  ...
  switch (sceneType) {
  case NERFSTUDIO:
    return ParseSceneNerfstudio(scene, strPath);
  case RTMV:
    return ParseSceneRTMV(scene, strPath);
  default:
    return ParseSceneMVSNet(scene, strPath);
  }
#else
  VERBOSE("error: C++17 is required to parse MVSNet format");
  return false;
...
int main(int argc, LPCTSTR *argv) {
  ...
  VERBOSE("Imported data: %u platforms, %u images, %u vertices (%s)",
          scene.platforms.size(), scene.images.size(),
