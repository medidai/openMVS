...
#define COLMAP_STEREO_CONSISTENCYGRAPHS_FOLDER                                 \
  COLMAP_STEREO_FOLDER _T("consistency_graphs/")
...
bool Application::Initialize(size_t argc, LPCTSTR *argv) {
  ...
  generic.add_options()(
      "help,h", "imports SfM or MVS scene stored in COLMAP undistoreted format "
                "OR exports MVS scene to COLMAP format")(
      "working-folder,w",
      ...
      "archive-type",
      boost::program_options::value(&OPT::nArchiveType)
          ->default_value(ARCHIVE_MVS),
      "project archive type: -1-interface, 0-text, 1-binary, 2-compressed "
      "binary")(
      "process-priority",
  ...
  config.add_options()(
      ...
      "input COLMAP folder containing cameras, images and points files OR "
      "input MVS project file")(
      "pointcloud-file,p",
      boost::program_options::value<std::string>(&OPT::strPointCloudFileName),
      "point-cloud with views file name (overwrite existing point-cloud)")(
      "output-file,o",
      boost::program_options::value<std::string>(&OPT::strOutputFileName),
      "output filename for storing the MVS project")(
      "image-folder",
      boost::program_options::value<std::string>(&OPT::strImageFolder)
          ->default_value(COLMAP_IMAGES_FOLDER),
      "folder to the undistorted images")(
      "normalize,f",
      boost::program_options::value(&OPT::bNormalizeIntrinsics)
          ->default_value(false),
      "normalize intrinsics while exporting to MVS format")(
      "force-points,e",
      boost::program_options::value(&OPT::bForceSparsePointCloud)
          ->default_value(false),
      "force exporting point-cloud as sparse points also even if dense "
      "point-cloud detected")(
      "binary",
      boost::program_options::value(&OPT::bBinary)->default_value(true),
      "use binary format for cameras, images and points files")(
      "no-points",
      boost::program_options::value(&OPT::bExportNoPoints)
          ->default_value(false),
      "export cameras, images and points files but not including the sparse "
      "point-cloud")("common-intrinsics",
                     boost::program_options::value(&OPT::bForceCommonIntrinsics)
                         ->default_value(false),
                     "force using common intrinsics for all cameras");
  ...
  try {
    ...
    boost::program_options::store(
        boost::program_options::command_line_parser((int)argc, argv)
  ...
  OPEN_LOGFILE(
      MAKE_PATH(APPNAME _T("-") + Util::getUniqueName(0) + _T(".log")));

  // print application details: version and command line
  Util::LogBuild();
  LOG(_T("Command line: ") APPNAME _T("%s"),
      Util::CommandLineToString(argc, argv).c_str());
  ...
  if (OPT::vm.count("help") || bInvalidCommand) {
    ...
    GET_LOG() << _T("\n"
                    "Import/export 3D reconstruction from COLMAP (TXT/BIN "
                    "format) and to COLMAP (TXT format). \n"
                    "In order to import a scene, run COLMAP SfM and next "
                    "undistort the images (only PINHOLE\n"
  ...
  const String strInputFileNameExt(
      Util::getFileExt(OPT::strInputFileName).ToLower());
  OPT::bFromOpenMVS = (strInputFileNameExt == MVS_EXT);
  if (OPT::bFromOpenMVS) {
    OPT::strImageFolder =
        MAKE_PATH_FULL(WORKING_FOLDER_FULL, OPT::strImageFolder);
...
const std::vector<String> mapCameraModel = {"SIMPLE_PINHOLE",
                                            "PINHOLE",
                                            "SIMPLE_RADIAL",
                                            "RADIAL",
                                            "OPENCV",
...
// tools
bool NextLine(std::istream &stream, std::istringstream &in,
  ...
  do {
    ...
    if (stream.fail())
      return false;
  } while (((bIgnoreEmpty && line.empty()) || line[0u] == '#') &&
           stream.good());
  in.clear();
  in.str(line);
  return true;
...
struct Camera {
  ...
  struct CameraEqualTo {
    bool operator()(const Camera &_Left, const Camera &_Right) const {
      return _Left.model == _Right.model && _Left.width == _Right.width &&
             _Left.height == _Right.height && _Left.params == _Right.params;
    }
  };
  ...
  bool WriteBIN(std::ostream &stream) {
    ...
    const int64 modelId(std::distance(
        mapCameraModel.begin(),
        std::find(mapCameraModel.begin(), mapCameraModel.end(), model)));
...
struct Image {
  ...
  bool ReadTXT(std::istream &stream) {
    ...
    if (!NextLine(stream, in))
      return false;
    in >> ID >> q.w() >> q.x() >> q.y() >> q.z() >> t(0) >> t(1) >> t(2) >>
        idCamera >> name;
    if (in.fail())
      return false;
  ...
  bool WriteTXT(std::ostream &out) const {
    out << ID << _T(" ") << q.w() << _T(" ") << q.x() << _T(" ") << q.y()
        << _T(" ") << q.z() << _T(" ") << t(0) << _T(" ") << t(1) << _T(" ")
        << t(2) << _T(" ") << idCamera << _T(" ") << name << std::endl;
    for (const Proj &proj : projs) {
      out << proj.p(0) << _T(" ") << proj.p(1) << _T(" ") << (int)proj.idPoint
          << _T(" ");
      if (out.fail())
        return false;
    }
...
struct Point {
  ...
  bool WriteTXT(std::ostream &out) const {
    ...
    out << ID << _T(" ") << p.x << _T(" ") << p.y << _T(" ") << p.z << _T(" ")
        << r << _T(" ") << g << _T(" ") << b << _T(" ") << e << _T(" ");
...
template <typename T> struct Mat {
  ...
  size_t GetNumBytes() const { return data_.size() * sizeof(T); }
...
bool DetermineInputSource(const String &filenameTXT, const String &filenameBIN,
                          std::ifstream &file, String &filenameCamera,
                          bool &binary) {
...
bool ImportScene(const String &strFolder, const String &strOutFolder,
  ...
  CamerasMap mapCameras;
  {
    ...
    if (!DetermineInputSource(filenameCamerasTXT, filenameCamerasBIN, file,
                              filenameCamera, binary)) {
      return false;
    }
    LOG_OUT() << "Reading cameras: " << filenameCamera << std::endl;

    typedef std::unordered_map<COLMAP::Camera, uint32_t,
                               COLMAP::Camera::CameraHash,
                               COLMAP::Camera::CameraEqualTo>
        CamerasSet;
    CamerasSet setCameras;
    COLMAP::Camera colmapCamera;
    while (file.good() && colmapCamera.Read(file, binary)) {
      const auto setIt(
          setCameras.emplace(colmapCamera, (uint32_t)scene.platforms.size()));
      ...
      platform.name = String::FormatString(
          _T("platform%03u"),
          colmapCamera.ID); // only one camera per platform supported
      Interface::Platform::Camera camera;
      camera.name = colmapCamera.model;
      camera.K = Interface::Mat33d::eye();
      // account for different pixel center conventions as COLMAP uses pixel
      // center at (0.5,0.5)
      ...
      if (OPT::bNormalizeIntrinsics) {
        ...
        camera.K = Camera::ScaleK<double>(
            camera.K, 1.0 / Camera::GetNormalizationScale(colmapCamera.width,
                                                          colmapCamera.height));
      } else {
  ...
  ImagesMap mapImages;
  {
    ...
    if (!DetermineInputSource(filenameImagesTXT, filenameImagesBIN, file,
                              filenameImages, binary)) {
      return false;
    }
    ...
    while (file.good() && imageColmap.Read(file, binary)) {
      ...
      Eigen::Map<EVec3d>(&pose.C.x) =
          -(imageColmap.q.inverse() * imageColmap.t);
      Interface::Image image;
      image.name =
          MAKE_PATH_REL(strOutFolder, OPT::strImageFolder + imageColmap.name);
  ...
  const String filenameDenseVisPoints(strFolder +
                                      COLMAP_DENSE_POINTS_VISIBILITY);
  {
    ...
    if (!DetermineInputSource(filenamePointsTXT, filenamePointsBIN, file,
                              filenamePoints, binary)) {
      return false;
    }
    ...
    while (file.good() && point.Read(file, binary)) {
      ...
      std::sort(vertex.views.begin(), vertex.views.end(),
                [](const Interface::Vertex::View &view0,
                   const Interface::Vertex::View &view1) {
  ...
  pointcloud.Release();
  if (File::access(filenameDensePoints) &&
      File::access(filenameDenseVisPoints)) {
    // parse dense point-cloud
    LOG_OUT() << "Reading points: " << filenameDensePoints << " and "
              << filenameDenseVisPoints << std::endl;
    ...
    if (!file.isOpen()) {
      VERBOSE("error: unable to open file '%s'",
              filenameDenseVisPoints.c_str());
      return false;
    }
  ...
  if (File::isFolder(pathDepthMaps) && File::isFolder(pathNormalMaps)) {
    ...
    CLISTDEF2IDX(IIndexArr, IIndex)
    imagesNeighbors((IIndex)scene.images.size());
    {
      const String filenameFusion(strFolder + COLMAP_PATCHMATCH);
      LOG_OUT() << "Reading patch-match configuration: " << filenameFusion
                << std::endl;
      ...
      while (true) {
        ...
        if (file.fail() || imageName.empty() || neighbors.empty())
          break;
        const auto it_image =
            std::find_if(mapImages.begin(), mapImages.end(),
    ...
    LOG_OUT() << "Reading depth-maps/normal-maps: " << pathDepthMaps << " and "
              << pathNormalMaps << std::endl;
    ...
    FOREACH(idx, scene.images) {
      ...
      for (const String &type : strType) {
        ...
        if (File::isFile(filenameDepthMaps)) {
          ...
          const String filenameNormalMaps(pathNormalMaps + filenameImage +
                                          type);
      ...
      if (!colDepthMap.data_.empty()) {
        ...
        const Interface::Platform::Pose pose(
            platform.GetPose(image.cameraID, image.poseID));
        const Interface::Mat33d K(
            platform.GetFullK(image.cameraID, (uint32_t)colDepthMap.width_,
                              (uint32_t)colDepthMap.height_));
        MVS::DepthMap depthMap((int)colDepthMap.height_,
                               (int)colDepthMap.width_);
        memcpy(depthMap.getData(), colDepthMap.data_.data(),
               colDepthMap.GetNumBytes());
        MVS::NormalMap normalMap;
        if (!colNormalMap.data_.empty()) {
          normalMap.create((int)colNormalMap.height_, (int)colNormalMap.width_);
          cv::merge(
              std::vector<cv::Mat>{
                  cv::Mat((int)colNormalMap.height_, (int)colNormalMap.width_,
                          CV_32F, (void *)colNormalMap.GetChannelPtr(0)),
                  cv::Mat((int)colNormalMap.height_, (int)colNormalMap.width_,
                          CV_32F, (void *)colNormalMap.GetChannelPtr(1)),
                  cv::Mat((int)colNormalMap.height_, (int)colNormalMap.width_,
                          CV_32F, (void *)colNormalMap.GetChannelPtr(2))},
              normalMap);
        }
        MVS::ConfidenceMap confMap;
        MVS::ViewsMap viewsMap;
        const auto depthMM(std::minmax_element(colDepthMap.data_.cbegin(),
                                               colDepthMap.data_.cend()));
        const MVS::Depth dMin(*depthMM.first), dMax(*depthMM.second);
        if (!ExportDepthDataRaw(
                strOutFolder + String::FormatString("depth%04u.dmap", image.ID),
                MAKE_PATH_FULL(strOutFolder, image.name), IDs, depthMap.size(),
                K, pose.R, pose.C, dMin, dMax, depthMap, normalMap, confMap,
                viewsMap))
          return false;
      }
    }
...
bool ImportPointCloud(const String &strPointCloudFileName, Interface &scene) {
  ...
  FOREACH(i, pointcloud.points) {
    ...
    FOREACH(j, pointcloud.pointViews[i]) {
      ...
      view.confidence =
          (pointcloud.pointWeights.empty() ? 0.f
                                           : pointcloud.pointWeights[i][j]);
    }
    scene.vertices.emplace_back(std::move(vertex));
    if (!pointcloud.colors.empty()) {
      const Pixel8U &c = pointcloud.colors[i];
      scene.verticesColor.emplace_back(
          Interface::Color{Interface::Col3{c.b, c.g, c.r}});
    }
    if (!pointcloud.normals.empty())
      scene.verticesNormal.emplace_back(
          Interface::Normal{pointcloud.normals[i]});
  }
  return true;
}

bool ExportScene(const String &strFolder, const Interface &scene,
                 bool bForceSparsePointCloud = false,
                 bool bForceCommonIntrinsics = false, bool noPoints = false,
                 bool binary = true) {
  ...
  {
    const String filenameCameras(
        strFolder + (binary ? COLMAP_CAMERAS_BIN : COLMAP_CAMERAS_TXT));
    LOG_OUT() << "Writing cameras: " << filenameCameras << std::endl;
    std::ofstream file(filenameCameras, binary
    ...
    if (binary) {
      ...
      file << _T("# Camera list with one line of data per camera:")
           << std::endl;
    ...
    for (uint32_t ID = 0; ID < (uint32_t)scene.platforms.size(); ++ID) {
      ...
      ASSERT(platform.cameras.size() ==
             1); // only one camera per platform supported
      ...
      if (camera.width == 0 || camera.height == 0) {
        ...
        for (const Interface::Image &image : scene.images) {
          if (image.platformID == ID && image.cameraID == 0 &&
              image.poseID != NO_ID) {
            pImage = &image;
            break;
          }
  ...
  constexpr uint32_t avgResolutionSmallView(640 * 480),
      avgResolutionLargeView(6000 * 4000);
  constexpr uint32_t avgPointsPerSmallView(3000), avgPointsPerLargeView(12000);
  {
    ...
    for (uint32_t ID = 0; ID < (uint32_t)scene.images.size(); ++ID) {
      ...
      img.name = MAKE_PATH_REL(OPT::strImageFolder,
                               MAKE_PATH_FULL(WORKING_FOLDER_FULL, image.name));
      ...
      const float linearFactor(
          float(avgResolutionLargeView - resolutionView) /
          (avgResolutionLargeView - avgResolutionSmallView));
      maxNumPointsSparse +=
          (avgPointsPerSmallView +
           (avgPointsPerLargeView - avgPointsPerSmallView) * linearFactor) /
          avgViewsPerPoint;
    }
  }

  // auto-select dense or sparse mode based on number of points
  const bool bSparsePointCloud(scene.vertices.size() <
                               (size_t)maxNumPointsSparse);
  if (bSparsePointCloud || bForceSparsePointCloud) {
    // write points list
    {
      const String filenamePoints(
          strFolder + (binary ? COLMAP_POINTS_BIN : COLMAP_POINTS_TXT));
      LOG_OUT() << "Writing points: " << filenamePoints << std::endl;
      std::ofstream file(filenamePoints,
                         binary ? std::ios::trunc | std::ios::binary
                                : std::ios::trunc);
      if (!file.good()) {
      ...
      if (binary) {
        ...
        file << _T("# 3D point list with one line of data per point:")
             << std::endl;
        file << _T("#   POINT3D_ID, X, Y, Z, R, G, B, ERROR, TRACK[] as ")
                _T("(IMAGE_ID, POINT2D_IDX)")
             << std::endl;
      }
      ...
      if (!noPoints) {
        for (uint32_t ID = 0; ID < (uint32_t)scene.vertices.size(); ++ID) {
          ...
          for (const Interface::Vertex::View &view : vertex.views) {
            ...
            point.tracks.emplace_back(
                COLMAP::Point::Track{img.ID, (uint32_t)img.projs.size()});
            COLMAP::Image::Proj proj;
            proj.idPoint = ID;
            const Point3 X(vertex.X);
            ProjectVertex_3x4_3_2(cameras[view.imageID].P.val, X.ptr(),
                                  proj.p.data());
            // account for different pixel center conventions as COLMAP uses
            // pixel center at (0.5,0.5)
            proj.p[0] += REAL(0.5);
            proj.p[1] += REAL(0.5);
            img.projs.emplace_back(proj);
          }
          point.c = scene.verticesColor.empty() ? Interface::Col3(255, 255, 255)
                                                : scene.verticesColor[ID].c;
          point.e = 0;
          if (numPoints3D != 0) {
    ...
    if (!noPoints) {
      ...
      // write fusion list
      {
        ...
        LOG_OUT() << "Writing fusion configuration: " << filenameFusion
                  << std::endl;
      ...
      {
        const String filenameFusion(strFolder + COLMAP_PATCHMATCH);
        LOG_OUT() << "Writing patch-match configuration: " << filenameFusion
                  << std::endl;
        std::ofstream file(filenameFusion);
        if (!file.good()) {
          VERBOSE("error: unable to open file '%s'", filenameFusion.c_str());
  ...
  if (!noPoints && !bSparsePointCloud) {
    ...
    const String filenameDenseVisPoints(strFolder +
                                        COLMAP_DENSE_POINTS_VISIBILITY);
    LOG_OUT() << "Writing points: " << filenameDensePoints << " and "
              << filenameDenseVisPoints << std::endl;
    File file(filenameDenseVisPoints, File::WRITE,
              File::CREATE | File::TRUNCATE);
    if (!file.isOpen()) {
      VERBOSE("error: unable to write file '%s'",
              filenameDenseVisPoints.c_str());
      return false;
    }
  ...
  // write images list
  {
    const String filenameImages(
        strFolder + (binary ? COLMAP_IMAGES_BIN : COLMAP_IMAGES_TXT));
    LOG_OUT() << "Writing images: " << filenameImages << std::endl;
    std::ofstream file(filenameImages, binary
                                           ? std::ios::trunc | std::ios::binary
                                           : std::ios::trunc);
    if (!file.good()) {
    ...
    if (binary) {
      ...
      file << _T("#   IMAGE_ID, QW, QX, QY, QZ, TX, TY, TZ, CAMERA_ID, NAME")
           << std::endl;
...
bool ExportIntrinsicsTxt(const String &fileName, const Interface &scene) {
  ...
  const Interface::Mat33d &K = scene.platforms[image.platformID].GetFullK(
      image.cameraID, pImage->GetWidth(), pImage->GetHeight());
  ...
  out << K4(0, 0) << _T(" ") << K4(0, 1) << _T(" ") << K4(0, 2) << _T(" ")
      << K4(0, 3) << _T("\n");
  out << K4(1, 0) << _T(" ") << K4(1, 1) << _T(" ") << K4(1, 2) << _T(" ")
      << K4(1, 3) << _T("\n");
  out << K4(2, 0) << _T(" ") << K4(2, 1) << _T(" ") << K4(2, 2) << _T(" ")
      << K4(2, 3) << _T("\n");
  out << K4(3, 0) << _T(" ") << K4(3, 1) << _T(" ") << K4(3, 2) << _T(" ")
      << K4(3, 3) << _T("\n");
  return !out.fail();
}
...
bool ExportImagesLog(const String &fileName, const Interface &scene) {
  ...
  for (IIndex ID : orderedImages) {
    ...
    out << T(0, 0) << _T(" ") << T(0, 1) << _T(" ") << T(0, 2) << _T(" ")
        << T(0, 3) << _T("\n");
    out << T(1, 0) << _T(" ") << T(1, 1) << _T(" ") << T(1, 2) << _T(" ")
        << T(1, 3) << _T("\n");
    out << T(2, 0) << _T(" ") << T(2, 1) << _T(" ") << T(2, 2) << _T(" ")
        << T(2, 3) << _T("\n");
    out << T(3, 0) << _T(" ") << T(3, 1) << _T(" ") << T(3, 2) << _T(" ")
        << T(3, 3) << _T("\n");
  }
  return !out.fail();
}
...
bool ExportImagesCamera(const String &pathName, const Interface &scene) {
  ...
  for (uint32_t ID = 0; ID < (uint32_t)scene.images.size(); ++ID) {
    ...
    String imageFileName(image.name);
    Util::ensureValidPath(imageFileName);
    const String fileName(pathName + Util::getFileNameExt(imageFileName) +
                          ".camera");
    std::ofstream out(fileName);
    if (!out.good()) {
      VERBOSE("error: unable to open file '%s'", fileName.c_str());
    ...
    if (image.platformID != NO_ID && image.cameraID != NO_ID) {
      ...
      const Interface::Platform::Camera &camera =
          platform.cameras[image.cameraID];
...
int main(int argc, LPCTSTR *argv) {
  ...
  if (OPT::bFromOpenMVS) {
    ...
    if (Util::getFileExt(OPT::strOutputFileName) == _T(".log")) {
      ...
      ExportIntrinsicsTxt(
          MAKE_PATH_FULL(WORKING_FOLDER_FULL, String("intrinsics.txt")), scene);
      ExportImagesLog(MAKE_PATH_SAFE(OPT::strOutputFileName), scene);
    } else if (Util::getFileExt(OPT::strOutputFileName) == _T(".camera")) {
      // write poses in Strecha camera format
      ExportImagesCamera(
          (OPT::strOutputFileName = Util::getFileFullName(
               MAKE_PATH_FULL(WORKING_FOLDER_FULL, OPT::strOutputFileName))) +
              PATH_SEPARATOR,
          scene);
    } else {
      // write COLMAP input data
      if (!OPT::strPointCloudFileName.empty() &&
          !ImportPointCloud(MAKE_PATH_SAFE(OPT::strPointCloudFileName), scene))
        return EXIT_FAILURE;
      Util::ensureFolderSlash(OPT::strOutputFileName);
      ExportScene(MAKE_PATH_SAFE(OPT::strOutputFileName), scene,
                  OPT::bForceSparsePointCloud, OPT::bForceCommonIntrinsics,
                  OPT::bExportNoPoints, OPT::bBinary);
    }
    VERBOSE("Input data exported: %u images & %u vertices (%s)",
            scene.images.size(), scene.vertices.size(),
            TD_TIMER_GET_FMT().c_str());
  } else {
    // read COLMAP input data
    Interface scene;
    const String strOutFolder(Util::getFilePath(
        MAKE_PATH_FULL(WORKING_FOLDER_FULL, OPT::strOutputFileName)));
    PointCloud pointcloud;
    ...
    if (!pointcloud.IsEmpty() &&
        !pointcloud.Save(
            MAKE_PATH_SAFE(Util::getFileFullName(OPT::strOutputFileName)) +
                _T(".ply"),
            true))
      return EXIT_FAILURE;
    VERBOSE("Exported data: %u images, %u points%s (%s)", scene.images.size(),
            scene.vertices.size(),
            pointcloud.IsEmpty() ? ""
                                 : String::FormatString(", %d dense points",
                                                        pointcloud.GetSize())
                                       .c_str(),
            TD_TIMER_GET_FMT().c_str());
  }
