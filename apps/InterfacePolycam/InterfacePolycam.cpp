...
bool Application::Initialize(size_t argc, LPCTSTR *argv) {
  ...
  generic.add_options()("help,h", "imports SfM scene stored Polycam format")(
      "working-folder,w",
  ...
  config.add_options()(
      ...
      "input folder containing Polycam camera poses, images and depth-maps")(
      "output-file,o",
...
bool ParseImage(Scene &scene, const String &imagePath, const String &cameraPath,
  ...
  const Eigen::Matrix3d R_session_arkitcam{
      {data["t_00"].get<double>(), data["t_01"].get<double>(),
       data["t_02"].get<double>()},
      {data["t_10"].get<double>(), data["t_11"].get<double>(),
       data["t_12"].get<double>()},
      {data["t_20"].get<double>(), data["t_21"].get<double>(),
       data["t_22"].get<double>()}};
  const Eigen::Vector3d t_session_arkitcam{data["t_03"].get<double>(),
                                           data["t_13"].get<double>(),
                                           data["t_23"].get<double>()};
  const Eigen::Affine3d T_session_arkitcam{
      Eigen::Affine3d(Eigen::Translation3d(t_session_arkitcam)) *
      Eigen::Affine3d(Eigen::AngleAxisd(R_session_arkitcam))};
  ...
  if (itNeighbors != data.end()) {
    const std::vector<uint64_t> neighborTimestamps =
        itNeighbors->get<std::vector<uint64_t>>();
    for (uint64_t timestamp : neighborTimestamps) {
      ...
      if (neighborID != imageData.ID)
        imageData.neighbors.emplace_back(
            ViewScore{neighborID, 0, 1.f, FD2R(15.f), 0.5f, 3.f});
    }
  }
  ...
  if (!depthPath.empty()) {
    ...
    if (!ExportDepthDataRaw(
            MAKE_PATH(String::FormatString("depth%04u.dmap", imageData.ID)),
            imageData.name, IDs, resolution, camera.K, pose.R, pose.C,
            (float)dMin, (float)dMax, depthMap, normalMap, confMap, viewsMap))
      return false;
  }
  return true;
}
...
bool ParseScene(Scene &scene, const String &scenePath) {
  ...
  size_t numCorrectedFolders(0), numCorrectedDepthFolders(0), numFolders(0),
      numDepthFolders(0);
  for (const auto &file :
       std::filesystem::directory_iterator(scenePath.c_str())) {
    ...
    else if (file.path().stem() == "cameras" || file.path().stem() == "images")
      ++numFolders;
  ...
  if (numCorrectedFolders == 2) {
    ...
    for (const auto &file : std::filesystem::directory_iterator(
             (scenePath + "corrected_images").c_str()))
    ...
    for (String &imagePath : imagePaths) {
      ...
      mapImageName.emplace(Util::getFileName(imagePath),
                           static_cast<IIndex>(mapImageName.size()));
    }
    for (const String &imagePath : imagePaths) {
      const String imageName = Util::getFileName(imagePath);
      const String cameraPath(scenePath + "corrected_cameras" +
                              PATH_SEPARATOR_STR + imageName + JSON_EXT);
      const String depthPath(numCorrectedDepthFolders
                                 ? scenePath + "corrected_depth" +
                                       PATH_SEPARATOR_STR + imageName +
                                       DEPTH_EXT
                                 : String());
    ...
    CLISTDEFIDX(String, IIndex) imagePaths;
    for (const auto &file :
         std::filesystem::directory_iterator((scenePath + "images").c_str()))
      imagePaths.emplace_back(file.path().string());
    ...
    for (const String &imagePath : imagePaths) {
      const String imageName = Util::getFileName(imagePath);
      const String cameraPath(scenePath + "cameras" + PATH_SEPARATOR_STR +
                              imageName + JSON_EXT);
      const String depthPath(numDepthFolders
                                 ? scenePath + "depth" + PATH_SEPARATOR_STR +
                                       imageName + DEPTH_EXT
                                 : String());
      if (!ParseImage(scene, imagePath, cameraPath, depthPath, mapImageName))
        return false;
...
int main(int argc, LPCTSTR *argv) {
  ...
  VERBOSE("Exported data: %u platforms, %u cameras, %u poses, %u images (%s)",
          scene.platforms.size(), scene.images.size(), scene.images.size(),
