...
Image::Image(MVS::IIndex _idx) : idx(_idx), texture(0) {}
Image::~Image() { Release(); }
...
void Image::AssignImage(cv::InputArray img) {
  ...
  if (pImg.pImage->cols % 4 != 0) {
    ...
    cv::resize(*pImg.pImage, *pImg.pImage,
               cv::Size((pImg.pImage->cols / 4) * 4, pImg.pImage->rows), 0, 0,
               cv::INTER_AREA);
  }
...
void Image::SetImage(cv::InputArray img) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, image.channels(), width, height, 0,
               (image.channels() == 1) ? GL_LUMINANCE : GL_BGR,
...
void Image::Bind() const { glBindTexture(GL_TEXTURE_2D, texture); }
/*----------------------------------------------------------------*/
