...
Window::Window()
    : window(NULL), pos(Eigen::Vector2d::Zero()),
...
void Window::SetName(LPCTSTR name) { glfwSetWindowTitle(window, name); }
...
void Window::UpdateView(const ImageArr &images,
                        const MVS::ImageArr &sceneImagesMVS) {
...
void Window::Resize(const cv::Size &_size) {
  ...
  camera.Resize(cv::Size(ROUND2INT(size.width * sizeScale),
                         ROUND2INT(size.height * sizeScale)));
}
...
void Window::Key(int k, int /*scancode*/, int action, int mod) {
  ...
  case GLFW_KEY_P:
    switch (sparseType) {
    case SPR_POINTS:
      sparseType = SPR_LINES;
      break;
    case SPR_LINES:
      sparseType = SPR_ALL;
      break;
    case SPR_ALL:
      sparseType = SPR_POINTS;
      break;
    }
    if (clbkCompilePointCloud != NULL)
      clbkCompilePointCloud();
...
void Window::MouseMove(double xpos, double ypos) {
  ...
  } else if (inputType.isSet(INP_MOUSE_MIDDLE)) {
