...
Camera::Camera(const AABB3d &_box, const Point3d &_center, float _scaleF,
               float _fov)
    : boxScene(_box), centerScene(_center),
      rotation(Eigen::Quaterniond::Identity()), center(Eigen::Vector3d::Zero()),
      dist(0), radius(100), fovDef(_fov), scaleFDef(_scaleF), prevCamID(NO_ID),
      currentCamID(NO_ID), maxCamID(0) {
  Reset();
}
...
Eigen::Matrix4d Camera::GetLookAt() const {
  ...
  Eigen::Matrix4d m;
  m << s(0), s(1), s(2), -eye.dot(s), v(0), v(1), v(2), -eye.dot(v), -n(0),
      -n(1), -n(2), eye.dot(n), 0.0, 0.0, 0.0, 1.0;
  return m;
}
void Camera::GetLookAt(Eigen::Vector3d &_eye, Eigen::Vector3d &_center,
                       Eigen::Vector3d &_up) const {
  const Eigen::Matrix3d R(GetRotation());
...
void Camera::Rotate(const Eigen::Vector2d &pos,
                    const Eigen::Vector2d &prevPos) {
...
void Camera::Translate(const Eigen::Vector2d &pos,
                       const Eigen::Vector2d &prevPos) {
  if (pos.isApprox(prevPos, ZEROTOLERANCE<double>()))
    return;
  ...
  Eigen::Vector3d centerScreen(
      (P * V * center.homogeneous().eval()).hnormalized());
  centerScreen.head<2>() += prevPos - pos;
  center = (V.inverse() * P.inverse() * centerScreen.homogeneous().eval())
               .hnormalized();
