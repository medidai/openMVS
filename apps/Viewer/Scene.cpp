...
public:
  ...
  EVTLoadImage(Scene *_pScene, MVS::IIndex _idx, unsigned _nMaxResolution = 0)
      : Event(EVT_JOB), pScene(_pScene), idx(_idx),
        nMaxResolution(_nMaxResolution) {}
};
...
public:
  ...
  bool Run(void *) {
    ...
    if (!scene.mesh.IsEmpty()) {
      Scene::OctreeMesh octMesh(
          scene.mesh.vertices,
          [](Scene::OctreeMesh::IDX_TYPE size,
             Scene::OctreeMesh::Type /*radius*/) { return size > 256; });
      scene.mesh.ListIncidentFaces();
      pScene->octMesh.Swap(octMesh);
    } else if (!scene.pointcloud.IsEmpty()) {
      Scene::OctreePoints octPoints(
          scene.pointcloud.points,
          [](Scene::OctreePoints::IDX_TYPE size,
             Scene::OctreePoints::Type /*radius*/) { return size > 512; });
      pScene->octPoints.Swap(octPoints);
    }
    return true;
  }
  EVTComputeOctree(Scene *_pScene) : Event(EVT_JOB), pScene(_pScene) {}
};
...
Scene::Scene(ARCHIVE_TYPE _nArchiveType)
    : nArchiveType(_nArchiveType), listPointCloud(0) {}
Scene::~Scene() { Release(); }
...
bool Scene::Init(const cv::Size &size, LPCTSTR windowName, LPCTSTR fileName,
  ...
  name = windowName;
  window.clbkOpenScene =
      DELEGATEBINDCLASS(Window::ClbkOpenScene, &Scene::Open, this);
...
bool Scene::Open(LPCTSTR fileName, LPCTSTR geometryFileName) {
  ...
  // init camera
  window.SetCamera(Camera(
      bounds, center == Point3d::INF ? Point3d(bounds.GetCenter()) : center,
      images.size() < 2
          ? 1.f
          : (float)imageBounds.EnlargePercent(REAL(1) / images.size())
                .GetSize()
                .norm()));
  window.camera.maxCamID = images.size();
  window.SetName(String::FormatString((name + _T(": %s")).c_str(),
                                      Util::getFileName(fileName).c_str()));
  window.clbkSaveScene =
      DELEGATEBINDCLASS(Window::ClbkSaveScene, &Scene::Save, this);
  window.clbkExportScene =
      DELEGATEBINDCLASS(Window::ClbkExportScene, &Scene::Export, this);
  window.clbkCenterScene =
      DELEGATEBINDCLASS(Window::ClbkCenterScene, &Scene::Center, this);
  window.clbkCompilePointCloud = DELEGATEBINDCLASS(
      Window::ClbkCompilePointCloud, &Scene::CompilePointCloud, this);
  window.clbkCompileMesh =
      DELEGATEBINDCLASS(Window::ClbkCompileMesh, &Scene::CompileMesh, this);
  window.clbkTogleSceneBox =
      DELEGATEBINDCLASS(Window::ClbkTogleSceneBox, &Scene::TogleSceneBox, this);
  window.clbkCropToBounds =
      DELEGATEBINDCLASS(Window::ClbkCropToBounds, &Scene::CropToBounds, this);
  if (scene.IsBounded())
    window.clbkCompileBounds = DELEGATEBINDCLASS(Window::ClbkCompileBounds,
                                                 &Scene::CompileBounds, this);
  if (!scene.IsEmpty())
    window.clbkRayScene =
        DELEGATEBINDCLASS(Window::ClbkRayScene, &Scene::CastRay, this);
  window.Reset(!scene.pointcloud.IsEmpty() && !scene.mesh.IsEmpty()
                   ? Window::SPR_NONE
                   : Window::SPR_ALL,
               MINF(2u, images.size()));
  return true;
...
bool Scene::Save(LPCTSTR _fileName, bool bRescaleImages) {
  ...
  const String fileName(_fileName != NULL
                            ? String(_fileName)
                            : Util::insertBeforeFileExt(sceneName, _T("_new")));
  MVS::Mesh mesh;
  ...
  if (imageScale > 0 && imageScale < 1) {
    ...
    const String folderName(
        Util::getFilePath(MAKE_PATH_FULL(WORKING_FOLDER_FULL, fileName)) +
        String::FormatString("images%d" PATH_SEPARATOR_STR,
                             ROUND2INT(imageScale * 100)));
...
bool Scene::Export(LPCTSTR _fileName, LPCTSTR exportType) const {
  ...
  const bool bPoints(scene.pointcloud.Save(
      lastFileName = (baseFileName + _T("_pointcloud.ply")),
      nArchiveType == ARCHIVE_MVS));
  const bool bMesh(scene.mesh.Save(
      lastFileName = (baseFileName + _T("_mesh") +
                      (exportType ? exportType
                                  : (Util::getFileExt(fileName) == _T(".obj")
                                         ? _T(".obj")
                                         : _T(".ply")))),
      cList<String>(), true));
#if TD_VERBOSE != TD_VERBOSE_OFF
  if (VERBOSITY_LEVEL > 2 && (bPoints || bMesh))
    scene.ExportCamerasMLP(Util::getFileFullName(lastFileName) + _T(".mlp"),
                           lastFileName);
#endif
  AABB3f aabb(true);
  if (scene.IsBounded()) {
...
void Scene::CompilePointCloud() {
  ...
  if ((window.sparseType & Window::SPR_POINTS) != 0) {
    ...
    if (window.colorSource == Window::COLORSOURCE::COL_DEPTH ||
        window.colorSource == Window::COLORSOURCE::COL_COMPOSITE ||
        window.colorSource == Window::COLORSOURCE::COL_NORMAL) {
      if (!depthData.Load(
              sceneName,
              window.colorSource == Window::COLORSOURCE::COL_NORMAL ? 3 : 1)) {
    ...
    FOREACH(i, scene.pointcloud.points) {
      ...
      if (window.colorSource == Window::COLORSOURCE::COL_DEPTH ||
        ...
        const float confidence =
            window.colorSource == Window::COLORSOURCE::COL_COMPOSITE
                ? 0.3f * confMap(j, k) +
                      0.7f * scene.pointcloud.pointWeights[i][0]
                : confMap(j, k);
      ...
      if (window.colorSource == Window::COLORSOURCE::COL_CONFIDENCE &&
          !scene.pointcloud.pointWeights.empty()) {
...
void Scene::CompileMesh() {
  ...
  do {
    ...
    FOREACH(idxFace, scene.mesh.faces) {
      if (!scene.mesh.faceTexindices.empty() &&
          scene.mesh.faceTexindices[idxFace] != texIdx)
        continue;
...
void Scene::CompileBounds() {
  ...
  if (window.bRenderBounds) {
    static const uint8_t indices[12 * 2] = {0, 2, 2, 3, 3, 1, 1, 0, 0, 6, 2, 4,
                                            3, 5, 1, 7, 6, 4, 4, 5, 5, 7, 7, 6};
...
void Scene::Draw() {
  ...
  if (window.bRenderCameras) {
    ...
    FOREACH(idx, images) {
      ...
      glPushMatrix();
      glMultMatrixd((GLdouble *)TransL2W((const Matrix3x3::EMat)camera.R,
                                         -(const Point3::EVec)camera.C)
                        .data());
      ...
      glDisable(GL_TEXTURE_2D);
      // draw camera frame
      const bool bSelectedCamera(window.selectionType == Window::SEL_CAMERA &&
                                 window.selectionIdx == idx);
      ...
      glBegin(GL_POINTS);
      glColor3f(1, 0, 0);
      glVertex3f(0, 0, 0); // camera position
      glColor3f(0, 1, 0);
      glVertex3f(0, 0, (float)scaleFocal); // image center
      glColor3f(0, 0, 1);
      glVertex3d((0.5 * imageData.width - pp.x) / focal, cy,
                 scaleFocal); // image up
      glEnd();
      // restore coordinate system
      glPopMatrix();
      // render image visibility info
      if (window.bRenderImageVisibility && idx != NO_ID &&
          idx == window.camera.currentCamID) {
        if (scene.pointcloud.IsValid()) {
          ...
          FOREACH(i, scene.pointcloud.points) {
            ...
            if (scene.pointcloud.pointViews[i].size() < window.minViews)
              continue;
            if (scene.pointcloud.pointViews[i].FindFirst(image.idx) ==
                MVS::PointCloud::ViewArr::NO_INDEX)
              continue;
  ...
  if (window.selectionType != Window::SEL_NA) {
    ...
    if (window.selectionType == Window::SEL_TRIANGLE) {
      glColor3f(0, 1, 0);
      glVertex3fv(window.selectionPoints[1].ptr());
      glColor3f(0, 0, 1);
      glVertex3fv(window.selectionPoints[2].ptr());
    }
    glEnd();
    if (window.bRenderViews && window.selectionType == Window::SEL_POINT) {
      if (!scene.pointcloud.pointViews.empty()) {
        glLineWidth(1.f);
        glBegin(GL_LINES);
        const MVS::PointCloud::ViewArr &views =
            scene.pointcloud
                .pointViews[(MVS::PointCloud::Index)window.selectionIdx];
        ASSERT(!views.empty());
  ...
  // draw coordinate axes
  {
    ...
    glViewport(window.size.width - axisWindowSize, 0, axisWindowSize,
               axisWindowSize);
    glLoadIdentity();
  ...
  case GLFW_RELEASE: {
    ...
    if (!octMesh.IsEmpty()) {
      ...
      if (intRay.pick.IsValid()) {
        ...
        const MVS::Mesh::Face &face =
            scene.mesh.faces[(MVS::Mesh::FIndex)newSelectionIdx];
    ...
    if (!octPoints.IsEmpty()) {
      ...
      const MVS::IntersectRayPoints intRay(octPoints, ray, scene.pointcloud,
                                           window.minViews);
      if (intRay.pick.IsValid() && intRay.pick.dist < minDist) {
        window.selectionType = Window::SEL_POINT;
        minDist = intRay.pick.dist;
        newSelectionIdx = intRay.pick.idx;
        newSelectionPoints[0] = newSelectionPoints[3] =
            scene.pointcloud.points[newSelectionIdx];
      }
    }
    ...
    FOREACH(idx, images) {
      ...
      ASSERT(imageData.IsValid());
      REAL dist;
      if (coneIntersect.Classify(imageData.camera.C, dist) == VISIBLE &&
          dist < minDist) {
    ...
    if (window.selectionType != Window::SEL_NA) {
      ...
      case Window::SEL_TRIANGLE: {
        DEBUG("Face selected:\n\tindex: %u\n\tvertex 1: %u (%g, %g, "
              "%g)\n\tvertex 2: %u (%g, %g, %g)\n\tvertex 3: %u (%g, %g, %g)",
              newSelectionIdx, scene.mesh.faces[newSelectionIdx][0],
              newSelectionPoints[0].x, newSelectionPoints[0].y,
              newSelectionPoints[0].z, scene.mesh.faces[newSelectionIdx][1],
              newSelectionPoints[1].x, newSelectionPoints[1].y,
              newSelectionPoints[1].z, scene.mesh.faces[newSelectionIdx][2],
              newSelectionPoints[2].x, newSelectionPoints[2].y,
              newSelectionPoints[2].z);
        break;
      }
      case Window::SEL_POINT: {
        DEBUG("Point selected:\n\tindex: %u (%g, %g, %g)%s", newSelectionIdx,
              newSelectionPoints[0].x, newSelectionPoints[0].y,
              newSelectionPoints[0].z,
              [&]() {
                if (scene.pointcloud.pointViews.empty())
                  return String();
                const MVS::PointCloud::ViewArr &views =
                    scene.pointcloud.pointViews[newSelectionIdx];
                ASSERT(!views.empty());
                String strViews(
                    String::FormatString("\n\tviews: %u", views.size()));
                FOREACH(v, views) {
                  ...
                  const MVS::Image &imageData = scene.images[idxImage];
                  const Point2 x(imageData.camera.TransformPointW2I(
                      Cast<REAL>(window.selectionPoints[0])));
                  const float conf =
                      scene.pointcloud.pointWeights.empty()
                          ? 0.f
                          : scene.pointcloud.pointWeights[newSelectionIdx][v];
                  strViews += String::FormatString(
                      "\n\t\t%s (%.2f %.2f pixel, %.2f conf)",
                      Util::getFileNameExt(imageData.name).c_str(), x.x, x.y,
                      conf);
                }
                return strViews;
              }()
      ...
      case Window::SEL_CAMERA: {
        ...
        const MVS::Camera &camera = imageData.camera;
        Point3 eulerAngles;
        camera.R.GetRotationAnglesZYX(eulerAngles.x, eulerAngles.y,
                                      eulerAngles.z);
        DEBUG("Camera selected:\n\tindex: %u (ID: %u)\n\tname: %s (mask "
              ...
              "\n\taverage depth: %.2g\n\tneighbors: %u",
              image.idx, imageData.ID,
              Util::getFileNameExt(imageData.name).c_str(),
              imageData.maskName.empty()
                  ? "none"
                  : Util::getFileNameExt(imageData.maskName).c_str(),
              imageData.width, imageData.height, camera.K(0, 0), camera.K(1, 1),
              camera.K(0, 2), camera.K(1, 2), camera.C.x, camera.C.y,
              camera.C.z, R2D(eulerAngles.x), R2D(eulerAngles.y),
              R2D(eulerAngles.z), imageData.avgDepth,
