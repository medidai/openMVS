...
public:
  Camera(const AABB3d &_box = AABB3d(true),
         const Point3d &_center = Point3d::ZERO, float _scaleF = 1,
         float _fov = 40);
  ...
  bool IsCameraViewMode() const {
    return prevCamID != currentCamID && currentCamID != NO_ID;
  }
