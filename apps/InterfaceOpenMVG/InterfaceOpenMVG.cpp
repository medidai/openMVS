...
bool ImportScene(const std::string &sList_filename,
                 const std::string &sBaf_filename, SfM_Scene &sceneBAF) {
  LOG_OUT() << "Reading:\n"
            << sList_filename << "\n"
            << sBaf_filename << std::endl;

  // Read view list file (view filename, id_intrinsic, id_pose)
  // Must be read first, since it allow to establish the link between the ViewId
  // and the camera/poses ids.
  std::map<std::pair<uint32_t, uint32_t>, uint32_t> map_cam_pose_toViewId;
  {
    ...
    while (file >> image.name >> image.id_camera >> image.id_pose) {
      ...
      map_cam_pose_toViewId[std::make_pair(image.id_camera, image.id_pose)] =
          count++;
      LOG_OUT() << image.name << ' ' << image.id_camera << ' ' << image.id_pose
                << std::endl;
    }
  }
  ...
  // Read BAF file
  {
    ...
    // Read the intrinsics (only support reading Pinhole Radial 3).
    {
      for (uint32_t i = 0; i < num_intrinsics; ++i) {
        ...
        Camera cam;
        cam.K << focal, 0, ppx, 0, focal, ppy, 0, 0, 1;
        LOG_OUT() << "\n" << cam.K << std::endl;
...
#ifndef _RELEASE
          LOG_OUT() << "observation:"
                    << " " << id_intrinsics << " " << id_pose << " " << x << " "
                    << y << std::endl;
#endif
          const auto itIntrPose(map_cam_pose_toViewId.find(
              std::make_pair(id_intrinsics, id_pose)));
          if (itIntrPose == map_cam_pose_toViewId.end()) {
            LOG_OUT() << "error: intrinsics-pose pair not existing"
                      << std::endl;
            continue;
          }
...
bool ExportScene(const std::string &sList_filename,
                 const std::string &sBaf_filename, const SfM_Scene &sceneBAF) {
  LOG_OUT() << "Writing:\n"
            << sList_filename << "\n"
            << sBaf_filename << std::endl;
  ...
  // Write view list file (view filename, id_intrinsic, id_pose)
  {
    ...
    for (uint32_t i = 0; i < sceneBAF.images.size(); ++i) {
      ...
      file << image.name << ' ' << image.id_camera << ' ' << image.id_pose
           << std::endl;
      LOG_OUT() << image.name << ' ' << image.id_camera << ' ' << image.id_pose
                << std::endl;
    }
  ...
  // Write BAF file
  {
    ...
    // Write the intrinsics (only support writing Pinhole Radial 3).
    {
      for (uint32_t i = 0; i < num_intrinsics; ++i) {
        ...
        file << cam.K(0, 0) << ' ' << cam.K(0, 2) << ' ' << cam.K(1, 2) << ' '
             << 0 << ' ' << 0 << ' ' << 0 << std::endl;
        LOG_OUT() << "\n" << cam.K << std::endl;
      }
    }
    ...
    // Write structure and visibility
    {
      ...
      for (uint32_t i = 0; i < num_points; ++i) {
        ...
        file << vertex.X[0] << ' ' << vertex.X[1] << ' ' << vertex.X[2]
             << std::endl;
        const uint32_t num_observations_for_point =
            (uint32_t)vertex.views.size();
        file << num_observations_for_point << std::endl;
        for (uint32_t j = 0; j < num_observations_for_point; ++j) {
          const uint32_t id_view = vertex.views[j];
          const Image &image = sceneBAF.images[id_view];
          file << image.id_camera << ' ' << image.id_pose << ' ' << 0 << ' '
               << 0 << std::endl;
#ifndef _RELEASE
          LOG_OUT() << "observation:"
                    << " " << image.id_camera << " " << image.id_pose
                    << std::endl;
#endif
        }
#ifdef _RELEASE
...
bool Application::Initialize(size_t argc, LPCTSTR *argv) {
  ...
  config.add_options()(
      "images-list-file,l",
      boost::program_options::value<std::string>(&OPT::strListFileName),
      "input filename containing image list")(
      "input-file,i",
      ...
      boost::program_options::value(&OPT::bNormalizeIntrinsics)
          ->default_value(true),
      "normalize intrinsics while exporting to OpenMVS format");
...
#ifdef _USE_OPENMVG
  OPT::bOpenMVGjson =
      (strInputFileNameExt == MVG2_EXT || strInputFileNameExt == MVG3_EXT);
  const bool bInvalidCommand(OPT::strInputFileName.IsEmpty() ||
                             (OPT::strListFileName.IsEmpty() &&
                              !OPT::bOpenMVGjson && !OPT::bOpenMVS2OpenMVG));
#else
  const bool bInvalidCommand(
      OPT::strInputFileName.IsEmpty() ||
      (OPT::strListFileName.IsEmpty() && !OPT::bOpenMVS2OpenMVG));
#endif
...
#ifndef _USE_OPENMVG
    GET_LOG()
        << "\nWARNING: Only " MVG_EXT
           " files supported! In order to import " MVG2_EXT " or " MVG3_EXT
           " files use the converter included in OpenMVG, or link OpenMVS to "
           "the latest version of OpenMVG during compile time.\n";
#endif
  }
  if (bInvalidCommand)
    return false;
  ...
  if (OPT::bOpenMVS2OpenMVG) {
    ...
    if (OPT::strOutputFileName.IsEmpty())
      OPT::strOutputFileName =
          Util::getFileFullName(OPT::strInputFileName) + MVS_EXT;
  }
...
int main(int argc, LPCTSTR *argv) {
  ...
  if (OPT::bOpenMVS2OpenMVG) {
    ...
    openMVS::MVS_IO::ExportScene(MAKE_PATH_SAFE(OPT::strListFileName),
                                 MAKE_PATH_SAFE(strOutputFileNameMVG),
                                 sceneBAF);

    VERBOSE("Input data exported: %u cameras & %u poses & %u images & %u "
            "vertices (%s)",
            sceneBAF.cameras.size(), sceneBAF.poses.size(),
            sceneBAF.images.size(), sceneBAF.vertices.size(),
    ...
    if (OPT::bOpenMVGjson) {
      ...
      if (!Load(sfm_data, strSfM_Data_Filename, ESfM_Data(ALL))) {
        VERBOSE("error: the input SfM_Data file '%s' cannot be read",
                strSfM_Data_Filename.c_str());
        return EXIT_FAILURE;
      }
      VERBOSE("Imported data: %u cameras, %u poses, %u images, %u vertices",
              sfm_data.GetIntrinsics().size(), sfm_data.GetPoses().size(),
              sfm_data.GetViews().size(), sfm_data.GetLandmarks().size());

      // OpenMVG can have not contiguous index, use a map to create the required
      // OpenMVS contiguous ID index
      ...
      for (const auto &intrinsic : sfm_data.GetIntrinsics()) {
        if (isPinhole(intrinsic.second.get()->getType())) {
          const Pinhole_Intrinsic *cam =
              dynamic_cast<const Pinhole_Intrinsic *>(intrinsic.second.get());
          if (map_intrinsic.count(intrinsic.first) == 0)
            map_intrinsic.insert(
                std::make_pair(intrinsic.first, scene.platforms.GetSize()));
...
#ifdef _USE_OPENMP
      const std::vector<Views::value_type> views(sfm_data.GetViews().cbegin(),
                                                 sfm_data.GetViews().cend());
...
#pragma omp critical
      ...
      for (const auto &view : sfm_data.GetViews()) {
        ...
        Util::ensureFolderSlash(pathRoot);
        const String srcImage(
            MAKE_PATH_FULL(WORKING_FOLDER_FULL, pathRoot + image.name));
        image.name = MAKE_PATH_FULL(WORKING_FOLDER_FULL,
                                    OPT::strOutputImageFolder + image.name);
...
#pragma omp critical
          ...
          const openMVG::geometry::Pose3 poseMVG(
          ...
          // just copy the image
          File::copyFile(srcImage, image.name);
          DEBUG_EXTRA("warning: uncalibrated image '%s'",
                      view.second->s_Img_path.c_str());
        }
        ++progress;
      }
      GET_LOGCONSOLE().Play();
      ...
      for (const auto &vertex : sfm_data.GetLandmarks()) {
        ...
        MVS::PointCloud::ViewArr &views =
            scene.pointcloud.pointViews.AddEmpty();
...
#endif
    {
      ...
      openMVS::MVS_IO::SfM_Scene sceneBAF;
      if (!openMVS::MVS_IO::ImportScene(MAKE_PATH_SAFE(OPT::strListFileName),
    ...
    if (OPT::bNormalizeIntrinsics) {
      ...
      FOREACH(p, scene.platforms) {
        ...
        FOREACH(c, platform.cameras) {
          ...
          const REAL fScale(REAL(1) / MVS::Camera::GetNormalizationScale(
    ...
    VERBOSE("Exported data: %u platforms, %u cameras, %u poses, %u images, %u "
            "vertices (%s)",
            scene.platforms.GetSize(), nCameras, nPoses, scene.images.GetSize(),
