...
bool Application::Initialize(size_t argc, LPCTSTR *argv) {
  ...
  generic.add_options()(
      "help,h",
      "imports SfM scene stored either in Metashape Agisoft/BlocksExchange or "
      "ContextCapture BlocksExchange XML format")(
      "working-folder,w",
  ...
  config.add_options()(
      ...
      "points-file,p",
      boost::program_options::value<std::string>(&OPT::strPointsFileName),
      "input filename containing the 3D points")(
      "output-file,o",
      boost::program_options::value<std::string>(&OPT::strOutputFileName),
      "output filename for storing the scene")(
      "output-image-folder",
      boost::program_options::value<std::string>(&OPT::strOutputImageFolder)
          ->default_value("undistorted_images"),
      "output folder to store undistorted images");
  ...
  OPEN_LOGFILE(
      MAKE_PATH(APPNAME _T("-") + Util::getUniqueName(0) + _T(".log")).c_str());
...
bool ParseImageListXML(tinyxml2::XMLDocument &doc, Scene &scene,
                       PlatformDistCoeffs &pltDistCoeffs, size_t &nCameras,
                       size_t &nPoses) {
  String strInputFileName(
      MAKE_PATH_FULL(WORKING_FOLDER_FULL, OPT::strInputFileName));
    ...
    goto InvalidDocument;
  {
    tinyxml2::XMLElement *document =
        doc.FirstChildElement(_T("document"))->FirstChildElement(_T("chunk"));
    if (document == NULL)
      goto InvalidDocument;
    {
      ...
      {
        tinyxml2::XMLElement *sensors =
            document->FirstChildElement(_T("sensors"));
        if (sensors == NULL)
          goto InvalidDocument;
        {
          for (tinyxml2::XMLElement *sensor = sensors->FirstChildElement();
               sensor != NULL; sensor = sensor->NextSiblingElement()) {
            unsigned ID;
            if (0 != _tcsicmp(sensor->Value(), _T("sensor")) ||
                sensor->QueryUnsignedAttribute(_T("id"), &ID) !=
              ...
              goto InvalidDocument;
            {
              ...
              if ((name = sensor->Attribute(_T("label"))) != NULL)
                platform.name = name;
              // parse intrinsics
              tinyxml2::XMLElement *calibration =
                  sensor->FirstChildElement(_T("calibration"));
              if (calibration == NULL)
                goto InvalidDocument;
              {
                if ((elem = calibration->FirstChildElement(_T("resolution"))) !=
                    NULL) {
                ...
                for (elem = calibration->FirstChildElement(); elem != NULL;
                     elem = elem->NextSiblingElement()) {
                  if (0 == _tcsicmp(elem->Value(), _T("f"))) {
                    camera.K(0, 0) = camera.K(1, 1) =
                        String::FromString<REAL>(elem->GetText());
                  } else if (0 == _tcsicmp(elem->Value(), _T("fx"))) {
                    elem->QueryDoubleText(&camera.K(0, 0));
                  } else if (0 == _tcsicmp(elem->Value(), _T("fy"))) {
                    elem->QueryDoubleText(&camera.K(1, 1));
                  } else if (0 == _tcsicmp(elem->Value(), _T("cx"))) {
                    elem->QueryDoubleText(&camera.K(0, 2));
                  } else if (0 == _tcsicmp(elem->Value(), _T("cy"))) {
                    elem->QueryDoubleText(&camera.K(1, 2));
                  } else if (0 == _tcsicmp(elem->Value(), _T("k1"))) {
                    elem->QueryDoubleText(&dc.k1);
                  } else if (0 == _tcsicmp(elem->Value(), _T("k2"))) {
                    elem->QueryDoubleText(&dc.k2);
                  } else if (0 == _tcsicmp(elem->Value(), _T("k3"))) {
                    elem->QueryDoubleText(&dc.k3);
                  } else if (0 == _tcsicmp(elem->Value(), _T("p1"))) {
                    elem->QueryDoubleText(&dc.p1);
                  } else if (0 == _tcsicmp(elem->Value(), _T("p2"))) {
                    elem->QueryDoubleText(&dc.p2);
                  } else if (0 == _tcsicmp(elem->Value(), _T("k4"))) {
                    elem->QueryDoubleText(&dc.k4);
                  } else if (0 == _tcsicmp(elem->Value(), _T("k5"))) {
                    elem->QueryDoubleText(&dc.k5);
                  } else if (0 == _tcsicmp(elem->Value(), _T("k6"))) {
                ...
                if (bMetashapeFile) {
                  ...
                  camera.K = camera.GetScaledK(
                      REAL(1) / Camera::GetNormalizationScale(
      ...
      // parse poses
      {
        tinyxml2::XMLElement *cameras =
            document->FirstChildElement(_T("cameras"));
        if (cameras == NULL)
          goto InvalidDocument;
        {
          PMatrix P;
          ...
          for (tinyxml2::XMLElement *camera = cameras->FirstChildElement();
               camera != NULL; camera = camera->NextSiblingElement()) {
            unsigned ID;
            if (0 != _tcsicmp(camera->Value(), _T("camera")) ||
                camera->QueryUnsignedAttribute(_T("id"), &ID) !=
                    tinyxml2::XML_SUCCESS)
              goto InvalidDocument;
            {
              // add new image
              ...
              LPCTSTR name;
              if ((name = camera->Attribute(_T("type"))) != NULL &&
                  _tcsicmp(name, _T("frame")) != 0) {
                DEBUG_EXTRA("warning: unsupported camera calibration '%s'",
                            name);
                continue;
              }
              ...
              imageData.platformID =
                  mapPlatformID.at(camera->UnsignedAttribute(_T("sensor_id")));
              imageData.cameraID =
                  0; // only one camera per platform supported by this format
              ...
              if ((elem = camera->FirstChildElement(_T("transform"))) == NULL ||
                  (argv = Util::CommandLineToArgvA(elem->GetText(), argc)) ==
                      NULL ||
              ...
              imageData.camera =
                  platform.GetCamera(imageData.cameraID, imageData.poseID);
              ++nPoses;
            }
          }
        ...
        // parse bounding-box
        {
          tinyxml2::XMLElement *region =
              document->FirstChildElement(_T("region"));
          if (region == NULL)
            goto InvalidDocument;
          {
            size_t argc;
...
// parse scene stored in ContextCapture BlocksExchange format containing
// cameras, images and sparse point-cloud
bool ParseBlocksExchangeXML(tinyxml2::XMLDocument &doc, Scene &scene,
                            PlatformDistCoeffs &pltDistCoeffs, size_t &nCameras,
                            size_t &nPoses) {
  String strInputFileName(
      MAKE_PATH_FULL(WORKING_FOLDER_FULL, OPT::strInputFileName));
  ...
  for (const tinyxml2::XMLElement *photogroup =
           photogroups->FirstChildElement();
       photogroup != NULL; photogroup = photogroup->NextSiblingElement()) {
    ...
    resolutions.emplace_back(elem->FirstChildElement("Width")->UnsignedText(),
    ...
    camera.R = RMatrix::IDENTITY;
    camera.C = CMatrix::ZERO;
    const float resolutionScale = Camera::GetNormalizationScale(
        resolutions.back().width, resolutions.back().height);
    if ((elem = photogroup->FirstChildElement("FocalLengthPixels")) != NULL) {
      camera.K(0, 0) = camera.K(1, 1) =
          photogroup->FirstChildElement("FocalLengthPixels")->DoubleText();
    } else {
      camera.K(0, 0) = camera.K(1, 1) =
          photogroup->FirstChildElement("FocalLength")->DoubleText() *
          resolutionScale /
          photogroup->FirstChildElement("SensorSize")->DoubleText();
    }
    ...
    DistCoeff &dc = pltDistCoeffs.AddEmpty().AddEmpty();
    {
      const tinyxml2::XMLElement *distortion =
          photogroup->FirstChildElement("Distortion");
      if (distortion) {
    ...
    ++nCameras;
    for (const tinyxml2::XMLElement *photo =
             photogroup->FirstChildElement("Photo");
         photo != NULL; photo = photo->NextSiblingElement()) {
      ...
      imageData.poseID = platform.poses.size();
      Platform::Pose &pose = platform.poses.AddEmpty();
      pose.R = Matrix3x3(elem->FirstChildElement("M_00")->DoubleText(),
      ...
      if ((elem = photoPose->FirstChildElement("Center")) == NULL)
        continue;
      pose.C = Point3(elem->FirstChildElement("x")->DoubleText(),
      ...
      else if (photo->FirstChildElement("NearDepth") != NULL &&
               photo->FirstChildElement("FarDepth") != NULL)
        imageData.avgDepth =
            (float)((photo->FirstChildElement("NearDepth")->DoubleText() +
                     photo->FirstChildElement("FarDepth")->DoubleText()) /
                    2);
      else
  ...
  const bool bLocalCoords(
      document->FirstChildElement("SRSId") == NULL ||
      ((elem = blocksExchange->FirstChildElement("SpatialReferenceSystems")) !=
           NULL &&
       (elem = elem->FirstChildElement("SRS")) != NULL &&
       (elem = elem->FirstChildElement("Name")) != NULL &&
       _tcsncmp(elem->GetText(), "Local Coordinates", 17) == 0));
  ...
  const tinyxml2::XMLElement *tiepoints =
      document->FirstChildElement("TiePoints");
  if (tiepoints == NULL)
    return true;
  for (const tinyxml2::XMLElement *tiepoint = tiepoints->FirstChildElement();
       tiepoint != NULL; tiepoint = tiepoint->NextSiblingElement()) {
    ...
    if ((elem = tiepoint->FirstChildElement("Color")) != NULL)
      scene.pointcloud.colors.emplace_back(
          (uint8_t)CLAMP(elem->FirstChildElement("Red")->DoubleText() * 255,
                         0.0, 255.0),
          (uint8_t)CLAMP(elem->FirstChildElement("Green")->DoubleText() * 255,
                         0.0, 255.0),
          (uint8_t)CLAMP(elem->FirstChildElement("Blue")->DoubleText() * 255,
                         0.0, 255.0));
    PointCloud::ViewArr views;
    for (const tinyxml2::XMLElement *view =
             tiepoint->FirstChildElement("Measurement");
         view != NULL; view = view->NextSiblingElement())
      views.emplace_back(
          mapImageID.at(view->FirstChildElement("PhotoId")->UnsignedText()));
    scene.pointcloud.pointViews.emplace_back(std::move(views));
  }
  return true;
}

// parse scene stored either in Metashape images list format or ContextCapture
// BlocksExchange format
bool ParseSceneXML(Scene &scene, PlatformDistCoeffs &pltDistCoeffs,
                   size_t &nCameras, size_t &nPoses) {
  // parse XML file
...
bool UndistortBrown(Image &imageData, uint32_t ID, const DistCoeff &dc,
  ...
  const cv::Vec<double, 8> &distCoeffs =
      *reinterpret_cast<const cv::Vec<REAL, 8> *>(dc.coeff);
  const KMatrix prevK(
      imageData.camera.GetK<REAL>(imageData.width, imageData.height));
#if 1
  const KMatrix &K(prevK);
#else
  const KMatrix K(cv::getOptimalNewCameraMatrix(
      prevK, distCoeffs, imageData.size(), 0.0, cv::Size(), NULL, true));
  ASSERT(K(0, 2) == Camera::ComposeK(prevK(0, 0), prevK(1, 1),
                                     imageData.width(),
                                     imageData.height())(0, 2));
  ASSERT(K(1, 2) == Camera::ComposeK(prevK(0, 0), prevK(1, 1),
                                     imageData.width(),
                                     imageData.height())(1, 2));
  if (K.IsEqual(prevK)) {
    int i(0);
...
// project all points in this image and keep those looking at the camera and are
// most in front
...
int main(int argc, LPCTSTR *argv) {
  ...
  if (!OPT::strPointsFileName.empty() &&
      !scene.pointcloud.Load(MAKE_PATH_SAFE(OPT::strPointsFileName)))
    return EXIT_FAILURE;
  const bool bAssignPoints(!scene.pointcloud.IsEmpty() &&
  ...
  // undistort images
  const String pathData(
      MAKE_PATH_FULL(WORKING_FOLDER_FULL, OPT::strOutputImageFolder));
...
#pragma omp flush(bAbort)
  ...
  FOREACH(ID, scene.images) {
    ...
    if (!imageData.IsValid())
      continue;
    if (!UndistortBrown(imageData, ID,
                        pltDistCoeffs[imageData.platformID][imageData.cameraID],
                        pathData)) {
#ifdef _USE_OPENMP
      bAbort = true;
#pragma omp flush(bAbort)
  ...
  if (scene.pointcloud.IsValid()) {
    ...
    if (!std::any_of(
            scene.images.begin(), scene.images.end(),
            [](const Image &imageData) { return imageData.avgDepth > 0; })) {
      ...
      FOREACH(idxPoint, scene.pointcloud.points) {
        ...
        for (const PointCloud::View &idxImage :
  ...
  VERBOSE("Exported data: %u platforms, %u cameras, %u poses, %u images, %u "
          "vertices, %g min / %g mean (%g std) / %g max average scene depth "
          "per image (%s)",
          scene.platforms.size(), nCameras, nPoses, scene.images.size(),
          scene.pointcloud.GetSize(), acc.minVal, acc.GetMean(),
          acc.GetStdDev(), acc.maxVal, TD_TIMER_GET_FMT().c_str());
  return EXIT_SUCCESS;
}
/*----------------------------------------------------------------*/
