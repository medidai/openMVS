...
class SummaryOnlyValidationTests(unittest.TestCase):
    ...
    def test_dvp_summary_accepts_explicitly_unavailable_initialization_census(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            frame = Path(directory) / "frame"
            summary, _dmap = make_summary_fixture(frame)
            summary["candidate_accounting_mode"] = (
            ...
            rewrite_census_row(frame, -1, **zero_initialization)

            valid, detail = validator.validate_frame_census(
                frame, summary, [-1, 0]
            )

            self.assertTrue(valid, detail)
            self.assertEqual(detail["intentionally_unavailable_iterations"], [-1])

            rewrite_census_row(
                frame, -1, **zero_initialization, candidate_tested_init=1
            )
            valid, detail = validator.validate_frame_census(
                frame, summary, [-1, 0]
            )
            ...
            self.assertTrue(any(
                "initialization candidate counters must remain zero" in error
