#!/usr/bin/env python3
"""Validate one schema-v2 through schema-v5 depth-map instrumentation frame."""
...
DVP_SCHEMA_VERSION = 1
DVP_OBSERVABILITY_SCHEMA_NAME = "openmvs.dmap.dvp_observability"
DVP_MAX_PROPOSALS = 2
DVP_FAMILY_CODES = {
...
DVP_FINAL_SOURCE_CODES = {
    "none": 0,
    "init": 1,
    "propagate": 2,
    "refine_depth": 3,
    "refine_normal": 4,
    "refine_random_normal": 5,
    "refine_surface_normal": 6,
    "filtered": 7,
    "changed_unknown": 8,
    "apd_anchor_propagate": 9,
    "apd_fitted_plane": 10,
    "apd_final_refinement": 11,
    "dvp_epipolar": 12,
}
DVP_REQUIRED_FLOAT_SIGNALS = {
    "dvp_incumbent_depth",
    "dvp_final_depth",
    "dvp_left_interval_minimum",
    "dvp_left_interval_maximum",
    "dvp_right_interval_minimum",
    "dvp_right_interval_maximum",
    "dvp_incumbent_cost",
    "dvp_winner_cost",
    "dvp_runner_up_cost",
    "dvp_winner_runner_up_gap",
    "dvp_depth_displacement",
    *{
        f"dvp_{field}_{proposal}"
        for proposal in range(DVP_MAX_PROPOSALS)
        for field in (
            "proposal_depth",
            "candidate_cost",
...
DVP_REQUIRED_RGBA_SIGNALS = {
    "dvp_selected_source_views",
    "dvp_direction_source_views",
    *{
        f"dvp_{field}_{proposal}"
        for proposal in range(DVP_MAX_PROPOSALS)
        for field in ("support_views", "occluded_views")
    },
}
DVP_REQUIRED_BYTE_SIGNALS = {
    "dvp_left_outer_count",
    "dvp_left_inner_count",
    "dvp_right_inner_count",
    "dvp_right_outer_count",
    "dvp_family",
    "dvp_unavailable_reason",
    "dvp_generated_count",
    "dvp_tested_count",
    "dvp_finite_count",
    "dvp_accepted_count",
    "dvp_tested_proposal_mask",
    "dvp_finite_proposal_mask",
    "dvp_accepted_proposal_mask",
    "dvp_winner_ordinal",
    "dvp_accepted",
    "dvp_final_winner",
    "dvp_final_update_source",
    "dvp_final_depth_retained",
    "dvp_native_depth_fallback",
    "dvp_left_interval_valid",
    "dvp_right_interval_valid",
    *{
        f"dvp_{field}_{proposal}"
        for proposal in range(DVP_MAX_PROPOSALS)
        for field in ("source_view", "support")
    },
}
DVP_REQUIRED_SIGNALS = (
    DVP_REQUIRED_FLOAT_SIGNALS
    | DVP_REQUIRED_RGBA_SIGNALS
    | DVP_REQUIRED_BYTE_SIGNALS
)
...
DVP_VISIBILITY_OBSERVABILITY_SCHEMA_NAME = (
    "openmvs.dmap.dvp_visibility_observability"
)
DVP_VISIBILITY_TRACE_SCHEMA_NAME = "openmvs.dmap.dvp_visibility_trace"
DVP_VISIBILITY_SCHEMA_VERSION = 1
DVP_VISIBILITY_REASON_NAMES = (
    "mode_disabled",
...
DVP_VISIBILITY_REQUIRED_RGBA_SIGNALS = {
    f"dvp_visibility_{name}_mask"
    for name in (
        "previous",
        "resolved",
        "next",
...
DVP_VISIBILITY_REQUIRED_WORD_SIGNALS = {
    f"dvp_visibility_{name}"
    for name in (
        "previous_weight_sum",
        "resolved_weight_sum",
        "next_weight_sum",
        "active_support_weight_sum",
        "denominator",
    )
}
DVP_VISIBILITY_REQUIRED_BYTE_SIGNALS = {
    f"dvp_visibility_{name}"
    for name in (
        ...
        "mode",
        "denominator_defined",
        "support_matches_resolved",
        "transition_status",
    )
} | {
    f"dvp_visibility_reason_{reason}"
    for reason in DVP_VISIBILITY_REASON_NAMES
}
DVP_VISIBILITY_REQUIRED_SIGNALS = (
    DVP_VISIBILITY_REQUIRED_RGBA_SIGNALS
    | DVP_VISIBILITY_REQUIRED_WORD_SIGNALS
    | DVP_VISIBILITY_REQUIRED_BYTE_SIGNALS
)
...
DVP_VISIBLE_NORMAL_OBSERVABILITY_SCHEMA_NAME = (
    "openmvs.dmap.dvp_visible_normal_observability"
)
DVP_VISIBLE_NORMAL_TRACE_SCHEMA_NAME = "openmvs.dmap.dvp_visible_normal_trace"
DVP_VISIBLE_NORMAL_SCHEMA_VERSION = 1
DVP_VISIBLE_NORMAL_EVALUATION_REASON_NAMES = (
    "valid",
    "invalid_normal",
...
DVP_VISIBLE_NORMAL_PROPOSAL_REASON_NAMES = (
    "invalid_argument_native_retained",
    "native_feasible",
    "constrained_retry",
    "retry_exhausted_native_fallback",
)
DVP_VISIBLE_NORMAL_PROPAGATION_REASON_NAMES = (
    "invalid_argument",
    "no_valid_candidate",
    "constrained_feasible",
    "no_feasible_native_fallback",
)
DVP_VISIBLE_NORMAL_REQUIRED_FLOAT_SIGNALS = {
    "dvp_visible_normal_current_max_dot",
    "dvp_visible_normal_current_max_violation",
    "dvp_visible_normal_propagation_native_best_cost",
    "dvp_visible_normal_propagation_constrained_best_cost",
    "dvp_visible_normal_propagation_selected_cost",
    *{
        f"dvp_visible_normal_{name}_{proposal}"
        for proposal in range(2)
        for name in ("native_max_dot", "selected_max_dot")
    },
}
DVP_VISIBLE_NORMAL_REQUIRED_WORD_SIGNALS = {
    f"dvp_visible_normal_{name}_{proposal}"
    for proposal in range(2)
    for name in ("retries_tested", "selected_retry")
}
DVP_VISIBLE_NORMAL_REQUIRED_RGBA_SIGNALS = {
    "dvp_visible_normal_selected_source_views",
}
DVP_VISIBLE_NORMAL_REQUIRED_BYTE_SIGNALS = {
    f"dvp_visible_normal_{name}"
    for name in (
        "support_count",
        "direction_count",
        "mode",
        ...
        "current_feasible",
        "current_reason",
        "current_rejected_direction",
        "propagation_tested_mask",
        "propagation_valid_mask",
        "propagation_feasible_mask",
        "propagation_rejected_mask",
        "propagation_native_best",
        "propagation_constrained_best",
        "propagation_selected",
        "propagation_reason",
        "propagation_fallback",
        "propagation_applied_constraint",
        "propagation_accepted",
        "refinement_native_tested_mask",
        "refinement_native_valid_mask",
        "refinement_native_feasible_mask",
        "refinement_retry_success_mask",
        "refinement_exhaustion_fallback_mask",
        "refinement_applied_retry_mask",
        "refinement_accepted_mask",
    )
} | {
    f"dvp_visible_normal_{name}_{proposal}"
    for proposal in range(2)
    for name in (
        "native_reason",
        "proposal_reason",
        "native_rejected_direction",
        "selected_rejected_direction",
    )
}
DVP_VISIBLE_NORMAL_REQUIRED_SIGNALS = (
    DVP_VISIBLE_NORMAL_REQUIRED_FLOAT_SIGNALS
    | DVP_VISIBLE_NORMAL_REQUIRED_WORD_SIGNALS
    | DVP_VISIBLE_NORMAL_REQUIRED_RGBA_SIGNALS
    | DVP_VISIBLE_NORMAL_REQUIRED_BYTE_SIGNALS
...
APD_SCHEMA_VERSION = 5
APD_SUMMARY_SCHEMA_NAME = "openmvs.dmap.apd_observability"
APD_SUMMARY_SCHEMA_VERSION = 5
APD_SUPPORTED_SCHEMA_VERSIONS = (1, 2, 3, 4, 5)
APD_RECORD_CONTRACTS = {
    ...
    4: {"state": 64, "update": 120, "trace": 1548, "candidate_slots": 22},
    5: {"state": 64, "update": 120, "trace": 1548, "candidate_slots": 24},
}
...
APD_SUMMARY_MEAN_RELATIVE_TOLERANCE = 1.0e-3
APD_BAD_COST = 1.2
...
def validate_frame_census(
    ...
    intentionally_unavailable_iterations: list[int] = []
    for line_index, row in matching_rows:
        ...
        dvp_iteration_only_initialization = (
            summary.get("candidate_accounting_mode")
            == "exact_dvp_active_objective_aggregate"
            and iteration == -1
        )
        expected_processed = 0 if dvp_iteration_only_initialization else area
        if processed != expected_processed:
            errors.append(
                f"{prefix}: processed={processed} does not match "
                f"expected census={expected_processed}"
            )
        if dvp_iteration_only_initialization:
            intentionally_unavailable_iterations.append(-1)
        ...
        elif dvp_iteration_only_initialization and any(candidate_values):
            errors.append(
                f"{prefix}: DVP iteration-only initialization candidate counters "
                "must remain zero"
            )
        elif (
    ...
    return not errors, {
        ...
        "intentionally_unavailable_iterations": intentionally_unavailable_iterations,
        "errors": errors,
    }
...
def validate_summary_only(arguments: Arguments, frame_dir: Path) -> dict[str, Any]:
    ...
    check("summary_schema", summary.get("schema_name") == "openmvs.dmap.frame_summary" and schema_version in {4, 5}, {
    ...
    dvp_observability = (
        summary.get("dvp_observability")
        if isinstance(summary.get("dvp_observability"), dict) else {}
    )
    dvp_aggregate_available = (
        schema_version == 5
        and dvp_observability.get("enabled") is True
        and dvp_observability.get("requested") is True
        and dvp_observability.get("summary_available") is True
        and dvp_observability.get("maps_available") is False
        and resource_plan.get("dvp_requested") is True
        and resource_plan.get("dvp_summary_available") is True
        and resource_plan.get("dvp_maps_available") is False
    )
    ...
    if exact_trace_available:
        ...
        dvp_exact_aggregate = (
            ...
            == "exact_dvp_active_objective_aggregate"
            and summary.get("confidence_gap_mode")
            == "exact_dvp_incumbent_proposal_runner_up_aggregate"
            and isinstance(dvp_observability.get("iterations"), list)
            and bool(dvp_observability.get("iterations"))
        )
        apd_dvp_exact_aggregate = (
            apd_aggregate_available
            and dvp_aggregate_available
            and summary.get("candidate_accounting_mode")
            == "exact_apd_and_dvp_active_objectives_aggregate"
            and summary.get("confidence_gap_mode")
            == "exact_apd_and_dvp_stage_specific_aggregate"
        )
        check("summary_exact_maps_unavailable", (
            generic_exact_unavailable or apd_exact_aggregate
            or dvp_exact_aggregate or apd_dvp_exact_aggregate
        ), {
            "candidate_accounting_mode": summary.get("candidate_accounting_mode"),
            "confidence_gap_mode": summary.get("confidence_gap_mode"),
            "apd_aggregate_available": apd_aggregate_available,
            "dvp_aggregate_available": dvp_aggregate_available,
            "unavailable_signals": sorted(unavailable),
        })
    ...
    result = {
        ...
        "dvp_validation": ({
            "available": True,
            "scope": "exact_active_process_counters",
            "family": dvp_observability.get("family"),
            "maps_available": False,
            "iterations": dvp_observability.get("iterations") or [],
        } if dvp_aggregate_available else {
            "available": False,
            "unavailable_reason": "DVP exact counters are inactive for this capture",
        }),
...
def validate_prefilter_only(arguments: Arguments, frame_dir: Path) -> dict[str, Any]:
    ...
    schema_valid = (
        ...
        and summary.get("schema_version") in {4, 5}
    )
    ...
    marker_valid = (
        ...
        and marker_summary.get("path") == "summary.json"
        and marker_summary.get("schema_version") == summary.get("schema_version")
...
def low_texture_decay_metadata(frame_dir: Path) -> dict[str, Any]:
    ...
    if metadata.get("schema_name") != "openmvs.dmap.run" or strict_int(
        ...
        result["unavailable_reason"] = "run_metadata.json is not a supported openmvs.dmap.run schema-v4/v5 artifact"
        return result
...
def exact_view_samples_metadata(frame_dir: Path) -> dict[str, Any]:
    ...
    if metadata.get("schema_name") != "openmvs.dmap.run" or strict_int(
        ...
        result["unavailable_reason"] = (
            "run_metadata.json is not a supported openmvs.dmap.run schema-v4/v5 artifact"
        )
        return result
    parameters = metadata.get("cuda_patchmatch_parameters")
    raw_view_samples = (
...
def validate_reference_patch_layout_contract(
    ...
    if (
        ...
        or strict_int(metadata.get("schema_version")) not in {4, 5}
    ):
        detail["errors"].append(
            "claimed layout requires openmvs.dmap.run schema v4 or v5"
        )
    parameters = metadata.get("cuda_patchmatch_parameters")
...
def validate_v4_exact(
    *,
    ...
    manifest_schema_version = strict_int(manifest.get("schema_version"))
    expected_candidate_slots = 24 if manifest_schema_version == 5 else 13
    declared_candidate_slots = strict_int(capture.get("candidate_slots"))
    candidate_slot_contract_valid = (
        declared_candidate_slots == expected_candidate_slots
        if manifest_schema_version == 5
        else declared_candidate_slots in {None, expected_candidate_slots}
    )
    check("v4_exact_candidate_slot_contract", candidate_slot_contract_valid, {
        "manifest_schema_version": manifest_schema_version,
        "declared_candidate_slots": declared_candidate_slots,
    ...
    for iteration in sorted(expected_iterations):
        ...
        if all(value is not None for value in (counts, tested_mask, finite_mask, accepted_mask)):
            ...
            count_mismatch = {
                "tested": int(np.count_nonzero(
                    counts[..., 0] != uint32_popcount(tested_uint, expected_candidate_slots)
                )),
                "finite": int(np.count_nonzero(
                    counts[..., 1] != uint32_popcount(finite_uint, expected_candidate_slots)
                )),
                "accepted": int(np.count_nonzero(
                    counts[..., 2] != uint32_popcount(accepted_uint, expected_candidate_slots)
                )),
            }
        ...
        if identity is not None:
            ...
            if np.any(winner_slots >= expected_candidate_slots) or np.any(
                (runner_slots >= expected_candidate_slots) & (runner_slots != 255)
            ):
...
def validate_apd_candidate_gap_semantics(
    *,
    working: np.ndarray,
    runner: np.ndarray,
    gap: np.ndarray,
    ...
    """Validate APD candidate-order gap semantics without conflating selection."""

    errors: list[str] = []
    if working.shape != runner.shape or runner.shape != gap.shape:
        return {
            "semantics": "unconstrained_evaluated_candidate_set_best_vs_runner_up",
            "shape_valid": False,
        }, ["working, runner-up, and gap maps have different shapes"]

    if constraint_mask is None:
        constraints = np.zeros(working.shape, dtype=bool)
        constraint_evidence = "not_active"
    elif constraint_mask.shape != working.shape:
        return {
            "semantics": "unconstrained_evaluated_candidate_set_best_vs_runner_up",
            "shape_valid": False,
        }, ["visible-normal constraint map has the wrong shape"]
    else:
        constraints = np.asarray(constraint_mask, dtype=bool)
        constraint_evidence = "exact_visible_normal_propagation_selection_map"

    runner_unavailable = runner == -1.0
    gap_unavailable = gap == -1.0
    availability_mismatch = runner_unavailable != gap_unavailable
    availability_mismatch_count = int(np.count_nonzero(availability_mismatch))
    if availability_mismatch_count:
        errors.append(
            "runner-up/gap availability differs at "
            f"{availability_mismatch_count} pixels"
        )

    available = ~runner_unavailable & ~gap_unavailable
    selected_gap = np.where(
        runner_unavailable, -1.0, np.maximum(runner - working, 0.0)
    )
    selected_difference = np.abs(gap - selected_gap)
    selected_divergence = available & (selected_difference > tolerance)
    uncovered_divergence = selected_divergence & ~constraints
    uncovered_divergence_count = int(np.count_nonzero(uncovered_divergence))
    if uncovered_divergence_count:
        errors.append(
            "candidate-set gap differs from the selected-winner gap outside a "
            "visible-normal propagation selection constraint at "
            f"{uncovered_divergence_count} pixels"
        )

    derived_best = runner - gap
    invalid_best = available & (
        ~np.isfinite(derived_best)
        | (derived_best < -tolerance)
        | (derived_best > runner + tolerance)
        | (working + tolerance < derived_best)
    )
    invalid_best_count = int(np.count_nonzero(invalid_best))
    if invalid_best_count:
        errors.append(
            "derived evaluated-candidate best violates ordering at "
            f"{invalid_best_count} pixels"
        )

    strict_pixels = available & ~constraints
    strict_closure_max_abs = (
        float(np.max(selected_difference[strict_pixels]))
        if np.any(strict_pixels) else 0.0
    )
    constrained_divergence = selected_divergence & constraints
    constrained_divergence_count = int(np.count_nonzero(constrained_divergence))
    return {
        "semantics": "unconstrained_evaluated_candidate_set_best_vs_runner_up",
        "selected_winner_relation": (
            "exact_except_at_visible_normal_propagation_selection_constraints"
            if constraint_mask is not None else "exact"
        ),
        "constraint_evidence": constraint_evidence,
        "shape_valid": True,
        "available_pixels": int(np.count_nonzero(available)),
        "constraint_pixels": int(np.count_nonzero(constraints)),
        "selected_winner_closure_pixels": int(
            np.count_nonzero(available & ~constrained_divergence)
        ),
        "selected_winner_gap_unavailable_pixels": constrained_divergence_count,
        "uncovered_divergence_pixels": uncovered_divergence_count,
        "availability_mismatch_pixels": availability_mismatch_count,
        "invalid_candidate_order_pixels": invalid_best_count,
        "strict_selected_closure_max_abs": strict_closure_max_abs,
        "maximum_selected_divergence": (
            float(np.max(selected_difference[available])) if np.any(available) else 0.0
        ),
    }, errors
...
def validate_apd_maps(
    *,
    ...
    mode = strict_int(observability.get("mode"))
    mode_role = observability.get("mode_role")
    full_mode = mode == 1 and mode_role == "full"
    deformation_only = mode == 2 and mode_role == "deformation_only"
    patchmatch_parameters_value = summary.get("cuda_patchmatch_parameters")
    patchmatch_parameters = (
        patchmatch_parameters_value
        if isinstance(patchmatch_parameters_value, dict) else {}
    )
    depth_edge_value = patchmatch_parameters.get("dvp_depth_edge")
    depth_edge = depth_edge_value if isinstance(depth_edge_value, dict) else {}
    depth_edge_enabled = (
        depth_edge.get("enabled") is True
        and (strict_int(depth_edge.get("mode")) or 0) > 0
    )
    visibility_capture_value = manifest.get("dvp_visibility_capture")
    visibility_capture = (
        visibility_capture_value
        if isinstance(visibility_capture_value, dict) else {}
    )
    visibility_active = (
        visibility_capture.get("requested") is True
        and visibility_capture.get("stage_active") is True
    )
    visible_normal_capture_value = manifest.get("dvp_visible_normal_capture")
    visible_normal_capture = (
        visible_normal_capture_value
        if isinstance(visible_normal_capture_value, dict) else {}
    )
    visible_normal_constraints_active = (
        visible_normal_capture.get("requested") is True
        and visible_normal_capture.get("stage_active") is True
        and visible_normal_capture.get("mode") in {"propagation", "refinement", "full"}
    )
    anchor_component_basis = (
        "visibility_aware_effective_anchor_leg"
        if visibility_active else "active_anchor_mean"
    )
    expected_component_contract = {
        "anchor_component_field": "anchor_mean_cost",
        "anchor_component_basis": anchor_component_basis,
        ...
        "visibility_fallback": (
            ...
            if visibility_active else "not_active"
        ),
    }
    ...
    source_names = (
        ...
        if capture_version == 2 else
        APD_UPDATE_SOURCE_NAMES[:12]
        if capture_version in {3, 4} else
        APD_UPDATE_SOURCE_NAMES
    )
    candidate_accounting_errors: list[str] = []
    if capture_version == 5:
        capture_accounting = capture.get("candidate_accounting")
        summary_accounting = observability.get("candidate_accounting")
        dvp_capture_value = manifest.get("dvp_capture")
        dvp_capture = (
            dvp_capture_value if isinstance(dvp_capture_value, dict) else {}
        )
        expected_dvp_active = (
            capture.get("requested") is True
            and capture.get("stage_active") is True
            and dvp_capture.get("requested") is True
        )
        expected_accounting = {
            "schema_name": "openmvs.dmap.apd_candidate_accounting",
            "schema_version": 1,
            "candidate_slots": 24,
            "dvp_candidate_slots": [22, 23],
            "dvp_active": expected_dvp_active,
            "update_source_codes": 13,
            "scope": "complete_active_process_pixel_working_objective",
        }
        if capture_accounting != expected_accounting:
            candidate_accounting_errors.append(
                "capture.candidate_accounting does not match APD schema v5"
            )
        if summary_accounting != expected_accounting:
            candidate_accounting_errors.append(
                "observability.candidate_accounting does not match APD schema v5"
            )
    apd_entries = [
        entry for entry in entries
        if (
    ...
    if capture.get("requested") is False and not apd_entries:
        ...
        if capture_version in {4, 5}:
            disabled_summary_expected.update({
                "mode_role": "control",
                "full_paper_mechanics_enabled": False,
                "active_mechanics": [],
                "required_mechanics_not_active": [],
            })
        capture_errors = [
        ...
        capture_errors.extend(candidate_accounting_errors)
        ...
        elif capture_version in {4, 5}:
            expected_capture = {
                "mode": 0,
                "mode_role": "control",
                "anchor_state": "unavailable_disabled",
                "anchor_candidate_slots": None,
                "native_propagation_candidate_slots": None,
                "fitted_plane_candidate_slot": None,
                "final_refinement_candidate_accounting": "not_active",
            }
            for field, expected in expected_capture.items():
                if capture.get(field) != expected:
                    capture_errors.append(
                        f"schema v{capture_version} disabled capture {field}: expected "
                        f"{expected!r}, got {capture.get(field)!r}"
                    )
    ...
    contract_errors.extend(candidate_accounting_errors)
    if not version_supported:
        contract_errors.append(
            f"schema_version {capture_version!r} is not supported; "
            f"expected one of {list(APD_SUPPORTED_SCHEMA_VERSIONS)}"
    ...
    if capture_version == 2:
        ...
        if capture.get("mode") != mode or capture.get("mode_role") != mode_role:
            contract_errors.append(
                f"schema v{capture_version} capture mode identity must match APD observability"
            )
        if full_mode:
            expected_capture = {
                "anchor_state": (
        ...
        elif deformation_only:
            expected_capture = {
                "anchor_state": "immutable_pre_iteration_support_for_native_checkerboard",
                "anchor_candidate_slots": None,
                "native_propagation_candidate_slots": [1, 8],
                "fitted_plane_candidate_slot": None,
                "final_refinement_candidate_accounting": "not_active",
            }
        else:
            expected_capture = {}
            contract_errors.append(
                f"schema v{capture_version} active capture requires mode 1/full or "
                "mode 2/deformation_only"
            )
        for field, expected in expected_capture.items():
            if capture.get(field) != expected:
                contract_errors.append(
                    f"schema v{capture_version} capture {field}: expected {expected!r}, "
    ...
    if capture_version in {4, 5}:
        expected_summary_contract.update({
            "mode": mode,
            "mode_role": mode_role,
            "full_paper_mechanics_enabled": full_mode,
        })
    ...
    component_contract = observability.get("component_contract")
    if capture_version == 5:
        if visibility_active and component_contract != expected_component_contract:
            summary_errors.append(
                "visibility-aware APD component contract is missing or invalid"
            )
        elif (
            component_contract is not None
            and component_contract != expected_component_contract
        ):
            summary_errors.append("APD component contract is invalid")
    implementation_label = observability.get("implementation_label")
    if implementation_label not in {
        "paper_partial",
        "paper_mechanics_complete_openmvs",
        "deformation_only_openmvs",
    }:
        summary_errors.append(
    ...
    if capture_version in {3, 4, 5}:
        implemented_through = observability.get("implemented_through")
        supported_checkpoints = (
            {"APD14-C3", "APD14-C4"} if capture_version == 3 else {"APD14-C5"}
        )
        if implemented_through not in supported_checkpoints:
            summary_errors.append(
                f"schema v{capture_version} implemented_through must be one of "
        ...
        validate_full_mechanics = capture_version == 3 or full_mode
        if validate_full_mechanics:
            if not isinstance(schedule, dict) or (
                schedule.get("enabled", True) is not True
                or schedule.get("ordered_stages") != [
                    "reliable_black",
                    "reliable_red",
                ...
                summary_errors.append(
                    f"schema v{capture_version} reliable-first schedule is missing or invalid"
                )
            if not isinstance(fitted_plane, dict) or (
                fitted_plane.get("enabled", True) is not True
                or fitted_plane.get("candidate_position") != (
                    "before random depth and normal refinement in each non-reliable pixel update"
                )
            ):
                summary_errors.append(
                    f"schema v{capture_version} fitted-plane contract is missing or invalid"
                )
            ...
            if not isinstance(final_refinement, dict) or (
                final_refinement.get("enabled", True) is not True
                or final_refinement.get("disparity_radius") != 5
                or minimum_improvement is None
                or abs(minimum_improvement - 0.1) > 1.0e-6
            ):
                summary_errors.append(
                    f"schema v{capture_version} final-refinement contract is missing or invalid"
                )
        elif deformation_only:
            omitted = observability.get("required_mechanics_not_active")
            expected_omitted = {
                "anchor_driven_view_selection",
                "anchor_plane_propagation",
                "reliable_first_schedule",
                "fitted_plane_proposal",
                "terminal_native_refinement",
            }
            if implementation_label != "deformation_only_openmvs":
                summary_errors.append(
                    "schema v4 deformation-only mode requires its explicit implementation label"
                )
            if not isinstance(omitted, list) or set(omitted) != expected_omitted:
                summary_errors.append(
                    f"schema v{capture_version} deformation-only mode must enumerate all omitted mechanics"
                )
            for name, value in {
                "schedule": schedule,
                ...
                if not isinstance(value, dict) or value.get("enabled") is not False:
                    summary_errors.append(
                        f"schema v{capture_version} deformation-only {name} must be explicitly disabled"
                    )
            anchor_update = observability.get("anchor_update")
            if not isinstance(anchor_update, dict) or anchor_update.get("enabled") is not False:
                summary_errors.append(
                    f"schema v{capture_version} deformation-only anchor update must be explicitly disabled"
                )
        elif capture_version in {4, 5}:
            summary_errors.append(
                f"schema v{capture_version} requires mode 1/full or mode 2/deformation_only"
            )
        if capture_version in {4, 5} and (full_mode or deformation_only):
            expected_active = {
                "reliability_profiles",
                "reliable_anchor_ransac",
                "deformable_patch_cost",
                "native_persistent_winner_rescore",
                "multiscale_reliability_transfer",
            }
            if full_mode:
            ...
            active_mechanics = observability.get("active_mechanics")
            if not isinstance(active_mechanics, list) or set(active_mechanics) != expected_active:
                summary_errors.append(
                    f"schema v{capture_version} active_mechanics does not match the selected mode"
                )
        if implemented_through in {"APD14-C4", "APD14-C5"}:
            ...
            if not isinstance(multiscale, dict) or (
                ...
                summary_errors.append(
                    f"{implemented_through} multiscale contract is missing or invalid"
                )
            ...
            if not isinstance(compatibility, dict) or not nonempty_text(
                ...
                summary_errors.append(
                    f"{implemented_through} coarsest compatibility behavior is missing"
                )
    ...
    for entry in apd_entries:
        ...
        if (
            ...
            and entry.get("candidate_slots") != candidate_slots
        ):
            metadata_errors.append(
                f"{label} candidate_slots: expected {candidate_slots!r}, "
                f"got {entry.get('candidate_slots')!r}"
            )
        if not nonempty_text(entry.get("semantics")):
            metadata_errors.append(f"{label} semantics must be non-empty")
        if capture_version == 5 and signal == "apd_anchor_mean_cost":
            component_basis = entry.get("component_basis")
            if visibility_active and component_basis != anchor_component_basis:
                metadata_errors.append(
                    f"{label} component_basis: expected "
                    f"{anchor_component_basis!r}, got {component_basis!r}"
                )
            elif (
                component_basis is not None
                and component_basis != anchor_component_basis
            ):
                metadata_errors.append(
                    f"{label} component_basis: expected "
                    f"{anchor_component_basis!r}, got {component_basis!r}"
                )
        if capture_version == 5 and signal == "apd_deformable_photometric_cost":
            expected_identity = (
                "0.25*center_cost + 0.75*anchor_component"
            )
            component_identity = entry.get("component_identity")
            if visibility_active and component_identity != expected_identity:
                metadata_errors.append(
                    f"{label} component_identity: expected "
                    f"{expected_identity!r}, got {component_identity!r}"
                )
            elif (
                component_identity is not None
                and component_identity != expected_identity
            ):
                metadata_errors.append(
                    f"{label} component_identity: expected "
                    f"{expected_identity!r}, got {component_identity!r}"
                )
        if capture_version == 5 and signal == "apd_winner_runner_up_gap":
            expected_gap_metadata = {
                "gap_basis": (
                    "unconstrained_evaluated_candidate_set_best_vs_runner_up"
                ),
                "selected_winner_relation": (
                    "exact_unless_optional_candidate_constraint_rejects_raw_best"
                ),
            }
            for field, expected in expected_gap_metadata.items():
                if entry.get(field) != expected:
                    metadata_errors.append(
                        f"{label} {field}: expected {expected!r}, "
    ...
    closure: dict[str, dict[str, float]] = {}
    candidate_stats: dict[str, Any] = {}
    fitted_plane_stats: dict[str, Any] = {}
    dispatch_stats: dict[str, Any] = {}
    gap_semantics: dict[str, Any] = {}
    ...
    if capture_version in {3, 4, 5}:
        categorical_ranges.update({
            ...
            # Schema v4 introduced deformation-only mode, where every pixel
            # deliberately remains in the undivided stage encoded as zero.
            "apd_update_stage": (1, 2) if capture_version == 3 else (0, 2),
    ...
    for iteration in sorted(expected_iterations):
        ...
        if active is not None and eligible is not None:
            if depth_edge_enabled:
                if np.any((active == 1) & (eligible != 1)):
                    relationship_errors.append(
                        f"iteration {iteration}: depth-edge-active pixel lacks pre-filter eligibility"
                    )
            elif np.any(active != eligible):
                relationship_errors.append(
        ...
        if capture_version is not None and capture_version >= 2:
            ...
            if all(value is not None for value in c2_arrays):
                ...
                if deformation_only:
                    if np.any(view_selection_mode != 0):
                        relationship_errors.append(
                            f"iteration {iteration}: deformation-only used non-native "
                            "view selection"
                        )
                elif np.any(active_mask & (view_selection_mode == 0)) or np.any(
        ...
        if capture_version in {3, 4, 5}:
            ...
            if all(value is not None for value in c3_arrays):
                ...
                dispatch_basis = (
                    "deformation_only_native_checkerboard"
                    if deformation_only else "apd_reliability_class"
                )
                ...
                if dispatch_reliability is not None:
                    expected_stage = (
                        np.zeros_like(update_stage)
                        if deformation_only else
                        np.where(dispatch_reliability == 2, 1, 2)
                    )
                ...
                fitted_score_available_mask = fitted_working >= 0.0
                fitted_usable_mask = (
                    fitted_score_available_mask & (fitted_working < APD_BAD_COST)
                )
                fitted_plane_stats[str(iteration)] = {
                    "evaluated_score_pixels": int(np.count_nonzero(
                        fitted_score_available_mask
                    )),
                    "usable_score_pixels": int(np.count_nonzero(
                        fitted_usable_mask
                    )),
                    "accepted_events": int(np.count_nonzero(
                        fitted_accepted_mask
                    )),
                    "accepted_non_usable_events": int(np.count_nonzero(
                        fitted_accepted_mask & ~fitted_usable_mask
                    )),
                    "usable_definition": (
                        "IEEE-finite, nonnegative, and strictly below fBadCost"
                    ),
                    "bad_cost": APD_BAD_COST,
                }
                if np.any(
                    fitted_score_available_mask & ~fitted_tested_mask
                ) or np.any(
                    fitted_accepted_mask & ~fitted_score_available_mask
                ):
                    relationship_errors.append(
                ...
                if deformation_only and (
                    np.any(fitted_available != 0)
                    or np.any(fitted_tested != 0)
                    or np.any(fitted_accepted != 0)
                    or np.any(fitted_working != -1.0)
                    or np.any(fitted_native != -1.0)
                ):
                    relationship_errors.append(
                        f"iteration {iteration}: deformation-only executed fitted-plane mechanics"
                    )
                ...
                if deformation_only and (
                    ...
                    relationship_errors.append(
                        f"iteration {iteration}: deformation-only executed terminal refinement"
                    )
        ...
        if all(value is not None for value in (runner, gap, working)):
            ...
            if visible_normal_constraints_active:
                propagation_constraint = logical(
                    "dvp_visible_normal_propagation_applied_constraint", iteration
                )
                if propagation_constraint is None:
                    relationship_errors.append(
                        f"iteration {iteration}: visible-normal gap constraint evidence "
                        "is unavailable"
                    )
                else:
                    # Refinement retries replace the proposal before scoring and
                    # therefore cannot change the evaluated-candidate ordering.
                    constraint_mask = propagation_constraint == 1
            gap_result, gap_errors = validate_apd_candidate_gap_semantics(
                working=working,
                runner=runner,
                gap=gap,
            ...
            gap_semantics[str(iteration)] = gap_result
            relationship_errors.extend(
                f"iteration {iteration}: {error}" for error in gap_errors
            )
            iteration_closure["winner_runner_up_gap_selected_unconstrained"] = (
                float(gap_result.get("strict_selected_closure_max_abs", float("inf")))
            )
        if all(value is not None for value in (
            active,
            center,
        ...
        if isinstance(row, dict) and all(value is not None for value in (
            ...
            if capture_version is not None and capture_version >= 2:
                view_mode_counts = (
                    {name: 0 for name in APD_VIEW_SELECTION_MODE_NAMES}
                    if deformation_only else
                    {
                        name: int(np.count_nonzero(
                            active_mask & (view_selection_mode == index)
                        ))
                ...
                expected_counts.update({
                    ...
                    "anchor_view_selection_attempted": (
                        0 if deformation_only else int(np.count_nonzero(active_mask))
                    ),
                    "anchor_view_selection_used": int(
                        0 if deformation_only else
            ...
            if capture_version == 3:
                ...
                stage_all = int(np.count_nonzero(update_stage == 0))
                expected_counts.update({
                    "stage_counts": {
                        "all_compatibility": stage_all,
                        "all_deformation_only": stage_all if deformation_only else 0,
                        "reliable_first": int(np.count_nonzero(update_stage == 1)),
                        "non_reliable_second": int(np.count_nonzero(update_stage == 2)),
                    },
                    "fitted_plane_available": int(
            ...
            has_active = bool(np.any(active_mask))
            expected_means = {
                ...
                "center_cost_mean": (
                    float(np.mean(center[active_mask], dtype=np.float64))
                    if has_active else None
                ),
                "anchor_mean_cost_mean": (
                    float(np.mean(anchor[active_mask], dtype=np.float64))
                    if has_active else None
                ),
                "working_cost_mean": (
                    float(np.mean(working[active_mask], dtype=np.float64))
                    if has_active else None
                ),
            ...
            mean_tolerances = {
                name: max(
                    APD_SUMMARY_MEAN_TOLERANCE,
                    APD_SUMMARY_MEAN_RELATIVE_TOLERANCE
            ...
            summary_stats[str(iteration)] = {
                ...
                "mean_relative_tolerance": APD_SUMMARY_MEAN_RELATIVE_TOLERANCE,
                "mean_tolerances": mean_tolerances,
    ...
    check("apd_map_relationships", not relationship_errors, {
        ...
        "deformable_activity_contract": (
            "depth_edge_active_is_subset_of_prefilter_eligible"
            if depth_edge_enabled else "active_equals_eligible"
        ),
        "candidate": candidate_stats,
        "fitted_plane": fitted_plane_stats,
        "dispatch": dispatch_stats,
    })
    check("apd_cost_closure", not closure_failures, {
        "tolerance": max(tolerance, APD_COMPONENT_CLOSURE_TOLERANCE),
        "per_iteration": closure,
        "candidate_gap_semantics": gap_semantics,
    })
...
def validate_dvp_maps(
    *,
    ...
    """Validate schema-v5 DVP mechanics from active records through artifacts."""

    capture = manifest.get("dvp_capture")
    capture = capture if isinstance(capture, dict) else {}
    observability = summary.get("dvp_observability")
    observability = observability if isinstance(observability, dict) else {}
    provenance = observability.get("provenance")
    ...
    present_entries = [
        ...
        if (
            isinstance(entry, dict)
            and str(entry.get("signal", "")).startswith("dvp_")
            and not str(entry.get("signal", "")).startswith("dvp_visibility_")
            and not str(entry.get("signal", "")).startswith("dvp_visible_normal_")
        )
    ]
    common_contract = (
        capture.get("schema_name") == DVP_SCHEMA_NAME
        and strict_int(capture.get("schema_version")) == DVP_SCHEMA_VERSION
        and observability.get("schema_name") == DVP_OBSERVABILITY_SCHEMA_NAME
        and strict_int(observability.get("schema_version")) == DVP_SCHEMA_VERSION
        and capture.get("author_code_exact_equivalence") is False
        and observability.get("author_code_exact_equivalence") is False
        and capture.get("family") == observability.get("family")
        and provenance.get("official_repository_commit")
        == "5b2b31c02941cfa28808b4e37f06a87e8dc0f2d9"
        and provenance.get("colleague_donor_commit")
        == "c6f0f2c0fe2e31ec68c54cac681cc3b1665d2f7b"
    )
    if not requested:
        disabled_valid = (
            common_contract
            and capture.get("maps_available") is False
            and capture.get("summary_available") is False
            and observability.get("requested") is False
            and observability.get("summary_available") is False
            and observability.get("maps_available") is False
            and observability.get("iterations") == []
            and not present_entries
        )
        check("dvp_disabled_contract", disabled_valid, {
            "capture": capture,
            "observability": observability,
            "present_signals": sorted(str(entry.get("signal")) for entry in present_entries),
        })
        return {
            "available": False,
            "requested": False,
            "family": capture.get("family"),
            "unavailable_reason": "DVP is inactive for this estimation stage",
        }

    family = str(capture.get("family") or "")
    family_code = DVP_FAMILY_CODES.get(family)
    num_iterations = strict_int(capture.get("num_iterations"))
    expected_iterations = (
        set(range(num_iterations))
        if num_iterations is not None and num_iterations > 0 else set()
    )
    promotion_expected = family in {
        "global_search_gated_v1", "dvp_eq11_interval_v1"
    }
    contract_valid = (
        common_contract
        and family_code is not None
        and capture.get("maps_available") is True
        and capture.get("summary_available") is True
        and capture.get("proposal_candidate_slots") == [22, 23]
        and capture.get("endpoint_samples_full_frame") is False
        and capture.get("endpoint_samples_targeted_trace") is True
        and capture.get("promotion_eligible") is promotion_expected
        and observability.get("promotion_eligible") is promotion_expected
        and observability.get("requested") is True
        and observability.get("summary_available") is True
        and observability.get("maps_available") is True
        and observability.get("measurement_basis")
        == "exact active Process<true> candidate generation, scoring, acceptance, and retained outcome"
        and strict_int(capture.get("update_record_bytes")) == 148
        and strict_int(observability.get("update_record_bytes")) == 148
        and strict_int(capture.get("counter_record_bytes")) == 280
        and strict_int(observability.get("counter_record_bytes")) == 280
        and strict_int(observability.get("trace_record_bytes")) == 700
        and capture.get("aggregate_sum_precision")
        == "float64 device atomic; counts are uint32 device atomic"
        and observability.get("aggregate_sum_precision")
        == "float64 device atomic; counts are uint32 device atomic"
        and expected_iterations
    )
    check("dvp_contract", contract_valid, {
        "family": family,
        "family_code": family_code,
        "capture": capture,
        "observability_record_bytes": {
            "update": observability.get("update_record_bytes"),
            "counter": observability.get("counter_record_bytes"),
            "trace": observability.get("trace_record_bytes"),
        },
    })
    ...
    expected_pairs = {
        (signal, iteration)
        for iteration in expected_iterations
        for signal in DVP_REQUIRED_SIGNALS
    }
    present_pairs = {
        (str(entry.get("signal")), strict_int(entry.get("logical_iteration")))
    ...
    for entry in present_entries:
        ...
        iteration = strict_int(entry.get("logical_iteration"))
        expected_dtype = (
            "float32" if signal in DVP_REQUIRED_FLOAT_SIGNALS
            else "uint8x4" if signal in DVP_REQUIRED_RGBA_SIGNALS
            else "uint8"
        )
        if iteration not in expected_iterations:
            metadata_errors.append(f"{signal}: invalid logical_iteration {iteration!r}")
        if entry.get("dtype") != expected_dtype:
            metadata_errors.append(
                f"{signal}:{iteration}: dtype {entry.get('dtype')!r}, expected {expected_dtype!r}"
            )
        for key, expected in (
            ("role", "dvp_logical_iteration_update"),
            ("stage", "iteration"),
            ("dvp_schema_name", DVP_SCHEMA_NAME),
            ("dvp_schema_version", DVP_SCHEMA_VERSION),
            ("measurement_quality", "exact"),
            ("measurement_basis", "active_process_pixel_candidate_path"),
        ):
            if entry.get(key) != expected:
                metadata_errors.append(
                    f"{signal}:{iteration}: {key}={entry.get(key)!r}, expected {expected!r}"
                )
    check("dvp_map_inventory", present_pairs == expected_pairs and not metadata_errors, {
        "expected_map_count": len(expected_pairs),
        "present_map_count": len(present_pairs),
        "missing": sorted(f"{signal}:{iteration}" for signal, iteration in expected_pairs - present_pairs),
        "unexpected": sorted(f"{signal}:{iteration}" for signal, iteration in present_pairs - expected_pairs),
        "metadata_errors": metadata_errors,
    })

    def values(signal: str, iteration: int) -> np.ndarray | None:
        record = logical_maps.get((signal, iteration))
        return record[2] if record is not None else None

    def close_count(actual: Any, expected: int) -> bool:
        return strict_int(actual) == expected

    def finite_mean(data: np.ndarray, available: np.ndarray) -> float | None:
        selected = np.asarray(data, dtype=np.float64)[available]
        return float(np.mean(selected)) if selected.size else None

    def close_float(actual: Any, expected: float | None) -> bool:
        if expected is None:
            return actual is None
        value = finite_number(actual)
        return value is not None and bool(np.isclose(
            value, expected, rtol=2e-5, atol=max(tolerance, 2e-6)
        ))

    summary_iterations = observability.get("iterations")
    summary_iterations = summary_iterations if isinstance(summary_iterations, list) else []
    summary_by_iteration = {
        strict_int(row.get("logical_iteration")): row
        for row in summary_iterations if isinstance(row, dict)
    }
    iteration_results: dict[str, Any] = {}
    all_domain_errors: dict[str, int] = {}
    for iteration in sorted(expected_iterations):
        required = {
            signal: values(signal, iteration) for signal in DVP_REQUIRED_SIGNALS
        }
        if any(data is None for data in required.values()):
            iteration_results[str(iteration)] = {"available": False}
            continue

        generated = required["dvp_generated_count"].astype(np.uint8)
        tested = required["dvp_tested_count"].astype(np.uint8)
        finite = required["dvp_finite_count"].astype(np.uint8)
        accepted_count = required["dvp_accepted_count"].astype(np.uint8)
        tested_mask = required["dvp_tested_proposal_mask"].astype(np.uint8)
        finite_mask = required["dvp_finite_proposal_mask"].astype(np.uint8)
        accepted_mask = required["dvp_accepted_proposal_mask"].astype(np.uint8)
        accepted = required["dvp_accepted"].astype(np.uint8)
        final_winner = required["dvp_final_winner"].astype(np.uint8)
        final_source = required["dvp_final_update_source"].astype(np.uint8)
        depth_retained = required["dvp_final_depth_retained"].astype(np.uint8)
        native_fallback = required["dvp_native_depth_fallback"].astype(np.uint8)
        left_valid = required["dvp_left_interval_valid"].astype(np.uint8)
        right_valid = required["dvp_right_interval_valid"].astype(np.uint8)
        winner_ordinal = required["dvp_winner_ordinal"].astype(np.uint8)
        unavailable_reason = required["dvp_unavailable_reason"].astype(np.uint8)
        family_values = required["dvp_family"].astype(np.uint8)
        incumbent_cost = required["dvp_incumbent_cost"].astype(np.float32)
        winner_cost = required["dvp_winner_cost"].astype(np.float32)
        runner_up_cost = required["dvp_runner_up_cost"].astype(np.float32)
        gap = required["dvp_winner_runner_up_gap"].astype(np.float32)
        incumbent_depth = required["dvp_incumbent_depth"].astype(np.float32)
        final_depth = required["dvp_final_depth"].astype(np.float32)
        displacement = required["dvp_depth_displacement"].astype(np.float32)
        candidate_costs = [
            required[f"dvp_candidate_cost_{proposal}"].astype(np.float32)
            for proposal in range(DVP_MAX_PROPOSALS)
        ]
        proposal_depths = [
            required[f"dvp_proposal_depth_{proposal}"].astype(np.float32)
            for proposal in range(DVP_MAX_PROPOSALS)
        ]

        generated_mask = np.where(
            generated == 0, 0, np.where(generated == 1, 1, 3)
        ).astype(np.uint8)
        errors = {
            "count_order": int(np.count_nonzero(
                (generated > DVP_MAX_PROPOSALS)
                | (tested > generated) | (finite > tested)
                | (accepted_count > finite)
            )),
            "tested_mask_count": int(np.count_nonzero(
                tested != uint32_popcount(tested_mask, DVP_MAX_PROPOSALS)
            )),
            "finite_mask_count": int(np.count_nonzero(
                finite != uint32_popcount(finite_mask, DVP_MAX_PROPOSALS)
            )),
            "accepted_mask_count": int(np.count_nonzero(
                accepted_count != uint32_popcount(accepted_mask, DVP_MAX_PROPOSALS)
            )),
            "mask_subset": int(np.count_nonzero(
                (tested_mask & ~generated_mask)
                | (finite_mask & ~tested_mask)
                | (accepted_mask & ~finite_mask)
                | (tested_mask > 3) | (finite_mask > 3) | (accepted_mask > 3)
            )),
            "accepted_flag": int(np.count_nonzero(
                accepted != (accepted_count > 0).astype(np.uint8)
            )),
            "family": int(np.count_nonzero(family_values != family_code)),
            "unavailable_reason": int(np.count_nonzero(unavailable_reason > 11)),
            "binary_domains": sum(int(np.count_nonzero(value > 1)) for value in (
                accepted, final_winner, depth_retained, native_fallback,
                left_valid, right_valid,
            )),
            "final_source": int(np.count_nonzero(final_source > 12)),
            "final_winner_source": int(np.count_nonzero(
                final_winner != (final_source == DVP_FINAL_SOURCE_CODES["dvp_epipolar"]).astype(np.uint8)
            )),
        }

        finite_bits: list[np.ndarray] = []
        for proposal in range(DVP_MAX_PROPOSALS):
            generated_bit = (generated_mask & (1 << proposal)) != 0
            tested_bit = (tested_mask & (1 << proposal)) != 0
            finite_bit = (finite_mask & (1 << proposal)) != 0
            accepted_bit = (accepted_mask & (1 << proposal)) != 0
            finite_bits.append(finite_bit)
            costs = candidate_costs[proposal]
            depths = proposal_depths[proposal]
            errors[f"proposal_{proposal}_depth"] = int(np.count_nonzero(
                np.where(generated_bit, ~(np.isfinite(depths) & (depths > 0)), depths != -1)
            ))
            errors[f"proposal_{proposal}_cost"] = int(np.count_nonzero(
                np.where(finite_bit, ~(np.isfinite(costs) & (costs >= 0)), costs != -1)
            ))
            errors[f"proposal_{proposal}_accepted"] = int(np.count_nonzero(
                accepted_bit & ~finite_bit
            ))
            errors[f"proposal_{proposal}_tested"] = int(np.count_nonzero(
                tested_bit & ~generated_bit
            ))

        decision_stack = np.stack([
            np.where(np.isfinite(incumbent_cost) & (incumbent_cost >= 0), incumbent_cost, np.inf),
            *[
                np.where(bit, cost, np.inf)
                for bit, cost in zip(finite_bits, candidate_costs, strict=True)
            ],
        ])
        ordering = np.argsort(decision_stack, axis=0, kind="stable")
        expected_winner_slot = ordering[0]
        expected_winner = np.take_along_axis(
            decision_stack, expected_winner_slot[None, ...], axis=0
        )[0]
        expected_runner = np.take_along_axis(
            decision_stack, ordering[1][None, ...], axis=0
        )[0]
        expected_ordinal = np.where(
            expected_winner_slot == 0, 255, expected_winner_slot - 1
        ).astype(np.uint8)
        expected_accepted = (expected_winner_slot != 0).astype(np.uint8)
        expected_gap = np.where(
            np.isfinite(expected_runner), expected_runner - expected_winner, -1
        ).astype(np.float32)
        expected_runner = np.where(np.isfinite(expected_runner), expected_runner, -1)
        errors["winner_ordinal"] = int(np.count_nonzero(winner_ordinal != expected_ordinal))
        errors["winner_accepted"] = int(np.count_nonzero(accepted != expected_accepted))
        errors["winner_cost"] = int(np.count_nonzero(~np.isclose(
            winner_cost, expected_winner, rtol=0, atol=max(tolerance, 2e-6)
        )))
        errors["runner_up_cost"] = int(np.count_nonzero(~np.isclose(
            runner_up_cost, expected_runner, rtol=0, atol=max(tolerance, 2e-6)
        )))
        errors["winner_runner_up_gap"] = int(np.count_nonzero(~np.isclose(
            gap, expected_gap, rtol=0, atol=max(tolerance, 2e-6)
        )))

        expected_sequential_mask = np.zeros(generated.shape, dtype=np.uint8)
        sequential_cost = incumbent_cost.copy()
        for proposal, (finite_bit, candidate_cost) in enumerate(
            zip(finite_bits, candidate_costs, strict=True)
        ):
            improves = finite_bit & (candidate_cost < sequential_cost)
            expected_sequential_mask |= improves.astype(np.uint8) << proposal
            sequential_cost = np.where(improves, candidate_cost, sequential_cost)
        errors["sequential_acceptance"] = int(np.count_nonzero(
            accepted_mask != expected_sequential_mask
        ))

        winning_depth = np.where(
            winner_ordinal == 0,
            proposal_depths[0],
            np.where(winner_ordinal == 1, proposal_depths[1], -1),
        )
        expected_depth_retained = (
            (accepted != 0) & (final_depth == winning_depth)
        ).astype(np.uint8)
        errors["depth_retained"] = int(np.count_nonzero(
            depth_retained != expected_depth_retained
        ))
        errors["final_winner_retains_depth"] = int(np.count_nonzero(
            (final_winner != 0) & (depth_retained == 0)
        ))
        valid_depths = (
            np.isfinite(incumbent_depth) & (incumbent_depth > 0)
            & np.isfinite(final_depth) & (final_depth > 0)
        )
        expected_displacement = np.where(
            valid_depths, np.abs(final_depth - incumbent_depth), -1
        )
        errors["depth_displacement"] = int(np.count_nonzero(~np.isclose(
            displacement, expected_displacement, rtol=0, atol=max(tolerance, 2e-6)
        )))

        parameters = observability.get("parameters")
        parameters = parameters if isinstance(parameters, dict) else {}
        mu = strict_int(parameters.get("mu"))
        if family == "dvp_eq11_interval_v1":
            left_min = required["dvp_left_interval_minimum"]
            left_max = required["dvp_left_interval_maximum"]
            right_min = required["dvp_right_interval_minimum"]
            right_max = required["dvp_right_interval_maximum"]
            expected_left_valid = (
                np.isfinite(left_min) & np.isfinite(left_max)
                & (left_min > 0) & (left_min < left_max)
            ).astype(np.uint8)
            expected_right_valid = (
                np.isfinite(right_min) & np.isfinite(right_max)
                & (right_min > 0) & (right_min < right_max)
            ).astype(np.uint8)
            errors["left_interval_valid"] = int(np.count_nonzero(
                left_valid != expected_left_valid
            ))
            errors["right_interval_valid"] = int(np.count_nonzero(
                right_valid != expected_right_valid
            ))
            errors["interval_proposal_count"] = int(np.count_nonzero(
                generated != left_valid + right_valid
            ))
            errors["native_fallback"] = int(np.count_nonzero(
                native_fallback != (generated == 0).astype(np.uint8)
            ))
            if mu is None or mu <= 0:
                errors["endpoint_support"] = width * height
            else:
                left_endpoint_supported = (
                    (required["dvp_left_outer_count"] >= mu)
                    & (required["dvp_left_inner_count"] >= mu)
                )
                right_endpoint_supported = (
                    (required["dvp_right_inner_count"] >= mu)
                    & (required["dvp_right_outer_count"] >= mu)
                )
                errors["endpoint_support"] = int(np.count_nonzero(
                    ((left_valid != 0) & ~left_endpoint_supported)
                    | ((right_valid != 0) & ~right_endpoint_supported)
                ))
                for proposal in range(DVP_MAX_PROPOSALS):
                    generated_bit = (generated_mask & (1 << proposal)) != 0
                    support = required[f"dvp_support_{proposal}"]
                    errors[f"proposal_{proposal}_support"] = int(np.count_nonzero(
                        generated_bit & (support != mu)
                    ))
            signed_0 = required["dvp_signed_offset_0"]
            signed_1 = required["dvp_signed_offset_1"]
            expected_signed_0 = np.where(left_valid != 0, -1, np.where(right_valid != 0, 1, 0))
            expected_signed_1 = np.where((left_valid != 0) & (right_valid != 0), 1, 0)
            errors["interval_side_0"] = int(np.count_nonzero(signed_0 != expected_signed_0))
            errors["interval_side_1"] = int(np.count_nonzero(signed_1 != expected_signed_1))

        summary_row = summary_by_iteration.get(iteration)
        counter_errors: list[str] = []
        if not isinstance(summary_row, dict):
            counter_errors.append("summary iteration is missing")
            summary_row = {}
        proposals = summary_row.get("proposals")
        proposals = proposals if isinstance(proposals, dict) else {}
        intervals = summary_row.get("intervals")
        intervals = intervals if isinstance(intervals, dict) else {}
        views = summary_row.get("views")
        views = views if isinstance(views, dict) else {}
        costs = summary_row.get("costs")
        costs = costs if isinstance(costs, dict) else {}
        geometry = summary_row.get("geometry")
        geometry = geometry if isinstance(geometry, dict) else {}
        unavailable_counts = summary_row.get("unavailable_reason_counts")
        unavailable_counts = unavailable_counts if isinstance(unavailable_counts, dict) else {}
        exact_counts = {
            "attempted_pixels": width * height,
            "proposal_available_pixels": int(np.count_nonzero(generated > 0)),
            "native_depth_fallback_pixels": int(np.count_nonzero(native_fallback)),
            "generated": int(generated.sum(dtype=np.uint64)),
            "tested": int(tested.sum(dtype=np.uint64)),
            "finite": int(finite.sum(dtype=np.uint64)),
            "accepted_events": int(accepted_count.sum(dtype=np.uint64)),
            "final_winner_pixels": int(final_winner.sum(dtype=np.uint64)),
            "final_depth_retained_pixels": int(depth_retained.sum(dtype=np.uint64)),
            "left_valid_pixels": int(left_valid.sum(dtype=np.uint64)),
            "right_valid_pixels": int(right_valid.sum(dtype=np.uint64)),
        }
        for key in ("attempted_pixels", "proposal_available_pixels", "native_depth_fallback_pixels"):
            if not close_count(summary_row.get(key), exact_counts[key]):
                counter_errors.append(f"{key} does not close to maps")
        for key in (
            "generated", "tested", "finite", "accepted_events",
            "final_winner_pixels", "final_depth_retained_pixels",
        ):
            if not close_count(proposals.get(key), exact_counts[key]):
                counter_errors.append(f"proposals.{key} does not close to maps")
        for key in ("left_valid_pixels", "right_valid_pixels"):
            if not close_count(intervals.get(key), exact_counts[key]):
                counter_errors.append(f"intervals.{key} does not close to maps")
        source_counts = proposals.get("final_update_source_counts")
        source_counts = source_counts if isinstance(source_counts, dict) else {}
        for name, code in DVP_FINAL_SOURCE_CODES.items():
            if not close_count(source_counts.get(name), int(np.count_nonzero(final_source == code))):
                counter_errors.append(f"final source {name} does not close to maps")
        if sum(strict_int(value) or 0 for value in source_counts.values()) != width * height:
            counter_errors.append("final update-source histogram does not cover every pixel")
        reason_names = (
        ...
        for code, name in enumerate(reason_names):
            if not close_count(unavailable_counts.get(name), int(np.count_nonzero(unavailable_reason == code))):
                counter_errors.append(f"unavailable reason {name} does not close to maps")

        selected_views = decode_uint32_rgba(required["dvp_selected_source_views"])
        direction_views = decode_uint32_rgba(required["dvp_direction_source_views"])
        if not close_float(views.get("selected_source_count_mean"), float(np.mean(uint32_popcount(selected_views)))):
            counter_errors.append("selected-source mean does not close to maps")
        if not close_float(views.get("direction_source_count_mean"), float(np.mean(uint32_popcount(direction_views)))):
            counter_errors.append("direction-source mean does not close to maps")
        endpoint_support_means = intervals.get("endpoint_support_mean")
        endpoint_support_means = (
            endpoint_support_means if isinstance(endpoint_support_means, dict) else {}
        )
        for name, signal in (
            ("left_outer", "dvp_left_outer_count"),
            ("left_inner", "dvp_left_inner_count"),
            ("right_inner", "dvp_right_inner_count"),
            ("right_outer", "dvp_right_outer_count"),
        ):
            if not close_float(
                endpoint_support_means.get(name),
                float(np.mean(required[signal], dtype=np.float64)),
            ):
                counter_errors.append(f"intervals.endpoint_support_mean.{name} does not close to maps")
        proposal_support_values: list[np.ndarray] = []
        reprojection_values: list[np.ndarray] = []
        relative_depth_values: list[np.ndarray] = []
        occluded_candidates = 0
        for proposal in range(DVP_MAX_PROPOSALS):
            generated_bit = (generated_mask & (1 << proposal)) != 0
            proposal_support_values.append(
                required[f"dvp_support_{proposal}"][generated_bit]
            )
            occluded_mask = decode_uint32_rgba(
                required[f"dvp_occluded_views_{proposal}"]
            )
            occluded_candidates += int(np.count_nonzero(generated_bit & (occluded_mask != 0)))
            reprojection = required[f"dvp_mean_reprojection_error_{proposal}"]
            relative_depth = required[f"dvp_mean_relative_depth_error_{proposal}"]
            reprojection_values.append(reprojection[generated_bit & (reprojection >= 0)])
            relative_depth_values.append(relative_depth[generated_bit & (relative_depth >= 0)])
        proposal_support_flat = np.concatenate(proposal_support_values)
        reprojection_flat = np.concatenate(reprojection_values)
        relative_depth_flat = np.concatenate(relative_depth_values)
        if not close_float(
            proposals.get("support_mean"),
            float(np.mean(proposal_support_flat, dtype=np.float64))
            if proposal_support_flat.size else None,
        ):
            counter_errors.append("proposals.support_mean does not close to maps")
        if not close_count(proposals.get("occluded_candidates"), occluded_candidates):
            counter_errors.append("proposals.occluded_candidates does not close to maps")
        cost_available = (
            np.isfinite(incumbent_cost) & (incumbent_cost >= 0)
            & np.isfinite(winner_cost) & (winner_cost >= 0)
        )
        gap_available = np.isfinite(gap) & (gap >= 0)
        displacement_available = np.isfinite(displacement) & (displacement >= 0)
        for name, data, available in (
            ("incumbent_mean", incumbent_cost, cost_available),
            ("winner_mean", winner_cost, cost_available),
            ("improvement_mean", incumbent_cost - winner_cost, cost_available),
            ("winner_runner_up_gap_mean", gap, gap_available),
        ):
            if not close_float(costs.get(name), finite_mean(data, available)):
                counter_errors.append(f"costs.{name} does not close to maps")
        if not close_count(costs.get("samples"), int(np.count_nonzero(cost_available))):
            counter_errors.append("costs.samples does not close to maps")
        if not close_float(
            geometry.get("depth_displacement_mean"),
            finite_mean(displacement, displacement_available),
        ):
            counter_errors.append("geometry.depth_displacement_mean does not close to maps")
        if not close_float(
            geometry.get("reprojection_error_mean_px"),
            float(np.mean(reprojection_flat, dtype=np.float64))
            if reprojection_flat.size else None,
        ):
            counter_errors.append("geometry.reprojection_error_mean_px does not close to maps")
        if not close_float(
            geometry.get("relative_depth_error_mean"),
            float(np.mean(relative_depth_flat, dtype=np.float64))
            if relative_depth_flat.size else None,
        ):
            counter_errors.append("geometry.relative_depth_error_mean does not close to maps")
        ...
        for name, count in errors.items():
            all_domain_errors[name] = all_domain_errors.get(name, 0) + count
        check(f"dvp_iteration_{iteration}_domains", not any(errors.values()), errors)
        check(f"dvp_iteration_{iteration}_counter_closure", not counter_errors, counter_errors)

    stage_root = frame_dir.parent.parent
    csv_path = stage_root / "instrumentation" / "dvp_iteration.csv"
    csv_errors: list[str] = []
        ...
        try:
            with csv_path.open(encoding="utf-8", newline="") as handle:
                csv_rows = [
                    ...
                    if csv_integer(row.get("image_id")) == strict_int(summary.get("image_id"))
                    and csv_integer(row.get("scale_number")) == strict_int(summary.get("scale_level"))
                ]
        except Exception as exc:
            csv_errors.append(str(exc))
    else:
        csv_errors.append("dvp_iteration.csv is missing")
    if {csv_integer(row.get("logical_iteration")) for row in csv_rows} != expected_iterations:
        csv_errors.append("CSV logical-iteration coverage is incomplete")
    for row in csv_rows:
        iteration = csv_integer(row.get("logical_iteration"))
        result = iteration_results.get(str(iteration), {})
        counts = result.get("counts") if isinstance(result, dict) else {}
        for csv_name, count_name in (
            ("attempted_pixels", "attempted_pixels"),
            ("proposal_available_pixels", "proposal_available_pixels"),
            ("native_depth_fallback_pixels", "native_depth_fallback_pixels"),
            ("proposals_generated", "generated"),
            ("proposals_tested", "tested"),
            ("proposals_finite", "finite"),
            ("proposals_accepted", "accepted_events"),
            ("final_winner_pixels", "final_winner_pixels"),
            ("final_depth_retained_pixels", "final_depth_retained_pixels"),
        ):
            if csv_integer(row.get(csv_name)) != counts.get(count_name):
                csv_errors.append(f"iteration {iteration}: {csv_name} does not close")
    check("dvp_iteration_csv", not csv_errors, {
        "path": str(csv_path), "rows": len(csv_rows), "errors": csv_errors,
    })

    trace_path = stage_root / "instrumentation" / "dvp_traces.jsonl"
    trace_available = observability.get("targeted_trace_available") is True
    trace_errors: list[str] = []
    trace_rows: list[dict[str, Any]] = []
    if trace_available:
        if not trace_path.is_file():
            trace_errors.append("targeted trace is declared available but dvp_traces.jsonl is missing")
        else:
            try:
        ...
        for index, row in enumerate(trace_rows):
            iteration = strict_int(row.get("logical_iteration"))
            trace_index = strict_int(row.get("trace_index"))
            x = strict_int(row.get("x"))
            y = strict_int(row.get("y"))
            prefix = f"row {index}"
            if (
                row.get("schema_name") != "openmvs.dmap.dvp_trace"
                or strict_int(row.get("schema_version")) != DVP_SCHEMA_VERSION
                or row.get("measurement_quality") != "exact"
                or row.get("measurement_basis") != "active_process_pixel_candidate_path"
                or row.get("family") != family
                or iteration not in expected_iterations
                or trace_index is None or trace_index < 0
                or x is None or y is None or not (0 <= x < width and 0 <= y < height)
            ):
                trace_errors.append(f"{prefix}: identity/schema fields are invalid")
                continue
            pair = (trace_index, iteration)
            if pair in observed_pairs:
                trace_errors.append(f"{prefix}: duplicate trace/iteration identity")
            observed_pairs.add(pair)
            decision = row.get("decision")
            decision = decision if isinstance(decision, dict) else {}
            proposals = row.get("proposals")
            proposals = proposals if isinstance(proposals, list) else []
            generated_count = strict_int(decision.get("generated_count"))
            tested_count = strict_int(decision.get("tested_count"))
            finite_count = strict_int(decision.get("finite_count"))
            accepted_count = strict_int(decision.get("sequential_acceptance_count"))
            accepted_proposal_mask = strict_int(decision.get("accepted_proposal_mask"))
            if generated_count != len(proposals):
                trace_errors.append(f"{prefix}: proposal count does not close")
            tested_from_rows = sum(item.get("tested") is True for item in proposals if isinstance(item, dict))
            finite_from_rows = sum(item.get("finite") is True for item in proposals if isinstance(item, dict))
            accepted_from_rows = sum(item.get("sequentially_accepted") is True for item in proposals if isinstance(item, dict))
            accepted_mask_from_rows = sum(
                (1 << proposal_index)
                for proposal_index, item in enumerate(proposals)
                if isinstance(item, dict) and item.get("sequentially_accepted") is True
            )
            if (
                tested_count != tested_from_rows
                or finite_count != finite_from_rows
                or accepted_count != accepted_from_rows
                or accepted_proposal_mask != accepted_mask_from_rows
            ):
                trace_errors.append(f"{prefix}: proposal decisions do not close")
            source_code = strict_int(decision.get("final_update_source_code"))
            if DVP_FINAL_SOURCE_CODES.get(str(decision.get("final_update_source"))) != source_code:
                trace_errors.append(f"{prefix}: final update source name/code mismatch")
            endpoint_samples = row.get("endpoint_samples")
            endpoint_samples = endpoint_samples if isinstance(endpoint_samples, dict) else {}
            endpoint_views = row.get("endpoint_sample_views")
            endpoint_views = endpoint_views if isinstance(endpoint_views, dict) else {}
            endpoint_records = row.get("endpoint_records")
            endpoint_records = endpoint_records if isinstance(endpoint_records, dict) else {}
            alpha = finite_number((observability.get("parameters") or {}).get("alpha"))
            beta = finite_number((observability.get("parameters") or {}).get("beta"))
            offsets = {
                "left_outer": -(alpha + beta) if alpha is not None and beta is not None else None,
                "left_inner": -alpha if alpha is not None else None,
                "right_inner": alpha,
                "right_outer": alpha + beta if alpha is not None and beta is not None else None,
            }
            for group, expected_offset in offsets.items():
                samples = endpoint_samples.get(group)
                records = endpoint_records.get(group)
                view_mask = strict_int(endpoint_views.get(group))
                if not isinstance(samples, list) or not isinstance(records, list) or view_mask is None:
                    trace_errors.append(f"{prefix}: endpoint group {group} is malformed")
                    continue
                expected_views = [view for view in range(32) if view_mask & (1 << view)]
                record_views = [strict_int(record.get("view")) for record in records if isinstance(record, dict)]
                record_depths = [finite_number(record.get("back_projected_depth")) for record in records if isinstance(record, dict)]
                record_offsets = [finite_number(record.get("offset_px")) for record in records if isinstance(record, dict)]
                if (
                    len(records) != len(samples)
                    or record_views != expected_views
                    or any(depth is None for depth in record_depths)
                    or not np.allclose(record_depths, samples, rtol=0, atol=max(tolerance, 2e-6))
                    or expected_offset is None
                    or any(offset is None or not np.isclose(offset, expected_offset) for offset in record_offsets)
                ):
                    trace_errors.append(f"{prefix}: endpoint group {group} does not close")

            map_fields = {
                "generated_count": "dvp_generated_count",
                "tested_count": "dvp_tested_count",
                "finite_count": "dvp_finite_count",
                "sequential_acceptance_count": "dvp_accepted_count",
                "accepted_proposal_mask": "dvp_accepted_proposal_mask",
                "final_update_source_code": "dvp_final_update_source",
            }
            for decision_name, signal in map_fields.items():
                map_values = values(signal, iteration)
                if map_values is None or strict_int(decision.get(decision_name)) != int(map_values[y, x]):
                    trace_errors.append(f"{prefix}: {decision_name} does not close to its map")
        trace_indices = {pair[0] for pair in observed_pairs}
        expected_pairs_for_traces = {
            (trace_index, iteration)
            for trace_index in trace_indices for iteration in expected_iterations
        }
        if not trace_rows or observed_pairs != expected_pairs_for_traces:
            trace_errors.append("targeted trace coverage is incomplete")
    elif trace_path.exists():
        trace_errors.append("DVP trace exists although targeted trace is unavailable")
    check("dvp_targeted_trace", not trace_errors, {
    ...
    return {
        ...
        "requested": True,
        "family": family,
        "promotion_eligible": promotion_expected,
        "expected_map_count": len(expected_pairs),
        "trace_available": trace_available,
        "trace_rows": len(trace_rows),
        "iterations": iteration_results,
        "domain_errors": all_domain_errors,
    }


def validate_dvp_visibility_maps(
    *,
    frame_dir: Path,
    manifest: dict[str, Any],
    summary: dict[str, Any],
    ...
    """Validate exact logical-iteration DVP visibility mechanics."""

    capture_value = manifest.get("dvp_visibility_capture")
    capture = capture_value if isinstance(capture_value, dict) else {}
    observability_value = summary.get("dvp_visibility_observability")
    observability = (
        observability_value if isinstance(observability_value, dict) else {}
    )
    present_entries = [
        ...
        if (
            isinstance(entry, dict)
            and str(entry.get("signal", "")).startswith("dvp_visibility_")
        )
    ]
    requested = capture.get("requested") is True
    stage_active = capture.get("stage_active") is True
    expected_reason_enum = {
        str(code): name for code, name in enumerate(DVP_VISIBILITY_REASON_NAMES)
    }
    num_views = strict_int(capture.get("num_views"))
    common_contract = (
        capture.get("schema_name") == DVP_VISIBILITY_SCHEMA_NAME
        and strict_int(capture.get("schema_version"))
        == DVP_VISIBILITY_SCHEMA_VERSION
        and observability.get("schema_name")
        == DVP_VISIBILITY_OBSERVABILITY_SCHEMA_NAME
        and strict_int(observability.get("schema_version"))
        == DVP_VISIBILITY_SCHEMA_VERSION
        and strict_int(observability.get("num_views")) == num_views
        and strict_int(capture.get("counter_record_bytes")) == 160
        and strict_int(observability.get("counter_record_bytes")) == 160
        and strict_int(capture.get("update_record_bytes")) == 76
        and strict_int(observability.get("update_record_bytes")) == 76
        and strict_int(capture.get("trace_record_bytes")) == 260
        and strict_int(observability.get("trace_record_bytes")) == 260
        and capture.get("aggregate_weight_sum_precision")
        == "uint64 device atomic"
        and observability.get("aggregate_weight_sum_precision")
        == "uint64 device atomic"
        and observability.get("reason_enum") == expected_reason_enum
        and capture.get("per_view_weights_full_frame") is False
        and capture.get("per_view_weights_targeted_trace") is True
        and capture.get("per_view_reason_targeted_trace") is True
        and capture.get("immutable_candidate_support") is True
    )
    ...
    if not requested:
        disabled_valid = (
            ...
            and capture.get("summary_available") is False
            and capture.get("maps_available") is False
            and capture.get("targeted_trace_available") is False
            and observability.get("requested") is False
            and observability.get("summary_available") is False
            and observability.get("maps_available") is False
            and observability.get("targeted_trace_available") is False
            and observability.get("iterations") == []
            and not present_entries
        )
        check("dvp_visibility_disabled_contract", disabled_valid, {
            "capture": capture,
            "observability": observability,
            "present_signals": sorted(
                str(entry.get("signal")) for entry in present_entries
            ),
        })
        return {
            "available": False,
            "requested": False,
            "unavailable_reason": "DVP visibility is inactive",
        }
    ...
    if not stage_active:
        inactive_valid = (
            common_contract
            and observability.get("requested") is True
            and observability.get("stage_active") is False
            and capture.get("summary_available") is False
            and capture.get("maps_available") is False
            and capture.get("targeted_trace_available") is False
            and observability.get("summary_available") is False
            and observability.get("maps_available") is False
            and observability.get("targeted_trace_available") is False
            and observability.get("measurement_quality") == "unavailable"
            and observability.get("iterations") == []
            and not present_entries
        )
        check("dvp_visibility_inactive_stage_contract", inactive_valid, {
            "capture": capture,
            "observability": observability,
            "present_signals": sorted(
                str(entry.get("signal")) for entry in present_entries
        ...
        return {
            ...
            "requested": True,
            "stage_active": False,
            "unavailable_reason": "visibility is unavailable in this APD stage",
        }
    ...
    contract_valid = (
        common_contract
        and 0 < (num_views or 0) <= 32
        and mode_code is not None
        and observability.get("mode") == mode
        and observability.get("enabled") is True
        and observability.get("requested") is True
        and observability.get("stage_active") is True
        and capture.get("summary_available") is True
        and observability.get("summary_available") is True
        and capture.get("maps_available") is True
        and observability.get("maps_available") is True
        and observability.get("measurement_quality") == "exact"
        and observability.get("measurement_basis")
        == (
            "same-stream Process<true> logical-iteration visibility "
            "transition and active candidate support"
        )
        and expected_iterations
    )
    check("dvp_visibility_contract", contract_valid, {
        "capture": capture,
        "observability_record_bytes": {
            "counter": observability.get("counter_record_bytes"),
            "update": observability.get("update_record_bytes"),
    ...
    expected_pairs = {
        (signal, iteration)
        for iteration in expected_iterations
        for signal in DVP_VISIBILITY_REQUIRED_SIGNALS
    }
    present_pairs = {
        (str(entry.get("signal")), strict_int(entry.get("logical_iteration")))
        for entry in present_entries
    ...
    for entry in present_entries:
        ...
        iteration = strict_int(entry.get("logical_iteration"))
        expected_dtype = (
            "uint8x4" if signal in DVP_VISIBILITY_REQUIRED_RGBA_SIGNALS
            else "uint16" if signal in DVP_VISIBILITY_REQUIRED_WORD_SIGNALS
            else "uint8"
        )
        if iteration not in expected_iterations:
            metadata_errors.append(
                f"{signal}: invalid logical_iteration {iteration!r}"
            )
        for key, expected in (
            ("dtype", expected_dtype),
            ("role", "dvp_visibility_logical_iteration_state"),
            ("stage", "iteration"),
            ("dvp_visibility_schema_name", DVP_VISIBILITY_SCHEMA_NAME),
            ("dvp_visibility_schema_version", DVP_VISIBILITY_SCHEMA_VERSION),
            ("measurement_quality", "exact"),
            (
                "measurement_basis",
            ...
            if entry.get(key) != expected:
                metadata_errors.append(
                    f"{signal}:{iteration}: {key}={entry.get(key)!r}, "
                    f"expected {expected!r}"
                )
    check(
        "dvp_visibility_map_inventory",
        present_pairs == expected_pairs and not metadata_errors,
        {
            "expected_map_count": len(expected_pairs),
            "present_map_count": len(present_pairs),
            "missing": sorted(
                f"{signal}:{iteration}"
                for signal, iteration in expected_pairs - present_pairs
            ),
            "unexpected": sorted(
                f"{signal}:{iteration}"
                for signal, iteration in present_pairs - expected_pairs
            ),
            "metadata_errors": metadata_errors,
        },
    )
    ...
    summary_iterations = observability.get("iterations")
    summary_iterations = (
        summary_iterations if isinstance(summary_iterations, list) else []
    )
    summary_by_iteration = {
        strict_int(row.get("logical_iteration")): row
        for row in summary_iterations if isinstance(row, dict)
    ...
    valid_view_mask = (
        ...
        if num_views == 32 else np.uint32((1 << int(num_views or 0)) - 1)
    )
    iteration_results: dict[str, Any] = {}
    for iteration in sorted(expected_iterations):
        required = {
            signal: values(signal, iteration)
            for signal in DVP_VISIBILITY_REQUIRED_SIGNALS
        }
        if any(data is None for data in required.values()):
            iteration_results[str(iteration)] = {"available": False}
            check(
                f"dvp_visibility_iteration_{iteration}_domains",
                False,
        ...
        masks = {
            name: decode_uint32_rgba(
                required[f"dvp_visibility_{name}_mask"]
            )
            for name in (
                "previous", "resolved", "next", "active_support",
                "restored", "rejected", "added", "removed",
                "candidate_tested", "candidate_finite",
            )
        }
        counts = {
            name: required[f"dvp_visibility_{name}_count"].astype(np.uint8)
            for name in (
                "previous_visible", "resolved_visible", "next_visible",
                "active_support", "restored", "rejected", "added", "removed",
            )
        }
        weight_sums = {
            name: required[f"dvp_visibility_{name}"].astype(np.uint16)
            for name in (
                "previous_weight_sum", "resolved_weight_sum",
                "next_weight_sum", "active_support_weight_sum", "denominator",
            )
        }
        reason_maps = {
            reason: required[f"dvp_visibility_reason_{reason}"].astype(np.uint8)
            for reason in DVP_VISIBILITY_REASON_NAMES
        }
        denominator_defined = required[
            "dvp_visibility_denominator_defined"
        ].astype(np.uint8)
        support_matches = required[
            "dvp_visibility_support_matches_resolved"
        ].astype(np.uint8)
        transition_status = required[
            "dvp_visibility_transition_status"
        ].astype(np.uint8)
        observed_mode = required["dvp_visibility_mode"].astype(np.uint8)

        errors: dict[str, int] = {}

        def record_error(name: str, condition: np.ndarray) -> None:
            errors[name] = int(np.count_nonzero(condition))
        ...
        for name in (
            ...
            mask = masks[name]
            record_error(f"{name}_mask_high_bits", (mask & ~valid_view_mask) != 0)
        count_mask_names = {
            "previous_visible": "previous",
            "resolved_visible": "resolved",
            "next_visible": "next",
        ...
        for count_name, mask_name in count_mask_names.items():
            record_error(
                f"{count_name}_count",
                counts[count_name] != uint32_popcount(masks[mask_name]),
            )
        record_error(
            "added_transition",
            masks["added"] != (masks["next"] & ~masks["previous"]),
        )
        record_error(
            "removed_transition",
            masks["removed"] != (masks["previous"] & ~masks["next"]),
        )
        record_error(
            "resolved_state",
            masks["resolved"] != (masks["previous"] | masks["restored"]),
        )
        record_error(
            "restored_previous_overlap",
            (masks["restored"] & masks["previous"]) != 0,
        )
        record_error(
            "rejected_state",
            masks["rejected"] != (valid_view_mask & ~masks["resolved"]),
        )
        record_error(
            "active_support_identity",
            masks["active_support"] != masks["resolved"],
        )
        record_error(
            "candidate_finite_subset",
            (masks["candidate_finite"] & ~masks["candidate_tested"]) != 0,
        )
        record_error("mode", observed_mode != mode_code)
        record_error("support_matches", support_matches != 1)
        record_error("transition_status", transition_status != 0)
        record_error(
            "denominator_identity",
            weight_sums["denominator"]
            != weight_sums["active_support_weight_sum"],
        )
        record_error(
            "denominator_defined",
            denominator_defined
            != (weight_sums["denominator"] > 0).astype(np.uint8),
        )
        record_error(
            "active_support_weight_identity",
            weight_sums["active_support_weight_sum"]
            != weight_sums["resolved_weight_sum"],
        )
        for prefix, count_name in (
            ("previous", "previous_visible"),
            ("resolved", "resolved_visible"),
            ("next", "next_visible"),
            ("active_support", "active_support"),
        ):
            data = weight_sums[f"{prefix}_weight_sum"]
            count = counts[count_name].astype(np.uint16)
            record_error(
                f"{prefix}_weight_domain",
                (data < count) | (data > count * np.uint16(255)),
            )
        reason_total = np.zeros((height, width), dtype=np.uint16)
        for data in reason_maps.values():
            reason_total += data.astype(np.uint16)
        record_error("reason_coverage", reason_total != num_views)
        record_error(
            "retained_reason_count",
            reason_maps["retained_previous_weight"]
            != counts["previous_visible"],
        )
        restored_reason_count = (
            reason_maps["restored_paper_2d"].astype(np.uint16)
            + reason_maps["restored_depth_gated"].astype(np.uint16)
        )
        rejected_reason_count = sum(
            (
                reason_maps[name].astype(np.uint16)
                for name in DVP_VISIBILITY_REASON_NAMES[2:10]
            ),
            np.zeros((height, width), dtype=np.uint16),
        )
        record_error(
            "restored_reason_count",
            restored_reason_count != counts["restored"],
        )
        record_error(
            "rejected_reason_count",
            rejected_reason_count != counts["rejected"],
        )
        record_error(
            "mode_disabled_reason",
            reason_maps["mode_disabled"] != 0,
        )
        inactive_restore_reason = (
            "restored_depth_gated"
            if mode == "paper_2d_restore_v1" else "restored_paper_2d"
        )
        record_error(
            "inactive_restore_reason",
            reason_maps[inactive_restore_reason] != 0,
        )
        ...
        exact_counts = {
            ...
            "previous_weight_sum": int(np.sum(
                weight_sums["previous_weight_sum"], dtype=np.uint64
            )),
            "resolved_weight_sum": int(np.sum(
                weight_sums["resolved_weight_sum"], dtype=np.uint64
            )),
            "next_weight_sum": int(np.sum(
                weight_sums["next_weight_sum"], dtype=np.uint64
            )),
            "active_support_weight_sum": int(np.sum(
                weight_sums["active_support_weight_sum"], dtype=np.uint64
            )),
            "denominator_sum": int(np.sum(
                weight_sums["denominator"], dtype=np.uint64
            )),
            "zero_denominator_pixels": int(np.count_nonzero(
                denominator_defined == 0
            )),
            "support_mismatch_pixels": int(np.count_nonzero(
                support_matches == 0
            )),
            "changed_pixels": int(np.count_nonzero(
                (masks["added"] | masks["removed"]) != 0
            )),
            "invalid_transition_pixels": int(np.count_nonzero(
                transition_status != 0
            )),
            "candidate_tested_pixels": int(np.count_nonzero(
                masks["candidate_tested"] != 0
            )),
            "candidate_finite_pixels": int(np.count_nonzero(
                masks["candidate_finite"] != 0
            )),
            "candidate_tested_count": int(np.sum(
                uint32_popcount(masks["candidate_tested"]), dtype=np.uint64
            )),
            "candidate_finite_count": int(np.sum(
                uint32_popcount(masks["candidate_finite"]), dtype=np.uint64
            )),
        }
        reason_counts = {
            reason: int(np.sum(data, dtype=np.uint64))
            for reason, data in reason_maps.items()
        }
        row = summary_by_iteration.get(iteration)
        row = row if isinstance(row, dict) else {}
        views_summary = row.get("views")
        views_summary = views_summary if isinstance(views_summary, dict) else {}
        weights_summary = row.get("weights")
        weights_summary = (
            weights_summary if isinstance(weights_summary, dict) else {}
        )
        transitions_summary = row.get("transitions")
        transitions_summary = (
            transitions_summary if isinstance(transitions_summary, dict) else {}
        )
        candidates_summary = row.get("candidates")
        candidates_summary = (
            candidates_summary if isinstance(candidates_summary, dict) else {}
        )
        counter_errors: list[str] = []
        closure_fields = (
            (row, "pixels", "pixels"),
            (views_summary, "previous_visible", "previous_visible_views"),
            (views_summary, "resolved_visible", "resolved_visible_views"),
            (views_summary, "next_visible", "next_visible_views"),
            (views_summary, "active_support", "active_support_views"),
            (views_summary, "restored", "restored_views"),
            (views_summary, "rejected", "rejected_views"),
            (views_summary, "added", "added_views"),
            (views_summary, "removed", "removed_views"),
            (weights_summary, "previous_sum", "previous_weight_sum"),
            (weights_summary, "resolved_sum", "resolved_weight_sum"),
            (weights_summary, "next_sum", "next_weight_sum"),
            (weights_summary, "active_support_sum", "active_support_weight_sum"),
            (weights_summary, "denominator_sum", "denominator_sum"),
            (weights_summary, "zero_denominator_pixels", "zero_denominator_pixels"),
            (transitions_summary, "support_mismatch_pixels", "support_mismatch_pixels"),
            (transitions_summary, "changed_pixels", "changed_pixels"),
            (transitions_summary, "invalid_transition_pixels", "invalid_transition_pixels"),
            (candidates_summary, "tested_pixels", "candidate_tested_pixels"),
            (candidates_summary, "finite_pixels", "candidate_finite_pixels"),
            (candidates_summary, "tested_count", "candidate_tested_count"),
            (candidates_summary, "finite_count", "candidate_finite_count"),
        )
        for container, field, expected_name in closure_fields:
            if strict_int(container.get(field)) != exact_counts[expected_name]:
                counter_errors.append(f"{field} does not close to maps")
        if row.get("mode") != mode:
            counter_errors.append("mode does not match the capture contract")
        if row.get("reason_counts") != reason_counts:
            counter_errors.append("reason_counts do not close to maps")
        ...
        check(
            f"dvp_visibility_iteration_{iteration}_counter_closure",
            not counter_errors,
            counter_errors,
        )

    stage_root = frame_dir.parent.parent
    csv_path = stage_root / "instrumentation" / "dvp_visibility_iteration.csv"
    csv_rows: list[dict[str, str]] = []
    csv_errors: list[str] = []
    if csv_path.is_file():
        try:
            with csv_path.open(encoding="utf-8", newline="") as handle:
                csv_rows = [
                    row for row in csv.DictReader(handle)
                    if csv_integer(row.get("image_id"))
                    == strict_int(summary.get("image_id"))
                    and csv_integer(row.get("scale_number"))
                    == strict_int(summary.get("scale_level"))
                ]
        except Exception as exc:
            csv_errors.append(str(exc))
    else:
        csv_errors.append("dvp_visibility_iteration.csv is missing")
    if {csv_integer(row.get("logical_iteration")) for row in csv_rows} != expected_iterations:
        csv_errors.append("CSV logical-iteration coverage is incomplete")
    csv_fields = (
        "pixels", "previous_visible_views", "resolved_visible_views",
        "next_visible_views", "active_support_views", "restored_views",
        "rejected_views", "added_views", "removed_views",
        "previous_weight_sum", "resolved_weight_sum", "next_weight_sum",
        "active_support_weight_sum", "denominator_sum",
        "zero_denominator_pixels", "support_mismatch_pixels",
        "changed_pixels", "invalid_transition_pixels",
        "candidate_tested_pixels", "candidate_finite_pixels",
        "candidate_tested_count", "candidate_finite_count",
    )
    for row in csv_rows:
        iteration = csv_integer(row.get("logical_iteration"))
        result = iteration_results.get(str(iteration), {})
        exact_counts = result.get("counts") if isinstance(result, dict) else {}
        for field in csv_fields:
            if csv_integer(row.get(field)) != exact_counts.get(field):
                csv_errors.append(f"iteration {iteration}: {field} does not close")
        if row.get("mode") != mode:
            csv_errors.append(f"iteration {iteration}: mode does not close")
        exact_reasons = (
            result.get("reason_counts") if isinstance(result, dict) else {}
        )
        for reason in DVP_VISIBILITY_REASON_NAMES:
            if csv_integer(row.get(f"reason_{reason}")) != exact_reasons.get(reason):
                csv_errors.append(
                    f"iteration {iteration}: reason_{reason} does not close"
                )
    check("dvp_visibility_iteration_csv", not csv_errors, {
        "path": str(csv_path), "rows": len(csv_rows), "errors": csv_errors,
    })

    trace_available = observability.get("targeted_trace_available") is True
    trace_path = stage_root / "instrumentation" / "dvp_visibility_traces.jsonl"
    trace_rows: list[dict[str, Any]] = []
    trace_errors: list[str] = []
    if trace_available:
        if not trace_path.is_file():
            trace_errors.append(
                "targeted trace is declared available but visibility trace is missing"
            )
        else:
            try:
                for line_number, line in enumerate(
                    ...
                    if not isinstance(value, dict):
                        raise ValueError(f"line {line_number} is not an object")
                    if (
                        strict_int(value.get("image_id"))
                        == strict_int(summary.get("image_id"))
                        and strict_int(value.get("scale_number"))
                        == strict_int(summary.get("scale_level"))
                    ):
                        trace_rows.append(value)
            except Exception as exc:
        ...
        for row_index, row in enumerate(trace_rows):
            ...
            y = strict_int(row.get("y"))
            if (
                row.get("schema_name") != DVP_VISIBILITY_TRACE_SCHEMA_NAME
                or strict_int(row.get("schema_version"))
                != DVP_VISIBILITY_SCHEMA_VERSION
                or row.get("measurement_quality") != "exact"
            ...
            if len(views) != num_views:
                trace_errors.append(f"{prefix}: per-view coverage is incomplete")
                continue
            previous_weights: list[int] = []
            ...
            for view_index, view in enumerate(views):
                ...
                previous = strict_int(view.get("previous_weight"))
                resolved = strict_int(view.get("resolved_weight"))
                next_weight = strict_int(view.get("next_weight"))
                active = strict_int(view.get("active_support_weight"))
                reason_code = strict_int(view.get("reason_code"))
                if (
                    strict_int(view.get("view_index")) != view_index
                    or previous is None or not 0 <= previous <= 255
                    or resolved is None or not 0 <= resolved <= 255
                    or next_weight is None or not 0 <= next_weight <= 255
                    or active is None or not 0 <= active <= 255
                    or reason_code is None
                    or not 0 <= reason_code < len(DVP_VISIBILITY_REASON_NAMES)
                    or view.get("reason")
                    != DVP_VISIBILITY_REASON_NAMES[reason_code]
                ):
                    trace_errors.append(f"{prefix}: view {view_index} is invalid")
                    continue
                previous_weights.append(previous)
                resolved_weights.append(resolved)
                next_weights.append(next_weight)
                active_weights.append(active)
                reason_codes.append(reason_code)
            if len(reason_codes) != num_views:
                continue

            def mask_from_weights(weights: list[int]) -> int:
                return sum(1 << view for view, weight in enumerate(weights) if weight)

            previous_mask = mask_from_weights(previous_weights)
            resolved_mask = mask_from_weights(resolved_weights)
            next_mask = mask_from_weights(next_weights)
            active_mask = mask_from_weights(active_weights)
            restored_mask = sum(
                1 << view for view, code in enumerate(reason_codes) if code in {10, 11}
            )
            rejected_mask = sum(
                1 << view for view, code in enumerate(reason_codes) if 2 <= code <= 9
            )
            state = row.get("state")
            ...
            expected_state = {
                ...
                "next_mask": next_mask,
                "active_support_mask": active_mask,
                "restored_mask": restored_mask,
                "rejected_mask": rejected_mask,
                "added_mask": next_mask & ~previous_mask & int(valid_view_mask),
                "removed_mask": previous_mask & ~next_mask & int(valid_view_mask),
                "transition_status_code": 0,
            }
            for field, expected in expected_state.items():
                if strict_int(state.get(field)) != expected:
                    trace_errors.append(f"{prefix}: state.{field} does not close")
            if state.get("transition_status") != "valid":
                trace_errors.append(f"{prefix}: transition status name is invalid")
            if state.get("active_support_matches_resolved") is not True:
                trace_errors.append(f"{prefix}: active support mismatch")
            weights = row.get("weights")
            weights = weights if isinstance(weights, dict) else {}
            expected_weights = {
                "previous_sum": sum(previous_weights),
                "resolved_sum": sum(resolved_weights),
                "next_sum": sum(next_weights),
                "active_support_sum": sum(active_weights),
                "denominator": sum(active_weights),
            }
            for field, expected in expected_weights.items():
                if strict_int(weights.get(field)) != expected:
                    trace_errors.append(f"{prefix}: weights.{field} does not close")
            if weights.get("denominator_defined") is not (sum(active_weights) > 0):
                trace_errors.append(f"{prefix}: denominator_defined does not close")
            observed_reasons = row.get("reason_counts")
            expected_reasons = {
                reason: reason_codes.count(code)
                for code, reason in enumerate(DVP_VISIBILITY_REASON_NAMES)
            }
            if observed_reasons != expected_reasons:
                trace_errors.append(f"{prefix}: reason_counts do not close")
            map_fields = {
                "previous_mask": "dvp_visibility_previous_mask",
                "resolved_mask": "dvp_visibility_resolved_mask",
                "next_mask": "dvp_visibility_next_mask",
                "active_support_mask": "dvp_visibility_active_support_mask",
                "restored_mask": "dvp_visibility_restored_mask",
                "rejected_mask": "dvp_visibility_rejected_mask",
                "added_mask": "dvp_visibility_added_mask",
                "removed_mask": "dvp_visibility_removed_mask",
            }
            for state_field, signal in map_fields.items():
                map_values = values(signal, int(iteration))
                if (
                    map_values is None
                    or strict_int(state.get(state_field))
                    != int(decode_uint32_rgba(map_values)[y, x])
                ):
                    trace_errors.append(
                        f"{prefix}: state.{state_field} does not close to its map"
                    )
        trace_indices = {pair[0] for pair in observed_pairs}
        ...
        if not trace_rows or observed_pairs != expected_trace_pairs:
            trace_errors.append("targeted trace coverage is incomplete")
    check("dvp_visibility_targeted_trace", not trace_errors, {
        "available": trace_available,
        "path": str(trace_path),
        "rows": len(trace_rows),
        "errors": trace_errors,
...
def validate_dvp_visible_normal_maps(
    *,
    frame_dir: Path,
    manifest: dict[str, Any],
    summary: dict[str, Any],
    ...
    """Validate exact visible-normal proposal decisions and aggregate closure."""

    capture_value = manifest.get("dvp_visible_normal_capture")
    capture = capture_value if isinstance(capture_value, dict) else {}
    observability_value = summary.get("dvp_visible_normal_observability")
    observability = (
        observability_value if isinstance(observability_value, dict) else {}
    )
    if capture_value is None and observability_value is None:
        check("dvp_visible_normal_legacy_absent_contract", True, {
            "available": False,
            "reason": "capture predates optional DVP visible-normal schema v1",
        })
        return {
            "available": False,
            "requested": False,
            "unavailable_reason": "capture predates DVP visible-normal observability",
        }
    present_entries = [
        entry for entry in entries
        if (
            isinstance(entry, dict)
            and str(entry.get("signal", "")).startswith("dvp_visible_normal_")
        )
    ]
    requested = capture.get("requested") is True
    stage_active = capture.get("stage_active") is True
    expected_evaluation_enum = {
        str(code): name
        for code, name in enumerate(DVP_VISIBLE_NORMAL_EVALUATION_REASON_NAMES)
    }
    expected_proposal_enum = {
        str(code): name
        for code, name in enumerate(DVP_VISIBLE_NORMAL_PROPOSAL_REASON_NAMES)
    }
    expected_propagation_enum = {
        str(code): name
        for code, name in enumerate(DVP_VISIBLE_NORMAL_PROPAGATION_REASON_NAMES)
    }
    common_contract = (
        capture.get("schema_name") == DVP_VISIBLE_NORMAL_SCHEMA_NAME
        and strict_int(capture.get("schema_version"))
        ...
        and strict_int(capture.get("counter_record_bytes")) == 160
        and strict_int(observability.get("counter_record_bytes")) == 160
        and strict_int(capture.get("update_record_bytes")) == 84
        and strict_int(observability.get("update_record_bytes")) == 84
        and strict_int(capture.get("trace_record_bytes")) == 344
        and strict_int(observability.get("trace_record_bytes")) == 344
        and capture.get("main_rng_schedule") == "native_unchanged"
        and capture.get("retry_rng") == "bounded_local_copy"
        and capture.get("propagation_fallback") == "native_best"
        and capture.get("refinement_fallback") == "native_proposal"
        and capture.get("actual_normal_vectors_full_frame") is False
        and capture.get("actual_normal_vectors_targeted_trace") is True
        and observability.get("evaluation_reason_enum")
        == expected_evaluation_enum
        and observability.get("proposal_reason_enum") == expected_proposal_enum
        and observability.get("propagation_reason_enum")
        == expected_propagation_enum
    )
    ...
    if not requested:
        ...
        check("dvp_visible_normal_disabled_contract", disabled_valid, {
            "capture": capture,
            "observability": observability,
            "present_signals": sorted(
                str(entry.get("signal")) for entry in present_entries
        ...
        return {
            "available": False,
            "requested": False,
            "unavailable_reason": "DVP visible normal is inactive",
        }
    ...
    if not stage_active:
        ...
        check("dvp_visible_normal_inactive_stage_contract", inactive_valid, {
            "capture": capture,
            "observability": observability,
            "present_signals": sorted(
                str(entry.get("signal")) for entry in present_entries
        ...
        return {
            ...
            "requested": True,
            "stage_active": False,
            "unavailable_reason": "visible normal is unavailable in this APD stage",
        }
    ...
    check("dvp_visible_normal_contract", contract_valid, {
        "capture": capture,
        "observability_record_bytes": {
            "counter": observability.get("counter_record_bytes"),
            "update": observability.get("update_record_bytes"),
    ...
    expected_pairs = {
        (signal, iteration)
        for iteration in expected_iterations
        for signal in DVP_VISIBLE_NORMAL_REQUIRED_SIGNALS
    }
    present_pairs = {
        (str(entry.get("signal")), strict_int(entry.get("logical_iteration")))
        for entry in present_entries
    ...
    for entry in present_entries:
        ...
        iteration = strict_int(entry.get("logical_iteration"))
        expected_dtype = (
            "float32" if signal in DVP_VISIBLE_NORMAL_REQUIRED_FLOAT_SIGNALS
            else "uint16" if signal in DVP_VISIBLE_NORMAL_REQUIRED_WORD_SIGNALS
            else "uint8x4" if signal in DVP_VISIBLE_NORMAL_REQUIRED_RGBA_SIGNALS
            else "uint8"
        )
        if iteration not in expected_iterations:
            metadata_errors.append(
        ...
        for key, expected in (
            ...
            ("stage", "iteration"),
            ("dvp_visible_normal_schema_name", DVP_VISIBLE_NORMAL_SCHEMA_NAME),
            (
                "dvp_visible_normal_schema_version",
                DVP_VISIBLE_NORMAL_SCHEMA_VERSION,
            ),
            ...
            (
                "measurement_basis",
                "active_process_pixel_visible_normal_decision",
            ),
        ):
            if entry.get(key) != expected:
                metadata_errors.append(
    ...
    check(
        "dvp_visible_normal_map_inventory",
        present_pairs == expected_pairs and not metadata_errors,
        {
            "expected_map_count": len(expected_pairs),
            "present_map_count": len(present_pairs),
    ...
    observability_iterations = observability.get("iterations")
    observability_rows = (
        observability_iterations if isinstance(observability_iterations, list) else []
    )
    observability_by_iteration = {
        strict_int(row.get("logical_iteration")): row
        for row in observability_rows if isinstance(row, dict)
    }
    iteration_results: list[dict[str, Any]] = []
    for iteration in sorted(expected_iterations):
        required = {
            signal: logical_maps.get((signal, iteration), (None, None, None))[2]
            for signal in DVP_VISIBLE_NORMAL_REQUIRED_SIGNALS
        }
        missing = sorted(signal for signal, values in required.items() if values is None)
        if missing:
            check(
                f"dvp_visible_normal_iteration_{iteration}_domains",
                False,
                {"missing": missing},
            )
            iteration_results.append({"logical_iteration": iteration, "missing": missing})
            continue

        byte = {
            signal: np.asarray(required[signal], dtype=np.uint8)
            for signal in DVP_VISIBLE_NORMAL_REQUIRED_BYTE_SIGNALS
        }
        word = {
            signal: np.asarray(required[signal], dtype=np.uint16)
            for signal in DVP_VISIBLE_NORMAL_REQUIRED_WORD_SIGNALS
        }
        selected_views = decode_uint32_rgba(
            required["dvp_visible_normal_selected_source_views"]
        )
        active = byte["dvp_visible_normal_mode"] == mode_code
        domain_errors: list[str] = []
        ...
        for name in binary_names:
            values = byte[f"dvp_visible_normal_{name}"]
            if np.any(values > 1):
                domain_errors.append(f"{name} is outside {{0,1}}")
        if np.any(byte["dvp_visible_normal_support_count"] > 32):
            domain_errors.append("support_count exceeds 32")
        if np.any(byte["dvp_visible_normal_direction_count"] > 33):
            domain_errors.append("direction_count exceeds 33")
        if np.any(
            byte["dvp_visible_normal_support_count"]
            > uint32_popcount(selected_views)
        ):
            domain_errors.append("support_count exceeds selected-source popcount")
        if np.any(
            active
            & (
                byte["dvp_visible_normal_direction_count"]
                != byte["dvp_visible_normal_support_count"] + 1
            )
        ):
            domain_errors.append("direction_count does not equal support_count+1")

        tested = byte["dvp_visible_normal_propagation_tested_mask"]
        valid = byte["dvp_visible_normal_propagation_valid_mask"]
        feasible = byte["dvp_visible_normal_propagation_feasible_mask"]
        rejected = byte["dvp_visible_normal_propagation_rejected_mask"]
        if np.any(valid & ~tested):
            domain_errors.append("propagation valid mask is not a tested subset")
        if np.any(feasible & ~valid):
            domain_errors.append("propagation feasible mask is not a valid subset")
        if np.any(rejected & ~valid):
            domain_errors.append("propagation rejected mask is not a valid subset")
        if np.any(feasible & rejected):
            domain_errors.append("propagation feasible and rejected masks overlap")

        refinement_masks = {
            name: byte[f"dvp_visible_normal_refinement_{name}_mask"]
            for name in (
                "native_tested",
                "native_valid",
                "native_feasible",
        ...
        if any(np.any(values > 3) for values in refinement_masks.values()):
            domain_errors.append("refinement mask uses bits outside two proposals")
        if np.any(refinement_masks["native_valid"] & ~refinement_masks["native_tested"]):
            domain_errors.append("refinement native-valid mask is not a tested subset")
        if np.any(refinement_masks["native_feasible"] & ~refinement_masks["native_valid"]):
            domain_errors.append("refinement native-feasible mask is not a valid subset")
        if np.any(refinement_masks["retry_success"] & refinement_masks["exhaustion_fallback"]):
            domain_errors.append("refinement retry-success and exhaustion masks overlap")
        if np.any(refinement_masks["applied_retry"] & ~refinement_masks["retry_success"]):
            domain_errors.append("refinement applied-retry is not a retry-success subset")

        for proposal in range(2):
            retries = word[f"dvp_visible_normal_retries_tested_{proposal}"]
            selected_retry = word[f"dvp_visible_normal_selected_retry_{proposal}"]
            if np.any(retries > 1000):
                domain_errors.append(f"proposal {proposal} retries exceed 1000")
            invalid_retry = (selected_retry != 65535) & (selected_retry >= retries)
            if np.any(invalid_retry):
                domain_errors.append(
                    f"proposal {proposal} selected retry is outside tested retries"
                )
            native_reason = byte[f"dvp_visible_normal_native_reason_{proposal}"]
            proposal_reason = byte[f"dvp_visible_normal_proposal_reason_{proposal}"]
            if np.any(native_reason >= len(DVP_VISIBLE_NORMAL_EVALUATION_REASON_NAMES)):
                domain_errors.append(f"proposal {proposal} native reason is invalid")
            if np.any(proposal_reason >= len(DVP_VISIBLE_NORMAL_PROPOSAL_REASON_NAMES)):
                domain_errors.append(f"proposal {proposal} proposal reason is invalid")

        current_valid = byte["dvp_visible_normal_current_valid"]
        current_feasible = byte["dvp_visible_normal_current_feasible"]
        current_reason = byte["dvp_visible_normal_current_reason"]
        if np.any(current_feasible & ~current_valid):
            domain_errors.append("current feasible flag is set for an invalid normal")
        if np.any(active & current_feasible & (current_reason != 0)):
            domain_errors.append("feasible current normal does not have valid reason")
        if np.any(active & current_valid & ~current_feasible & (current_reason != 5)):
            domain_errors.append("rejected current normal lacks hemisphere reason")

        row = observability_by_iteration.get(iteration)
        closure_errors: list[str] = []
        if not isinstance(row, dict):
            closure_errors.append("missing observability iteration row")
        else:
            current = row.get("current") if isinstance(row.get("current"), dict) else {}
            propagation = (
                row.get("propagation")
                if isinstance(row.get("propagation"), dict) else {}
            )
            refinement = (
                row.get("refinement")
                if isinstance(row.get("refinement"), dict) else {}
            )
            expected_counts = {
                "pixels": int(np.count_nonzero(active)),
                "selected_support_views": int(
                    np.sum(byte["dvp_visible_normal_support_count"][active], dtype=np.uint64)
                ),
                "current.valid": int(np.count_nonzero(active & (current_valid != 0))),
                "current.feasible": int(np.count_nonzero(active & (current_feasible != 0))),
                "current.rejected": int(np.count_nonzero(active & (current_valid != 0) & (current_feasible == 0))),
                "current.invalid": int(np.count_nonzero(active & (current_valid == 0))),
                "propagation.tested": int(np.sum(uint32_popcount(tested[active], bits=8), dtype=np.uint64)),
                "propagation.valid": int(np.sum(uint32_popcount(valid[active], bits=8), dtype=np.uint64)),
                "propagation.feasible": int(np.sum(uint32_popcount(feasible[active], bits=8), dtype=np.uint64)),
                "propagation.rejected": int(np.sum(uint32_popcount(rejected[active], bits=8), dtype=np.uint64)),
                "refinement.native_tested": int(np.sum(uint32_popcount(refinement_masks["native_tested"][active], bits=2), dtype=np.uint64)),
                "refinement.native_valid": int(np.sum(uint32_popcount(refinement_masks["native_valid"][active], bits=2), dtype=np.uint64)),
                "refinement.native_feasible": int(np.sum(uint32_popcount(refinement_masks["native_feasible"][active], bits=2), dtype=np.uint64)),
                "refinement.native_rejected": int(np.sum(uint32_popcount((refinement_masks["native_tested"] & ~refinement_masks["native_feasible"])[active], bits=2), dtype=np.uint64)),
                "refinement.retries_tested": int(sum(np.sum(word[f"dvp_visible_normal_retries_tested_{proposal}"][active], dtype=np.uint64) for proposal in range(2))),
                "refinement.retry_success": int(np.sum(uint32_popcount(refinement_masks["retry_success"][active], bits=2), dtype=np.uint64)),
                "refinement.exhaustion": int(np.sum(uint32_popcount(refinement_masks["exhaustion_fallback"][active], bits=2), dtype=np.uint64)),
                "refinement.fallback": int(np.sum(uint32_popcount(refinement_masks["exhaustion_fallback"][active], bits=2), dtype=np.uint64)),
                "refinement.applied_retry": int(np.sum(uint32_popcount(refinement_masks["applied_retry"][active], bits=2), dtype=np.uint64)),
                "refinement.accepted": int(np.sum(uint32_popcount(refinement_masks["accepted"][active], bits=2), dtype=np.uint64)),
            }
            observed_counts = {
                "pixels": strict_int(row.get("pixels")),
                "selected_support_views": strict_int(row.get("selected_support_views")),
                "current.valid": strict_int(current.get("valid")),
                "current.feasible": strict_int(current.get("feasible")),
                "current.rejected": strict_int(current.get("rejected")),
                "current.invalid": strict_int(current.get("invalid")),
                "propagation.tested": strict_int(propagation.get("tested")),
                "propagation.valid": strict_int(propagation.get("valid")),
                "propagation.feasible": strict_int(propagation.get("feasible")),
                "propagation.rejected": strict_int(propagation.get("rejected")),
                "refinement.native_tested": strict_int(refinement.get("native_tested")),
                "refinement.native_valid": strict_int(refinement.get("native_valid")),
                "refinement.native_feasible": strict_int(refinement.get("native_feasible")),
                "refinement.native_rejected": strict_int(refinement.get("native_rejected")),
                "refinement.retries_tested": strict_int(refinement.get("retries_tested")),
                "refinement.retry_success": strict_int(refinement.get("retry_success")),
                "refinement.exhaustion": strict_int(refinement.get("exhaustion")),
                "refinement.fallback": strict_int(refinement.get("fallback")),
                "refinement.applied_retry": strict_int(refinement.get("applied_retry")),
                "refinement.accepted": strict_int(refinement.get("accepted")),
            }
            for name, expected in expected_counts.items():
                if observed_counts.get(name) != expected:
                    closure_errors.append(
                        f"{name}: observed {observed_counts.get(name)!r}, expected {expected}"
                    )

        valid_iteration = not domain_errors and not closure_errors
        check(
            f"dvp_visible_normal_iteration_{iteration}_domains",
            not domain_errors,
            {"errors": domain_errors},
        ...
        check(
            f"dvp_visible_normal_iteration_{iteration}_counter_closure",
            not closure_errors,
            {"errors": closure_errors},
        )
        iteration_results.append({
            "logical_iteration": iteration,
            "valid": valid_iteration,
            "active_pixels": int(np.count_nonzero(active)),
            "domain_errors": domain_errors,
            "closure_errors": closure_errors,
        })

    trace_available = capture.get("targeted_trace_available") is True
    trace_path = frame_dir.parent.parent / "instrumentation" / "dvp_visible_normal_traces.jsonl"
    trace_errors: list[str] = []
    trace_rows: list[dict[str, Any]] = []
    if trace_available:
        if not trace_path.is_file():
            trace_errors.append("targeted trace is declared available but file is missing")
        else:
            for line_number, line in enumerate(
                trace_path.read_text(encoding="utf-8").splitlines(), start=1
            ):
                ...
                try:
                    ...
                    trace_errors.append(f"line {line_number}: {exc}")
                    continue
                if not isinstance(value, dict):
                    trace_errors.append(f"line {line_number}: row is not an object")
                    continue
                if (
                    strict_int(value.get("image_id"))
                    != strict_int(summary.get("image_id"))
                    or strict_int(value.get("scale_number"))
                    != strict_int(summary.get("scale_level"))
                ):
                    continue
                if (
                    value.get("schema_name") != DVP_VISIBLE_NORMAL_TRACE_SCHEMA_NAME
                    or strict_int(value.get("schema_version"))
                    != DVP_VISIBLE_NORMAL_SCHEMA_VERSION
                    or value.get("measurement_quality") != "exact"
                    or value.get("measurement_basis")
                    != "same_stream_Process_true_logical_iteration_proposals"
                ):
                    trace_errors.append(f"line {line_number}: invalid trace contract")
                if str(value.get("mode") or "") != mode:
                    trace_errors.append(f"line {line_number}: mode mismatch")
                logical_iteration = strict_int(value.get("logical_iteration"))
                if logical_iteration not in expected_iterations:
                    trace_errors.append(f"line {line_number}: invalid logical iteration")
                for field in (
                    "current",
                    ...
                    if field not in value:
                        trace_errors.append(f"line {line_number}: missing {field}")
                trace_rows.append(value)
            observed_pairs = {
                (strict_int(row.get("trace_index")), strict_int(row.get("logical_iteration")))
                for row in trace_rows
            }
            trace_indices = {trace_index for trace_index, _iteration in observed_pairs}
            expected_trace_pairs = {
                (trace_index, iteration)
                for trace_index in trace_indices for iteration in expected_iterations
            }
            if not trace_rows or observed_pairs != expected_trace_pairs:
                trace_errors.append("targeted trace coverage is incomplete")
    check("dvp_visible_normal_targeted_trace", not trace_errors, {
        "available": trace_available,
        "path": str(trace_path),
        "rows": len(trace_rows),
        "errors": trace_errors,
...
def validate(arguments: Arguments) -> dict[str, Any]:
    ...
    check("schema_version", schema_version in {2, 3, 4, 5} and summary_schema_version == schema_version, {
    ...
    dvp_capture = manifest.get("dvp_capture") or {}
    dvp_available = bool(
        schema_version == 5
        and isinstance(dvp_capture, dict)
        and dvp_capture.get("requested") is True
        and dvp_capture.get("maps_available") is True
    )
    expected_candidate_mode = (
        "exact_apd_and_dvp_active_objectives_full_frame"
        if apd_available and dvp_available
        else "exact_apd_working_objective_full_frame" if apd_available
        else "exact_dvp_active_objective_full_frame" if dvp_available
        else "exact_production_hot_kernel_full_frame"
        if schema_version in {4, 5} and exact_available
        else "unavailable_post_pass_snapshot"
    )
    expected_gap_mode = (
        "exact_apd_and_dvp_stage_specific_full_frame"
        if apd_available and dvp_available
        else "exact_apd_working_winner_runner_up_full_frame" if apd_available
        else "exact_dvp_incumbent_proposal_runner_up_full_frame" if dvp_available
    ...
    for entry in entries:
        ...
        logical_iteration = (
            ...
            if schema_version in {3, 4, 5} and "logical_iteration" in entry
            else None
        )
    ...
    } if schema_version in {3, 4, 5} else shape_errors)
    ...
    dvp_validation: dict[str, Any] | None = None
    dvp_visibility_validation: dict[str, Any] | None = None
    dvp_visible_normal_validation: dict[str, Any] | None = None
    if schema_version in {3, 4, 5}:
    ...
    if schema_version in {4, 5}:
        ...
        if schema_version == 5:
            dvp_validation = validate_dvp_maps(
                frame_dir=frame_dir,
                manifest=manifest,
                summary=summary,
                entries=entries,
            ...
            dvp_visibility_validation = validate_dvp_visibility_maps(
                frame_dir=frame_dir,
                manifest=manifest,
                summary=summary,
                entries=entries,
            ...
            dvp_visible_normal_validation = validate_dvp_visible_normal_maps(
                frame_dir=frame_dir,
                manifest=manifest,
                summary=summary,
                entries=entries,
    ...
    if filtering_path.is_file():
        ...
        ratio_required = schema_version in {4, 5} or keep_ratio is not None or summary_keep_ratio is not None
        expected_keep_ratio = (
    ...
    if dvp_validation is not None:
        result["dvp_validation"] = dvp_validation
    if dvp_visibility_validation is not None:
        result["dvp_visibility_validation"] = dvp_visibility_validation
    if dvp_visible_normal_validation is not None:
        result["dvp_visible_normal_validation"] = dvp_visible_normal_validation
    result = machine_readable_result(result)
    if arguments.output is not None:
        output = arguments.output.expanduser().resolve()
        output.parent.mkdir(parents=True, exist_ok=True)
