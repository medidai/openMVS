...
DVP_LOSSLESS_RASTER_PIXEL_SIGNALS = {
    ...
    *{
        ...
        for field in ("source_view", "support", "support_views", "occluded_views")
    },
}
...
BUILTIN_CATEGORY_LEGENDS = {
    ...
    "apd_update_source": {
        ...
        "12": "DVP epipolar proposal",
    },
    "dvp_family": {
        "0": "disabled", "1": "historical global", "2": "gated global",
        "3": "historical midpoint", "4": "Eq. 11 disjoint intervals",
    },
    "dvp_unavailable_reason": {
        "0": "available", "1": "family disabled", "2": "geometry unavailable",
        "3": "invalid reference depth", "4": "no selected source view",
        "5": "invalid epipolar direction", "6": "insufficient endpoint support",
        "7": "invalid interval order", "8": "no finite global candidate",
        "9": "global reprojection gate", "10": "global support gate",
        "11": "global occlusion rejection",
    },
    "dvp_final_update_source": {
        "0": "none", "1": "initialization", "2": "propagation",
        "3": "depth refinement", "4": "normal refinement",
    ...
    "dvp_winner_ordinal": {"0": "proposal 0", "1": "proposal 1", "255": "unavailable"},
    "dvp_tested_proposal_mask": {"0": "none", "1": "proposal 0", "2": "proposal 1", "3": "both"},
    "dvp_finite_proposal_mask": {"0": "none", "1": "proposal 0", "2": "proposal 1", "3": "both"},
    "dvp_accepted_proposal_mask": {"0": "none", "1": "proposal 0", "2": "proposal 1", "3": "both"},
    **{
        signal: {"0": "false", "1": "true"}
        for signal in (
            "dvp_accepted", "dvp_final_winner", "dvp_final_depth_retained",
            "dvp_native_depth_fallback", "dvp_left_interval_valid",
            "dvp_right_interval_valid",
        )
    },
}
CRITICAL_PIXEL_SIGNALS = {
    ...
    "dvp_final_depth_retained",
    "dvp_native_depth_fallback",
    "dvp_proposal_depth_0",
    "dvp_proposal_depth_1",
    "dvp_candidate_cost_0",
    "dvp_candidate_cost_1",
    "dvp_support_0",
    "dvp_support_1",
...
MAX_DVP_TRACE_REPORT_ROWS = 4096
MAX_DVP_TRACE_REPORT_BYTES = 128 * 1024 * 1024
MAX_DVP_ITERATION_CSV_BYTES = 16 * 1024 * 1024
...
INVESTIGATION_GUIDE = {
    ...
    "recipes": [
        ...
        {
            "key": "dvp",
            "title": "Debug DVP epipolar depth proposals",
            "summary": "Follow endpoint evidence through interval construction, proposal scoring, sequential acceptance, and the final retained depth.",
            "steps": [
                "Use Same logical iteration and select the DVP mechanics preset.",
                "Start with proposal availability, native fallback, unavailable reason, and the independent left/right interval-valid maps.",
                "Inspect endpoint support per side; Eq. 11 requires at least mu samples in both the outer and inner groups for that side, while the opposite side may remain unavailable.",
                "Compare each proposal depth and side/offset with its candidate cost, support-view mask, occlusion mask, and global-gate errors.",
                "Read tested, finite, and accepted proposal masks before interpreting winner ordinal, winner/runner-up costs, and the gap.",
                "Distinguish a full DVP plane winner from a retained DVP depth followed by a normal-only refinement.",
                "Use selected-pixel traces to inspect exact endpoint view identities, offsets, back-projected depths, disjoint intervals, and the sequential decision.",
                "Return to same-binary Process<false> quality runs for annotations, repeatability, precision, coverage, and runtime.",
            ],
            "look_for": [
                "Intervals supported by the configured mu views without merging a disjoint left/right gap.",
                "Accepted proposals with a positive incumbent-to-winner gain and stable retained depth.",
                "Global proposals passing reprojection, support, relative-depth, and occlusion gates rather than relying on one weak view.",
                "Changes in proposal coverage that improve annotation residuals without adding boundary or cross-surface noise.",
            ],
            "cautions": [
                "Historical global and midpoint families are compatibility arms and are not promotion eligible.",
                "For Eq. 11, signed offset identifies the left or right interval; endpoint records carry the actual alpha/beta pixel offsets.",
                "Final full-hypothesis winner and final depth retained are intentionally different fields because later normal-only refinement can preserve depth while replacing the plane.",
                "Process<true> mechanics output is diagnostic-only; quality conclusions require a fresh Process<false> control.",
            ],
            "action_label": "Set up DVP mechanics inspection",
            "action": {
                "alignment": "same",
                "map_preset": "dvp",
                "target": "mechanics-heading",
                "signals": [
                    "reference_rgb", "dvp_unavailable_reason",
                    "dvp_native_depth_fallback", "dvp_left_interval_valid",
                    "dvp_right_interval_valid", "dvp_left_outer_count",
                    "dvp_left_inner_count", "dvp_right_inner_count",
                    "dvp_right_outer_count", "dvp_left_interval_minimum",
                    "dvp_left_interval_maximum", "dvp_right_interval_minimum",
                    "dvp_right_interval_maximum", "dvp_proposal_depth_0",
                    "dvp_proposal_depth_1", "dvp_candidate_cost_0",
                    "dvp_candidate_cost_1", "dvp_support_0", "dvp_support_1",
                    "dvp_accepted_proposal_mask", "dvp_winner_ordinal",
                    "dvp_winner_runner_up_gap", "dvp_final_update_source",
                    "dvp_final_winner", "dvp_final_depth_retained",
...
def investigation_guide_for_registry(
    ...
    *,
    ...
    recipe_mechanisms = {
        ...
        "dvp": {"geometry", "cost", "candidate_update", "view_selection"},
    ...
    if not any(signal_id.startswith("dvp_") for signal_id in descriptors):
        guide["recipes"] = [
            recipe for recipe in guide["recipes"] if recipe.get("key") != "dvp"
        ]
...
def _trace_stage_capture_evidence(
    *,
    ...
    for summary_path in summary_paths:
        ...
        summary_schema_version = _trace_integer(summary.get("schema_version"))
        manifest_schema_version = _trace_integer(manifest.get("schema_version"))
        ...
        frame_valid = (
            ...
            and summary_schema_version in EXACT_MAP_SUPPORTED_SCHEMA_VERSIONS
            and manifest.get("schema_name") == "openmvs.dmap.map_manifest"
            and manifest_schema_version == summary_schema_version
            ...
            and _trace_integer(manifest_binding.get("schema_version"))
            == manifest_schema_version
            and manifest_binding.get("complete") is True
            and summary_binding.get("path") == "summary.json"
            and _trace_integer(summary_binding.get("schema_version"))
        ...
        if frame_valid:
            ...
            errors.append(
                f"{summary_path.relative_to(instrumentation_root)} lacks valid "
                "schema-v4/v5 exact completion evidence"
            )
    ...
    if matching_frames == 0:
        errors.append("no schema-v4/v5 map frame matches the requested image")
    return {
        "valid": (
            command_valid and matching_frames > 0
            and valid_frames == matching_frames and not resource_plan_errors
        ),
        "measurement_basis": "schema_v4_v5_exact_map_completion",
        "estimation_stage": estimation_stage,
        "geometric_iteration": geometric_iteration,
            ...
            try:
                ...
                if capability_name not in capabilities:
                    ...
                    if (
                        ...
                        or _model_integer(metadata.get("schema_version")) not in {4, 5}
                    ):
                        record["contract_valid"] = False
                        record["capability_status"] = "invalid"
                        record["unavailable_reason"] = (
...
def dvp_observability_records(
    ...
    """Load bounded schema-v1 DVP aggregate and targeted-trace evidence."""
    ...
    for run_scene in run_scenes:
        ...
        common = {
            ...
            "repeat": identity[1], "scene_id": identity[2],
            "estimation_stage": identity[3], "geometric_iteration": identity[4],
            "capture_profile": str(getattr(run_scene, "capture_profile", "summary")),
        }
        metadata: dict[str, Any] = {}
        ...
        parameters = metadata.get("cuda_patchmatch_parameters")
        contract = (
            parameters.get("dvp_epipolar")
            if isinstance(parameters, dict) else None
        )
        if not isinstance(contract, dict):
            contract = {}
        dvp_enabled = contract.get("enabled") is True
        if contract:
            contracts.append({**common, **json_value(contract)})

        iteration_path = root / "instrumentation" / "dvp_iteration.csv"
        if iteration_path.is_file() and not iteration_path.is_symlink():
            try:
                size = iteration_path.stat().st_size
                if size <= 0 or size > MAX_DVP_ITERATION_CSV_BYTES:
                    raise ValueError(
                        f"DVP iteration CSV size {size} is outside the report limit"
                    )
                with iteration_path.open(newline="", encoding="utf-8") as handle:
                    for row in csv.DictReader(handle):
                        parsed: dict[str, Any] = {}
                        ...
                        normalized = canonical_pyramid_record({**parsed, **common})
                        normalized["source_path"] = relative_path(iteration_path, output_dir)
                        iteration_rows.append(json_value(normalized))
            except (OSError, UnicodeError, ValueError, csv.Error) as exc:
                trace_sources.append({
                    **common, "kind": "iteration_summary", "available": False,
            ...
            trace_sources.append({
                **common, "kind": "iteration_summary", "available": False,
                "path": relative_path(iteration_path, output_dir),
                "reason": "DVP is enabled but dvp_iteration.csv is unavailable",
            })

        trace_path = root / "instrumentation" / "dvp_traces.jsonl"
        source = {
            **common, "kind": "targeted_trace", "available": False,
            "path": relative_path(trace_path, output_dir), "rows_included": 0,
            "truncated": False, "reason": "not requested",
            ...
            try:
                with trace_path.open(encoding="utf-8") as handle:
                    for line_number, line in enumerate(handle, start=1):
                        ...
                        if line_bytes > MAX_TRACE_JSONL_LINE_BYTES:
                            raise ValueError(
                                f"DVP trace line {line_number} exceeds the line-size limit"
                            )
                        if (
                            len(trace_rows) - source_row_start >= MAX_DVP_TRACE_REPORT_ROWS
                            or consumed_bytes > MAX_DVP_TRACE_REPORT_BYTES
                        ):
                            source["truncated"] = True
                            source["reason"] = (
                                "report embedding limit reached; the complete JSONL remains "
                        ...
                        if not isinstance(row, dict):
                            raise ValueError(
                                f"DVP trace line {line_number} is not a JSON object"
                            )
                        if (
                            ...
                            raise ValueError(
                                f"DVP trace line {line_number} has an unsupported schema"
                            )
                        normalized = canonical_pyramid_record({**row, **common})
                        normalized["source_path"] = relative_path(trace_path, output_dir)
                        trace_rows.append(json_value(normalized))
                ...
                source["available"] = source["rows_included"] > 0
                if not source["available"] and not source["reason"]:
                    source["reason"] = "DVP trace JSONL contains no records"
            except (json.JSONDecodeError, OSError, UnicodeError, ValueError) as exc:
                del trace_rows[source_row_start:]
                source["rows_included"] = 0
                source["reason"] = str(exc)
        elif dvp_enabled:
            source["reason"] = "DVP targeted trace was not requested or was not admitted"
        if dvp_enabled or trace_path.exists():
            trace_sources.append(source)
    return iteration_rows, trace_rows, trace_sources, contracts
...
def build_report_model(
    *,
    ...
    (
        ...
        dvp_trace_sources,
        dvp_contracts,
    ) = dvp_observability_records(run_scenes, output_dir)
    ...
    model = {
        ...
        "mechanics": {
            ...
            "dvp_contracts": dvp_contracts,
            "dvp_iterations": dvp_iterations,
            "dvp_traces": dvp_traces,
            "dvp_trace_sources": dvp_trace_sources,
            ...
            "availability": {
                ...
                "dvp_iteration_mechanics": bool(dvp_iterations),
                "dvp_targeted_traces": bool(dvp_traces),
...
def validate_report_model(model: dict[str, Any], output_dir: Path) -> dict[str, Any]:
    ...
    dvp_errors: list[str] = []
    dvp_families = {
        "historical_global_v0", "global_search_gated_v1",
        "historical_midpoint_v1", "dvp_eq11_interval_v1",
    }
    for index, row in enumerate(mechanics.get("dvp_contracts") or []):
        if not isinstance(row, dict):
            dvp_errors.append(f"contracts[{index}] is not an object")
            continue
        if row.get("family") not in dvp_families and row.get("enabled") is True:
            dvp_errors.append(f"contracts[{index}] has an invalid active family")
        provenance = row.get("provenance") or {}
        if row.get("enabled") is True and (
            provenance.get("official_repository_commit")
            != "5b2b31c02941cfa28808b4e37f06a87e8dc0f2d9"
            or provenance.get("colleague_donor_commit")
            != "c6f0f2c0fe2e31ec68c54cac681cc3b1665d2f7b"
        ):
            dvp_errors.append(f"contracts[{index}] does not match the pinned sources")
    dvp_iteration_identities: set[tuple[Any, ...]] = set()
    for index, row in enumerate(mechanics.get("dvp_iterations") or []):
        if not isinstance(row, dict):
            dvp_errors.append(f"iterations[{index}] is not an object")
            continue
        identity = (
            row.get("run"), _identity_integer(row.get("repeat")),
            row.get("scene_id"), row.get("estimation_stage"),
            _model_integer(row.get("geometric_iteration")),
            _model_integer(row.get("pyramid_level")),
            _model_integer(row.get("logical_iteration")),
        )
        if identity in dvp_iteration_identities:
            dvp_errors.append(f"iterations[{index}] duplicates identity {identity}")
        dvp_iteration_identities.add(identity)
        counts = [
            _model_integer(row.get(name))
            for name in (
                "proposals_generated", "proposals_tested",
                "proposals_finite", "proposals_accepted",
            )
        ]
        if any(value is None or value < 0 for value in counts):
            dvp_errors.append(f"iterations[{index}] has invalid proposal counts")
        elif not (counts[0] >= counts[1] >= counts[2] >= counts[3]):
            dvp_errors.append(f"iterations[{index}] proposal counts do not close")
        attempted = _model_integer(row.get("attempted_pixels"))
        available = _model_integer(row.get("proposal_available_pixels"))
        fallback = _model_integer(row.get("native_depth_fallback_pixels"))
        if (
            attempted is None or available is None or fallback is None
            or attempted < 0 or available < 0 or fallback < 0
            or available > attempted
        ):
            dvp_errors.append(f"iterations[{index}] has invalid availability/fallback counts")
        elif row.get("family") == "dvp_eq11_interval_v1":
            if available + fallback != attempted:
                dvp_errors.append(
                    f"iterations[{index}] Eq. 11 availability/fallback does not close"
                )
        elif fallback != 0:
            dvp_errors.append(
                f"iterations[{index}] non-Eq. 11 family records native fallback"
            )
    dvp_trace_identities: set[tuple[Any, ...]] = set()
    for index, row in enumerate(mechanics.get("dvp_traces") or []):
        if not isinstance(row, dict):
            dvp_errors.append(f"traces[{index}] is not an object")
            continue
        if (
            row.get("schema_name") != "openmvs.dmap.dvp_trace"
            or row.get("schema_version") != 1
        ):
            dvp_errors.append(f"traces[{index}] has an unsupported schema")
        identity = (
            row.get("run"), _identity_integer(row.get("repeat")),
            row.get("scene_id"), _model_integer(row.get("image_id")),
            _model_integer(row.get("scale_number")),
            _model_integer(row.get("logical_iteration")),
            _model_integer(row.get("trace_index")),
        )
        if identity in dvp_trace_identities:
            dvp_errors.append(f"traces[{index}] duplicates identity {identity}")
        dvp_trace_identities.add(identity)
    add(
        "dvp_observability",
        not dvp_errors,
        {
            "contracts": len(mechanics.get("dvp_contracts") or []),
            "iterations": len(mechanics.get("dvp_iterations") or []),
            "traces": len(mechanics.get("dvp_traces") or []),
            "errors": dvp_errors,
        },
    )
    ...
    for index, row in enumerate(drilldowns.get("entries") or []):
        ...
        for source_index, source in enumerate(trace_data.get("sources") or []):
            ...
            if (
                ...
                != "schema_v4_v5_exact_map_completion"
