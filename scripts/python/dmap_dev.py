...
def validate_completed_run_mode(run_dir: Path, mode: str) -> tuple[bool, str]:
    ...
    for stage_index, (estimation_stage, geometric_iteration, stage_root) in enumerate(stage_roots):
        ...
        for frame_dir in depthmap_dirs:
            ...
            if (
                manifest_schema_version not in EXACT_MAP_SUPPORTED_SCHEMA_VERSIONS
                or summary.get("schema_version") != manifest_schema_version
            ):
                return False, (
                    f"frame {frame_dir.name} deep capture requires supported "
                    "schema-v4/v5 "
...
summary::marker { color:#4f7f9f }
    ...
    if exact_cost_evolution.empty:
        lines.extend(["_Unavailable: no schema-v4/v5 exact hot-kernel cost maps were admitted for these captures._", ""])
    else:
        exact_cost_rows = [
            [
...
def terminal_exact_trace_capture(frame: pd.Series) -> dict[str, Any]:
    """Load terminal schema-v4/v5 maps without conflating estimation stages."""
    ...
    if manifest.get("schema_name") != "openmvs.dmap.map_manifest":
        ...
        return result
    if schema_version not in EXACT_MAP_SUPPORTED_SCHEMA_VERSIONS:
        result["capture_error"] = (
            f"map manifest schema_version={schema_version} is unsupported; "
            f"expected one of {sorted(EXACT_MAP_SUPPORTED_SCHEMA_VERSIONS)}"
        )
        return result
    ...
    if exact_capture.get("available") is not True:
        result["capture_error"] = str(
            exact_capture.get("unavailable_reason")
            or "schema-v4/v5 exact capture is unavailable"
        )
        return result
    ...
    if (
        ...
        result["capture_error"] = (
            "schema-v4/v5 logical counters are invalid or inconsistent: "
...
def exact_evidence_trace_recommendations(
    ...
    return {
        "mode": "schema4_v5_terminal_geometric_exact",
...
def generate_trace_manifest(
    ...
    path.write_text(yaml.safe_dump({
        ...
        "selection_method": (
            ...
            "paired terminal geometric schema-v4/v5 exact evidence when available; "
