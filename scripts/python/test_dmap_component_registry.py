...
class ComponentRegistryTest(unittest.TestCase):
    ...
    def test_apd_signals_have_explicit_score_domain_semantics(self) -> None:
        expected = {
            ...
            "apd_winner_runner_up_gap": (
                "candidate_update", "apd_working_objective", "map"
            ),
        ...
        for signal, values in expected.items():
            with self.subTest(signal=signal):
                ...
                self.assertEqual(descriptor.minimum_profile, "deep")
                if signal == "apd_winner_runner_up_gap":
                    self.assertEqual(descriptor.label, "APD evaluated-candidate gap")
                    self.assertIn(
                        "unconstrained evaluated-candidate ordering",
    ...
    def test_dvp_signals_have_explicit_mechanics_semantics(self) -> None:
        expected = {
            "dvp_left_interval_valid": ("geometry", "dvp_interval", "enum"),
            "dvp_incumbent_cost": ("cost", "dvp_candidate_objective", "map"),
            "dvp_winner_runner_up_gap": (
                "candidate_update", "dvp_candidate_objective", "map"
            ),
            "dvp_accepted_proposal_mask": (
                "candidate_update", "dvp_candidate_update", "enum"
            ),
            "dvp_final_depth_retained": (
                "candidate_update", "dvp_candidate_update", "enum"
            ),
            "dvp_proposal_depth_0": ("geometry", "dvp_proposal", "map"),
            "dvp_candidate_cost_1": ("cost", "dvp_candidate_objective", "map"),
            "dvp_support_views_0": (
                "view_selection", "dvp_view_evidence", "enum"
            ),
            "dvp_mean_reprojection_error_1": (
                "geometry", "dvp_global_gate", "map"
            ),
        }
