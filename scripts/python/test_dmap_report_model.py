...
def write_completed_trace_drilldown(
    ...
    *,
    ...
    exact_map_schema_version: int = 4,
) -> tuple[str, Path]:
    ...
    if exact_capture:
        ...
        (frame / "summary.json").write_text(json.dumps({
            ...
            "schema_version": exact_map_schema_version,
            "image_id": 7,
...
class DMapReportModelTests(unittest.TestCase):
    def test_exact_trace_capture_accepts_schema_v5(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            request_sha256, _traces = write_completed_trace_drilldown(
                root,
                [trace_row(-1), trace_row(0)],
                exact_map_schema_version=5,
            )
            capture_root = root / "drilldowns" / "captures" / request_sha256
            evidence = dmap_report_model._exact_trace_capture_evidence(
                capture_root / "runs" / "base" / "scene-a", 7
            )

            self.assertTrue(evidence["valid"], evidence)
            self.assertEqual(
                evidence["measurement_basis"],
    ...
    def test_reference_patch_layout_is_validated_and_retained_per_run_stage(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            metadata_v5 = json.loads(
                (instrumentation / "run_metadata.json").read_text(encoding="utf-8")
            )
            metadata_v5["schema_version"] = 5
            (instrumentation / "run_metadata.json").write_text(
                json.dumps(metadata_v5), encoding="utf-8"
            )
            schema_v5 = dmap_report_model.reference_patch_layout_records(
                [run_scene], output
            )[0]
            self.assertTrue(schema_v5["available"])
            self.assertTrue(schema_v5["contract_valid"])
    ...
    def test_dvp_observability_loader_preserves_contract_iterations_and_traces(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            output = root / "report"
            instrumentation = root / "capture"
            ...
            (instrumentation / "run_metadata.json").write_text(json.dumps({
                "schema_name": "openmvs.dmap.run",
                "schema_version": 5,
                "cuda_patchmatch_parameters": {"dvp_epipolar": contract},
            }), encoding="utf-8")
            (sidecars / "dvp_iteration.csv").write_text(
                "image_id,scale_number,logical_iteration,family,promotion_eligible,"
                "attempted_pixels,proposal_available_pixels,native_depth_fallback_pixels,"
                "proposals_generated,proposals_tested,proposals_finite,proposals_accepted,"
                "final_winner_pixels,final_depth_retained_pixels\n"
                "7,0,0,dvp_eq11_interval_v1,1,100,40,60,44,44,44,8,3,7\n",
                encoding="utf-8",
            )
            trace = {
                ...
                "endpoint_records": {
                    "right_inner": [
                        {"view": 2, "offset_px": 1.0, "back_projected_depth": 1.25}
                    ],
                },
                "proposals": [
                    {"ordinal": 0, "depth": 1.3, "candidate_cost": 0.2}
                ],
                "decision": {"winner_ordinal": 0, "dvp_depth_retained": True},
            }
            (sidecars / "dvp_traces.jsonl").write_text(
                json.dumps(trace) + "\n", encoding="utf-8"
            )
            run_scene = SimpleNamespace(
                label="candidate [deep]", repeat=0, scene_id="scene-a",
            ...
            iterations, traces, sources, contracts = (
                dmap_report_model.dvp_observability_records([run_scene], output)
            )

            self.assertEqual(len(iterations), 1)
            self.assertEqual(iterations[0]["pyramid_level"], 0)
            self.assertEqual(iterations[0]["proposal_available_pixels"], 40)
            self.assertEqual(len(traces), 1)
            self.assertEqual(
                traces[0]["endpoint_records"]["right_inner"][0]["view"], 2
            )
            self.assertEqual(len(contracts), 1)
            self.assertEqual(contracts[0]["family"], "dvp_eq11_interval_v1")
            trace_source = next(
                row for row in sources if row["kind"] == "targeted_trace"
            )
            self.assertTrue(trace_source["available"])
            ...
            model = {
                ...
                "aggregates": {"regressions": []},
                "mechanics": {
                    "dvp_contracts": contracts,
                    "dvp_iterations": iterations,
                    "dvp_traces": traces,
                },
            }
            validation = dmap_report_model.validate_report_model(model, output)
            check = next(
                row for row in validation["checks"]
                if row["name"] == "dvp_observability"
            )
            self.assertTrue(check["passed"], check)
            model["mechanics"]["dvp_iterations"][0][
                "native_depth_fallback_pixels"
            ] = 59
            invalid = dmap_report_model.validate_report_model(model, output)
            check = next(
                row for row in invalid["checks"]
            ...
            model["mechanics"]["dvp_contracts"][0]["family"] = (
                "global_search_gated_v1"
            )
            model["mechanics"]["dvp_contracts"][0]["claim_label"] = (
                "openmvs_experimental_mechanism"
            )
            model["mechanics"]["dvp_iterations"][0]["family"] = (
                "global_search_gated_v1"
            )
            model["mechanics"]["dvp_iterations"][0][
                "native_depth_fallback_pixels"
            ] = 0
            global_validation = dmap_report_model.validate_report_model(model, output)
            global_check = next(
                row for row in global_validation["checks"]
                if row["name"] == "dvp_observability"
            )
            self.assertTrue(global_check["passed"], global_check)
    ...
    def test_investigation_html_is_self_contained_and_links_canonical_report(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("valid schema-v4/v5 exact-map completion evidence", text)
