...
def make_apd_validation_fixture() -> tuple[dict, dict, list[dict], dict]:
    ...
    source_counts = {name: 0 for name in validator.APD_UPDATE_SOURCE_NAMES[:12]}
...
def promote_apd_fixture_to_c5_full(
    manifest: dict,
    summary: dict,
    entries: list[dict],
) -> None:
    ...
    observability.update({
        ...
        "mode_role": "full",
        "full_paper_mechanics_enabled": True,
        "implementation_label": "paper_mechanics_complete_openmvs",
        "implemented_through": "APD14-C5",
        "required_mechanics_not_yet_implemented": [],
    ...
    observability["schedule"]["enabled"] = True
    observability["fitted_plane"]["enabled"] = True
    observability["final_refinement"]["enabled"] = True
    observability["anchor_update"] = {"enabled": True}
    observability["iterations"][0]["updates"]["stage_counts"][
        "all_deformation_only"
    ] = 0


def promote_apd_fixture_to_v5(
    manifest: dict,
    summary: dict,
    entries: list[dict],
    *,
    ...
    promote_apd_fixture_to_c5_full(manifest, summary, entries)
    accounting = {
        "schema_name": "openmvs.dmap.apd_candidate_accounting",
        "schema_version": 1,
        "candidate_slots": 24,
        "dvp_candidate_slots": [22, 23],
        "dvp_active": dvp_active,
        "update_source_codes": 13,
        "scope": "complete_active_process_pixel_working_objective",
    }
    manifest["apd_capture"].update({
        "schema_version": 5,
        "candidate_accounting": accounting.copy(),
    })
    manifest["dvp_capture"] = {"requested": dvp_active}
    summary["apd_observability"].update({
        "schema_version": 5,
        "candidate_accounting": accounting.copy(),
    })
    ...
    for entry in entries:
        entry["apd_schema_version"] = 5
        if entry["signal"] == "apd_winner_runner_up_gap":
            # Match the schema-v5 metadata emitted by the C++ producer.
            entry["gap_basis"] = "unconstrained_evaluated_candidate_set_best_vs_runner_up"
            entry["selected_winner_relation"] = "exact_unless_optional_candidate_constraint_rejects_raw_best"
        if entry["signal"] in {
            "apd_candidate_tested_mask",
            "apd_candidate_finite_mask",
            "apd_candidate_accepted_mask",
        }:
            entry["candidate_slots"] = 24
...
class DMapInstrumentationSchemaV4Tests(unittest.TestCase):
    ...
    def test_apd_schema_v4_accepts_c5_full_contract(self) -> None:
        manifest, summary, entries, logical_maps = make_apd_validation_fixture()
        promote_apd_fixture_to_c5_full(manifest, summary, entries)
        checks: list[dict] = []
    ...
    def test_apd_schema_v5_accepts_combined_dvp_candidates_and_source(self) -> None:
        manifest, summary, entries, logical_maps = make_apd_validation_fixture()
        promote_apd_fixture_to_v5(
            manifest, summary, entries, dvp_active=True
        )
        pixels = WIDTH * HEIGHT
        base_mask = 519681 | (1 << 21)
        combined_mask = float(base_mask | (1 << 22) | (1 << 23))

        def fill(signal: str, value: int | float) -> None:
            entry, path, values = logical_maps[(signal, 0)]
            logical_maps[(signal, 0)] = (
                entry, path, np.full(values.shape, value, dtype=values.dtype)
            )

        fill("apd_candidate_tested_mask", combined_mask)
        fill("apd_candidate_finite_mask", combined_mask)
        fill("apd_candidate_accepted_mask", float(1 << 22))
        fill("apd_candidate_tested_count", 13)
        fill("apd_candidate_finite_count", 13)
        fill("apd_candidate_accepted_count", 1)
        fill("apd_update_source", 12)
        fill("apd_winner_slot", 22)
        fill("apd_runner_up_slot", 23)
        source_counts = summary["apd_observability"]["iterations"][0][
            "updates"
        ]["source_counts"]
        source_counts["none"] = 0
        source_counts["dvp_epipolar"] = pixels
        checks: list[dict] = []
    ...
    def test_apd_schema_v5_rejects_missing_gap_metadata(self) -> None:
        for field in ("gap_basis", "selected_winner_relation"):
            with self.subTest(field=field):
                manifest, summary, entries, logical_maps = make_apd_validation_fixture()
                promote_apd_fixture_to_v5(manifest, summary, entries, dvp_active=False)
                entry = next(item for item in entries if item["signal"] == "apd_winner_runner_up_gap")
                del entry[field]
                checks = []
                validator.validate_apd_maps(
                    manifest=manifest, summary=summary, entries=entries,
                    logical_maps=logical_maps, width=WIDTH, height=HEIGHT,
                    tolerance=0.0, expect_geometric_zero=True,
                    check=lambda name, passed, detail: checks.append({
                        "name": name, "passed": passed, "detail": detail,
                    }),
                )
                metadata = next(check for check in checks if check["name"] == "apd_map_metadata")
                self.assertFalse(metadata["passed"])
                self.assertTrue(any(field in error for error in metadata["detail"]))

    def test_apd_schema_v5_requires_visibility_component_contract(self) -> None:
        manifest, summary, entries, logical_maps = make_apd_validation_fixture()
        promote_apd_fixture_to_v5(
            manifest, summary, entries, dvp_active=False
        )
        manifest["dvp_visibility_capture"] = {
            "requested": True,
            "stage_active": True,
        }
        component_contract = {
        ...
        summary["apd_observability"]["component_contract"] = component_contract
        for entry in entries:
            if entry["signal"] == "apd_anchor_mean_cost":
                entry["component_basis"] = (
                    "visibility_aware_effective_anchor_leg"
                )
            elif entry["signal"] == "apd_deformable_photometric_cost":
        ...
        summary["apd_observability"]["component_contract"][
            "anchor_component_basis"
        ] = "active_anchor_mean"
        corrupted: list[dict] = []
        validator.validate_apd_maps(
        ...
        contract_check = next(
            check for check in corrupted
            if check["name"] == "apd_summary_contract"
        )
        self.assertFalse(contract_check["passed"])

    def test_apd_schema_v5_accepts_depth_edge_filtered_active_subset(self) -> None:
        manifest, summary, entries, logical_maps = make_apd_validation_fixture()
        promote_apd_fixture_to_v5(
            manifest, summary, entries, dvp_active=False
        )
        ...
        for signal in (
            ...
            fill(signal, 0)
        fill("apd_anchor_accepted_slot", 255)
        base_mask = 519681 & ~(0xFF << 13)
        fill("apd_candidate_tested_mask", float(base_mask))
        fill("apd_candidate_finite_mask", float(base_mask))
        fill("apd_candidate_tested_count", base_mask.bit_count())
        fill("apd_candidate_finite_count", base_mask.bit_count())

        iteration = summary["apd_observability"]["iterations"][0]
        iteration["updates"].update({
            "deformable_updates": 0,
            "working_gap_samples": 0,
            "working_gap_mean": None,
            "center_cost_mean": None,
            "anchor_mean_cost_mean": None,
            "working_cost_mean": None,
            "native_persistent_cost_mean": None,
            "immutable_anchor_state_updates": 0,
        })
        iteration["anchor_view_selection"].update({
            "attempted_pixels": 0,
            "anchor_evidence_pixels": 0,
            "mode_counts": {
                name: 0 for name in validator.APD_VIEW_SELECTION_MODE_NAMES
            },
        })
        iteration["anchor_propagation"].update({
            "tested_candidates": 0,
            "finite_candidates": 0,
            "accepted_events": 0,
            "final_winner_pixels": 0,
            "best_working_cost_samples": 0,
            "best_working_cost_mean": None,
            "accepted_native_cost_samples": 0,
            "accepted_native_cost_mean": None,
        })
        iteration["fitted_plane"].update({
            "available_pixels": 0,
            "tested_pixels": 0,
            "finite_pixels": 0,
            "accepted_events": 0,
        ...
        self.assertEqual(
            relationship["detail"]["deformable_activity_contract"],
            "depth_edge_active_is_subset_of_prefilter_eligible",
        )

    def test_apd_summary_accepts_bounded_full_frame_float_atomic_error(self) -> None:
        manifest, summary, entries, logical_maps = make_apd_validation_fixture()
        promote_apd_fixture_to_v5(
            manifest, summary, entries, dvp_active=False
        )
        iteration = summary["apd_observability"]["iterations"][0]
        iteration["anchor_model"]["anchor_count_mean"] = 6.005
        iteration["global_minimum_cost"]["mean"] = 0.1009
        iteration["profile_separation"]["mean"] = 0.3009
        checks: list[dict] = []
        ...
        self.assertTrue(all(check["passed"] for check in checks), checks)
        closure = next(
            check for check in checks if check["name"] == "apd_summary_map_closure"
        )
        self.assertEqual(
            closure["detail"]["per_iteration"]["0"][
                "mean_relative_tolerance"
            ],
            validator.APD_SUMMARY_MEAN_RELATIVE_TOLERANCE,
        )

    def test_apd_schema_v5_accepts_fitted_plane_acceptance_above_bad_cost(self) -> None:
        manifest, summary, entries, logical_maps = make_apd_validation_fixture()
        promote_apd_fixture_to_v5(
            manifest, summary, entries, dvp_active=False
        )
        ...
        base_tested = 519681 | (1 << 21)
        fill("apd_fitted_plane_working_cost", validator.APD_BAD_COST)
        fill("apd_fitted_plane_accepted", 1)
        fill("apd_candidate_tested_mask", float(base_tested))
        fill("apd_candidate_finite_mask", float(519681))
        fill("apd_candidate_accepted_mask", float(1 << 21))
        fill("apd_candidate_tested_count", 11)
        fill("apd_candidate_finite_count", 10)
        fill("apd_candidate_accepted_count", 1)
        fitted = summary["apd_observability"]["iterations"][0]["fitted_plane"]
        fitted["finite_pixels"] = 0
        fitted["accepted_events"] = pixels
        checks: list[dict] = []
        ...
        self.assertTrue(all(check["passed"] for check in checks), checks)
        self.assertEqual(
            result["fitted_plane"]["0"]["accepted_non_usable_events"],
            pixels,
        )

    def test_apd_schema_v4_accepts_c5_deformation_only_contract(self) -> None:
        manifest, summary, entries, logical_maps = make_apd_validation_fixture()
        promote_apd_fixture_to_c5_full(manifest, summary, entries)
        pixels = WIDTH * HEIGHT
        capture = manifest["apd_capture"]
        ...
        observability.update({
            "mode": 2,
            "mode_name": "deformation_only",
            "mode_role": "deformation_only",
            "full_paper_mechanics_enabled": False,
            "implementation_label": "deformation_only_openmvs",
            "active_mechanics": [
                "reliability_profiles",
                "reliable_anchor_ransac",
                "deformable_patch_cost",
        ...
        observability["schedule"]["enabled"] = False
        observability["fitted_plane"]["enabled"] = False
        observability["final_refinement"]["enabled"] = False
        observability["anchor_update"]["enabled"] = False

        def fill(signal: str, value: float | int) -> None:
            entry, path, values = logical_maps[(signal, 0)]
            logical_maps[(signal, 0)] = (
                entry,
                path,
                np.full_like(values, value),
            )

        fill("apd_view_selection_mode", 0)
        fill("apd_anchor_evidence_count", 0)
        fill("apd_anchor_proposal_count", 0)
        fill("apd_anchor_finite_count", 0)
        fill("apd_best_anchor_working_cost", -1.0)
        fill("apd_candidate_tested_mask", float((1 << 13) - 1))
        fill("apd_candidate_finite_mask", float((1 << 13) - 1))
        fill("apd_candidate_tested_count", 13)
        fill("apd_candidate_finite_count", 13)
        fill("apd_update_stage", 0)
        fill("apd_fitted_plane_working_cost", -1.0)
        fill("apd_fitted_plane_native_cost", -1.0)
        fill("apd_fitted_plane_available", 0)
        fill("apd_fitted_plane_tested", 0)
        fill("apd_fitted_plane_accepted", 0)
        fill("apd_final_refinement_incumbent_cost", -1.0)
        fill("apd_final_refinement_best_cost", -1.0)
        fill("apd_final_refinement_improvement", -1.0)
        fill("apd_final_refinement_depth", -1.0)
        fill("apd_final_refinement_offset", 0)
        fill("apd_final_refinement_tested_count", 0)
        fill("apd_final_refinement_finite_count", 0)
        fill("apd_final_refinement_accepted", 0)

        iteration = observability["iterations"][0]
        iteration["updates"]["stage_counts"] = {
            "all_compatibility": pixels,
            "all_deformation_only": pixels,
            "reliable_first": 0,
            "non_reliable_second": 0,
        }
        iteration["anchor_view_selection"] = {
            "attempted_pixels": 0,
            "anchor_evidence_pixels": 0,
            "mode_counts": {
                name: 0 for name in validator.APD_VIEW_SELECTION_MODE_NAMES
        ...
        iteration["final_native_refinement"] = {
            "eligible_pixels": 0,
            "tested_candidates": 0,
            "finite_candidates": 0,
            "accepted_pixels": 0,
