(function () {
  ...
  const COLUMN_HELP = {
    ...
    "Update source": "Accepted update attribution for the traced logical state. The adjacent quality badge is exact only for hot-kernel rows backed by valid schema-v4/v5 exact-map completion evidence; legacy post-pass rows remain proxy.",
  ...
  const FALLBACK_GUIDE = {
    ...
    recipes: [
      ...
      {
        key: "dvp", title: "Debug DVP epipolar depth proposals",
        summary: "Follow endpoint evidence through disjoint intervals, proposal scoring, acceptance, and retained depth.",
        steps: [
          "Use Same logical iteration and the DVP mechanics preset.",
          "Check proposal availability, native fallback, unavailable reason, and independent left/right interval validity.",
          "Inspect all four endpoint-support groups, then compare proposal depth, cost, support, and gate errors.",
          "Separate accepted proposal, final full-hypothesis winner, and final depth retained.",
          "Use the selected-pixel trace for endpoint view identities, offsets, back-projected depths, and exact decisions.",
        ],
        look_for: ["Intervals with complete mu-view support.", "Accepted DVP depths that survive later refinement and improve final geometry."],
        cautions: ["Historical families are compatibility arms.", "Process<true> mechanics do not establish quality."],
        action_label: "Load DVP evidence",
      },
      {
  ...
  const GUIDE_RECIPE_ACTIONS = {
    ...
    dvp: {
      alignment: "same", mapPreset: "dvp", target: "mechanics-heading",
      preferredSignals: [
        "reference_rgb", "dvp_unavailable_reason", "dvp_native_depth_fallback",
        "dvp_left_interval_valid", "dvp_right_interval_valid",
        "dvp_left_outer_count", "dvp_left_inner_count", "dvp_right_inner_count",
        "dvp_right_outer_count", "dvp_left_interval_minimum", "dvp_left_interval_maximum",
        "dvp_right_interval_minimum", "dvp_right_interval_maximum",
        "dvp_proposal_depth_0", "dvp_proposal_depth_1",
        "dvp_candidate_cost_0", "dvp_candidate_cost_1",
        "dvp_support_0", "dvp_support_1", "dvp_accepted_proposal_mask",
        "dvp_winner_ordinal", "dvp_winner_runner_up_gap",
        "dvp_final_update_source", "dvp_final_winner",
        "dvp_final_depth_retained", "dvp_depth_displacement",
      ],
      mechanisms: ["geometry", "cost", "candidate_update", "view_selection"],
      keywords: ["dvp", "epipolar", "interval", "proposal"],
    },
    multiscale: {
  ...
  function renderStatus() {
    ...
    byId("contract-alerts").innerHTML = [
      ...
      `<span class="status-pill ${mechanics.dvp_iteration_mechanics ? "good" : "warning"}">DVP iteration mechanics ${mechanics.dvp_iteration_mechanics ? "available" : "unavailable"}</span>`,
      `<span class="status-pill ${mechanics.dvp_targeted_traces ? "good" : "warning"}">DVP targeted traces ${mechanics.dvp_targeted_traces ? "available" : "unavailable"}</span>`,
  ...
  function renderSelectors() {
    ...
    controls.mapPreset.innerHTML = [
      ...
      ["apd", "APD mechanics"], ["dvp", "DVP mechanics"], ["view", "View selection"],
  ...
  function dvpIntervalSvg(trace) {
    const decision = trace.decision || {};
    const intervals = trace.intervals || {};
    const proposals = trace.proposals || [];
    const values = [decision.incumbent_depth, decision.final_depth];
    [intervals.left, intervals.right].forEach((interval) => {
      if (interval?.valid) values.push(interval.minimum, interval.maximum);
    });
    proposals.forEach((proposal) => values.push(proposal.depth));
    const finite = values.map(Number).filter((value) => Number.isFinite(value) && value > 0);
    if (finite.length < 2) return `<span class="mechanics-unavailable">interval plot unavailable</span>`;
    let low = Math.min(...finite), high = Math.max(...finite);
    const margin = Math.max((high - low) * 0.08, Math.abs(low) * 1e-5, 1e-6);
    low -= margin; high += margin;
    const width = 360, height = 108, left = 48, right = 12;
    const x = (value) => left + (Number(value) - low) * (width - left - right) / (high - low);
    const intervalBar = (interval, y, color, label) => {
      if (!interval?.valid) return `<text x="4" y="${y + 4}" font-size="9" fill="#8a949d">${label} invalid</text>`;
      const x0 = x(Math.min(Number(interval.minimum), Number(interval.maximum)));
      const x1 = x(Math.max(Number(interval.minimum), Number(interval.maximum)));
      return `<text x="4" y="${y + 4}" font-size="9" fill="${color}">${label}</text><line x1="${x0}" x2="${x1}" y1="${y}" y2="${y}" stroke="${color}" stroke-width="7" stroke-linecap="round"/>`;
    };
    const proposalMarks = proposals.map((proposal, index) => Number.isFinite(Number(proposal.depth))
      ? `<circle cx="${x(proposal.depth)}" cy="72" r="4" fill="${proposal.sequentially_accepted ? "#3f8f6b" : "#c85a54"}"/><text x="${x(proposal.depth) + 5}" y="75" font-size="8" fill="#334155">P${index}</text>` : "").join("");
    return `<svg class="dvp-interval-plot" viewBox="0 0 ${width} ${height}" role="img" aria-label="DVP disjoint depth intervals and proposals">
      <rect x="${left}" y="8" width="${width - left - right}" height="78" fill="#f8fafc" stroke="#cad5df"/>
      ${intervalBar(intervals.left, 26, "#4169a1", "left")}${intervalBar(intervals.right, 48, "#a05d3b", "right")}
      ${proposalMarks}
      <line x1="${x(decision.incumbent_depth)}" x2="${x(decision.incumbent_depth)}" y1="12" y2="84" stroke="#111827" stroke-dasharray="3 3"/>
      <text x="${left}" y="101" font-size="8" fill="#5b6772">${escapeHtml(format(low, 6))}</text><text x="${width - right - 48}" y="101" font-size="8" fill="#5b6772">${escapeHtml(format(high, 6))}</text>
    </svg>`;
  }

  function dvpTraceCards(rows) {
    if (!rows.length) return "";
    const shown = rows.slice(0, 24);
    const cards = shown.map((row) => {
      const decision = row.decision || {};
      const intervals = row.intervals || {};
      const endpointRows = Object.entries(row.endpoint_records || {}).flatMap(([group, samples]) =>
        (samples || []).map((sample) => [group, sample.view, format(sample.offset_px, 2), format(sample.back_projected_depth, 7)]));
      const proposalRows = (row.proposals || []).map((proposal) => [
        proposal.ordinal, format(proposal.depth, 7), format(proposal.signed_offset, 3),
        proposal.source_view ?? "interval", proposal.support, proposal.support_views,
        proposal.occluded_views, format(proposal.mean_reprojection_error_px, 4),
        format(proposal.mean_relative_depth_error, 6), format(proposal.candidate_cost, 7),
        proposal.tested, proposal.finite, proposal.sequentially_accepted,
      ]);
      return `<details class="dvp-trace-card"><summary>${escapeHtml(row.run)} / ${escapeHtml(row.family || "DVP")} / ${escapeHtml(row.label || "pixel")} / (${escapeHtml(row.x)}, ${escapeHtml(row.y)})</summary>
        <div class="dvp-trace-grid"><div>${dvpIntervalSvg(row)}<p class="mechanics-note">Left interval ${escapeHtml(intervals.left?.valid ?? false)} [${escapeHtml(format(intervals.left?.minimum, 7))}, ${escapeHtml(format(intervals.left?.maximum, 7))}]; right interval ${escapeHtml(intervals.right?.valid ?? false)} [${escapeHtml(format(intervals.right?.minimum, 7))}, ${escapeHtml(format(intervals.right?.maximum, 7))}]. Invalid intervals remain explicit.</p></div>
        <dl class="apd-trace-facts"><dt>Availability</dt><dd>${escapeHtml(row.unavailable_reason || "none")}</dd><dt>Selected/direction views</dt><dd>${escapeHtml(row.selected_source_views ?? "n/a")} / ${escapeHtml(row.direction_source_views ?? "n/a")}</dd><dt>Incumbent depth/cost</dt><dd>${escapeHtml(format(decision.incumbent_depth, 7))} / ${escapeHtml(format(decision.incumbent_cost, 7))}</dd><dt>Winner/runner/gap</dt><dd>${escapeHtml(format(decision.winner_cost, 7))} / ${escapeHtml(format(decision.runner_up_cost, 7))} / ${escapeHtml(format(decision.winner_runner_up_gap, 7))}</dd><dt>Proposal outcome</dt><dd>winner ${escapeHtml(decision.winner_ordinal ?? "none")}; mask ${escapeHtml(decision.accepted_proposal_mask ?? 0)}; accepted ${escapeHtml(decision.dvp_accepted ?? false)}</dd><dt>Final outcome</dt><dd>${escapeHtml(decision.final_update_source || "none")}; full hypothesis ${escapeHtml(decision.dvp_final_hypothesis_winner ?? false)}; depth retained ${escapeHtml(decision.dvp_depth_retained ?? false)}</dd><dt>Final depth/displacement</dt><dd>${escapeHtml(format(decision.final_depth, 7))} / ${escapeHtml(format(decision.depth_displacement, 7))}</dd><dt>Native fallback</dt><dd>${escapeHtml(decision.native_depth_fallback ?? false)}</dd></dl></div>
        <div class="apd-trace-tables">${smallTable(["Endpoint group", "View", "Offset px", "Back-projected depth"], endpointRows) || `<p class="mechanics-unavailable">Endpoint identities unavailable for this family.</p>`}${smallTable(["Proposal", "Depth", "Side/offset", "Source view", "Support", "Support mask", "Occluded mask", "Mean reproj px", "Mean relative depth", "Cost", "Tested", "Finite", "Accepted"], proposalRows) || `<p class="mechanics-unavailable">No DVP proposal was generated.</p>`}</div>
      </details>`;
    }).join("");
    const omitted = rows.length - shown.length;
    return `${cards}${omitted > 0 ? `<p class="mechanics-note">Showing 24 of ${rows.length} matching DVP trace rows; use the recorded JSONL source for the complete bounded capture.</p>` : ""}`;
  }
  ...
  function renderMechanics() {
    ...
    const apdIterationRows = apdIterationEvidence.map((row) => {
      ...
      return [
        ...
        Number(row.anchor_view_selection_attempted || 0) > 0
          ? formatPercent(Number(row.anchor_view_selection_used || 0) / Number(row.anchor_view_selection_attempted))
          : "not active",
        ...
        Number(row.stage_all_deformation_only || 0) > 0
          ? `native all=${row.stage_all_deformation_only}`
          : `reliable/non-reliable=${row.stage_reliable_first || 0}/${row.stage_non_reliable_second || 0}`,
    ...
    const dvpIterationRows = aligned(mechanics.dvp_iterations || []).map((row) => {
      const attempted = Math.max(1, Number(row.attempted_pixels || 0));
      return [
        row.run, pyramidLevelLabel(pyramidLevelOf(row)),
        `iteration ${Number(row.logical_iteration) + 1}`, row.family,
        row.promotion_eligible, row.attempted_pixels,
        `${row.proposal_available_pixels} (${formatPercent(Number(row.proposal_available_pixels || 0) / attempted)})`,
        `${row.native_depth_fallback_pixels} (${formatPercent(Number(row.native_depth_fallback_pixels || 0) / attempted)})`,
        `${row.proposals_generated}/${row.proposals_tested}/${row.proposals_finite}/${row.proposals_accepted}`,
        `${row.final_winner_pixels} (${formatPercent(Number(row.final_winner_pixels || 0) / attempted)})`,
        `${row.final_depth_retained_pixels} (${formatPercent(Number(row.final_depth_retained_pixels || 0) / attempted)})`,
        `${row.left_interval_valid_pixels}/${row.right_interval_valid_pixels}`,
        `${format(row.left_outer_support_mean, 2)}/${format(row.left_inner_support_mean, 2)}/${format(row.right_inner_support_mean, 2)}/${format(row.right_outer_support_mean, 2)}`,
        format(row.incumbent_cost_mean, 6), format(row.winner_cost_mean, 6),
        format(row.improvement_mean, 7), format(row.winner_runner_up_gap_mean, 6),
        format(row.depth_displacement_mean, 7),
        `${format(row.selected_source_view_count_mean, 2)}/${format(row.direction_source_view_count_mean, 2)}`,
        prefixedCounts(row, "unavailable_"), prefixedCounts(row, "final_source_"),
      ];
    });
    ...
    const dvpTraceRows = aligned(mechanics.dvp_traces || [], true).sort((left, right) =>
      String(left.run).localeCompare(String(right.run)) || Number(left.trace_index) - Number(right.trace_index));
    ...
    const apdContractRows = (mechanics.apd_contracts || []).filter(contractMatch).map((row) => [
      row.run, row.mode_name || row.mode, row.mode_role || "legacy",
      row.implementation_label || "unavailable",
      row.implemented_through || "unavailable", row.target_label || "unavailable",
      row.exact_author_code_equivalence_claimed ?? "n/a",
      (row.active_mechanics || []).join(", ") || "none",
      (row.required_mechanics_not_active || []).join(", ") || "none",
    ...
    const dvpContractRows = (mechanics.dvp_contracts || []).filter(contractMatch).map((row) => [
      row.run, row.family || "disabled", row.promotion_eligible ?? false,
      row.claim_label || "disabled", row.author_code_exact_equivalence ?? false,
      `a=${format(row.parameters?.alpha, 2)}, b=${format(row.parameters?.beta, 2)}, mu=${row.parameters?.mu ?? "n/a"}`,
      `radius=${row.parameters?.global_search_radius ?? "n/a"}, reproj=${format(row.parameters?.global_reprojection_threshold_px, 3)} px, rel-depth=${format(row.parameters?.global_relative_depth_threshold, 5)}`,
      row.provenance?.official_repository_commit || "unavailable",
      row.provenance?.colleague_donor_commit || "unavailable",
      (row.scope_exclusions || []).join(", ") || "none",
    ]);
    const dvpTraceSourceRows = (mechanics.dvp_trace_sources || []).filter(contractMatch).map((row) => [
      row.run, row.kind, row.available, row.rows_included ?? "n/a", row.truncated ?? false,
      row.reason || "available", row.path || "unavailable",
    ]);
    ...
    if (apdContractRows.length || apdMultiscaleRows.length || apdIterationRows.length || apdTraceSourceRows.length) {
      extensionBlocks.push(`<div class="mechanics-block apd-mechanics-block"><h3>Adaptive Patch Deformation mechanics <small>(iteration schemas v1-v4 / multiscale schema v1)</small></h3><p class="mechanics-note">Exact complete-iteration evidence. Schema v4 identifies the selected causal arm: full APD uses reliable-first scheduling, anchor-driven view selection and propagation, fitted-plane proposals, and terminal native refinement; deformation-only retains native checkerboard scheduling, view selection, propagation geometry, and refinement while applying deformable scoring to supported unreliable pixels. Persistent winners are conventionally rescored. Process&lt;true&gt; mechanics do not establish quality.</p>${smallTable(["Run", "Mode", "Role", "Implementation label", "Checkpoint", "Target label", "Exact author-code equivalence", "Active mechanics", "Inactive mechanics", "Required mechanics unavailable"], apdContractRows)}<h4>Stage clocks and transferred state</h4><p class="mechanics-note">The coarsest stage remains native. Its output reliability seed is compatibility-derived from the exact final selected-view mask because historical native Monte Carlo weights are unavailable. Later stages consume the transferred reliability map exactly at iteration zero.</p>${smallTable(["Run", "Pyramid", "Stage", "Clock", "Transfer", "Schedule", "Eta", "RANSAC threshold", "Reliable in", "Reliable out", "Anchors in", "Anchors out", "Eligible in", "Eligible out", "Classifier weights", "Valid", "Errors"], apdMultiscaleRows) || unavailable("APD multiscale stage records", false)}<h4>Complete logical iterations</h4>${smallTable(["Run", "Pyramid", "State", "Classified", "Reliable", "Deformable eligible", "Valid RANSAC", "Mean anchors", "Deformable updates", "Profile min cost", "Profile separation", "Center cost", "Anchor cost", "Working cost", "Native cost", "Working gap", "Anchor-view use", "View modes", "Accepted/finite/tested anchors", "Final anchor winners", "Best anchor cost", "Accepted anchor native", "Stage schedule", "Fitted accepted/finite/tested/available", "Final accepted/finite/tested/eligible", "Winner sources"], apdIterationRows) || unavailable("APD complete-iteration counters", false)}${smallTable(["Run", "Artifact", "Available", "Rows embedded", "Truncated", "Reason", "Source"], apdTraceSourceRows)}<h4>Selected-pixel APD profiles, view evidence, fitted plane, and anchor candidates</h4>${apdTraceCards(apdTraceRows) || unavailable("APD targeted profile and anchor traces", false)}</div>`);
    }
    if (dvpContractRows.length || dvpIterationRows.length || dvpTraceSourceRows.length) {
      extensionBlocks.push(`<div class="mechanics-block dvp-mechanics-block"><h3>DVP epipolar proposal mechanics <small>(schema v1)</small></h3><p class="mechanics-note">Exact active Process&lt;true&gt; evidence for endpoint collection, disjoint interval construction, proposal scoring, sequential acceptance, and retained outcome. Complete logical iterations combine both checkerboards; phase identity remains timing-only. Historical global and midpoint families are compatibility arms. Process&lt;true&gt; mechanics do not establish quality.</p>
        <h4>Contract and provenance</h4>${smallTable(["Run", "Family", "Promotion eligible", "Claim label", "Exact author-code equivalence", "Eq. 11 parameters", "Global gates", "Official source", "Donor source", "Explicit exclusions"], dvpContractRows) || unavailable("DVP runtime contract", false)}
        <h4>Complete logical iterations</h4><p class="mechanics-note">Proposal events count generated candidates; available/fallback and final outcomes count pixels. A retained DVP depth can be followed by a normal-only refinement, so final full-hypothesis winner and final depth retained are intentionally separate.</p>${smallTable(["Run", "Pyramid", "State", "Family", "Eligible", "Pixels", "Proposal pixels", "Native fallback", "Generated/tested/finite/accepted", "Final full winners", "Final depths retained", "Left/right intervals", "Endpoint support LO/LI/RI/RO", "Incumbent cost", "Winner cost", "Mean improvement", "Winner gap", "Depth displacement", "Selected/direction views", "Unavailable reasons", "Final sources"], dvpIterationRows) || unavailable("DVP complete-iteration counters", false)}
        ${smallTable(["Run", "Artifact", "Available", "Rows embedded", "Truncated", "Reason", "Source"], dvpTraceSourceRows)}
        <h4>Selected-pixel endpoints, intervals, proposals, and outcomes</h4>${dvpTraceCards(dvpTraceRows) || unavailable("DVP targeted endpoint traces", false)}</div>`);
    }
