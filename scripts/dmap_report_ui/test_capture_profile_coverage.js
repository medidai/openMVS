...
function modelWithDvpEvidence() {
  ...
  Object.assign(frame, {
    logical_iterations: [0], pyramid_levels: [0],
    logical_iterations_by_pyramid_level: {0: [0]},
    run_frames: ["baseline", "candidate"].map((run) => ({
      run, repeat: 0, scene_id: "scene-a", image_id: 17,
      estimation_stage: "photometric", geometric_iteration: null,
      capture_profile: "deep", pyramid_level: 0,
      iterations: [{
        run, repeat: 0, scene_id: "scene-a", image_id: 17,
        estimation_stage: "photometric", geometric_iteration: null,
        pyramid_level: 0, logical_iteration: 0,
  ...
  const contract = (run) => ({
    run, configured_run: run, repeat: 0, scene_id: "scene-a",
    estimation_stage: "photometric", geometric_iteration: null,
    capture_profile: "trace", enabled: true,
    family: "dvp_eq11_interval_v1", promotion_eligible: true,
    claim_label: "paper_mechanics_complete_openmvs",
    author_code_exact_equivalence: false,
    parameters: {
      alpha: 1, beta: 4, mu: 3, global_search_radius: 160,
      global_reprojection_threshold_px: 2,
      global_relative_depth_threshold: 0.01,
    },
    provenance: {
    ...
    scope_exclusions: ["depth_edge_prior", "dvp_plus_plus"],
  });
  const iteration = (run) => ({
    run, configured_run: run, repeat: 0, scene_id: "scene-a", image_id: 17,
    estimation_stage: "photometric", geometric_iteration: null,
    capture_profile: "deep", pyramid_level: 0, logical_iteration: 0,
    family: "dvp_eq11_interval_v1", promotion_eligible: 1,
    attempted_pixels: 100, proposal_available_pixels: 40,
    native_depth_fallback_pixels: 60, proposals_generated: 45,
    proposals_tested: 45, proposals_finite: 44, proposals_accepted: 12,
    final_winner_pixels: 5, final_depth_retained_pixels: 11,
    left_interval_valid_pixels: 15, right_interval_valid_pixels: 30,
    left_outer_support_mean: 3.1, left_inner_support_mean: 3.2,
    right_inner_support_mean: 3.3, right_outer_support_mean: 3.4,
    incumbent_cost_mean: 0.4, winner_cost_mean: 0.35,
    improvement_mean: 0.05, winner_runner_up_gap_mean: 0.02,
    depth_displacement_mean: 0.001,
    selected_source_view_count_mean: 4.2, direction_source_view_count_mean: 3.8,
    unavailable_none: 40, unavailable_insufficient_endpoint_support: 60,
    final_source_dvp_epipolar: 5, final_source_refine_normal: 6,
  });
  const trace = (run) => ({
    schema_name: "openmvs.dmap.dvp_trace", schema_version: 1,
    run, configured_run: run, repeat: 0, scene_id: "scene-a", image_id: 17,
    estimation_stage: "photometric", geometric_iteration: null,
    capture_profile: "trace", pyramid_level: 0, logical_iteration: 0,
    trace_index: 0, x: 3, y: 4, label: "synthetic-eq11-trace",
    family: "dvp_eq11_interval_v1", promotion_eligible: true,
    unavailable_reason: "none", selected_source_views: 7,
    direction_source_views: 6,
    intervals: {
      left: {valid: true, minimum: 1.1, maximum: 1.2},
      right: {valid: true, minimum: 1.3, maximum: 1.4},
    },
    endpoint_records: {
      left_outer: [{view: 2, offset_px: -5, back_projected_depth: 1.1}],
      left_inner: [{view: 2, offset_px: -1, back_projected_depth: 1.2}],
      right_inner: [{view: 5, offset_px: 1, back_projected_depth: 1.3}],
      right_outer: [{view: 5, offset_px: 5, back_projected_depth: 1.4}],
    },
    proposals: [{
      ordinal: 0, depth: 1.15, signed_offset: -1, source_view: null,
      support: 3, support_views: 7, occluded_views: 0,
      mean_reprojection_error_px: -1, mean_relative_depth_error: -1,
      candidate_cost: 0.2, tested: true, finite: true,
      sequentially_accepted: true,
    }],
    decision: {
      incumbent_depth: 1.25, incumbent_cost: 0.3,
      winner_cost: 0.2, runner_up_cost: 0.3, winner_runner_up_gap: 0.1,
      winner_ordinal: 0, accepted_proposal_mask: 1, dvp_accepted: true,
      final_update_source: "refine_normal",
      dvp_final_hypothesis_winner: false, dvp_depth_retained: true,
      final_depth: 1.15, depth_displacement: 0.1,
      native_depth_fallback: false,
    },
  });
  model.mechanics.dvp_contracts = [contract("baseline"), contract("candidate")];
  model.mechanics.dvp_iterations = [iteration("baseline"), iteration("candidate")];
  model.mechanics.dvp_traces = [trace("baseline"), trace("candidate")];
  model.mechanics.dvp_trace_sources = ["baseline", "candidate"].map((run) => ({
    run, configured_run: run, repeat: 0, scene_id: "scene-a",
    estimation_stage: "photometric", geometric_iteration: null,
    capture_profile: "trace", kind: "targeted_trace", available: true,
    rows_included: 1, truncated: false, reason: "available",
    path: `../traces/${run}/dvp_traces.jsonl`,
  }));
  model.mechanics.availability = {
    dvp_iteration_mechanics: true, dvp_targeted_traces: true,
  };
  return model;
}
...
test("targeted traces follow their configured run into deep mechanics profiles", (context) => {
  ...
  assert.match(mechanics, /iteration schemas v1-v4/);
...
test("DVP mechanics render exact contracts, interval traces, and proposal attribution", (context) => {
  if (!browserBinary()) return context.skip("Chrome or Chromium is unavailable");
  const hash = [
    "baseline=baseline", "variant=candidate", "scene=scene-a",
    "frame=frame-scene-a-17", "captureStage=photometric",
    "pyramidLevel=0", "alignment=same", "iteration=0", "mapPreset=dvp",
  ].join("&");
  const dom = dumpDom(modelWithDvpEvidence(), `#${hash}`);
  const mechanics = mechanicsSection(dom);
  assert.equal((mechanics.match(/class="mechanics-block dvp-mechanics-block"/g) || []).length, 1);
  assert.match(mechanics, /DVP epipolar proposal mechanics/);
  assert.match(mechanics, /paper_mechanics_complete_openmvs/);
  assert.match(mechanics, /5b2b31c02941cfa28808b4e37f06a87e8dc0f2d9/);
  assert.match(mechanics, /c6f0f2c0fe2e31ec68c54cac681cc3b1665d2f7b/);
  assert.match(mechanics, /45\/45\/44\/12/);
  assert.match(mechanics, /40 \(40\.0%\)/);
  assert.match(mechanics, /60 \(60\.0%\)/);
  assert.equal((mechanics.match(/class="dvp-trace-card"/g) || []).length, 2);
  assert.equal((mechanics.match(/class="dvp-interval-plot"/g) || []).length, 2);
  assert.match(mechanics, /Endpoint group[\s\S]*?left_outer[\s\S]*?right_outer/);
  assert.match(mechanics, /Proposal[\s\S]*?Source view[\s\S]*?Support mask/);
  assert.match(mechanics, /Final outcome[\s\S]*?refine_normal[\s\S]*?depth retained/);
  assert.match(mechanics, /Native fallback[\s\S]*?false/);

  const singleArm = dumpDom(
    modelWithDvpEvidence(),
    `#${hash.replace("variant=candidate", "variant=baseline")}`,
  );
  assert.equal(
    (mechanicsSection(singleArm).match(/class="dvp-trace-card"/g) || []).length,
    1,
  );
});
