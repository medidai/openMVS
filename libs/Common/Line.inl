...
int TLine<TYPE, DIMS>::Optimize(const POINT *points, size_t size,
                                const RobustNormFunctor &robust, int maxIters) {
  ASSERT(DIMS == 3);
  ...
  struct OptimizationFunctor {
    ...
    static void Residuals(const double *x, int nPoints, const void *pData,
      ...
      for (size_t i = 0; i < data.size; ++i)
        fvec[i] =
            data.robust(line.Distance(data.points[i].template cast<double>()));
    }
  ...
  lm_control_struct control = {1.e-6, 1.e-7, 1.e-8,
                               1.e-8, 100.0, maxIters}; // lm_control_float;
...
template <typename TYPE, int DIMS>
inline typename TLine<TYPE, DIMS>::VECTOR
TLine<TYPE, DIMS>::GetNormDir() const {
...
inline TYPE TLine<TYPE, DIMS>::DistanceSq(const POINT &pt) const {
  ...
  if (DIMS == 2)
    return TYPE(SQUARE(l[0] * r[1] - r[0] * l[1]) /
                (l[0] * l[0] + l[1] * l[1]));
  ASSERT(DIMS == 3);
...
// Returns 0 if it coincides with the first point, and 1 if it coincides with
// the second point.
template <typename TYPE, int DIMS>
...
template <typename TYPE, int DIMS>
inline typename TLine<TYPE, DIMS>::POINT
TLine<TYPE, DIMS>::ProjectPoint(const POINT &p) const {
  const VECTOR vL(pt2 - pt1);
  ASSERT(!ISZERO(vL.squaredNorm()));
  const VECTOR vP(p - pt1);
...
TPoint3<TYPEE>
FitLineOnline<TYPE, TYPEW>::GetLine(TLine<TYPEE, 3> &line) const {
