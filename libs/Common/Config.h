...
#if _MSC_VER > 1400
#ifndef NTDDI_VERSION // There's now just one symbol to specify the minimum
                      // target operating system.
#define NTDDI_VERSION                                                          \
  NTDDI_WIN7 // All the other symbols are set automatically to the appropriate
             // values for the target operating system.
#endif
#else
...
#define _CRT_SECURE_CPP_OVERLOAD_SECURE_NAMES                                  \
  0 // disable automatically overloading CPP names to secure versions
#endif
#if 0 && defined(_DEBUG) &&                                                    \
    !defined(_ITERATOR_DEBUG_LEVEL) // might not build if linking statically to
                                    // 3rd party libraries
#define _ITERATOR_DEBUG_LEVEL                                                  \
  1 // disable std iterator debugging even in Debug, as it is very slow
#endif
#endif
...
// Define platform type
    ...
    defined(__arm64__) || defined(__mips64)
#define _ENVIRONMENT64
#else
#define _ENVIRONMENT32
#endif
...
#define _SUPPORT_CPP11
    ...
    (__clang_major__ >= 4 || (__clang_major__ >= 3 && __clang_minor__ >= 4))
...
#define NOINITVTABLE                                                           \
  __declspec(novtable) // disable generating code to initialize the vfptr in the
                       // constructor(s) and destructor of the class
#define DECRESTRICT                                                            \
  __declspec(restrict) // applied to a function declaration or definition that
                       // returns a pointer type and tells the compiler that the
                       // function returns an object that will not be aliased
                       // with any other pointers
#define NOALIAS                                                                \
  __declspec(noalias) // applied to a function declaration or definition that
                      // returns a pointer type and tells the compiler that the
                      // function call does not modify or reference visible
                      // global state and only modifies the memory pointed to
                      // directly by pointer parameters (first-level
                      // indirections)
...
#define HOT                                                                    \
  __attribute__((hot)) __attribute__((optimize("-O3"))) __attribute__((        \
      optimize("-ffast-math")))    // optimize for speed, even in debug
...
#ifdef _MSC_VER
#define _DEBUGINFO
#define _CRTDBG_MAP_ALLOC // enable this to show also the filename (DEBUG_NEW
                          // should also be defined in each file)
#include <crtdbg.h>
#include <cstdlib>
#ifdef _INC_CRTDBG
#define SIMPLE_ASSERT(exp)                                                     \
  {                                                                            \
    if (!(exp) &&                                                              \
        1 == _CrtDbgReport(_CRT_ASSERT, __FILE__, __LINE__, NULL, #exp))       \
      _CrtDbgBreak();                                                          \
  }
#define ASSERT(exp, ...)                                                       \
  {                                                                            \
    ...
    if (!bIgnore && !(exp)) {                                                  \
      PRINT_ASSERT_MSG(exp, ##__VA_ARGS__);                                    \
      if (!(bIgnore = !(1 == _CrtDbgReport(_CRT_ASSERT, __FILE__, __LINE__,    \
                                           NULL, #exp))))                      \
        _CrtDbgBreak();                                                        \
    }                                                                          \
  }
...
#define TRACE(...)                                                             \
  {                                                                            \
    TCHAR buffer[2048];                                                        \
    _sntprintf(buffer, 2048, __VA_ARGS__);                                     \
...
#define STATIC_ASSERT(expression)                                              \
  MAYBEUNUSED typedef char CTA##__LINE__                                       \
      [::SEACAVE_ASSERT::compile_time_assert<(bool)(expression)>::value]

#define ASSERT_ARE_SAME_TYPE(type1, type2)                                     \
  MAYBEUNUSED typedef char AAST##__LINE__                                      \
      [::SEACAVE_ASSERT::assert_are_same_type<type1, type2>::value]

#define ASSERT_ARE_NOT_SAME_TYPE(type1, type2)                                 \
  MAYBEUNUSED typedef char AANST##__LINE__                                     \
      [::SEACAVE_ASSERT::assert_are_not_same_type<type1, type2>::value]
/*----------------------------------------------------------------*/
