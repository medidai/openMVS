...
template <typename TYPE, int DIMS> class TTriangle {
...
public:
  ...
  inline const POINT &operator[](BYTE i) const {
    ASSERT(i < 3);
    return (&a)[i];
  }
  inline POINT &operator[](BYTE i) {
    ASSERT(i < 3);
    return (&a)[i];
  }
}; // class TTriangle
...
public:
  ...
  template <bool bCull> bool Intersects(const TRIANGLE &, TYPE *t) const;
  template <bool bCull>
  bool Intersects(const TRIANGLE &, TYPE fL, TYPE *t) const;
  bool Intersects(const PLANE &plane, bool bCull, TYPE *t, POINT *pPtHit) const;
  inline TYPE IntersectsDist(const PLANE &plane) const;
  inline POINT Intersects(const PLANE &plane) const;
  bool Intersects(const PLANE &plane, bool bCull, TYPE fL, TYPE *t,
                  POINT *pPtHit) const;
...
template <typename TYPE, int DIMS> class TCylinder {
  STATIC_ASSERT(DIMS > 1 && DIMS <= 3);
...
public:
  ...
  TYPE maxHeight; // std::numeric_limits<TYPE>::lowest() <= minHeight <= 0 <
                  // maxHeight <= std::numeric_limits<TYPE>::max()

  //---------------------------------------

  inline TCylinder() {}

  inline TCylinder(const RAY &_ray, TYPE _radius, TYPE _minHeight = TYPE(0),
                   TYPE _maxHeight = std::numeric_limits<TYPE>::max())
      : ray(_ray), radius(_radius), minHeight(_minHeight),
        maxHeight(_maxHeight) {
    ASSERT(TYPE(0) >= minHeight && minHeight < maxHeight);
  }

  inline bool IsFinite() const {
    return maxHeight < std::numeric_limits<TYPE>::max() &&
           minHeight > std::numeric_limits<TYPE>::lowest();
  }
...
// Basic cone class
template <typename TYPE, int DIMS> class TCone {
  STATIC_ASSERT(DIMS > 1 && DIMS <= 3);
...
public:
  ...
  RAY ray; // ray origin is the cone vertex and ray direction is the cone axis
           // direction
  TYPE angle;     // cone angle, in radians, must be inside [0, pi/2]
  TYPE minHeight; // heights satisfying:
  TYPE maxHeight; // 0 <= minHeight < maxHeight <=
                  // std::numeric_limits<TYPE>::max()

  //---------------------------------------

  inline TCone() {}
  inline TCone(const RAY &_ray, TYPE _angle, TYPE _minHeight = TYPE(0),
               TYPE _maxHeight = std::numeric_limits<TYPE>::max())
      : ray(_ray), angle(_angle), minHeight(_minHeight), maxHeight(_maxHeight) {
    ASSERT(TYPE(0) <= minHeight && minHeight < maxHeight);
  }
}; // class TCone

// Structure used to compute the intersection between a cone and the given
// sphere/point
template <typename TYPE, int DIMS> class TConeIntersect {
  STATIC_ASSERT(DIMS > 1 && DIMS <= 3);
...
public:
  ...
  // cache angle derivatives, to avoid calling trigonometric functions in
  // geometric queries
