...
inline TOBB<TYPE, DIMS>::TOBB(bool)
    : m_rot(MATRIX::Identity()), m_pos(POINT::Zero()), m_ext(POINT::Zero()) {}
template <typename TYPE, int DIMS>
inline TOBB<TYPE, DIMS>::TOBB(const AABB &aabb) {
  Set(aabb);
}
template <typename TYPE, int DIMS>
inline TOBB<TYPE, DIMS>::TOBB(const MATRIX &rot, const POINT &ptMin,
                              const POINT &ptMax) {
  Set(rot, ptMin, ptMax);
}
template <typename TYPE, int DIMS>
...
template <typename TYPE, int DIMS>
inline TOBB<TYPE, DIMS>::TOBB(const POINT *pts, size_t n, const TRIANGLE *tris,
                              size_t s) {
  Set(pts, n, tris, s);
} // constructor
template <typename TYPE, int DIMS>
...
inline TOBB<TYPE, DIMS>::TOBB(const TOBB<CTYPE, DIMS> &rhs)
    : m_rot(rhs.m_rot.template cast<TYPE>()),
      ...
      m_ext(rhs.m_ext.template cast<TYPE>()) {} // copy constructor
/*----------------------------------------------------------------*/
...
template <typename TYPE, int DIMS>
inline void TOBB<TYPE, DIMS>::Set(const MATRIX &rot, const POINT &ptMin,
                                  const POINT &ptMax) {
  m_rot = rot;
...
inline void TOBB<TYPE, DIMS>::Set(const POINT *pts, size_t n,
  ...
  for (size_t i = 0; i < s; ++i) {
    ...
    cxx +=
        (TYPE(9) * mui(0) * mui(0) + p(0) * p(0) + q(0) * q(0) + r(0) * r(0)) *
        Ai12;
    cxy +=
        (TYPE(9) * mui(0) * mui(1) + p(0) * p(1) + q(0) * q(1) + r(0) * r(1)) *
        Ai12;
    cxz +=
        (TYPE(9) * mui(0) * mui(2) + p(0) * p(2) + q(0) * q(2) + r(0) * r(2)) *
        Ai12;
    cyy +=
        (TYPE(9) * mui(1) * mui(1) + p(1) * p(1) + q(1) * q(1) + r(1) * r(1)) *
        Ai12;
    cyz +=
        (TYPE(9) * mui(1) * mui(2) + p(1) * p(2) + q(1) * q(2) + r(1) * r(2)) *
        Ai12;
    czz +=
        (TYPE(9) * mui(2) * mui(2) + p(2) * p(2) + q(2) * q(2) + r(2) * r(2)) *
        Ai12;
  }
...
inline void TOBB<TYPE, DIMS>::SetRotation(const MATRIX &C) {
  ...
  ASSERT(es.eigenvalues()(0) < es.eigenvalues()(1) &&
         es.eigenvalues()(1) < es.eigenvalues()(2));
...
inline void TOBB<TYPE, DIMS>::SetBounds(const POINT *pts, size_t n) {
  ASSERT(n >= DIMS);
  ASSERT(ISEQUAL((m_rot * m_rot.transpose()).trace(), TYPE(3)) &&
         ISEQUAL(m_rot.determinant(), TYPE(1)));
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (minim(0) > p_prime(0))
      minim(0) = p_prime(0);
    if (minim(1) > p_prime(1))
      minim(1) = p_prime(1);
    if (minim(2) > p_prime(2))
      minim(2) = p_prime(2);
    if (maxim(0) < p_prime(0))
      maxim(0) = p_prime(0);
    if (maxim(1) < p_prime(1))
      maxim(1) = p_prime(1);
    if (maxim(2) < p_prime(2))
      maxim(2) = p_prime(2);
  }
...
template <typename TYPE, int DIMS> inline void TOBB<TYPE, DIMS>::BuildBegin() {
  m_rot = MATRIX::Zero();
  m_pos = POINT::Zero();
  m_ext = POINT::Zero();
}
...
template <typename TYPE, int DIMS> inline void TOBB<TYPE, DIMS>::BuildEnd() {
...
inline void TOBB<TYPE, DIMS>::GetCorners(POINT pts[numCorners]) const {
  if (DIMS == 2) {
    const POINT pEAxis[2] = {m_rot.row(0) * m_ext[0], m_rot.row(1) * m_ext[1]};
  ...
  if (DIMS == 3) {
    const POINT pEAxis[3] = {m_rot.row(0) * m_ext[0], m_rot.row(1) * m_ext[1],
                             m_rot.row(2) * m_ext[2]};
    const POINT pos(m_rot.transpose() * m_pos);
...
bool TOBB<TYPE, DIMS>::Intersects(const POINT &pt) const {
  ...
  if (DIMS == 2) {
    return ABS(dist[0]) <= m_ext[0] && ABS(dist[1]) <= m_ext[1];
  }
  if (DIMS == 3) {
    return ABS(dist[0]) <= m_ext[0] && ABS(dist[1]) <= m_ext[1] &&
           ABS(dist[2]) <= m_ext[2];
