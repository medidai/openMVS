...
#define DECLARE_LOG()                                                          \
protected:                                                                     \
  static const Log::Idx ms_nLogType;
...
protected:
  ...
  ClbkRecordMsgArrayPtr
      m_arrRecordClbk;      // the array with all registered listeners
...
// streaming
  ...
  StreamMap m_streams; // stream object used to handle one log with operator <<
                       // (one for each thread)
  CriticalSection
      m_cs; // mutex used to ensure multi-thread safety for accessing m_streams
#else
  std::ostringstream
      m_stream; // stream object used to handle one log with operator <<
#endif
#endif
...
protected:
  ...
  bool bManageConsole;       // remember if the console is created here or is an
                             // existing console
};
