...
#if defined(_MSC_VER)
#pragma warning(push)
#pragma warning(disable : 4244) // 'argument': conversion from '__int64' to
...
protected:                                                                     \
  ...
  SClass(const SClass &);                                                      \
                                                                               \
public:                                                                        \
  static inline SClass &GetInstance() {                                        \
    static SClass instance;                                                    \
    return instance;                                                           \
  }
...
protected:                                                                     \
  SClass();                                                                    \
  SClass(const SClass &);                                                      \
  static SClass *ms_pInstance;                                                 \
                                                                               \
public:                                                                        \
  static SClass *&GetInstanceRef();                                            \
                                                                               \
public:                                                                        \
  static SClass *GetInstancePtr();                                             \
                                                                               \
public:                                                                        \
  static SClass &GetInstance();
...
template <typename TYPE, typename REALTYPE = REAL> struct RealType {
  typedef typename std::conditional<std::is_floating_point<TYPE>::value, TYPE,
                                    REALTYPE>::type type;
};

template <typename T> inline T MINF3(const T &x1, const T &x2, const T &x3) {
  return MINF(MINF(x1, x2), x3);
}
template <typename T> inline T MAXF3(const T &x1, const T &x2, const T &x3) {
  return MAXF(MAXF(x1, x2), x3);
}

template <typename T> FORCEINLINE T RANDOM() { return T(RAND()) / RAND_MAX; }

template <typename T1, typename T2> union TAliasCast {
  T1 f;
  T2 i;
  inline TAliasCast() {}
  inline TAliasCast(T1 v) : f(v) {}
  inline TAliasCast(T2 v) : i(v) {}
  inline TAliasCast &operator=(T1 v) {
    f = v;
    return *this;
  }
  inline TAliasCast &operator=(T2 v) {
    i = v;
    return *this;
  }
...
namespace SEACAVE {
// print the given message composed of any number of arguments to the given
// stream
template <typename... Args>
std::ostringstream &PrintMessageToStream(std::ostringstream &oss,
                                         Args &&...args) {
...
template <typename... Args> std::string PrintMessageToString(Args &&...args) {
...
#define RGBC(clr)                                                              \
  (RGBA((BYTE)((clr).fR * 255), (BYTE)((clr).fG * 255),                        \
        (BYTE)((clr).fB * 255), (BYTE)((clr).fA * 255)))
#define RGB24TO8(r, g, b)                                                      \
  ((BYTE)((((WORD)r) * 30 + ((WORD)g) * 59 + ((WORD)b) * 11) / 100))
#define RGB24TO16(r, g, b)                                                     \
  ((((WORD)(((BYTE)(r)) >> 3)) << 11) | (((WORD)(((BYTE)(g)) >> 2)) << 5) |    \
   ((WORD)(((BYTE)(b)) >> 3)))
...
#define TIMER_START()                                                          \
  SEACAVE::Timer::SysType timerStart = SEACAVE::Timer::GetSysTime()
#define TIMER_UPDATE() timerStart = SEACAVE::Timer::GetSysTime()
#define TIMER_GET()                                                            \
  SEACAVE::Timer::SysTime2TimeMs(SEACAVE::Timer::GetSysTime() - timerStart)
...
template <typename T> struct MakeIdentity {
  using type = T;
};
template <typename T>
using MakeSigned =
    typename std::conditional<std::is_integral<T>::value, std::make_signed<T>,
                              SEACAVE::MakeIdentity<T>>::type;

template <typename T1, typename T2> constexpr T1 Cast(const T2 &v) {
  return static_cast<T1>(v);
}

template <typename T> constexpr T &NEGATE(T &a) { return (a = -a); }
...
template <typename T> inline T LOGN(const T &a) { return T(log(a)); }
template <typename T> inline T LOG10(const T &a) { return T(log10(a)); }
template <typename T> constexpr T powi(T base, unsigned exp) {
  T result(1);
  while (exp) {
    if (exp & 1)
      result *= base;
...
template <unsigned N> constexpr inline int log2i() {
  return 1 + log2i<(N >> 1)>();
}
...
template <typename T> inline T arithmeticSeries(T n, T a1 = 1, T d = 1) {
...
template <typename T> constexpr T combinations(const T &n, const T &k) {
  SIMPLE_ASSERT(n >= k);
#if 1
  T num = n;
...
template <bool bSafe> inline float FPOW2(float p) {
  if (bSafe && p < -126.f) {
    return 0.f;
  } else {
    ASSERT(p >= -126.f);
...
template <bool bSafe> inline float FEXP(float v) {
...
// by leveraging the fact that their binary representation is close to a log2
// representation)
...
template <typename T> FORCEINLINE T cbrt_halley(const T &a, const T &R) {
  const T a3 = a * a * a;
  const T a3R = a3 + R;
  return a * (a3R + R) / (a3 + a3R);
}
// fast cubic root (variable precision)
template <typename T, int N> FORCEINLINE T fast_cbrt(const T &x) {
  return cbrt_halley(fast_cbrt<T, N - 1>(x), x);
}
template <> FORCEINLINE double fast_cbrt<double, 1>(const double &x) {
  return cbrt_halley((double)cbrt5((float)x), x);
}
template <> FORCEINLINE float fast_cbrt<float, 1>(const float &x) {
...
FORCEINLINE int PopCnt(uint32_t bb) { return __builtin_popcount(bb); }
FORCEINLINE int PopCnt(uint64_t bb) { return __builtin_popcountll(bb); }
FORCEINLINE int PopCnt15(uint64_t bb) { return __builtin_popcountll(bb); }
FORCEINLINE int PopCntSparse(uint64_t bb) { return __builtin_popcountll(bb); }

#elif defined(_USE_SSE) && defined(_M_AMD64) // 64 bit windows

FORCEINLINE int PopCnt(uint32_t bb) { return (int)_mm_popcnt_u32(bb); }
FORCEINLINE int PopCnt(uint64_t bb) { return (int)_mm_popcnt_u64(bb); }
FORCEINLINE int PopCnt15(uint64_t bb) { return (int)_mm_popcnt_u64(bb); }
FORCEINLINE int PopCntSparse(uint64_t bb) { return (int)_mm_popcnt_u64(bb); }

#else

// general purpose population count
template <typename T> constexpr int PopCnt(T bb) {
  STATIC_ASSERT(std::is_integral<T>::value && std::is_unsigned<T>::value);
  return std::bitset<sizeof(T) * 8>(bb).count();
}
template <> inline int PopCnt(uint64_t bb) {
...
// (xs routines at stereopsis: http://www.stereopsis.com/sree/fpu2006.html by
// Sree Kotay)
const double _float2int_doublemagic =
    6755399441055744.0; // 2^52 * 1.5, uses limited precision to floor
const double _float2int_doublemagicdelta = (1.5e-8);
const double _float2int_doublemagicroundeps =
    (.5f -
     _float2int_doublemagicdelta); // almost .5f = .5f - 1e^(number of exp bit)
...
#endif
template <typename INTTYPE = int> FORCEINLINE INTTYPE Floor2Int(float x) {
#ifdef _FAST_FLOAT2INT
...
template <typename INTTYPE = int> FORCEINLINE INTTYPE Floor2Int(double x) {
#ifdef _FAST_FLOAT2INT
...
template <typename INTTYPE = int> FORCEINLINE INTTYPE Ceil2Int(float x) {
#ifdef _FAST_FLOAT2INT
  return CRound2Int(double(x) + _float2int_doublemagicroundeps);
#else
...
template <typename INTTYPE = int> FORCEINLINE INTTYPE Ceil2Int(double x) {
#ifdef _FAST_FLOAT2INT
  return CRound2Int(x + _float2int_doublemagicroundeps);
#else
  return static_cast<INTTYPE>(ceil(x));
...
template <typename INTTYPE = int> FORCEINLINE INTTYPE Round2Int(float x) {
#ifdef _FAST_FLOAT2INT
...
template <typename INTTYPE = int> FORCEINLINE INTTYPE Round2Int(double x) {
#ifdef _FAST_FLOAT2INT
...
// Cubic interpolation
template <typename Type>
inline Type cerp(const Type &u0, const Type &u1, const Type &u2, const Type &u3,
                 float x) {
...
struct ALIGN(16) sse_vec4f {
  ...
  inline sse_vec4f(float f0, float f1, float f2, float f3)
      : x(f0), y(f1), z(f2), w(f3) {}
...
struct sse_f_t {
  ...
  inline sse_f_t(real_t f0, real_t f1, real_t f2, real_t f3)
      : v(set(f0, f1, f2, f3)) {}
  ...
  static inline sse_t set(real_t f0, real_t f1, real_t f2, real_t f3) {
  ...
  static inline int floor2int(real_t f) {
    return _mm_cvtt_ss2si(_mm_load_ss(&f));
  }
#ifdef _WIN32
  static inline real_t sum(sse_t s) {
    return (s.m128_f32[0] + s.m128_f32[2]) + (s.m128_f32[1] + s.m128_f32[3]);
  }
  static inline real_t sum3(sse_t s) {
    return (s.m128_f32[0] + s.m128_f32[2]) + s.m128_f32[1];
  }
#else
  static inline real_t sum(sse_t s) {
    real_t *f = (real_t *)(&s);
...
public:
  ...
  static inline sse_t set(const real_t &f0, const real_t &f1) {
  ...
  static inline int floor2int(const real_t &f) {
...
inline void sse_prefetch(const void *p) {
...
template <typename _Tp> inline bool ISFINITE(const _Tp *x, size_t n) {
...
template <typename _Tp> inline bool ISINSIDE(_Tp v, _Tp l0, _Tp l1) {
  ASSERT(l0 < l1);
  return l0 <= v && v < l1;
}
template <typename _Tp> inline bool ISINSIDES(_Tp v, _Tp l0, _Tp l1) {
  return l0 < l1 ? ISINSIDE(v, l0, l1) : ISINSIDE(v, l1, l0);
}

template <typename _Tp> inline _Tp CLAMP(_Tp v, _Tp l0, _Tp l1) {
  ASSERT(l0 <= l1);
  return MINF(MAXF(v, l0), l1);
}
template <typename _Tp> inline _Tp CLAMPS(_Tp v, _Tp l0, _Tp l1) {
  return l0 <= l1 ? CLAMP(v, l0, l1) : CLAMP(v, l1, l0);
}

template <typename _Tp> inline _Tp SIGN(_Tp x) {
  if (x > _Tp(0))
    return _Tp(1);
  if (x < _Tp(0))
    return _Tp(-1);
...
template <typename _Tp> inline _Tp ABS(_Tp x) { return std::abs(x); }

template <typename _Tp> constexpr _Tp ZEROTOLERANCE() { return _Tp(0); }
template <> constexpr float ZEROTOLERANCE() { return FZERO_TOLERANCE; }
template <> constexpr double ZEROTOLERANCE() { return ZERO_TOLERANCE; }

template <typename _Tp> constexpr _Tp EPSILONTOLERANCE() {
  return std::numeric_limits<_Tp>::epsilon();
}
template <> constexpr float EPSILONTOLERANCE() { return 0.00001f; }
template <> constexpr double EPSILONTOLERANCE() { return 1e-10; }
...
template <typename _Tp> inline _Tp INVZERO(_Tp) {
  return std::numeric_limits<_Tp>::max();
}

template <typename _Tp> inline _Tp INVERT(_Tp x) {
  return (x == _Tp(0) ? INVZERO(x) : _Tp(1) / x);
}

template <typename _Tp> inline _Tp SAFEDIVIDE(_Tp x, _Tp y) {
  return (y == _Tp(0) ? INVZERO(y) : x / y);
}
/*----------------------------------------------------------------*/
...
// 2D point struct
template <typename TYPE> class TPoint2 : public cv::Point_<TYPE> {
public:
  typedef TYPE Type;
...
public:
  ...
  template <typename T>
  inline TPoint2(const cv::Point_<T> &rhs) : Base((Type)rhs.x, (Type)rhs.y) {}
  template <typename T>
  inline TPoint2(const cv::Matx<T, 2, 1> &rhs) : Base(rhs(0), rhs(1)) {}
  template <typename T>
  inline TPoint2(const cv::Matx<T, 1, 2> &rhs) : Base(rhs(0), rhs(1)) {}
#ifdef _USE_EIGEN
  ...
  explicit inline TPoint2(const cv::Point3_<TYPE> &pt)
      : Base(pt.x / pt.z, pt.y / pt.z) {}

  template <typename T> inline TPoint2 &operator=(const cv::Point_<T> &rhs) {
    Base::operator=(rhs);
    return *this;
  }
  template <typename T>
  inline TPoint2 &operator=(const cv::Matx<T, 2, 1> &rhs) {
    operator Vec &() = rhs;
    return *this;
  }
  template <typename T>
  inline TPoint2 &operator=(const cv::Matx<T, 1, 2> &rhs) {
    operator VecT &() = rhs;
    return *this;
  }
#ifdef _USE_EIGEN
  inline TPoint2 &operator=(const EVec &rhs) {
  ...
  template <typename T> inline operator TPoint2<T>() const {
    return TPoint2<T>((T)x, (T)y);
  }
...
template <typename TYPE>
const TPoint2<TYPE> TPoint2<TYPE>::INF(std::numeric_limits<TYPE>::infinity(),
...
// 3D point struct
template <typename TYPE> class TPoint3 : public cv::Point3_<TYPE> {
public:
  typedef TYPE Type;
...
public:
  inline TPoint3() {}
  template <typename T>
  inline TPoint3(const cv::Point3_<T> &rhs)
      : Base((Type)rhs.x, (Type)rhs.y, (Type)rhs.z) {}
  template <typename T>
  inline TPoint3(const cv::Matx<T, 3, 1> &rhs) : Base(rhs(0), rhs(1), rhs(2)) {}
  template <typename T>
  inline TPoint3(const cv::Matx<T, 1, 3> &rhs) : Base(rhs(0), rhs(1), rhs(2)) {}
#ifdef _USE_EIGEN
  inline TPoint3(const EVec &rhs) { operator EVecMap() = rhs; }
#endif
  explicit inline TPoint3(const TYPE &_x) : Base(_x, _x, _x) {}
  inline TPoint3(const TYPE &_x, const TYPE &_y, const TYPE &_z)
      : Base(_x, _y, _z) {}
  template <typename T>
  inline TPoint3(const cv::Point_<T> &pt, const T &_z = T(1))
      : Base(pt.x, pt.y, _z) {}
  template <typename T1, typename T2>
  inline TPoint3(const cv::Point_<T1> &pt, const T2 &_z)
      : Base(pt.x, pt.y, _z) {}

  template <typename T> inline TPoint3 &operator=(const cv::Point3_<T> &rhs) {
    Base::operator=(rhs);
    return *this;
  }
  template <typename T>
  inline TPoint3 &operator=(const cv::Matx<T, 3, 1> &rhs) {
    operator Vec &() = rhs;
    return *this;
  }
  template <typename T>
  inline TPoint3 &operator=(const cv::Matx<T, 1, 3> &rhs) {
    operator VecT &() = rhs;
    return *this;
  }
#ifdef _USE_EIGEN
  inline TPoint3 &operator=(const EVec &rhs) {
    operator EVecMap() = rhs;
    return *this;
  }
#endif
  ...
  // conversion to another data type
  template <typename T> inline operator TPoint3<T>() const {
    return TPoint3<T>((T)x, (T)y, (T)z);
  }
  ...
  // Access point as transposed vector equivalent
  inline operator const VecT &() const {
  ...
  inline void RotateAngleAxis(const TPoint3 &rot) {
    return (*this) = RotateAngleAxis((*this), rot);
  }
...
template <typename TYPE>
const TPoint3<TYPE> TPoint3<TYPE>::INF(std::numeric_limits<TYPE>::infinity(),
                                       std::numeric_limits<TYPE>::infinity(),
                                       std::numeric_limits<TYPE>::infinity());
/*----------------------------------------------------------------*/
...
public:
  inline TMatrix() {}
  template <typename T>
  inline TMatrix(const cv::Matx<T, m, n> &rhs) : Base(rhs) {}
  template <typename T>
  inline TMatrix(const cv::Point_<T> &rhs) : Base(rhs.x, rhs.y) {}
  template <typename T>
  inline TMatrix(const cv::Point3_<T> &rhs) : Base(rhs.x, rhs.y, rhs.z) {}
  ...
  TMatrix(TYPE v0, TYPE v1, TYPE v2, TYPE v3, TYPE v4,
          TYPE v5); //!< 1x6, 2x3, 3x2 or 6x1 matrix
  TMatrix(TYPE v0, TYPE v1, TYPE v2, TYPE v3, TYPE v4, TYPE v5,
          TYPE v6); //!< 1x7 or 7x1 matrix
  TMatrix(TYPE v0, TYPE v1, TYPE v2, TYPE v3, TYPE v4, TYPE v5, TYPE v6,
          TYPE v7); //!< 1x8, 2x4, 4x2 or 8x1 matrix
  TMatrix(TYPE v0, TYPE v1, TYPE v2, TYPE v3, TYPE v4, TYPE v5, TYPE v6,
          TYPE v7, TYPE v8); //!< 1x9, 3x3 or 9x1 matrix
  TMatrix(TYPE v0, TYPE v1, TYPE v2, TYPE v3, TYPE v4, TYPE v5, TYPE v6,
          TYPE v7, TYPE v8, TYPE v9); //!< 1x10, 2x5 or 5x2 or 10x1 matrix
  TMatrix(TYPE v0, TYPE v1, TYPE v2, TYPE v3, TYPE v4, TYPE v5, TYPE v6,
          TYPE v7, TYPE v8, TYPE v9, TYPE v10,
          TYPE v11); //!< 1x12, 2x6, 3x4, 4x3, 6x2 or 12x1 matrix
  TMatrix(TYPE v0, TYPE v1, TYPE v2, TYPE v3, TYPE v4, TYPE v5, TYPE v6,
          TYPE v7, TYPE v8, TYPE v9, TYPE v10, TYPE v11, TYPE v12,
          TYPE v13); //!< 1x14, 2x7, 7x2 or 14x1 matrix
  TMatrix(TYPE v0, TYPE v1, TYPE v2, TYPE v3, TYPE v4, TYPE v5, TYPE v6,
          TYPE v7, TYPE v8, TYPE v9, TYPE v10, TYPE v11, TYPE v12, TYPE v13,
          TYPE v14, TYPE v15);        //!< 1x16, 4x4 or 16x1 matrix
  explicit TMatrix(const TYPE *vals); //!< initialize from a plain array

  TMatrix(const TMatrix<TYPE, m, n> &a, const TMatrix<TYPE, m, n> &b,
          cv::Matx_AddOp)
      : Base(a, b, cv::Matx_AddOp()) {}
  TMatrix(const TMatrix<TYPE, m, n> &a, const TMatrix<TYPE, m, n> &b,
          cv::Matx_SubOp)
      : Base(a, b, cv::Matx_SubOp()) {}
  template <typename TYPE2>
  TMatrix(const TMatrix<TYPE, m, n> &a, TYPE2 alpha, cv::Matx_ScaleOp)
      : Base(a, alpha, cv::Matx_ScaleOp()) {}
  TMatrix(const TMatrix<TYPE, m, n> &a, const TMatrix<TYPE, m, n> &b,
          cv::Matx_MulOp)
      : Base(a, b, cv::Matx_MulOp()) {}
  TMatrix(const TMatrix<TYPE, m, n> &a, const TMatrix<TYPE, m, n> &b,
          cv::Matx_DivOp)
      : Base(a, b, cv::Matx_DivOp()) {}
  template <int l>
  TMatrix(const TMatrix<TYPE, m, l> &a, const TMatrix<TYPE, l, n> &b,
          cv::Matx_MatMulOp)
      : Base(a, b, cv::Matx_MatMulOp()) {}
  TMatrix(const TMatrix<TYPE, n, m> &a, cv::Matx_TOp)
      : Base(a, cv::Matx_TOp()) {}

  template <typename T>
  inline TMatrix &operator=(const cv::Matx<T, m, n> &rhs) {
    Base::operator=(rhs);
    return *this;
  }
  inline TMatrix &operator=(const cv::Mat &rhs) {
    Base::operator=(rhs);
    return *this;
  }
#ifdef _USE_EIGEN
  inline TMatrix &operator=(const EMat &rhs) {
...
template <typename TYPE, int m, int n>
const TMatrix<TYPE, m, n> TMatrix<TYPE, m, n>::ZERO(TMatrix::zeros());
template <typename TYPE, int m, int n>
const TMatrix<TYPE, m, n> TMatrix<TYPE, m, n>::IDENTITY(TMatrix::eye());
template <typename TYPE, int m, int n>
const TMatrix<TYPE, m, n> TMatrix<TYPE, m, n>::INF(
    TMatrix::all(std::numeric_limits<TYPE>::infinity()));
/*----------------------------------------------------------------*/

// generic matrix struct
template <typename TYPE> class TDMatrix : public cv::Mat_<TYPE> {
public:
  typedef TYPE Type;
...
public:
  ...
  inline TDMatrix(int _rows, int _cols, TYPE *_data,
                  size_t _step = Base::AUTO_STEP)
      : Base(_rows, _cols, _data, _step) {}
  inline TDMatrix(const Size &sz) : Base(sz) {}
  inline TDMatrix(const Size &sz, const TYPE &v) : Base(sz, v) {}
  inline TDMatrix(const Size &sz, TYPE *_data, size_t _step = Base::AUTO_STEP)
      : Base(sz.height, sz.width, _data, _step) {}
#ifdef _USE_EIGEN
  inline TDMatrix(const EMat &rhs) { operator EMatMap() = rhs; }
#endif

  inline TDMatrix &operator=(const Base &rhs) {
    Base::operator=(rhs);
    return *this;
  }
  inline TDMatrix &operator=(const cv::MatExpr &rhs) {
    Base::operator=(rhs);
    return *this;
  }
#ifdef _USE_EIGEN
  inline TDMatrix &operator=(const EMat &rhs) {
    operator EMatMap() = rhs;
    return *this;
  }
#endif
  ...
  inline void memset(uint8_t v) {
    ASSERT(dims == 2 && cv::Mat::isContinuous());
    ::memset(data, v, row_stride() * rows);
  }
  inline void fill(const TYPE &v) {
    ASSERT(dims == 2);
    for (int i = 0; i < rows; ++i)
      for (int j = 0; j < cols; ++j)
        cv::Mat::at<TYPE>(i, j) = v;
  }
  ...
  inline size_t elem_stride() const {
    ASSERT(dims == 2 && step[1] == sizeof(TYPE));
    return step[1];
  }
  ...
  inline size_t memory_size() const {
    return cv::Mat::total() * cv::Mat::elemSize();
  }

  /// Is this coordinate inside the 2D matrix?
  template <typename T>
  static inline typename std::enable_if<std::is_integral<T>::value, bool>::type
  isInside(const cv::Point_<T> &pt, const cv::Size &size) {
    return pt.x >= T(0) && pt.y >= T(0) && pt.x < T(size.width) &&
           pt.y < T(size.height);
  }
  template <typename T>
  static inline
  ...
  template <typename T> inline bool isInside(const cv::Point_<T> &pt) const {
  ...
  inline bool isInsideWithBorder(const Size &pt, int border) const {
    return pt.width >= border && pt.height >= border &&
           pt.width < Base::size().width - border &&
           pt.height < Base::size().height - border;
  }
  template <typename T>
  inline typename std::enable_if<std::is_integral<T>::value, bool>::type
  isInsideWithBorder(const cv::Point_<T> &pt, int border) const {
    return pt.x >= border && pt.y >= border &&
           pt.x < Base::size().width - border &&
           pt.y < Base::size().height - border;
  }
  template <typename T>
  inline typename std::enable_if<std::is_floating_point<T>::value, bool>::type
  isInsideWithBorder(const cv::Point_<T> &pt, int border) const {
    return pt.x >= T(border) && pt.y >= T(border) &&
           pt.x <= T(Base::size().width - (border + 1)) &&
           pt.y <= T(Base::size().height - (border + 1));
  }
  template <typename T, int border>
  inline typename std::enable_if<std::is_integral<T>::value, bool>::type
  isInsideWithBorder(const cv::Point_<T> &pt) const {
    return pt.x >= border && pt.y >= border &&
           pt.x < Base::size().width - border &&
           pt.y < Base::size().height - border;
  }
  ...
  static inline void clip(TPoint2<T> &ptMin, TPoint2<T> &ptMax,
                          const cv::Size &size) {
    if (ptMin.x < T(border))
      ptMin.x = T(border);
    if (ptMin.y < T(border))
  ...
  /// Access an element from the matrix. Bounds checking is only performed in
  /// debug mode.
  ...
  inline const TYPE *getData() const {
    ASSERT(cv::Mat::empty() || cv::Mat::isContinuous());
  ...
  // Access point as Eigen::Map equivalent
  inline operator const CEMatMap() const {
    return CEMatMap(getData(), rows, cols);
  }
...
template <typename TYPE> class TDVector : public TDMatrix<TYPE> {
public:
  typedef TYPE Type;
...
public:
  ...
  inline TDVector(int _rows, TYPE *_data, size_t _step = Base::AUTO_STEP)
      : Base(_rows, 1, _data, _step) {}

  inline TDVector &operator=(const Base &rhs) {
    BaseBase::operator=(rhs);
    return *this;
  }
  inline TDVector &operator=(const BaseBase &rhs) {
    BaseBase::operator=(rhs);
    return *this;
  }
  inline TDVector &operator=(const cv::MatExpr &rhs) {
    BaseBase::operator=(rhs);
    return *this;
  }
  ...
  void getKroneckerProduct(const TDVector<TYPE> &arg,
                           TDVector<TYPE> &dst) const;
};
/*----------------------------------------------------------------*/
...
// For example BGR means that the blue byte is stored first and the red byte
// last, which correspond to the RGB format on a little-endian machine.
...
template <typename TYPE> struct TPixel {
  union {
    struct {
  ...
  inline TPixel() {}
  template <typename T>
  inline TPixel(const TPixel<T> &p)
#if _COLORMODE == _COLORMODE_BGR
...
#if _COLORMODE == _COLORMODE_BGR
      : b(TYPE(col & 0xFF)), g(TYPE((col >> 8) & 0xFF)),
        r(TYPE((col >> 16) & 0xFF)){}
#endif
#if _COLORMODE == _COLORMODE_RGB
      : r(TYPE((col >> 16) & 0xFF)),
  ...
  inline TPixel &set(TYPE _r, TYPE _g, TYPE _b) {
    r = _r;
    g = _g;
    b = _b;
    return *this;
  }
  inline TPixel &set(const TYPE *clr) {
    c[0] = clr[0];
    c[1] = clr[1];
    c[2] = clr[2];
    return *this;
  }
  inline void get(TYPE &_r, TYPE &_g, TYPE &_b) const {
    _r = r;
    _g = g;
    _b = b;
  }
  inline void get(TYPE *clr) const {
    clr[0] = c[0];
    clr[1] = c[1];
    clr[2] = c[2];
  }
  // set/get from alternative type
  inline TPixel &set(ALT _r, ALT _g, ALT _b) {
    r = TYPE(_r);
    g = TYPE(_g);
    b = TYPE(_b);
    return *this;
  }
  inline TPixel &set(const ALT *clr) {
    c[0] = TYPE(clr[0]);
    c[1] = TYPE(clr[1]);
    c[2] = TYPE(clr[2]);
    return *this;
  }
  inline void get(ALT &_r, ALT &_g, ALT &_b) const {
    _r = ALT(r);
    _g = ALT(g);
    _b = ALT(b);
  }
  inline void get(ALT *clr) const {
    clr[0] = ALT(c[0]);
    clr[1] = ALT(c[1]);
    clr[2] = ALT(c[2]);
  }
  template <typename T>
  inline TPixel<typename std::enable_if<!std::is_floating_point<TYPE>::value ||
                                            !std::is_same<T, uint8_t>::value,
                                        T>::type>
  cast() const {
    return TPixel<T>(T(r), T(g), T(b));
  }
  template <typename T>
  inline TPixel<typename std::enable_if<std::is_floating_point<TYPE>::value &&
                                            std::is_same<T, uint8_t>::value,
                                        T>::type>
  cast() const {
    return TPixel<uint8_t>((uint8_t)CLAMP(ROUND2INT(r), 0, 255),
  ...
  template <typename T> inline operator TPoint3<T>() const {
    return TPoint3<T>(T(c[0]), T(c[1]), T(c[2]));
  }
  ...
  inline operator cv::Scalar() const {
    return cv::Scalar(c[0], c[1], c[2], TYPE(0));
  }
  // compare
  inline bool operator==(const TPixel &col) const {
    return (memcmp(c, col.c, sizeof(TPixel)) == 0);
  }
  inline bool operator!=(const TPixel &col) const {
    return (memcmp(c, col.c, sizeof(TPixel)) != 0);
  }
  // operators
  inline TPixel operator*(const TPixel &v) const {
    return TPixel(r * v.r, g * v.g, b * v.b);
  }
  template <typename T> inline TPixel operator*(T v) const {
    return TPixel((TYPE)(v * r), (TYPE)(v * g), (TYPE)(v * b));
  }
  template <typename T> inline TPixel &operator*=(T v) {
  ...
  template <typename T> inline TPixel operator+(T v) const {
    return TPixel((TYPE)(r + v), (TYPE)(g + v), (TYPE)(b + v));
  }
  template <typename T> inline TPixel &operator+=(T v) {
    return (*this = operator+(v));
  }
  ...
  inline uint32_t toDWORD() const {
    return RGBA((uint8_t)r, (uint8_t)g, (uint8_t)b, (uint8_t)0);
  }
  // tools
...
template <>
inline TPixel<float> &TPixel<float>::set(uint8_t _r, uint8_t _g, uint8_t _b) {
  r = float(_r) / 255;
  g = float(_g) / 255;
  b = float(_b) / 255;
  return *this;
...
template <>
inline void TPixel<float>::get(uint8_t &_r, uint8_t &_g, uint8_t &_b) const {
  _r = (uint8_t)CLAMP(ROUND2INT(r * 255), 0, 255);
  _g = (uint8_t)CLAMP(ROUND2INT(g * 255), 0, 255);
  _b = (uint8_t)CLAMP(ROUND2INT(b * 255), 0, 255);
}
template <>
inline TPixel<uint8_t> &TPixel<uint8_t>::set(float _r, float _g, float _b) {
  r = (uint8_t)CLAMP(ROUND2INT(_r * 255), 0, 255);
  g = (uint8_t)CLAMP(ROUND2INT(_g * 255), 0, 255);
  b = (uint8_t)CLAMP(ROUND2INT(_b * 255), 0, 255);
  return *this;
}
template <>
inline void TPixel<uint8_t>::get(float &_r, float &_g, float &_b) const {
  _r = float(r) / 255;
  _g = float(g) / 255;
  _b = float(b) / 255;
}
...
template <typename TYPE> struct TColor {
  union {
    struct {
#if _COLORMODE == _COLORMODE_BGR
      TYPE b;
  ...
  inline TColor() {}
  template <typename T>
  inline TColor(const TColor<T> &p)
  ...
  explicit inline TColor(uint32_t col)
      : r(TYPE((col >> 16) & 0xFF)), g(TYPE((col >> 8) & 0xFF)),
        b(TYPE(col & 0xFF)), a(TYPE((col >> 24) & 0xFF)) {}
  // set/get from default type
  inline TColor &set(TYPE _r, TYPE _g, TYPE _b,
                     TYPE _a = ColorType<TYPE>::ONE) {
    r = _r;
    g = _g;
    b = _b;
    a = _a;
  ...
  inline TColor &set(const TYPE *clr) {
    c[0] = clr[0];
    c[1] = clr[1];
    c[2] = clr[2];
    c[3] = clr[3];
  ...
  inline void get(TYPE &_r, TYPE &_g, TYPE &_b, TYPE &_a) const {
    _r = r;
    _g = g;
    _b = b;
    _a = a;
  ...
  // set/get from alternative type
  inline TColor &set(ALT _r, ALT _g, ALT _b, ALT _a = ColorType<TYPE>::ALTONE) {
    r = TYPE(_r);
    g = TYPE(_g);
    b = TYPE(_b);
    a = TYPE(_a);
  ...
  inline TColor &set(const ALT *clr) {
    c[0] = TYPE(clr[0]);
    c[1] = TYPE(clr[1]);
    c[2] = TYPE(clr[2]);
    c[3] = TYPE(clr[3]);
  ...
  inline void get(ALT &_r, ALT &_g, ALT &_b, ALT &_a) const {
    _r = ALT(r);
    _g = ALT(g);
    _b = ALT(b);
    _a = ALT(a);
  ...
  template <typename T>
  inline TColor<typename std::enable_if<!std::is_floating_point<TYPE>::value ||
                                            !std::is_same<T, uint8_t>::value,
                                        T>::type>
  cast() const {
    return TColor<T>(T(r), T(g), T(b), T(a));
  }
  template <typename T>
  inline TColor<typename std::enable_if<std::is_floating_point<TYPE>::value &&
                                            std::is_same<T, uint8_t>::value,
                                        T>::type>
  cast() const {
    return TColor<uint8_t>((uint8_t)CLAMP(ROUND2INT(r), 0, 255),
                           (uint8_t)CLAMP(ROUND2INT(g), 0, 255),
                           (uint8_t)CLAMP(ROUND2INT(b), 0, 255),
  ...
  // access as cv::Scalar equivalent
  inline operator cv::Scalar() const {
    return cv::Scalar(c[0], c[1], c[2], c[3]);
  }
  // compare
  inline bool operator==(const TColor &col) const {
    return (memcmp(c, col.c, sizeof(TColor)) == 0);
  }
  inline bool operator!=(const TColor &col) const {
    return (memcmp(c, col.c, sizeof(TColor)) != 0);
  }
  ...
  template <typename T> inline TColor operator*(T v) const {
    return TColor((TYPE)(v * r), (TYPE)(v * g), (TYPE)(v * b), (TYPE)(v * a));
  }
  template <typename T> inline TColor &operator*=(T v) {
    return (*this = operator*(v));
  }
  inline TColor operator/(const TColor &v) const {
    return TColor(r / v.r, g / v.g, b / v.b, a / v.a);
  ...
  template <typename T> inline TColor operator+(T v) const {
    return TColor((TYPE)(r + v), (TYPE)(g + v), (TYPE)(b + v), (TYPE)(a + v));
  }
  template <typename T> inline TColor &operator+=(T v) {
    return (*this = operator+(v));
  }
...
template <>
inline TColor<float> &TColor<float>::set(uint8_t _r, uint8_t _g, uint8_t _b,
                                         uint8_t _a) {
  r = float(_r) / 255;
  g = float(_g) / 255;
  b = float(_b) / 255;
...
template <>
inline void TColor<float>::get(uint8_t &_r, uint8_t &_g, uint8_t &_b,
                               uint8_t &_a) const {
  _r = uint8_t(r * 255);
  _g = uint8_t(g * 255);
  _b = uint8_t(b * 255);
...
template <>
inline TColor<uint8_t> &TColor<uint8_t>::set(float _r, float _g, float _b,
                                             float _a) {
  r = uint8_t(_r * 255);
  g = uint8_t(_g * 255);
  b = uint8_t(_b * 255);
...
template <>
inline void TColor<uint8_t>::get(float &_r, float &_g, float &_b,
                                 float &_a) const {
  _r = float(r) / 255;
  _g = float(g) / 255;
  _b = float(b) / 255;
...
template <> inline bool TColor<uint8_t>::operator==(const TColor &col) const {
  return (*((const uint32_t *)c) == *((const uint32_t *)col.c));
}
template <> inline bool TColor<uint8_t>::operator!=(const TColor &col) const {
  return (*((const uint32_t *)c) != *((const uint32_t *)col.c));
}
template <> inline uint32_t TColor<uint8_t>::toDWORD() const {
...
template <typename TYPE> class TImage : public TDMatrix<TYPE> {
public:
  typedef TYPE Type;
  typedef TDMatrix<TYPE> Base;
  typedef cv::Size Size;
...
public:
  ...
  inline TImage(const Size &sz, TYPE *_data, size_t _step = Base::AUTO_STEP)
      : Base(sz.height, sz.width, _data, _step) {}
#ifdef _SUPPORT_CPP11
  inline TImage(cv::Mat &&rhs) : Base(std::forward<cv::Mat>(rhs)) {}

  inline TImage &operator=(cv::Mat &&rhs) {
    BaseBase::operator=(std::forward<cv::Mat>(rhs));
    return *this;
  }
#endif
  inline TImage &operator=(const Base &rhs) {
    BaseBase::operator=(rhs);
    return *this;
  }
  inline TImage &operator=(const BaseBase &rhs) {
    BaseBase::operator=(rhs);
    return *this;
  }
  inline TImage &operator=(const cv::MatExpr &rhs) {
    BaseBase::operator=(rhs);
    return *this;
  }
  ...
  template <typename T> TYPE sample(const TPoint2<T> &pt) const;
  template <typename T> TYPE sampleSafe(const TPoint2<T> &pt) const;
  ...
  TYPE sample(const TPoint2<T> &pt, const Functor &functor,
              const TYPE &dv) const;
  template <typename T, typename Functor>
  TYPE sampleSafe(const TPoint2<T> &pt, const Functor &functor,
                  const TYPE &dv) const;

  template <typename SAMPLER, typename INTERTYPE>
  INTERTYPE sample(const SAMPLER &sampler,
                   const TPoint2<typename SAMPLER::Type> &pt) const;

  template <typename T>
  void toGray(TImage<T> &out, int code, bool bNormalize = false,
              bool bSRGB = false) const;

  static cv::Size computeResize(const cv::Size &size, REAL scale);
  static cv::Size computeResize(const cv::Size &size, REAL scale,
                                unsigned resizes);
  unsigned computeMaxResolution(unsigned &level, unsigned minImageSize = 320,
                                unsigned maxImageSize = INT_MAX) const;
  static unsigned computeMaxResolution(unsigned width, unsigned height,
                                       unsigned &level,
                                       unsigned minImageSize = 320,
                                       unsigned maxImageSize = INT_MAX);

  template <typename T, typename PARSER>
  static void RasterizeTriangle(const TPoint2<T> &v1, const TPoint2<T> &v2,
                                const TPoint2<T> &v3, PARSER &parser);
  template <typename T, typename PARSER, bool CULL = true>
  static void RasterizeTriangleBary(const TPoint2<T> &v1, const TPoint2<T> &v2,
                                    const TPoint2<T> &v3, PARSER &parser);
  template <typename T, typename PARSER>
  static void RasterizeTriangleDepth(TPoint3<T> p1, TPoint3<T> p2,
                                     TPoint3<T> p3, PARSER &parser);

  template <typename T, typename PARSER>
  static void DrawLine(const TPoint2<T> &p1, const TPoint2<T> &p2,
                       PARSER &parser);

  typedef void(STCALL *FncDrawPointAntialias)(const ImageRef &,
                                              const ImageRef &, float, float,
                                              void *);
  static bool DrawLineAntialias(Point2f x1, Point2f x2,
                                FncDrawPointAntialias fncDrawPoint,
                                void *pData = NULL);
  static bool DrawLineAntialias(const ImageRef &x1, const ImageRef &x2,
                                FncDrawPointAntialias fncDrawPoint,
                                void *pData = NULL);
...
template <typename TYPE> class TBitMatrix {
public:
  typedef TYPE Type;
  typedef cv::Size Size;
  struct Index {
...
public:
  ...
  inline TBitMatrix(int _rows, int _cols = 1)
      : rows(_rows), cols(_cols),
        data(rows && cols ? new Type[computeLength(rows, cols)] : NULL) {}
  inline TBitMatrix(int _rows, int _cols, uint8_t v)
      : rows(_rows), cols(_cols),
        data(rows && cols ? new Type[computeLength(rows, cols)] : NULL) {
    if (!empty())
      memset(v);
  }
  inline TBitMatrix(const Size &sz)
      : rows(sz.height), cols(sz.width),
        data(rows && cols ? new Type[computeLength(sz)] : NULL) {}
  inline TBitMatrix(const Size &sz, uint8_t v)
      : rows(sz.height), cols(sz.width),
        data(rows && cols ? new Type[computeLength(sz)] : NULL) {
    if (!empty())
      memset(v);
  ...
  inline void memset(uint8_t v) {
    ASSERT(!empty());
    ::memset(data, v, sizeof(Type) * length());
  }
  ...
  inline bool isSet(int i) const {
    ASSERT(!empty() && i < area());
    const Index idx(computeIndex(i));
    return (data[idx.idx] & idx.flag) != 0;
  }
  inline bool isSet(int i, int j) const {
    ASSERT(!empty() && i < rows && j < cols);
    const Index idx(computeIndex(i, j, cols));
    return (data[idx.idx] & idx.flag) != 0;
  }
  inline bool isSet(const ImageRef &ir) const {
    ASSERT(!empty() && ir.y < rows && ir.x < cols);
    const Index idx(computeIndex(ir, cols));
    return (data[idx.idx] & idx.flag) != 0;
  }
  ...
  inline void set(const ImageRef &ir, bool v) {
    if (v)
      set(ir);
    else
      unset(ir);
  ...
  inline void set(const ImageRef &ir) {
    ASSERT(!empty() && ir.y < rows && ir.x < cols);
    const Index idx(computeIndex(ir, cols));
    data[idx.idx] |= idx.flag;
  }
  ...
  inline void unset(const ImageRef &ir) {
    ASSERT(!empty() && ir.y < rows && ir.x < cols);
    const Index idx(computeIndex(ir, cols));
    data[idx.idx] &= ~idx.flag;
  }
  ...
  inline void flip(const ImageRef &ir) {
    ASSERT(!empty() && ir.y < rows && ir.x < cols);
    const Index idx(computeIndex(ir, cols));
    data[idx.idx] ^= idx.flag;
  }
  ...
  inline bool isInside(const Size &pt) const {
    return pt.width >= 0 && pt.height >= 0 && pt.width < size().width &&
           pt.height < size().height;
  }
  template <typename T> inline bool isInside(const cv::Point_<T> &pt) const {
    return int(pt.x) >= 0 && int(pt.y) >= 0 && int(pt.x) < size().width &&
  ...
  inline bool isInsideWithBorder(const Size &pt, int border) const {
    return pt.width >= border && pt.height >= border &&
           pt.width < size().width - border &&
           pt.height < size().height - border;
  }
  template <typename T>
  inline bool isInsideWithBorder(const cv::Point_<T> &pt, int border) const {
    return int(pt.x) >= border && int(pt.y) >= border &&
           int(pt.x) < size().width - border &&
           int(pt.y) < size().height - border;
  }
  template <typename T, int border>
  ...
  static inline int computeLength(int size) {
    return (size + numBitsPerCell - 1) >> numBitsShift;
  }
  static inline int computeLength(int _rows, int _cols) {
    return computeLength(_rows * _cols);
  }
  static inline int computeLength(const Size &sz) {
    return computeLength(sz.area());
  }

  static inline Index computeIndex(int i) {
    return Index(i >> numBitsShift, Type(1) << (i & (numBitsPerCell - 1)));
  }
  static inline Index computeIndex(int i, int j, int stride) {
    return computeIndex(i * stride + j);
  }
  static inline Index computeIndex(const ImageRef &ir, int stride) {
    return computeIndex(ir.y * stride + ir.x);
  }
...
struct TAccumulator {
  ...
  inline TAccumulator()
      : value(INITTO(static_cast<Type *>(NULL), 0)), weight(0) {}
  inline TAccumulator(const Type &v, const WeightType &w)
      : value(v), weight(w) {}
  ...
  inline TAccumulator operator+(const TAccumulator &accum) const {
    return TAccumulator(value + accum.value, weight + accum.weight);
  }
  ...
  inline AccumType NormalizedFull() const { return value / weight; }
  inline Type Normalized() const { return Type(NormalizedFull()); }
#ifdef _USE_BOOST
  // implement BOOST serialization
  template <class Archive>
  void serialize(Archive &ar, const unsigned int /*version*/) {
...
// structure used for sorting some indices by their score (decreasing by
// default)
template <typename IndexType, typename ScoreType> struct TIndexScore {
  IndexType idx;
  ScoreType score;
  inline TIndexScore() {}
  inline TIndexScore(IndexType _idx, ScoreType _score)
      : idx(_idx), score(_score) {}
  // compare by index (increasing)
  inline bool operator<(IndexType i) const { return (idx < i); }
  inline bool operator==(IndexType i) const { return (idx == i); }
  // compare by score (decreasing)
  inline bool operator<(const TIndexScore &r) const {
    return (score > r.score);
  }
  inline bool operator==(const TIndexScore &r) const {
    return (score == r.score);
  }
  static bool STCALL CompareByIndex(const TIndexScore &l,
                                    const TIndexScore &r) {
    return (l.idx < r.idx);
  }
  static bool STCALL CompareByScore(const TIndexScore &l,
                                    const TIndexScore &r) {
...
#endif
        // get index
        inline operator PairIndex() const {
    return idx;
  }
...
inline PairIdx MakePairIdx(uint32_t idxImageA, uint32_t idxImageB) {
  return (idxImageA < idxImageB ? PairIdx(idxImageA, idxImageB)
                                : PairIdx(idxImageB, idxImageA));
}
/*----------------------------------------------------------------*/
...
template <typename TYPE, int m, int n>
inline String cvMat2String(const TMatrix<TYPE, m, n> &mat,
                           LPCSTR format = "% 10.4f ") {
  return cvMat2String(cv::Mat(mat), format);
}
template <typename TYPE>
inline String cvMat2String(const TPoint3<TYPE> &pt,
                           LPCSTR format = "% 10.4f ") {
  return cvMat2String(cv::Mat(pt), format);
}
/*----------------------------------------------------------------*/
...
/// rotation matrices are members of the Special Orthogonal Lie group SO3. This
/// group can be parameterized three numbers (a vector in the space of the Lie
/// Algebra). In this class, the three parameters are the finite rotation
/// vector, i.e. a three-dimensional vector whose direction is the axis of
/// rotation and whose length is the angle of rotation in radians.
/// Exponentiating this vector gives the matrix, and the logarithm of the matrix
/// gives this vector.
...
public:
  ...
  /// creates an SO3 as a rotation that takes Vector a into the direction of
  /// Vector b with the rotation axis along a ^ b. If |a ^ b| == 0, it creates
  /// the identity rotation. An assertion will fail if Vector a and Vector b are
  ...
  SO3(const Vec3 &a, const Vec3 &b) {
    ...
    if (nrmSq == Precision(0)) {
      ...
      // if not, this means that the rotation is 180 degrees, which leads to an
      // ambiguity in the rotation axis
  ...
  void coerce() {
    ...
    mat.row(2).normalize();
    // check for positive determinant <=> right handed coordinate system of row
    // vectors
  ...
  /// the Lie Algebra. See the Detailed Description for details of this vector.
  inline Vec3 ln() const;

  /// Right-multiply by another rotation matrix
  template <typename P> inline SO3 &operator*=(const SO3<P> &rhs) {
    *this = *this * rhs;
    return *this;
  }
...
/// rotation matrices are members of the Special Orthogonal Lie group SO2. This
/// group can be parameterized with one number (the rotation angle).
...
public:
  ...
  inline void coerce() {
    mat.row(0).normalize();
    mat.row(1) =
        (mat.row(1) - mat.row(0) * (mat.row(0).dot(mat.row(1)))).normalized();
  }
  ...
  inline static Mat2 generator() {
    Mat2 result;
    result(0, 0) = Precision(0);
    result(0, 1) = Precision(-1);
    result(1, 0) = Precision(1);
    result(1, 1) = Precision(0);
    return result;
  }
