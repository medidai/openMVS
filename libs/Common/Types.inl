...
template <typename T> struct hash<cv::Point_<T>> {
...
template <typename T> struct hash<cv::Point3_<T>> {
...
template <> struct hash<SEACAVE::PairIdx> {
  size_t operator()(const SEACAVE::PairIdx &v) const {
    return std::hash<SEACAVE::PairIdx::PairIndex>()(v.idx);
  }
};

// adds the given key-value pair in the map, overwriting the current value if
// the key exists
...
INT_TYPE cvRANSACUpdateNumIters(REAL_TYPE p, REAL_TYPE ep, INT_TYPE modelPoints,
                                INT_TYPE maxIters) {
  ASSERT(p >= 0 && p <= 1);
  ASSERT(ep >= 0 && ep <= 1);
  // avoid inf's & nan's
  REAL_TYPE num =
      MAXF(REAL_TYPE(1) - p, SEACAVE::EPSILONTOLERANCE<REAL_TYPE>());
  ...
  return (denom >= 0 || -num >= (-denom) * maxIters
              ? maxIters
              : ROUND2INT<INT_TYPE>(num / denom));
}
#endif
...
// Convenience creation functions. In the far future, there may be variadic
// templates here.
...
#define CV_HELP_WRAP_PROPERTY(r_type, w_type, name, internal_name,             \
                              internal_obj)                                    \
...
#define CV_WRAP_SAME_PROPERTY(type, name, internal_obj)                        \
  CV_WRAP_PROPERTY(type, name, name, internal_obj)
#define CV_WRAP_SAME_PROPERTY_S(type, name, internal_obj)                      \
...
const typename ColorType<TYPE>::value_type ColorType<TYPE>::ONE(1);
template <typename TYPE>
const typename ColorType<TYPE>::alt_type ColorType<TYPE>::ALTONE(1);

template <typename TYPE> const TPixel<TYPE> TPixel<TYPE>::BLACK(0, 0, 0);
template <typename TYPE>
const TPixel<TYPE> TPixel<TYPE>::WHITE(ColorType<TYPE>::ONE,
                                       ColorType<TYPE>::ONE,
                                       ColorType<TYPE>::ONE);
template <typename TYPE>
const TPixel<TYPE> TPixel<TYPE>::GRAY(0.8f * ColorType<TYPE>::ONE,
                                      0.8f * ColorType<TYPE>::ONE,
                                      0.8f * ColorType<TYPE>::ONE);
template <typename TYPE>
const TPixel<TYPE> TPixel<TYPE>::RED(ColorType<TYPE>::ONE, 0, 0);
template <typename TYPE>
const TPixel<TYPE> TPixel<TYPE>::GREEN(0, ColorType<TYPE>::ONE, 0);
template <typename TYPE>
const TPixel<TYPE> TPixel<TYPE>::BLUE(0, 0, ColorType<TYPE>::ONE);
template <typename TYPE>
const TPixel<TYPE> TPixel<TYPE>::YELLOW(ColorType<TYPE>::ONE,
                                        ColorType<TYPE>::ONE, 0);
template <typename TYPE>
const TPixel<TYPE> TPixel<TYPE>::MAGENTA(ColorType<TYPE>::ONE, 0,
                                         ColorType<TYPE>::ONE);
template <typename TYPE>
const TPixel<TYPE> TPixel<TYPE>::CYAN(0, ColorType<TYPE>::ONE,
                                      ColorType<TYPE>::ONE);

template <typename TYPE>
const TColor<TYPE> TColor<TYPE>::BLACK(0, 0, 0, ColorType<TYPE>::ONE);
template <typename TYPE>
const TColor<TYPE>
    TColor<TYPE>::WHITE(ColorType<TYPE>::ONE, ColorType<TYPE>::ONE,
                        ColorType<TYPE>::ONE, ColorType<TYPE>::ONE);
template <typename TYPE>
const TColor<TYPE>
    TColor<TYPE>::GRAY(0.8f * ColorType<TYPE>::ONE, 0.8f * ColorType<TYPE>::ONE,
                       0.8f * ColorType<TYPE>::ONE, ColorType<TYPE>::ONE);
template <typename TYPE>
const TColor<TYPE> TColor<TYPE>::RED(ColorType<TYPE>::ONE, 0, 0,
                                     ColorType<TYPE>::ONE);
template <typename TYPE>
const TColor<TYPE> TColor<TYPE>::GREEN(0, ColorType<TYPE>::ONE, 0,
                                       ColorType<TYPE>::ONE);
template <typename TYPE>
const TColor<TYPE> TColor<TYPE>::BLUE(0, 0, ColorType<TYPE>::ONE,
                                      ColorType<TYPE>::ONE);
template <typename TYPE>
const TColor<TYPE> TColor<TYPE>::YELLOW(ColorType<TYPE>::ONE,
                                        ColorType<TYPE>::ONE, 0,
                                        ColorType<TYPE>::ONE);
template <typename TYPE>
const TColor<TYPE> TColor<TYPE>::MAGENTA(ColorType<TYPE>::ONE, 0,
                                         ColorType<TYPE>::ONE,
                                         ColorType<TYPE>::ONE);
template <typename TYPE>
const TColor<TYPE> TColor<TYPE>::CYAN(0, ColorType<TYPE>::ONE,
                                      ColorType<TYPE>::ONE,
                                      ColorType<TYPE>::ONE);
...
FORCEINLINE SEACAVE::TMatrix<TYPE, m, n>
SQUARE(const SEACAVE::TMatrix<TYPE, m, n> &v) {
...
FORCEINLINE SEACAVE::TColor<TYPE> SQUARE(const SEACAVE::TColor<TYPE> &v) {
  return SEACAVE::TColor<TYPE>(SQUARE(v.r), SQUARE(v.g), SQUARE(v.b),
                               SQUARE(v.a));
}
...
template <typename TYPE, int m, int n>
FORCEINLINE SEACAVE::TMatrix<TYPE, m, n>
SQRT(const SEACAVE::TMatrix<TYPE, m, n> &v) {
  SEACAVE::TMatrix<TYPE, m, n> nv;
  for (int i = 0; i < m * n; ++i)
    nv(i) = SQRT(v.val[i]);
  return nv;
...
template <typename TYPE, int m, int n>
FORCEINLINE SEACAVE::TMatrix<TYPE, m, n>
EXP(const SEACAVE::TMatrix<TYPE, m, n> &v) {
  SEACAVE::TMatrix<TYPE, m, n> nv;
  for (int i = 0; i < m * n; ++i)
    nv(i) = EXP(v.val[i]);
  return nv;
...
template <typename TYPE, int m, int n>
FORCEINLINE SEACAVE::TMatrix<TYPE, m, n>
LOGN(const SEACAVE::TMatrix<TYPE, m, n> &v) {
  SEACAVE::TMatrix<TYPE, m, n> nv;
  for (int i = 0; i < m * n; ++i)
    nv(i) = LOGN(v.val[i]);
  return nv;
...
template <typename TYPE, int m, int n>
FORCEINLINE SEACAVE::TMatrix<TYPE, m, n>
ABS(const SEACAVE::TMatrix<TYPE, m, n> &v) {
  SEACAVE::TMatrix<TYPE, m, n> nv;
  for (int i = 0; i < m * n; ++i)
    nv(i) = ABS(v.val[i]);
  return nv;
...
// ISZERO
template <typename TYPE> FORCEINLINE bool ISZERO(const cv::Point_<TYPE> &v) {
  return (ISZERO(v.x) && ISZERO(v.y));
}
template <typename TYPE> FORCEINLINE bool ISZERO(const cv::Point3_<TYPE> &v) {
...
// ISEQUAL
template <typename TYPE>
FORCEINLINE bool ISEQUAL(const cv::Point_<TYPE> &v1,
                         const cv::Point_<TYPE> &v2) {
  return (ISEQUAL(v1.x, v2.x) && ISEQUAL(v1.y, v2.y));
}
template <typename TYPE>
FORCEINLINE bool ISEQUAL(const cv::Point3_<TYPE> &v1,
                         const cv::Point3_<TYPE> &v2) {
  return (ISEQUAL(v1.x, v2.x) && ISEQUAL(v1.y, v2.y) && ISEQUAL(v1.z, v2.z));
}
template <typename TYPE, int m, int n>
FORCEINLINE bool ISEQUAL(const cv::Matx<TYPE, m, n> &v1,
                         const cv::Matx<TYPE, m, n> &v2) {
  for (int i = 0; i < m * n; ++i)
...
FORCEINLINE SEACAVE::TPoint2<INTTYPE> Floor2Int(const cv::Point_<TYPE> &v) {
  return SEACAVE::TPoint2<INTTYPE>(FLOOR2INT<INTTYPE>(v.x),
                                   FLOOR2INT<INTTYPE>(v.y));
}
template <typename TYPE, typename INTTYPE = int>
FORCEINLINE SEACAVE::TPoint2<INTTYPE> Ceil2Int(const cv::Point_<TYPE> &v) {
  return SEACAVE::TPoint2<INTTYPE>(CEIL2INT<INTTYPE>(v.x),
                                   CEIL2INT<INTTYPE>(v.y));
}
template <typename TYPE, typename INTTYPE = int>
FORCEINLINE SEACAVE::TPoint2<INTTYPE> Round2Int(const cv::Point_<TYPE> &v) {
  return SEACAVE::TPoint2<INTTYPE>(ROUND2INT<INTTYPE>(v.x),
                                   ROUND2INT<INTTYPE>(v.y));
}

template <typename TYPE, typename INTTYPE = int>
FORCEINLINE SEACAVE::TPoint3<INTTYPE> Floor2Int(const cv::Point3_<TYPE> &v) {
  return SEACAVE::TPoint3<INTTYPE>(FLOOR2INT<INTTYPE>(v.x),
                                   FLOOR2INT<INTTYPE>(v.y),
                                   FLOOR2INT<INTTYPE>(v.z));
}
template <typename TYPE, typename INTTYPE = int>
FORCEINLINE SEACAVE::TPoint3<INTTYPE> Ceil2Int(const cv::Point3_<TYPE> &v) {
  return SEACAVE::TPoint3<INTTYPE>(
      CEIL2INT<INTTYPE>(v.x), CEIL2INT<INTTYPE>(v.y), CEIL2INT<INTTYPE>(v.z));
}
template <typename TYPE, typename INTTYPE = int>
FORCEINLINE SEACAVE::TPoint3<INTTYPE> Round2Int(const cv::Point3_<TYPE> &v) {
  return SEACAVE::TPoint3<INTTYPE>(ROUND2INT<INTTYPE>(v.x),
                                   ROUND2INT<INTTYPE>(v.y),
                                   ROUND2INT<INTTYPE>(v.z));
}

template <typename TYPE, int m, int n, typename INTTYPE = int>
FORCEINLINE SEACAVE::TMatrix<INTTYPE, m, n>
Floor2Int(const cv::Matx<TYPE, m, n> &v) {
...
template <typename TYPE, int m, int n, typename INTTYPE = int>
FORCEINLINE SEACAVE::TMatrix<INTTYPE, m, n>
Ceil2Int(const cv::Matx<TYPE, m, n> &v) {
  SEACAVE::TMatrix<INTTYPE, m, n> nv;
  for (int i = 0; i < m * n; ++i)
...
template <typename TYPE, int m, int n, typename INTTYPE = int>
FORCEINLINE SEACAVE::TMatrix<INTTYPE, m, n>
Round2Int(const cv::Matx<TYPE, m, n> &v) {
  SEACAVE::TMatrix<INTTYPE, m, n> nv;
  for (int i = 0; i < m * n; ++i)
...
template <typename TYPE> FORCEINLINE bool ISFINITE(const cv::Point_<TYPE> &v) {
  return (ISFINITE(v.x) && ISFINITE(v.y));
}
template <typename TYPE> FORCEINLINE bool ISFINITE(const cv::Point3_<TYPE> &v) {
  return (ISFINITE(v.x) && ISFINITE(v.y) && ISFINITE(v.z));
}
template <typename TYPE, int m, int n>
...
FORCEINLINE Eigen::Matrix<Scalar, Rows, Cols>
INITTO(const Eigen::Matrix<Scalar, Rows, Cols> *, Value v) {
  return Eigen::Matrix<Scalar, Rows, Cols>::Constant(static_cast<Scalar>(v));
}
/*----------------------------------------------------------------*/

template <typename TYPE> inline IDX MinIndex(const TYPE *data, IDX size) {
  ASSERT(size > 0);
  IDX idx = 0;
...
template <typename TYPE> inline IDX MaxIndex(const TYPE *data, IDX size) {
  ASSERT(size > 0);
  IDX idx = 0;
  for (IDX i = 1; i < size; ++i)
    if (data[idx] < data[i])
...
inline ACCTYPE dot(const TYPE *a, const TYPE *b, size_t n) {
  ...
  while (a < lastgroup) {
    s += ACCTYPE(a[0]) * ACCTYPE(b[0]) + ACCTYPE(a[1]) * ACCTYPE(b[1]) +
         ACCTYPE(a[2]) * ACCTYPE(b[2]) + ACCTYPE(a[3]) * ACCTYPE(b[3]);
    a += 4;
    b += 4;
  }
#endif
...
template <>
inline double dot<double, double>(const double *l, const double *r,
                                  size_t size) {
...
inline float dot(float v1, float v2) { return v1 * v2; }
inline double dot(double v1, double v2) { return v1 * v2; }
template <typename TYPE>
inline TYPE dot(const cv::Point_<TYPE> *v1, const cv::Point_<TYPE> *v2,
                size_t size) {
  return dot((const TYPE *)v1, (const TYPE *)v2, size * 2);
}
template <typename TYPE>
inline TYPE dot(const cv::Point3_<TYPE> *v1, const cv::Point3_<TYPE> *v2,
                size_t size) {
  return dot((const TYPE *)v1, (const TYPE *)v2, size * 3);
}
template <typename TYPE, int m>
inline TYPE dot(const cv::Vec<TYPE, m> *v1, const cv::Vec<TYPE, m> *v2,
                size_t size) {
  return dot((const TYPE *)v1, (const TYPE *)v2, size * m);
}

template <typename TYPE, int m, int n>
inline TMatrix<TYPE, m, n> cross(const TMatrix<TYPE, m, n> &l,
                                 const TMatrix<TYPE, m, n> &r) {
  return ((const typename TMatrix<TYPE, m * n, 1>::Vec &)l)
      .cross((const typename TMatrix<TYPE, m * n, 1>::Vec &)r);
}
template <typename TYPE, int m, int n>
inline TMatrix<TYPE, m, n> cross(const cv::Matx<TYPE, m, n> &l,
                                 const cv::Matx<TYPE, m, n> &r) {
  return ((const typename TMatrix<TYPE, m * n, 1>::Vec &)l)
      .cross((const typename TMatrix<TYPE, m * n, 1>::Vec &)r);
}
...
inline typename RealType<TYPE>::type normSq(const TDMatrix<TYPE> &v) {
  typedef typename RealType<TYPE>::type real;
  return cv::normL2Sqr<TYPE, real>(v.cv::Mat::template ptr<const TYPE>(),
                                   v.area());
}
template <typename TYPE>
...
inline REAL normSq(int v) { return SQUARE((REAL)v); }
inline REAL normSq(unsigned v) { return SQUARE((REAL)v); }
inline float normSq(float v) { return SQUARE(v); }
inline double normSq(double v) { return SQUARE(v); }
template <typename TYPE>
inline typename RealType<TYPE>::type normSq(const cv::Point_<TYPE> *v,
                                            size_t size) {
  typedef typename RealType<TYPE>::type real;
...
template <typename TYPE, int m>
inline typename RealType<TYPE>::type normSq(const cv::Vec<TYPE, m> *v,
                                            size_t size) {
  typedef typename RealType<TYPE>::type real;
...
inline ACCTYPE normSq(const TYPE *a, size_t n) {
  ...
  const TYPE *const lastgroup = last - 3;
  while (a < lastgroup) {
    s += SQUARE(ACCTYPE(a[0])) + SQUARE(ACCTYPE(a[1])) + SQUARE(ACCTYPE(a[2])) +
         SQUARE(ACCTYPE(a[3]));
    a += 4;
  }
#endif
...
inline ACCTYPE normSq(const TYPE *a, const TYPE *b, size_t n) {
  ...
  const TYPE *const lastgroup = last - 3;
  while (a < lastgroup) {
    const INTTYPE v0(a[0] - b[0]), v1(a[1] - b[1]), v2(a[2] - b[2]),
        v3(a[3] - b[3]);
...
inline ACCTYPE normSqDelta(TYPE *a, size_t n, const ACCTYPE avg) {
  ...
  const TYPE *const lastgroup = last - 3;
  while (a < lastgroup) {
    const ACCTYPE v0(a[0] -= avg), v1(a[1] -= avg), v2(a[2] -= avg),
        v3(a[3] -= avg);
    s += v0 * v0 + v1 * v1 + v2 * v2 + v3 * v3;
    a += 4;
  }
...
inline typename RealType<TYPE>::type norm(const TDMatrix<TYPE> &v) {
  typedef typename RealType<TYPE>::type real;
  return SQRT(cv::normL2Sqr<TYPE, real>(v.cv::Mat::template ptr<const TYPE>(),
                                        v.area()));
}
...
template <typename TYPE> inline void normalize(TPoint2<TYPE> &v) {
  (typename TPoint2<TYPE>::cvVec &)v =
      cv::normalize((const typename TPoint2<TYPE>::cvVec &)v);
}
template <typename TYPE> inline void normalize(cv::Point_<TYPE> &v) {
  (typename TPoint2<TYPE>::cvVec &)v =
      cv::normalize((const typename TPoint2<TYPE>::cvVec &)v);
}
template <typename TYPE> inline void normalize(TPoint3<TYPE> &v) {
  (typename TPoint3<TYPE>::cvVec &)v =
      cv::normalize((const typename TPoint3<TYPE>::cvVec &)v);
}
template <typename TYPE> inline void normalize(cv::Point3_<TYPE> &v) {
  (typename TPoint3<TYPE>::cvVec &)v =
      cv::normalize((const typename TPoint3<TYPE>::cvVec &)v);
}
template <typename TYPE, int m, int n>
inline void normalize(TMatrix<TYPE, m, n> &v) {
  (typename TMatrix<TYPE, m * n, 1>::Vec &)v =
      cv::normalize((const typename TMatrix<TYPE, m * n, 1>::Vec &)v);
}
template <typename TYPE, int m, int n>
...
inline bool equal(const _Tp &left, const _Tp &right,
                  const _Tp & = std::numeric_limits<_Tp>::epsilon()) {
  return (left == right);
}
template <>
...
template <>
inline bool equal<float>(const float &left, const float &right,
                         const float &eps) {
  // the code below does not work when one value is zero
  if (left == 0. || right == 0.)
    return (ABS(left - right) <= eps);
...
/** @brief comparison function for floating point values */
template <typename _Tp> // left < right
inline bool less(const _Tp &left, const _Tp &right,
...
/** @brief comparison function for floating point values */
template <typename _Tp> // left <= right
inline bool lessEqual(const _Tp &left, const _Tp &right,
                      const _Tp &eps = std::numeric_limits<_Tp>::epsilon()) {
...
/** @brief comparison function for floating point values */
template <typename _Tp> // left > right
inline bool greater(const _Tp &left, const _Tp &right,
                    const _Tp &eps = std::numeric_limits<_Tp>::epsilon()) {
  if (left == 0. || right == 0.)
...
/** @brief comparison function for floating point values */
template <typename _Tp> // left >= right
inline bool greaterEqual(const _Tp &left, const _Tp &right,
                         const _Tp &eps = std::numeric_limits<_Tp>::epsilon()) {
...
#endif // WIN32
template <typename _Tp> inline unsigned decimalsToStore() {
  if (std::numeric_limits<_Tp>::is_integer) {
    // integer types can display exactly one decimal more than their guaranteed
    // precision digits
...
template <typename TYPE, typename TYPEM>
inline TPoint2<TYPE> operator/(const cv::Point_<TYPE> &pt0,
                               const cv::Point_<TYPEM> &pt1) {
  return TPoint2<TYPE>(pt0.x / pt1.x, pt0.y / pt1.y);
}
template <typename TYPE, typename TYPEM>
inline TPoint2<TYPE> &operator/=(cv::Point_<TYPE> &pt0,
                                 const cv::Point_<TYPEM> &pt1) {
  pt0.x /= pt1.x;
  pt0.y /= pt1.y;
...
template <typename TYPE, typename TYPEM>
inline TPoint3<TYPE> operator/(const cv::Point3_<TYPE> &pt0,
                               const cv::Point3_<TYPEM> &pt1) {
  return TPoint3<TYPE>(pt0.x / pt1.x, pt0.y / pt1.y, pt0.z / pt1.z);
}
template <typename TYPE, typename TYPEM>
inline TPoint3<TYPE> &operator/=(cv::Point3_<TYPE> &pt0,
                                 const cv::Point3_<TYPEM> &pt1) {
  pt0.x /= pt1.x;
  pt0.y /= pt1.y;
  pt0.z /= pt1.z;
...
// TPixel operators
template <typename TYPE>
inline TPixel<TYPE> operator/(const TPixel<TYPE> &pt0,
                              const TPixel<TYPE> &pt1) {
...
// TColor operators
template <typename TYPE>
inline TColor<TYPE> operator/(const TColor<TYPE> &pt0,
                              const TColor<TYPE> &pt1) {
  return TColor<TYPE>(pt0.r / pt1.r, pt0.g / pt1.g, pt0.b / pt1.b,
                      pt0.a / pt1.a);
}
template <typename TYPE>
...
// TMatrix operators
template <typename TYPE, int m, int n>
inline TMatrix<TYPE, m, n> operator+(const TMatrix<TYPE, m, n> &m1,
                                     const TMatrix<TYPE, m, n> &m2) {
  return TMatrix<TYPE, m, n>(m1, m2, cv::Matx_AddOp());
}
template <typename TYPE, int m, int n>
inline TMatrix<TYPE, m, n> operator+(const TMatrix<TYPE, m, n> &m1,
                                     const cv::Matx<TYPE, m, n> &m2) {
  return cv::Matx<TYPE, m, n>(m1, m2, cv::Matx_AddOp());
}
template <typename TYPE, int m, int n>
inline TMatrix<TYPE, m, n> operator+(const cv::Matx<TYPE, m, n> &m1,
                                     const TMatrix<TYPE, m, n> &m2) {
  return TMatrix<TYPE, m, n>(m1, m2, cv::Matx_AddOp());
}
...
inline TMatrix<TYPE, m, n> operator*(const TMatrix<TYPE, m, l> &m1,
                                     const TMatrix<TYPE, l, n> &m2) {
...
inline TMatrix<TYPE, m, n> operator/(const TMatrix<TYPE, m, n> &mat, TYPE2 v) {
  typedef typename std::conditional<std::is_floating_point<TYPE2>::value, TYPE2,
                                    REAL>::type real_t;
...
// operator /
template <typename TYPE, typename TYPEM>
FORCEINLINE SEACAVE::TPoint2<TYPE> operator/(TYPEM n,
                                             const cv::Point_<TYPE> &d) {
  return SEACAVE::TPoint2<TYPE>(n / d.x, n / d.y);
}
template <typename TYPE, typename TYPEM>
FORCEINLINE SEACAVE::TPoint3<TYPE> operator/(TYPEM n,
                                             const cv::Point3_<TYPE> &d) {
  return SEACAVE::TPoint3<TYPE>(n / d.x, n / d.y, n / d.z);
}
template <typename TYPE, typename TYPEM>
FORCEINLINE SEACAVE::TPixel<TYPE> operator/(TYPEM n,
                                            const SEACAVE::TPixel<TYPE> &d) {
  return SEACAVE::TPixel<TYPE>(n / d.r, n / d.g, n / d.b);
}
template <typename TYPE, typename TYPEM>
FORCEINLINE SEACAVE::TColor<TYPE> operator/(TYPEM n,
                                            const SEACAVE::TColor<TYPE> &d) {
...
#define DEFINE_GENERIC_CVDATATYPE(tp, ctp)                                     \
  namespace cv {                                                               \
...
#define DEFINE_GENERIC_CVDATADEPTH(tp, ctp)                                    \
  namespace cv {                                                               \
  template <> class DataDepth<tp> {                                            \
  public:                                                                      \
    enum { value = DataDepth<ctp>::value, fmt = DataDepth<ctp>::fmt };         \
  };                                                                           \
  }
...
template <typename T> constexpr T sRGB2RGB(T x) {
  return x <= T(0.04045) ? x * (T(1) / T(12.92))
                         : POW((x + T(0.055)) * (T(1) / T(1.055)), T(2.4));
}
template <typename T> constexpr T RGB2sRGB(T x) {
  return x <= T(0.0031308) ? T(12.92) * x
                           : T(1.055) * POW(x, T(1) / T(2.4)) - T(0.055);
}
...
template <typename TI, typename TO> struct NormRGB_t {
  const TO v;
  inline NormRGB_t(TI _v) : v(TO(_v) * (TO(1) / TO(255))) {}
  inline operator TO() const { return v; }
};
template <typename TI, typename TO> struct RGBUnNorm_t {
  const TO v;
  inline RGBUnNorm_t(TI _v) : v(TO(_v) * TO(255)) {}
  inline operator TO() const { return v; }
};
template <typename TI = float, typename TO = uint8_t> struct RoundF2U_t {
  const TO v;
  inline RoundF2U_t(TI _v) : v(ROUND2INT<TO>(_v)) {}
  inline operator TO() const { return v; }
};
template <typename TI, typename TO> struct sRGB2RGB_t {
  const TO v;
  inline sRGB2RGB_t(TI _v) : v(sRGB2RGB(TO(_v))) {}
  inline operator TO() const { return v; }
};
template <typename TI, typename TO> struct NormsRGB2RGB_t {
  const TO v;
  inline NormsRGB2RGB_t(TI _v) : v(sRGB2RGB(TO(_v) * (TO(1) / TO(255)))) {}
  inline operator TO() const { return v; }
};
template <> struct NormsRGB2RGB_t<uint8_t, float> {
  const float v;
  inline NormsRGB2RGB_t(uint8_t _v) : v(g_ptrsRGB82RGBf[_v]) {}
  inline operator float() const { return v; }
};
template <typename TI, typename TO> struct NormsRGB2RGBUnNorm_t {
  const TO v;
  inline NormsRGB2RGBUnNorm_t(TI _v)
      : v(sRGB2RGB(TO(_v) * (TO(1) / TO(255))) * TO(255)) {}
  inline operator TO() const { return v; }
};
template <> struct NormsRGB2RGBUnNorm_t<uint8_t, float> {
  const float v;
...
template <typename FLT> struct OppositeType {
  typedef FLT Type;
};
template <> struct OppositeType<float> {
  typedef double Type;
};
template <> struct OppositeType<double> {
  typedef float Type;
};
// Point2
...
template <typename TYPE>
TPoint3<TYPE> TPoint3<TYPE>::RotateAngleAxis(const TPoint3 &pt,
                                             const TPoint3 &angle_axis) {
  TPoint3 result;
  ...
  if (theta2 > TYPE(1e-12)) {
    ...
    result = pt * costheta + w_cross_pt * sintheta +
             w * (TYPE(1.0) - costheta) * w_dot_pt;
  } else {
...
template <typename TYPE, int m, int n>
inline TMatrix<TYPE, m, n>::TMatrix(TYPE v0, TYPE v1, TYPE v2, TYPE v3, TYPE v4,
                                    TYPE v5, TYPE v6, TYPE v7, TYPE v8, TYPE v9,
                                    TYPE v10, TYPE v11) {
...
template <typename TYPE, int m, int n>
inline TMatrix<TYPE, n, n - m>
TMatrix<TYPE, m, n>::RightNullSpace(int flags /*= 0*/) const {
  STATIC_ASSERT(n > m);
  const cv::SVD svd(*this, flags | cv::SVD::FULL_UV);
  // the orthonormal basis of the null space is formed by the columns
  // of svd.vt such that the corresponding singular values are 0 (n - m or
  // svd.vt.cols - svd.w.rows)
...
template <typename TYPE, int m, int n>
inline TMatrix<TYPE, n, 1>
TMatrix<TYPE, m, n>::RightNullVector(int flags /*= 0*/) const {
...
template <typename TYPE, int m, int n>
inline TMatrix<TYPE, m, 1>
TMatrix<TYPE, m, n>::LeftNullVector(int flags /*= 0*/) const {
...
template <typename TYPE> inline TYPE TDMatrix<TYPE>::getDetSquare() const {
  ASSERT(cols == rows);
  const int dim = rows;
...
template <typename TYPE> inline void TDMatrix<TYPE>::makeSymmetric() {
  ASSERT(cols == rows);
  const int num = cols;
  for (int r = 0; r < num; r++) {
    for (int c = r; c < num; c++) {
      Base::operator()(c, r) = Base::operator()(r, c) =
          TYPE((Base::operator()(c, r) + Base::operator()(r, c)) * 0.5);
    }
  }
}

template <typename TYPE> inline TYPE TDMatrix<TYPE>::getNormL1() const {
  TYPE result = 0;
  for (const TYPE *dataP = getData() + Base::total() - 1; dataP >= getData();
       dataP--)
    result += ABS(*dataP);
  return result;
}

template <typename TYPE> inline double TDMatrix<TYPE>::getNormL2() const {
  double result = 0;
  for (const TYPE *dataP = getData() + Base::total() - 1; dataP >= getData();
       dataP--)
    ...
    for (int j = 0; j < A_cols; j++) {
      for (int r = 0; r < B_rows; r++) {
        for (int s = 0; s < B_cols; s++) {
          dest(i * B_rows + r, j * B_cols + s) =
              Base::operator()(i, j) * B(r, s);
        }
      }
    }
  }
...
template <typename TYPE> void TDMatrix<TYPE>::SwapRows(int i, int r) {
...
template <typename TYPE> void TDMatrix<TYPE>::GaussJordan() {
  ...
  for (int r = 0; r < max; r++) { ///<< r is the working row
    ...
    for (; r + offs < numc && Base::operator()(r, r + offs) == 0.; offs++) {
      ...
      int rr;
      for (rr = r + 1; rr < numr && Base::operator()(rr, r + offs) == 0.;
           rr++) {
      }
      if (rr != numr) {
        MDOUT("found next non-zero element in column "
              << r + offs << " and row " << rr << ", swapping rows" << endl);
        SwapRows(r, rr);
        break;
      }
      MDOUT("only zeros below [" << r << "][" << r + offs
                                 << "], increasing offset\n");
    }
    if (r + offs == numc) {
    ...
    //	the current row
    MDOUT("eliminitaing entries in working column ("
          << r + offs << ") below working row (" << r << ")\n");
...
template <typename TYPE>
void TDVector<TYPE>::getKroneckerProduct(const TDVector<TYPE> &arg,
...
template <typename TYPE> TPixel<TYPE> TPixel<TYPE>::gray2color(WT gray) {
  ...
  const auto Base = [&Interpolate](WT val) -> WT {
    if (val <= WT(0.125))
      return WT(0);
    if (val <= WT(0.375))
      return Interpolate(WT(2) * val - WT(1), WT(0), WT(-0.75), WT(1),
                         WT(-0.25));
    if (val <= WT(0.625))
      return WT(1);
    if (val <= WT(0.87))
  ...
  return TPixel<TYPE>().set(Base(gray + WT(0.25)), Base(gray),
                            Base(gray - WT(0.25)));
}

// Generate random color
template <typename TYPE> TPixel<TYPE> TPixel<TYPE>::random() {
...
TYPE TImage<TYPE>::sample(const TPoint2<T> &pt) const {
  ...
  return (BaseBase::operator()(ly, lx) * x1 +
          BaseBase::operator()(ly, lx + 1) * x) *
             y1 +
         (BaseBase::operator()(ly + 1, lx) * x1 +
          BaseBase::operator()(ly + 1, lx + 1) * x) *
             y;
}
template <typename TYPE>
template <typename T>
...
// sample by bilinear interpolation, using only pixels that meet the user
// condition
template <typename TYPE>
template <typename T, typename TV, typename Functor>
bool TImage<TYPE>::sample(TV &v, const TPoint2<T> &pt,
                          const Functor &functor) const {
  const int lx((int)pt.x);
  const int ly((int)pt.y);
  const T x(pt.x - lx), x1(T(1) - x);
  const T y(pt.y - ly), y1(T(1) - y);
  const TYPE &x0y0(BaseBase::operator()(ly, lx));
  const bool b00(functor(x0y0));
  const TYPE &x1y0(BaseBase::operator()(ly, lx + 1));
  const bool b10(functor(x1y0));
  const TYPE &x0y1(BaseBase::operator()(ly + 1, lx));
  const bool b01(functor(x0y1));
  const TYPE &x1y1(BaseBase::operator()(ly + 1, lx + 1));
  const bool b11(functor(x1y1));
  if (!b00 && !b10 && !b01 && !b11)
    return false;
  v = TV(y1 * (x1 * Cast<T>(b00 ? x0y0 : (b10 ? x1y0 : (b01 ? x0y1 : x1y1))) +
               x * Cast<T>(b10 ? x1y0 : (b00 ? x0y0 : (b11 ? x1y1 : x0y1)))) +
         y * (x1 * Cast<T>(b01 ? x0y1 : (b11 ? x1y1 : (b00 ? x0y0 : x1y0))) +
              x * Cast<T>(b11 ? x1y1 : (b01 ? x0y1 : (b10 ? x1y0 : x0y0)))));
  return true;
}
template <typename TYPE>
template <typename T, typename TV, typename Functor>
bool TImage<TYPE>::sampleSafe(TV &v, const TPoint2<T> &pt,
                              const Functor &functor) const {
  const int lx((int)pt.x);
  const int ly((int)pt.y);
  const T x(pt.x - lx), x1(T(1) - x);
  const T y(pt.y - ly), y1(T(1) - y);
  const TYPE &x0y0(getPixel(ly, lx));
  const bool b00(functor(x0y0));
  const TYPE &x1y0(getPixel(ly, lx + 1));
  const bool b10(functor(x1y0));
  const TYPE &x0y1(getPixel(ly + 1, lx));
  const bool b01(functor(x0y1));
  const TYPE &x1y1(getPixel(ly + 1, lx + 1));
  const bool b11(functor(x1y1));
  if (!b00 && !b10 && !b01 && !b11)
    return false;
  v = TV(y1 * (x1 * Cast<T>(b00 ? x0y0 : (b10 ? x1y0 : (b01 ? x0y1 : x1y1))) +
...
TYPE TImage<TYPE>::sample(const TPoint2<T> &pt, const Functor &functor,
                          const TYPE &dv) const {
  const int lx((int)pt.x);
  const int ly((int)pt.y);
  const T x(pt.x - lx), x1(T(1) - x);
  ...
  const bool b11(functor(x1y1));
  return TYPE(
      y1 * (x1 * Cast<T>(b00 ? x0y0 : dv) + x * Cast<T>(b10 ? x1y0 : dv)) +
      y * (x1 * Cast<T>(b01 ? x0y1 : dv) + x * Cast<T>(b11 ? x1y1 : dv)));
}
template <typename TYPE>
template <typename T, typename Functor>
TYPE TImage<TYPE>::sampleSafe(const TPoint2<T> &pt, const Functor &functor,
                              const TYPE &dv) const {
  const int lx((int)pt.x);
  const int ly((int)pt.y);
  const T x(pt.x - lx), x1(T(1) - x);
...
// same as above, sample image at a specified position, but using the given
// sampler
#include "Sampler.inl"
template <typename TYPE>
template <typename SAMPLER, typename INTERTYPE>
INTERTYPE
TImage<TYPE>::sample(const SAMPLER &sampler,
                     const TPoint2<typename SAMPLER::Type> &pt) const {
  return Sampler::Sample<TImage<TYPE>, SAMPLER, TPoint2<typename SAMPLER::Type>,
                         INTERTYPE>(*this, sampler, pt);
}

// convert color image to gray
template <typename TYPE>
template <typename T>
void TImage<TYPE>::toGray(TImage<T> &out, int code, bool bNormalize,
                          bool bSRGB) const {
#if 1
  typedef typename RealType<T, float>::type Real;
  ASSERT(code == cv::COLOR_RGB2GRAY || code == cv::COLOR_RGBA2GRAY ||
  ...
  if (bSRGB) {
    if (bNormalize) {
      ...
      for (const ST *src = cv::Mat::template ptr<ST>(); dst != dstEnd;
           src += scn)
        *dst++ = T(cb * ColConv(src[0]) + cg * ColConv(src[1]) +
                   cr * ColConv(src[2]));
    } else {
...
cv::Size TImage<TYPE>::computeResize(const cv::Size &size, REAL scale) {
  return cv::Size(cv::saturate_cast<int>((REAL)size.width * scale),
...
template <typename TYPE>
unsigned TImage<TYPE>::computeMaxResolution(unsigned width, unsigned height,
                                            unsigned &level,
...
template <typename TYPE>
unsigned TImage<TYPE>::computeMaxResolution(unsigned &level,
                                            unsigned minImageSize,
                                            unsigned maxImageSize) const {
  return computeMaxResolution((unsigned)width(), (unsigned)height(), level,
                              minImageSize, maxImageSize);
}
/*----------------------------------------------------------------*/

// Raster the given triangle and output the position of each pixel of the
// triangle; based on "Advanced Rasterization" by Nick (Nicolas Capens)
// http://devmaster.net/forums/topic/1145-advanced-rasterization
template <typename TYPE>
template <typename T, typename PARSER>
void TImage<TYPE>::RasterizeTriangle(const TPoint2<T> &v1, const TPoint2<T> &v2,
...
void TImage<TYPE>::RasterizeTriangleBary(const TPoint2<T> &v1,
                                         const TPoint2<T> &v2,
                                         const TPoint2<T> &v3, PARSER &parser) {
  ...
  if (boxMax.x < T(0) || boxMin.x > T(size.width - 1) || boxMax.y < T(0) ||
      boxMin.y > T(size.height - 1))
    return;
...
template <typename T, typename PARSER>
inline void _ProcessScanLine(int y, const TPoint3<T> &pa, const TPoint3<T> &pb,
                             const TPoint3<T> &pc, const TPoint3<T> &pd,
                             PARSER &parser) {
  // Thanks to current Y, we can compute the gradient to compute others values
  // like the starting X (sx) and ending X (ex) to draw between if pa.y == pb.y
  // or pc.y == pd.y, gradient is forced to 1
  const T gradient1 =
      CLAMP(pa.y != pb.y ? (T(y) - pa.y) / (pb.y - pa.y) : T(1), T(0), T(1));
  const T gradient2 =
      CLAMP(pc.y != pd.y ? (T(y) - pc.y) / (pd.y - pc.y) : T(1), T(0), T(1));
...
// Raster the given triangle and output the position and depth of each pixel of
// the triangle; based on "Learning how to write a 3D software engine �
// Rasterization & Z-Buffering" by Nick (David Rousset)
...
template <typename TYPE>
template <typename T, typename PARSER>
void TImage<TYPE>::RasterizeTriangleDepth(TPoint3<T> p1, TPoint3<T> p2,
                                          TPoint3<T> p3, PARSER &parser) {
  // sorting the points in order to always have this order on screen p1, p2 & p3
  // with p1 always up (thus having the Y the lowest possible to be near the top
  // screen) then p2 between p1 & p3
  if (p1.y > p2.y)
    std::swap(p1, p2);
  if (p2.y > p3.y)
    std::swap(p2, p3);
...
template <typename TYPE>
template <typename T, typename PARSER>
void TImage<TYPE>::DrawLine(const TPoint2<T> &p1, const TPoint2<T> &p2,
                            PARSER &parser) {
#if 0
...
template <typename TYPE> bool TImage<TYPE>::Load(const String &fileName) {
...
template <typename TYPE> bool TImage<TYPE>::Save(const String &fileName) const {
  ...
  try {
    ...
    VERBOSE("error: saving image '%s' (exception: %s)", fileName.c_str(),
            ex.what());
    return false;
  }
  return true;
...
// derivative of an image by convolving with the kernel horizontally and
// vertically
inline const TMatrix<double, 3, 3> &CreateDerivativeKernel3x3() {
  static const double kernel[3 * 3] = {-1.0 / 8.0, 0.0, 1.0 / 8.0,
                                       -2.0 / 8.0, 0.0, 2.0 / 8.0,
                                       -1.0 / 8.0, 0.0, 1.0 / 8.0};
  return *((const TMatrix<double, 3, 3> *)kernel);
}
inline const TMatrix<double, 3, 3> &CreateDerivativeKernel3x3xx() {
  static const double kernel[3 * 3] = {1.0 / 6.0, -2.0 / 6.0, 1.0 / 6.0,
...
// same as above, but using central differences like approach: Noise Robust
// Gradient Operators (see:
...
inline const TMatrix<double, 5, 7> &CreateDerivativeKernel5x7() {
  static const double kernel[5 * 7] = {
      -1.0 / 512.0,  -4.0 / 512.0, -5.0 / 512.0,  0.0,           5.0 / 512.0,
      4.0 / 512.0,   1.0 / 512.0,  -4.0 / 512.0,  -16.0 / 512.0, 20.0 / 512.0,
      0.0,           20.0 / 512.0, 16.0 / 512.0,  4.0 / 512.0,   -6.0 / 512.0,
      -24.0 / 512.0, 30.0 / 512.0, 0.0,           30.0 / 512.0,  24.0 / 512.0,
      6.0 / 512.0,   -4.0 / 512.0, -16.0 / 512.0, 20.0 / 512.0,  0.0,
      20.0 / 512.0,  16.0 / 512.0, 4.0 / 512.0,   -1.0 / 512.0,  -4.0 / 512.0,
      -5.0 / 512.0,  0.0,          5.0 / 512.0,   4.0 / 512.0,   1.0 / 512.0};
...
inline int ComputeGaussianKernelWidth(double sigma) {
  ...
  const double precisehalfwidth(
      GaussianInversePositive(truncation_factor, sigma));
...
void Convolve(const TImage<Type> &in, const DVector64F &kernel,
              TImage<Type> &out) {
...
#undef static_convolution
                default : for (int y = 0; y < height; ++y) {
      for (int x = 0; x < width; ++x) {
        double sum = 0;
        ...
        for (int k = -half_width; k <= half_width; ++k) {
          if (vertical) {
            if (y + k >= 0 && y + k < height) {
              sum += double(src[k * src_line_stride]) *
                     kernel(2 * half_width - (k + half_width));
            }
          } else {
            if (x + k >= 0 && x + k < width) {
              sum += double(src[k * src_stride]) *
                     kernel(2 * half_width - (k + half_width));
            }
          }
        }
...
template <typename Type>
inline void ConvolveHorizontal(const TImage<Type> &in, const DVector64F &kernel,
                               TImage<Type> &out) {
  Convolve<Type, false>(in, kernel, out);
}
template <typename Type>
inline void ConvolveVertical(const TImage<Type> &in, const DVector64F &kernel,
                             TImage<Type> &out) {
...
template <typename Type>
void BlurredImage(const TImage<Type> &in, TImage<Type> &blurred,
                  double sigma = 0.32) {
...
template <typename Type>
void BlurredImageAndDerivatives(const TImage<Type> &in, TImage<Type> &blurred,
                                TImage<Type> grad[2], double sigma = 0.32) {
  ASSERT(in.channels() == 1);
...
template <typename TYPE> TYPE InvertMatrix3x3(const TYPE *m, TYPE *mi) {
  const TYPE d(m[0] * (m[4] * m[8] - m[5] * m[7]) +
               m[1] * (m[5] * m[6] - m[3] * m[8]) +
               m[2] * (m[3] * m[7] - m[4] * m[6]));
  if (d == TYPE(0))
    return TYPE(0);
...
inline void eigen_SO3_exp(const typename Eigen::SO3<Precision>::Vec3 &w,
                          typename Eigen::SO3<Precision>::Mat3 &R) {
  ...
  if (theta_sq < Precision(1e-8)) {
    ...
    if (theta_sq < Precision(1e-6)) {
      ...
      A = Precision(1) -
          theta_sq * one_6th * (Precision(1) - one_20th * theta_sq);
    } else {
...
template <typename Precision>
inline typename Eigen::SO3<Precision>::Mat3
Eigen::SO3<Precision>::exp(const Vec3 &w) const {
  Mat3 result;
...
/// Lie Algebra. See the Detailed Description for details of this vector.
template <typename Precision>
inline void eigen_SO3_ln(const typename Eigen::SO3<Precision>::Mat3 &R,
                         typename Eigen::SO3<Precision>::Vec3 &w) {
  const Precision cos_angle((R(0, 0) + R(1, 1) + R(2, 2) - Precision(1)) *
                            Precision(0.5));
...
inline Eigen::Matrix<P, 3, 1, O>
operator*(const Eigen::SO3<P> &lhs, const Eigen::Matrix<P, 3, 1, O> &rhs) {
  return lhs.get_matrix() * rhs;
}
/// Left-multiply by a Vector
/// @relates SO3
template <typename P, int O>
inline Eigen::Matrix<P, 3, 1, O> operator*(const Eigen::Matrix<P, 3, 1, O> &lhs,
                                           const Eigen::SO3<P> &rhs) {
  return lhs * rhs.get_matrix();
}
/// Right-multiply by a matrix
/// @relates SO3
template <typename P, int C, int O>
inline Eigen::Matrix<P, 3, C, O>
operator*(const Eigen::SO3<P> &lhs, const Eigen::Matrix<P, 3, C, O> &rhs) {
  return lhs.get_matrix() * rhs;
}
/// Left-multiply by a matrix
/// @relates SO3
template <typename P, int R, int O>
inline Eigen::Matrix<P, R, 3, O> operator*(const Eigen::Matrix<P, R, 3, O> &lhs,
                                           const Eigen::SO3<P> &rhs) {
  return lhs * rhs.get_matrix();
}
/*----------------------------------------------------------------*/
...
template <typename Precision>
inline void eigen_SO2_exp(const Precision &d,
                          typename Eigen::SO2<Precision>::Mat2 &R) {
  R(0, 0) = R(1, 1) = cos(d);
  R(1, 0) = sin(d);
  R(0, 1) = -R(1, 0);
}
template <typename Precision>
inline typename Eigen::SO2<Precision>::Mat2
Eigen::SO2<Precision>::exp(const Precision &d) const {
  Mat2 result;
...
template <typename Precision>
inline void eigen_SO2_ln(const typename Eigen::SO2<Precision>::Mat2 &R,
                         Precision &d) {
  d = atan2(R(1, 0), R(0, 0));
}
template <typename Precision>
...
/// @relates SO2
template <typename P, int O>
inline Eigen::Matrix<P, 2, 1, O>
operator*(const Eigen::SO2<P> &lhs, const Eigen::Matrix<P, 2, 1, O> &rhs) {
  return lhs.get_matrix() * rhs;
}
/// Left-multiply by a Vector
/// @relates SO2
template <typename P, int O>
inline Eigen::Matrix<P, 2, 1, O> operator*(const Eigen::Matrix<P, 2, 1, O> &lhs,
                                           const Eigen::SO2<P> &rhs) {
  return lhs * rhs.get_matrix();
}
/// Right-multiply by a Matrix
/// @relates SO2
template <typename P, int C, int O>
inline Eigen::Matrix<P, 2, C, O>
operator*(const Eigen::SO2<P> &lhs, const Eigen::Matrix<P, 2, C, O> &rhs) {
  return lhs.get_matrix() * rhs;
}
/// Left-multiply by a Matrix
/// @relates SO2
template <typename P, int R, int O>
inline Eigen::Matrix<P, R, 2, O> operator*(const Eigen::Matrix<P, R, 2, O> &lhs,
                                           const Eigen::SO2<P> &rhs) {
  return lhs * rhs.get_matrix();
}
/*----------------------------------------------------------------*/
...
template <class Archive, typename _Tp>
inline void serialize(Archive &ar, cv::Mat_<_Tp> &m,
...
void serialize(Archive &ar, cv::Matx<_Tp, m, n> &_m,
               const unsigned int /*version*/) {
  ar & _m.val;
}
...
void serialize(Archive &ar, cv::Vec<_Tp, cn> &v,
               const unsigned int /*version*/) {
...
template <class Archive, typename _Tp>
void serialize(Archive &ar, cv::Point_<_Tp> &pt,
               const unsigned int /*version*/) {
  ar & pt.x & pt.y;
}
...
template <class Archive, typename _Tp>
void serialize(Archive &ar, cv::Point3_<_Tp> &pt,
               const unsigned int /*version*/) {
  ar & pt.x & pt.y & pt.z;
}
...
template <class Archive, typename _Tp>
void serialize(Archive &ar, cv::Size_<_Tp> &sz,
               const unsigned int /*version*/) {
...
template <class Archive, typename _Tp>
void serialize(Archive &ar, cv::Rect_<_Tp> &rc,
               const unsigned int /*version*/) {
...
template <class Archive, class Scalar, int _Rows, int _Cols, int _Options,
          int _MaxRows, int _MaxCols>
inline void
save(Archive &ar,
     const Eigen::Matrix<Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols> &M,
     const unsigned int /*version*/) {
...
inline void
load(Archive &ar,
     Eigen::Matrix<Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols> &M,
     const unsigned int /*version*/) {
  ar >> make_nvp("data", make_array(M.data(), _Rows * _Cols));
}
...
// include headers that implement a archive in simple text and binary format or
// XML format
#if defined(_MSC_VER)
#pragma warning(push)
...
template <typename TYPE>
bool SerializeSave(const TYPE &obj, std::ofstream &fs, ARCHIVE_TYPE type,
                   unsigned flags = boost::archive::no_header) {
...
template <typename TYPE>
bool SerializeSave(const TYPE &obj, const SEACAVE::String &fileName,
                   ARCHIVE_TYPE type,
                   unsigned flags = boost::archive::no_header) {
...
template <typename TYPE>
bool SerializeLoad(TYPE &obj, std::ifstream &fs, ARCHIVE_TYPE type,
                   unsigned flags = boost::archive::no_header) {
  try {
...
template <typename TYPE>
bool SerializeLoad(TYPE &obj, const SEACAVE::String &fileName,
                   ARCHIVE_TYPE type,
                   unsigned flags = boost::archive::no_header) {
...
class MiniDumper {
public:
  static inline void Create(const SEACAVE::String &strAppNameANSI,
                            SEACAVE::String strDumpPathANSI,
                            MINIDUMP_TYPE dumpType = MiniDumpNormal,
                            bool bOutOfProcess = true) {
    static MiniDumper oMiniDumper(strAppNameANSI, strDumpPathANSI, dumpType,
                                  bOutOfProcess);
  }
  static inline void Create(const SEACAVE::String &strAppNameANSI,
                            SEACAVE::String strDumpPathANSI, int verbosity,
                            bool bOutOfProcess = true) {
  ...
  MiniDumper(const SEACAVE::String &strAppNameANSI,
             SEACAVE::String strDumpPathANSI,
             MINIDUMP_TYPE dumpType = MiniDumpNormal, bool bOutOfProcess = true)
      : crash_server(NULL), handler(NULL) {
    if (strDumpPathANSI.IsEmpty())
      strDumpPathANSI = SEACAVE::Util::getCurrentFolder();
    const std::wstring strDumpPath(strDumpPathANSI.begin(),
                                   strDumpPathANSI.end());
    ...
    if (bOutOfProcess) {
      ...
      strPipeName =
          L"\\\\.\\pipe\\BreakpadCrashServices\\" + strAppName + L"-" + strUUID;
      crash_server = new google_breakpad::CrashGenerationServer(
          strPipeName, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
          true, &strDumpPath);
    ...
    handler = new google_breakpad::ExceptionHandler(
        strDumpPath, NULL, NULL, NULL,
        google_breakpad::ExceptionHandler::HANDLER_ALL, dumpType,
        (crash_server ? strPipeName.c_str() : NULL), &custom_info);
  }
  ~MiniDumper() {
    delete handler;
    delete crash_server;
