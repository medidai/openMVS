...
public:
  ...
  inline operator float() const { return toFloat(val); }
  inline hfloat operator-() const { return fromFloat2HFloat(-toFloat(val)); }
  ...
  inline hfloat &operator+=(hfloat v) { return *this = *this + v; }
  inline hfloat &operator+=(float v) { return *this = *this + v; }
  inline hfloat operator-(hfloat v) {
    return fromFloat2HFloat(toFloat(val) - toFloat(v.val));
  }
  inline hfloat operator-(float v) {
  ...
  static inline short fromFloat(float f) {
    ASSERT(ISFINITE(f) && (ABS(f) == 0.f || ABS(f) >= min()) &&
           ABS(f) <= max());
