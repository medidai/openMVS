...
template <typename TYPE, int DIMS = 3> class TPlane {
...
public:
  ...
  int Optimize(const POINT *, size_t, const RobustNormFunctor &robust,
               int maxIters = 100);
...
struct FitPlaneOnline {
  ...
  TPoint3<TYPEE> GetPlane(TPlane<TYPEE, 3> &plane) const;
};
/*----------------------------------------------------------------*/

// Basic 3D frustum class
// (represented as 6 planes oriented toward outside the frustum volume)
template <typename TYPE, int DIMS = 6> class TFrustum {
...
public:
  ...
  inline TFrustum() {}
  inline TFrustum(const MATRIX4x4 &, TYPE width, TYPE height,
                  TYPE nearZ = TYPE(0.0001), TYPE farZ = TYPE(1000));
  inline TFrustum(const MATRIX3x4 &, TYPE width, TYPE height,
                  TYPE nearZ = TYPE(0.0001), TYPE farZ = TYPE(1000));

  void Set(const MATRIX4x4 &, TYPE width, TYPE height,
           TYPE nearZ = TYPE(0.0001), TYPE farZ = TYPE(1000));
  void Set(const MATRIX3x4 &, TYPE width, TYPE height,
           TYPE nearZ = TYPE(0.0001), TYPE farZ = TYPE(1000));
