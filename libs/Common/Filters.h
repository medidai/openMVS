...
public:
  ...
  BufferedInputStream(InputStream *aStream, size_t aBufSize)
      : Base(aStream), buf(new uint8_t[aBufSize]), bufSize(aBufSize), cache(0),
        pos(0) {
    ASSERT(aBufSize > 0);
  }
  virtual ~BufferedInputStream() { delete[] buf; }
...
public:
  ...
  FilteredInputStream(InputStream *pFile, size_t nFilteredSize, size_t nBufSize,
                      void *pData)
      : Base(pFile), buf(new uint8_t[nBufSize]), filteredSize(nFilteredSize),
        bufSize(nBufSize), valid(0), pos(0), filter(pData), more(true) {}
  virtual ~FilteredInputStream() { delete[] buf; }
  ...
  size_f_t getSize() const override { return filteredSize; }
...
public:
  ...
  FilteredOutputStream(OutputStream *aFile, size_t aBufSize, void *pData)
      : Base(aFile), buf(new uint8_t[aBufSize]), bufSize(aBufSize),
        filter(pData), flushed(false) {}
...
public:
  ...
  TokenInputStream(InputStream *aStream, TCHAR aToken = _T('\n'))
      : Base(aStream), pos(TOKEN_MAXBUF), eos(false), token(aToken) {
    arrIgnore[0] = _T('\0');
  }
  virtual ~TokenInputStream() {}

  void emptyBuffer() { setPos(getPos()); }

  TCHAR getToken() const { return token; }

  void setToken(TCHAR aToken) { token = aToken; }

  LPCTSTR getTrimTokens() const { return arrIgnore; }
  ...
  size_t readLine(MemFile &memFile) {
    ...
    while (true) {
      ...
      const size_t ret =
          readLine((LPTSTR)(memFile.getBuffer() + memFile.getSize()), 4096);
      if (ret == STREAM_ERROR)
        return STREAM_ERROR;
      if (ret <= 4096) {
...
public:
  ...
  size_f_t getSize() const override { return size; }
  ...
  size_f_t getPos() const override { return pos; }
