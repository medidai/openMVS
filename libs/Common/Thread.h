...
public:
  ...
  enum Priority { IDLE = -2, LOW = -1, NORMAL = 0, HIGH = 1 };
  ...
  typedef struct STARTER_DATA {
    FncStart fnStarter;
    void *pData;
    STARTER_DATA(FncStart _fnStarter, void *_pData)
        : fnStarter(_fnStarter), pData(_pData) {}
  } StarterData;
  ...
  bool start(FncStart pfnStarter, void *pData = NULL) {
    join();
    return ((threadHandle = CreateThread(NULL, 0, &starterConvertor,
                                         new StarterData(pfnStarter, pData), 0,
                                         &threadId)) != NULL);
  }
  inline bool start() { return start(&starter, this); }
  void stop() {
    if (threadHandle) {
  ...
  inline void setThreadPriority(Priority p) const {
    ::SetThreadPriority(threadHandle, convertPriority(p));
  }
  inline Priority getThreadPriority() const {
    return convertPriority((PriorityOS)::GetThreadPriority(threadHandle));
  }
  static inline void setThreadPriority(thread_t th, Priority p) {
    ::SetThreadPriority(th, convertPriority(p));
  }
  static inline Priority getThreadPriority(thread_t th) {
    return convertPriority((PriorityOS)::GetThreadPriority(th));
  }
  ...
  static inline int32_t safeInc(volatile int32_t &v) {
    return InterlockedIncrement((volatile LONG *)&v);
  };
  static inline int32_t safeDec(volatile int32_t &v) {
    return InterlockedDecrement((volatile LONG *)&v);
  };
  static inline int32_t safeExchange(volatile int32_t &target, int32_t value) {
    return InterlockedExchange((volatile LONG *)&target, value);
  };
  static inline int32_t safeCompareExchange(volatile int32_t &target,
                                            int32_t comp, int32_t value) {
    return InterlockedCompareExchange((volatile LONG *)&target, value, comp);
  };

  STATIC_ASSERT(sizeof(int64_t) == sizeof(LONGLONG));
  static inline int64_t safeInc(volatile int64_t &v) {
    return InterlockedIncrement64((volatile LONGLONG *)&v);
  };
  static inline int64_t safeDec(volatile int64_t &v) {
    return InterlockedDecrement64((volatile LONGLONG *)&v);
  };
  static inline int64_t safeExchange(volatile int64_t &target, int64_t value) {
    return InterlockedExchange64((volatile LONGLONG *)&target, value);
  };
  static inline int64_t safeCompareExchange(volatile int64_t &target,
                                            int64_t comp, int64_t value) {
    return InterlockedCompareExchange64((volatile LONGLONG *)&target, value,
                                        comp);
  };
  ...
  bool start() { return start(&starter, this); }
  void stop() {
    if (threadHandle != 0) {
  ...
  static inline int32_t safeCompareExchange(volatile int32_t &target,
                                            int32_t comp, int32_t value) {
    return __sync_val_compare_and_swap(&target, comp, value);
  }
...
protected:
  ...
  enum PriorityOS { OS_IDLE = 19, OS_LOW = 10, OS_NORMAL = 0, OS_HIGH = -10 };
...
public:
  inline ThreadPool() {}
  inline ThreadPool(size_type nThreads)
      : _threads(nThreads > 0 ? nThreads : Thread::hardwareConcurrency()) {}
  inline ThreadPool(size_type nThreads, Thread::FncStart pfnStarter,
                    void *pData = NULL)
      : _threads(nThreads > 0 ? nThreads : Thread::hardwareConcurrency()) {
    start(pfnStarter, pData);
  }
  inline ~ThreadPool() { join(); }

#ifdef _SUPPORT_CPP11
  inline ThreadPool(ThreadPool &&rhs)
      : _threads(std::forward<Threads>(rhs._threads)) {}
