...
String Util::getApplicationFolder() {
  ...
  if ((homedir = getenv("HOME")) == NULL)
    homedir = getpwuid(getuid())->pw_dir;
  String dir(String(homedir) + PATH_SEPARATOR + String(_T(".config")) +
             PATH_SEPARATOR);
#endif // _MSC_VER
  return ensureUnifySlash(dir);
}
...
String Util::GetRAMInfo() {
  ...
  return formatBytes(memInfo.totalPhysical) + _T(" Physical Memory ") +
         formatBytes(memInfo.totalVirtual) + _T(" Virtual Memory");
}
...
#define _WIN32_WINNT_WIN10 0x0A00
  ...
  return String(n.sysname) + " " + String(n.release) + " (" +
         String(n.machine) + ")";
...
#if defined(_SUPPORT_CPP17) &&                                                 \
    (defined(__APPLE__) || !defined(__GNUC__) || (__GNUC__ > 7))

  const std::filesystem::space_info si = std::filesystem::space(path.c_str());
  return String::FormatString("%s (%s) space",
                              formatBytes(si.available).c_str(),
                              formatBytes(si.capacity).c_str());
...
CPUINFO GetCPUInfo() {
  ...
  *((int *)(info.vendor + 8)) = CPUInfo[2];
  ...
  info.bAVX =
      (CPUInfo[2] & 0x18000000) == 0x18000000; // test bits 28,27 for AVX
  info.bFMA =
      (CPUInfo[2] & 0x18001000) == 0x18001000; // test bits 28,27,12 for FMA
  ...
  if ((strncmp(info.vendor, "AuthenticAMD", 12) == 0) && info.bEXT) { // AMD
    CPUID(CPUInfo, 0x80000001); // CPUID will copy ext. feat. bits to EDX and
                                // cpu type to EAX
    info.b3DNOWEX =
        (CPUInfo[3] & 0x40000000) != 0; // indicates AMD extended 3DNow+!
...
// Function to detect SSE availability in operating system.
bool OSSupportsSSE() { return false; }
// Function to detect AVX availability in operating system.
bool OSSupportsAVX() { return false; }
/*----------------------------------------------------------------*/
#endif // _PLATFORM_X86
...
#endif
  LOG(_T("CPU: %s (%u cores)"), Util::GetCPUInfo().c_str(),
...
#endif
#ifdef _USE_SSE
  if (!SIMD_ENABLED.isSet(Util::SSE))
    LOG(_T("warning: no SSE compatible CPU or OS detected"));
  else if (!SIMD_ENABLED.isSet(Util::AVX))
    LOG(_T("warning: no AVX compatible CPU or OS detected"));
  else
    LOG(_T("SSE & AVX compatible CPU & OS detected"));
#endif
}
...
void Util::LogMemoryInfo() {
  ...
  LOG(_T("\tPeakWorkingSetSize %s"),
      SEACAVE::Util::formatBytes(pmc.PeakWorkingSetSize).c_str());
  LOG(_T("\tWorkingSetSize %s"),
      SEACAVE::Util::formatBytes(pmc.WorkingSetSize).c_str());
  LOG(_T("\tQuotaPeakPagedPoolUsage %s"),
      SEACAVE::Util::formatBytes(pmc.QuotaPeakPagedPoolUsage).c_str());
  LOG(_T("\tQuotaPagedPoolUsage %s"),
      SEACAVE::Util::formatBytes(pmc.QuotaPagedPoolUsage).c_str());
  LOG(_T("\tQuotaPeakNonPagedPoolUsage %s"),
      SEACAVE::Util::formatBytes(pmc.QuotaPeakNonPagedPoolUsage).c_str());
  LOG(_T("\tQuotaNonPagedPoolUsage %s"),
      SEACAVE::Util::formatBytes(pmc.QuotaNonPagedPoolUsage).c_str());
  LOG(_T("\tPagefileUsage %s"),
      SEACAVE::Util::formatBytes(pmc.PagefileUsage).c_str());
  LOG(_T("\tPeakPagefileUsage %s"),
      SEACAVE::Util::formatBytes(pmc.PeakPagefileUsage).c_str());
  LOG(_T("} ENDINFO"));
}
#else  // _MSC_VER
void Util::LogMemoryInfo() {
...
Util::MemoryInfo Util::GetMemoryInfo() {
  ...
  return MemoryInfo(status.dwTotalPhys, status.dwAvailPhys,
  ...
  return MemoryInfo((size_t)info.totalram * (size_t)info.mem_unit,
...
// Parses a ASCII command line string and returns an array of pointers to the
