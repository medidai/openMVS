...
//
// For a (relative) sampling position x (in [0,1]) between two (consecutive)
// points :
//
//   A  .... x ......... B
//
...
//
// Note: The following functors generalize the sampling to more than two
// neighbors They all contains the width variable that specify the number of
// neighbors used for sampling
//
...
template <typename TYPE> struct Linear {
  typedef TYPE Type;
...
//
// Interpolation weight is for A,B,C and D pixels given a x position as
...
template <typename TYPE> struct Cubic {
  typedef TYPE Type;

  enum { halfWidth = 2 };
  enum { width = halfWidth * 2 };

  // Sharpness coefficient used to control sharpness of the cubic curve (between
  // 0.5 to 0.75) cubic(0,1/2): 0.5 gives better mathematically result (ie:
  // approximation at 3 order precision - Catmull-Rom)
  ...
  inline TYPE CubicInter01(const TYPE x) const {
    ...
    return ((TYPE(2) - sharpness) * x - (TYPE(3) - sharpness)) * x * x +
           TYPE(1);
  }
...
// Sampler spline16 -> Interpolation on 4 points used for 2D ressampling (16 =
// 4x4 sampling) Cubic interpolation with 0-derivative at edges (ie at A and D
// points) See Helmut Dersch for more details
//
// Some refs :
...
// f1''(0) = f2''(0)
// f2''(1) = f3''(0)
// Curve is, at boundary, with second derivative set to zero (it's a constraint
// introduced by Dersch), ie: f1''(-1) = 0 f3''(2) = 0
//
...
//
//
// When you have coefs, you just have to express your curve as a linear
// combination of the control points, fort ex with f2 :
//
//
...
//
// substituting boundary conditions gives the correct coefficients for
// y0,y1,y2,y3 giving the final sampling scheme
template <typename TYPE> struct Spline16 {
  typedef TYPE Type;
...
template <typename TYPE> struct Spline36 {
  typedef TYPE Type;
  ...
  inline void operator()(const TYPE x, TYPE *const weight) const {
    weight[0] = ((TYPE(1) / TYPE(11) * x - TYPE(45) / TYPE(209)) * x +
                 TYPE(26) / TYPE(209)) *
                x;
    weight[1] = ((TYPE(-6) / TYPE(11) * x + TYPE(270) / TYPE(209)) * x -
                 TYPE(156) / TYPE(209)) *
                x;
    weight[2] = ((TYPE(13) / TYPE(11) * x - TYPE(453) / TYPE(209)) * x -
                 TYPE(3) / TYPE(209)) *
                    x +
                TYPE(1);
    weight[3] = ((TYPE(-13) / TYPE(11) * x + TYPE(288) / TYPE(209)) * x +
                 TYPE(168) / TYPE(209)) *
                x;
    weight[4] = ((TYPE(6) / TYPE(11) * x - TYPE(72) / TYPE(209)) * x -
                 TYPE(42) / TYPE(209)) *
                x;
    weight[5] = ((TYPE(-1) / TYPE(11) * x + TYPE(12) / TYPE(209)) * x +
                 TYPE(7) / TYPE(209)) *
                x;
  }
};
...
template <typename TYPE> struct Spline64 {
  typedef TYPE Type;
  ...
  inline void operator()(const TYPE x, TYPE *const weight) const {
    weight[0] = ((TYPE(-1) / TYPE(41) * x + TYPE(168) / TYPE(2911)) * x -
                 TYPE(97) / TYPE(2911)) *
                x;
    weight[1] = ((TYPE(6) / TYPE(41) * x - TYPE(1008) / TYPE(2911)) * x +
                 TYPE(582) / TYPE(2911)) *
                x;
    weight[2] = ((TYPE(-24) / TYPE(41) * x + TYPE(4032) / TYPE(2911)) * x -
                 TYPE(2328) / TYPE(2911)) *
                x;
    weight[3] = ((TYPE(49) / TYPE(41) * x - TYPE(6387) / TYPE(2911)) * x -
                 TYPE(3) / TYPE(2911)) *
                    x +
                TYPE(1);
    weight[4] = ((TYPE(-49) / TYPE(41) * x + TYPE(4050) / TYPE(2911)) * x +
                 TYPE(2340) / TYPE(2911)) *
                x;
    weight[5] = ((TYPE(24) / TYPE(41) * x - TYPE(1080) / TYPE(2911)) * x -
                 TYPE(624) / TYPE(2911)) *
                x;
    weight[6] = ((TYPE(-6) / TYPE(41) * x + TYPE(270) / TYPE(2911)) * x +
                 TYPE(156) / TYPE(2911)) *
                x;
    weight[7] = ((TYPE(1) / TYPE(41) * x - TYPE(45) / TYPE(2911)) * x -
                 TYPE(26) / TYPE(2911)) *
                x;
  }
};
...
inline TYPE Sample(const IMAGE &image, const SAMPLER &sampler,
                   const POINT &pt) {
