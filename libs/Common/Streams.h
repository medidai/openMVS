...
public:
  ...
  enum { LAYER_ID_IN = 0 };
  InputStream *getInputStream(int typ = LAYER_ID_IN) override {
    return (typ == LAYER_ID_IN ? this : (InputStream *)NULL);
  }
};
...
public:
  ...
  template <class T> inline OutputStream &operator<<(const T &val) {
  ...
  enum { LAYER_ID_OUT = 0 };
  OutputStream *getOutputStream(int typ = LAYER_ID_OUT) override {
    return (typ == LAYER_ID_OUT ? this : (OutputStream *)NULL);
  }
};

class GENERAL_API NOINITVTABLE IOStream : public InputStream,
                                          public OutputStream {
public:
  InputStream *getInputStream(int typ = LAYER_ID_IN) override {
    return InputStream::getInputStream(typ);
  }
  OutputStream *getOutputStream(int typ = LAYER_ID_OUT) override {
    return OutputStream::getOutputStream(typ);
  }
  IOStream *getIOStream(int typIn = LAYER_ID_IN,
                        int typOut = LAYER_ID_OUT) override {
    return ((InputStream *)this)->getInputStream(typIn) != NULL &&
                   ((OutputStream *)this)->getOutputStream(typOut) != NULL
               ? this
               : (IOStream *)NULL;
  }
};
...
template <bool managed = true> class LayerInputStream : public InputStream {
public:
  LayerInputStream(InputStream *aStream) : s(aStream) { ASSERT(s != NULL); }
  virtual ~LayerInputStream() noexcept {
    if (managed)
      delete s;
  }

  InputStream *getInputStream(int typ = InputStream::LAYER_ID_IN) override {
    return s->getInputStream(typ);
  }

  size_t read(void *wbuf, size_t len) override { return s->read(wbuf, len); }

  size_f_t getSize() const override { return s->getSize(); }

  bool setPos(size_f_t wpos) override { return s->setPos(wpos); }

  size_f_t getPos() const override { return s->getPos(); }

protected:
  InputStream *const s;
};

template <bool managed = true> class LayerOutputStream : public OutputStream {
public:
  LayerOutputStream(OutputStream *aStream) : s(aStream) {}
  virtual ~LayerOutputStream() noexcept {
    if (managed)
      delete s;
  }

  OutputStream *getOutputStream(int typ = OutputStream::LAYER_ID_OUT) override {
    return s->getOutputStream(typ);
  }
  ...
  size_t flush() override { return s->flush(); }
...
template <bool managed = true> class LayerIOStream : public IOStream {
public:
  LayerIOStream(InputStream *aStream) : s(aStream), sIn(aStream), sOut(NULL) {
    ASSERT(aStream != NULL);
  }
  LayerIOStream(OutputStream *aStream) : s(aStream), sIn(NULL), sOut(aStream) {
    ASSERT(aStream != NULL);
  }
  LayerIOStream(InputStream *streamIn, OutputStream *streamOut)
      : s(NULL), sIn(streamIn), sOut(streamOut) {
    ASSERT(sIn != NULL || sOut != NULL);
  }
  virtual ~LayerIOStream() noexcept {
    if (managed) {
      delete sIn;
      delete sOut;
    }
  ...
  InputStream *getInputStream(int typ = InputStream::LAYER_ID_IN) override {
    return (sIn ? sIn->getInputStream(typ) : NULL);
  }
  OutputStream *getOutputStream(int typ = OutputStream::LAYER_ID_OUT) override {
    return (sOut ? sOut->getOutputStream(typ) : NULL);
  }
  ...
  size_t read(void *wbuf, size_t len) override { return sIn->read(wbuf, len); }

  size_t write(const void *buf, size_t len) override {
    return sOut->write(buf, len);
  }

  size_t flush() override { return sOut->flush(); }
