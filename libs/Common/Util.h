...
public:
  ...
  THistogram(const std::pair<Type, Type> &range, size_t bins = 10)
      : Start(range.first), End(range.second),
        BinInterval(Type(bins) / (End - Start)), Freq(bins, 0), Overflow(0),
        Underflow(0) {}
  ...
  std::string ToString(const std::string &sTitle = "") const {
    ...
    for (size_t i = 0, n = Freq.size(); i < n; ++i)
      os << static_cast<float>(End - Start) / n * static_cast<float>(i)
         << "\t|\t" << Freq[i] << "\n";
    os << ">=" << End << "\t|\t" << Overflow << "\n";
    return os.str();
  }
...
public:
  ...
  static String getUniqueName(TCHAR dash = '-') {
    ...
    SYSTEMTIME st;
    GetLocalTime(&st);
    LPTSTR szTime = szDate + GetDateFormat(LOCALE_USER_DEFAULT, 0, &st,
                                           _T("yy''MM''dd"), szDate, 80);
    ...
    if (dash)
      return String::FormatString("%s%c%s%c%06X", szDate, dash, szTime, dash,
                                  ID);
  ...
  static String &ensureValidFolderPath(String &path) {
    return simplifyPath(
        ensureFolderSlash(ensureUnifySlash(strTrim(path, _T("\"")))));
  }
...
#endif // _MSC_VER
#ifdef UNICODE
                        *reinterpret_cast<const DWORD *>(path) ==
                            0x5C005C00 /*"\\\\"*/));
#else
                        *reinterpret_cast<const WORD *>(path) ==
                            0x5C5C /*"\\\\"*/));
#endif // UNICODE
  }
  ...
  static inline bool isParentFolder(LPCTSTR path, int off = 0) {
    // returns true if the folder starting at the given position in path is the
    // parent folder ".."
  ...
  static String getProcessFolder() { return getFilePath(getAppName()); }
  ...
  static String getSimplifiedPath(String path) { return simplifyPath(path); }

  static String getRelativePath(const String &currentPath,
                                const String &targetPath) {
    // returns the path to the target relative to the current path;
    // both current and target paths must be full paths;
    // current path is assumed to be a folder, while target path can be also a
    // file
...
#ifdef _MSC_VER
           _tcsicmp(currentPathValues[idxCurrentPath],
                    targetPathValues[idxTargetPath]) == 0
#else
           _tcscmp(currentPathValues[idxCurrentPath],
                   targetPathValues[idxTargetPath]) == 0
#endif
    )
...
// returns the path shared by the given paths
  ...
  static String insertAfterFilePath(const String &path, const String &extra) {
    const String::size_type i = path.rfind(PATH_SEPARATOR);
    return (i != String::npos) ? String(path).insert(i + 1, extra)
                               : extra + path;
  }
  ...
  // remove all instances of a given sequence of characters from the beginning
  ...
  static void strSplit(const String &str, TCHAR delim,
                       CLISTDEF2(String) & values) {
    values.Empty();
  ...
  static void strSplit(const String &str, const String &delim,
                       CLISTDEF2(String) & values) {
    ASSERT(!delim.empty());
    values.Empty();
    String::size_type start(0), end(0);
  ...
  static String formatBytes(int64_t aBytes) {
    if (aBytes < (int64_t)1024) {
      ...
      return String::FormatString("%.02fMB",
                                  (double)aBytes / (1024.0 * 1024.0));
    } else if (aBytes < (int64_t)1024 * 1024 * 1024 * 1024) {
      return String::FormatString("%.02fGB",
                                  (double)aBytes / (1024.0 * 1024.0 * 1024.0));
    } else {
      return String::FormatString(
          "%.02fTB", (double)aBytes / (1024.0 * 1024.0 * 1024.0 * 1024.0));
    }
  }
  ...
  static String toString(const wchar_t *wsz) {
    ...
    if (std::wcsrtombs(&mbstr[0], &wsz, mbstr.size(), &state) ==
  ...
  char *to_next;
  if (converter.out(state, ws.data(), ws.data() + ws.length(), from_next,
                    &to[0], &to[0] + to.size(), to_next) != converter_type::ok)
    return String();
  ...
  static int toInt(LPCTSTR aString) { return atoi(aString); }
  static uint32_t toUInt32Hex(LPCTSTR aString) {
    uint32_t val;
    sscanf(aString, "%x", &val);
    return val;
  }
  static uint32_t toUInt32(LPCTSTR aString) { return (uint32_t)atoi(aString); }
  static double toDouble(LPCTSTR aString) { return atof(aString); }
  static float toFloat(LPCTSTR aString) { return (float)atof(aString); }
  ...
  struct MemoryInfo {
    ...
    MemoryInfo(size_t tP = 0, size_t fP = 0, size_t tV = 0, size_t fV = 0)
        : totalPhysical(tP), freePhysical(fP), totalVirtual(tV),
          freeVirtual(fV) {}
  };
  ...
  struct Progress {
    ...
    CriticalSection
        cs; // multi-threading safety only for the incremental operator

    Progress(const String &_msg, size_t _total, Timer::Type _slp = 100 /*ms*/)
        : msg(_msg), total(_total), slp(_slp), start(Timer::GetSysTime()),
          lastElapsed(0), lastMsgLen(0), processed(0) {}
    ~Progress() {
      if (processed)
        close();
    }
    ...
    void process() {
      ...
      const Timer::Type elapsed(
          Timer::SysTime2TimeMs(Timer::GetSysTime() - start));
      ...
      const Timer::Type remaining(percentage < 0.01f &&
                                          (done < 10 || elapsed < 10 * 1000)
                                      ? Timer::Type(0)
                                      : elapsed / percentage - elapsed);
      // display progress
      print(String::FormatString(_T("%s %u (%.2f%%, %s, ETA %s)..."),
                                 msg.c_str(), done, percentage * 100.f,
    ...
    void close() {
      ...
      const Timer::Type elapsed(
          Timer::SysTime2TimeMs(Timer::GetSysTime() - start));
      // display progress
      print(String::FormatString(_T("%s %u (100%%, %s)"), msg.c_str(), total,
