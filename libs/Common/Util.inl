...
// K is assumed to be the [3,3] triangular matrix with: fx, fy, s, cx, cy and
// scale 1
template <typename TYPE1, typename TYPE2, typename TYPE3>
inline void NormalizeProjection(const TYPE1 *K, const TYPE2 *x, TYPE3 *n) {
  ASSERT(ISZERO(K[3]) && ISZERO(K[6]) && ISZERO(K[7]) &&
         ISEQUAL(K[8], TYPE1(1)));
...
inline void NormalizeProjectionInv(const TYPE1 *K, const TYPE2 *x, TYPE3 *n) {
  ASSERT(ISZERO(K[3]) && ISZERO(K[6]) && ISZERO(K[7]) &&
         ISEQUAL(K[8], TYPE1(1)));
  n[0] = TYPE3((K[4] * x[0] - K[1] * x[1] + (K[5] * K[1] - K[2] * K[4])) /
               (K[0] * K[4]));
...
inline void ComputeRelativeRotation(const TMatrix<TYPE, 3, 3> &Ri,
                                    const TMatrix<TYPE, 3, 3> &Rj,
                                    TMatrix<TYPE, 3, 3> &Rij) {
  Rij = Rj * Ri.t();
} // ComputeRelativeRotation
template <typename TYPE>
inline void
ComputeRelativePose(const TMatrix<TYPE, 3, 3> &Ri, const TPoint3<TYPE> &Ci,
                    const TMatrix<TYPE, 3, 3> &Rj, const TPoint3<TYPE> &Cj,
                    TMatrix<TYPE, 3, 3> &Rij, TPoint3<TYPE> &Cij) {
  Rij = Rj * Ri.t();
  Cij = Ri * (Cj - Ci);
} // ComputeRelativePose
/*----------------------------------------------------------------*/
...
// given two corresponding normalized projections and the pose of the second
// camera relative to the first one; returns the triangulated 3D point from the
// point of view of the first camera
template <typename TYPE1, typename TYPE2>
bool TriangulatePoint3D(const TMatrix<TYPE1, 3, 3> &R, const TPoint3<TYPE1> &C,
  ...
  const TPoint2<TYPE1> lambda((d * b.x + c * b.y) * invDet,
                              (a * b.y - c * b.x) * invDet);
...
template <typename TYPE1, typename TYPE2>
bool TriangulatePoint3D(const TMatrix<TYPE1, 3, 3> &K1,
                        const TMatrix<TYPE1, 3, 3> &K2,
                        const TMatrix<TYPE1, 3, 3> &R1,
                        const TMatrix<TYPE1, 3, 3> &R2,
...
template <typename TYPE>
TMatrix<TYPE, 3, 3> HomographyMatrixComposition(const TMatrix<TYPE, 3, 3> &R,
                                                const TPoint3<TYPE> &C,
                                                const TMatrix<TYPE, 3, 1> &n,
                                                const TYPE &d) {
...
TMatrix<TYPE, 3, 3> HomographyMatrixComposition(const TMatrix<TYPE, 3, 3> &R,
                                                const TPoint3<TYPE> &C,
                                                const TMatrix<TYPE, 3, 1> &n,
                                                const TMatrix<TYPE, 3, 1> &X) {
  return HomographyMatrixComposition(R, C, n, -n.dot(X));
}
template <typename TYPE>
TMatrix<TYPE, 3, 3> HomographyMatrixComposition(const TMatrix<TYPE, 3, 3> &Ri,
                                                const TPoint3<TYPE> &Ci,
                                                const TMatrix<TYPE, 3, 3> &Rj,
                                                const TPoint3<TYPE> &Cj,
                                                const TMatrix<TYPE, 3, 1> &n,
                                                const TYPE &d) {
#if 0
...
template <typename TYPE>
inline TMatrix<TYPE, 3, 3> TransformE2F(const TMatrix<TYPE, 3, 3> &E,
                                        const TMatrix<TYPE, 3, 3> &K1,
                                        const TMatrix<TYPE, 3, 3> &K2) {
  return K2.inv().t() * E * K1.inv();
} // TransformE2F
// transform fundamental matrix to essential matrix
template <typename TYPE>
inline TMatrix<TYPE, 3, 3> TransformF2E(const TMatrix<TYPE, 3, 3> &F,
                                        const TMatrix<TYPE, 3, 3> &K1,
                                        const TMatrix<TYPE, 3, 3> &K2) {
  return K2.t() * F * K1;
} // TransformF2E
...
template <typename TYPE>
inline TMatrix<TYPE, 3, 3>
CreateCrossProductMatrix3(const TMatrix<TYPE, 3, 1> &x) {
  TMatrix<TYPE, 3, 3> X;
  X(0, 0) = TYPE(0);
  X(1, 0) = -x(2);
  X(2, 0) = x(1);
...
template <typename TYPE>
inline TMatrix<TYPE, 3, 3>
CreateCrossProductMatrix3T(const TMatrix<TYPE, 3, 1> &x) {
  TMatrix<TYPE, 3, 3> X;
  X(0, 0) = TYPE(0);
  X(1, 0) = x(2);
  X(2, 0) = -x(1);
...
template <typename TYPE>
inline TMatrix<TYPE, 3, 3> CreateE(const TMatrix<TYPE, 3, 3> &R,
                                   const TMatrix<TYPE, 3, 1> &C) {
  const TMatrix<TYPE, 3, 1> t = -(R * C);
  return CreateCrossProductMatrix3(t) * R;
} // CreateE
// compose fundamental matrix from the given rotation, direction and camera
// matrix
template <typename TYPE>
inline TMatrix<TYPE, 3, 3>
CreateF(const TMatrix<TYPE, 3, 3> &R, const TMatrix<TYPE, 3, 1> &C,
        const TMatrix<TYPE, 3, 3> &K1, const TMatrix<TYPE, 3, 3> &K2) {
...
inline void RQDecomp3x3(Eigen::Matrix<Scalar, 3, 3> M,
  ...
  //       | 0 -s  c |
  Eigen::Matrix<Scalar, 2, 1> cs =
      Eigen::Matrix<Scalar, 2, 1>(M(2, 2), M(2, 1)).normalized();
  Eigen::Matrix<Scalar, 3, 3> Qx{
      {1, 0, 0}, {0, cs.x(), cs.y()}, {0, -cs.y(), cs.x()}};
  R.noalias() = M * Qx;
  ASSERT(std::abs(R(2, 1)) < FLT_EPSILON);
  R(2, 1) = 0;
  // find Givens rotation for y axis:
  //       | c  0 -s |
  //  Qy = | 0  1  0 |, c = m33/sqrt(m31^2 + m33^2), s = -m31/sqrt(m31^2 +
  //  m33^2)
  //       | s  0  c |
  cs = Eigen::Matrix<Scalar, 2, 1>(R(2, 2), -R(2, 0)).normalized();
  Eigen::Matrix<Scalar, 3, 3> Qy{
      {cs.x(), 0, -cs.y()}, {0, 1, 0}, {cs.y(), 0, cs.x()}};
  M.noalias() = R * Qy;
...
template <typename TYPE>
inline void RQDecomp3x3(const TMatrix<TYPE, 3, 3> &M, TMatrix<TYPE, 3, 3> &R,
                        TMatrix<TYPE, 3, 3> &Q) {
...
template <typename TYPE>
FORCEINLINE TYPE ComputeAngle(const TMatrix<TYPE, 3, 3> &R1,
                              const TMatrix<TYPE, 3, 3> &R2) {
  return ComputeAngle(TMatrix<TYPE, 3, 3>(R1 * R2.t()));
} // ComputeAngle
/*----------------------------------------------------------------*/

// compute the Frobenius (or Euclidean) norm of the distance between the two
// matrices
...
template <typename TYPE, int m, int n>
FORCEINLINE TYPE FrobeniusNorm(const TMatrix<TYPE, m, n> &M1,
...
template <typename TYPE>
bool CheckCollinearity(const TPoint2<TYPE> *ptr, int count,
  ...
  for (int i = (checkPartialSubsets ? count - 1 : 2); i < count; ++i) {
    ...
    for (int j = 1; j < i; ++j) {
      ...
      for (int k = 0; k < j; ++k) {
        ...
        if (ABS(d2.x * d1.y - d2.y * d1.x) <=
            FLT_EPSILON * (ABS(d1.x) + ABS(d1.y) + ABS(d2.x) + ABS(d2.y)))
          return true;
      }
    }
  }
...
template <typename TYPE>
bool CheckCollinearity(const TPoint3<TYPE> *ptr, int count,
                       bool checkPartialSubsets = false) {
  for (int i = (checkPartialSubsets ? count - 1 : 2); i < count; ++i) {
    // check that the i-th selected point does not belong
    // to a line connecting some previously selected points
...
// compute the corresponding ray for a given projection matrix P[3,4] and image
// point pt[2,1] output ray[3,1]
template <typename TYPE1, typename TYPE2>
...
inline void ProjectVertex_3x4_3_3(const TYPE1 *P, const TYPE1 *X, TYPE2 *pt) {
  pt[0] = (TYPE2)(P[0 * 4 + 0] * X[0] + P[0 * 4 + 1] * X[1] +
                  P[0 * 4 + 2] * X[2] + P[0 * 4 + 3]);
  pt[1] = (TYPE2)(P[1 * 4 + 0] * X[0] + P[1 * 4 + 1] * X[1] +
                  P[1 * 4 + 2] * X[2] + P[1 * 4 + 3]);
  pt[2] = (TYPE2)(P[2 * 4 + 0] * X[0] + P[2 * 4 + 1] * X[1] +
                  P[2 * 4 + 2] * X[2] + P[2 * 4 + 3]);
...
inline void ProjectVertex_3x4_4_3(const TYPE1 *P, const TYPE1 *X, TYPE2 *pt) {
  pt[0] = (TYPE2)(P[0 * 4 + 0] * X[0] + P[0 * 4 + 1] * X[1] +
                  P[0 * 4 + 2] * X[2] + P[0 * 4 + 3] * X[3]);
  pt[1] = (TYPE2)(P[1 * 4 + 0] * X[0] + P[1 * 4 + 1] * X[1] +
                  P[1 * 4 + 2] * X[2] + P[1 * 4 + 3] * X[3]);
  pt[2] = (TYPE2)(P[2 * 4 + 0] * X[0] + P[2 * 4 + 1] * X[1] +
                  P[2 * 4 + 2] * X[2] + P[2 * 4 + 3] * X[3]);
} // ProjectVertex_3x4_4_3
// (optimized ProjectVertex for R[3,3], C[3,1] and X[3,1], output pt[2,1])
template <typename TYPE1, typename TYPE2>
inline void ProjectVertex_3x3_3_3_2(const TYPE1 *R, const TYPE1 *C,
                                    const TYPE1 *X, TYPE2 *pt) {
  const TYPE1 T[3] = {X[0] - C[0], X[1] - C[1], X[2] - C[2]};
  const TYPE1 invW(
      INVERT(R[2 * 3 + 0] * T[0] + R[2 * 3 + 1] * T[1] + R[2 * 3 + 2] * T[2]));
  pt[0] = (TYPE2)((R[0 * 3 + 0] * T[0] + R[0 * 3 + 1] * T[1] +
                   R[0 * 3 + 2] * T[2]) *
                  invW);
  pt[1] = (TYPE2)((R[1 * 3 + 0] * T[0] + R[1 * 3 + 1] * T[1] +
                   R[1 * 3 + 2] * T[2]) *
                  invW);
} // ProjectVertex_3x3_3_3_2
// (optimized ProjectVertex for R[3,3], C[3,1] and X[3,1], output pt[3,1])
template <typename TYPE1, typename TYPE2>
inline void ProjectVertex_3x3_3_3_3(const TYPE1 *R, const TYPE1 *C,
                                    const TYPE1 *X, TYPE2 *pt) {
  const TYPE1 T[3] = {X[0] - C[0], X[1] - C[1], X[2] - C[2]};
  pt[0] =
      (TYPE2)(R[0 * 3 + 0] * T[0] + R[0 * 3 + 1] * T[1] + R[0 * 3 + 2] * T[2]);
  pt[1] =
      (TYPE2)(R[1 * 3 + 0] * T[0] + R[1 * 3 + 1] * T[1] + R[1 * 3 + 2] * T[2]);
  pt[2] =
      (TYPE2)(R[2 * 3 + 0] * T[0] + R[2 * 3 + 1] * T[1] + R[2 * 3 + 2] * T[2]);
...
inline void ProjectVertex_3x3_2_2(const TYPE1 *H, const TYPE2 *X, TYPE3 *pt) {
  const TYPE1 invZ(
      INVERT(H[2 * 3 + 0] * X[0] + H[2 * 3 + 1] * X[1] + H[2 * 3 + 2]));
  pt[0] = (TYPE3)((H[0 * 3 + 0] * X[0] + H[0 * 3 + 1] * X[1] + H[0 * 3 + 2]) *
                  invZ);
  pt[1] = (TYPE3)((H[1 * 3 + 0] * X[0] + H[1 * 3 + 1] * X[1] + H[1 * 3 + 2]) *
                  invZ);
...
inline void ProjectVertex_3x4_3_2(const TYPE1 *P, const TYPE1 *X, TYPE2 *pt) {
  const TYPE1 invW(INVERT(P[2 * 4 + 0] * X[0] + P[2 * 4 + 1] * X[1] +
                          P[2 * 4 + 2] * X[2] + P[2 * 4 + 3]));
  pt[0] = (TYPE2)((P[0 * 4 + 0] * X[0] + P[0 * 4 + 1] * X[1] +
                   P[0 * 4 + 2] * X[2] + P[0 * 4 + 3]) *
                  invW);
...
inline void ProjectVertex_3x4t_3_4(const TYPE1 *P, const TYPE2 *X, TYPE3 *pt) {
  pt[0] =
      (TYPE3)(P[0 * 4 + 0] * X[0] + P[1 * 4 + 0] * X[1] + P[2 * 4 + 0] * X[2]);
  pt[1] =
      (TYPE3)(P[0 * 4 + 1] * X[0] + P[1 * 4 + 1] * X[1] + P[2 * 4 + 1] * X[2]);
  pt[2] =
      (TYPE3)(P[0 * 4 + 2] * X[0] + P[1 * 4 + 2] * X[1] + P[2 * 4 + 2] * X[2]);
  pt[3] =
      (TYPE3)(P[0 * 4 + 3] * X[0] + P[1 * 4 + 3] * X[1] + P[2 * 4 + 3] * X[2]);
} // ProjectVertex_3x4t_3_4
// (optimized ProjectVertex for P[3,4].t() and X[3,1], output pt[3,1] - the
// first 3 coordinates)
template <typename TYPE1, typename TYPE2, typename TYPE3>
...
// given the camera matrix, compute the error between the given 2D point and the
// reprojected 3D point; note that here we're using "dirty" projection method
// which is suitable only for visualization; on the upside, it shouldn't matter
// because if a point is projected as infinite, there's something wrong with it
// anyway; (optimized ComputeReprojectionError for P[3,4] and X[3,1], output
// pt[2,1])
template <typename TYPE1, typename TYPE2>
inline TYPE2 ComputeReprojectionError_3x4_3_2(const TYPE1 *P, const TYPE1 *X,
                                              const TYPE2 *pt) {
...
template <typename TYPE1, typename TYPE2>
inline TYPE2 ComputeReprojectionError_3x4_4_2(const TYPE1 *P, const TYPE1 *X,
                                              const TYPE2 *pt) {
  TYPE2 reprojection[2];
  ProjectVertex_3x4_4_2(P, X, reprojection);
  return SQUARE(reprojection[0] - pt[0]) + SQUARE(reprojection[1] - pt[1]);
} // ComputeReprojectionError_3x4_4_2
// (optimized ComputeReprojectionError for R[3,3], C[3,1] and X[3,1], output
// pt[2,1])
template <typename TYPE1, typename TYPE2>
inline TYPE2 ComputeReprojectionError_3x3_3_3_2(const TYPE1 *R, const TYPE1 *C,
                                                const TYPE1 *X,
                                                const TYPE2 *pt) {
  TYPE2 reprojection[2];
...
template <typename TYPE> inline TYPE PointDepth(const TYPE *P, const TYPE *X) {
  return P[2 * 4 + 0] * X[0] + P[2 * 4 + 1] * X[1] + P[2 * 4 + 2] * X[2] +
         P[2 * 4 + 3];
} // PointDepth
/*----------------------------------------------------------------*/
...
template <typename TYPE1, typename TYPE2>
inline TYPE2 ProjectPoint(const TYPE1 *P, const TYPE1 *X, const TYPE2 *proj,
                          TYPE2 *reproj) {
  ...
  return SQUARE(reproj[0] / reproj[2] - proj[0]) +
         SQUARE(reproj[1] / reproj[2] - proj[1]);
} // ProjectPoint
template <typename TYPE1, typename TYPE2>
inline TYPE2 ProjectPoint(const TYPE1 *R, const TYPE1 *C, const TYPE1 *X,
                          const TYPE2 *proj, TYPE2 *reproj) {
  // reproject the 3D point on this view
...
template <typename TYPE1, typename TYPE2>
inline bool IsPointInlier(const TYPE1 *P, const TYPE1 *X, const TYPE2 *proj,
                          TYPE2 *reproj, TYPE2 thresholdSq) {
  // reproject the 3D point on this view
  ProjectVertex_3x4_3_3(P, X, reproj);
  ...
  return (reproj[2] > TYPE2(0) &&
          (reproj[3] = (SQUARE(reproj[0] / reproj[2] - proj[0]) +
                        SQUARE(reproj[1] / reproj[2] - proj[1]))) <=
              thresholdSq);
} // IsPointInlier
template <typename TYPE1, typename TYPE2>
inline bool IsPointInlier(const TYPE1 *R, const TYPE1 *C, const TYPE1 *X,
                          const TYPE2 *proj, TYPE2 *reproj, TYPE2 thresholdSq) {
  // reproject the 3D point on this view
  ProjectVertex_3x3_3_3_3(R, C, X, reproj);
  // filter out points behind the camera or
  // having the reprojection error bigger than the given threshold
...
// given a rotation matrix, return the angle in radians corresponding to the
// axis/angle decomposition
template <typename TYPE> inline TYPE AngleFromRotationMatrix(const TYPE *R) {
  const TYPE a =
      (R[0 * 3 + 0] + R[1 * 3 + 1] + R[2 * 3 + 2] - TYPE(1)) / TYPE(2);
  if (a < TYPE(-1))
    return acos(TYPE(-1));
  if (a > TYPE(1))
    return acos(TYPE(1));
...
inline TYPE2 ComputeAngle(const TYPE1 *V1, const TYPE1 *V2) {
  return CLAMP(TYPE2((V1[0] * V2[0] + V1[1] * V2[1] + V1[2] * V2[2]) /
...
inline TYPE2 ComputeAngleN(const TYPE1 *V1, const TYPE1 *V2) {
  return CLAMP(TYPE2(V1[0] * V2[0] + V1[1] * V2[1] + V1[2] * V2[2]), TYPE2(-1),
               TYPE2(1));
} // ComputeAngleN
// given three 3D points,
// compute the angle between the vectors formed by the first point with the
// other two returns cos(angle)
template <typename TYPE1, typename TYPE2 = TYPE1>
...
// compute the angle between the two vectors formed by the first and second pair
// of points returns cos(angle)
template <typename TYPE1, typename TYPE2 = TYPE1>
inline TYPE2 ComputeAngle(const TYPE1 *X1, const TYPE1 *C1, const TYPE1 *X2,
                          const TYPE1 *C2) {
...
// compute its normal (plane's normal oriented according to the given points
// order)
template <typename TYPE>
inline TPoint3<TYPE> ComputeTriangleNormal(const TPoint3<TYPE> &v0,
                                           const TPoint3<TYPE> &v1,
                                           const TPoint3<TYPE> &v2) {
...
TYPE ComputeTriangleAreaSqLenSq(TYPE lenaSq, TYPE lenbSq, TYPE lencSq) {
  return SQUARE(lenaSq + lenbSq + lencSq) / TYPE(2) -
         (lenaSq * lenaSq + lenbSq * lenbSq + lencSq * lencSq);
}
template <typename TYPE>
...
template <typename TYPE>
TYPE EdgeFunction(const TPoint2<TYPE> &x0, const TPoint2<TYPE> &x1,
                  const TPoint2<TYPE> &x2) {
  return TYPE((x2 - x0).cross(x1 - x0));
}
template <typename TYPE>
TYPE ComputeTriangleArea(const TPoint2<TYPE> &x0, const TPoint2<TYPE> &x1,
                         const TPoint2<TYPE> &x2) {
  return (TYPE)abs(EdgeFunction(x0, x1, x2) / TYPE(2));
}
// compute area for a triangle defined by three 3D points
template <typename TYPE>
TYPE ComputeTriangleAreaSq(const TPoint3<TYPE> &x0, const TPoint3<TYPE> &x1,
                           const TPoint3<TYPE> &x2) {
  return (TYPE)normSq((x1 - x0).cross(x2 - x0)) / TYPE(4);
}
template <typename TYPE>
TYPE ComputeTriangleArea(const TPoint3<TYPE> &x0, const TPoint3<TYPE> &x1,
                         const TPoint3<TYPE> &x2) {
  return (TYPE)sqrt(ComputeTriangleAreaSq(x0, x1, x2));
} // ComputeTriangleArea
/*----------------------------------------------------------------*/

// compute signed volume of the tetrahedron defined by the given triangle
// connected to origin
template <typename TYPE>
TYPE ComputeTriangleVolume(const TPoint3<TYPE> &x0, const TPoint3<TYPE> &x1,
                           const TPoint3<TYPE> &x2) {
  return (TYPE)(x0.dot(x1.cross(x2)) / TYPE(6));
} // ComputeTriangleVolume
/*----------------------------------------------------------------*/

// compute a shape quality measure of the triangle composed by vertices
// (v0,v1,v2) returns 2*AreaTri/(MaxEdge^2) in range [0, 0.866] (ex: equilateral
// sqrt(3)/2, half-square 1/2, up to a line that has zero quality)
template <typename TYPE>
inline TYPE ComputeTriangleQuality(const TPoint3<TYPE> &v0,
                                   const TPoint3<TYPE> &v1,
                                   const TPoint3<TYPE> &v2) {
  ...
  if (a == 0)
    return 0; // area zero triangles have surely zero quality
...
template <typename TYPE>
inline TPoint2<TYPE>
BarycentricCoordinatesUV(const TPoint3<TYPE> &A, const TPoint3<TYPE> &B,
                         const TPoint3<TYPE> &C, const TPoint3<TYPE> &P) {
#if 0
	// the triangle normal
  ...
  return TPoint2<TYPE>((d11 * d20 - d01 * d21) * invDenom, // alpha
...
template <typename TYPE>
inline TPoint3<TYPE>
BarycentricCoordinates(const TPoint3<TYPE> &A, const TPoint3<TYPE> &B,
                       const TPoint3<TYPE> &C, const TPoint3<TYPE> &P) {
  TPoint2<TYPE> b(BarycentricCoordinatesUV(A, B, C, P));
  return TPoint3<TYPE>(b.x,                // alpha
                       b.y,                // beta
...
template <typename TYPE>
inline TPoint2<TYPE>
BarycentricCoordinatesUV(const TPoint2<TYPE> &A, const TPoint2<TYPE> &B,
                         const TPoint2<TYPE> &C, const TPoint2<TYPE> &P) {
  ...
  return TPoint2<TYPE>((d11 * D.x - d01 * D.y) * invDet, // alpha
...
template <typename TYPE>
inline TPoint3<TYPE>
BarycentricCoordinates(const TPoint2<TYPE> &A, const TPoint2<TYPE> &B,
                       const TPoint2<TYPE> &C, const TPoint2<TYPE> &P) {
  TPoint2<TYPE> b(BarycentricCoordinatesUV(A, B, C, P));
  return TPoint3<TYPE>(b.x,                // alpha
                       b.y,                // beta
...
inline TPoint3<TYPE> CorrectBarycentricCoordinates(TPoint2<TYPE> b) {
  ...
  ASSERT((b.x >= 0) && (b.y >= 0) && (b.x + b.y <= 1) &&
         ISEQUAL(b.x + b.y + z, TYPE(1)));
  return TPoint3<TYPE>(b.x, b.y, z);
}
template <typename TYPE>
inline TPoint3<TYPE>
PerspectiveCorrectBarycentricCoordinates(const TPoint3<TYPE> &b, TYPE z0,
                                         TYPE z1, TYPE z2) {
...
// Encodes/decodes a 3D vector in two parameters for the direction and one
// parameter for the scale
template <typename T, typename TR>
...
template <typename T> inline T MaxDepthDifference(T d, T threshold) {
#if 0
...
template <typename T> inline T DepthSimilarity(T d0, T d1) {
  ASSERT(d0 > 0);
#if 0
...
template <typename T>
inline bool IsNormalSimilar(const TPoint3<T> &n0, const TPoint3<T> &n1,
                            T threshold = T(0.996194698) /*COS(FD2R(5.f))*/) {
...
// given an array of values and their bound, approximate the area covered, in
// percentage
...
inline TYPE ComputeCoveredArea(const TYPE *values, size_t size,
  ...
  typedef Eigen::Map<const Matrix, Eigen::Unaligned, Eigen::OuterStride<>>
      MapMatrix;
...
inline void ComputeCovarianceMatrix(const TYPE *values, size_t size, TYPE *cov,
                                    int stride = n) {
  ASSERT(size > 0);
  typedef Eigen::Matrix<TYPE, 1, n, Eigen::RowMajor> Vector;
  typedef Eigen::Matrix<TYPE, Eigen::Dynamic, n, Eigen::RowMajor> Matrix;
...
template <typename TYPE, typename TYPEW>
inline void ComputeGeometricMean(const TYPE *values, size_t size,
                                 TYPEW &gmean) {
  ASSERT(size > 0);
  TYPEW prod(1);
  for (size_t i = 0; i < size; ++i)
...
template <typename TYPE, typename TYPEW>
inline void ComputeMeanStdOffline(const TYPE *values, size_t size, TYPEW &mean,
                                  TYPEW &stddev) {
  ASSERT(size > 0);
  TYPEW sum(0);
...
template <typename TYPE, typename TYPEW>
inline void ComputeMeanStdOnline(const TYPE *values, size_t size, TYPEW &mean,
                                 TYPEW &stddev) {
  ASSERT(size > 0);
  TYPEW sum(0), sumSq(0);
  FOREACHRAWPTR(pVal, values, size) {
...
struct MeanStd {
  ...
  MeanStd(const Type *values, size_t _size) : MeanStd() {
  ...
  TYPEW GetMean() const {
    return static_cast<TYPEW>(sum / static_cast<TypeR>(size));
  }
  TYPEW GetRMS() const {
    return static_cast<TYPEW>(SQRT(sumSq / static_cast<TypeR>(size)));
  }
  TYPEW GetVarianceN() const {
    return static_cast<TYPEW>(sumSq - SQUARE(sum) / static_cast<TypeR>(size));
  }
  TYPEW GetVariance() const {
    return static_cast<TYPEW>(GetVarianceN() / static_cast<TypeR>(size));
  }
...
struct MeanStdMinMax : MeanStd<TYPE, TYPEW, ARGTYPE, TYPER> {
  ...
  MeanStdMinMax(const Type *values, size_t _size) : MeanStdMinMax() {
    Compute(values, _size);
  }
  void Update(ArgType v) {
    if (minVal > v)
...
// 1986 returns the pair(median,trust_region) upper-bound threshold =
// median+trust_region lower-bound threshold = median-trust_region
template <typename TYPE, typename TYPEW>
inline std::pair<TYPEW, TYPEW>
ComputeX84Threshold(const TYPE *const values, size_t size,
                    TYPEW mul = TYPEW(5.2), const uint8_t *mask = NULL) {
  ASSERT(size > 0);
...
template <typename TYPE, typename TYPEW>
inline std::tuple<TYPEW, TYPEW, TYPEW, TYPEW, TYPEW>
ComputeQuartileThreshold(const TYPE *const values, size_t size,
                         TYPEW mul = TYPEW(1.5), const uint8_t *mask = NULL) {
  ASSERT(size > 0);
...
inline REAL ComputeSNR(const TDMatrix<TYPE> &x0, const TDMatrix<TYPE> &x) {
  ...
  if (ISZERO(x0Norm) && err > 0)
    ret = 0;
  else if (err > 0)
    ret = 20.0 * std::log10(x0Norm / err);
  return ret;
} // ComputeSNR
template <typename TYPE, int N>
inline REAL ComputeSNR(const TMatrix<TYPE, N, 1> &x0,
                       const TMatrix<TYPE, N, 1> &x) {
  return ComputeSNR(TDMatrix<TYPE>(N, 1, (TYPE *)x0.val),
                    TDMatrix<TYPE>(N, 1, (TYPE *)x.val));
} // ComputeSNR
template <typename TYPE>
...
template <typename TYPE, int N>
inline REAL ComputePSNR(const TMatrix<TYPE, N, 1> &x0,
                        const TMatrix<TYPE, N, 1> &x) {
  return ComputePSNR(TDMatrix<TYPE>(N, 1, (TYPE *)x0.val),
                     TDMatrix<TYPE>(N, 1, (TYPE *)x.val));
} // ComputePSNR
/*----------------------------------------------------------------*/
...
template <typename TYPE>
inline TYPE BandwidthGaussianKDE(const TMatrix<TYPE, 2, 1> *samples,
                                 size_t numSamples, TYPE sigma = 0,
                                 TYPE *pSigma = NULL) {
  if (sigma == 0) {
    TYPE avg(0);
  ...
  // (see "Applied Smoothing Techniques for Data Analysis" by Adrian W, Bowman &
  // Adelchi Azzalini (1997))
  return POW(TYPE(4) / (TYPE(3) * numSamples), TYPE(1) / TYPE(5)) * sigma;
}
// estimate density at the given value
template <typename TYPE>
inline TYPE KernelDensityEstimation(const TMatrix<TYPE, 2, 1> *samples,
                                    size_t numSamples, TYPE x, TYPE bandwidth) {
  TYPE y(0);
...
// INSERTER should have defined the operator(TYPE,TYPE) that receives a pair of
// value and density
template <typename TYPE, typename INSERTER>
void KernelDensityEstimation(const TMatrix<TYPE, 2, 1> *samples,
                             size_t numSamples, TYPE bandwidth, TYPE xMin,
                             TYPE xMax, size_t steps, INSERTER &inserter) {
...
// same as above, but tries to detect local minimum/maximum regions and sample
// them denser; INSERTER should have defined also the function SamplePeak(bool)
// that receives true/false if the region contains a maximum/minimum point
// respectively and returns true/false if the region should or not be sampled
// more often; at the end of the peak sampling EndPeak() is called
template <typename TYPE, typename INSERTER>
void KernelDensityEstimation(const TMatrix<TYPE, 2, 1> *samples,
                             size_t numSamples, TYPE bandwidth, TYPE xMin,
                             TYPE xMax, TYPE stepMax, INSERTER &inserter) {
  size_t smallSteps(0);
  TYPE step(stepMax);
  TYPE x(xMin), last_x;
  ...
  do {
    ...
    if (smallSteps) {
      ...
      if (ABS(SIGN(last_delta) + SIGN(delta)) < 2 &&
          inserter.SamplePeak(last_delta > 0)) {
...
inline void ExampleKDE() {
  ...
  struct SamplesInserter {
    ...
    inline bool SamplePeak(bool bMaxRegion) const { return bMaxRegion; }
    inline void EndPeak() {}
  };
...
// normalize image points (inhomogeneous 2D) so that their centroid and typical
// magnitude of the vector is (1,1)
template <typename TYPE, typename HTYPE>
void NormalizePoints(const CLISTDEF0(TPoint2<TYPE>) & pointsIn,
                     CLISTDEF0(TPoint2<TYPE>) & pointsOut,
                     TMatrix<HTYPE, 3, 3> *pH = NULL) {
  // find centroid
  ...
  *pPt *= scale;
  // initialize normalizing homography matrix from Scale and Translation (H = S
  // * T);
  if (pH != NULL) {
...
// normalize scene points (inhomogeneous 3D) so that their centroid and typical
// magnitude of the vector is (1,1,1) This normalization algorithm is suitable
// only for compact distribution of points (see Hartley04 p180)
template <typename TYPE, typename HTYPE>
void NormalizePoints(const CLISTDEF0(TPoint3<TYPE>) & pointsIn,
                     CLISTDEF0(TPoint3<TYPE>) & pointsOut,
                     TMatrix<HTYPE, 4, 4> *pH = NULL) {
  // find centroid
