...
typedef struct SMLVALUE_TYPE {
  ...
  SMLVALUE_TYPE(const SMLVALUE_TYPE &r) : data(r.data) {
    ((SMLVALUE_TYPE &)r).data = NULL;
  }
...
public:
  ...
  void SetFncItem(TFncInitItem, TFncSaveItem, TFncReleaseItem,
                  void *data = NULL);
...
private:
  ...
  TFncReleaseItem
      m_fncReleaseItem; // callback function used to release each item
  void *m_fncItemData;
};
/*----------------------------------------------------------------*/
