...
template <typename TYPE> inline TQuaternion<TYPE>::~TQuaternion() {}

template <typename TYPE> inline TQuaternion<TYPE>::TQuaternion() : Base() {}

template <typename TYPE>
inline TQuaternion<TYPE>::TQuaternion(const Base &q) : Base(q) {}
...
template <typename TYPE> inline void TQuaternion<TYPE>::Invert() {
  Base::operator()(0) *= -1;
  Base::operator()(1) *= -1;
  Base::operator()(2) *= -1;
}
...
template <typename TYPE> inline void TQuaternion<TYPE>::MakeUnique() {
  // project quaternion to upper hemisphere (first ima.part >= 0)
  TYPE *qThis = this->val;
  if ((qThis[3] <= QUATERNION_EPSILON) && (qThis[3] >= -QUATERNION_EPSILON)) {
    if ((qThis[1] <= QUATERNION_EPSILON) && (qThis[1] >= -QUATERNION_EPSILON)) {
...
template <typename TYPE>
inline void TQuaternion<TYPE>::Mult(const TQuaternion<TYPE> &arg,
...
template <typename TYPE>
inline typename TQuaternion<TYPE>::Vec
TQuaternion<TYPE>::MultVec(const Vec &vec) const {
  Vec r;
  MultVec(vec, r);
  return r;
}

template <typename TYPE> inline void TQuaternion<TYPE>::SetIdentity() {
  Base::operator()(0) = Base::operator()(1) = Base::operator()(2) = 0;
  Base::operator()(3) = 1;
}

template <typename TYPE>
inline void TQuaternion<TYPE>::SetQuaternion(TYPE real, TYPE i, TYPE j,
                                             TYPE k) {
...
template <typename TYPE>
inline void TQuaternion<TYPE>::SetValueAsAxisRad(TYPE x, TYPE y, TYPE z,
                                                 TYPE w) {
...
template <typename TYPE>
typename TQuaternion<TYPE>::Mat
TQuaternion<TYPE>::GetQuaternionMultMatrixLeft() const {
  Mat res;
...
template <typename TYPE>
typename TQuaternion<TYPE>::Mat
TQuaternion<TYPE>::GetQuaternionMultMatrixRight() const {
  Mat res;
...
template <typename TYPE> TYPE TQuaternion<TYPE>::GetRotationAngle() const {
  const TYPE angle = TYPE(2 * acos((*this)[3]));
  return std::isnan(angle) &&
                 ABS(ABS((*this)[3]) - TYPE(1)) < QUATERNION_EPSILON
             ? TYPE(0)
             : angle;
}
...
template <typename TYPE>
TQuaternion<TYPE>
TQuaternion<TYPE>::InterpolateLinear(const TQuaternion<TYPE> &to,
                                     const TYPE &t) const {
...
template <typename TYPE>
TQuaternion<TYPE> TQuaternion<TYPE>::Interpolate(const TQuaternion<TYPE> &to,
                                                 const TYPE &t) const {
  BIASASSERT(t >= 0.0 && t <= 1.0);
  TQuaternion<TYPE> p = *this;
  TQuaternion<TYPE> q = to;
...
template <typename TYPE>
void TQuaternion<TYPE>::EnforceRigidCouplingConstraint(
    TQuaternion<TYPE> &other) {
...
template <typename TYPE>
void TQuaternion<TYPE>::EnforceRigidCouplingConstraint(
    std::vector<TQuaternion<TYPE>> &quats) {
...
template <typename TYPE>
inline TRMatrixBase<TYPE>::TRMatrixBase() : Base(Base::IDENTITY) {}

template <typename TYPE>
inline TRMatrixBase<TYPE>::TRMatrixBase(const Mat &mat) : Base(mat) {
...
template <typename TYPE> void TRMatrixBase<TYPE>::Set(const Vec &wa, TYPE phi) {
  ...
  if (wnorm < std::numeric_limits<TYPE>::epsilon()) {
    CPC_ERROR("Vector " << wa << " is close to zero (norm = " << wnorm
...
template <typename TYPE>
void TRMatrixBase<TYPE>::SetFromColumnVectors(const Vec &v0, const Vec &v1,
                                              const Vec &v2) {
  val[0] = v0[0];
  val[1] = v1[0];
  val[2] = v2[0];
...
template <typename TYPE>
void TRMatrixBase<TYPE>::SetFromRowVectors(const Vec &v0, const Vec &v1,
                                           const Vec &v2) {
  val[0] = v0[0];
  val[1] = v0[1];
  val[2] = v0[2];
...
void TRMatrixBase<TYPE>::GetQuaternion(Quat &quat) const {
  ...
  TYPE tr =
      Base::operator()(0, 0) + Base::operator()(1, 1) + Base::operator()(2, 2);
  ...
  if (tr > 0.0) {
    ...
    s = sqrt(Base::operator()(i, i) -
             (Base::operator()(j, j) + Base::operator()(k, k)) + 1.0);
  ...
  // @see
  // http://en.wikipedia.org/wiki/Rotation_representation_%28mathematics%29#Rotation_matrix_.E2.86.94_quaternion
  // @deprecated

  // Determine max. denominator
  TYPE d[4] = {1 + Base::operator()(0, 0) - Base::operator()(1, 1) -
                   Base::operator()(2, 2),
               1 - Base::operator()(0, 0) + Base::operator()(1, 1) -
                   Base::operator()(2, 2),
               1 - Base::operator()(0, 0) - Base::operator()(1, 1) +
                   Base::operator()(2, 2),
               1 + Base::operator()(0, 0) + Base::operator()(1, 1) +
  ...
  if (maxidx == 0) {
    quat[0] = 0.5 * sqrt(1 + Base::operator()(0, 0) - Base::operator()(1, 1) -
                         Base::operator()(2, 2));
    quat[3] =
        0.25 * (Base::operator()(2, 1) - Base::operator()(1, 2)) / quat[0];
    quat[1] =
        0.25 * (Base::operator()(0, 1) + Base::operator()(1, 0)) / quat[0];
    quat[2] =
        0.25 * (Base::operator()(0, 2) + Base::operator()(2, 0)) / quat[0];
  } else if (maxidx == 1) {
    quat[1] = 0.5 * sqrt(1 - Base::operator()(0, 0) + Base::operator()(1, 1) -
                         Base::operator()(2, 2));
    quat[3] =
        0.25 * (Base::operator()(0, 2) - Base::operator()(2, 0)) / quat[1];
    quat[2] =
        0.25 * (Base::operator()(1, 2) + Base::operator()(2, 1)) / quat[1];
    quat[0] =
        0.25 * (Base::operator()(1, 0) + Base::operator()(0, 1)) / quat[1];
  } else if (maxidx == 2) {
    quat[2] = 0.5 * sqrt(1 - Base::operator()(0, 0) - Base::operator()(1, 1) +
                         Base::operator()(2, 2));
    quat[3] =
        0.25 * (Base::operator()(1, 0) - Base::operator()(0, 1)) / quat[2];
    quat[0] =
        0.25 * (Base::operator()(2, 0) + Base::operator()(0, 2)) / quat[2];
    quat[1] =
        0.25 * (Base::operator()(2, 1) + Base::operator()(1, 2)) / quat[2];
  } else {
    quat[3] = 0.5 * sqrt(1 + Base::operator()(0, 0) + Base::operator()(1, 1) +
                         Base::operator()(2, 2));
    quat[0] =
        0.25 * (Base::operator()(2, 1) - Base::operator()(1, 2)) / quat[3];
    quat[1] =
        0.25 * (Base::operator()(0, 2) - Base::operator()(2, 0)) / quat[3];
    quat[2] =
        0.25 * (Base::operator()(1, 0) - Base::operator()(0, 1)) / quat[3];
  }
  ...
  if (trace > 0) {
    ...
    if (Base::operator()(0, 0) > Base::operator()(1, 1) &&
        Base::operator()(0, 0) > Base::operator()(2, 2)) {
      TYPE s = 2.0 * sqrt(1.0 + Base::operator()(0, 0) -
                          Base::operator()(1, 1) - Base::operator()(2, 2));
      ...
      TYPE s = 2.0 * sqrt(1.0 + Base::operator()(1, 1) -
                          Base::operator()(0, 0) - Base::operator()(2, 2));
      ...
      TYPE s = 2.0 * sqrt(1.0 + Base::operator()(2, 2) -
                          Base::operator()(0, 0) - Base::operator()(1, 1));
  ...
  quat[3] =
      sqrt(max(0.0, 1.0 + Base::operator()(0, 0) + Base::operator()(1, 1) +
                        Base::operator()(2, 2))) /
      2.0;
  quat[0] =
      sqrt(max(0.0, 1.0 + Base::operator()(0, 0) - Base::operator()(1, 1) -
...
template <typename TYPE> TYPE TRMatrixBase<TYPE>::GetRotationAngle() const {
  Vec Axis;
  TYPE Angle;
  GetRotationAxisAngle(Axis, Angle);
  return Angle;
...
template <typename TYPE>
int TRMatrixBase<TYPE>::GetRotationAnglesXYZ(TYPE &PhiX, TYPE &PhiY,
                                             TYPE &PhiZ) const {
#if 1
  if (Base::operator()(0, 2) < 1) {
    ...
    } else { // WARNING.  Not a unique solution.
      PhiY = -FHALF_PI;
  ...
  if ((Base::operator()(0, 2) == 1.0) || (Base::operator()(0, 2) == -1.0)) {
    ASSERT("Rotation for Y-axis is +-PI/2, can not decompose X-/Z-component!" ==
           NULL);
    return -1;
  }
  PhiY = asin(Base::operator()(0, 2));
...
template <typename TYPE>
int TRMatrixBase<TYPE>::GetRotationAnglesZXY(TYPE &PhiX, TYPE &PhiY,
                                             TYPE &PhiZ) const {
  if ((Base::operator()(0, 2) == 1.0) || (Base::operator()(0, 2) == -1.0)) {
...
template <typename TYPE>
int TRMatrixBase<TYPE>::GetRotationAnglesYXZ(TYPE &PhiY, TYPE &PhiX,
                                             TYPE &PhiZ) const {
  if ((Base::operator()(0, 2) == 1.0) || (Base::operator()(0, 2) == -1.0)) {
    DEBUG("Rotation for Y-axis is +-PI/2, can not decompose X-/Z-component!");
    return -1;
...
template <typename TYPE>
int TRMatrixBase<TYPE>::GetRotationAnglesZYX(TYPE &PhiX, TYPE &PhiY,
                                             TYPE &PhiZ) const {
  if ((Base::operator()(0, 2) == 1.0) || (Base::operator()(0, 2) == -1.0)) {
  ...
  for (unsigned i = 0; i < 3; i++) {
    for (unsigned j = 0; j < 3; j++) {
      if (!equal(Base::operator()(i, j), RCompare[i][j], TYPE(1e-12))) {
        SLOG("Matrices are not same !" << *this << " " << RCompare
                                       << std::endl);
        SLOG(" at " << i << " " << j << " "
                    << (Base::operator()(i, j)) - (RCompare[i][j])
                    << std::endl);
        ABORT("BIASABORT");
      }
    }
...
template <typename TYPE>
void TRMatrixBase<TYPE>::SetFromOrthogonalBasis(const Vec &xxx, const Vec &yyy,
                                                const Vec &zzz) {
  SetFromRowVectors(normalized(xxx), normalized(yyy), normalized(zzz));
}
...
template <typename TYPE>
TRMatrixBase<TYPE> &TRMatrixBase<TYPE>::SetFromDir2Dir(const Vec &dir0,
                                                       const Vec &dir1) {
...
template <typename TYPE>
inline typename TRMatrixBase<TYPE>::Vec
TRMatrixBase<TYPE>::GetRotationAxisAngle() const {
#if 0
  ...
  const TYPE costheta(
      MINF(MAXF(((*this)(0, 0) + (*this)(1, 1) + (*this)(2, 2) - kOne) / kTwo,
                TYPE(-1.0)),
           kOne));
...
// Thus we use an alternate more complicated and expensive formula.
  ...
  for (int i = 0; i < 3; ++i) {
    angle_axis[i] =
        theta * sqrt(((*this)(i, i) - costheta) * inv_one_minus_costheta);
  ...
  if (!ok && VERBOSITY_LEVEL > verbose) {
    ...
    for (unsigned i = 0; i < 3; i++) {
      ...
      if (!equal(norm(cl[i]), TYPE(1), eps)) {
        res << "col[" << i << "].NormL2() != 1) (= " << norm(cl[i]) << ") ";
      }
    }
    if (!equal(cl[0].dot(cl[1]), TYPE(0), eps)) {
      res << "col[0].ScalarProduct(c[1]) != 0 (= " << cl[0].dot(cl[1]) << ") ";
    }
    if (!equal(cl[0].dot(cl[2]), TYPE(0), eps)) {
      res << "col[0].ScalarProduct(c[2]) != 0 (= " << cl[0].dot(cl[2]) << ") ";
    }
    if (!equal(cl[1].dot(cl[2]), TYPE(0), eps)) {
      res << "col[1].ScalarProduct(c[2]) != 0 (= " << cl[1].dot(cl[2]) << ") ";
    }
...
template <typename TYPE> void TRMatrixBase<TYPE>::EnforceOrthogonality() {
#if 1
  // use SVD decomposition to find the closest orthogonal matrix
  const cv::SVD svd(*this, cv::SVD::MODIFY_A | cv::SVD::FULL_UV);
  *this = svd.u * svd.vt;
#elif 1
  // replaces the 3 x 3 matrix with the nearest (least squares) orthogonal
  // matrix; a combination of Babylonian iteration and Berthold KP Horn's
  // formula R = M (MT M)^-1/2
  TYPE *r = this->val;
  TYPE m[9], ri[9], x[9];
  memcpy(m, r, 9 * sizeof(TYPE));
  // By trial and error I found that 4 iterates always works unless your matrix
  // is waaay off. Limit 10.
  for (unsigned iterate = 0; iterate != 10; iterate++) {
    // Invert r. Use inverse and not transpose since r is only approximately
    // orthogonal (transpose will result in slow, oscillating convergence).
    InvertMatrix3x3(r, ri);

    // Get x = ri m + mt r.
    x[0] = m[6] * r[6] + ri[2] * m[6] + m[3] * r[3] + ri[1] * m[3] +
           m[0] * ri[0] + m[0] * r[0];
    x[1] = m[6] * r[7] + ri[2] * m[7] + m[3] * r[4] + ri[1] * m[4] +
           m[0] * r[1] + ri[0] * m[1];
    x[2] = m[6] * r[8] + ri[2] * m[8] + m[3] * r[5] + ri[1] * m[5] +
           m[0] * r[2] + ri[0] * m[2];
    x[3] = r[6] * m[7] + ri[5] * m[6] + m[3] * ri[4] + r[3] * m[4] +
           m[0] * ri[3] + r[0] * m[1];
    x[4] = m[7] * r[7] + ri[5] * m[7] + m[4] * ri[4] + m[4] * r[4] +
           m[1] * ri[3] + m[1] * r[1];
    x[5] = m[7] * r[8] + ri[5] * m[8] + m[4] * r[5] + ri[4] * m[5] +
           m[2] * ri[3] + m[1] * r[2];
    x[6] = m[6] * ri[8] + r[6] * m[8] + m[3] * ri[7] + m[0] * ri[6] +
           r[3] * m[5] + r[0] * m[2];
    x[7] = m[7] * ri[8] + r[7] * m[8] + m[4] * ri[7] + m[1] * ri[6] +
           r[4] * m[5] + r[1] * m[2];
    x[8] = m[8] * ri[8] + m[8] * r[8] + m[5] * ri[7] + m[2] * ri[6] +
           m[5] * r[5] + m[2] * r[2];
...
inline bool IsRotationMatrix(const Eigen::Matrix<TYPE, 3, 3, O> &R) {
  ...
  return (ISEQUAL(R.determinant(), TYPE(1)) &&
          ISEQUAL((R * R.transpose()).trace(), TYPE(3)));
} // IsRotationMatrix
/*----------------------------------------------------------------*/
...
// using log(R) as in: "Efficient and Robust Large-Scale Rotation Averaging",
// 2013 same result as above, but returns directly the angle
template <typename TYPE>
inline TYPE ComputeAngleSO3(const TMatrix<TYPE, 3, 3> &I) {
  return TYPE(norm(((const TRMatrixBase<TYPE> &)I).GetRotationAxisAngle()));
} // ComputeAngleSO3
template <typename TYPE>
FORCEINLINE TYPE ComputeAngleSO3(const TMatrix<TYPE, 3, 3> &R1,
