...
public:
  ...
  inline File(LPCTSTR aFileName, int access, int mode, int flags = 0)
  ...
  bool isOpen() const { return h != FILE_INVALID_HANDLE; }
  ...
  bool setPos(size_f_t pos) override {
    ASSERT(isOpen());
    LONG x = (LONG)(pos >> 32);
    return (::SetFilePointer(h, (DWORD)(pos & 0xffffffff), &x, FILE_BEGIN) !=
  ...
  virtual bool setEndPos(size_f_t pos) {
    ASSERT(isOpen());
    LONG x = (LONG)(pos >> 32);
    return (::SetFilePointer(h, (DWORD)(pos & 0xffffffff), &x, FILE_END) !=
            INVALID_SET_FILE_POINTER);
  }
  ...
  virtual bool movePos(size_f_t pos) {
    ASSERT(isOpen());
    LONG x = (LONG)(pos >> 32);
    return (::SetFilePointer(h, (DWORD)(pos & 0xffffffff), &x, FILE_CURRENT) !=
            INVALID_SET_FILE_POINTER);
  }
  ...
  static bool copyFile(LPCTSTR source, LPCTSTR target) {
    return ::CopyFile(source, target, FALSE) == TRUE;
  }

  static size_f_t getSize(LPCTSTR aFileName) {
    const HANDLE fh =
        ::CreateFile(aFileName, FILE_READ_ATTRIBUTES, FILE_SHARE_READ, NULL,
  ...
  static size_f_t findFiles(const String &_strPath, const String &strMask,
                            bool bProcessSubdir, FileInfoArr &arrFiles) {
    // List all the files.
    WIN32_FIND_DATA fd;
    HANDLE hFind;
...
#ifdef LARGEFILESIZE
        fileInfo.size = (((size_f_t)fd.nFileSizeLow) |
                         (((size_f_t)fd.nFileSizeHigh) << 32));
#else
    ...
    if (hFind != FILE_INVALID_HANDLE) {
      do {
        ...
        totalSize += findFiles(strPath + fd.cFileName + PATH_SEPARATOR, strMask,
                               true, arrFiles);
      } while (FindNextFile(hFind, &fd));
      FindClose(hFind);
    }
  ...
  bool setPos(size_f_t pos) override {
    ASSERT(isOpen());
    return lseek(h, (off_t)pos, SEEK_SET) != (off_t)-1;
  }
  virtual bool setEndPos(size_f_t pos) {
    ASSERT(isOpen());
    return lseek(h, (off_t)pos, SEEK_END) != (off_t)-1;
  }
  virtual bool movePos(size_f_t pos) {
    ASSERT(isOpen());
    return lseek(h, (off_t)pos, SEEK_CUR) != (off_t)-1;
  }
  ...
  // a file can be a regular file, a symbolic link, a FIFO or a socket, but not
  // a device
  ...
  template <class VECTOR> inline size_t write(const VECTOR &arr) {
  ...
  template <class VECTOR> inline size_t read(VECTOR &arr) {
  ...
  InputStream *getInputStream(int typ = InputStream::LAYER_ID_IN) override {
    return (typ == LAYER_ID_IN ? static_cast<InputStream *>(this)
                               : IOStream::getInputStream(typ));
  }
  enum { LAYER_ID_OUT = 3 };
  OutputStream *getOutputStream(int typ = OutputStream::LAYER_ID_OUT) override {
    return (typ == LAYER_ID_OUT ? static_cast<OutputStream *>(this)
                                : IOStream::getOutputStream(typ));
  }
