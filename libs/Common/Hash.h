...
private:
  ...
  template <size_t N> inline uint32_t _Hash(const char (&str)[N]) const {
    ...
    return str[N - 1] +
           65599 * (str[N - 2] + 65599 * _Hash((truncated_str)str));
  }
  inline uint32_t _Hash(const char (&str)[3]) const {
    return str[2] + 65599 * (str[1] + 65599 * str[0]);
  }
  inline uint32_t _Hash(const char (&str)[2]) const {
    return str[1] + 65599 * str[0];
  }
...
template <typename Type> class cHashTable {
public:
...
public:
  ...
  inline const Type *GetEnd() const {
    return m_values.GetData() + m_values.GetSize();
  }
...
private:
  inline Size _StaticKeyToID(Key key, Size size) const {
    return key & (size - 1);
  }
  inline Size _KeyToID(Key key) const {
    return _StaticKeyToID(key, (Size)m_indices.GetSize());
  }
  inline Idx _IDToIndex(Size id) const { return m_indices[id] - 1; }
  inline Idx _KeyToIndex(Key key) const {
    return (m_indices.IsEmpty() ? KEY_NA : _IDToIndex(_KeyToID(key)));
  }

  // Completely discards then rebuilds all indices based on the current set of
  // keys
...
public:
  ...
  inline const Type *Find(Key key) const {
    ...
    return (index < m_keys.GetSize() && m_keys[index] == key) ? &m_values[index]
                                                              : NULL;
  }
  ...
  // Retrieves a value from the hash -- but it will only be valid if it exists,
  // so be careful. In most cases you will either want to use an Exists() check
  // first, or simply use the Find function.
  ...
  Type &operator[](Key key) {
    ...
    if (index != KEY_NA) {
      ...
      if (oldKey == key) {
        ...
        // Setting the key was unsuccessful due to another entry colliding with
        // our key; we must expand the indices until we find a set of keys that
        // will match.
        Size newSize = m_indices.GetSize();
        while (newSize < (KEY_MAXSIZE >> 1)) {
          newSize = newSize << 1;
          // Find the next best size for the hash that would make both keys
          // unique
    ...
    // Add this new entry to the index list using the current array size as the
    // 1-based index
...
public:
  ...
  static inline Key HashKey(const uint8_t *data, Size size) {
    return HashKeyFNV(data, size);
  }
  static inline Key HashKey(LPCTSTR sz) {
    return HashKey((const uint8_t *)sz, (Size)_tcslen(sz));
  }
  static inline Key HashKey(const String &str) {
    return HashKey((const uint8_t *)str.c_str(), (Size)str.size());
  }
  ...
  inline const Type *Find(const String &key) const {
    return Find(HashKey(key));
  }
  ...
  inline bool Exists(const String &key) const {
    return Exists(HashKey(key.c_str()));
  }
  inline Type &operator[](LPCTSTR key) { return (*this)[HashKey(key)]; }
  inline Type &operator[](const String &key) {
    return (*this)[HashKey(key.c_str())];
  }
  inline const Type &operator[](LPCTSTR key) const {
    return (*this)[HashKey(key)];
  }
  inline const Type &operator[](const String &key) const {
    return (*this)[HashKey(key.c_str())];
  }
};
...
public:
  ...
  inline iterator Insert(const Key &key, bool &bExisted) {
    return Insert(key, Type(), bExisted);
  }
