...
template <typename TYPE, int DIMS> class TOBB {
...
public:
  ...
  template <typename CTYPE> inline TOBB(const TOBB<CTYPE, DIMS> &);

  inline void Set(const AABB &); // build from AABB
  inline void Set(const MATRIX &rot, const POINT &ptMin,
                  const POINT &ptMax); // build from rotation matrix from world
                                       // to local, and local min/max corners
  inline void Set(const POINT *pts, size_t n); // build from points
  inline void Set(const POINT *pts, size_t n, const TRIANGLE *tris,
                  size_t s); // build from triangles
  inline void Set(const MATRIX &C, const POINT *pts,
                  size_t n); // build from covariance matrix
  inline void
  SetRotation(const MATRIX &C); // build rotation only from covariance matrix
  inline void
  SetBounds(const POINT *pts,
            size_t n); // build size and center only from given points
