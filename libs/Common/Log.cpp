...
// generate default log type
  ...
  appName =
      (Util::getFileExt(appName) == _T(".exe") ? Util::getFileName(appName)
                                               : Util::getFileNameExt(appName));
 ...
 */
void Log::ResetTypes() { m_arrLogTypes.Empty(); }
...
void Log::_Record(Idx lt, LPCTSTR szFormat, va_list args) {
...
#endif
#ifdef LOG_DATE
  szPtrTime += GetDateFormat(LOCALE_USER_DEFAULT, 0, &st, _T("dd'.'MM'.'yy"),
                             szPtrTime, 80) -
               1;
#endif
#if defined(LOG_DATE) && defined(LOG_TIME)
...
#endif
#ifdef LOG_TIME
  szPtrTime += GetTimeFormat(LOCALE_USER_DEFAULT, 0, &st, _T("HH':'mm':'ss"),
                             szPtrTime, 80) -
               1;
#endif
#else // _MSC_VER
  ...
  LPCTSTR const logType(lt < m_arrLogTypes.GetSize() ? m_arrLogTypes[lt]
  ...
  if ((size_t)_vsntprintf(szBuffer, 2048, szFormat, args) > 2048) {
    ...
    m_message.FormatSafe("%s [%s] %s" LINE_SEPARATOR_STR, szTime, logType,
                         String::FormatStringSafe(szFormat, args).c_str());
  } else {
    // enough space for all the string, print directly
    m_message.Format("%s [%s] %s" LINE_SEPARATOR_STR, szTime, logType,
                     szBuffer);
  }
  TRACE(m_message);
...
bool LogFile::Open(LPCTSTR logName) {
  ...
  GET_LOG().RegisterListener(
      DELEGATEBINDCLASS(Log::ClbkRecordMsg, &LogFile::Record, this));
  return true;
}
void LogFile::Close() {
  if (m_ptrFile == NULL)
    return;
  GET_LOG().UnregisterListener(
      DELEGATEBINDCLASS(Log::ClbkRecordMsg, &LogFile::Record, this));
  m_ptrFile = NULL;
}
...
#endif
      m_cout(NULL), m_coutOld(NULL), m_cerr(NULL), m_cerrOld(NULL),
...
void LogConsole::Open() {
  ...
  if (_fileno(stdout) == -1 || _get_osfhandle(fileno(stdout)) == -1) {
    int hCrt =
        ::_open_osfhandle((intptr_t)GetStdHandle(STD_OUTPUT_HANDLE), _O_TEXT);
    if (hCrt != -1) {
  ...
  if (_fileno(stderr) == -1 || _get_osfhandle(fileno(stderr)) == -1) {
    int hCrt =
        ::_open_osfhandle((intptr_t)::GetStdHandle(STD_ERROR_HANDLE), _O_TEXT);
    if (hCrt != -1) {
  ...
  if (_fileno(stdin) == -1 || _get_osfhandle(fileno(stdin)) == -1) {
    int hCrt =
        ::_open_osfhandle((intptr_t)::GetStdHandle(STD_INPUT_HANDLE), _O_TEXT);
    if (hCrt != -1) {
  ...
  GET_LOG().RegisterListener(
      DELEGATEBINDCLASS(Log::ClbkRecordMsg, &LogConsole::Record, this));
}
