...
public:
  ...
  inline Type GetCurrent() const {
    return m_fTimeScale * SysTime2Time(m_nCrntTime);
  }
...
public:
  ...
  static inline Type GetTimeMs() { return GetTimeFactor() * GetSysTime(); }
  // get current time in seconds
  static inline Type GetTime() {
    return 0.001f * GetTimeFactor() * GetSysTime();
  }
  // convert given time to milliseconds
  static inline Type SysTime2TimeMs(SysType t) { return GetTimeFactor() * t; }
...
public:
  AutoTimer(Type &duration) : m_duration(duration) {
    m_duration = Timer::GetTime();
  }
...
public:
  AutoAddTimer(Type &duration) : m_duration(duration) {
