...
public:
  ...
  Event *GetEvent(); // block until an event arrives and get the first event
                     // pending in the queue
  Event *
  GetEvent(uint32_t millis); // block until an event arrives or time expires and
                             // get the first event pending in the queue
  Event *GetEventLast(); // block until an event arrives and get the last event
                         // pending in the queue
  Event *
  GetEventLast(uint32_t millis); // block until an event arrives or time expires
                                 // and get the last event pending in the queue
...
public:
  ...
  inline EventThreadPool(size_type nThreads, Thread::FncStart pfnStarter,
                         void *pData = NULL)
