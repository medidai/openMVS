...
public:
  ...
  inline String(LPCTSTR sz, size_t offset, size_t count)
      : Base(sz, offset, count) {}
  ...
  inline String &operator=(Base &&rhs) {
...
#endif
  inline String &operator=(TCHAR rhs) {
    Base::operator=(rhs);
    return *this;
  }
  inline String &operator=(LPCTSTR rhs) {
    Base::operator=(rhs);
    return *this;
  }
  ...
  inline String &operator+=(const Base &rhs) {
    *this = (*this) + rhs;
    return *this;
  }
  inline String &operator+=(const String &rhs) {
  ...
  inline void ToUpper(String &out) const {
    ...
    std::transform(begin(), end(), out.begin(),
                   [](TCHAR c) { return (TCHAR)std::toupper(c); });
  }
  ...
  inline void ToLower(String &out) const {
    out.resize(size());
    std::transform(begin(), end(), out.begin(),
                   [](TCHAR c) { return (TCHAR)std::tolower(c); });
  }
  ...
  template <class T> static String ToString(const T &val) {
    std::ostringstream os;
    os << val;
    return os.str();
  }
  template <class T> static String ToStringHex(const T &val) {
    std::ostringstream os;
    os << std::hex << val;
    return os.str();
  }

  template <class T> static void FromString(const String &str, T &val) {
    std::istringstream is(str);
    is >> val;
  }
  template <class T> static T FromString(const String &str, const T &def) {
    T val(def);
    FromString(str, val);
    return val;
  }
  ...
  template <class T> inline void From(T &val) const { FromString(*this, val); }
  template <class T> inline T From(const T &def) const {
    T val(def);
    From(val);
    return val;
  }
  template <class T> inline T From() const {
    T val;
    From(val);
    return val;
  }

  static int CompareAlphabetically(const void *elem, const void *key) {
    return _tcscmp(((const String *)elem)->c_str(),
                   ((const String *)key)->c_str());
  }
  static int CompareAlphabeticallyInv(const void *elem, const void *key) {
    return _tcscmp(((const String *)key)->c_str(),
                   ((const String *)elem)->c_str());
  }
...
inline String operator+(const String &lhs, TCHAR rhs) {
  return std::operator+(lhs, rhs);
}
inline String operator+(const String &lhs, LPCTSTR rhs) {
  return std::operator+(lhs, rhs);
}
...
inline String operator+(LPCTSTR lhs, const String &rhs) {
  return std::operator+(lhs, rhs);
}
...
#ifdef _SUPPORT_CPP11
inline String operator+(String &&lhs, TCHAR rhs) {
  return std::operator+(std::forward<String::Base>(lhs), rhs);
}
inline String operator+(String &&lhs, LPCTSTR rhs) {
  return std::operator+(std::forward<String::Base>(lhs), rhs);
}
inline String operator+(String &&lhs, const std::string &rhs) {
  return std::operator+(std::forward<String::Base>(lhs), rhs);
}
inline String operator+(TCHAR lhs, String &&rhs) {
  return std::operator+(lhs, std::forward<String::Base>(rhs));
}
inline String operator+(LPCTSTR lhs, String &&rhs) {
  return std::operator+(lhs, std::forward<String::Base>(rhs));
}
...
inline String operator+(String &&lhs, String &&rhs) {
  return std::operator+(std::forward<String::Base>(lhs),
                        std::forward<String::Base>(rhs));
}
#endif
/*----------------------------------------------------------------*/
