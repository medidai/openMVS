...
// Tracks accesses of some hash-able type T and records the least recently
// accessed.
template <typename T> class ListFIFO {
public:
  ...
  bool Contains(const T &key) const { return map.find(key) != map.end(); }

  // return the keys currently in cache
  const std::list<T> &GetCachedValues() const { return order; }
