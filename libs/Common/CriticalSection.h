 ...
 * It is meant to be used in situations where the risk for lock conflict is very
 * low, i e locks that are held for a very short time. The lock is _not_
 * recursive, i e if the same thread will try to grab the lock it'll hang in a
 * never-ending loop. The lock is not fair, i e the first to try to enter a
 * locked lock is not guaranteed to be the first to get it when it's freed...
 */
class FastCriticalSection {
public:
  FastCriticalSection() : state(0) {}

  void Clear() { Thread::safeExchange(state, 0); }

  void Enter() {
    while (Thread::safeCompareExchange(state, 0, 1) != 0)
      Thread::yield();
  }
  bool TryEnter() { return (Thread::safeCompareExchange(state, 0, 1) == 0); }
  void Leave() { Thread::safeDec(state); }

protected:
  volatile Thread::safe_t state;
};

template <class T> class SimpleLock {
public:
...
template <class T> class SimpleLockTry {
public:
