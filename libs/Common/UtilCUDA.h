...
// outputs the proper CUDA error code in the event that a CUDA host call returns
// an error
...
#ifdef _DEBUG
  VERBOSE("CUDA error at %s:%d: %s (%s (code %d) - %s)", __FILE__, __LINE__,
          errorMessage, szName, static_cast<unsigned>(result), szError);
#else
  DEBUG("CUDA error: %s (%s (code %d) - %s)", errorMessage, szName,
        static_cast<unsigned>(result), szError);
#endif
...
#define checkCudaError(val)                                                    \
  {                                                                            \
    const CUresult ret(SEACAVE::CUDA::__reportCudaError(val, #val));           \
    if (ret != CUDA_SUCCESS)                                                   \
      return ret;                                                              \
  }

// outputs the proper CUDA error code and abort in the event that a CUDA host
...
template <typename T> inline T align(T o, T a) {
  a -= T(1);
  return (o + a) & ~a;
}

// initialize the given CUDA device and add it to the array of initialized
// devices; if the given device is -1, the best available device is selected
...
CUresult ptxGetFunc(const CUmodule &hModule, LPCSTR functionName,
                    CUfunction &hKernel);

// add a new parameter to the given kernel
template <typename T>
inline CUresult addKernelParam(CUfunction &hKernel, int &paramOffset,
                               const T &param) {
  paramOffset = align(paramOffset, (int)alignof(T));
  const CUresult result(
      cuParamSetv(hKernel, paramOffset, (void *)&param, sizeof(T)));
...
inline CUresult copyMemDevice(const void *data, size_t size,
...
inline CUresult createReplicaDevice(const void *data, size_t size,
                                    CUdeviceptr &dataDevice) {
...
inline CUresult fetchMemDevice(void *data, size_t size,
                               const CUdeviceptr dataDevice) {
...
public:
  ...
  inline MemDevice(const void *pDataHost, size_t size) : pData(0) {
    reportCudaError(Reset(pDataHost, size));
  }
  template <typename TYPE>
  inline MemDevice(const TImage<TYPE> &param) : pData(0) {
    reportCudaError(Reset(param));
  }
  template <typename TYPE, typename ARG_TYPE, int useConstruct, int grow,
            typename IDX_TYPE>
  inline MemDevice(
      const cList<TYPE, ARG_TYPE, useConstruct, grow, IDX_TYPE> &param)
      : pData(0) {
    reportCudaError(Reset(param));
  }
  ...
  MemDevice &operator=(MemDevice &rhs) {
    pData = rhs.pData;
    rhs.pData = 0;
    return *this;
  }
  ...
  template <typename TYPE> inline CUresult Reset(const TImage<TYPE> &param) {
  ...
  template <typename TYPE, typename ARG_TYPE, int useConstruct, int grow,
            typename IDX_TYPE>
  inline CUresult
  Reset(const cList<TYPE, ARG_TYPE, useConstruct, grow, IDX_TYPE> &param) {
    ASSERT(!param.IsEmpty());
    return Reset(param.GetData(), param.GetDataSize());
  }

  CUresult SetData(const void *pDataHost, size_t size);
  template <typename TYPE> inline CUresult SetData(const TImage<TYPE> &param) {
    ASSERT(!param.empty() && param.isContinuous());
  ...
  template <typename TYPE, typename ARG_TYPE, int useConstruct, int grow,
            typename IDX_TYPE>
  inline CUresult
  SetData(const cList<TYPE, ARG_TYPE, useConstruct, grow, IDX_TYPE> &param) {
    ASSERT(!param.IsEmpty());
    return SetData(param.GetData(), param.GetDataSize());
  }

  CUresult GetData(void *pDataHost, size_t size) const;
  template <typename TYPE> inline CUresult GetData(TImage<TYPE> &param) const {
    ASSERT(!param.empty() && param.isContinuous());
  ...
  template <typename TYPE, typename ARG_TYPE, int useConstruct, int grow,
            typename IDX_TYPE>
  inline CUresult
  GetData(cList<TYPE, ARG_TYPE, useConstruct, grow, IDX_TYPE> &param) const {
    ASSERT(!param.IsEmpty());
    return GetData(param.GetData(), param.GetDataSize());
  }

  inline operator CUdeviceptr() const { return pData; }
};
...
public:
  inline EventRT(unsigned flags = CU_EVENT_DEFAULT) {
    reportCudaError(Reset(flags));
  }
  inline ~EventRT() { Release(); }

  inline bool IsValid() const { return (hEvent != NULL); }
  void Release();
  CUresult Reset(unsigned flags = CU_EVENT_DEFAULT);

  inline operator CUevent() const { return hEvent; }
};
...
public:
  inline StreamRT(unsigned flags = CU_STREAM_DEFAULT) {
    reportCudaError(Reset(flags));
  }
  inline ~StreamRT() { Release(); }

  inline bool IsValid() const { return (hStream != NULL); }
  void Release();
  CUresult Reset(unsigned flags = CU_STREAM_DEFAULT);

  inline operator CUstream() const { return hStream; }
...
public:
  ...
  inline ModuleRT(LPCSTR program, int mode = JIT::AUTO) {
    Reset(program, mode);
  }
  inline ~ModuleRT() { Release(); }

  inline bool IsValid() const { return (hModule != NULL); }
  void Release();
  CUresult Reset(LPCSTR program, int mode = JIT::AUTO);

  inline operator CUmodule() const { return hModule; }
};
...
public:
  ...
  CUfunction hKernel;
  MemDeviceArr
      inDatas; // array of pointers to the allocated memory read by the program
  MemDeviceArr outDatas; // array of pointers to the allocated memory written by
                         // the program
  int paramOffset;       // used during parameter insertion to remember current
                         // parameter position
...
public:
  ...
  inline KernelRT(const ModuleRTPtr &_ptrModule, LPCSTR functionName)
      : ptrModule(_ptrModule) {
    Reset(functionName);
  }
  inline KernelRT(LPCSTR program, LPCSTR functionName, int mode = JIT::AUTO) {
    Reset(program, functionName, mode);
  }
  ...
  struct InputParam {
    const void *data; // pointer to host data to be allocated and copied to the
                      // CUDA device
    size_t size;      // size in bytes of the data
    inline InputParam() {}
    inline InputParam(const void *_data, size_t _size)
...
//  args - variadic parameters to be passed to the kernel
  ...
  CUresult operator()(int numThreads, Args &&...args) {
    ...
    const int nBlocks(
        MAXF((numThreads + numBlockThreads - 1) / numBlockThreads, 1));
    if ((result = cuFuncSetBlockShape(hKernel, numBlockThreads, 1, 1)) !=
        CUDA_SUCCESS)
      return result;
    if (ptrStream != NULL)
  ...
  CUresult operator()(const TPoint2<int> &numThreads, Args &&...args) {
    ...
    // launch the kernel (Driver API)
    const CUdevprop &deviceProp = CUDA::devices.back().prop;
    const REAL scale(MINF(REAL(1), SQRT((REAL)deviceProp.maxThreadsPerBlock /
                                        (REAL)(numThreads.x * numThreads.y))));
    const SEACAVE::TPoint2<int> numBlockThreads(
        FLOOR2INT(SEACAVE::TPoint2<REAL>(numThreads) * scale));
    ...
    if ((result = cuFuncSetBlockShape(hKernel, numBlockThreads.x,
                                      numBlockThreads.y, 1)) != CUDA_SUCCESS)
      return result;
    if (ptrStream != NULL)
  ...
  struct ReturnParam {
    void *data;  // pointer to host data to be written with the output data from
                 // the CUDA device
    size_t size; // size in bytes of the data
    inline ReturnParam() {}
  ...
  inline CUresult GetResult(const MemDevice &memDev,
                            const ReturnParam &param) const {
  ...
  inline CUresult GetResult(int idx, const TImage<TYPE> &param) const {
    ...
    return GetResult(idx,
                     ReturnParam(param.getData(), sizeof(TYPE) * param.area()));
  }
  template <typename TYPE, typename ARG_TYPE, int useConstruct, int grow,
            typename IDX_TYPE>
  inline CUresult GetResult(
      int idx,
      const cList<TYPE, ARG_TYPE, useConstruct, grow, IDX_TYPE> &param) const {
    ASSERT(!param.IsEmpty());
...
protected:
  ...
  template <typename T> inline CUresult _AddParam(const T &param) {
  ...
  template <typename TYPE, typename ARG_TYPE, int useConstruct, int grow,
            typename IDX_TYPE>
  inline CUresult
  _AddParam(const cList<TYPE, ARG_TYPE, useConstruct, grow, IDX_TYPE> &param) {
    ASSERT(!param.IsEmpty());
    return _AddParam(InputParam(param.GetData(), param.GetDataSize()));
  }
#ifdef _SUPPORT_CPP11
  template <typename T> inline CUresult AddParam(T &&param) {
...
template <typename T> struct traits {
  static const CUarray_format format;
};
...
template <> struct traits<hfloat> {
  static const CUarray_format format = CU_AD_FORMAT_HALF;
};
...
template <typename TYPE> class TArrayRT {
public:
  typedef TYPE Type;
...
public:
  ...
  inline TArrayRT(const cv::Size &size, unsigned flags = 0) : hArray(NULL) {
    reportCudaError(Reset(size, flags));
  }
  inline TArrayRT(unsigned width, unsigned height, unsigned depth = 0,
                  unsigned flags = 0)
      : hArray(NULL) {
    reportCudaError(Reset(width, height, depth, flags));
  }
  ...
  inline bool IsValid() const { return (hArray != NULL); }
  void Release() {
    if (hArray) {
  ...
  CUresult Reset(unsigned width, unsigned height, unsigned depth = 0,
                 unsigned flags = 0) {
    Release();
  ...
  operator CUarray() const { return hArray; }
  operator CUarray &() { return hArray; }
  CUDA_ARRAY3D_DESCRIPTOR GetDescriptor() const {
    CUDA_ARRAY3D_DESCRIPTOR prop;
    cuArray3DGetDescriptor(&prop, hArray);
    return prop;
  }
  unsigned Width() const { return (unsigned)GetDescriptor().Width; }
  unsigned Height() const { return (unsigned)GetDescriptor().Height; }
  unsigned Depth() const { return (unsigned)GetDescriptor().Depth; }
  unsigned NumChannels() const { return GetDescriptor().NumChannels; }
  CUarray_format Format() const { return GetDescriptor().Format; }
  unsigned Flags() const { return GetDescriptor().Flags; }
  size_t Size() const {
    return sizeof(Type) * Width() * Height() * (Depth() > 0 ? Depth() : 1) *
           NumChannels();
  }
...
template <class TYPE> class TTextureRT {
public:
...
public:
  ...
  inline TTextureRT(const ModuleRTPtr &_ptrModule, LPCSTR texrefName,
                    CUfilter_mode filtermode = CU_TR_FILTER_MODE_POINT,
                    CUaddress_mode addrmode = CU_TR_ADDRESS_MODE_CLAMP,
                    bool bNormalizedCoords = false)
      : ptrModule(_ptrModule) {
    Reset(texrefName, filtermode, addrmode, bNormalizedCoords);
  }
  ...
  CUresult Reset(LPCSTR texrefName,
                 CUfilter_mode filtermode = CU_TR_FILTER_MODE_POINT,
                 CUaddress_mode addrmode = CU_TR_ADDRESS_MODE_CLAMP,
                 bool bNormalizedCoords = false) {
    ...
    cuTexRefSetFormat(hTexref, ARRAY::traits<Type>::format,
                      cv::DataType<Type>::channels);
    return result;
  }
  ...
  CUresult Fetch(ArrayType &array) { return cuTexRefGetArray(hTexref, array); }
};
...
template <class TYPE> class TSurfaceRT {
public:
  typedef TArrayRT<TYPE> ArrayType;
  typedef typename ArrayType::Type Type;
  typedef typename ArrayType::ImageType ImageType;
...
public:
  ...
  inline TSurfaceRT(const ModuleRTPtr &_ptrModule, LPCSTR surfrefName)
      : ptrModule(_ptrModule) {
    Reset(surfrefName);
  }
  ...
  CUresult Reset(LPCSTR surfrefName) {
    ...
    ASSERT(ptrModule != NULL && ptrModule->IsValid());
    const CUresult result(
        cuModuleGetSurfRef(&hSurfref, *ptrModule, surfrefName));
    if (result != CUDA_SUCCESS)
      Release();
    return result;
  }
