...
inline TTriangle<TYPE, DIMS>::TTriangle(const POINT &p0, const POINT &p1,
                                        const POINT &p2)
    : a(p0), b(p1), c(p2) {} // constructors
/*----------------------------------------------------------------*/

// set attributes
template <typename TYPE, int DIMS>
inline void TTriangle<TYPE, DIMS>::Set(const POINT &p0, const POINT &p1,
                                       const POINT &p2) {
  a = p0;
  b = p1;
  c = p2;
...
// get center
template <typename TYPE, int DIMS>
inline typename TTriangle<TYPE, DIMS>::POINT
TTriangle<TYPE, DIMS>::GetCenter() const {
  return (a + b + c) / TYPE(3);
}
// get AABB
template <typename TYPE, int DIMS>
inline typename TTriangle<TYPE, DIMS>::AABB
TTriangle<TYPE, DIMS>::GetAABB() const {
  return AABB(&operator[](0), 3);
}
// get plane
template <typename TYPE, int DIMS>
inline typename TTriangle<TYPE, DIMS>::PLANE
TTriangle<TYPE, DIMS>::GetPlane() const {
  return PLANE(a, b, c);
}
/*----------------------------------------------------------------*/
...
template <typename TYPE, int DIMS>
inline TRay<TYPE, DIMS>::TRay(const POINT &pt0, const POINT &pt1,
                              bool /*bPoints*/)
    : m_vDir((pt1 - pt0).normalized()), m_pOrig(pt0) {} // constructors
/*----------------------------------------------------------------*/
...
template <typename TYPE, int DIMS>
inline void TRay<TYPE, DIMS>::SetFromPoints(const POINT &pt0,
                                            const POINT &pt1) {
  m_vDir = (pt1 - pt0).normalized();
  m_pOrig = pt0;
}
template <typename TYPE, int DIMS>
inline TYPE TRay<TYPE, DIMS>::SetFromPointsLen(const POINT &pt0,
                                               const POINT &pt1) {
  m_vDir = pt1 - pt0;
...
inline void TRay<TYPE, DIMS>::DeTransform(const MATRIX &_m) {
  ...
  const VECTOR4 vcOrig(m_pOrig[0] - m(3, 0), m_pOrig[1] - m(3, 1),
                       m_pOrig[2] - m(3, 2));
...
bool TRay<TYPE, DIMS>::Intersects(const TRIANGLE &tri, TYPE *pt) const {
  ...
  // check if intersection is outside of the line segment defined by points a
  // and c
  ...
  if (v < -ZEROTOLERANCE<TYPE>() * TYPE(10))
    return false;
  // check if intersection is outside of the line segment defined by points b
  // and c
...
// same as above, but returns also the distance on the ray corresponding to the
// closest point
template <typename TYPE, int DIMS>
...
bool TRay<TYPE, DIMS>::Intersects(const OBB &obb, TYPE &t) const {
  ...
  if (!ISZERO(f)) {
    ...
    if (tmax < 0.0f)
      return false;
  } else if (((-e - obb.m_ext[0]) > 0.0f) || ((-e + obb.m_ext[0]) < 0.0f))
    return false;
  ...
  if (!ISZERO(f)) {
    ...
    if (tmax < 0.0f)
      return false;
  } else if (((-e - obb.m_ext[1]) > 0.0f) || ((-e + obb.m_ext[1]) < 0.0f))
    return false;
  ...
  if (!ISZERO(f)) {
    ...
    if (tmax < 0.0f)
      return false;
  } else if (((-e - obb.m_ext[2]) > 0) || ((-e + obb.m_ext[2]) < 0))
    return false;
...
bool TRay<TYPE, DIMS>::Intersects(const OBB &obb, TYPE fL, TYPE *t) const {
  ...
  if (!ISZERO(f)) {
    ...
    if (tmax < 0.0f)
      return false;
  } else if (((-e - obb.m_ext[0]) > 0) || ((-e + obb.m_ext[0]) < 0))
    return false;
  ...
  if (!ISZERO(f)) {
    ...
    if (tmax < 0.0f)
      return false;
  } else if (((-e - obb.m_ext[1]) > 0) || ((-e + obb.m_ext[1]) < 0))
    return false;
...
template <typename TYPE, int DIMS>
bool TRay<TYPE, DIMS>::Intersects(const PLANE &plane, bool bCull, TYPE *t,
                                  POINT *pPtHit) const {
...
template <typename TYPE, int DIMS>
inline typename TRay<TYPE, DIMS>::POINT
TRay<TYPE, DIMS>::Intersects(const PLANE &plane) const {
...
template <typename TYPE, int DIMS>
bool TRay<TYPE, DIMS>::Intersects(const PLANE &plane, bool bCull, TYPE fL,
...
// Rays assumed to be coplanar; returns only the distance from the origin of the
// first ray. P = R.origin + s * R.dir
...
// No coplanarity assumption; returns the shortest line segment connecting the
// two rays: P1 = R1.origin + s1 * R1.dir P2 = R2.origin + s2 * R2.dir
...
// Returns 0 if it coincides with the ray origin, positive value if in front,
...
template <typename TYPE, int DIMS>
inline typename TRay<TYPE, DIMS>::POINT
TRay<TYPE, DIMS>::ProjectPoint(const POINT &pt) const {
  ASSERT(ISEQUAL(m_vDir.norm(), TYPE(1)));
...
// Same as above, but returns the distance even if the point projection is
// behind the origin.
template <typename TYPE, int DIMS>
...
bool TCylinder<TYPE, DIMS>::Intersects(const SPHERE &sphere) const {
  struct Check {
    static bool Intersection(const RAY &ray, const SPHERE &sphere,
                             const VECTOR &CmV, TYPE t, TYPE radius,
                             TYPE height) {
...
template <typename TYPE> bool TestRayTriangleIntersection(unsigned iters) {
  ...
  for (unsigned iter = 0, lives = iters / 100; iter < iters; ++iter) {
    const Type scale(10);
    const Triangle triangle(Point::Random() * scale, Point::Random() * scale,
                            Point::Random() * scale);
    Type t;
    ...
    const Point side(((triangle[o].template cast<double>() +
                       triangle[(o + 1) % 3].template cast<double>()) /
                      2.0)
                         .template cast<TYPE>());
