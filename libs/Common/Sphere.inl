...
inline TSphere<TYPE, DIMS>::TSphere(const POINT &p1, const POINT &p2,
                                    const POINT &p3) {
  Set(p1, p2, p3);
} // constructor
/*----------------------------------------------------------------*/
...
template <typename TYPE, int DIMS>
inline void TSphere<TYPE, DIMS>::Set(const POINT &p1, const POINT &p2,
                                     const POINT &p3) {
  ...
  if (B + C <= A) {
    ...
    center = (beta * alpha.squaredNorm() - alpha * beta.squaredNorm())
                     .cross(alphaXbeta) /
                 (alphaXbeta.squaredNorm() * TYPE(2)) +
             *c;
  }
}
/*----------------------------------------------------------------*/
