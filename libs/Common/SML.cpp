...
SML::SML(const String &name)
    : m_strName(name), m_fncInitItem(NULL), m_fncSaveItem(NULL),
      m_fncReleaseItem(NULL), m_fncItemData(NULL) {}
...
bool SML::ParseSection(TokenIStream &filter, MemFile &memFile) {
  while (true) {
    ...
    if (!sectionFilter.isEOS()) {
      ...
      ASSERT(memFile.getSize() - posMemFile ==
             lenValues * (size_f_t)sizeof(TCHAR));
      ASSERT(posMemFile == 0 ||
    ...
    pChild->SetFncItem(m_fncInitItem, m_fncSaveItem, m_fncReleaseItem,
                       m_fncItemData);
...
bool SML::ParseValues(MemFile &valuesMemFile) {
  ...
  do {
    ...
    String szName;
    if (filterValue.trimBackLine(memValue) == lenName ||
        lenNameValueReal == lenName) {
...
bool SML::SaveBracketize(OSTREAM &oStream, const String &strIndent,
                         SAVEFLAG flags) const {
  // save its name
...
bool SML::SaveIntern(OSTREAM &oStream, const String &strIndent,
                     SAVEFLAG flags) const {
  ...
  if (flgs.isAnySet(SORT | SORTINV)) {
    ...
    for (SMLITEMMAP::const_iterator item = GetBegin(); item != GetEnd(); ++item)
      if (!m_fncSaveItem || m_fncSaveItem((*item).second, m_fncItemData))
        lines.InsertSort(
            String::FormatString(_T("%s%s " SML_NAMETOKEN " %s" SML_VALUETOKEN),
                                 strIndent.c_str(), (*item).first.c_str(),
                                 (*item).second.val.c_str()),
            (flgs.isAnySet(SORT) ? String::CompareAlphabetically
                                 : String::CompareAlphabeticallyInv));
    FOREACH(l, lines)
    ...
    for (SMLITEMMAP::const_iterator item = GetBegin(); item != GetEnd(); ++item)
      if (!m_fncSaveItem || m_fncSaveItem((*item).second, m_fncItemData))
        oStream.print(_T("%s%s " SML_NAMETOKEN " %s" SML_VALUETOKEN),
                      strIndent.c_str(), (*item).first.c_str(),
                      (*item).second.val.c_str());
  }
  ...
  FOREACH(i, m_arrChildren) {
    ...
    // skip empty children
    if (!flgs.isSet(SAVEEMPTY) && pSML->IsEmpty() &&
        pSML->m_arrChildren.IsEmpty())
      continue;
    // insert a new line to separate from the above section (only for visual
    // aspect)
    if (bFirst)
      bFirst = false;
    else
...
IDX SML::InsertChildUnique(const LPSML pSML) {
  const std::pair<IDX, bool> res(
      m_arrChildren.InsertSortUnique(pSML, SML::Compare));
  if (res.second)
    delete pSML;
  return res.first;
}
 ...
 */
void SML::RemoveChild(IDX idx) { m_arrChildren.RemoveAtMove(idx); }
/*----------------------------------------------------------------*/
 ...
 */
void SML::SetFncItem(TFncInitItem fncInit, TFncSaveItem fncSave,
                     TFncReleaseItem fncRelease, void *data) {
...
int STCALL SML::Compare(const void *l, const void *r) {
  return _tcscmp((*((const SML **)l))->GetName(),
                 (*((const SML **)r))->GetName());
}
/*----------------------------------------------------------------*/
