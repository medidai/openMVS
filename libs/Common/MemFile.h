...
public:
  ...
  MemFile(size_t initialSize)
      : m_buffer(new BYTE[initialSize]), m_sizeBuffer(initialSize), m_size(0),
        m_pos(0) {}
  MemFile(BYTE *data, size_t size)
      : m_buffer(data), m_sizeBuffer(0), m_size(size), m_pos(0) {}
  virtual ~MemFile() { close(); }
  ...
  virtual size_f_t getSizeBuffer() const { return m_sizeBuffer; }

  virtual size_f_t getSizeLeft() const { return m_size - m_pos; }

  virtual size_f_t getSize() const { return m_size; }
  ...
  virtual size_f_t getPos() const { return m_pos; }
  ...
  virtual bool movePos(size_f_t delataPos) { return setPos(m_pos + delataPos); }

  virtual bool isEOF() { return (m_pos == m_size); }
  ...
  virtual BYTE *getData() { return m_buffer + m_pos; }

  virtual BYTE *getBuffer() { return m_buffer; }

  virtual size_t flush() { return 0; }

protected:
  BYTE *m_buffer;        // buffer where we will store the data
  size_f_t m_sizeBuffer; // size of the whole buffer, 0 if no buffer or not
                         // allocated here
