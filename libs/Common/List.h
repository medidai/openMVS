...
#define ARR2IDX(arr)                                                           \
  typename std::remove_reference<decltype(arr)>::type::size_type
#define SIZE2IDX(arr)                                                          \
  typename std::remove_const<                                                  \
      typename std::remove_reference<decltype(arr)>::type>::type
#else
...
#define FOREACHPTR(var, arr)                                                   \
  for (auto var = (arr).begin(), var##End = (arr).end(); var != var##End; ++var)
#endif
#ifndef RFOREACHPTR
#define RFOREACHPTR(var, arr)                                                  \
  for (auto var = (arr).end(), var##Begin = (arr).begin(); var-- != var##Begin;)
#endif

// raw data array iterator by index
#ifndef FOREACHRAW
#define FOREACHRAW(var, sz)                                                    \
  for (SIZE2IDX(sz) var = 0, var##Size = (sz); var < var##Size; ++var)
#endif
#ifndef RFOREACHRAW
...
#define FOREACHRAWPTR(var, arr, sz)                                            \
  for (auto var = (arr), var##End = var + sz; var != var##End; ++var)
#endif
#ifndef RFOREACHRAWPTR
#define RFOREACHRAWPTR(var, arr, sz)                                           \
  for (auto var##Begin = (arr), var = var##Begin + sz; var-- != var##Begin;)
#endif

// constructs a cList reference to a given raw data array
#ifndef CLISTREFRAW
#define CLISTREFRAW(CLIST, var, arr, sz)                                       \
  uint8_t _ArrData##var[sizeof(CLIST)];                                        \
  new (_ArrData##var) CLIST(sz, const_cast<CLIST::Type *>(arr));               \
  const CLIST &var(*reinterpret_cast<const CLIST *>(_ArrData##var))
#endif
// constructs a cList reference to a given std::_vector
#ifndef CLISTREFVECTOR
#define CLISTREFVECTOR(CLIST, var, vec)                                        \
  uint8_t _ArrData##var[sizeof(CLIST)];                                        \
  new (_ArrData##var) CLIST(vec.size(), const_cast<CLIST::Type *>(&vec[0]));   \
  const CLIST &var(*reinterpret_cast<const CLIST *>(_ArrData##var))
#endif

#define CLISTDEF0(TYPE) SEACAVE::cList<TYPE, const TYPE &, 0>
#define CLISTDEF2(TYPE) SEACAVE::cList<TYPE, const TYPE &, 2>
#define CLISTDEF0IDX(TYPE, IDXTYPE)                                            \
  SEACAVE::cList<TYPE, const TYPE &, 0, 16, IDXTYPE>
#define CLISTDEFIDX(TYPE, IDXTYPE)                                             \
  SEACAVE::cList<TYPE, const TYPE &, 1, 16, IDXTYPE>
#define CLISTDEF2IDX(TYPE, IDXTYPE)                                            \
  SEACAVE::cList<TYPE, const TYPE &, 2, 16, IDXTYPE>
...
template <typename TYPE, typename ARG_TYPE = const TYPE &, int useConstruct = 1,
          int grow = 16, typename IDX_TYPE = IDX>
class cList {
public:
  typedef TYPE Type;
  typedef ARG_TYPE ArgType;
  typedef IDX_TYPE IDX;
  typedef int(STCALL *TFncCompare)(
      const void *elem,
      const void *key); // returns 0 if equal, otherwise <0 or >0 respectively
  ...
  cList(IDX size)
      : _size(size), _vectorSize(size),
        _vector(
            (TYPE *)operator new[](static_cast<size_t>(size) * sizeof(TYPE))) {
    ASSERT(size > 0 && size < NO_INDEX);
    _ArrayConstruct(_vector, size);
  }

  // construct a list containing size initialized elements and allocated space
  // for _reserved elements
  cList(IDX size, IDX _reserved)
      : _size(size), _vectorSize(_reserved),
        _vector((TYPE *)operator new[](static_cast<size_t>(_reserved) *
                                       sizeof(TYPE))) {
  ...
  cList(InputIt first, InputIt last, bool /*dummy*/)
      : _size(std::distance(first, last)), _vectorSize(_size),
        _vector(
            (TYPE *)operator new[](static_cast<size_t>(_size) * sizeof(TYPE))) {
    while (first != last)
      Insert(*first++);
  }
  ...
  cList(const cList &rList)
    ...
    _vector = (TYPE *)(operator new[](static_cast<size_t>(_vectorSize) *
                                      sizeof(TYPE)));
    _ArrayCopyConstruct(_vector, rList._vector, _size);
  }
  // move constructor: creates a move-copy of the given list
  cList(cList &&rList)
      : _size(rList._size), _vectorSize(rList._vectorSize),
        _vector(rList._vector) {
    rList._Init();
  }

  // constructor a list from a raw data array
  explicit inline cList(TYPE *pDataBegin, TYPE *pDataEnd)
      : _size((IDX)(pDataEnd - pDataBegin)), _vectorSize(_size) {
    if (_vectorSize == 0)
      return;
    _vector =
        (TYPE *)operator new[](static_cast<size_t>(_vectorSize) * sizeof(TYPE));
    _ArrayCopyConstruct(_vector, pDataBegin, _size);
  }

  // constructor a list from a raw data array, taking ownership of the array
  // memory
  explicit inline cList(IDX nSize, TYPE *pData)
      : _size(nSize), _vectorSize(nSize), _vector(pData) {}

  inline ~cList() { _Release(); }

  // move the content from the given list
  inline cList &operator=(cList &&rList) { return CopyOfRemove(rList); }
  // copy the content from the given list
  inline cList &operator=(const cList &rList) { return CopyOf(rList); }
  ...
  inline cList &CopyOf(const cList &rList, bool bForceResize = false) {
    ...
    if (bForceResize || _vectorSize < rList._vectorSize) {
      ...
      _ArrayCopyConstruct(_vector + _size, rList._vector + _size,
                          rList._size - _size);
    }
    _size = rList._size;
    return *this;
  ...
  inline cList &CopyOf(const TYPE *pData, IDX nSize,
  ...
  inline void MemsetValue(ARG_TYPE val) { std::fill_n(_vector, _size, val); }
  ...
  inline void Reset(IDX newSize) {
    ...
    _vector =
        (TYPE *)operator new[](static_cast<size_t>(newSize) * sizeof(TYPE));
  ...
  inline TYPE *GetData() const { return _vector; }
  ...
  inline TYPE *Begin() const { return _vector; }

  inline TYPE *End() const { return _vector + _size; }
...
// Adds a new empty element at the end of the array and pass the arguments to
// its constructor.
#ifdef _SUPPORT_CPP11
  template <typename... Args> inline TYPE &AddConstruct(Args &&...args) {
    if (_vectorSize <= _size)
      _Grow(_vectorSize + grow);
...
// Adds the new element at the end of the array.
#ifdef _SUPPORT_CPP11
  template <typename T> inline void Insert(T &&elem) {
    if (_vectorSize <= _size)
      _Grow(_vectorSize + grow);
    if (useConstruct)
...
#ifdef _SUPPORT_CPP11
  template <typename T> inline void SetAt(IDX index, T &&elem) {
    if (_size <= index)
      Resize(index + 1);
...
#ifdef _SUPPORT_CPP11
  template <typename T> inline void AddAt(IDX index, T &&elem) {
    if (index < _size)
...
#ifdef _SUPPORT_CPP11
  template <typename T> inline void InsertAt(IDX index, T &&elem) {
    if (useConstruct)
  ...
  template <typename RTYPE = typename std::conditional<
                std::is_floating_point<TYPE>::value, TYPE, REAL>::type>
  ...
  inline ArgType GetMax() const { return *std::max_element(Begin(), End()); }
  template <typename Functor>
  ...
  inline IDX GetMaxIdx(const Functor &functor) const {
    return static_cast<IDX>(std::max_element(Begin(), End(), functor) -
                            Begin());
  }
#ifdef _SUPPORT_CPP11
  ...
  inline std::pair<IDX, IDX> GetMinMaxIdx() const {
    const auto minmax(std::minmax_element(Begin(), End()));
    return std::make_pair(static_cast<IDX>(minmax.first - Begin()),
                          static_cast<IDX>(minmax.second - Begin()));
  }
  template <typename Functor>
  ...
  template <typename Functor> inline void Sort(const Functor &functor) {
  ...
  template <int N> inline void StoreTop(ARG_TYPE elem) {
  ...
  // remove duplicated values; the values are expected to be sorted, set bSort
  // if not
  template <bool bSort = true> inline void RemoveDuplicates() {
    if (bSort)
      Sort();
  ...
  inline void RemoveAtMove(IDX index) {
    ...
    if (index + 1 == _size) {
      ...
      _ArrayMoveConstructFwd(_vector + index, _vector + index + 1,
                             (--_size) - index);
    }
  }
  ...
  inline void RemoveAtMove(IDX index, IDX count) {
    ...
    if (index + count == _size) {
      ...
      _ArrayMoveConstructFwd(_vector + index, _vector + index + count,
                             (_size -= count) - index);
    }
  }
  ...
  inline void Reset() { _Init(); }

  // Delete also the pointers (take care to use this function only if the
  ...
  inline IDX GetCapacity() const { return _vectorSize; }

  inline IDX GetSize() const { return _size; }

  inline bool IsEmpty() const { return (_size == 0); }

  static inline unsigned GetConstructType() { return useConstruct; }
  static inline IDX GetGrowSize() { return grow; }
...
protected:
  ...
  inline void _Grow(IDX newVectorSize) {
    ...
    const size_t growCapacity(
        static_cast<size_t>(expoVectorSize - _vectorSize) * sizeof(TYPE));
    if (growCapacity > maxGrowCapacity)
      expoVectorSize =
          _vectorSize + static_cast<IDX>(maxGrowCapacity / sizeof(TYPE));
    ...
    _vector = (TYPE *)operator new[](static_cast<size_t>(newVectorSize) *
                                     sizeof(TYPE));
  ...
  static inline void _ArrayCopyConstruct(TYPE *RESTRICT dst,
                                         const TYPE *RESTRICT src, IDX n) {
    if (useConstruct) {
      while (n--)
  ...
  static inline void _ArrayCopyRestrict(TYPE *RESTRICT dst,
                                        const TYPE *RESTRICT src, IDX n) {
    if (useConstruct) {
      while (n--)
        dst[n] = src[n];
...
public:
...
#ifdef _SUPPORT_CPP11
  inline cList(std::initializer_list<Type> l)
      : _size(0), _vectorSize((size_type)l.size()), _vector(NULL) {
    ASSERT(l.size() < NO_INDEX);
    if (_vectorSize == 0)
      return;
    _vector =
        (Type *)operator new[](static_cast<size_t>(_vectorSize) * sizeof(Type));
    const Type *first(l.begin());
    do
      new (_vector + _size++) Type(*first++);
    while (first != l.end());
  }
#endif
  ...
  inline void insert(const_iterator it, const_reference elem) {
    InsertAt(it - this->_vector, elem);
  }
#ifdef _SUPPORT_CPP11
  template <typename... Args> inline reference emplace_back(Args &&...args) {
    return AddConstruct(std::forward<Args>(args)...);
  }
...
template <typename IDX_TYPE> inline bool ValidIDX(const IDX_TYPE &idx) {
  return (idx != DECLARE_NO_INDEX(IDX_TYPE));
}
/*----------------------------------------------------------------*/

// some test functions
template <bool dummy> inline bool cListTest(unsigned iters) {
  for (unsigned i = 0; i < iters; ++i) {
    ...
    for (size_t i = 0; i < arrR.size(); ++i) {
      const int e = arrR[i];
      if (arrC[i] != e || arr0[i] != e || arr1[i] != e || arr2[i] != e) {
    ...
    if (arrR.size() != arr0.GetSize() || arrR.size() != arr1.GetSize() ||
...
template <typename TYPE, int N> class cListFixed {
public:
  typedef TYPE Type;
  ...
  inline bool IsEmpty() const { return (_size == 0); }
  inline IDX GetSize() const { return _size; }
  inline const TYPE *Begin() const { return _vector; }
  inline TYPE *Begin() { return _vector; }
  inline const TYPE *End() const { return _vector + _size; }
  inline TYPE *End() { return _vector + _size; }
  inline const TYPE &First() const {
    ASSERT(_size > 0);
    return _vector[0];
  }
  ...
  inline void InsertAt(IDX index, ArgType elem) {
    ASSERT(_size < N);
    memmove(_vector + index + 1, _vector + index,
            sizeof(TYPE) * (_size++ - index));
    _vector[index] = elem;
  }
...
const typename cListFixed<TYPE, N>::IDX
    cListFixed<TYPE, N>::NO_INDEX(DECLARE_NO_INDEX(IDX));
/*----------------------------------------------------------------*/
