/** \file SRDelegate.hpp
 *
 * This is a C++11 implementation by janezz55(code.google) for the original "The
 * Impossibly Fast C++ Delegates" authored by Sergey Ryazanov.
 *
 * This is a copy checkouted from
 * https://code.google.com/p/cpppractice/source/browse/trunk/delegate.hpp on
 * 2014/06/07. Last change in the chunk was r370 on Feb 9, 2014.
 *
...
public:
  ...
  delegate(delegate &&d)
      : object_ptr_(d.object_ptr_), stub_ptr_(d.stub_ptr_),
        deleter_(d.deleter_), store_(d.store_), store_size_(d.store_size_) {
  ...
  template <class C, typename = typename ::std::enable_if<
                         ::std::is_class<C>::value, C>::type>
  ...
  template <R (*const function_ptr)(A...)> static delegate from(void) noexcept {
...
private:
  ...
  static typename ::std::enable_if<
      !(is_member_pair<T>::value || is_const_member_pair<T>::value), R>::type
...
struct hash<::fastdelegate::delegate<R(A...)>> {
  size_t operator()(::fastdelegate::delegate<R(A...)> const &d) const noexcept {
    ...
    return hash<decltype(d.stub_ptr_)>()(d.stub_ptr_) + 0x9e3779b9 +
           (seed << 6) + (seed >> 2);
  }
};
} // namespace std
