...
// the current cell should be split farther, given its number of items and
// radius, ex:
//	 [](Octree::IDX_TYPE size, Octree::Type radius) {
//	 	return size > SIZE && radius > RADIUS;
//	 }
// where:
//   SIZE is the minimum number of items contained by the cell so that this to
//   be divided further RADIUS is the minimum size of the cell allowed to be
//   divided further
// both conditions represent exclusive limits and both should be true for the
// division to take place
template <typename ITEMARR_TYPE, typename TYPE, int DIMS,
          typename DATA_TYPE = uint32_t>
class TOctree {
...
public:
  ...
  public:
    ...
    typedef struct {
      IDX_TYPE idxBegin; // index in the global array of the first item
                         // contained by this cell
      SIZE_TYPE
          size; // number of items contained by this cell in the global array
  ...
  public:
    ...
    static inline POINT_TYPE ComputeChildCenter(const POINT_TYPE &, TYPE,
                                                unsigned);

    inline bool IsLeaf() const { return (m_child == NULL); }
    inline const CELL_TYPE &GetChild(int i) const {
      ASSERT(!IsLeaf() && i < numChildren);
      return m_child[i];
    }
    inline const NODE_TYPE &Node() const {
      ASSERT(!IsLeaf());
      return m_node;
    }
    ...
    inline const LEAF_TYPE &Leaf() const {
      ASSERT(IsLeaf());
      return m_leaf;
    }
    inline LEAF_TYPE &Leaf() {
      ASSERT(IsLeaf());
      return m_leaf;
    }
    inline const POINT_TYPE &GetCenter() const { return Node().center; }
    inline AABB_TYPE GetAabb(TYPE radius) const {
      return AABB_TYPE(Node().center, radius);
    }
    inline AABB_TYPE GetChildAabb(unsigned idxChild, TYPE radius) const {
      const TYPE childRadius(radius / TYPE(2));
      return AABB_TYPE(ComputeChildCenter(Node().center, childRadius, idxChild),
                       childRadius);
    }
    inline IDX_TYPE GetFirstItemIdx() const { return Leaf().idxBegin; }
    inline IDX_TYPE GetLastItemIdx() const {
      return Leaf().idxBegin + Leaf().size;
    }
  ...
  public:
    ...
    union { // a LEAF_TYPE or NODE_TYPE object, if it is a leaf or not
            // respectively
      NODE_TYPE m_node;
      LEAF_TYPE m_leaf;
    };
  };
  typedef SEACAVE::cList<CELL_TYPE *, CELL_TYPE *, 0, 256, IDX_TYPE>
      CELLPTRARR_TYPE;
  ...
  struct IndexInserter {
    ...
    void operator()(const IDX_TYPE *idices, SIZE_TYPE size) {
      indices.Join(idices, size);
    }
  };
...
public:
  ...
  template <typename Functor> void Insert(const ITEMARR_TYPE &, Functor split);
  template <typename Functor>
  void Insert(const ITEMARR_TYPE &, const AABB_TYPE &, Functor split);

  template <typename INSERTER> inline void CollectCells(INSERTER &) const;
  template <typename INSERTER>
  void CollectCells(const CELL_TYPE &, INSERTER &) const;
  template <typename PARSER> inline void ParseCells(PARSER &);
  ...
  inline void Collect(IDX_TYPE maxNeighbors, IDXARR_TYPE &indices,
                      const AABB_TYPE &aabb) const;

  template <typename INSERTER>
  inline void Collect(INSERTER &inserter, const POINT_TYPE &center,
                      TYPE radius) const;
  inline void Collect(IDXARR_TYPE &indices, const POINT_TYPE &center,
                      TYPE radius) const;
  inline void Collect(IDX_TYPE maxNeighbors, IDXARR_TYPE &indices,
                      const POINT_TYPE &center, TYPE radius) const;
  ...
  void SplitVolume(float maxArea, AREAESTIMATOR &areaEstimator,
...
protected:
  template <typename Functor> struct _InsertData {
  ...
  void _Insert(CELL_TYPE &, const POINT_TYPE &center, TYPE radius,
               IDX_TYPE start, IDX_TYPE size, _InsertData<Functor> &);

  template <typename PARSER> void _ParseCells(CELL_TYPE &, TYPE, PARSER &);
  ...
  template <typename FTYPE, int FDIMS, typename INSERTER>
  void _Traverse(const CELL_TYPE &, TYPE, const TFrustum<FTYPE, FDIMS> &,
                 INSERTER &) const;
  template <typename FTYPE, int FDIMS, typename PARSER>
  void _TraverseCells(CELL_TYPE &, TYPE, const TFrustum<FTYPE, FDIMS> &,
                      PARSER &);

  template <typename AREAESTIMATOR, typename CHUNKINSERTER>
  void _SplitVolume(const CELL_TYPE &parentCell, TYPE parentRadius,
                    unsigned idxChild, float maxArea,
                    AREAESTIMATOR &areaEstimator, CHUNKINSERTER &chunkInserter,
                    const UnsignedArr &indices = UnsignedArr{0, 1, 2, 3, 4, 5,
                                                             6, 7});

protected:
  const ITEM_TYPE *m_items; // original input items (the only condition is that
                            // every item to resolve to a position)
  IDXARR_TYPE m_indices;    // indices to input items re-arranged spatially (as
                            // dictated by the octree)
...
public:
  typedef struct DEBUGINFO_TYPE {
    ...
    void operator+=(const DEBUGINFO_TYPE &r) {
      ...
      memSize += r.memSize;
      memStruct += r.memStruct;
      memItems += r.memItems;
      numItems += r.numItems;
      numNodes += r.numNodes;
      numLeaves += r.numLeaves;
      if (minDepth > r.minDepth)
        minDepth = r.minDepth;
      if (maxDepth < r.maxDepth)
        maxDepth = r.maxDepth;
      avgDepth /= numNodes;
    }
  } DEBUGINFO;
