...
template <typename TYPE, int DIMS>
inline TAABB<TYPE, DIMS>::TAABB(const POINT &pt) : ptMin(pt), ptMax(pt) {}
template <typename TYPE, int DIMS>
...
inline TAABB<TYPE, DIMS>::TAABB(const TAABB<CTYPE, DIMS> &rhs)
    : ptMin(rhs.ptMin.template cast<TYPE>()),
      ptMax(rhs.ptMax.template cast<TYPE>()) {} // copy constructor
/*----------------------------------------------------------------*/
...
template <typename TYPE, int DIMS>
inline typename TAABB<TYPE, DIMS>::ALIGNEDBOX
TAABB<TYPE, DIMS>::GetAlignedBox() const {
...
template <typename TYPE, int DIMS>
inline typename TAABB<TYPE, DIMS>::POINT
TAABB<TYPE, DIMS>::GetCorner(BYTE i) const {
  POINT ptCorner;
...
template <typename TYPE, int DIMS>
bool TAABB<TYPE, DIMS>::IntersectsComplete(const TAABB &aabb,
...
template <typename TYPE, int DIMS>
bool TAABB<TYPE, DIMS>::IntersectsComplete(const POINT &pt,
                                           TYPE tolerance) const {
...
template <typename TYPE, int DIMS>
unsigned TAABB<TYPE, DIMS>::SplitBy(const POINT &pt, TAABB child[numChildren],
                                    unsigned &idxFirst) const {
  idxFirst = 0;
  unsigned size;
  int a = 0;
  do {
  ...
  do {
    ...
    for (unsigned b = 0; b < size; ++b) {
      n += child[(idxFirst + b) % numChildren].SplitBy(
          pt[a], a, child + ((idxFirst + size + n) % numChildren));
      if (n == 0)
        goto MAIN_LOOP_END;
    }
    idxFirst += size;
