...
struct ErrorIndex {
  ...
  size_t second;
  inline ErrorIndex() {}
  inline ErrorIndex(double _first, size_t _second)
...
static ErrorIndex
bestNFA(double logalpha0, const ErrorIndexArr &e, double loge0,
        double maxThresholdSq, const std::vector<float> &logc_n,
        const std::vector<float> &logc_k, double multError = 1.0) {
  ErrorIndex bestIndex(DBL_MAX, NSAMPLES);
  const size_t n = e.size();
  for (size_t k = NSAMPLES + 1; k <= n && e[k - 1].first <= maxThresholdSq;
       ++k) {
    const double logalpha =
        logalpha0 + multError * log10(e[k - 1].first + FLT_EPSILON);
    const double NFA =
        loge0 + logalpha * (double)(k - NSAMPLES) + logc_n[k] + logc_k[k];
...
ACRANSAC(Kernel &kernel, Sampler &sampler, std::vector<size_t> &vec_inliers,
  ...
  const double loge0 =
      log10((double)Kernel::MAX_MODELS * (nData - Kernel::MINIMUM_SAMPLES));
...
#endif
    for (typename Kernel::Models::const_iterator itModel = vec_models.cbegin();
         itModel != vec_models.cend(); ++itModel) {
      ...
      ErrorIndex best = bestNFA<Kernel::MINIMUM_SAMPLES>(
          kernel.logalpha0(), vec_residuals, loge0, maxThresholdSq, vec_logc_n,
          vec_logc_k, kernel.multError());

      if (best.first <
          minNFA /*&& vec_residuals[best.second-1].first < errorSqMax*/) {
        ...
        model = *itModel;
        DEBUG_LEVEL(4, "\titer=% 3u inliers=%u threshold=%g nfa=%g", iter,
                    best.second, SQRT(errorSqMax), minNFA);
#ifdef ACRANSAC_SAMPLE_INLIERS
        better = true;
#else
        nIter = cvRANSACUpdateNumIters(
            confidence, (double)(nData - best.second) / nData,
...
template <typename Kernel, typename Sampler>
void RANSAC(Kernel &kernel, Sampler &sampler, std::vector<size_t> &vec_inliers,
            typename Kernel::Model &model, double threshold,
#ifndef ACRANSAC_SAMPLE_INLIERS
            double confidence = 0.9999,
#endif
            size_t nIter = 0) {
...
#ifdef _USE_MSAC
        best_score = score;
        DEBUG_LEVEL(4, "\titer=%3u inliers=%u score=%g", iter,
                    vec_inliers.size(), best_score);
#else
...
#else
        nIter = cvRANSACUpdateNumIters(
            confidence, (double)(nData - vec_inliers.size()) / nData,
            unsigned(Kernel::MINIMUM_SAMPLES), (unsigned)nTrialsRound);
#endif
#endif
      }
...
public:
  ...
  ACKernelAdaptor(const DMatrix32F &x1, float w1, float h1,
...
public:
  ...
  ACKernelAdaptorResection(const DMatrix32F &x2d, const DMatrix &x3D)
      : x2d_(x2d), x3D_(x3D), logalpha0_(log10(M_PI)) {
    ASSERT(2 == x2d_.cols);
    ASSERT(3 == x3D_.cols);
...
public:
  ...
  ACKernelAdaptorEssential(const DMatrix32F &x1, float w1, float h1,
                           const Matrix3x3f &invK1, const DMatrix32F &x2,
  ...
  inline void EvaluateModel(const Model &model) { solver.EvaluateModel(model); }

  inline double Error(size_t sample) const {
    return solver.Error(*x1_.ptr<const Point2f>((int)sample),
                        *x2_.ptr<const Point2f>((int)sample));
  }
