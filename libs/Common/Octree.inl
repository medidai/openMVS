...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
inline void
TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::CELL_TYPE::Swap(CELL_TYPE &rhs) {
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
inline unsigned
TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::CELL_TYPE::ComputeChild(
    const POINT_TYPE &item) const {
  ASSERT(!IsLeaf());
  unsigned idx = 0;
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::CELL_TYPE::ComputeCenter(
    POINT_TYPE centers[]) {
  if (DIMS == 1) {
    centers[0] << -1;
    centers[1] << 1;
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
inline typename TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::POINT_TYPE
TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::CELL_TYPE::ComputeChildCenter(
    const POINT_TYPE &center, TYPE radius, unsigned idxChild) {
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
size_t
TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::CELL_TYPE::GetNumItemsHeld()
    const {
  if (IsLeaf())
    return GetNumItems();
  size_t numItems = 0;
...
inline TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::TOctree(
    const ITEMARR_TYPE &items, Functor split) {
  Insert(items, split);
}
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename Functor>
inline TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::TOctree(
    const ITEMARR_TYPE &items, const AABB_TYPE &aabb, Functor split) {
...
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_Insert(
    CELL_TYPE &cell, const POINT_TYPE &center, TYPE radius, IDX_TYPE start,
    IDX_TYPE size, _InsertData<Functor> &insertData) {
  ASSERT(size > 0);
  ...
  if (bForceSplit || insertData.split(size, radius)) {
    ...
    for (unsigned i = 0; i < CELL_TYPE::numChildren; ++i) {
      ...
      insertData.successors[childD.End(i)] =
          _InsertData<Functor>::NO_INDEX; // mark the end of child successors
      const POINT_TYPE childCenter(
          CELL_TYPE::ComputeChildCenter(center, childRadius, i));
      _Insert<Functor, false>(child, childCenter, childRadius, childD.Start(i),
                              childD.Size(i), insertData);
    }
  } else {
    // init leaf
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename Functor>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Insert(
    const ITEMARR_TYPE &items, const AABB_TYPE &aabb, Functor split) {
  Release();
  m_items = items.data();
  ...
  std::iota(insertData.successors.begin(), insertData.successors.end(),
            IDX_TYPE(1));
...
template <typename Functor>
inline void
TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Insert(const ITEMARR_TYPE &items,
                                                     Functor split) {
...
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::CollectCells(
    const CELL_TYPE &cell, INSERTER &inserter) const {
  if (cell.IsLeaf()) {
...
// calls parser for each leaf of the octree (the IDX_TYPE operator has to be
// defined)
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename INSERTER>
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::CollectCells(
...
// calls parser for each leaf of the octree (the CELL_TYPE operator has to be
// defined)
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename PARSER>
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename INSERTER>
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_Collect(
    const CELL_TYPE &cell, const AABB_TYPE &aabb, INSERTER &inserter) const {
  if (cell.IsLeaf()) {
...
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_Collect(
    const CELL_TYPE &cell, TYPE radius, const COLLECTOR &collector,
    INSERTER &inserter) const {
  ASSERT(!cell.IsLeaf());
  const TYPE childRadius = radius / TYPE(2);
  for (int i = 0; i < CELL_TYPE::numChildren; ++i) {
    ...
    if (childCell.IsLeaf()) {
      if (collector.Intersects(
              CELL_TYPE::ComputeChildCenter(cell.GetCenter(), childRadius, i),
              childRadius))
        inserter(m_indices.data() + childCell.GetFirstItemIdx(),
                 childCell.GetNumItems());
    } else {
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename INSERTER>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Collect(
    INSERTER &inserter, const AABB_TYPE &aabb) const {
  _Collect(m_root, aabb, inserter);
}
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Collect(
    IDXARR_TYPE &indices, const AABB_TYPE &aabb) const {
...
template <typename INSERTER>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Collect(
    INSERTER &inserter, const POINT_TYPE &center, TYPE radius) const {
  _Collect(m_root, AABB_TYPE(center, radius), inserter);
}
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Collect(
    IDXARR_TYPE &indices, const POINT_TYPE &center, TYPE radius) const {
  IndexInserter inserter(indices);
  _Collect(m_root, AABB_TYPE(center, radius), inserter);
}
...
template <typename INSERTER, typename COLLECTOR>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Collect(
    INSERTER &inserter, const COLLECTOR &collector) const {
  _Collect(m_root, m_radius, collector, inserter);
}
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename COLLECTOR>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Collect(
    IDXARR_TYPE &indices, const COLLECTOR &collector) const {
  IndexInserter inserter(indices);
  _Collect(m_root, m_radius, collector, inserter);
}
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Collect(
    IDX_TYPE maxNeighbors, IDXARR_TYPE &indices, const AABB_TYPE &aabb) const {
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Collect(
    IDX_TYPE maxNeighbors, IDXARR_TYPE &indices, const POINT_TYPE &center,
    TYPE radius) const {
...
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_Traverse(
    const CELL_TYPE &cell, TYPE radius, const TFrustum<FTYPE, FDIMS> &frustum,
    INSERTER &inserter) const {
  ASSERT(!cell.IsLeaf());
...
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_TraverseCells(
    CELL_TYPE &cell, TYPE radius, const TFrustum<FTYPE, FDIMS> &frustum,
    PARSER &parser) {
  ASSERT(!cell.IsLeaf());
  switch (frustum.Classify(cell.GetAabb(radius))) {
  case CLIPPED: {
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename FTYPE, int FDIMS, typename INSERTER>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Traverse(
    const TFrustum<FTYPE, FDIMS> &frustum, INSERTER &inserter) const {
  _Traverse(m_root, m_radius, frustum, inserter);
}
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename FTYPE, int FDIMS>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::Traverse(
    const TFrustum<FTYPE, FDIMS> &frustum, IDXARR_TYPE &indices) const {
  _Traverse(m_root, m_radius, frustum, IndexInserter(indices));
}
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename FTYPE, int FDIMS, typename PARSER>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::TraverseCells(
    const TFrustum<FTYPE, FDIMS> &frustum, PARSER &parser) {
  _TraverseCells(m_root, m_radius, frustum, parser);
}
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename FTYPE, int FDIMS>
inline void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::TraverseCells(
    const TFrustum<FTYPE, FDIMS> &frustum, CELLPTRARR_TYPE &leaves) {
...
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_SplitVolume(
    const CELL_TYPE &parentCell, TYPE parentRadius, unsigned idxChild,
    float maxArea, AREAESTIMATOR &areaEstimator, CHUNKINSERTER &chunkInserter,
  ...
  struct GenerateSamples {
    ...
    GenerateSamples(const UnsignedArr &_indices)
        : indices(_indices), numSamples((unsigned)indices.size()),
          halfSamples(numSamples / 2), numCommonAxis(halfSamples == 4 ? 1 : 2),
          arrHalfIndices(0, numSamples) {
    ...
    CommonAxis:
      ...
      arrHalfIndices.emplace_back(std::move(halfIndices),
                                  std::move(compHalfIndices));
    }
  };
  ...
  if (indices.size() == 1) {
    _SplitVolume(cell, radius, indices.front(), maxArea, areaEstimator,
                 chunkInserter);
    return;
  }
  if (indices.size() == 2) {
    _SplitVolume(cell, radius, indices.front(), maxArea, areaEstimator,
                 chunkInserter);
    _SplitVolume(cell, radius, indices.back(), maxArea, areaEstimator,
                 chunkInserter);
    return;
  }
  ...
  // split mesh children and retain the components with surface smaller than the
  // given area
  const cList<PairIndices> halfIndices(
      std::move(GenerateSamples(indices).arrHalfIndices));
  IDX bestSplit(NO_ID);
  float bestArea(0);
  Point2f bestAs;
  ...
  if (bestSplit != NO_ID) {
    ...
    if (bestAs[0] < maxArea)
      ...
      _SplitVolume(parentCell, parentRadius, idxChild, maxArea, areaEstimator,
                   chunkInserter, pairHalfIndices.first);
    if (bestAs[1] < maxArea)
      chunkInserter(cell, radius, pairHalfIndices.second);
    else
      _SplitVolume(parentCell, parentRadius, idxChild, maxArea, areaEstimator,
                   chunkInserter, pairHalfIndices.second);
    return;
  }
  ...
  if (halfIndices.front().first.size() == 4) {
    ...
    for (const PairIndices &pairHalfIndices : halfIndices) {
      const cList<PairIndices> qIndicesFirst(
          std::move(GenerateSamples(pairHalfIndices.first).arrHalfIndices));
      const cList<PairIndices> qIndicesSecond(
          std::move(GenerateSamples(pairHalfIndices.second).arrHalfIndices));
    ...
    if (bestArea > 0) {
      ...
      for (unsigned i = 0; i < 4; ++i) {
        if (bestAs[i] < maxArea) {
          ...
          _SplitVolume(cell, radius, bestQIndices[i][0], maxArea, areaEstimator,
                       chunkInserter);
          _SplitVolume(cell, radius, bestQIndices[i][1], maxArea, areaEstimator,
                       chunkInserter);
        }
      }
      return;
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
template <typename AREAESTIMATOR, typename CHUNKINSERTER>
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::SplitVolume(
    float maxArea, AREAESTIMATOR &areaEstimator, CHUNKINSERTER &chunkInserter) {
  CELL_TYPE parent;
  ...
  _SplitVolume(parent, m_radius * TYPE(2), 0, maxArea, areaEstimator,
               chunkInserter);
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::_GetDebugInfo(
    const CELL_TYPE &cell, unsigned nDepth, DEBUGINFO &info) const {
  if (cell.IsLeaf()) {
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::GetDebugInfo(
    DEBUGINFO *pInfo, bool bPrintStats) const {
  DEBUGINFO localInfo;
...
template <typename ITEMARR_TYPE, typename TYPE, int DIMS, typename DATA_TYPE>
void TOctree<ITEMARR_TYPE, TYPE, DIMS, DATA_TYPE>::LogDebugInfo(
    const DEBUGINFO &info) {
  // VERBOSE("NoItems: %d; Mem %s; MemItems %s; MemStruct %s; AvgMemStruct
  // %.2f%%%%; NoNodes %d; NoLeaf %d; AvgLeaf %.2f%%%%; AvgDepth %.2f; MinDepth
  // %d; MaxDepth %d",
  VERBOSE("NumItems %d; Mem %s (%s items, %s struct - %.2f%%%%); NumNodes %d "
          "(leaves %d - %.2f%%%%); Depth %.2f (%d min, %d max)",
          info.numItems, Util::formatBytes(info.memSize).c_str(),
          Util::formatBytes(info.memItems).c_str(),
          Util::formatBytes(info.memStruct).c_str(),
          double(info.memStruct) * 100.0 / info.memSize, info.numNodes,
          info.numLeaves, float(info.numLeaves * 100) / info.numNodes,
...
inline bool OctreeTest(unsigned iters, unsigned maxItems = 1000,
                       bool bRandom = true) {
  ...
  aabb.Set(Eigen::Map<const POINT_TYPE>(ptMinData),
           Eigen::Map<const POINT_TYPE>(ptMaxData));
  aabb.Enlarge(ZEROTOLERANCE<TYPE>() * TYPE(10));
