...
template <typename TYPE, int DIMS>
inline TPlane<TYPE, DIMS>::TPlane(const POINT &p0, const POINT &p1,
                                  const POINT &p2) {
  Set(p0, p1, p2);
}
...
template <typename TYPE, int DIMS>
inline void TPlane<TYPE, DIMS>::Set(const POINT &p0, const POINT &p1,
                                    const POINT &p2) {
...
int TPlane<TYPE, DIMS>::Optimize(const POINT *points, size_t size,
  ...
  struct OptimizationFunctor {
    ...
    OptimizationFunctor(const POINT *_points, size_t _size,
                        const RobustNormFunctor &_robust)
        : points(_points), size(_size), robust(_robust) {
  ...
  lmmin(numParams, arrParams, (int)size, &functor,
...
template <typename TYPE, int DIMS> inline void TPlane<TYPE, DIMS>::Negate() {
  m_vN = -m_vN;
  m_fD = -m_fD;
} // Negate
template <typename TYPE, int DIMS>
...
template <typename TYPE, int DIMS>
inline TPlane<TYPE, DIMS>
TPlane<TYPE, DIMS>::Transformed(const MATRIX &m) const {
...
template <typename TYPE, int DIMS>
inline typename TPlane<TYPE, DIMS>::POINT
TPlane<TYPE, DIMS>::ProjectPoint(const POINT &p) const {
...
bool TPlane<TYPE, DIMS>::Clip(const RAY &ray, TYPE fL, RAY *pF, RAY *pB) const {
  ...
  if (n == BACK) {
    if (pB)
      pB->Set(ray.m_pOrig, ray.m_vDir);
    if (pF)
      pF->Set(ptHit, ray.m_vDir);
  }
  // ray comes from planes front side
  else if (n == FRONT) {
    if (pF)
      pF->Set(ray.m_pOrig, ray.m_vDir);
    if (pB)
      pB->Set(ptHit, ray.m_vDir);
  }
...
template <typename TYPE, int DIMS>
bool TPlane<TYPE, DIMS>::Intersects(const POINT &p0, const POINT &p1,
                                    const POINT &p2) const {
  const GCLASS n(Classify(p0));
  if ((n == Classify(p1)) && (n == Classify(p2)))
    return false;
  return true;
} // Intersects(Tri)
/*----------------------------------------------------------------*/
...
FitPlaneOnline<TYPE, TYPEW, bFitLineMode>::FitPlaneOnline()
    : sumX(0), sumSqX(0), sumXY(0), sumXZ(0), sumY(0), sumSqY(0), sumYZ(0),
      sumZ(0), sumSqZ(0), size(0) {}
template <typename TYPE, typename TYPEW, bool bFitLineMode>
...
template <typename TYPE, typename TYPEW, bool bFitLineMode>
TPoint3<TYPEW>
FitPlaneOnline<TYPE, TYPEW, bFitLineMode>::GetModel(TPoint3<TYPEW> &avg,
                                                    TPoint3<TYPEW> &dir) const {
  const TYPEW avgX(sumX / (TYPEW)size), avgY(sumY / (TYPEW)size),
      avgZ(sumZ / (TYPEW)size);
  ...
  // the plane normal is simply the eigenvector corresponding to least
  // eigenvalue
...
TPoint3<TYPEE> FitPlaneOnline<TYPE, TYPEW, bFitLineMode>::GetPlane(
    TPlane<TYPEE, 3> &plane) const {
...
TYPE FitPlane(const TPoint3<TYPE> *points, size_t size, TPlane<TYPE> &plane) {
  // compute a point on the plane, which is shown to be the centroid of the
  // points
  const Eigen::Map<
      const Eigen::Matrix<TYPE, Eigen::Dynamic, 3, Eigen::RowMajor>>
      vPoints((const TYPE *)points, size, 3);
  ...
  // 1 2
  // 3 4 5
  Eigen::Matrix<TYPE, 3, 3, Eigen::RowMajor> A(
      Eigen::Matrix<TYPE, 3, 3, Eigen::RowMajor>::Zero());
  ...
  const Eigen::SelfAdjointEigenSolver<
      Eigen::Matrix<TYPE, 3, 3, Eigen::RowMajor>>
      es(A);
...
template <typename TYPE, int DIMS>
inline TFrustum<TYPE, DIMS>::TFrustum(const MATRIX4x4 &m, TYPE w, TYPE h,
                                      TYPE n, TYPE f) {
  Set(m, w, h, n, f);
}
template <typename TYPE, int DIMS>
inline TFrustum<TYPE, DIMS>::TFrustum(const MATRIX3x4 &m, TYPE w, TYPE h,
                                      TYPE n, TYPE f) {
  Set(m, w, h, n, f);
} // Constructor
/*----------------------------------------------------------------*/

// Set frustum planes, normals pointing outwards, from SfM projection matrix and
// image plane details
template <typename TYPE, int DIMS>
void TFrustum<TYPE, DIMS>::Set(const MATRIX4x4 &m, TYPE w, TYPE h, TYPE n,
                               TYPE f) {
  const VECTOR ltn(0, 0, n), rtn(w * n, 0, n), lbn(0, h * n, n),
      rbn(w * n, h * n, n);
  const VECTOR ltf(0, 0, f), rtf(w * f, 0, f), lbf(0, h * f, f),
      rbf(w * f, h * f, f);
  const MATRIX4x4 inv(m.inverse());
  const VECTOR corners[] = {(inv * ltn.homogeneous()).template topRows<3>(),
...
template <typename TYPE, int DIMS>
void TFrustum<TYPE, DIMS>::Set(const MATRIX3x4 &m, TYPE w, TYPE h, TYPE n,
                               TYPE f) {
...
// Set frustum planes, normals pointing outwards, from the OpenGL
