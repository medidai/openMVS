...
int _convertSMVer2Cores(int major, int minor) {
  ...
  // properly
  VERBOSE("MapSMtoCores for SM %d.%d is undefined; default to use %d cores/SM",
...
CUresult _gpuCheckDeviceId(int devID, Device &device) {
  ...
  if (devID >= device_count) {
    VERBOSE("CUDA error: device [%d] is not a valid GPU device (%d CUDA "
            "capable GPU device(s) detected)",
            devID, device_count);
    return CUDA_ERROR_INVALID_DEVICE;
  }
  checkCudaError(cuDeviceGetAttribute(&device.computeMode,
                                      CU_DEVICE_ATTRIBUTE_COMPUTE_MODE, devID));
  if (device.computeMode == CU_COMPUTEMODE_PROHIBITED) {
    VERBOSE("CUDA error: device is running in <Compute Mode Prohibited>");
    return CUDA_ERROR_PROFILER_DISABLED;
  }
  checkCudaError(
      cuDeviceComputeCapability(&device.major, &device.minor, devID));
  if (device.major < 1) {
...
CUresult _gpuGetMaxGflopsDeviceId(Device &bestDevice) {
  ...
  for (int current_device = 0; current_device < device_count;
       ++current_device) {
    Device device;
    if (reportCudaError(cuDeviceGetAttribute(&device.computeMode,
    ...
    if (device.computeMode == CU_COMPUTEMODE_PROHIBITED)
      continue;
    if (reportCudaError(cuDeviceComputeCapability(
            &device.major, &device.minor, current_device)) != CUDA_SUCCESS)
      continue;
  ...
  for (Device &device : devices) {
    ...
    if (reportCudaError(cuDeviceGetAttribute(
            &multiProcessorCount, CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT,
            device.ID)) != CUDA_SUCCESS)
      continue;
    int clockRate;
    if (reportCudaError(cuDeviceGetAttribute(&clockRate,
      ...
      continue;
    size_t compute_perf =
        (size_t)multiProcessorCount * sm_per_multiproc * clockRate;
    if (compute_perf > max_compute_perf &&
        (best_SM_arch <
             3 || // if we find GPU with SM major > 2, search only these
         device.major == best_SM_arch)) // if our device==dest_SM_arch, choose
                                        // this, or else pass
    {
...
CUresult initDevice(int deviceID) {
  ...
  if (device.major < 3) {
    VERBOSE("CUDA error: compute capability 3.2 or greater required (available "
            "%d.%d for device[%d])",
            device.ID, device.major, device.minor);
    return CUDA_ERROR_INVALID_DEVICE;
  }
  devices.emplace_back(device);
  checkCudaError(
      cuCtxCreate(&devices.back().ctx, CU_CTX_SCHED_AUTO, device.ID));
  ...
  DEBUG("CUDA device %d initialized: %s (compute capability %d.%d; memory %s)",
        device.ID, name, device.major, device.minor,
        Util::formatBytes(memSize).c_str());
#endif
  return CUDA_SUCCESS;
}
...
CUresult ptxJIT(LPCSTR program, CUmodule &hModule, int mode) {
  ...
  if (mode == JIT::FILE || (mode == JIT::AUTO && programLen < 256)) {
    ...
    myErr = cuLinkAddData(lState, CU_JIT_INPUT_PTX, (void *)program,
                          programLen + 1, 0, 0, 0, 0);
  }
...
CUresult KernelRT::Reset(LPCSTR program, LPCSTR functionName, int mode) {
  // compile the module (program) from PTX and get the function handle (Driver
  // API)
...
CUresult KernelRT::GetResult(const CUdeviceptr data,
...
CUresult
KernelRT::GetResult(const std::initializer_list<ReturnParam> &params) const {
