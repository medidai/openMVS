 ...
 *  @test tested with TestRotationConversion.cpp
        ...
        x 	       y 	        z 	       w          Description
        0 	       0 	        0 	       1          Identity
 quaternion, no rotation 1 	       0 	        0 	       0
 180' turn around X axis 0 	       1 	        0 	       0
 180' turn around Y axis 0 	       0 	        1 	       0
 180' turn around Z axis sqrt(0.5)    0 	        0 	       sqrt(0.5)
 90' rotation around X axis 0 	       sqrt(0.5) 	0 	       sqrt(0.5)
 90' rotation around Y axis 0 	       0 	        sqrt(0.5)  sqrt(0.5) 90'
 rotation around Z axis -sqrt(0.5)    0 	        0 	       sqrt(0.5)
 -90' rotation around X axis 0           -sqrt(0.5)    0 	       sqrt(0.5)
 -90' rotation around Y axis 0 	       0 	       -sqrt(0.5)  sqrt(0.5)
 -90' rotation around Z axis

        @author grest 06/2003
*/
template <typename TYPE> class TQuaternion : public TMatrix<TYPE, 4, 1> {
public:
...
public:
  ...
  */
   ...
   */
  inline void Mult(const TQuaternion<TYPE> &arg, TQuaternion<TYPE> &res) const;
  ...
  */
  ...
  @note Quaternions are assumed to be unit quaternions! */
  TQuaternion<TYPE> Interpolate(const TQuaternion<TYPE> &to,
                                const TYPE &t) const;
  ...
  */
  ...
  inline bool IsNormalized() const { return ISEQUAL(norm(*this), TYPE(1)); }
  ...
  /** Enforces rigid coupling constraint for all quaternion in given vector.
          @author esquivel 02/2012 */
  static void
  EnforceRigidCouplingConstraint(std::vector<TQuaternion<TYPE>> &quats);
}; // class
template <typename TYPE>
const TQuaternion<TYPE> TQuaternion<TYPE>::IDENTITY(0, 0, 0, 1);
/*----------------------------------------------------------------*/
...
/** @class TRMatrixBase
        ...
        @author frahm, woelk **/
template <typename TYPE> class TRMatrixBase : public TMatrix<TYPE, 3, 3> {
public:
...
public:
  ...
  template <typename T>
  inline TRMatrixBase(const cv::Matx<T, 3, 3> &rhs) : Base(rhs) {}
  ...
  /** @brief Initialization from rotation axis w and angle phi (in rad) using
   * Rodrigues' formula */
  ...
  /** @brief Initialization with the rotation from direction dir0 to direction
   * dir1 */
  inline TRMatrixBase(const Vec &dir0, const Vec &dir1);

  template <typename T>
  inline TRMatrixBase &operator=(const cv::Matx<T, 3, 3> &rhs) {
    BaseBase::operator=(rhs);
    return *this;
  }
  inline TRMatrixBase &operator=(const cv::Mat &rhs) {
    BaseBase::operator=(rhs);
    return *this;
  }
#ifdef _USE_EIGEN
  inline TRMatrixBase &operator=(const typename Base::EMat &rhs) {
    Base::operator=(rhs);
    return *this;
  }
#endif
  ...
  inline void SetXYZ(const Vec &r) { SetXYZ(r[0], r[1], r[2]); }
  ...
  inline void SetZYX(const Vec &r) { SetZYX(r[0], r[1], r[2]); }
  ...
  inline void SetZXY(const Vec &r) { SetZXY(r[0], r[1], r[2]); }
  ...
  void SetFromColumnVectors(const Vec &v0, const Vec &v1, const Vec &v2);

  /** set this matrix from 3 vectors, each representing a row */
  void SetFromRowVectors(const Vec &v0, const Vec &v1, const Vec &v2);
  ...
  inline Quat GetQuaternion() const {
    Quat q;
    GetQuaternion(q);
    return q;
  }
  ...
  /** @brief Set rotation matrix from direction and up vector
          ...
          direction is (0,0,-1) and up is (0,1,0) the resulting matrix is
     identity.
  ...
  /** @brief Set rotation matrix from an eye point and a look-at target point
   * and the up vector */
  ...
  // modify the rotation matrix by the given parametrized delta rotation
  // (axis-angle)
...
#if 1
    return (ISEQUAL((float)cv::determinant(*this), 1.f) &&
            ISEQUAL((float)cv::trace((*this) * (*this).t()), 3.f));
#else
    return (ISEQUAL((TYPE)cv::determinant(*this), TYPE(1)) &&
            ISEQUAL((TYPE)cv::trace((*this) * (*this).t()), TYPE(3)));
#endif
  }
