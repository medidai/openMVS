...
// Queue template
**************************************************************************************/
template <class TYPE, class ARG_TYPE = const TYPE &, int useConstruct = 1,
          int xGrow = 16>
class cQueue {
public:
  cQueue(int_t xSize = 16) : first(-1), last(-1), vectorSize(0), vector(NULL) {
    if (xSize > 0)
      Grow(xSize);
  }
  ...
  inline TYPE *GetData() { return vector; }
  ...
  inline void Empty() { first = last = -1; }

  // same as Empty(), plus free all allocated memory
  inline void Release() {
    ASSERT((vector != NULL && vectorSize > 0) ||
           (vector == NULL && vectorSize == 0));
    if (vector != NULL) {
      if (useConstruct) {
        if (first > last) {
  ...
  inline TYPE &operator[](uint_t index) const { return GetHead(index); }
