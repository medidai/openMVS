...
const lm_control_struct lm_control_double = {LM_USERTOL,
                                             LM_USERTOL,
                                             LM_USERTOL,
                                             LM_USERTOL,
                                             100.,
...
const lm_control_struct lm_control_float = {1.e-7,
                                            1.e-7,
                                            1.e-7,
                                            1.e-7,
                                            100.,
...
void lmmin(int n_par, double *par, int m_dat, const void *data,
  ...
  fvec = (double *)malloc((2 * m + 5 * n + n * m) * sizeof(double) +
                          n * sizeof(int));
  ...
  lm_lmdif(m, n, par, fvec, control->ftol, control->xtol, control->gtol,
           control->maxcall, control->epsilon,
           (control->scale_diag ? diag : NULL), (control->scale_diag ? 1 : 2),
           control->stepbound, status->info, status->nfev, fjac, ipvt, qtf, wa1,
           wa2, wa3, wa4, evaluate, data
#ifdef LMFIT_PRINTOUT
           ,
...
void lm_lmdif(int m, int n, double *x, double *fvec, double ftol, double xtol,
  ...
  double actred, delta, dirder, eps, fnorm, fnorm1, gnorm, par, pnorm, prered,
      ratio, step, sum, temp1, temp2, temp3, xnorm;
  ...
  do {
    ...
    if (epsfcn == 0) {
      ...
      if (nfev == 0) {
        ...
        /*** outer: evaluate function at starting point, calculate norm, and get
      ...
      if (nfev == 0) {
        ...
        eps = sqrt(MAXF(epsfcn, LM_MACHEP)); /* for calculating the Jacobian by
                                                forward differences */
      }
      ...
      for (j = 0; j < n; ++j) {
        ...
        if (info < 0)
          return;           /* user requested break */
        step = x[j] - temp; /* due to float point arithmetic errors most of the
                               time this is not equal to step */
        for (i = 0; i < m; ++i)
    ...
    do {
      ...
      printf("lmdif/ actred=%.10e prered=%.10e ratio=%.10e"
             ...
             actred, prered, prered != 0 ? ratio : 0., SQUARE(temp1),
             SQUARE(temp2), dirder);
#endif
      ...
      if (ratio <= 0.25) {
        double temp =
            (actred >= 0 ? 0.5 : 0.5 * dirder / (dirder + 0.5 * actred));
