...
This software implements the IBFS (Incremental Breadth First Search) maximum
flow algorithm from "Maximum flows by incremental breadth-first search" Andrew
V. Goldberg, Sagi Hed, Haim Kaplan, Robert E. Tarjan, and Renato F. Werneck. In
Proceedings of the 19th European conference on Algorithms, ESA'11, pages
457-468. ISBN 978-3-642-23718-8 2011
...
aforementioned paper in any resulting publication and appropriately credit it.
...
void IBFSGraph::initSize(int numNodes, int numEdges) {
  ...
  // allocate arcs
  size_t arcMemsize =
      sizeof(TmpArc) * (numEdges * 2) + sizeof(TmpEdge) * numEdges;
...
void IBFSGraph::initGraphFast() {
  ...
  // tmpEdges:				edges read
  // node.label: 				index into arcs array of first
  // out arc node.firstArc-tmpArcs: 	index into arcs array of next out arc to
  // be allocated
  //							(initially the first out
  //arc)
  ...
  // tmpEdges:				edges read
  // tmpArcs:					arcs with reverse pointer but no
  // node id node.label: 				index into arcs array of
  // first out arc node.firstArc-tmpArcs: 	index into arcs array of last
  // allocated out arc
  ...
  // arcs:					arcs array
  // node.label: 				index into arcs array of first
  // out arc node.firstArc-tmpArcs: 	index into arcs array of last allocated
  // out arc arc.head = tail of arc
...
template <bool dirS> void IBFSGraph::growth() {
  ...
  for (Node **active = active0.list; active != (active0.list + active0.len);
       active++) {
    // get active node
    x = (*active);
    testNode(x);
...
void IBFSGraph::testTree() {
  ...
  for (x = nodes; x != nodeEnd; x++) {
    if (x->label != numNodes &&
        (x->label > topLevelS || x->label < -topLevelT)) {
    ...
    for (a = x->firstArc; a != (x + 1)->firstArc; a++) {
      ...
      if (sTree ? (a->head->label > (x->label + 1))
                : (a->head->label < (x->label - 1))) {
...
bool IBFSGraph::readFromFile(char *filename, bool checkCompile) {
  ...
  while (fgets(line, MAX_LINE_LEN, pFile) != NULL) {
    ...
    case 'p':
      ...
      if (c1 == 'm' && c2 == 'a' && c3 == 'x') {
        sscanf(line, "%c %c%c%c %d %d", &c, &c1, &c2, &c3, &declaredNumOfNodes,
               &declaredNumOfEdges);
      } else {
    ...
    case 'n':
      ...
      if (capacity != 0 || capacity2 != 0) {
        ...
        if (checkCompile) {
          ...
          memcpy(buffer + sizeof(char) + sizeof(int) + sizeof(int), &capacity,
                 sizeof(int));
          memcpy(buffer + sizeof(char) + sizeof(int) + sizeof(int) +
                     sizeof(int),
                 &capacity2, sizeof(int));
    ...
    case 'a':
      sscanf(line, "%c %d %d %d %d", &c, &nodeId1, &nodeId2, &capacity,
             &capacity2);
      if (nodeId1 < 0 || nodeId1 >= declaredNumOfNodes || nodeId2 < 0 ||
      ...
      if (checkCompile) {
        ...
        memcpy(buffer + sizeof(char) + sizeof(int) + sizeof(int), &capacity,
               sizeof(int));
        memcpy(buffer + sizeof(char) + sizeof(int) + sizeof(int) + sizeof(int),
               &capacity2, sizeof(int));
        fwrite(&buffer, 1, bufferSize, pFileCompiled);
      }
      break;
  ...
  if (currentNumOfEdges != declaredNumOfEdges) {
    fprintf(stdout,
            "inconsistent number of edges: differs from declared %d != %d\n",
...
bool IBFSGraph::readCompiled(FILE *pFile) {
  ...
  for (int line = 0; !feof(pFile); line++) {
    ...
    memcpy(&capacity, buffer + sizeof(char) + sizeof(int) + sizeof(int),
           sizeof(int));
    memcpy(&capacity2,
           buffer + sizeof(char) + sizeof(int) + sizeof(int) + sizeof(int),
           sizeof(int));
    if (buffer[0] == 'n') {
      if (capacity != 0 || capacity2 != 0) {
        addNode(nodeId1, capacity, capacity2);
      ...
      if (nodeId1 < 0 || nodeId1 >= declaredNumOfNodes || nodeId2 < 0 ||
          nodeId2 >= declaredNumOfNodes) {
        fprintf(stdout,
                "inconsistent node index in compiled file %d,%d line %d\n",
