...
###########
        ...
        // g.initSize(numNodes, numEdges) indicate the number of nodes and edges
in the graph.
        // Number of edges does not include edges from the source and to the
sink! g->initSize(4, 5);

        // g.addNode(nodeID, capFromSource, capToSink) indicate node is
connected
...
public:
  IBFSGraph();
  ~IBFSGraph();
  void setVerbose(bool a_verbose) { verbose = a_verbose; }

  void initSize(int numNodes, int numEdges);
  void addEdge(int nodeIndexFrom, int nodeIndexTo, EdgeCap capacity,
               EdgeCap reverseCapacity);
  void addNode(int nodeIndex, EdgeCap capacityFromSource,
               EdgeCap capacityToSink);
  ...
  inline IBFSStats getStats() { return stats; }
  inline EdgeCap getFlow() { return flow; }
  inline size_t getNumNodes() { return nodeEnd - nodes; }
  inline size_t getNumArcs() { return arcEnd - arcs; }
...
inline void IBFSGraph::addNode(int nodeIndex, EdgeCap capacitySource,
                               EdgeCap capacitySink) {
...
inline void IBFSGraph::addEdge(int nodeIndexFrom, int nodeIndexTo,
                               EdgeCap capacity, EdgeCap reverseCapacity) {
