...
protected:
  struct DirectedEdge {
    ...
    inline DirectedEdge(NodeID _nodeID1, NodeID _nodeID2)
        : nodeID1(_nodeID1), nodeID2(_nodeID2) {}
  };
...
public:
  ...
  inline void SetNumNodes(NodeID nNodes) { nodes.resize(nNodes); }
  inline NodeID GetNumNodes() const { return (NodeID)nodes.size(); }
  ...
  inline void SetSmoothCost(FncSmoothCost func) { fncSmoothCost = func; }
...
#pragma omp parallel for reduction(+ : energy)
    ...
    for (int_t edgeID = 0; edgeID < (int_t)edges.size(); ++edgeID) {
      ...
      energy +=
          fncSmoothCost(edge.nodeID1, edge.nodeID2, nodes[edge.nodeID1].label,
      ...
      for (int_t edgeID = 0; edgeID < (int_t)edges.size(); ++edgeID) {
        ...
        for (size_t j = 0; j < labels2.size(); ++j) {
          ...
          for (size_t k = 0; k < labels1.size(); ++k) {
            ...
            EnergyType energy(
                nodes[edge.nodeID1].dataCosts[k] +
...
#pragma omp parallel for
  ...
  EnergyType Optimize() {
    ...
    while (true) {
      ...
      DEBUG_ULTIMATE("\t%2u. e: %g\td: %g\tt: %s", i, last_energy, diff,
    ...
    if (diff == EnergyType(0)) {
      DEBUG_ULTIMATE("Inference converged in %u iterations: %g energy (%s)", i,
                     energy, TD_TIMER_GET_FMT().c_str());
    } else {
      DEBUG_ULTIMATE(
          "Inference aborted (energy increased): %u iterations, %g energy (%s)",
          i, energy, TD_TIMER_GET_FMT().c_str());
    }
    return energy;
  }

  inline LabelID GetLabel(NodeID nodeID) const { return nodes[nodeID].label; }
};
/*----------------------------------------------------------------*/
