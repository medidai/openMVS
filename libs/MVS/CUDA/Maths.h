...
__device__ inline Point3i RoundToInt3(const Point3 &v) {
  return Point3i(__float2int_rn(v.x()), __float2int_rn(v.y()),
                 __float2int_rn(v.z()));
}
__device__ inline Point3i FloorToInt3(const Point3 &v) {
  return Point3i(__float2int_rd(v.x()), __float2int_rd(v.y()),
                 __float2int_rd(v.z()));
}
#endif

template <typename T> __device__ constexpr T Square(const T x) { return x * x; }

template <typename T> __device__ constexpr void Swap(T &a, T &b) {
  const T c(a);
  a = b;
  b = c;
}

// mod is always positive, whereas % is the remainder, which can be positive or
// negative
template <typename T> __device__ T inline Mod(T a, T b) {
  const T result = a % b;
  if (result < 0)
    return result + b;
  return result;
...
inline __device__ Point2i GetThreadIndex2() {
  return Point2i(blockIdx.x * blockDim.x + threadIdx.x,
                 blockIdx.y * blockDim.y + threadIdx.y);
}

inline __device__ Point3i GetThreadIndex3() {
  return Point3i(blockIdx.x * blockDim.x + threadIdx.x,
                 blockIdx.y * blockDim.y + threadIdx.y,
...
__device__ inline int Point2Idx(int stride, const Point2i &pixel,
                                int numChannels = 1) {
...
__device__ inline bool IsInImage(const int x, const int y, const int width,
                                 const int height) {
  return x >= 0 && y >= 0 && x < width && y < height;
}
__device__ inline bool IsInImage(const Point2i &pixel, const Point2i &size) {
  return pixel.x() >= 0 && pixel.y() >= 0 && pixel.x() < size.x() &&
         pixel.y() < size.y();
}
/*----------------------------------------------------------------*/
