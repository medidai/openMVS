...
struct Pose {
  ...
  __host__ __device__ Pose(const Matrix3 &R, const Point3 &C) : R(R), C(C) {}
...
struct Camera {
  ...
  __host__ __device__ Camera(const LinearCameraModel &model, const Pose &pose,
                             int width = 0, int height = 0)
      : model(model), pose(pose), size(width, height) {}
  __host__ __device__ Camera(const Matrix3 &K, const Matrix3 &R,
                             const Point3 &C, int width = 0, int height = 0)
  ...
  __host__ __device__ inline Point3
  TransformPointI2W(const Point2 &x, const float depth = 1.f) const {
