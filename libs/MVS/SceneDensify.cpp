...
bool DepthMapsData::SelectViews(DepthData &depthData) {
  ...
  if (!Scene::FilterNeighborViews(depthData.neighbors, fMinArea, fMinScale,
                                  fMaxScale, fMinAngle, fMaxAngle, nMaxViews)) {
...
// from disk, if 0, the reference depth-map is initialized from sparse
// point-cloud, and if -1, the depth-maps are not initialized returns false if
// there are no good neighbors to estimate the depth-map
bool DepthMapsData::InitViews(DepthData &depthData, IIndex idxNeighbor,
  ...
  if (idxNeighbor != NO_ID) {
    ...
    if (g_nVerbosityLevel > 2) {
      ...
      VERBOSE("Reference image %3u paired with %u views:%s (%u shared points)",
              idxImage, depthData.images.size() - 1, msg.c_str(),
  ...
  if (loadDepthMaps > 0) {
    ...
    ASSERT(depthData.normalMap.empty() ||
           viewRef.image.size() == depthData.normalMap.size());
    if (depthData.normalMap.empty()) {
      // estimate normal map
      EstimateNormalMap(viewRef.camera.K, depthData.depthMap,
                        depthData.normalMap);
    }
...
// roughly estimate depth and normal maps by triangulating the sparse
// point-cloud and interpolating normal and depth for all pixels
...
void *STCALL DepthMapsData::ScoreDepthMapTmp(void *arg) {
  ...
  while ((idx = (IDX)Thread::safeInc(estimator.idxPixel)) <
    ...
    const Normal viewDir(
        Cast<float>(static_cast<const Point3 &>(estimator.X0)));
    ...
    for (int v = 0; v < size.y; ++v) {
      ...
      if (seg_list_count < speckle_size) {
        ...
        for (unsigned i = 0; i < seg_list_count; ++i) {
          ...
          if (!normalMap.empty())
            normalMap(seg_list[i]) = Normal::ZERO;
          if (!confMap.empty())
            confMap(seg_list[i]) = 0;
        }
      }
    }
  }
...
// adjust confidence-map based on the depth-map and the confidence-maps of the
// neighbor depth-maps
bool DepthMapsData::AdjustConfidenceFast(DepthData &depthDataRef,
                                         const IIndexArr &idxNeighbors) {
  TD_TIMER_STARTD();

  // set confidence as the similarity of the depth values in the neighbor
  // depth-maps to the depth value from the reference depth-map
  ...
  for (int r = 0; r < depthDataRef.depthMap.rows; ++r) {
    for (int c = 0; c < depthDataRef.depthMap.cols; ++c) {
      ...
      // check if the point's depth is similar to the depth in the neighbor
      // depth-maps and keep the smallest difference
      Depth minDiff(1.f);
      ...
      // if confidence-map is available, the final confidence is a combination
      // of the photometric and similarity confidence
      ...
      newConfMap(r, c) =
          bKeep ? 0.3f * confPhoto + 0.7f * confSimilarity
  ...
  if (!SaveConfidenceMap(
          ComposeDepthFilePath(imageRef.GetID(), "adjusted.fast.cmap"),
          newConfMap))
    return false;

  DEBUG("Confidence-map %3u fast-adjusted using %u other images: %u/%u depths "
        "discarded (%s)",
...
// filter confidence-map, one pixel at a time, using confidence based fusion of
// neighbor pixels
bool DepthMapsData::AdjustConfidence(DepthData &depthDataRef,
  ...
  for (int i = 0; i < sizeRef.height; ++i) {
    for (int j = 0; j < sizeRef.width; ++j) {
      ...
      if (nPosViews >= nMinViewsAdjust && posConf > negConf &&
          ISINSIDE(avgDepth /= posConf, depthDataRef.dMin, depthDataRef.dMax)) {
        // consider this pixel an inlier
        newConfMap(xRef) = 1.f - MINF(((negConf + 0.2f) * nPosViews) /
                                          (posConf * MAXF(nNegViews, 1u)),
                                      1.f);
      } else {
  ...
  if (!SaveConfidenceMap(
          ComposeDepthFilePath(imageRef.GetID(), "adjusted.cmap"), newConfMap))
    return false;

  DEBUG("Confidence-map %3u adjusted using %u other images: %u/%u depths "
        "discarded (%s)",
        imageRef.GetID(), N, nDiscarded, nProcessed,
        TD_TIMER_GET_FMT().c_str());
  return true;
} // AdjustConfidence
/*----------------------------------------------------------------*/
...
#pragma omp flush(bAbort)
    ...
    if (depthData.normalMap.empty()) {
      EstimateNormalMap(depthData.images.front().camera.K, depthData.depthMap,
                        depthData.normalMap);
...
void DepthMapsData::MergeDepthMaps(PointCloud &pointcloud, bool bEstimateColor,
  ...
  FOREACH(idxImage, arrDepthData) {
    ...
    DEBUG_ULTIMATE("Depth-map for reference image %3u merged using %u "
                   "depth-maps: %u new points (%s)",
                   idxImage, depthData.images.size() - 1,
                   pointcloud.points.size() - nNumPointsPrev,
                   TD_TIMER_GET_FMT().c_str());
...
//  - numDMapsReserveFusion: maximum number of depth-maps for which to reserve
//  memory for fusion
size_t GetAvailableMemory(const DepthDataArr &arrDepthData,
                          const BoolArr &fusedDMaps,
                          IIndex numDMapsReserveFusion,
                          size_t currentCacheMemory = 0) {
  size_t resolution(0);
  IIndex numDMaps(0);
  FOREACH(idxImage, arrDepthData) {
  ...
  const size_t neededPointCloudMemory(ROUND2INT<size_t>(
      resolution *
      (1 /*depth*/ + 1 /*color*/ + 3 /*normal*/ + 1 /*confidence*/) *
      4 /*bytes*/ * 0.35 /*unique pixels per depth-map*/));
  const size_t freeMemory(currentCacheMemory + memInfo.freePhysical);
  const size_t safetyMemory(
      MAXF(ROUND2INT<size_t>(memInfo.totalPhysical * 0.08),
           size_t(1 * 1024 * 1024 * 1024ull) /*1GB*/));
  const size_t neededMemory(neededPointCloudMemory + safetyMemory);
  const size_t minDMapsMemory(
      resolution / numDMaps * 8 /*min dmaps in memory*/ *
      (1 /*depth*/ + 3 /*normal*/ + 1 /*confidence*/) * 4 /*bytes*/);
  if (freeMemory < neededMemory) {
    DEBUG("warning: not enough memory to cache depth-maps (%luMB needed, %luMB "
          "available)",
          neededMemory / 1024 / 1024, freeMemory / 1024 / 1024);
    return MINF(currentCacheMemory, minDMapsMemory);
  }
  return freeMemory - neededMemory;
} // GetAvailableMemory

// finds the best depth-map to fuse next that maximizes the number of neighbors
// already in cache
std::tuple<unsigned, unsigned, unsigned>
FetchBestNextDMapIndex(const DepthDataArr &arrDepthData,
                       const DMapCache &cacheDMaps, const BoolArr &fusedDMaps) {
  const IIndexArr cachedImages = cacheDMaps.GetCachedImageIndices(true);
  IIndex bestImageIdx = NO_ID;
  unsigned bestImageScore = 0,
           bestImageSize = std::numeric_limits<unsigned>::max();
  FOREACH(idxImage, arrDepthData) {
    const DepthData &depthData = arrDepthData[idxImage];
    if (!depthData.IsValid())
      continue;
  ...
  return std::make_tuple(bestImageIdx, bestImageScore,
                         static_cast<unsigned>(cachedImages.size()));
...
void DepthMapsData::FuseDepthMaps(PointCloud &pointcloud, bool bEstimateColor,
  ...
  const unsigned nMinViewsFuse(
      MINF(OPTDENSE::nMinViewsFuse, arrDepthData.size()));
  ...
  const size_t nPointsEstimate(arrDepthData.size() *
                               9000); // TODO: better estimate number of points
  ...
  fusedDMaps.Memset(0);
  DMapCache cacheDMaps(
      arrDepthData, depthDataLoadFlags,
  ...
  for (; numDMapsFused < arrDepthData.size(); ++numDMapsFused) {
    TD_TIMER_STARTD();
    // find the best depth-map to fuse next as the one with the most neighbors
    // already in cache
...
#pragma omp parallel for
    ...
    ASSERT(depthData.depthMap.size() == depthData.size &&
           imageData.GetSize() == depthData.size);
    ...
    for (int i = 0; i < depthData.size.height; ++i) {
      for (int j = 0; j < depthData.size.width; ++j) {
        ...
        const PointCloud::Normal normal(
            !depthData.normalMap.empty()
                ? Cast<Normal::Type>(imageData.camera.R.t() *
    ...
    ASSERT(pointcloud.points.size() == pointcloud.pointViews.size() &&
           pointcloud.points.size() == pointcloud.pointWeights.size() &&
           pointcloud.points.size() == projs.size());
    DEBUG_ULTIMATE("Depth-map for reference image %3u fused using %u "
                   "depth-maps: %u new points, %u/%u cached images (%s)",
                   idxImage, depthData.images.size() - 1,
                   pointcloud.points.size() - nNumPointsPrev,
                   numImageNeighborsInCache, numImagesInCache,
                   TD_TIMER_GET_FMT().c_str());
    ...
    if (numDMapsFused % numDMapsReserveFusion == 0)
      cacheDMaps.SetMaxMemory(GetAvailableMemory(arrDepthData, fusedDMaps,
                                                 numDMapsReserveFusion,
                                                 cacheDMaps.GetUsedMemory()));
  }
  GET_LOGCONSOLE().Play();
  progress.close();
  arrDepthIdx.Release();
  ...
  DEBUG_EXTRA("Depth-maps fused and filtered: %u depth-maps, %u depths, %u "
              "points (%d%%%%), %.2f hits in %.2f cached (%s)",
              numDMapsFused, nDepths, pointcloud.points.size(),
              ROUND2INT((100.f * pointcloud.points.size()) / nDepths),
              static_cast<double>(totalNumImageNeighborsInCache) /
                  numDMapsFused,
              static_cast<double>(totalNumImagesInCache) / numDMapsFused,
              TD_TIMER_GET_FMT().c_str());

  if (bEstimateNormal && !pointcloud.points.empty() &&
      pointcloud.normals.empty()) {
...
#pragma omp parallel for
    ...
    DEBUG_EXTRA("Normals estimated for the dense point-cloud: %u normals (%s)",
                pointcloud.GetSize(), TD_TIMER_GET_FMT().c_str());
  }
} // FuseDepthMaps
...
// join points very likely to represent the same 3D point and
// filter out points blocking the view
void DepthMapsData::DenseFuseDepthMaps(PointCloud &pointcloud,
                                       bool bEstimateColor,
                                       bool _bEstimateNormal) {
  TD_TIMER_STARTD();
  ...
  const float maxReprojErrorSq(
      SQUARE(OPTDENSE::fDepthReprojectionErrorThreshold));
  const IIndex numDMapsReserveFusion(10);
  const bool bEstimateNormal(
      true); // always estimate normals as they are needed for the fusion
  size_t nDepths(0);
  ...
  Point3d fusedNormal;
  Pixel32F fusedColor;
  const auto FusePoint = [&](IIndex ID, const ImageRef &x,
                             unsigned fuseDepth) -> void {
    const auto lambda = [&](IIndex ID, const ImageRef &x, unsigned fuseDepth,
                            const auto &FusePointImpl) -> void {
      ...
      if (fuseDepth > 0) {
        ...
        ASSERT(pt.z > Depth(0) ||
               !IsDepthSimilar(depth, pt.z, OPTDENSE::fDepthDiffThreshold));
      ...
      const PointCloud::Point X(image.camera.TransformPointI2W(
          Point3(REAL(x.x), REAL(x.y), REAL(depth))));
      ...
      if (++fuseDepth >= OPTDENSE::nMaxFuseDepth ||
          fusedPoints[0].size() >= OPTDENSE::nMaxPointsFuse)
        return;
      ...
      for (const ViewScore &neighbor : image.pImageData->neighbors) {
        ...
        const ImageRef nextx(
            ROUND2INT(nextDepthData.GetCamera().ProjectPointP(X)));
...
#pragma omp parallel for
    ...
    for (int i = 0; i < depthData.size.height; ++i) {
      for (int j = 0; j < depthData.size.width; ++j) {
        ...
        if (fusedPoints[0].size() >= OPTDENSE::nMinPixelsFuse &&
            fusedViews.size() >= nMinViewsFuse) {
          // create the corresponding 3D point
          pointcloud.points.emplace_back(fusedPoints[0].GetMedian(),
          ...
          if (bEstimateColor)
            pointcloud.colors.emplace_back(
                (fusedColor / (float)fusedPoints[0].size()).cast<uint8_t>());
        }
        if (!fusedViews.empty()) {
  ...
  DEBUG_EXTRA("Depth-maps dense fused and filtered: %u depth-maps, %u depths, "
              "%u points (%d%%%%), %.2f hits in %.2f cached (%s)",
              numDMapsFused, nDepths, pointcloud.points.size(),
              ROUND2INT((100.f * pointcloud.points.size()) / nDepths),
              static_cast<double>(totalNumImageNeighborsInCache) /
                  numDMapsFused,
...
DenseDepthMapData::DenseDepthMapData(Scene &_scene, int _nFusionMode)
    : scene(_scene), depthMaps(_scene), idxImage(0), sem(1),
      nEstimationGeometricIter(-1), nFusionMode(_nFusionMode) {
  if (nFusionMode < 0) {
...
bool Scene::DenseReconstruction(int nFusionMode, bool bCrop2ROI,
  ...
  case OPTDENSE::FUSE_DENSEFILTER:
    ...
    data.depthMaps.DenseFuseDepthMaps(pointcloud,
...
bool Scene::ComputeDepthMaps(DenseDepthMapData &data) {
  ...
  if (IsEmpty() && !ImagesHaveNeighbors()) {
    VERBOSE("warning: empty point-cloud, rough neighbor views selection based "
            "on image pairs baseline");
...
#endif
        VERBOSE("error: preparing images for dense reconstruction failed "
...
#pragma omp critical
  ...
  if (SEACAVE::CUDA::desiredDeviceID >= -1 && data.nFusionMode >= 0) {
    data.depthMaps.pmCUDA =
        new MVS::CUDA::PatchMatch(SEACAVE::CUDA::desiredDeviceID);
  ...
  if ((OPTDENSE::nOptimize &
       (OPTDENSE::ADJUST_CONFIDENCE | OPTDENSE::ADJUST_CONFIDENCE_FAST)) != 0) {
...
void Scene::DenseReconstructionEstimate(void *pData) {
  ...
  while (true) {
    ...
    case EVT_PROCESSIMAGE: {
      ...
      const bool depthmapComputed(
          data.nFusionMode < 0 ||
          (data.nFusionMode >= 0 && data.nEstimationGeometricIter < 0 &&
           File::access(
      ...
      if (depthmapComputed && data.nFusionMode >= 0) {
        if (OPTDENSE::nOptimize & OPTDENSE::OPTIMIZE) {
          if (!depthData.Load(
                  ComposeDepthFilePath(depthData.GetView().GetID(), "dmap"))) {
            VERBOSE("error: invalid depth-map '%s'",
                    ComposeDepthFilePath(depthData.GetView().GetID(), "dmap")
                        .c_str());
            exit(EXIT_FAILURE);
          }
    ...
    case EVT_ESTIMATEDEPTHMAP: {
      ...
      // request next image initialization to be performed while computing this
      // depth-map
      data.events.AddEvent(
          new EVTProcessImage((uint32_t)Thread::safeInc(data.idxImage)));
      // extract depth map
    ...
    case EVT_SAVEDEPTHMAP: {
      ...
      // save depth map as image
      if (g_nVerbosityLevel > 2) {
        ExportDepthMap(ComposeDepthFilePath(depthData.GetView().GetID(), "png"),
                       depthData.depthMap);
        ExportConfidenceMap(
...
void Scene::DenseReconstructionFilter(void *pData) {
  ...
  while ((evt = data.events.GetEvent(0)) != NULL) {
    ...
    case EVT_FILTERDEPTHMAP: {
      ...
      if (((OPTDENSE::nOptimize & OPTDENSE::ADJUST_CONFIDENCE_FAST) != 0 &&
           data.depthMaps.AdjustConfidenceFast(depthData, idxNeighbors)) |
          ((OPTDENSE::nOptimize & OPTDENSE::ADJUST_CONFIDENCE) != 0 &&
           data.depthMaps.AdjustConfidence(depthData, idxNeighbors))) {
    ...
    case EVT_ADJUSTDEPTHMAP: {
      ...
      if (!confMapFast.empty())
        File::deleteFile(ComposeDepthFilePath(depthData.GetView().GetID(),
                                              "adjusted.fast.cmap")
                             .c_str());
      if (!confMap.empty())
        File::deleteFile(
            ComposeDepthFilePath(depthData.GetView().GetID(), "adjusted.cmap")
                .c_str());
      if (confMapFast.empty()) {
        ...
        // set confidence-map as the best confidence if both confMap and
        // congMapFast are set
        for (int r = 0; r < depthData.confMap.rows; ++r) {
          for (int c = 0; c < depthData.confMap.cols; ++c) {
            ...
            depthData.confMap(r, c) =
                conf > 0 && confFast > 0 ? MAXF(conf, confFast) : 0.f;
          }
        }
        confMapFast.release();
        confMap.release();
      ...
      if (g_nVerbosityLevel > 2) {
        ...
        ExportPointCloud(
            ...
            *depthData.images.First().pImageData, depthMap, normalMap);
      }
#endif
...
void Scene::PointCloudFilter(int thRemove) {
  ...
  struct Collector {
    ...
    Collector(const Cone::RAY &ray, Real angle, const PointCloud &_pointcloud,
              IntArr &_visibility)
        : cone(ray, angle), coneIntersect(cone), pointcloud(_pointcloud),
