...
DMapFilterResourcePlan PlanDMapFilterResources(uint64_t pixels, const char* component)
{
	...
	const unsigned confidenceSummaryBytesPerPixel(plan.confidenceRequested ? 8u : 0u);
	...
	const char* unavailableReason=NULL)
{
	const Image& reference(scene.images[idxImage]);
	const String frameDir(DMapInstrumentFrameDir(reference, geometricIteration));
	// The current 2.4 branch retired ADJUST_CONFIDENCE_FAST. Keep its schema slot so old
	// reports remain readable, but report the method as disabled instead of restoring it.
	const bool fastEnabled(false);
	...
	methods.push_back(method("adjust_confidence_fast", fastEnabled, fast,
		"retired compatibility slot; ADJUST_CONFIDENCE_FAST is not present in the current 2.4 base"));
	methods.push_back(method("adjust_confidence", fullEnabled, full,
		"fusion-survival posterior from local plane reliability, soft multi-view confirmation, and free-space violations"));
	...
	nlohmann::json artifact = {
		...
		{"neighbors", neighbors},
		{"parameters", {
			{"compatibility_fast_status", "retired_in_current_2_4_base"},
			{"minimum_neighbor_confidence", 1.f-OPTDENSE::fNCCThresholdKeep},
			{"reprojection_threshold_pixels", OPTDENSE::fDepthReprojectionErrorThreshold},
			{"relative_depth_threshold", OPTDENSE::fDepthDiffThreshold},
			{"prior_strength", ConfRefine::PRIOR_STRENGTH},
			{"confirmation_tau", ConfRefine::CONFIRM_TAU},
			{"prior_gate", ConfRefine::PRIOR_GATE},
			{"photometric_floor", ConfRefine::PHOTO_FLOOR},
			{"anti_cascade_floor", ConfRefine::CONF_FLOOR},
			{"free_space_violation_weight", ConfRefine::VIOLATION_W},
			{"free_space_violation_margin", ConfRefine::VIOLATION_MARGIN},
		}},
...
void Scene::DenseReconstructionEstimate(void* pData)
{
	...
	while (true) {
		...
		case EVT_PROCESSIMAGE: {
			...
			#ifdef _USE_DMAP_INSTRUMENTATION
			if (!data.depthMaps.InitViews(depthData, data.neighborsMap.IsEmpty()?NO_ID:data.neighborsMap[evtImage.idxImage], OPTDENSE::nNumViews, !depthmapComputed, nLoadDepthMaps, data.nEstimationGeometricIter)) {
			#else
		...
		case EVT_SAVEDEPTHMAP: {
			...
			DepthData& depthData(data.depthMaps.arrDepthData[idx]);
			#ifdef _USE_DMAP_INSTRUMENTATION
			const bool observeConfidenceAdjustment(
				(OPTDENSE::nOptimize & OPTDENSE::ADJUST_CONFIDENCE) != 0 &&
				data.nEstimationGeometricIter >= 0 &&
				data.nEstimationGeometricIter+1 == (int)OPTDENSE::nEstimationGeometricIters &&
				!depthData.depthMap.empty() && DMapInstrumentImageEnabled(images[idx]));
			...
			if (observeConfidenceAdjustment) {
				confidenceSummaryAvailable = PrepareDMapFilterInstrumentation(*this, idx,
					data.nEstimationGeometricIter, DMapFilterPixelCount(images[idx], &depthData.depthMap),
					"confidence_adjustment", confidenceResourcePlan, confidenceStorageLease);
				if (confidenceSummaryAvailable) {
					observedNeighbors.Reserve(8);
					for (const ViewScore& neighbor: depthData.neighbors) {
						if (neighbor.ID >= data.depthMaps.arrDepthData.size() ||
							!data.depthMaps.arrDepthData[neighbor.ID].IsValid())
							continue;
			...
			if (observeConfidenceAdjustment) {
				...
				const char* status(
					...
					(!depthData.bConfAdjusted ? "failed" : (inputAvailable ? "completed" : "resource_unavailable")));
				const char* reason(
					!confidenceSummaryAvailable ? confidenceResourcePlan.reason.c_str() :
					(!depthData.bConfAdjusted ? "integrated confidence adjustment did not produce an output" :
						(inputAvailable ? NULL : "raw confidence is unavailable after fused in-kernel adjustment")));
				const DMapArtifactUsage confidenceUsage(WriteDMapConfidenceAdjustmentArtifacts(*this, idx,
...
void Scene::DenseReconstructionFilter(void* pData)
{
	...
	while ((evt=data.events.GetEvent(0)) != NULL) {
		...
		case EVT_FILTERDEPTHMAP: {
			...
			// Both mutually exclusive adjustment modes use the same deferred swap.
		...
		case EVT_ADJUSTDEPTHMAP: {
			...
			DMapFilterStorageLease confidenceStorageLease;
			bool confidenceSummaryAvailable(false);
			ConfidenceMap confidenceInput, confidenceFullOutput;
			IIndexArr observedNeighbors;
			if (observeConfidenceAdjustment) {
				confidenceSummaryAvailable = PrepareDMapFilterInstrumentation(*this, idx,
					geometricIteration, DMapFilterPixelCount(images[idx], &depthData.depthMap),
					"confidence_adjustment", confidenceResourcePlan, confidenceStorageLease);
				if (confidenceSummaryAvailable) {
					confidenceInput = depthData.confMap.clone();
					confidenceFullOutput = depthData.confMapAdjusted.clone();
					observedNeighbors.Reserve(8);
					for (const ViewScore& neighbor: depthData.neighbors) {
						if (neighbor.ID >= data.depthMaps.arrDepthData.size() ||
							!data.depthMaps.arrDepthData[neighbor.ID].IsValid())
			...
			if (observeConfidenceAdjustment) {
				const DMapArtifactUsage usage(WriteDMapConfidenceAdjustmentArtifacts(*this, idx,
					...
					NULL,
					confidenceSummaryAvailable ? &confidenceFullOutput : NULL,
