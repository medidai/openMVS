...
static LPCSTR const g_szMeshRefineModule =
    ...
    // the depth-map is computed by rasterizing all triangles (using a brute
    // force scan-line approach) and storing only the closest ones; additionally
    // the face index and barycentric coordinates are stored for each pixel
    ...
    "}\n"
    "\n"
    // kernel used to invalidate pixels that don't have valid both depth and
    // face index
    ...
    "\n"
    "	BB00_2:\n"
    "	tex.2d.v4.f32.f32 {%f141, %f142, %f143, %f144}, [texImageRef, {%f185, "
    "%f186}];\n"
    "	{\n"
    "	.reg .b16 %temp;\n"
    ...
    "}\n"
    "\n"
    // kernel used to compute the mean for all image pixels for a given windows
    // size
    ...
    "}\n"
    "\n"
    // kernel used to compute the variance for all image pixels for a given
    // windows size
    ...
    "}\n"
    "\n"
    // kernel used to compute the covariance for all image pixels for a given
    // windows size
    ...
    "}\n"
    "\n"
    // kernel used to compute the gradient of the ZNCC score for all image
    // pixels
    ...
    "}\n"
    "\n"
    // kernel used to compute the photometric gradient for all vertices seen by
    // an image pair
    ...
    "	tex.2d.v4.u32.f32 {%r29, %r30, %r31, %r32}, [texImageRef, {%f106, "
    "%f231}];\n"
    "	mov.b32 %f140, %r29;\n"
    "	tex.2d.v4.u32.f32 {%r34, %r35, %r36, %r37}, [texImageRef, {%f230, "
    "%f231}];\n"
    "	mov.b32 %f141, %r34;\n"
    "	sub.f32 %f142, %f140, %f141;\n"
    "	add.f32 %f113, %f231, 0f3F800000;\n"
    "	tex.2d.v4.u32.f32 {%r39, %r40, %r41, %r42}, [texImageRef, {%f230, "
    "%f113}];\n"
    "	mov.b32 %f143, %r39;\n"
    ...
    "	ret;\n"
    "}\n"
    // kernel used to combine the photo and both smoothness gradients for all
    // vertices
...
public:
  ...
  struct CameraCUDA {
    ...
    inline CameraCUDA() {}
    inline CameraCUDA(const Camera &camera, const Image8U::Size &_size)
        : P(camera.P), R(camera.R), C(camera.C), K(camera.K),
          invK(camera.GetInvK()), size(_size) {}
  };

public:
  MeshRefineCUDA(Scene &_scene, unsigned _nAlternatePair = true,
                 float _weightRegularity = 1.5f,
                 float _ratioRigidityElasticity = 0.8f,
                 unsigned _nResolutionLevel = 0, unsigned _nMinResolution = 640,
                 unsigned nMaxViews = 8);
  ~MeshRefineCUDA();

  bool IsValid() const {
    return module != NULL && module->IsValid() && !pairs.IsEmpty();
  }
  ...
  void ProjectMesh(const CameraFaces &cameraFaces, const Camera &camera,
                   const Image8U::Size &size, uint32_t idxImage);
  void ProcessPair(uint32_t idxImageA, uint32_t idxImageB);
  void ImageMeshWarp(const Camera &cameraA, const Camera &cameraB,
                     const Image8U::Size &size, uint32_t idxImageA,
                     uint32_t idxImageB);
  void ComputeLocalVariance(const SEACAVE::CUDA::ArrayRT16F &image,
                            const Image8U::Size &size,
                            SEACAVE::CUDA::MemDevice &imageMean,
                            SEACAVE::CUDA::MemDevice &imageVar);
  void ComputeLocalZNCC(const Image8U::Size &size);
  void ComputePhotometricGradient(const Camera &cameraA, const Camera &cameraB,
                                  const Image8U::Size &size, uint32_t idxImageA,
                                  uint32_t idxImageB, uint32_t numVertices,
                                  float RegularizationScale);
  void ComputeSmoothnessGradient(uint32_t numVertices);
  void CombineGradients(uint32_t numVertices);

public:
  const float weightRegularity; // a scalar regularity weight to balance between
                                // photo-consistency and regularization terms
  float ratioRigidityElasticity;   // a scalar ratio used to compute the
...
MeshRefineCUDA::MeshRefineCUDA(Scene &_scene, unsigned _nAlternatePair,
                               float _weightRegularity,
                               float _ratioRigidityElasticity,
                               unsigned _nResolutionLevel,
                               unsigned _nMinResolution, unsigned nMaxViews)
    : weightRegularity(_weightRegularity),
      ...
      nAlternatePair(_nAlternatePair), scene(_scene), images(_scene.images) {
...
MeshRefineCUDA::~MeshRefineCUDA() { scene.mesh.ReleaseExtra(); }
...
bool MeshRefineCUDA::InitKernels(int device) {
  ...
  if (SEACAVE::CUDA::devices.IsEmpty() &&
      SEACAVE::CUDA::initDevice(device) != CUDA_SUCCESS)
    return false;
  ...
  if (kernelCrossCheckProjection.Reset(module, "CrossCheckProjection") !=
      CUDA_SUCCESS)
    return false;
  ...
  if (kernelComputeImageDZNCC.Reset(module, "ComputeImageDZNCC") !=
      CUDA_SUCCESS)
    return false;
  ASSERT(kernelComputeImageDZNCC.IsValid());
  if (kernelComputePhotometricGradient.Reset(
          module, "ComputePhotometricGradient") != CUDA_SUCCESS)
    return false;
  ASSERT(kernelComputePhotometricGradient.IsValid());
  if (kernelUpdatePhotoGradNorm.Reset(module, "UpdatePhotoGradNorm") !=
      CUDA_SUCCESS)
    return false;
  ASSERT(kernelUpdatePhotoGradNorm.IsValid());
  if (kernelComputeSmoothnessGradient.Reset(
          module, "ComputeSmoothnessGradient") != CUDA_SUCCESS)
    return false;
  ...
  if (kernelCombineAllGradients.Reset(module, "CombineAllGradients") !=
      CUDA_SUCCESS)
    return false;
  ASSERT(kernelCombineAllGradients.IsValid());

  // init textures
  if (texImageRef.Reset(module, "texImageRef", CU_TR_FILTER_MODE_LINEAR) !=
      CUDA_SUCCESS)
    return false;
...
#pragma omp flush(bAbort)
  ...
  FOREACH(idxImage, views) {
    ...
    reportCudaError(
        view.image.SetData(cvtImage<float, hfloat>(view.imageHost)));
...
void MeshRefineCUDA::ListVertexFacesPost() {
  ...
  ASSERT(!scene.mesh.vertices.IsEmpty() &&
         scene.mesh.vertices.GetSize() == scene.mesh.vertexVertices.GetSize());
  // set vertex vertices
...
void MeshRefineCUDA::ListCameraFaces() {
  ...
  FOREACH(idxImage, images) {
    const Image &imageData = images[idxImage];
    if (imageData.IsValid())
      ProjectMesh(arrCameraFaces[idxImage], imageData.camera,
                  views[idxImage].size, idxImage);
  }
}
...
// images
void MeshRefineCUDA::SubdivideMesh(uint32_t maxArea, float fDecimate,
  ...
  if (!bNoDecimation) {
    if (fDecimate > 0.f) {
      ...
      if (medianArea < maxAreaf) {
        ...
        scene.mesh.Clean(MAXF(0.1f, medianArea / maxAreaf), 0.f, false,
...
// make sure there are no edges too small or too long
  ...
  DEBUG_EXTRA("Mesh subdivided: %u/%u -> %u/%u vertices/faces", numVertsOld,
              numFacesOld, scene.mesh.vertices.GetSize(),
...
void MeshRefineCUDA::ComputeNormalFaces() {
  ...
  reportCudaError(Mesh::kernelComputeFaceNormal((int)numFaces, vertices, faces,
                                                faceNormals, numFaces));
}
...
void MeshRefineCUDA::ScoreMesh(float *gradients) {
  ...
  FOREACHPTR(pPair, pairs) {
    ...
    case 1: {
      const PairIdx pair(iteration % 2 ? PairIdx(pPair->j, pPair->i)
                                       : PairIdx(pPair->i, pPair->j));
    ...
    default:
      for (int ip = 0; ip < 2; ++ip) {
        const PairIdx pair(ip ? PairIdx(pPair->j, pPair->i)
                              : PairIdx(pPair->i, pPair->j));
        ProcessPair(pair.i, pair.j);
      }
    }
...
void MeshRefineCUDA::ProjectMesh(const CameraFaces &cameraFaces,
                                 const Camera &camera,
                                 const Image8U::Size &size, uint32_t idxImage) {
  View &view = views[idxImage];
  // init depth-map
  ...
  // project mesh
  reportCudaError(kernelProjectMesh((int)faceIDsView.GetSize(), vertices, faces,
                                    faceIDsView, view.depthMap, view.faceMap,
                                    view.baryMap, CameraCUDA(camera, size),
                                    faceIDsView.GetSize()));
  kernelProjectMesh.Reset();
  // cross-check valid depth and face index
  reportCudaError(kernelCrossCheckProjection(size, view.depthMap, view.faceMap,
...
void MeshRefineCUDA::ProcessPair(uint32_t idxImageA, uint32_t idxImageB) {
  ...
  const float RegularizationScale(
      (float)((REAL)(imageDataA.avgDepth * imageDataB.avgDepth) /
              (cameraA.GetFocalLength() * cameraB.GetFocalLength())));
  ComputePhotometricGradient(cameraA, cameraB, sizeA, idxImageA, idxImageB,
...
void MeshRefineCUDA::ImageMeshWarp(const Camera &cameraA, const Camera &cameraB,
                                   const Image8U::Size &size,
  ...
  // project image
  reportCudaError(kernelImageMeshWarp(
      size, views[idxImageA].depthMap, views[idxImageB].depthMap, mask,
      CameraCUDA(cameraA, size), CameraCUDA(cameraB, size)));
#if 0
	// debug view
...
void MeshRefineCUDA::ComputeLocalVariance(
    const SEACAVE::CUDA::ArrayRT16F &image, const Image8U::Size &size,
    SEACAVE::CUDA::MemDevice &imageMean, SEACAVE::CUDA::MemDevice &imageVar) {
  surfImageRef.Bind(image);
  reportCudaError(kernelComputeImageMean(size, mask, imageMean, size.width,
                                         size.height, HalfSize));
  reportCudaError(kernelComputeImageVar(size, imageMean, mask, imageVar,
...
void MeshRefineCUDA::ComputeLocalZNCC(const Image8U::Size &size) {
  reportCudaError(kernelComputeImageCov(size, imageMeanA, imageMeanAB, mask,
                                        imageCov, size.width, size.height,
                                        HalfSize));
  reportCudaError(kernelComputeImageZNCC(size, imageCov, imageVarA, imageVarAB,
                                         mask, imageZNCC, size.width,
                                         size.height, HalfSize));
  reportCudaError(kernelComputeImageDZNCC(
      size, imageMeanA, imageMeanAB, imageVarA, imageVarAB, imageZNCC, mask,
      imageDZNCC, size.width, size.height, HalfSize));
#if 0
	// debug view
...
void MeshRefineCUDA::ComputePhotometricGradient(
    const Camera &cameraA, const Camera &cameraB, const Image8U::Size &size,
    uint32_t idxImageA, uint32_t idxImageB, uint32_t numVertices,
    float RegularizationScale) {
  // compute photometric gradient for all visible vertices
  reportCudaError(cuMemsetD32(photoGradPixels, 0, numVertices));
  reportCudaError(kernelComputePhotometricGradient(
      size, faces, faceNormals, views[idxImageA].depthMap,
      views[idxImageA].faceMap, views[idxImageA].baryMap, imageDZNCC, mask,
      photoGrad, photoGradPixels, CameraCUDA(cameraA, size),
      CameraCUDA(cameraB, size), RegularizationScale));
  // update photometric gradient norm for all visible vertices
  reportCudaError(kernelUpdatePhotoGradNorm(numVertices, photoGradNorm,
                                            photoGradPixels, numVertices));
#if 0
	// debug view
...
void MeshRefineCUDA::ComputeSmoothnessGradient(uint32_t numVertices) {
  ...
  reportCudaError(kernelComputeSmoothnessGradient(
      (int)numVertices, vertices, vertexVerticesCont, vertexVerticesSizes,
      vertexVerticesPointers, smoothGrad1, numVertices, uint8_t(0)));
  reportCudaError(kernelComputeSmoothnessGradient(
      (int)numVertices, smoothGrad1, vertexVerticesCont, vertexVerticesSizes,
      vertexVerticesPointers, smoothGrad2, numVertices, uint8_t(1)));
#if 0		
	// debug view
...
void MeshRefineCUDA::CombineGradients(uint32_t numVertices) {
  ...
  if (ratioRigidityElasticity >= 1.f) {
    reportCudaError(kernelCombineGradients((int)numVertices, photoGrad,
                                           photoGradNorm, smoothGrad2,
    ...
    reportCudaError(kernelCombineAllGradients(
        (int)numVertices, photoGrad, photoGradNorm, smoothGrad1, smoothGrad2,
...
bool Scene::RefineMeshCUDA(unsigned nResolutionLevel, unsigned nMinResolution,
  ...
  MeshRefineCUDA refine(*this, nAlternatePair, fRegularityWeight,
                        fRatioRigidityElasticity, nResolutionLevel,
                        nMinResolution, nMaxViews);
  if (!refine.IsValid())
    return false;
  ...
  for (unsigned nScale = 0; nScale < nScales; ++nScale) {
    ...
    for (int iter = 0; iter < iters; ++iter) {
      ...
      DEBUG_EXTRA("\t%2d. g: %.5f (%.3e - %.3e)\ts: %.3f", iter + 1,
                  gradients.norm(), gradients.norm() / mesh.vertices.GetSize(),
                  gv / mesh.vertices.GetSize(), gstep);
      gstep *= 0.98f;
