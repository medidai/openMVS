...
void Mesh::ListIncidentFaces() {
  ...
  FOREACH(iF, faces) {
    ...
    for (int v = 0; v < 3; ++v) {
      ...
      ASSERT(vfs.Find(iF) == FaceIdxArr::NO_INDEX ||
             vfs.Find(iF) == vfs.size() - 1 /*for degenerate faces*/);
...
// extract array face adjacencies for each face in the mesh (3 * number of
// faces); each triple describes the adjacent face triangles for a given face in
// the following edge order: v1v2, v2v3, v3v1; NO_ID indicates there is no
// adjacent face on that edge
void Mesh::ListIncidentFaceFaces() {
  ...
  struct inserter_data_t {
    ...
    FaceFaces &faces;
    int idx;
    inline inserter_data_t(FIndex _idxF, FaceFaces &_faces)
        : idxF(_idxF), faces(_faces), idx(0) {}
  ...
  FOREACH(f, faces) {
    ...
    for (int v = 0; v < 3; ++v) {
      ...
      std::set_intersection(facesI.begin(), facesI.end(), facesJ.begin(),
                            facesJ.end(), faceBackInserter);
...
void Mesh::ComputeNormalFaces() {
  ...
  if (kernelComputeFaceNormal.IsValid()) {
    reportCudaError(kernelComputeFaceNormal(
        (int)faces.size(), vertices, faces,
        CUDA::KernelRT::OutputParam(faceNormals.GetDataSize()), faces.size()));
...
// normals. The weight is simply the angle of the involved wedge. Described in:
// G. Thurmer, C. A. Wuthrich "Computing vertex normals from polygonal facets",
...
//  - fMaxGradient: maximum angle (in degrees) difference between neighbor
//  normals that is
//    allowed to take into consideration; higher angles are ignored
//  - fOriginalWeight: weight (0..1] to use for current normal value when
//  averaging with neighbor normals
//  - nIterations: number of times to repeat the smoothening process
void Mesh::SmoothNormalFaces(float fMaxGradient, float fOriginalWeight,
  ...
  for (unsigned rep = 0; rep < nIterations; ++rep) {
    ...
    FOREACH(idxFace, faces) {
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        if (ComputeAngleN(originalNormal.ptr(), neighborNormal.ptr()) >=
            cosMaxGradient)
          sumNeighborNormals += neighborNormal;
      }
      const Normal avgNeighborsNormal = normalized(sumNeighborNormals);
      const Normal newFaceNormal =
          normalized(originalNormal * fOriginalWeight +
...
void Mesh::GetEdgeVertices(FIndex f0, FIndex f1, uint32_t *vs0,
                           uint32_t *vs1) const {
...
Mesh::FIndex Mesh::GetEdgeAdjacentFace(FIndex idxFace, VIndex iV0,
                                       VIndex iV1) const {
  ...
  for (FIndex iF : vertexFaces[iV0]) {
    ...
    if (iF != idxFace) {
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        if (face[i] == iV1) {
          // check if there are more than two adjacent faces (manifold
          // constraint)
          if (idxFaceAdj != NO_ID)
            return NO_ID;
          // check if edge vertices ordering is opposite in the two faces
          // (manifold constraint)
...
void Mesh::GetAdjVertexFaces(VIndex idxVCenter, VIndex idxVAdj,
                             FaceIdxArr &indices) const {
  ASSERT(vertexFaces.size() == vertices.size());
    ...
    ProcessComponent:
      ...
      do {
        ...
        for (int i = 0; i < 3; ++i) {
          ...
          const FIndex idxFaceAdj(
              GetEdgeAdjacentFace(idxFaceCurrent, idxVert, idxVertAdj));
...
class Vertex : public vcg::Vertex<UsedTypes, vcg::vertex::Coord3f,
                                  vcg::vertex::Normal3f, vcg::vertex::VFAdj,
                                  vcg::vertex::Mark, vcg::vertex::BitFlags> {};
class Face
    : public vcg::Face<UsedTypes, vcg::face::VertexRef, vcg::face::Normal3f,
                       vcg::face::FFAdj, vcg::face::VFAdj, vcg::face::Mark,
                       vcg::face::BitFlags> {};
class Edge : public vcg::Edge<UsedTypes, vcg::edge::VertexRef, vcg::edge::Mark,
                              vcg::edge::BitFlags> {};
...
typedef vcg::SimpleTempData<Mesh::VertContainer, vcg::math::Quadric<double>>
    QuadricTemp;
...
class TriEdgeCollapse
    : public vcg::tri::TriEdgeCollapseQuadric<Mesh, VertexPair, TriEdgeCollapse,
                                              QHelper> {
public:
  typedef vcg::tri::TriEdgeCollapseQuadric<Mesh, VertexPair, TriEdgeCollapse,
                                           QHelper>
      TECQ;
  inline TriEdgeCollapse(const VertexPair &p, int i,
...
void Mesh::Clean(float fDecimate, float fSpurious, bool bRemoveSpikes,
                 unsigned nCloseHoles, unsigned nSmooth, float fEdgeLength,
                 bool bLastClean) {
  ...
  CLEAN::Mesh mesh;
  {
    CLEAN::Mesh::VertexIterator vi =
        vcg::tri::Allocator<CLEAN::Mesh>::AddVertices(mesh, vertices.size());
    ...
    CLEAN::Mesh::FaceIterator fi =
        vcg::tri::Allocator<CLEAN::Mesh>::AddFaces(mesh, faces.size());
  ...
  if (fDecimate < 1) {
    ...
    pp.QualityThr = 0.3; // Quality Threshold for penalizing bad shaped faces:
                         // the value is in the range [0..1], 0 accept any kind
                         // of face (no penalties), 0.5 penalize faces with
                         // quality < 0.5, proportionally to their shape
    pp.PreserveBoundary = false; // the simplification process tries to not
                                 // affect mesh boundaries during simplification
    pp.PreserveTopology =
        false; // avoid all collapses that cause a topology change in the mesh
               // (like closing holes, squeezing handles, etc); if checked the
               // genus of the mesh should stay unchanged
    pp.QualityWeight =
        false; // use the Per-Vertex quality as a weighting factor for the
               // simplification: the weight is used as an error amplification
               // value, so a vertex with a high quality value will not be
               // simplified and a portion of the mesh with low quality values
               // will be aggressively simplified
    pp.NormalCheck = false; // try to avoid face flipping effects and try to
                            // preserve the original orientation of the surface
    pp.OptimalPlacement =
        true; // each collapsed vertex is placed in the position minimizing the
              // quadric error; it can fail (creating bad spikes) in case of
              // very flat areas; if disabled edges are collapsed onto one of
              // the two original vertices and the final mesh is composed by a
              // subset of the original vertices
    pp.QualityQuadric = false; // add additional simplification constraints that
                               // improves the quality of the simplification of
                               // the planar portion of the mesh
    // decimate
    ...
    Util::Progress progress(_T("Decimated faces"),
    ...
    DeciSession.SetTimeBudget(
        0.1f); // this allow to update the progress bar 10 time for sec...
    ...
    progress.close();
    DEBUG_ULTIMATE("Mesh decimated: %d -> %d faces", OriginalFaceNum,
                   TargetFaceNum);
  }
  ...
  // clean mesh
  {
    ...
    for (int i = 0; i < 10; ++i) {
      vcg::tri::UpdateTopology<CLEAN::Mesh>::FaceFace(mesh);
      vcg::tri::UpdateTopology<CLEAN::Mesh>::VertexFace(mesh);
      const int nSplitNonManifoldVertices =
          vcg::tri::Clean<CLEAN::Mesh>::SplitNonManifoldVertex(mesh, 0.1f);
      DEBUG_ULTIMATE("Split %d non-manifold vertices",
                     nSplitNonManifoldVertices);
      if (nSplitNonManifoldVertices == 0)
        break;
    }
#else
    const int nNonManifoldVertices =
  ...
  if (fSpurious > 0) {
    ...
    const float thLongEdge(SQRT(edgeLens.GetNth(edgeLens.size() * 95 / 100)) *
                           fSpurious);
    const size_t numLongFaces(
        vcg::tri::UpdateSelection<CLEAN::Mesh>::FaceOutOfRangeEdge(mesh, 0,
                                                                   thLongEdge));
    for (CLEAN::Mesh::FaceIterator fi = mesh.face.begin();
         fi != mesh.face.end(); ++fi)
      if (!(*fi).IsD() && (*fi).IsS())
        vcg::tri::Allocator<CLEAN::Mesh>::DeleteFace(mesh, *fi);
    DEBUG_ULTIMATE("Removed %d faces with edges longer than %f", numLongFaces,
                   thLongEdge);
    // remove isolated components
    const float thLongSize(SQRT(edgeLens.GetNth(edgeLens.size() * 55 / 100)) *
                           fSpurious);
    vcg::tri::UpdateTopology<CLEAN::Mesh>::FaceFace(mesh);
    const std::pair<int, int> delInfo(
        vcg::tri::Clean<CLEAN::Mesh>::RemoveSmallConnectedComponentsDiameter(
            mesh, thLongSize));
    DEBUG_ULTIMATE("Removed %d connected components out of %d", delInfo.second,
                   delInfo.first);
  }
  ...
  if (bRemoveSpikes) {
    ...
    while (true) {
      ...
      vcg::tri::UpdateTopology<CLEAN::Mesh>::VertexFace(mesh);
      int nSpikes(0);
      for (CLEAN::Mesh::VertexIterator vi = mesh.vert.begin();
           vi != mesh.vert.end(); ++vi) {
        if (vi->IsD())
          continue;
      ...
      for (CLEAN::Mesh::FaceIterator fi = mesh.face.begin();
        ...
        if (!fi->IsD() &&
            (fi->V(0)->IsD() || fi->V(1)->IsD() || fi->V(2)->IsD()))
          vcg::tri::Allocator<CLEAN::Mesh>::DeleteFace(mesh, *fi);
      }
...
#if 1
    // When closing holes it tries to prevent the creation of faces that
    // intersect faces adjacent to the boundary of the hole. It is an heuristic,
    // non intersecting hole filling can be NP-complete.
    const int holeCnt(vcg::tri::Hole<CLEAN::Mesh>::EarCuttingIntersectionFill<
                      vcg::tri::SelfIntersectionEar<CLEAN::Mesh>>(
...
#endif
    DEBUG_ULTIMATE("Closed %d holes and added %d new faces", holeCnt,
                   mesh.fn - OriginalSize);
  }
...
#if 1
    vcg::tri::Smooth<CLEAN::Mesh>::VertexCoordLaplacian(mesh, (int)nSmooth,
                                                        false, false);
#else
    vcg::tri::Smooth<CLEAN::Mesh>::VertexCoordLaplacianHC(mesh, (int)nSmooth,
                                                          false);
#endif
  ...
  // clean mesh
  if (bLastClean &&
      (fSpurious > 0 || bRemoveSpikes || nCloseHoles > 0 || nSmooth > 0)) {
    const int nNonManifoldFaces =
        vcg::tri::Clean<CLEAN::Mesh>::RemoveNonManifoldFace(mesh);
    DEBUG_ULTIMATE("Removed %d non-manifold faces", nNonManifoldFaces);
#if 0 // not working
  ...
  DEBUG("Cleaned mesh: %u vertices, %u faces (%s)", vertices.size(),
        faces.size(), TD_TIMER_GET_FMT().c_str());
} // Clean
/*----------------------------------------------------------------*/

// project vertices and compute bounding-box;
// account for diferences in pixel center convention: while OpenMVS uses the
// same convention as OpenCV and DirectX 9 where the center of a pixel is
// defined at integer coordinates, i.e. the center is at (0, 0) and the top left
// corner is at (-0.5, -0.5), DirectX 10+, OpenGL, and Vulkan convention is the
// center of a pixel is defined at half coordinates, i.e. the center is at (0.5,
...
void Mesh::FaceTexcoordsNormalize(TexCoordArr &newFaceTexcoords,
                                  bool flipY) const {
  ...
  if (texturesDiffuse.empty()) {
    ...
    if (flipY) {
      FOREACH(i, faceTexcoords) {
        ...
        newFaceTexcoords[i] =
            TexCoord((texcoord.x + halfPixel.x) * invNorm.x,
...
void Mesh::FaceTexcoordsUnnormalize(TexCoordArr &newFaceTexcoords,
                                    bool flipY) const {
  ASSERT(HasTextureCoordinates());
  newFaceTexcoords.resize(faceTexcoords.size());
  if (texturesDiffuse.empty()) {
    ...
    FOREACH(i, texturesDiffuse) {
      ASSERT(!texturesDiffuse[i].empty());
      scales[i] = TexCoord((float)texturesDiffuse[i].cols,
                           (float)texturesDiffuse[i].rows);
    }
    if (flipY) {
      FOREACH(i, faceTexcoords) {
        const TexCoord &texcoord = faceTexcoords[i];
        ...
        newFaceTexcoords[i] =
            TexCoord(texcoord.x * scale.x - halfPixel.x,
...
struct Vertex {
  Mesh::Vertex v;
  Mesh::Normal n;
  static void InitLoadProps(PLY &ply, int elem_count, Mesh::VertexArr &vertices,
  ...
  static void Select(PLY &ply) { ply.put_element_setup(elem_names[0]); }
...
struct Face {
  ...
  Mesh::TexIndex texId;
  float weight;
  static void InitLoadProps(PLY &ply, int elem_count, Mesh::FaceArr &faces,
  ...
  static void Select(PLY &ply) { ply.put_element_setup(elem_names[1]); }
  static void InitSaveProps(PLY &ply, int elem_count, bool bFaces,
                            bool bTexcoord, bool bTexnumber,
...
const PLY::PlyProperty Face::props[] = {
    {"vertex_indices", PLY::Uint32, PLY::Uint32, offsetof(Face, face.pFace), 1,
     PLY::Uint8, PLY::Uint8, offsetof(Face, face.num)},
    {"texcoord", PLY::Float32, PLY::Float32, offsetof(Face, tex.pTex), 1,
     PLY::Uint8, PLY::Uint8, offsetof(Face, tex.num)},
...
bool Mesh::Load(const String &fileName) {
  ...
  DEBUG_EXTRA("Mesh '%s' loaded: %u vertices, %u faces (%s)",
              Util::getFileNameExt(fileName).c_str(), vertices.size(),
              faces.size(), TD_TIMER_GET_FMT().c_str());
  return true;
}
...
bool Mesh::LoadPLY(const String &fileName) {
  ...
  for (int i = 0; i < ply.get_elements_count(); i++) {
    ...
    if (PLY::equal_strings(BasicPLY::elem_names[0], elem_name)) {
      ...
      BasicPLY::Face::InitLoadProps(ply, elem_count, faces, faceTexcoords,
                                    faceTexindices);
      BasicPLY::Face face;
      FOREACH(f, faces) {
        ...
        if (!faceTexcoords.empty()) {
          if (face.tex.num != 6) {
            DEBUG_EXTRA("error: unsupported mesh file (texture coordinates not "
                        "per face vertex)");
            return false;
          }
          memcpy(faceTexcoords.data() + f * 3, face.tex.pTex,
                 sizeof(TexCoord) * 3);
...
bool Mesh::LoadOBJ(const String &fileName) {
  ...
  vertices.CopyOf(&model.get_vertices()[0],
                  (VIndex)model.get_vertices().size());
  ...
  if (!model.get_normals().empty()) {
    ...
    vertexNormals.CopyOf(&model.get_normals()[0],
                         (VIndex)model.get_normals().size());
  }
  ...
  FOREACH(groupIdx, model.get_groups()) {
    ...
    // store texture
    ObjModel::MaterialLib::Material *pMaterial(
        model.GetMaterial(group.material_name));
    ...
    for (const tinygltf::Primitive &gltfPrimitive : gltfMesh.primitives) {
      ...
      // read vertices
      {
        ...
        if (gltfAccessor.type != TINYGLTF_TYPE_VEC3)
          continue;
        const tinygltf::BufferView &gltfBufferView =
            gltfModel.bufferViews[gltfAccessor.bufferView];
        const tinygltf::Buffer &buffer =
            gltfModel.buffers[gltfBufferView.buffer];
        ...
        if (gltfAccessor.componentType == TINYGLTF_COMPONENT_TYPE_FLOAT) {
          ASSERT(gltfBufferView.byteLength ==
                 sizeof(Vertex) * gltfAccessor.count);
          memcpy(mesh.vertices.data(), pData, gltfBufferView.byteLength);
        } else if (gltfAccessor.componentType ==
      ...
      // read faces
      {
        ...
        if (gltfAccessor.componentType == TINYGLTF_COMPONENT_TYPE_INT ||
          ...
          ASSERT(gltfBufferView.byteLength ==
                 sizeof(uint32_t) * gltfAccessor.count);
...
bool Mesh::Save(const String &fileName, const cList<String> &comments,
                bool bBinary) const {
  if (IsEmpty())
    return false;
  TD_TIMER_STARTD();
  ...
  if (ext == _T(".obj"))
    ...
    ret = SavePLY(ext != _T(".ply") ? String(fileName + _T(".ply")) : fileName,
                  comments, bBinary);
  if (!ret)
    return false;
...
bool Mesh::SavePLY(const String &fileName, const cList<String> &comments,
                   bool bBinary, bool bTexLossless) const {
  ASSERT(!fileName.empty());
  ...
  using namespace MeshInternal;
  PLY ply;
  if (!ply.write(fileName, 2, BasicPLY::elem_names,
                 bBinary ? PLY::BINARY_LE : PLY::ASCII)) {
  ...
  if (HasTexture()) {
    FOREACH(texId, texturesDiffuse) {
      const String textureFileName(Util::getFileFullName(fileName) +
                                   std::to_string((unsigned)texId).c_str() +
                                   (bTexLossless ? _T(".png") : _T(".jpg")));
      ply.append_comment(
          (_T("TextureFile ") + Util::getFileNameExt(textureFileName)).c_str());
  ...
  BasicPLY::Vertex::InitSaveProps(ply, (int)vertices.size(),
                                  !vertexNormals.empty());
  BasicPLY::Face::InitSaveProps(ply, (int)faces.size(), !faces.empty(),
                                !faceTexcoords.empty(),
...
bool Mesh::SaveOBJ(const String &fileName) const {
  ...
  // store vertices
  STATIC_ASSERT(sizeof(ObjModel::Vertex) == sizeof(Vertex));
  model.get_vertices().insert(model.get_vertices().begin(), vertices.begin(),
                              vertices.end());
  ...
  if (!vertexNormals.empty()) {
    ...
    model.get_normals().insert(model.get_normals().begin(),
                               vertexNormals.begin(), vertexNormals.end());
  }
  ...
  do {
    ObjModel::Group &group = model.AddGroup(
        HasTexture() ? String::FormatString("material_%02u", idxTexture)
                     : String(""));
    group.faces.reserve(texturesDiffuse.empty()
                            ? faces.size()
                            : faces.size() / texturesDiffuse.size());
    FOREACH(idxFace, faces) {
...
template <typename T>
void ExtendBufferGLTF(const T *src, size_t size, tinygltf::Buffer &dst,
...
bool Mesh::SaveGLTF(const String &fileName, bool bBinary) const {
  ...
  for (size_t meshId = 0; meshId < meshes.size(); meshId++) {
    ...
    // setup vertices
    {
      ...
      vertexPositionAccessor.minValues = {box.ptMin.x(), box.ptMin.y(),
                                          box.ptMin.z()};
      vertexPositionAccessor.maxValues = {box.ptMax.x(), box.ptMax.y(),
                                          box.ptMax.z()};
    ...
    // setup faces
    {
      STATIC_ASSERT(3 * sizeof(Face::Type) ==
                    sizeof(Face)); // FaceArr should be continuous
    ...
    if (mesh.HasTexture()) {
      ...
      ExtendBufferGLTF(normFaceTexcoords.data(), normFaceTexcoords.size(),
                       gltfBuffer, vertexTexcoordBufferView.byteOffset,
      ...
      image.name = Util::getFileFullName(fileName) + "_" +
                   std::to_string(meshId).c_str();
      ...
      mesh.texturesDiffuse[0].copyTo(
          cv::Mat(mesh.texturesDiffuse[0].size(), CV_8UC3, image.image.data()));
  ...
  struct Tools {
    static bool WriteImageData(const std::string *basepath,
                               ...
                               tinygltf::Image *image, bool embedImages,
                               void *) {
      ASSERT(!embedImages);
  ...
  return gltf.WriteGltfSceneToFile(&gltfModel, fileName, bEmbedImages,
                                   bEmbedBuffers, bPrettyPrint, bBinary);
} // Save
/*----------------------------------------------------------------*/

bool Mesh::Save(const FacesChunkArr &chunks, const String &fileName,
                const cList<String> &comments, bool bBinary) const {
  if (chunks.size() < 2)
    return Save(fileName, comments, bBinary);
  FOREACH(i, chunks) {
    const Mesh mesh(SubMesh(chunks[i].faces));
    if (!mesh.Save(Util::insertBeforeFileExt(
                       fileName, String::FormatString("_chunk%02u", i)),
                   comments, bBinary))
      return false;
  }
  return true;
...
bool Mesh::Save(const VertexArr &vertices, const String &fileName,
                bool bBinary) {
  ASSERT(!fileName.empty());
  Util::ensureFolder(fileName);
  ...
  using namespace MeshInternal;
  PLY ply;
  if (!ply.write(fileName, 1, BasicPLY::elem_names,
                 bBinary ? PLY::BINARY_LE : PLY::ASCII)) {
    DEBUG_EXTRA("error: can not create the mesh file");
    return false;
  }
...
#define edge_size(h)                                                           \
  v_norm((h)->vertex()->point() - (h)->next()->next()->vertex()->point())
...
public:
  ...
  char removal_status; // for self intersection removal: U -  unvisited; 'P' -
                       // partially valid; V - valid for connected components: U
                       // - unvisited; V - visited
  ...
  inline bool isTrinagle() const {
    return this->halfedge()->vertex() ==
           this->halfedge()->next()->next()->next()->vertex();
  }
  ...
  inline Normal normal() const {
    return CGAL::cross_product(get_point(1) - get_point(0),
                               get_point(2) - get_point(0));
  }

  inline double area() const { return v_norm(normal()) / 2; }
};
...
static void ComputeStatsEdge(const Polyhedron &p, Stats &stats) {
  ...
  for (Edge_const_iterator ei = p.edges_begin(); ei != p.edges_end(); ei++) {
    const double tmpEdge(
        v_norm(ei->vertex()->point() - ei->prev()->vertex()->point()));
...
static void ComputeStatsLaplacian(const Polyhedron &p, Stats &stats) {
  ...
  stats.min = FLT_MAX;
  stats.max = 0;
  for (Vertex_const_iterator vi = p.vertices_begin(); vi != p.vertices_end();
       vi++) {
...
#define REPLACE_POINT(V, P1, P2, PMIDDLE)                                      \
  (((V == P1) || (V == P2)) ? (PMIDDLE) : (V))
...
// test weather when performing the edge collapse we change the signed area of
// any triangle
...
static bool CanFlipEdge(Vertex::Halfedge_handle h) {
  ...
  if ((h->next() == null_h) || (h->prev() == null_h) ||
      (h->opposite() == null_h) || (h->opposite()->next() == null_h))
    return false;
  ...
  Vector v_perp_1 =
      CGAL::cross_product(h->vertex()->point() - h->next()->vertex()->point(),
  ...
  Vector v_perp_2 =
      CGAL::cross_product(h->opposite()->next()->vertex()->point() -
                              h->opposite()->vertex()->point(),
                          h->vertex()->point() - h->prev()->vertex()->point());
  if (v_angle(v_perp_1, v_perp_2) > D2R(20))
    return false;

  return (h->next()->opposite()->facet() !=
...
static void SplitEdge(Polyhedron &p, Vertex::Halfedge_handle h, int mode = 1) {
  ...
  if (mode == 1) { // middle
    ...
    const double ratio(v_norm(p3 - p2) * cos(OppositeAngle(h->next())) /
                       v_norm(p1 - p2));
    ...
    for (Vertex_iterator vi = p.vertices_begin(); vi != p.vertices_end();) {
      ...
      if (degree == 4) {
        ...
        while (edge_stats != current_edge_stats) {
          // std::cout << "current edge stats:" << current_edge_stats <<
          // std::endl;
          c++;
          current_edge_stats = (float)edge_size(c);
        }
        ...
        CGAL_For_all(c, d) {
          if (CanCollapseEdge(c->opposite())) {
            // if ((c->opposite()->vertex()==vi) && (vi!=p.vertices_end()))
            // vi++;
  ...
  case 3: {
    ...
    do {
      ...
      for (Edge_iterator ei = p.edges_begin(); ei != p.edges_end(); ++ei) {
        ...
        float cost1((float)MINF(
            MINF(MINF(MINF(MINF(p_angle(p3, p1, p2), p_angle(p1, p3, p2)),
                           p_angle(p4, p1, p3)),
                      p_angle(p4, p3, p1)),
                 p_angle(p1, p4, p2)),
            p_angle(p1, p2, p3)));
        float cost2((float)MINF(
            MINF(MINF(MINF(MINF(p_angle(p1, p2, p4), p_angle(p1, p4, p2)),
                           p_angle(p3, p4, p2)),
                      p_angle(p4, p2, p3)),
                 p_angle(p4, p1, p2)),
            p_angle(p4, p3, p2)));
...
// Description:
//  It iterates through all the mesh vertices and it tries to fix degenerate
//  triangles. There are conditions that check for large and small angles.
// Parameters:
//  - degenerateAngleDeg
//     - for large angles: if an angle is bigger than degenerateAngleDeg.
//     - a good values to use is typically 170
//  - collapseRatio
//     - for small angles: given the corresponding edges (a,b,c) in all
//     permutations, if (a/b < collapseRatio) & (a/c < collapseRatio)
//     - a good value to use is 0.1
static int FixDegeneracy(Polyhedron &p, double collapseRatio,
                         double degenerateAngleDeg) {
  DEBUG_LEVEL(3, "Fix degeneracy: %g collapse-ratio, %g degenerate-angle",
              collapseRatio, degenerateAngleDeg);
  double edge[3];
  ...
  for (Facet_iterator fi = p.facets_begin(); fi != p.facets_end();) {
    ...
    if (fi->triangle().is_degenerate()) {
      ...
      fi->halfedge()->vertex()->point() =
          fi->halfedge()->vertex()->point() + Vector(0, 0, delta);
      fi->halfedge()->next()->vertex()->point() =
          fi->halfedge()->next()->vertex()->point() + Vector(0, delta, 0);
    ...
    for (i = 0; i < 3; i++) {
      if ((edge[i] / edge[(i + 1) % 3] < collapseRatio) &&
        ...
        if (CanCollapseEdge(edge_h[i])) {
          while ((fi != p.facets_end()) &&
                 (fi == edge_h[i]->opposite()->facet()))
            fi++;
...
static void FixAllDegeneracy(Polyhedron &p, double collapseRatio,
                             double degenerateAngleDeg) {
  int runs(0);
  do {
...
static void ComputeConnectedComponents(
    Polyhedron &p, std::vector<MeshConnectedComponent> &connected_components) {
    ...
    if (i->removal_status == 'U') { // start a new component
      ...
      while (!facet_queue.empty()) { // fill the current component
        ...
        do {
          ...
          if ((opposite_f != Vertex::Facet_handle()) &&
              (opposite_f->removal_status == 'U')) {
...
static void Smooth(Polyhedron &p, double delta, int mode = 0) {
  ...
  for (Vertex_iterator vi = p.vertices_begin(); vi != p.vertices_end(); vi++) {
    ...
    case 0:
      displacement =
          vi->laplacian * delta /*vert->getMeanCurvatureFlow().Norm()*/;
      break;
    case 1:
      displacement =
          ComputeVectorComponent(vi->normal, vi->laplacian, 0) * delta;
      break;
    case 2:
      displacement =
          ComputeVectorComponent(vi->normal, vi->laplacian, 1) * delta;
      break;
    case 3:
    ...
    case 4:
      ...
      break;
    case 5: {
      Vector d_tan(ComputeVectorComponent(vi->normal, vi->laplacian, 0) *
                   delta);
      Vector d_norm(ComputeVectorComponent(vi->normal, vi->laplacian, 1) *
                    delta);
      displacement =
          (v_norm(d_tan) > 2 * v_norm(d_norm) ? d_tan : Vector(0, 0, 0));
    } break;
    case 6:
...
// Description:
// - The goal of this method is to ensure that all the edges of the mesh are
// within the interval [epsilonMin,epsilonMax].
...
// FixDegeneracy(collapseRatio,degenerate_angle_deg) and performs some local
// smoothing, based on the operating mode. Parameters:
// - [epsilonMin, epsilonMax] - the desired edge interval (negative if to be
// used as multiplier of the initial mean edge length)
// - collapseRatio, degenerate_angle_deg - parameters used to invoke
// FixDegeneracy (see function for more details)
// - mode : 0 - fixDegeneracy=No  smoothing=Yes;
//          1 - fixDegeneracy=Yes smoothing=Yes; (default)
//         10 - fixDegeneracy=Yes smoothing=No;
// - max_iter (default=30) - maximum number of iterations to be performed; since
// there is no guarantee that one operations (such as a collapse, for example)
//   will not in turn generate new degeneracies, operations are being performed
//   on the mesh in an iterative fashion.
static void EnsureEdgeSize(Polyhedron &p, double epsilonMin, double epsilonMax,
                           double collapseRatio, double degenerate_angle_deg,
                           int mode, int max_iters, int comp_size_threshold) {
  if (mode > 0)
  ...
  DEBUG_LEVEL(3,
              ...
              epsilonMin, epsilonMax, edge.min, edge.max);
  ...
  while ((edge.min < epsilonMin || edge.max > epsilonMax) && (no_ops > 0) &&
         (++iters < max_iters)) {
    no_ops = 0;
    if (iters > 1)
      ComputeStatsEdge(p, edge);
  ...
  if (VERBOSITY_LEVEL > 2) {
    ...
    VERBOSE("Edge size in [%g, %g] (requested in [%g, %g]): %d ops, %d iters",
            edge.min, edge.max, epsilonMin, epsilonMax, total_no_ops, iters);
  }
#endif
}
...
static void ComputeVertexNormals(Polyhedron &p) {
  ...
  for (Vertex_iterator vi = p.vertices_begin(); vi != p.vertices_end(); vi++) {
    ...
    if (nrm != 0)
      normal = normal / nrm;
#else
    // only temporarily set here, it will be set in normal when computing the
    // robust measure
...
#if ROBUST_NORMALS > 0
static std::vector<std::pair<Vertex *, int>>
GetRingNeighbourhood(Vertex &v, int ring_size, bool include_original) {
...
static void ComputeVertexRobustNormal(Vertex &v, int maxRings) {
  std::vector<std::pair<Vertex *, int>> neighs(
      GetRingNeighbourhood(v, maxRings, true));
  Vector &normal(v.normal);
  normal = CGAL::NULL_VECTOR;
  for (std::vector<std::pair<Vertex *, int>>::const_iterator n_it =
           neighs.cbegin();
       n_it != neighs.cend(); ++n_it) {
...
static void ComputeVertexLaplacian(Vertex &v) {
  // formula taken from "Mesh Smoothing via Mean and Median Filtering Applied to
  // Face Normals"
  ...
  do {
    ...
    result_laplacian =
        result_laplacian + (vi->prev()->vertex()->point() - CGAL::ORIGIN);
  } while (++vi != v.vertex_begin());
  result_laplacian =
      result_laplacian / (double)order - (v.point() - CGAL::ORIGIN);
#else
  float w_total(0);
...
static void ComputeVertexLaplacianDeriv(Vertex &v) {
  ...
  do {
    ++order;
    v.laplacian_deriv =
        v.laplacian_deriv + (vi->prev()->vertex()->laplacian - v.laplacian);
  } while (++vi != v.vertex_begin());
...
static void UpdateMeshData(Polyhedron &p) {
  ...
  // set border edges
  for (Halfedge_iterator hi = p.border_halfedges_begin();
       hi != p.halfedges_end(); hi++)
    hi->vertex()->setBorder();
}
...
#if TD_VERBOSE != TD_VERBOSE_OFF
    if (bProblems)
      DEBUG_EXTRA("warning: ignoring the following facet(s) violating the "
                  "manifold constraint:%s",
                  msgFaces.c_str());
#endif
...
static bool ImportMesh(Polyhedron &p, const Mesh::VertexArr &vertices,
                       const Mesh::FaceArr &faces) {
...
static bool ExportMesh(const Polyhedron &p, Mesh::VertexArr &vertices,
                       Mesh::FaceArr &faces) {
  ...
  CGAL::Inverse_index<Polyhedron::Vertex_const_iterator> index(
      p.vertices_begin(), p.vertices_end());
  for (Polyhedron::Face_const_iterator it = p.facets_begin(),
                                       ite = p.facets_end();
       it != ite; ++it) {
    ...
    for (int i = 0; i < 3; ++i, ++hc)
      facet[i] =
          (Mesh::FIndex)index[Polyhedron::Vertex_const_iterator(hc->vertex())];
#endif
  }
...
void Mesh::EnsureEdgeSize(float epsilonMin, float epsilonMax,
                          float collapseRatio, float degenerate_angle_deg,
                          int mode, int max_iters) {
  CLN::Polyhedron p;
  CLN::ImportMesh(p, vertices, faces);
  Release();
  CLN::EnsureEdgeSize(p, epsilonMin, epsilonMax, collapseRatio,
                      degenerate_angle_deg, mode, max_iters, 0);
...
void Mesh::Subdivide(const AreaArr &maxAreas, uint32_t maxArea) {
  ...
  // (each new vertex index corresponds to the edge opposed to the existing
  // vertex index)
  struct SplitFace {
    VIndex idxVert[3];
    bool bSplit;
  ...
  FOREACH(f, maxAreas) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      vertices.emplace_back(
          (vertices[face[(i + 1) % 3]] + vertices[face[(i + 2) % 3]]) * 0.5f);
    }
  ...
  ASSERT(faces.size() - (faces.capacity() / 3) /*initial size*/ >
         mapSplits.size());
  for (const auto &s : mapSplits)
...
void Mesh::Decimate(VertexIdxArr &verticesRemove) {
  ...
  FOREACHPTR(pIdxV, verticesRemove) {
    ...
    FOREACHPTR(pHole, holes) {
      ...
      ASSERT(verts[(idxL + verts.size() - 1) % verts.size()] ==
             hole[(idxS + 1) % hole.size()]);
...
void Mesh::CloseHoleQuality(VertexIdxArr &verts) {
  struct CandidateFace {
    ...
    CandidateFace() {}
    // the vertices of the given face must be in the order they appear on the
    // border of the hole (the middle face vertex must be between the first and
    // third on the border)
    CandidateFace(VIndex v0, VIndex v1, VIndex v2, const Mesh &mesh)
        : face(v0, v1, v2) {
      ...
      // compute quality as a composition of dihedral angle and
      // area/sum(edge^2); the dihedral angle uses the normal of the edge faces
      ...
      aspectRatio =
          ComputeTriangleQuality(mesh.vertices[face[0]], mesh.vertices[face[1]],
    ...
    // so if we need the ear with minimal dihedral angle, we must reverse the
    // sign of the comparison. The concave elements must be all in the end of
    // the heap, sorted accordingly, So if only one of the two ear is Concave
    // that one is always the minimum one.
  ...
  FOREACH(v, verts) {
    if (candidateFaces
            .emplace_back(verts[v], verts[(v + 1) % verts.size()],
                          verts[(v + 2) % verts.size()], *this)
            .aspectRatio < 0)
...
// remove degenerate faces, with one or more identical vertices or very close
// vertices (0 - disabled); unreferenced vertices and non-manifold
// edges/vertices can be created, so should be followed by
// RemoveUnreferencedVertices() and FixNonManifold()
Mesh::FIndex Mesh::RemoveDegenerateFaces(Type thArea) {
  ...
  const FIndex numRemovedFaces =
      facesRemove.size() + RemoveDegenerateFaces(0.f);
...
// so iterate till no zero-area faces are encountered or max number of
...
// crop mesh such that none of its faces is touching or outside the given
// bounding-box
...
// return the number of removed vertices, and optionally the list of duplicated
// vertices
Mesh::VIndex Mesh::RemoveDuplicatedVertices(VertexIdxArr *duplicatedVertices) {
  ...
  VertexIdxArr mapVertices(vertices.size());
  {
    ...
    FOREACH(i, vertices) {
      ...
      const auto ret =
          mapVertexIndex.emplace(vertex, (VIndex)mapVertexIndex.size());
      if (ret.second) {
        // new vertex found
        mapVertices[i] = i;
      } else {
...
// convert textured mesh to store texture coordinates per vertex instead of per
// face
...
//  - sampleMesh: uniformly samples points on the mesh (0 - disabled, <0 -
//  number of points, >0 - sample density per square unit)
//  - planeThreshold: threshold used to estimate the ground plane (0 - auto)
Planef Mesh::EstimateGroundPlane(const ImageArr &images, float sampleMesh,
  ...
  return pointcloud.EstimateGroundPlane(images, planeThreshold,
                                        fileExportPlane);
}
/*----------------------------------------------------------------*/
...
Mesh::Type Mesh::ComputeArea(FIndex idxFace) const {
  const Face &face = faces[idxFace];
  return ComputeTriangleArea(vertices[face[0]], vertices[face[1]],
                             vertices[face[2]]);
}

// computes the area of the mesh surface as the sum of the signed areas of its
// faces
REAL Mesh::ComputeArea() const {
  REAL area(0);
  for (const Face &face : faces)
    area += ComputeTriangleArea(vertices[face[0]], vertices[face[1]],
                                vertices[face[2]]);
  return area;
}
...
REAL Mesh::ComputeVolume() const {
  REAL volume(0);
  for (const Face &face : faces)
    volume += ComputeTriangleVolume(vertices[face[0]], vertices[face[1]],
                                    vertices[face[2]]);
  return volume;
}
/*----------------------------------------------------------------*/
...
void Mesh::SamplePoints(REAL samplingDensity, PointCloud &pointcloud) const {
  ...
  const REAL area(ComputeArea());
  const unsigned theoreticNumberOfPoints(
      CEIL2INT<unsigned>(area * samplingDensity));
  return SamplePoints(samplingDensity, theoreticNumberOfPoints, pointcloud);
}
void Mesh::SamplePoints(REAL samplingDensity, unsigned mumPointsTheoretic,
  ...
  FOREACH(idxFace, faces) {
    ...
    for (unsigned i = 0; i < pointsToAdd; ++i) {
      ...
      // in A. S. Glassner, editor, Graphics Gems, pages 24-28. Academic Press,
      // 1990
      REAL x(dist(rnd));
      REAL y(dist(rnd));
...
void Mesh::Project(const Camera &camera, DepthMap &depthMap) const {
  struct RasterMesh : TRasterMesh<RasterMesh> {
    ...
    RasterMesh(const VertexArr &_vertices, const Camera &_camera,
               DepthMap &_depthMap)
...
void Mesh::Project(const Camera &camera, DepthMap &depthMap,
                   Image8U3 &image) const {
  ...
  struct RasterMesh : TRasterMesh<RasterMesh> {
    ...
    FIndex idxFaceTex;
    TexCoord xt;
    RasterMesh(const Mesh &_mesh, const Camera &_camera, DepthMap &_depthMap,
               Image8U3 &_image)
...
// project mesh to the given camera plane, computing also the normal-map (in
// camera space)
void Mesh::Project(const Camera &camera, DepthMap &depthMap,
                   NormalMap &normalMap) const {
  ASSERT(vertexNormals.size() == vertices.size());
  struct RasterMesh : TRasterMesh<RasterMesh> {
    typedef TRasterMesh<RasterMesh> Base;
    const Mesh &mesh;
    ...
    RasterMesh(const Mesh &_mesh, const Camera &_camera, DepthMap &_depthMap,
        ...
        : Base(_mesh.vertices, _camera, _depthMap), mesh(_mesh),
          normalMap(_normalMap), R(camera.R) {}
    inline void Clear() {
      Base::Clear();
      normalMap.memset(0);
    }
    ...
    void Raster(const ImageRef &pt, const Triangle &t, const Point3f &bary) {
      ...
      if (depth == Depth(0) || depth > z) {
        depth = z;
        normalMap(pt) =
            R * normalized(mesh.vertexNormals[idxVerts[0]] * pbary[0] +
  ...
  struct RasterMesh : TRasterMesh<RasterMesh> {
    ...
    inline bool ProjectVertex(const Mesh::Vertex &pt, int v, Triangle &t) {
      return (t.ptc[v] = camera.TransformPointW2C(Cast<REAL>(pt))).z > 0 &&
             depthMap.isInsideWithBorder<float, 3>(
                 t.pti[v] = camera.TransformPointOrthoC2I(t.ptc[v]));
    }
    void Raster(const ImageRef &pt, const Triangle &t, const Point3f &bary) {
...
void Mesh::ProjectOrtho(const Camera &camera, DepthMap &depthMap,
                        Image8U3 &image) const {
  ASSERT(!faceTexcoords.empty() && !texturesDiffuse.empty());
  struct RasterMesh : TRasterMesh<RasterMesh> {
    typedef TRasterMesh<RasterMesh> Base;
...
// assuming the mesh is properly oriented, ortho-project it to a camera looking
// from top to down
void Mesh::ProjectOrthoTopDown(unsigned resolution, Image8U3 &image,
                               Image8U &mask, Point3 &center) const {
  ...
  center = camera.TransformPointI2W(
      Point3(xCenter.x, xCenter.y,
...
bool Mesh::Split(FacesChunkArr &chunks, float maxArea) {
  ...
  struct AreaInserter {
    ...
    float area;
    inline void operator()(const Octree::IDX_TYPE *indices,
                           Octree::SIZE_TYPE size) {
  ...
  struct ChunkInserter {
    ...
    void operator()(const Octree::CELL_TYPE &parentCell,
                    Octree::Type parentRadius, const UnsignedArr &children) {
  ...
  if (chunks.size() < 2)
    return false;
  DEBUG_EXTRA("Mesh split (%g max-area): %u chunks (%s)", maxArea,
              chunks.size(), TD_TIMER_GET_FMT().c_str());
  return true;
} // Split
/*----------------------------------------------------------------*/
...
#endif
bool Mesh::TransferTexture(Mesh &mesh, const FaceIdxArr &faceSubsetIndices,
                           unsigned borderSize, unsigned textureSize) {
  ASSERT(HasTexture() && mesh.HasTextureCoordinates());
  if (mesh.texturesDiffuse.empty()) {
    // create the texture at specified resolution and
    // scale the UV-coordinates to the new resolution (assuming normalized
    // coordinates)
  ...
  const FIndex num_faces(faceSubsetIndices.empty() ? mesh.faces.size()
                                                   : faceSubsetIndices.size());
...
#pragma omp parallel for schedule(dynamic)
    ...
    FOREACHRAW(idx, num_faces) {
#endif
      const FIndex idxFace(faceSubsetIndices.empty() ? idx
                                                     : faceSubsetIndices[idx]);
      ...
      Image8U::RasterizeTriangleBary<TexCoord::Type, RasterTriangle, false>(
          tri[0], tri[1], tri[2], data);
    }
  } else {
    // the two meshes are different, transfer the texture by finding the closest
    // point on the two surfaces
    if (vertexFaces.size() != vertices.size())
      ListIncidentFaces();
    ...
    struct OctreeIntersectRayMesh : IntersectRayMesh {
      OctreeIntersectRayMesh(const Octree &octree, const Mesh &_mesh,
                             const Ray3f &_ray)
...
#pragma omp parallel for schedule(dynamic)
      ...
      struct RasterTriangle {
        ...
        const OctreeIntersectRayMesh intRay(octree, meshRef, ray);
          ...
          if (intRay.pick.IsValid()) {
            ...
            const Vertex baryRef(CorrectBarycentricCoordinates(
                BarycentricCoordinatesUV(meshRef.vertices[refFace[0]],
                                         meshRef.vertices[refFace[1]],
                                         meshRef.vertices[refFace[2]], refX)));
            const TexCoord *tri = meshRef.faceTexcoords.data() + refIdxFace * 3;
            const TexCoord x(tri[0] * baryRef.x + tri[1] * baryRef.y +
                             tri[2] * baryRef.z);
            const Pixel8U color(meshRef.texturesDiffuse[texId].sample(x));
            ...
            mask(pt) = 0;
             ...
             mesh.GetFaceTextureIndex(idxFace)};
#elif USE_MESH_INT == USE_MESH_BVH
      } data{tree,
             *this,
             mesh,
  ...
  if (borderSize > 0) {
    ...
    cv::distanceTransform(mask, dists, labels, cv::DIST_L1, 3,
                          cv::DIST_LABEL_PIXEL);
...
bool Mesh::InitKernels(int device) {
  ...
  if (!kernelComputeFaceNormal.IsValid()) {
    // kernel used to compute face normal, given the array of face vertices and
    // vertex positions
    STATIC_ASSERT(sizeof(Vertex) == sizeof(float) * 3);
    STATIC_ASSERT(sizeof(Face) == sizeof(VIndex) * 3 &&
                  sizeof(VIndex) == sizeof(uint32_t));
...
#define FUNC "ComputeFaceNormal"
    LPCSTR const szKernel =
        ...
        "\n"
        ".visible .entry " FUNC "(\n"
        "	.param .u64 .ptr param_1, // array vertices (float*3 * "
        "numVertices)\n"
        "	.param .u64 .ptr param_2, // array faces (uint32_t*3 * "
        "numFaces)\n"
        "	.param .u64 .ptr param_3, // array normals (float*3 * "
        "numFaces) [out]\n"
...
#ifdef _USE_OPENMP
// test mesh projection on the image using multi-threaded and single-threaded
// rasterization
bool MVS::TestMeshProjectionMT(const Mesh &mesh, const Image &image) {
  ...
  struct RasterMesh : TRasterMesh<RasterMesh> {
    typedef TRasterMesh<RasterMesh> Base;
    FaceMap &faceMap;
    RasterMesh(const Mesh::VertexArr &_vertices, const Camera &_camera,
               DepthMap &_depthMap, FaceMap &_faceMap)
  ...
  struct TriangleRasterizer {
    ...
    RasterMesh::Triangle triangle;
    Mesh::FIndex idxFace;
    inline cv::Size Size() const { return rasterizer->Size(); }
...
#pragma omp parallel for firstprivate(triangleRasterizer) schedule(dynamic)
  ...
  VERBOSE("Mesh rasterization: %u different depths, %u different faces",
          numDiffDepths, numDiffFaces);
