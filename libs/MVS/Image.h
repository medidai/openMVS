...
public:
  ...
  uint32_t ID; // global ID of the image (the ID given outside the current scene
               // - ex. the index in the full list of image files)
  ...
  ViewScoreArr
      neighbors;  // scored neighbor images (image indices ordered by score)
...
public:
  ...
  inline String GetMaskFileName() const {
    return maskName.empty() ? Util::getFileFullName(name) + ".mask.png"
                            : maskName;
  }
  ...
  unsigned RecomputeMaxResolution(unsigned &level, unsigned minImageSize,
                                  unsigned maxImageSize = INT_MAX) const;

  Image GetImage(const PlatformArr &platforms, double scale,
                 bool bUseImage = true) const;
  Camera GetCamera(const PlatformArr &platforms,
                   const Image8U::Size &resolution) const;
  void UpdateCamera(const PlatformArr &platforms);
  REAL ComputeFOV(int dir = 0) const;

  static bool StereoRectifyImages(const Image &image1, const Image &image2,
  ...
  static float Disparity2Depth(const Matrix4x4 &Q, const Point2f &u, float d,
                               Point2f &pt);
  static float Disparity2Distance(const Matrix3x3 &K, const Matrix4x4 &Q,
                                  const Point2f &u, float d);
  static bool Depth2Disparity(const Matrix4x4 &Q, const Point2f &u, float d,
                              float &disparity);
  ...
  void save(Archive &ar, const unsigned int /*version*/) const {
    ...
    ar & relName;
    const String relMaskName(
        maskName.empty() ? String()
  ...
  void load(Archive &ar, const unsigned int /*version*/) {
    ...
    ar & maskName;
    maskName = maskName.empty() ? String()
                                : MAKE_PATH_FULL(WORKING_FOLDER_FULL, maskName);
    ar & width & height;
    ar & neighbors;
    ar & avgDepth;
  }
