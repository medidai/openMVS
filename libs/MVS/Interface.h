...
#define MVSI_PROJECT_VER                                                       \
  ((uint32_t)7) // identifies the version of a project stream
...
// simple cv::Matx
template <typename Type, int m, int n> class Matx {
public:
  typedef Type value_type;
  ...
  template <int k> Matx<Type, m, k> operator*(const Matx<Type, n, k> &M) const {
    Matx<Type, m, k> R;
    for (int r = 0; r < m; r++) {
      for (int l = 0; l < k; l++) {
        R(r, l) = Type(0);
...
struct ArchiveSave {
  ...
  template <typename _Tp> ArchiveSave &operator&(const _Tp &obj);
};
struct ArchiveLoad {
  std::istream &stream;
  uint32_t version;
  ArchiveLoad(std::istream &_stream, uint32_t _version)
      : stream(_stream), version(_version) {}
  template <typename _Tp> ArchiveLoad &operator&(_Tp &obj);
};

template <typename _Tp> bool Save(ArchiveSave &a, const _Tp &obj) {
  const_cast<_Tp &>(obj).serialize(a, a.version);
  return true;
}
template <typename _Tp> bool Load(ArchiveLoad &a, _Tp &obj) {
  obj.serialize(a, a.version);
  return true;
}

template <typename _Tp> ArchiveSave &ArchiveSave::operator&(const _Tp &obj) {
  Save(*this, obj);
  return *this;
}
template <typename _Tp> ArchiveLoad &ArchiveLoad::operator&(_Tp &obj) {
  Load(*this, obj);
  return *this;
}
...
#define ARCHIVE_DEFINE_TYPE(TYPE)                                              \
  template <> inline bool Save<TYPE>(ArchiveSave & a, const TYPE &v) {         \
    a.stream.write((const char *)&v, sizeof(TYPE));                            \
    return true;                                                               \
  }                                                                            \
  template <> inline bool Load<TYPE>(ArchiveLoad & a, TYPE & v) {              \
...
template <typename _Tp> inline bool Load(ArchiveLoad &a, cv::Point3_<_Tp> &pt) {
...
template <> inline bool Load<std::string>(ArchiveLoad &a, std::string &s) {
  uint64_t size;
  Load(a, size);
  if (size > 0) {
    s.resize(size);
...
template <typename _Tp> inline bool Load(ArchiveLoad &a, std::vector<_Tp> &v) {
  uint64_t size;
  Load(a, size);
  if (size > 0) {
    v.resize(size);
...
struct Interface {
  ...
  struct Platform {
    ...
    struct Camera {
      ...
      std::string bandName;   // camera's band name, ex: RGB, BLUE, GREEN, RED,
                              // NIR, THERMAL, etc (optional)
      ...
      Mat33d K; // camera's intrinsics matrix (normalized if image resolution
                // not specified), where integer coordinates is by convention
                // the pixel center
      ...
      static uint32_t GetNormalizationScale(uint32_t width, uint32_t height) {
        return std::max(width, height);
      }
      uint32_t GetNormalizationScale() const {
        return GetNormalizationScale(width, height);
      }

      // project point: camera to image (homogeneous) coordinates
      inline Pos3d operator*(const Pos3d &X) const {
        return Pos3d(K(0, 2) + K(0, 0) * X.x / X.z,
                     K(1, 2) + K(1, 1) * X.y / X.z, 1.0);
      }
      // back-project point: image (z is the depth) to camera coordinates
      inline Pos3d operator/(const Pos3d &x) const {
        return Pos3d((x.x - K(0, 2)) * x.z / K(0, 0),
                     (x.y - K(1, 2)) * x.z / K(1, 1), 1.0);
      }
    ...
    struct Pose {
      Mat33d R; // platform's rotation matrix that rotates a point from world to
                // camera coordinate system
      Pos3d C;  // platform's translation vector (position) in world coordinate
                // system
      ...
      inline Pos3d operator*(const Pos3d &X) const { return R * (X - C); }
      // back-project point: local to world coordinates
      inline Pos3d operator/(const Pos3d &X) const { return R.t() * X + C; }
    ...
    const Mat33d &SetFullK(uint32_t cameraID, const Mat33d &K, uint32_t width,
                           uint32_t height, bool normalize = false) {
      Camera &camera = cameras[cameraID];
      if (normalize) {
        camera.width = camera.height = 0;
        camera.K = ScaleK(
            K, 1.0 / (double)Camera::GetNormalizationScale(width, height));
      } else {
        camera.width = width;
    ...
    Mat33d GetFullK(uint32_t cameraID, uint32_t width, uint32_t height) const {
      ...
      if (!camera.IsNormalized() && camera.width == width &&
          camera.height == height)
        return camera.K;
      return ScaleK(camera.K,
                    (double)Camera::GetNormalizationScale(width, height) /
                        (camera.IsNormalized()
                             ? 1.0
                             : (double)camera.GetNormalizationScale()));
    }
  ...
  struct Image {
    // structure describing how an other image relates to this image in terms of
    // overlap, i.e. how many 3D points are shared between the two images,
    // base-line and common area, useful for ex. when selecting the best images
    // to densly match with
    struct ViewScore {
      ...
      float score; // aggregated image score relative to the reference image
                   // (larger is better)
    ...
    uint32_t ID; // image global-ID, ex. the ID given outside the current scene,
                 // like the index in the full list of image files (optional)
    ...
    std::vector<ViewScore>
        viewScores; // list of view scores for this image (optional)

    Image()
        : platformID(NO_ID), cameraID(NO_ID), poseID(NO_ID), ID(NO_ID),
          minDepth(0), avgDepth(0), maxDepth(0) {}
  ...
  struct OBB {
    ...
    bool IsValid() const {
      return ptMin.x < ptMax.x && ptMin.y < ptMax.y && ptMin.z < ptMax.z;
    }
  ...
  NormalArr
      verticesNormal;     // array of reconstructed 3D points' normal (optional)
  ...
  Mat44d transform; // transformation used to convert from absolute to relative
  ...
  void Transform(const Mat33d &rotation, const Pos3d &translation,
                 const double scale) {
...
// see MVS::ExportDepthDataRaw() and MVS::ImportDepthDataRaw() for usage
// example:
//  - image-resolution at which the depth-map was estimated
//  - depth-map-resolution, for now only the same resolution as the image is
//  supported
//  - min/max-depth of the values in the depth-map
//  - image-file-name is the path to the reference color image
//  - image-IDs are the reference view ID and neighbor view IDs used to estimate
//  the depth-map (global ID)
//  - camera/rotation/position matrices (row-major) is the absolute pose
//  corresponding to the reference view
//  - depth-map: the pixels' depth
//  - normal-map (optional): the 3D point normal in camera space; same
//  resolution as the depth-map
//  - confidence-map (optional): the 3D point confidence (usually a value in
//  [0,1]); same resolution as the depth-map
//  - views-map (optional): the pixels' views, indexing image-IDs starting after
//  first view (up to 4); same resolution as the depth-map
struct HeaderDepthDataRaw {
  ...
  // image file name length followed by the characters: uint16_t nFileNameSize;
  // char* FileName number of view IDs followed by view ID and neighbor view
  // IDs: uint32_t nIDs; uint32_t* IDs camera, rotation and position matrices
  // (row-major) at image resolution: double K[3][3], R[3][3], C[3] depth,
  // normal, confidence maps: float depthMap[height][width],
  // normalMap[height][width][3], confMap[height][width]
  inline HeaderDepthDataRaw() : name(0), type(0), padding(0) {}
  static uint16_t HeaderDepthDataRawName() {
    return *reinterpret_cast<const uint16_t *>("DR");
  }
};
/*----------------------------------------------------------------*/
