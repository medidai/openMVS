...
DMapCache::DMapCache(DepthDataArr &_arrDepthData, unsigned _loadFlags,
...
bool DMapCache::UseImage(IIndex idxImage) const {
  ...
  const String fileName(
      ComposeDepthFilePath(arrDepthData[idxImage].GetView().GetID(), "dmap"));
  while (!std::filesystem::is_regular_file(
      static_cast<const std::string &>(fileName)))
...
bool DMapCache::EjectOldest() const {
  ...
  usedMemory -= arrDepthData[idxImage].GetMemorySize();
  // release the depth-data; no need to save the depth-data to disk as it is
  // already saved
