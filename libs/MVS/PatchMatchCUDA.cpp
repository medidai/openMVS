...
void PatchMatch::AllocatePatchMatchCUDA(const cv::Mat1f &image) {
  ...
  CUDA_CHECK(cudaMalloc((void **)&cudaTextureImages,
                        sizeof(cudaTextureObject_t) * num_images));
  CUDA_CHECK(cudaMalloc((void **)&cudaCameras, sizeof(Camera) * num_images));
  if (params.bGeomConsistency)
    CUDA_CHECK(cudaMalloc((void **)&cudaTextureDepths,
                          sizeof(cudaTextureObject_t) * (num_images - 1)));

  const size_t size = image.size().area();
  depthNormalEstimates = new Point4[size];
  CUDA_CHECK(
      cudaMalloc((void **)&cudaDepthNormalEstimates, sizeof(Point4) * size));
...
void PatchMatch::AllocateImageCUDA(size_t i, const cv::Mat1f &image,
                                   bool bInitImage, bool bInitDepthMap) {
  const cudaChannelFormatDesc channelDesc =
      cudaCreateChannelDesc(32, 0, 0, 0, cudaChannelFormatKindFloat);

  if (bInitImage) {
    CUDA_CHECK(cudaMallocArray(&cudaImageArrays[i], &channelDesc, image.cols,
                               image.rows));
    ...
    CUDA_CHECK(
        cudaCreateTextureObject(&textureImages[i], &resDesc, &texDesc, NULL));
  }
  ...
  if (params.bGeomConsistency && i > 0) {
    ...
    CUDA_CHECK(cudaMallocArray(&cudaDepthArrays[i - 1], &channelDesc,
    ...
    CUDA_CHECK(cudaCreateTextureObject(&textureDepths[i - 1], &resDesc,
                                       &texDesc, NULL));
  }
}
...
void PatchMatch::EstimateDepthMap(DepthData &depthData) {
  ...
  const unsigned totalScaleNumber(
      params.bGeomConsistency ? 0u : OPTDENSE::nSubResolutionLevels);
  ...
  if (params.bGeomConsistency &&
      cudaDepthArrays.size() < (size_t)params.nNumViews) {
  ...
  for (unsigned scaleNumber = totalScaleNumber + 1; scaleNumber-- > 0;) {
    ...
    DepthData currentDepthData(
        DepthMapsData::ScaleDepthData(fullResDepthData, scale));
    ...
    if (scaleNumber != totalScaleNumber) {
      ...
      cv::resize(lowResNormalMap, depthData.normalMap, size, 0, 0,
                 cv::INTER_NEAREST);
      cv::resize(lowResViewsMap, depthData.viewsMap, size, 0, 0,
                 cv::INTER_NEAREST);
      CUDA_CHECK(
          cudaMalloc((void **)&cudaLowDepths, sizeof(float) * size.area()));
    } else {
    ...
    if (totalScaleNumber) {
      ...
      } else if (scaleNumber == totalScaleNumber ||
                 (!params.bGeomConsistency &&
                  OPTDENSE::nEstimationGeometricIters)) {
        // smallest sub-resolution OR highest resolution and geometric
        // consistency is not running but enabled
    ...
    for (IIndex i = 0; i < numImages; ++i) {
      ...
      const Camera camera(
          Eigen::Map<const SEACAVE::Matrix3x3::EMat>(view.camera.K.val)
              .cast<float>(),
          Eigen::Map<const SEACAVE::Matrix3x3::EMat>(view.camera.R.val)
              .cast<float>(),
          Eigen::Map<const SEACAVE::Point3::EVec>(view.camera.C.ptr())
              .cast<float>(),
          image.cols, image.rows);
      // store camera and image
      if (i == 0 &&
          (prevNumImages < numImages || images[0].size() != image.size())) {
      ...
      if (i >= prevNumImages) {
      ...
      } else if (images[i].size() != image.size()) {
        ...
        AllocateImageCUDA(i, image, true, !view.depthMap.empty());
      } else if (params.bGeomConsistency && i > 0 &&
                 (view.depthMap.empty() != (cudaDepthArrays[i - 1] == NULL))) {
      ...
      CUDA_CHECK(cudaMemcpy2DToArray(
          cudaImageArrays[i], 0, 0, image.ptr<float>(), image.step[0],
          image.cols * sizeof(float), image.rows, cudaMemcpyHostToDevice));
      if (params.bGeomConsistency && i > 0 && !view.depthMap.empty()) {
        ...
        CUDA_CHECK(cudaMemcpy2DToArray(cudaDepthArrays[i - 1], 0, 0,
                                       depthMap.ptr<float>(), depthMap.step[0],
                                       sizeof(float) * depthMap.cols,
                                       depthMap.rows, cudaMemcpyHostToDevice));
      }
    ...
    // setup CUDA memory
    CUDA_CHECK(cudaMemcpy(cudaTextureImages, textureImages.data(),
                          sizeof(cudaTextureObject_t) * numImages,
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(cudaCameras, cameras.data(),
                          sizeof(Camera) * numImages, cudaMemcpyHostToDevice));
    if (params.bGeomConsistency) {
      // set previously computed depth-maps
      ASSERT(depthData.depthMap.size() == depthData.GetView().image.size());
      CUDA_CHECK(cudaMemcpy(cudaTextureDepths, textureDepths.data(),
                            sizeof(cudaTextureObject_t) * params.nNumViews,
                            cudaMemcpyHostToDevice));
    }
    ...
    for (int r = 0; r < depthData.depthMap.rows; ++r) {
      ...
      for (int c = 0; c < depthData.depthMap.cols; ++c) {
        ...
        depthNormal.topLeftCorner<3, 1>() =
            Eigen::Map<const Normal::EVec>(n.ptr());
        depthNormal.w() = depthData.depthMap(r, c);
      }
    }
    CUDA_CHECK(cudaMemcpy(cudaDepthNormalEstimates, depthNormalEstimates,
                          sizeof(Point4) * depthData.depthMap.size().area(),
                          cudaMemcpyHostToDevice));

    // load low resolution depth-map into CUDA memory
    if (params.bLowResProcessed) {
      ASSERT(depthData.depthMap.isContinuous());
      CUDA_CHECK(cudaMemcpy(cudaLowDepths, depthData.depthMap.ptr<float>(),
                            sizeof(float) * depthData.depthMap.size().area(),
                            cudaMemcpyHostToDevice));
    }

    // run CUDA patch-match
    ASSERT(!depthData.viewsMap.empty());
    RunCUDA(depthData.confMap.getData(),
            (uint32_t *)depthData.viewsMap.getData());
    ...
    for (int r = 0; r < depthData.depthMap.rows; ++r) {
      for (int c = 0; c < depthData.depthMap.cols; ++c) {
        ...
        if (scaleNumber == 0) {
          // converted ZNCC [0-2] score, where 0 is best, to [0-1] confidence,
          // where 1 is best
          ...
          if (depth > 0) {
            const uint32_t bitviews(
                *reinterpret_cast<const uint32_t *>(views.val));
            int j = 0;
            for (int i = 0; i < 32; ++i) {
              if (bitviews & (1 << i)) {
                views[j] = i;
  ...
  if (OPTDENSE::nIgnoreMaskLabel >= 0) {
    ...
    BitMatrix mask;
    if (DepthEstimator::ImportIgnoreMask(
  ...
  DEBUG_EXTRA("Depth-map for image %3u %s: %dx%d (%s)",
              ...
              depthData.images.GetSize() > 2
                  ...
                  : String::FormatString("with image %3u estimated",
                        ...
                        .c_str(),
              images.front().cols, images.front().rows,
