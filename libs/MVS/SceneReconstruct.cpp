...
public:
  ...
  inline void AddEdge(node_type n1, node_type n2, value_type capacity,
                      value_type reverseCapacity) {
    ASSERT(ISFINITE(capacity) && capacity >= 0 && ISFINITE(reverseCapacity) &&
           reverseCapacity >= 0);
...
public:
  ...
  typedef boost::adjacency_list_traits<out_edge_list_t, vertex_list_t,
                                       boost::directedS>
      graph_traits;
  ...
  typedef boost::adjacency_list<out_edge_list_t, vertex_list_t,
                                boost::directedS, size_t, Edge>
      graph_type;
...
public:
  MaxFlow(size_t numNodes)
      : graph(numNodes + 2), S(node_type(numNodes)),
        T(node_type(numNodes + 1)) {}
  ...
  void AddEdge(node_type n1, node_type n2, value_type capacity,
               value_type reverseCapacity) {
    ASSERT(ISFINITE(capacity) && capacity >= 0 && ISFINITE(reverseCapacity) &&
           reverseCapacity >= 0);
  ...
  value_type ComputeMaxFlow() {
    ...
    return boost::boykov_kolmogorov_max_flow(
        graph, boost::get(&Edge::capacity, graph),
...
// construct the mesh out of the dense point-cloud using Delaunay
// tetrahedralization & graph-cut method see "Exploiting Visibility Information
// in Surface Reconstruction to Preserve Weakly Supported Surfaces", Jancosek
// and Pajdla, 2015
namespace DELAUNAY {
...
struct vert_info_t {
  ...
  struct view_t {
    ...
    inline view_t() {}
    inline view_t(PointCloud::View _idxView, Type _weight)
        : idxView(_idxView), weight(_weight) {}
...
#ifdef DELAUNAY_WEAKSURF
  view_info_t *viewsInfo; // each view caches the two faces from the point
                          // towards the camera and the end (used only by the
  ...
  void InsertViews(const PointCloud &pc, PointCloud::Index idxPoint) {
    ...
    const PointCloud::WeightArr *pweights(
        pc.pointWeights.IsEmpty() ? NULL : pc.pointWeights.Begin() + idxPoint);
    ASSERT(pweights == NULL || _views.GetSize() == pweights->GetSize());
    FOREACH(i, _views) {
      const PointCloud::View viewID(_views[i]);
      const PointCloud::Weight weight(pweights ? (*pweights)[i]
                                               : PointCloud::Weight(1));
...
typedef CGAL::Triangulation_vertex_base_with_info_3<vert_info_t, kernel_t>
    vertex_base_t;
typedef CGAL::Triangulation_cell_base_with_info_3<cell_size_t, kernel_t>
    cell_base_t;
typedef CGAL::Triangulation_data_structure_3<vertex_base_t, cell_base_t>
    triangulation_data_structure_t;
typedef CGAL::Delaunay_triangulation_3<kernel_t, triangulation_data_structure_t,
...
struct camera_cell_t {
  ...
  std::vector<facet_t> facets; // all facets on the convex-hull in view of the
                               // camera (ordered by importance)
};
...
struct adjacent_vertex_back_inserter_t {
  ...
  const point_t &p;
  vertex_handle_t &v;
  inline adjacent_vertex_back_inserter_t(const delaunay_t &_delaunay,
                                         const point_t &_p, vertex_handle_t &_v)
  ...
  inline void operator=(const vertex_handle_t &w) {
    ...
    if (!delaunay.is_infinite(w) &&
        delaunay.geom_traits().compare_distance_3_object()(
            p, w->point(), v->point()) == CGAL::SMALLER)
      v = w;
  }
};
...
#endif
static inline int orientation(const point_t &a, const point_t &b,
                              const point_t &c, const point_t &p) {
#if 0
...
#if 1
  const double det((pqx * pry - prx * pqy) * (p.z() - pz) -
                   (pqx * psy - psx * pqy) * (c.z() - pz) +
  ...
  const double det(
      CGAL::determinant(pqx, pqy, pqz, prx, pry, prz, psx, psy, psz));
...
inline bool checkPointInside(const point_t &a, const point_t &b,
                             const point_t &c, const point_t &d,
                             const point_t &o, const point_t &p) {
  return (orientation(o, a, b, p) == CGAL::POSITIVE &&
...
void fetchCellFacets(const delaunay_t &Tr,
                     const std::vector<facet_t> &hullFacets,
                     const cell_handle_t &cell, const Image &imageData,
  ...
  ASSERT(facets.empty());
  const TFrustum<REAL, 4> frustum(imageData.camera.P, imageData.width,
                                  imageData.height, 0, 1);
  // loop over all cells
...
struct intersection_t {
  ...
  vertex_handle_t v1;  // vertex for vertex intersection, 1st edge vertex for
                       // edge intersection
  ...
  const Ray3 ray; // the ray from starting point into the direction of the end
                  // point (point -> camera/end-point)
  inline intersection_t() {}
  inline intersection_t(const Point3 &pt, const Point3 &dir)
      : dist(-FLT_MAX), bigger(true), ray(pt, dir) {}
};

// Check if a segment (p, q) is coplanar with edges of a triangle (a, b, c):
//  coplanar [in,out] : pointer to the 3 int array of indices of the edges
//  coplanar with pq
// return number of entries in coplanar
inline int checkEdges(const point_t &a, const point_t &b, const point_t &c,
                      const point_t &p, const point_t &q, int coplanar[3]) {
  int nCoplanar(0);
...
//  coplanar [out] : pointer to the 3 int array of indices of the edges coplanar
//  with (s)
// return -1 if there is no intersection or
// the number of edges coplanar with the segment (0 = intersection inside the
// triangle)
...
// Find which facet is intersected by the segment (seg) and return next facets
// to check:
...
// return false if no intersection found and the end of the segment was not
// reached
bool intersect(const delaunay_t &Tr, const segment_t &seg,
               const std::vector<facet_t> &in_facets,
               std::vector<facet_t> &out_facets, intersection_t &inter) {
  ASSERT(!in_facets.empty());
  static const int facet_vertex_order[] = {2, 1, 3, 2, 2, 3, 0, 2,
                                           0, 3, 1, 0, 0, 1, 2, 0};
  int coplanar[3];
  ...
  for (const facet_t &in_facet : in_facets) {
    ...
    if (nb_coplanar >= 0) {
      ...
      if ((interDist > prevDist) != inter.bigger)
        continue;
      // vertices of facet i: j = 4 * i, vertices =
      // facet_vertex_order[j,j+1,j+2] negative orientation
      ...
      case 1: {
        ...
        out_facets.clear();
        const edge_t out_edge(inter.facet.first, facet_vertex_order[i1 + 0],
                              facet_vertex_order[i1 + 1]);
        const typename delaunay_t::Cell_circulator efc(
            Tr.incident_cells(out_edge));
      ...
      case 2: {
        ...
        struct cell_back_inserter_t {
          ...
          inline cell_back_inserter_t(const delaunay_t &_Tr,
              ...
              : Tr(_Tr), v(inter.v1), current_cell(inter.facet.first),
                out_facets(_out_facets) {}
        ...
        Tr.finite_incident_cells(inter.v1,
  ...
  // Bad end: no intersection found and we are not at the end of the segment
  // (very rarely, but it happens)!
  out_facets.clear();
  return false;
}

// same as above, but simplified only to find face intersection (otherwise
// terminate); terminate if cell containing the segment endpoint is found or if
// an infinite cell is encountered
bool intersectFace(const delaunay_t &Tr, const segment_t &seg,
                   const std::vector<facet_t> &in_facets,
                   std::vector<facet_t> &out_facets, intersection_t &inter) {
  int coplanar[3];
  for (std::vector<facet_t>::const_iterator it = in_facets.cbegin();
       it != in_facets.cend(); ++it) {
...
inline bool intersectFace(const delaunay_t &Tr, const segment_t &seg,
                          const vertex_handle_t &v, const cell_handle_t &cell,
                          std::vector<facet_t> &out_facets,
...
edge_cap_t freeSpaceSupport(const delaunay_t &Tr,
                            const std::vector<cell_info_t> &infoCells,
...
// making sure the facet orientation is kept (as in
// CGAL::Triangulation_3::triangle()) return the vertex handles of the triangle
struct triangle_vhandles_t {
  vertex_handle_t verts[3];
  triangle_vhandles_t() {}
  triangle_vhandles_t(vertex_handle_t _v0, vertex_handle_t _v1,
                      vertex_handle_t _v2)
...
inline triangle_vhandles_t getTriangle(cell_handle_t cell, int i) {
  ASSERT(i >= 0 && i <= 3);
  if ((i & 1) == 0)
    return triangle_vhandles_t(cell->vertex((i + 2) & 3),
                               cell->vertex((i + 1) & 3),
                               cell->vertex((i + 3) & 3));
  return triangle_vhandles_t(cell->vertex((i + 1) & 3),
                             cell->vertex((i + 2) & 3),
                             cell->vertex((i + 3) & 3));
}

// Compute the angle between the plane containing the given facet and the cell's
// circumscribed sphere return cosines of the angle
...
// First, iteratively create a Delaunay triangulation of the existing
// point-cloud by inserting point by point, iif the point to be inserted is not
// closer than distInsert pixels in at least one of its views to the projection
// of any of already inserted points. Next, the score is computed for all the
// edges of the directed graph composed of points as vertices. Finally,
// graph-cut algorithm is used to split the tetrahedrons in inside and outside,
// and the surface is such extracted.
bool Scene::ReconstructMesh(float distInsert, bool bUseFreeSpaceSupport,
                            bool bUseOnlyROI, unsigned nItersFixNonManifold,
                            float kSigma, float kQual, float kb, float kf,
                            float kRel, float kAbs, float kOutl, float kInf) {
  ...
  std::vector<facet_t> hullFacets;
  {
    ...
    // sort vertices
    typedef CGAL::Spatial_sort_traits_adapter_3<delaunay_t::Geom_traits,
                                                point_t *>
        Search_traits;
    CGAL::spatial_sort(indices.begin(), indices.end(),
                       Search_traits(&vertices[0], delaunay.geom_traits()));
    // insert vertices
        ...
        if (lt == delaunay_t::VERTEX) {
          ...
          if (delaunay.dimension() < 3) {
            ...
            while (true) {
              ...
              delaunay.adjacent_vertices(
                  nearest,
                  adjacent_vertex_back_inserter_t(delaunay, p, nearest));
              if (v == nearest)
                break;
            }
          }
          ...
          FOREACHPTR(pViewID, views) {
            ...
            const Point3f pe(imageData.camera.ProjectPointP3(
                CGAL2MVS<float>(nearest->point())));
            if (!IsDepthSimilar(pn.z, pe.z) ||
                normSq(Point2f(pn) - Point2f(pe)) > distInsertSq) {
    ...
    cell_size_t ciID(0);
    for (delaunay_t::All_cells_iterator ci = delaunay.all_cells_begin(),
                                        eci = delaunay.all_cells_end();
         ci != eci; ++ci, ++ciID) {
      ci->info() = ciID;
    ...
    FOREACH(i, images) {
      ...
      fetchCellFacets<CGAL::POSITIVE>(delaunay, hullFacets, camCell.cell,
                                      imageData, camCell.facets);
      // link all cells contained by the camera to the source
      for (const facet_t &f : camCell.facets)
        infoCells[f.first->info()].s = kInf;
    }

    DEBUG_EXTRA("Delaunay tetrahedralization completed: %u points -> %u "
                "vertices, %u (+%u) cells, %u (+%u) faces (%s)",
                indices.size(), delaunay.number_of_vertices(),
                delaunay.number_of_finite_cells(),
                delaunay.number_of_cells() - delaunay.number_of_finite_cells(),
                delaunay.number_of_finite_facets(),
                delaunay.number_of_facets() -
                    delaunay.number_of_finite_facets(),
  ...
  // add alpha_vis(point) to cell's directed edge in the graph
  {
    ...
    for (delaunay_t::Finite_edges_iterator ei = delaunay.finite_edges_begin(),
                                           eei = delaunay.finite_edges_end();
         ei != eei; ++ei) {
    ...
    {
      TD_TIMER_STARTD();
      Util::Progress progress(_T("Points weighted"),
...
#pragma omp critical
        vi = vertexIter++;
#else
      for (delaunay_t::Vertex_iterator vi = delaunay.vertices_begin(),
                                       vie = delaunay.vertices_end();
           vi != vie; ++vi) {
#endif
        ...
        FOREACH(v, vert.views) {
          ...
          if (!intersect(delaunay, segCamPoint, camCell.facets, facets, inter))
            continue;
          do {
            // assign score, weighted by the distance from the point to the
            // intersection
            const edge_cap_t w(
                alpha_vis *
                (1.f - EXP(-SQUARE((float)inter.dist) * inv2SigmaSq)));
            edge_cap_t &f(
                infoCells[inter.facet.first->info()].f[inter.facet.second]);
#ifdef DELAUNAY_USE_OPENMP
#pragma omp atomic
#endif
            f += w;
          } while (intersect(delaunay, segCamPoint, facets, facets, inter));
          ASSERT(facets.empty() && inter.type == intersection_t::VERTEX &&
                 inter.v1 == vi);
#ifdef DELAUNAY_WEAKSURF
          ...
          const cell_handle_t endCell(
              delaunay.locate(segEndPoint.source(), vi->cell()));
          ASSERT(endCell != cell_handle_t());
          fetchCellFacets<CGAL::NEGATIVE>(delaunay, hullFacets, endCell,
                                          imageData, facets);
...
#ifdef DELAUNAY_WEAKSURF
    // enforce t-edges for each point-camera pair with free-space support
    // weights
...
#pragma omp critical
      ...
      for (delaunay_t::Vertex_iterator vi = delaunay.vertices_begin(),
        ...
        FOREACH(v, vert.views) {
          ...
          // find faces intersected by the point-camera segment and keep the max
          // free-space support score
          const Point3f bgnPoint(pt -
                                 vecCamPoint * (invLenCamPoint * sigma * kf));
          const segment_t segPointBgn(p, MVS2CGAL(bgnPoint));
          intersection_t inter;
          if (!intersectFace(delaunay, segPointBgn, vi,
                             vert.viewsInfo[v].cell2Cam, facets, inter))
            continue;
          edge_cap_t beta(0);
          do {
            const edge_cap_t fs(
                freeSpaceSupport(delaunay, infoCells, inter.facet.first));
            if (beta < fs)
              beta = fs;
          } while (intersectFace(delaunay, segPointBgn, facets, facets, inter));
          // find faces intersected by the point-endpoint segment
          const Point3f endPoint(pt +
                                 vecCamPoint * (invLenCamPoint * sigma * kb));
          const segment_t segPointEnd(p, MVS2CGAL(endPoint));
          if (!intersectFace(delaunay, segPointEnd, vi,
                             vert.viewsInfo[v].cell2End, facets, inter))
            continue;
...
#pragma omp atomic
      ...
      DEBUG_ULTIMATE("\tt-edge reinforcement completed in %s",
                     TD_TIMER_GET_FMT().c_str());
    }
#endif

    DEBUG_EXTRA(
        "Delaunay tetrahedras weighting completed: %u cells, %u faces (%s)",
  ...
  // run graph-cut and extract the mesh
  {
    ...
    for (delaunay_t::All_cells_iterator ci = delaunay.all_cells_begin(),
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        const edge_cap_t q(
            (1.f - MINF(computePlaneSphereAngle(delaunay, facet_t(ci, i)),
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        for (int v = 0; v < 3; ++v) {
          ...
          const auto pairItID(mapVertices.insert(
              std::make_pair(vh.for_compact_container(),
                             (Mesh::VIndex)mesh.vertices.GetSize())));
          if (pairItID.second)
    ...
    DEBUG_EXTRA("Delaunay tetrahedras graph-cut completed (%g flow): %u "
                "vertices, %u faces (%s)",
                maxflow, mesh.vertices.GetSize(), mesh.faces.GetSize(),
                TD_TIMER_GET_FMT().c_str());
  }
