...
public:
  ...
  typedef SEACAVE::cList<VertexIdxArr, const VertexIdxArr &, 2, 8192, VIndex>
      VertexVerticesArr;
  typedef SEACAVE::cList<FaceIdxArr, const FaceIdxArr &, 2, 8192, VIndex>
      VertexFacesArr;
  ...
  typedef SEACAVE::cList<TexCoord, const TexCoord &, 0, 8192, FIndex>
      TexCoordArr;

  typedef uint8_t TexIndex;
  typedef SEACAVE::cList<TexIndex, const TexIndex &, 0, 8192, FIndex>
      TexIndexArr;
  typedef SEACAVE::cList<Image8U3, const Image8U3 &, 2, 4, TexIndex>
      Image8U3Arr;

  typedef TPoint3<FIndex> FaceFaces;
  typedef SEACAVE::cList<FaceFaces, const FaceFaces &, 0, 8192, FIndex>
      FaceFacesArr;
  ...
  struct FacesInserter {
    ...
    FacesInserter(FaceIdxArr &_cameraFaces) : cameraFaces(_cameraFaces) {}
  ...
  typedef SEACAVE::cList<FaceChunk, const FaceChunk &, 2, 16, uint32_t>
      FacesChunkArr;
...
public:
  ...
  NormalArr vertexNormals; // for each vertex, the normal to the surface in that
                           // point (optional)
  VertexVerticesArr vertexVertices; // for each vertex, the list of adjacent
                                    // vertices (optional)
  VertexFacesArr vertexFaces; // for each vertex, the ordered list of faces
                              // containing it (optional)
  BoolArr vertexBoundary; // for each vertex, stores if it is at the boundary or
                          // not (optional)

  NormalArr faceNormals;  // for each face, the normal to it (optional)
  FaceFacesArr faceFaces; // for each face, the list of adjacent faces, NO_ID
                          // for border edges (optional)
  TexCoordArr
      faceTexcoords; // for each face, the texture-coordinates corresponding to
                     // its vertices, 3x num faces OR for each vertex (optional)
  TexIndexArr faceTexindices; // for each face, the corresponding index of the
                              // texture (optional)

  Image8U3Arr
      texturesDiffuse; // textures containing the diffuse color (optional)
...
public:
#ifdef _USE_CUDA
  inline Mesh() { InitKernels(SEACAVE::CUDA::desiredDeviceID); }
#endif
  ...
  bool HasTexture() const {
    return HasTextureCoordinates() && !texturesDiffuse.empty();
  }
  bool HasTextureCoordinates() const {
    ASSERT(faceTexcoords.empty() || faces.size() * 3 == faceTexcoords.size() ||
           vertices.size() == faceTexcoords.size());
    return !faceTexcoords.empty();
  }
  bool HasTextureCoordinatesPerVertex() const {
    return !faceTexcoords.empty() && vertices.size() == faceTexcoords.size();
  }
  ...
  void SmoothNormalFaces(float fMaxGradient = 25.f,
                         float fOriginalWeight = 0.5f,
  ...
  unsigned FixNonManifold(float magDisplacementDuplicateVertices = 0.01f,
                          VertexIdxArr *duplicatedVertices = NULL);
  void Clean(float fDecimate = 0.7f, float fSpurious = 10.f,
             bool bRemoveSpikes = true, unsigned nCloseHoles = 30,
             unsigned nSmoothMesh = 2, float fEdgeLength = 0,
             bool bLastClean = true);

  void EnsureEdgeSize(float minEdge = -0.5f, float maxEdge = -4.f,
                      float collapseRatio = 0.2,
                      float degenerate_angle_deg = 150, int mode = 1,
                      int max_iters = 50);
  ...
  TexIndex GetFaceTextureIndex(FIndex idxF) const {
    return faceTexindices.empty() ? 0 : faceTexindices[idxF];
  }
  void FaceTexcoordsNormalize(TexCoordArr &newFaceTexcoords,
                              bool flipY = true) const;
  void FaceTexcoordsUnnormalize(TexCoordArr &newFaceTexcoords,
                                bool flipY = true) const;

  inline Normal FaceNormal(const Face &f) const {
    return ComputeTriangleNormal(vertices[f[0]], vertices[f[1]],
                                 vertices[f[2]]);
  }
  ...
  Planef EstimateGroundPlane(const ImageArr &images, float sampleMesh = 0,
  ...
  void SamplePoints(REAL samplingDensity, unsigned mumPointsTheoretic,
                    PointCloud &) const;
  ...
  void Project(const Camera &camera, DepthMap &depthMap,
               NormalMap &normalMap) const;
  void ProjectOrtho(const Camera &camera, DepthMap &depthMap) const;
  void ProjectOrtho(const Camera &camera, DepthMap &depthMap,
                    Image8U3 &image) const;
  void ProjectOrthoTopDown(unsigned resolution, Image8U3 &image, Image8U &mask,
                           Point3 &center) const;

  bool Split(FacesChunkArr &, float maxArea);
  Mesh SubMesh(const FaceIdxArr &faces) const;

  bool TransferTexture(Mesh &mesh, const FaceIdxArr &faceSubsetIndices = {},
                       unsigned borderSize = 3, unsigned textureSize = 4096);

  // file IO
  bool Load(const String &fileName);
  bool Save(const String &fileName,
            const cList<String> &comments = cList<String>(),
            bool bBinary = true) const;
  bool Save(const FacesChunkArr &, const String &fileName,
            const cList<String> &comments = cList<String>(),
            bool bBinary = true) const;
  static bool Save(const VertexArr &vertices, const String &fileName,
                   bool bBinary = true);
...
protected:
  ...
  bool SavePLY(const String &fileName,
               const cList<String> &comments = cList<String>(),
               bool bBinary = true, bool bTexLossless = true) const;
...
template <typename DERIVED> struct TRasterMeshBase {
  ...
  inline void Clear() { depthMap.memset(0); }
  inline cv::Size Size() const { return depthMap.size(); }
  ...
  inline Point3f PerspectiveCorrectBarycentricCoordinates(const Triangle &t,
                                                          const Point3f &bary) {
    return SEACAVE::PerspectiveCorrectBarycentricCoordinates(
        bary, (float)t.ptc[0].z, (float)t.ptc[1].z, (float)t.ptc[2].z);
  }
  inline float ComputeDepth(const Triangle &t, const Point3f &pbary) {
    return pbary[0] * (float)t.ptc[0].z + pbary[1] * (float)t.ptc[1].z +
           pbary[2] * (float)t.ptc[2].z;
  }
  ...
  struct TriangleRasterizer {
    ...
    TriangleRasterizer(Triangle &t, Rasterizer &r)
        : triangle(t), rasterizer(r) {}
...
// used to render a mesh
template <typename DERIVED> struct TRasterMesh : TRasterMeshBase<DERIVED> {
  ...
  TRasterMesh(const Mesh::VertexArr &_vertices, const Camera &_camera,
              DepthMap &_depthMap)
  ...
  void Project(const Mesh::Face &facet, TriangleRasterizer &tr) {
    ...
    // draw triangle
    Image8U3::RasterizeTriangleBary(tr.triangle.pti[0], tr.triangle.pti[1],
                                    tr.triangle.pti[2], tr);
  }
...
struct IntersectRayMesh {
  ...
  inline bool Intersects(const typename Octree::POINT_TYPE &center,
                         typename Octree::Type radius) const {
  ...
  void operator()(const IDX *idices, IDX size) {
    ...
    for (Mesh::FIndex idxFace : set) {
      ...
      REAL dist;
      if (ray.Intersects<true>(Triangle3(Cast<REAL>(mesh.vertices[face[0]]),
                                         Cast<REAL>(mesh.vertices[face[1]]),
                                         Cast<REAL>(mesh.vertices[face[2]])),
                               &dist)) {
        ASSERT(dist >= 0);
        if (pick.dist > dist) {
          pick.dist = dist;
