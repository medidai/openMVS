...
enum Mask { empty = 0, border = 128, interior = 255 };
...
struct MeshTexture {
  ...
  struct RasterMesh : TRasterMesh<RasterMesh> {
    ...
    void Raster(const ImageRef &pt, const Triangle &t, const Point3f &bary) {
      ...
      if (depth == 0 || depth > z) {
        depth = z;
        faceMap(pt) =
            validFace && (validFace = (mask(pt) != 0)) ? idxFace : NO_ID;
      }
    }
  };
  ...
  struct TexturePatch {
    ...
    RectsBinPack::Rect
        rect; // the bounding box in the view containing the patch
  };
  typedef cList<TexturePatch, const TexturePatch &, 1, 1024, FIndex>
      TexturePatchArr;
  ...
  struct SeamVertex {
    struct Patch {
      ...
      Edges edges; // the edges starting from this vertex, contained in this
                   // patch (exactly two for manifold meshes)
  ...
  struct VertexPatchIterator {
    ...
    inline VertexPatchIterator(const PatchIndex &patchIndex,
                               const SeamVertices &seamVertices)
        : idx(NO_ID) {
    ...
    inline operator uint32_t() const { return idxPatch; }
    inline bool Next() {
      if (pPatches == NULL)
        return (idx++ == NO_ID);
  ...
  struct SampleImage {
    ...
    Color GetColor() const { return accumColor.Normalized(); }
  };
...
public:
  MeshTexture(Scene &_scene, unsigned _nResolutionLevel = 0,
              unsigned _nMinResolution = 640);
  ~MeshTexture();
...
#if TEXOPT_FACEOUTLIER != TEXOPT_FACEOUTLIER_NA
  bool FaceOutlierDetection(FaceDataArr &faceDatas,
                            float fOutlierThreshold) const;
#endif
  ...
  IIndexArr SelectBestView(const FaceDataArr &faceDatas, FIndex fid,
                           unsigned minCommonCameras,
                           float ratioAngleToQuality) const;

  bool FaceViewSelection(unsigned minCommonCameras, float fOutlierThreshold,
  ...
  void GenerateTexture(bool bGlobalSeamLeveling, bool bLocalSeamLeveling,
                       ...
                       unsigned nRectPackingHeuristic, Pixel8U colEmpty,
                       float fSharpnessWeight, int maxTextureSize);
...
protected:
  ...
  static void PoissonBlending(const Image32F3 &src, Image32F3 &dst,
                              const Image8U &mask, float bias = 1.f);

public:
  const unsigned nResolutionLevel; // how many times to scale down the images
                                   // before mesh optimization
  const unsigned nMinResolution;   // how many times to scale down the images
                                   // before mesh optimization
  ...
  PairIdxArr
      seamEdges; // the (face-face) edges connecting different texture patches
  Mesh::FaceIdxArr components; // for each face, stores the texture patch index
                               // to which belongs
  IndexArr
      mapIdxPatch; // remap texture patch indices after invalid patches removal
  SeamVertices seamVertices; // array of vertices on the border between two or
                             // more patches

  // valid the entire time
  Mesh::VertexFacesArr
      &vertexFaces; // for each vertex, the list of faces containing it
  BoolArr &
      vertexBoundary; // for each vertex, stores if it is at the boundary or not
  Mesh::FaceFacesArr &faceFaces; // for each face, the list of adjacent faces,
                                 // NO_ID for border edges (optional)
  Mesh::TexCoordArr
      &faceTexcoords; // for each face, the texture-coordinates of the vertices
  Mesh::TexIndexArr
      &faceTexindices; // for each face, the texture-coordinates of the vertices
...
// the returned mask has the same size as the image and is set to zero for
// invalid pixels
static Image8U DetectInvalidImageRegions(const Image8U3 &image) {
  ...
  if (imageGray(0, 0) == 0)
    cv::floodFill(imageGray, mask, cv::Point(0, 0), 255, NULL, cv::Scalar(0),
                  upDiff, flags);
  if (imageGray(image.rows / 2, 0) == 0)
    cv::floodFill(imageGray, mask, cv::Point(0, image.rows / 2), 255, NULL,
                  cv::Scalar(0), upDiff, flags);
  if (imageGray(image.rows - 1, 0) == 0)
    cv::floodFill(imageGray, mask, cv::Point(0, image.rows - 1), 255, NULL,
                  cv::Scalar(0), upDiff, flags);
  if (imageGray(image.rows - 1, image.cols / 2) == 0)
    cv::floodFill(imageGray, mask, cv::Point(image.cols / 2, image.rows - 1),
                  255, NULL, cv::Scalar(0), upDiff, flags);
  if (imageGray(image.rows - 1, image.cols - 1) == 0)
    cv::floodFill(imageGray, mask, cv::Point(image.cols - 1, image.rows - 1),
                  255, NULL, cv::Scalar(0), upDiff, flags);
  if (imageGray(image.rows / 2, image.cols - 1) == 0)
    cv::floodFill(imageGray, mask, cv::Point(image.cols - 1, image.rows / 2),
                  255, NULL, cv::Scalar(0), upDiff, flags);
  if (imageGray(0, image.cols - 1) == 0)
    cv::floodFill(imageGray, mask, cv::Point(image.cols - 1, 0), 255, NULL,
                  cv::Scalar(0), upDiff, flags);
  if (imageGray(0, image.cols / 2) == 0)
    cv::floodFill(imageGray, mask, cv::Point(image.cols / 2, 0), 255, NULL,
                  cv::Scalar(0), upDiff, flags);
  mask = (mask(cv::Rect(1, 1, imageGray.cols, imageGray.rows)) == 0);
  return mask;
}

MeshTexture::MeshTexture(Scene &_scene, unsigned _nResolutionLevel,
                         unsigned _nMinResolution)
    : nResolutionLevel(_nResolutionLevel), nMinResolution(_nMinResolution),
      ...
      vertices(_scene.mesh.vertices), faces(_scene.mesh.faces),
      images(_scene.images), scene(_scene) {}
...
bool MeshTexture::ListCameraFaces(FaceDataViewArr &facesDatas,
                                  float fOutlierThreshold, int nIgnoreMaskLabel,
...
#pragma omp flush(bAbort)
  ...
  for (IIndex idxView : views) {
    ...
    const unsigned imageSize(
        imageData.RecomputeMaxResolution(level, nMinResolution));
    if ((imageData.image.empty() ||
         MAXF(imageData.width, imageData.height) != imageSize) &&
...
#pragma omp flush(bAbort)
    ...
    grad[0] = cv::Mat(imageGradMag.rows, imageGradMag.cols,
                      cv::DataType<real>::type, (void *)mGrad[0].data());
    mGrad[1].resize(imageGradMag.rows, imageGradMag.cols);
    grad[1] = cv::Mat(imageGradMag.rows, imageGradMag.cols,
                      cv::DataType<real>::type, (void *)mGrad[1].data());
#if 1
    cv::Sobel(imageGradMag, grad[0], cv::DataType<real>::type, 1, 0, 3,
              1.0 / 8.0);
    cv::Sobel(imageGradMag, grad[1], cv::DataType<real>::type, 0, 1, 3,
              1.0 / 8.0);
#elif 1
  ...
  cv::filter2D(imageGradMag, grad[1], cv::DataType<real>::type, kernel.t());
    ...
    // apply some blur on the gradient to lower noise/glossiness effects onto
    // face-quality score
    cv::GaussianBlur(imageGradMag, imageGradMag, cv::Size(15, 15), 0, 0,
                     cv::BORDER_DEFAULT);
    ...
    if (nIgnoreMaskLabel >= 0) {
      // import mask
      BitMatrix bmask;
      DepthEstimator::ImportIgnoreMask(imageData, imageData.GetSize(),
                                       (uint8_t)OPTDENSE::nIgnoreMaskLabel,
                                       bmask, &rasterer.mask);
    } else if (nIgnoreMaskLabel == -1) {
      // creating mask to discard invalid regions created during image radial
      // undistortion
      rasterer.mask = DetectInvalidImageRegions(imageData.image);
#if TD_VERBOSE != TD_VERBOSE_OFF
      if (VERBOSITY_LEVEL > 3)
        cv::imwrite(String::FormatString("umask%04d.png", idxView),
                    rasterer.mask);
#endif
    }
    rasterer.Clear();
...
#pragma omp critical
#endif
    {
      ...
      // + area: the higher the area the more gradient scores will be added to
      // the face quality
      // + sharpness: sharper image or image resolution or how close is to the
      // face will result in higher gradient on the same face
      //				ON GLOSS IMAGES it happens to have a
      //high volatile sharpness depending on how the light reflects under
      //different angles
      ...
      // tries to increase chances of a camera with perpendicular view on the
      // surface (smoothened normals) to be selected
      FOREACH(idxFace, facesDatas) {
        ...
        const Vertex faceCenter(
            (vertices[f[0]] + vertices[f[1]] + vertices[f[2]]) / 3.f);
        ...
        const float cosFaceCam(
            MAXF(0.001f, ComputeAngle(camDir.ptr(), faceNormal.ptr())));
  ...
  if (fOutlierThreshold > 0) {
    ...
    // (views for which the face is occluded by a dynamic object in the scene,
    // ex. pedestrians)
...
// order the camera view scores with highest score first and return the list of
// first <minCommonCameras> cameras ratioAngleToQuality represents the ratio in
// witch we combine normal angle to quality for a face to obtain the selection
// score
...
IIndexArr MeshTexture::SelectBestView(const FaceDataArr &faceDatas, FIndex fid,
                                      unsigned minCommonCameras,
                                      float ratioAngleToQuality) const {
  ASSERT(!faceDatas.empty());
#if 1
  ...
  const Vertex faceCenter((vertices[f[0]] + vertices[f[1]] + vertices[f[2]]) /
                          3.f);
  ...
  FOREACH(idxFaceData, faceDatas) {
    ...
    scores[idxFaceData] =
        ratioAngleToQuality * cosFaceCam +
        (1.f - ratioAngleToQuality) * faceData.quality / maxQuality;
  }
  // and sort the scores from to highest to smallest to get the best overall
  // cameras
  IIndexArr scorePodium(faceDatas.size());
  std::iota(scorePodium.begin(), scorePodium.end(), 0);
  scorePodium.Sort(
      [&scores](IIndex i, IIndex j) { return scores[i] > scores[j]; });
  ...
  // and sort the scores in smallest to highest to get the best overall camera
  // for current virtual face
  CLISTDEF0IDX(float, IIndex) scores(faceDatas.size());
  scores.Memset(0);
...
static bool IsFaceVisible(const MeshTexture::FaceDataArr &faceDatas,
                          const IIndexArr &cameraList) {
...
void MeshTexture::CreateVirtualFaces(const FaceDataViewArr &facesDatas,
  ...
  do {
    ...
    const Normal &normalCenter =
        scene.mesh.faceNormals[virtualFaceCenterFaceID];
    ...
    if (centerFaceDatas.empty()) {
      ...
      const IIndexArr selectedCams =
          SelectBestView(centerFaceDatas, virtualFaceCenterFaceID,
                         minCommonCameras, ratioAngleToQuality);
      ...
      do {
        ...
        const float cosFaceToCenter(
            ComputeAngleN(normalCenter.ptr(), faceNormal.ptr()));
        ...
        for (int i = 0; i < 3; ++i) {
          ...
          if (fIdx == NO_ID)
            continue;
          if (!selectedFaces[fIdx] &&
              queuedFaces.find(fIdx) == queuedFaces.end()) {
...
bool MeshTexture::FaceOutlierDetection(FaceDataArr &faceDatas,
                                       float thOutlier) const {
  // consider as outlier if the absolute difference to the median is outside
  // this threshold
  if (thOutlier <= 0)
...
// A multi-variate normal distribution which is NOT normalized such that the
// integral is 1
// - centered is the vector for which the function is to be evaluated with the
// mean subtracted [Nx1]
...
bool MeshTexture::FaceOutlierDetection(FaceDataArr &faceDatas,
  ...
  const float minCovariance(1e-3f); // if all covariances drop below this the
                                    // outlier detection aborted
  ...
  for (unsigned iter = 0; iter < maxIterations; ++iter) {
    ...
    const Eigen::Block<Eigen::Matrix3Xd, 3, Eigen::Dynamic,
                       !Eigen::Matrix3Xd::IsRowMajor>
    ...
    // (FullPivLU not the fastest, but gives feedback about numerical stability
    // during inversion)
    ...
    FOREACH(i, faceDatas) {
      const Eigen::Vector3d color(
          ((const Color::EVec)faceDatas[i].color).cast<double>());
      const double gaussValue(
          MultiGaussUnnormalized<double, 3>(color, mean, covarianceInv));
...
bool MeshTexture::FaceViewSelection(unsigned minCommonCameras,
  ...
  // create texture patches
  {
    ...
    if (!ListCameraFaces(facesDatas, fOutlierThreshold, nIgnoreMaskLabel,
                         views))
      return false;
    ...
    // construct and use virtual faces for patch creation instead of actual mesh
    // faces; the virtual faces are composed of coplanar triangles sharing same
    // views
    ...
    if (bUseVirtualFaces) {
      ...
      FaceDataViewArr virtualFacesDatas;
      VirtualFaceIdxsArr
          virtualFaces; // stores each virtual face as an array of mesh face ID
      CreateVirtualFaces(facesDatas, virtualFacesDatas, virtualFaces,
                         minCommonCameras);
      Mesh::FaceIdxArr mapFaceToVirtualFace(
          faces.size()); // for each mesh face ID, store the virtual face ID
                         // witch contains it
      ...
      { // for each virtual face, the list of virtual faces with at least one
        // vertex in common
          ...
          for (FIndex idxFace : vf) {
            ...
            for (int i = 0; i < 3; ++i) {
              ...
              if (mapFaceToVirtualFace[fAdj] == idxVF)
                continue;
              if (fAdj != idxFace &&
                  vfNeighbors.Find(mapFaceToVirtualFace[fAdj]) ==
                      Mesh::FaceIdxArr::NO_INDEX) {
      ...
      FOREACH(idxFace, virtualFaces) {
        MAYBEUNUSED const Mesh::FIndex idx(
            (Mesh::FIndex)boost::add_vertex(graph));
        ASSERT(idx == idxFace);
      }
      FOREACH(idxVirtualFace, virtualFaces) {
        ...
        for (FIndex idxVirtualFaceAdj : afaces) {
          ...
          const bool bInvisibleFaceAdj(
              virtualFacesDatas[idxVirtualFaceAdj].empty());
      ...
      labels.resize(faces.size());
      {
        ...
        LBPInference inference;
        {
          ...
          FOREACH(f, virtualFaces) {
            for (boost::tie(ei, eie) = boost::out_edges(f, graph); ei != eie;
                 ++ei) {
    ...
    // create the graph of faces: each vertex is a face and the edges are the
    // edges shared by the faces
    ...
    // create texture patches
    {
      ...
      for (boost::tie(ei, eie) = boost::edges(graph); ei != eie; ++ei) {
        ...
        ASSERT(components.empty() ||
               components[fSource] == components[fTarget]);
...
void MeshTexture::CreateSeamVertices() {
  ...
  // each patch contains the list of edges belonging to that texture patch,
  // starting from that vertex (usually there are pairs of edges in each patch,
  // representing the two edges starting from that vertex separating two valid
  // patches)
  VIndex vs[2];
  ...
  for (const PairIdx &edge : seamEdges) {
    ...
    const auto itSeamVertex0(
        mapVertexSeam.emplace(std::make_pair(vs[0], seamVertices.size())));
    if (itSeamVertex0.second)
      seamVertices.emplace_back(vs[0]);
    SeamVertex &seamVertex0 = seamVertices[itSeamVertex0.first->second];

    const auto itSeamVertex1(
        mapVertexSeam.emplace(std::make_pair(vs[1], seamVertices.size())));
...
void MeshTexture::GlobalSeamLeveling() {
  ...
  ASSERT(vertices.size() <
         static_cast<VIndex>(std::numeric_limits<MatIdx>::max()));
  MatIdx rowsX(0);
  ...
  ASSERT(rows.size() / 2 <
         static_cast<IDX>(std::numeric_limits<MatIdx>::max()));
  ...
  for (const SeamVertex &seamVertex : seamVertices) {
    ...
    FOREACH(i, indices) {
      ...
      for (const SeamVertex::Patch::Edge &edge : patch0.edges) {
        ...
        const SeamVertex::Patches::IDX idxPatch1(
            seamVertex1.patches.Find(patch0.idxPatch));
    ...
    const VertexPatch2RowMap &vertpatch2row =
  ...
  Eigen::Matrix<float, Eigen::Dynamic, 3, Eigen::RowMajor> colorAdjustments(
      rowsX, 3);
  {
    // init CG solver
...
#pragma omp parallel for
    ...
    for (int channel = 0; channel < 3; ++channel) {
      ...
      DEBUG_LEVEL(3, "\tcolor channel %d: %d iterations, %g residual", channel,
  ...
  for (unsigned i = 0; i < numPatches; ++i) {
    ...
    struct RasterPatch {
      ...
      inline void operator()(const ImageRef &pt, const Point3f &bary) {
        ...
        image(pt) =
            colors[0] * bary.x + colors[1] * bary.y + colors[2] * bary.z;
      }
    } data(imageAdj);
    for (const FIndex idxFace : texturePatch.faces) {
      ...
      for (int v = 0; v < 3; ++v)
        data.colors[v] =
            colorAdjustments.row(vertpatch2rows[face[v]].at(idxPatch));
      ...
      ColorMap::RasterizeTriangleBary(data.tri[0], data.tri[1], data.tri[2],
                                      data);
    }
    // dilate with one pixel width, in order to make sure patch border smooths
    // out a little
...
#define ERODEDIR(rd, cd)                                                       \
  {                                                                            \
    ...
    const Type vl(mask.isInside(ImageRef(cl, rl)) ? mask(rl, cl)               \
                                                  : uint8_t(empty));           \
    const Type vr(mask.isInside(ImageRef(cr, rr)) ? mask(rr, cr)               \
                                                  : uint8_t(empty));           \
...
#undef ERODEDIR
  ...
  for (int r = 0; r < mask.rows; ++r) {
    for (int c = 0; c < mask.cols; ++c) {
      ...
      if (v != interior)
        continue;
      if (mask(r - 1, c) == empty || mask(r, c - 1) == empty ||
          mask(r + 1, c) == empty || mask(r, c + 1) == empty)
        v = border;
    }
  }
...
#define ISEMPTY(mask, x, y) (mask(y, x) == empty)
  ...
  // iteratively erode all border pixels
  {
    ...
    for (int s = 0; s < stripWidth; ++s) {
      ...
      for (PixelVector::const_iterator it = emptyPixels.cbegin();
           it != emptyPixels.cend(); ++it)
        orgMask(*it) = empty;
...
#undef ISEMPTY
  ...
  for (PixelSet::const_iterator it = borderPixels.cbegin();
       it != borderPixels.cend(); ++it)
    mask(*it) = border;
...
inline MeshTexture::Color ColorLaplacian(const Image32F3 &img, int i) {
  ...
  return img(i - width) + img(i - 1) + img(i + 1) + img(i + width) -
         img(i) * 4.f;
}

void MeshTexture::PoissonBlending(const Image32F3 &src, Image32F3 &dst,
                                  const Image8U &mask, float bias) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    case interior: {
      ...
      // all indices should be either border conditions or part of the
      // optimization
      ASSERT(idxUp != -1 && idxLeft != -1 && idxCenter != -1 &&
             idxRight != -1 && idxDown != -1);
  ...
  // (faster, but not working if EIGEN_DEFAULT_TO_ROW_MAJOR is defined, bug
  // inside Eigen)
...
// adjust texture patches locally, so that the border continues smoothly inside
// the patch
#ifdef TEXOPT_USE_OPENMP
#pragma omp parallel for schedule(dynamic)
  for (int i = 0; i < (int)numPatches; ++i) {
  ...
  for (unsigned i = 0; i < numPatches; ++i) {
    ...
    for (const SeamVertex &seamVertex0 : seamVertices) {
      ...
      for (const SeamVertex::Patch::Edge &edge0 : patch0.edges) {
        ...
        const SeamVertex::Patch &patch0Adj =
            seamVertex1.patches[idxVertPatch0Adj];
        const TexCoord p0Adj(patch0Adj.proj - offset);
        // find the other patch sharing the same edge (edge with same adjacent
        // vertex)
        FOREACH(idxVertPatch1, seamVertex0.patches) {
          ...
          // select the same edge belonging to the second patch leaving from the
          // adjacent vertex
          const uint32_t idxVertPatch1Adj(
              seamVertex1.patches.Find(patch1.idxPatch));
          ASSERT(idxVertPatch1Adj != SeamVertex::Patches::NO_INDEX);
          const SeamVertex::Patch &patch1Adj =
              seamVertex1.patches[idxVertPatch1Adj];
          ...
          struct RasterPatch {
            ...
            const float length;
            const Sampler sampler;
            inline RasterPatch(Image32F3 &_image, Image8U &_mask,
                               const Image32F3 &_image0,
                               const Image8U3 &_image1, const TexCoord &_p0,
                               const TexCoord &_p0Adj, const TexCoord &_p1,
                               const TexCoord &_p1Adj)
                : image(_image), mask(_mask), image0(_image0), image1(_image1),
                  p0(_p0), p0Dir(_p0Adj - _p0), p1(_p1), p1Dir(_p1Adj - _p1),
            ...
            inline void operator()(const ImageRef &pt) {
              ...
              const Color color0(
                  image0.sample<Sampler, Color>(sampler, samplePos0));
              const TexCoord samplePos1(p1 + p1Dir * l);
              const Color color1(
                  image1.sample<Sampler, Color>(sampler, samplePos1) / 255.f);
          ...
          // as a manifold edge is shared by maximum two face (one in each
          // patch), which we found already
          break;
        }
      }
      ...
      for (const SeamVertex::Patch &patch : seamVertex0.patches) {
        ...
        accumColor.Add(img.sample<Sampler, Color>(sampler, patch.proj) / 255.f,
                       1.f);
      }
...
void MeshTexture::GenerateTexture(bool bGlobalSeamLeveling,
                                  ...
                                  int maxTextureSize) {
  // project patches in the corresponding view and compute texture-coordinates
  // and bounding-box
  const int border(2);
...
#pragma omp parallel for schedule(dynamic)
  ...
  for (TexturePatch *pTexturePatch = texturePatches.Begin(),
       ...
       pTexturePatch < pTexturePatchEnd; ++pTexturePatch) {
    ...
    texturePatch.rect.width =
        CEIL2INT(aabb.ptMax[0] - aabb.ptMin[0]) + border * 2;
    texturePatch.rect.height =
  ...
  if (texturePatches.size() > 2 &&
      (bGlobalSeamLeveling || bLocalSeamLeveling)) {
    // create seam vertices and edges
    CreateSeamVertices();
    ...
    if (bGlobalSeamLeveling) {
      TD_TIMER_STARTD();
      GlobalSeamLeveling();
      DEBUG_ULTIMATE("\tglobal seam leveling completed (%s)",
    ...
    if (bLocalSeamLeveling) {
      TD_TIMER_STARTD();
      LocalSeamLeveling();
      DEBUG_ULTIMATE("\tlocal seam leveling completed (%s)",
                     TD_TIMER_GET_FMT().c_str());
    }
  }
  ...
  for (unsigned i = 0; i < texturePatches.size() - 1; ++i) {
    ...
    for (unsigned j = 1; j < texturePatches.size(); ++j) {
      ...
      if (texturePatchBig.label != texturePatchSmall.label)
        continue;
      if (!RectsBinPack::IsContainedIn(texturePatchSmall.rect,
                                       texturePatchBig.rect))
        continue;
      // translate texture coordinates
      const TexCoord offset(texturePatchSmall.rect.tl() -
                            texturePatchBig.rect.tl());
  ...
  // create texture
  {
    ...
    FOREACH(i, texturePatches) {
      if (maxTextureSize > 0 &&
        ...
        DEBUG("error: a patch of size %u x %u does not fit the texture",
              texturePatches[i].rect.width, texturePatches[i].rect.height);
    ...
    CLISTDEF2IDX(RectsBinPack::RectWIdxArr, TexIndex) placedRects;
    {
      ...
      const unsigned typeSplit(
          (nRectPackingHeuristic - typeRectsBinPack * 100) / 10);
      ...
      while (!unplacedRects.empty()) {
        ...
        case 0: {
          ...
          newPlacedRects = pack.Insert(
              unplacedRects,
              (MaxRectsBinPack::FreeRectChoiceHeuristic)typeHeuristic);
          break;
        }
        case 1: {
          SkylineBinPack pack(textureSize, textureSize, typeSplit != 0);
          newPlacedRects =
              pack.Insert(unplacedRects,
        ...
        DEBUG_ULTIMATE("\tpacking texture completed: %u initial patches, %u "
                       "placed patches, %u texture-size, %u textures (%s)",
                       texturePatches.size(), newPlacedRects.size(),
...
#pragma omp parallel for schedule(dynamic)
...
#endif
        const TexturePatch &texturePatch =
            texturePatches[placedRects[idxTexture][idxPlacedPatch].patchIdx];
        const RectsBinPack::Rect &rect =
            placedRects[idxTexture][idxPlacedPatch].rect;
        // copy patch image
    ...
    if (fSharpnessWeight > 0) {
      ...
      for (auto &textureDiffuse : texturesDiffuse) {
        ...
        cv::GaussianBlur(textureDiffuse, blurryTextureDiffuse, cv::Size(),
                         sigma);
        cv::addWeighted(textureDiffuse, 1 + fSharpnessWeight,
                        blurryTextureDiffuse, -fSharpnessWeight, 0,
                        textureDiffuse);
      }
    }
  }
...
// texture mesh
//  - minCommonCameras: generate texture patches using virtual faces composed of
//  coplanar triangles sharing at least this number of views (0 - disabled, 3 -
//  good value)
//  - fSharpnessWeight: sharpness weight to be applied on the texture (0 -
//  disabled, 0.5 - good value)
//  - nIgnoreMaskLabel: label value to ignore in the image mask, stored in the
//  MVS scene or next to each image with '.mask.png' extension (-1 - auto
//  estimate mask for lens distortion, -2 - disabled)
bool Scene::TextureMesh(unsigned nResolutionLevel, unsigned nMinResolution,
                        ...
                        float fRatioDataSmoothness, bool bGlobalSeamLeveling,
                        bool bLocalSeamLeveling, unsigned nTextureSizeMultiple,
                        unsigned nRectPackingHeuristic, Pixel8U colEmpty,
                        float fSharpnessWeight, int nIgnoreMaskLabel,
                        int maxTextureSize, const IIndexArr &views) {
  MeshTexture texture(*this, nResolutionLevel, nMinResolution);

  // assign the best view to each face
  {
    TD_TIMER_STARTD();
    if (!texture.FaceViewSelection(minCommonCameras, fOutlierThreshold,
                                   fRatioDataSmoothness, nIgnoreMaskLabel,
                                   views))
      return false;
    DEBUG_EXTRA("Assigning the best view to each face completed: %u faces, %u "
                "patches (%s)",
                mesh.faces.size(), texture.texturePatches.size(),
                TD_TIMER_GET_FMT().c_str());
  }
  ...
  // generate the texture image and atlas
  {
    ...
    texture.GenerateTexture(bGlobalSeamLeveling, bLocalSeamLeveling,
                            ...
                            colEmpty, fSharpnessWeight, maxTextureSize);
    DEBUG_EXTRA("Generating texture atlas and image completed: %u patches, %u "
                "image size, %u textures (%s)",
                texture.texturePatches.size(), mesh.texturesDiffuse[0].width(),
