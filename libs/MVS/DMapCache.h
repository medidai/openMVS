...
class DMapCache {
public:
  explicit DMapCache(DepthDataArr &arrDepthData, unsigned loadFlags,
                     size_t max_memory_bytes);

  // check if the list is empty
  bool IsEmpty() const {
    ASSERT((usedMemory == 0) == fifo.IsEmpty());
    return fifo.IsEmpty();
  }
  ...
  void DisableMemoryCheck() {
    disabledMaxMemory = maxMemory;
    maxMemory = 0;
  }
  void EnableMemoryCheck() {
    if (disabledMaxMemory) {
      maxMemory = disabledMaxMemory;
      disabledMaxMemory = 0;
      Eject();
    }
  }
  ...
  void SkipMemoryCheckIdxImage(IIndex idxImage = NO_ID) {
    skipMemoryCheckIdxImage = idxImage;
  }
