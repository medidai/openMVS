...
const String keys = "{help h usage ? |        | print this message             "
                    "                                   }"
                    "{GT             |None    | optional ground-truth "
                    "disparity (MPI-Sintel or Middlebury format) }"
                    "{dst_path       |None    | optional path to save the "
                    "resulting filtered disparity map        }"
                    "{dst_raw_path   |None    | optional path to save raw "
                    "disparity map before filtering          }"
                    "{algorithm      |bm      | stereo matching method (bm or "
                    "sgbm)                               }"
                    "{filter         |wls_conf| used post-filtering (wls_conf "
                    "or wls_no_conf)                     }"
                    "{no-display     |        | don't display results          "
                    "                                   }"
                    "{no-downscale   |        | force stereo matching on "
                    "full-sized views to improve quality      }"
                    "{dst_conf_path  |None    | optional path to save the "
                    "confidence map used in filtering        }"
                    "{vis_mult       |1.0     | coefficient used to scale "
                    "disparity map visualizations            }"
...
cv::Rect computeROI(cv::Size2i src_sz,
                    cv::Ptr<cv::StereoMatcher> matcher_instance) {
...
int disparityFiltering(cv::Mat left, cv::Mat right, int argc,
                       const LPCSTR *argv) {
  ...
  if (parser.get<int>("window_size") >= 0) // user provided window_size value
    ...
    if (algo == "sgbm")
      ...
      wsize = 7; // default window size for BM on downscaled views (downscaling
                 // is performed only for wls_conf)
    else
  ...
  if (max_disp <= 0 || max_disp % 16 != 0) {
    std::cout << "Incorrect max_disparity value: it should be positive and "
                 "divisible by 16";
    return -1;
  }
  ...
  if (filter == "wls_conf") // filtering with confidence (significantly better
                            // quality than wls_no_conf)
  {
    if (!no_downscale) {
      // downscale the views to speed-up the matching stage, as we will need to
      // compute both left and right disparity maps for confidence map
      // computation
      //! [downscale]
      max_disp /= 2;
      if (max_disp % 16 != 0)
    ...
    if (algo == "bm") {
      ...
      cv::Ptr<cv::StereoBM> left_matcher =
          cv::StereoBM::create(max_disp, wsize);
      ...
      matching_time =
          ((double)cv::getTickCount() - matching_time) / cv::getTickFrequency();
      //! [matching]
    } else if (algo == "sgbm") {
      cv::Ptr<cv::StereoSGBM> left_matcher =
          cv::StereoSGBM::create(-max_disp / 2 + 1, max_disp, wsize);
  ...
  */
  ...
  if (!noGT) {
    ...
    percent_bad_before =
        cv::ximgproc::computeBadPixelPercent(GT_disp, left_disp, ROI);
    ...
    std::cout << "Percent of bad pixels before filtering: "
              << percent_bad_before << std::endl;
    std::cout << "Percent of bad pixels after filtering:  " << percent_bad_after
              << std::endl;
  }
...
void PrefilterXSobel(const cv::Mat &src, cv::Mat &dst, int ftzero = 31) {
  ...
  for (int x = 0; x < TABSZ; x++)
    tab[x] = (uint8_t)(x - OFS < -ftzero  ? 0
                       : x - OFS > ftzero ? ftzero * 2
                                          : x - OFS + ftzero);
  ...
  for (y = 0; y < size.height - 1; y += 2) {
    ...
    const uint8_t *srow0 = y > 0             ? srow1 - src.step
                           : size.height > 1 ? srow1 + src.step
                                             : srow1;
    const uint8_t *srow2 = y < size.height - 1 ? srow1 + src.step
                           : size.height > 1   ? srow1 - src.step
                                               : srow1;
    ...
    if (useSIMD) {
      ...
      for (; x <= size.width - 9; x += 8) {
        __m128i c0 =
            _mm_unpacklo_epi8(_mm_loadl_epi64((__m128i *)(srow0 + x - 1)), z);
        __m128i c1 =
            _mm_unpacklo_epi8(_mm_loadl_epi64((__m128i *)(srow1 + x - 1)), z);
        __m128i d0 =
            _mm_unpacklo_epi8(_mm_loadl_epi64((__m128i *)(srow0 + x + 1)), z);
        __m128i d1 =
            _mm_unpacklo_epi8(_mm_loadl_epi64((__m128i *)(srow1 + x + 1)), z);
        ...
        __m128i c2 =
            _mm_unpacklo_epi8(_mm_loadl_epi64((__m128i *)(srow2 + x - 1)), z);
        __m128i c3 =
            _mm_unpacklo_epi8(_mm_loadl_epi64((__m128i *)(srow3 + x - 1)), z);
        __m128i d2 =
            _mm_unpacklo_epi8(_mm_loadl_epi64((__m128i *)(srow2 + x + 1)), z);
        __m128i d3 =
            _mm_unpacklo_epi8(_mm_loadl_epi64((__m128i *)(srow3 + x + 1)), z);
        ...
        __m128i v0 =
            _mm_add_epi16(d0, _mm_add_epi16(d2, _mm_add_epi16(d1, d1)));
        __m128i v1 =
            _mm_add_epi16(d1, _mm_add_epi16(d3, _mm_add_epi16(d2, d2)));
...
public:
  ...
  EVTPixelProcess(cv::Size s, volatile Thread::safe_t &idx, FncPixel f)
      : Event(EVT_JOB), size(s), idxPixel(idx), fncPixel(f) {}
};
...
public:
  ...
  EVTPixelAccumInc(int s, volatile Thread::safe_t &idx, FncPixel f)
      : Event(EVT_JOB), numPixels(s), idxPixel(idx), fncPixel(f) {}
};
...
public:
  ...
  EVTPixelAccumDec(volatile Thread::safe_t &idx, FncPixel f)
      : Event(EVT_JOB), idxPixel(idx), fncPixel(f) {}
};
/*----------------------------------------------------------------*/

// S T R U C T S ///////////////////////////////////////////////////

// - P1 and P2s are algorithm constants very similar to those from the original
// SGM algorithm;
//   they are set to defaults according to the patch size
// - alpha & beta form the final P2 as P2*(1+alpha*e^(-DI^2/(2*beta^2)))
//   where DI is the difference in image intensity I(x)-I(x_prev) in [0,255]
//   range
// - subpixelSteps represents how much sub-pixel accuracy is searched/stored;
//   if 1 no sub-pixel precision, if for example 4 a 0.25 sub-pixel accuracy is
//   stored; the stored value is quantized and represented as integer:
//   val=(float)valStored/subpixelSteps
SemiGlobalMatcher::SemiGlobalMatcher(SgmSubpixelMode _subpixelMode,
                                     Disparity _subpixelSteps, AccumCost _P1,
                                     AccumCost P2, float P2alpha, float P2beta)
    : subpixelMode(_subpixelMode), subpixelSteps(_subpixelSteps), P1(_P1),
      P2s(GenerateP2s(P2, P2alpha, P2beta)) {}

SemiGlobalMatcher::~SemiGlobalMatcher() {}

CLISTDEF0IDX(SemiGlobalMatcher::AccumCost, int)
SemiGlobalMatcher::GenerateP2s(AccumCost P2, float P2alpha, float P2beta) {
  CLISTDEF0IDX(AccumCost, int) P2s(256);
  FOREACH(i, P2s)
  P2s[i] = (AccumCost)ROUND2INT(
      P2 * (1.f + P2alpha * EXP(-SQUARE((float)i) / (2.f * SQUARE(P2beta)))));
  return P2s;
}
...
void SemiGlobalMatcher::Match(const Scene &scene, IIndex idxImage,
                              IIndex numNeighbors, unsigned minResolution) {
  const Image &leftImage = scene.images[idxImage];
  const float fMinScore(
      MAXF(leftImage.neighbors.front().score * OPTDENSE::fViewMinScoreRatio,
  ...
  FOREACH(idxNeighbor, leftImage.neighbors) {
    ...
    const String pairName(MAKE_PATH(
        String::FormatString("%04u_%04u", leftImage.ID, rightImage.ID)));
    if (File::isPresent((pairName + ".dimap").c_str()) ||
        File::isPresent(MAKE_PATH(String::FormatString(
            "%04u_%04u.dimap", rightImage.ID, leftImage.ID))))
      continue;
    TD_TIMER_STARTD();
    IndexArr points;
    Matrix3x3 H;
    ...
    MaskMap leftMaskMap, rightMaskMap;
    {
      ...
      FOREACH(idxPoint, scene.pointcloud.points) {
        const PointCloud::ViewArr &views =
            scene.pointcloud.pointViews[idxPoint];
      ...
      if (!Image::StereoRectifyImages(leftImage, rightImage, leftPoints,
                                      rightPoints, leftData.imageColor,
                                      rightData.imageColor, leftMaskMap,
                                      rightMaskMap, H, Q))
        continue;
    ...
    const LPCSTR argv[] = {"disparityFiltering", "-algorithm=sgbm",
                           "-max_disparity=160", "-no-downscale"};
    disparityFiltering(leftData.imageColor, rightData.imageColor,
                       (int)SizeOfArray(argv), argv);
#endif
// color to gray conversion
#if SGM_SIMILARITY == SGM_SIMILARITY_CENSUS
    leftData.imageColor.toGray(leftData.imageGray, cv::COLOR_BGR2GRAY, false,
                               true);
    rightData.imageColor.toGray(rightData.imageGray, cv::COLOR_BGR2GRAY, false,
                                true);
#else
    leftData.imageColor.toGray(leftData.imageGray, cv::COLOR_BGR2GRAY, true,
                               true);
    rightData.imageColor.toGray(rightData.imageGray, cv::COLOR_BGR2GRAY, true,
                                true);
#endif
    ...
    if (minResolution) {
      ...
      Image8U::computeMaxResolution(leftData.imageGray.width(),
                                    leftData.imageGray.height(),
                                    resolutionLevel, minResolution);
      scale = REAL(1) / MAXF(2, POWI(2, resolutionLevel));
    }
    const bool tSGM(!ISEQUAL(scale, REAL(1)));
    DisparityMap leftDisparityMap, rightDisparityMap;
    AccumCostMap costMap;
    do {
#if 0
      ...
      const cv::Size sizeValid(size.width - 2 * halfWindowSizeX,
                               size.height - 2 * halfWindowSizeY);
      const bool bFirstLevel(leftDisparityMap.empty());
      if (bFirstLevel) {
        // initialize the disparity-map with a rough estimate based on the
        // sparse point-cloud
        // TODO: remove DepthData::ViewData dependency
        Image leftImageLevel(
            leftImage.GetImage(scene.platforms, scale * 0.5, false));
        ...
        DepthMap depthMap;
        Depth dMin, dMax;
        TriangulatePoints2DepthMap(image, scene.pointcloud, points, depthMap,
                                   dMin, dMax, true);
        points.Release();
        Matrix3x3 H2(H);
        Matrix4x4 Q2(Q);
        ...
        const cv::Size sizeValidHalf(sizeHalf.width - 2 * halfWindowSizeX,
                                     sizeHalf.height - 2 * halfWindowSizeY);
        ...
        const cv::Rect ROI(halfWindowSizeX, halfWindowSizeY, sizeValid.width,
                           sizeValid.height);
      ...
      if (tSGM) {
        ...
        numCosts =
            Disparity2RangeMap(rightDisparityMap, rightMaskMap,
                               bFirstLevel ? 11 : 5, bFirstLevel ? 33 : 7);
      } else {
        // extract global min and max disparities
        Range range{std::numeric_limits<Disparity>::max(),
                    std::numeric_limits<Disparity>::min()};
      ...
      if (tSGM) {
        numCosts =
            Disparity2RangeMap(leftDisparityMap, leftMaskMap,
                               bFirstLevel ? 11 : 5, bFirstLevel ? 33 : 7);
        imageCosts.resize(numCosts);
        imageAccumCosts.resize(numCosts);
      } else {
...
// check disparity-map cross-consistency
...
#endif
      if (bFirstLevel) {
        // perform a rigorous filtering of the estimated disparity maps in order
        // to estimate the common region or interest and set the validity masks
        ConsistencyCrossCheck(leftDisparityMap, rightDisparityMap);
        ConsistencyCrossCheck(rightDisparityMap, leftDisparityMap);
        cv::filterSpeckles(leftDisparityMap, NO_DISP, OPTDENSE::nSpeckleSize,
                           5);
        cv::filterSpeckles(rightDisparityMap, NO_DISP, OPTDENSE::nSpeckleSize,
                           5);
    ...
    DEBUG_EXTRA("Disparity-map for images %3u and %3u: %dx%d (%s)",
                leftImage.ID, rightImage.ID, leftImage.width, leftImage.height,
                TD_TIMER_GET_FMT().c_str());
    ExportPointCloud(pairName + ".ply", leftImage, leftDisparityMap, Q,
                     subpixelSteps);
    ExportDisparityMap(pairName + ".png", leftDisparityMap);
    ExportDisparityDataRawFull(pairName + ".dimap", leftDisparityMap, costMap,
                               leftImage.GetSize(), H, Q, subpixelSteps);
#else
    // convert disparity-map to final depth-map for the left image
    DepthMap depthMap(leftImage.image.size());
    ConfidenceMap confMap;
    Disparity2DepthMap(leftDisparityMap, costMap, H, Q, subpixelSteps, depthMap,
                       confMap);
    DEBUG_EXTRA("Depth-map for images %3u and %3u: %dx%d (%s)", leftImage.ID,
                rightImage.ID, depthMap.width(), depthMap.height(),
                TD_TIMER_GET_FMT().c_str());
    ExportDepthMap(pairName + ".png", depthMap);
    MVS::ExportPointCloud(pairName + ".ply", leftImage, depthMap, NormalMap());
    ExportDepthDataRaw(pairName + ".dmap", leftImage.name,
                       IIndexArr{leftImage.ID, rightImage.ID},
                       leftImage.GetSize(), leftImage.camera.K,
                       leftImage.camera.R, leftImage.camera.C, 0, FLT_MAX,
                       depthMap, NormalMap(), confMap);
#endif
  }
}

void SemiGlobalMatcher::Fuse(const Scene &scene, IIndex idxImage,
                             IIndex numNeighbors, unsigned minViews,
                             DepthMap &depthMap, ConfidenceMap &confMap) {
  TD_TIMER_STARTD();
  const Image &leftImage = scene.images[idxImage];
  const float fMinScore(
      MAXF(leftImage.neighbors.front().score * OPTDENSE::fViewMinScoreRatio,
  ...
  FOREACH(idxNeighbor, leftImage.neighbors) {
    ...
    AccumCostMap costMap;
    if (!ImportDisparityDataRawFull(
            MAKE_PATH(String::FormatString("%04u_%04u.dimap", leftImage.ID,
                                           rightImage.ID)),
            disparityMap, costMap, imageSize, H, Q, subpixelSteps)) {
      if (!ImportDisparityDataRawFull(
              MAKE_PATH(String::FormatString("%04u_%04u.dimap", rightImage.ID,
                                             leftImage.ID)),
              disparityMap, costMap, imageSize, H, Q, subpixelSteps)) {
        DEBUG("warning: no disparity-data file found for image pair (%u,%u)",
      ...
      RMatrix poseR;
      CMatrix poseC;
      ComputeRelativePose(rightImage.camera.R, rightImage.camera.C,
                          leftImage.camera.R, leftImage.camera.C, poseR, poseC);
      Matrix4x4 P(Matrix4x4::IDENTITY);
      AssembleProjectionMatrix(leftImage.camera.K, poseR, poseC,
                               reinterpret_cast<Matrix3x4 &>(P));
      Matrix4x4 invK(Matrix4x4::IDENTITY);
      cv::Mat(rightImage.camera.GetInvK())
          .copyTo(cv::Mat(4, 4, cv::DataType<Matrix4x4::Type>::type,
                          invK.val)(cv::Rect(0, 0, 3, 3)));
      Q = P * invK * Q;
    }
    // convert disparity-map to final depth-map for the left image
    PairData &pair = pairs.emplace_back(leftImage.image.size());
    if (!ProjectDisparity2DepthMap(disparityMap, costMap, Q, subpixelSteps,
                                   pair.depthMap, pair.depthRangeMap,
                                   pair.confMap)) {
      pairs.RemoveLast();
      continue;
    }
    ...
    if (VERBOSITY_LEVEL > 3) {
      const String pairName(MAKE_PATH(
          String::FormatString("depth%04u_%04u", leftImage.ID, rightImage.ID)));
  ...
  // fuse available depth-maps such that for each pixel set its depth as the
  // average of the largest cluster of agreeing depths; pixel depths values
  // agree if their trust regions overlap
  depthMap.create(leftImage.image.size());
  ...
  for (int r = 0; r < depthMap.rows; ++r) {
    for (int c = 0; c < depthMap.cols; ++c) {
      ...
      const Cluster &cluster =
          clusters.GetMax([](const Cluster &i, const Cluster &j) {
            return i.views.size() < j.views.size();
          });
  ...
  if (VERBOSITY_LEVEL > 2) {
    const String fileName(
        MAKE_PATH(String::FormatString("depth%04u", leftImage.ID)));
...
void SemiGlobalMatcher::Match(const ViewData &leftImage,
                              const ViewData &rightImage,
                              DisparityMap &disparityMap,
                              AccumCostMap &costMap) {
  ...
  // compute costs
  {
    ...
    const float eps(
        1e-3f); // used suppress the effect of noise in untextured regions
    auto pixel = [&](int idx, int r, int c) {
      ...
      struct Compute {
        ...
        static float WeightColor(const Image8U3 &image, const Pixel8U &center,
                                 const ImageRef &x) {
          // color weight [0..1]
        ...
        static float WeightSpatial(int x, int y) {
          // spatial weight [0..1]
          static const float sigmaSpatial(
              -1.f /
              (2.f * SQUARE(0.4f * MAXF<int>(windowSizeX, windowSizeY))));
      ...
      for (int i = -halfWindowSizeY; i <= halfWindowSizeY; ++i) {
        for (int j = -halfWindowSizeX; j <= halfWindowSizeX; ++j) {
          ...
          w.normSq0 +=
              ...
              (pw.weight = EXP(
                   Compute::WeightColor(leftImage.imageColor, colCenter, x) +
                   Compute::WeightSpatial(j, i)));
      ...
      for (int d = pixel.range.minDisp; d < pixel.range.maxDisp; ++d) {
        ...
        }
        {
          ...
          *costs++ =
              (ncc <= 0 ? Cost(255)
                        : (Cost)ROUND2INT((1.f - MINF(ncc, 1.f)) * 255.f));
        }
      NEXT_COST:;
      }
#endif
  ...
  // accumulate costs
  {
    ...
    auto pixelAccum = [&](const Cost *costs, const LineData &Lp, LineData &Ls,
                          AccumCost *accums, ImageGray::Type DI) {
      struct Compute {
        static inline void MINS(AccumCost &m, AccumCost v) {
          if (m > v)
            m = v;
        }
      };
      ...
      if (minDisp >= maxDisp) {
        ...
        for (const AccumCost *L = Lp.L + (minDisp - Lp.R.minDisp),
                             *endL = L + (maxDisp - minDisp);
             L < endL; ++L)
  ...
  do {                                                                         \
    ...
    lines.NextLine();                                                          \
      ...
      { // width-down
        ...
        FOREACH(i, threads)
        threads.AddEvent(
            new EVTPixelAccumInc(sizeValid.width, idxPixel, pixels));
        WaitThreadWorkers(threads.size());
      }
      { // height-right
        auto pixels = [&](int y) {
        ...
        FOREACH(i, threads)
        threads.AddEvent(
            new EVTPixelAccumInc(sizeValid.height, idxPixel, pixels));
        WaitThreadWorkers(threads.size());
      }
      { // width-up
        auto pixels = [&](int x) {
      ...
      if (numDirs == 4) {
        ...
        { // width-left-down
          ...
          FOREACH(i, threads)
          threads.AddEvent(
              new EVTPixelAccumInc(sizeValid.width - 1, idxPixel, pixels));
        }
        { // height-left-down
          auto pixels = [&](int y) {
            ImageRef u(sizeValid.width - 1, y);
...
#undef ACCUM_PIXELS
      ...
      struct AccumLines {
        ...
        AccumLines(Disparity _maxNumDisp)
            : maxNumDisp(_maxNumDisp), linesBuffer(NULL) {}
        ...
        const LineData &operator()(int idxDir, int r, int c) const {
          return lines[idxDir][r][c];
        }
        LineData &operator()(int idxDir, int r, int c) {
          return lines[idxDir][r][c];
        }
      };
      AccumLines lines(maxNumDisp);
#define ACCUM_PIXELS(dx, dy, _x)                                               \
  ...
  for (int idxDir = 0; idxDir < numDirs; ++idxDir) {                           \
    ...
    const ImageGray::Type DI(                                                  \
        leftImage.imageGray(r, c) -                                            \
        (leftImage.imageGray.isInside(xp) ? leftImage.imageGray(xp) : Igray)); \
  ...
  // select best disparity and cost
  {
    ...
    auto pixel = [&](int idx) {
      ...
      if (pixel.range.isValid()) {
        ...
        for (const AccumCost *accum = accums + 1,
                             *accumEnd = accums + pixel.range.numDisp();
             accum < accumEnd; ++accum) {
          if (*bestAccum > *accum)
            bestAccum = accum;
        }
        disparityMap(idx) =
            pixel.range.minDisp + (Disparity)(bestAccum - accums);
...
void SemiGlobalMatcher::CensusTransform(const Image8U &imageGray,
                                        CensusMap &imageCensus) {
...
// Compute search range from the given disparity-map and setup pixel-map at
// twice the scale; the validity mask-map is considered as well and upscaled in
// the same time; return the total size of the disparities searched
SemiGlobalMatcher::Index SemiGlobalMatcher::Disparity2RangeMap(
    const DisparityMap &disparityMap, const MaskMap &maskMap,
    Disparity minNumDisp, Disparity minNumDispInvalid) {
  ASSERT(!disparityMap.empty() && disparityMap.width() < maskMap.width() &&
         disparityMap.height() < maskMap.height());
  ...
  for (int r = 0; r < disparityMap.rows; ++r) {
    ...
    const Mask *pm(
        maskMap.ptr<const Mask>(r * 2 + halfWindowSizeY, halfWindowSizeX));
    for (int c = 0, c2 = 0; c < disparityMap.cols; ++c, pm += 2) {
      ...
      if (*pm == INVALID) {
        ...
        // set search range
        if (disps.size() < 3) {
          range.maxDisp = MINF((Disparity)(disparityMap.width() * 2 / 3),
                               minNumDispInvalid);
          ...
          if (numDisp < minNumDisp) {
            ...
            if (numDisp > maxNumDisp) {
              range.minDisp =
                  disp - (maxNumDisp * (disp - minmax.first * 2) + 1) / numDisp;
              range.maxDisp =
                  disp +
                  (maxNumDisp * (minmax.second * 2 + 1 - disp) + 1) / numDisp;
              numDisp = range.numDisp();
            } else {
              range.minDisp = disp - numDisp / 2;
              range.maxDisp = disp + (numDisp + 1) / 2;
    ...
    const int _offsete((r + 1 == disparityMap.rows
                            ? size2x.height
                            : r * 2 + halfWindowSizeY + 2) *
                       size2x.width);
    for (int _offset = offset + size2x.width; _offset < _offsete;
         _offset += size2x.width) {
...
// Check for consistency between a left-to-right and right-to-left pair of
// stereo results; the results are expected to be opposite in sign but equal in
// magnitude; the valid disparities are returned in the left map
void SemiGlobalMatcher::ConsistencyCrossCheck(DisparityMap &l2r,
                                              const DisparityMap &r2l,
                                              Disparity thCross) {
  ASSERT(thCross >= 0);
  ...
  auto pixel = [&](int, int r, int c) {
    ...
    if (ld == NO_DISP)
      return;
    // compute the corresponding disparity pixel according to the disparity
    // value
...
void SemiGlobalMatcher::FilterByCost(DisparityMap &disparityMap,
                                     const AccumCostMap &costMap,
                                     AccumCost th) {
  ASSERT(th > 0);
...
//  thValid is the number of valid disparities encountered to consider the valid
//  region starts
void SemiGlobalMatcher::ExtractMask(const DisparityMap &disparityMap,
                                    MaskMap &maskMap, int thValid) {
...
#define MASK_PIXEL()                                                           \
  ...
  // left-right direction
  {
    ...
    if (!threads.empty()) {
      ...
      FOREACH(i, threads)
      threads.AddEvent(
          new EVTPixelAccumInc(disparityMap.height(), idxPixel, pixel));
      WaitThreadWorkers(threads.size());
    } else
      for (int r = 0; r < disparityMap.rows; ++r)
        pixel(r);
...
// by translating to the x coordinated dictated by the disparity and negating
// the sign; the sub-pixel steps is assumed to be one and the input
// disparity-map has been cross-checked for consistency
void SemiGlobalMatcher::FlipDirection(const DisparityMap &l2r,
                                      DisparityMap &r2l) {
  ASSERT(!l2r.empty());
  ...
  auto pixel = [&](int r) {
    for (int c = 0; c < l2r.cols; ++c) {
      ...
      if (d == NO_DISP)
        continue;
      // compute the corresponding disparity pixel according to the disparity
      // value and set right disparity value
      for (int x = MAXF(c + d - 1, 0), xe = MINF(c + d + 2, r2l.width());
           x < xe; ++x)
        r2l(r, x) = -d;
    }
  };
...
// Translate disparity-map between left-to-right and right-to-left stereo pair
// by translating to the x coordinated dictated by the disparity and negating
// the sign; the input disparity-map has been cross-checked for consistency
...
void SemiGlobalMatcher::RefineDisparityMap(DisparityMap &disparityMap) const {
  ...
  if (subpixelMode == SUBPIXEL_NA) {
    ...
    auto pixel = [&](int, int r, int c) {
      ...
      if (d == NO_DISP)
        return;
      ASSERT((int)d * subpixelSteps >
             (int)std::numeric_limits<Disparity>::min());
      ASSERT((int)d * subpixelSteps <
             (int)std::numeric_limits<Disparity>::max());
      d *= subpixelSteps;
    };
    ASSERT(threads.IsEmpty());
    if (!threads.empty()) {
      ...
      FOREACH(i, threads)
      threads.AddEvent(
          new EVTPixelProcess(disparityMap.size(), idxPixel, pixel));
      WaitThreadWorkers(threads.size());
    } else
      for (int r = 0; r < disparityMap.rows; ++r)
        for (int c = 0; c < disparityMap.cols; ++c)
  ...
  typedef float real;
  struct Fit {
    static real linear(real x) { return x / real(2); }
    static real poly4(real x) { return (x * x * x * x + x) / real(4); }
    static real parabola(real x) { return x / (x + real(1)); }
    static real sine(real x) {
      return real(0.5) * (SIN((x - real(1)) * real(HALF_PI)) + real(1));
    }
    static real cosine(real x) { return (real(1) - COS(x * (real)(PI / 3.0))); }
    ...
    static real semisubpixel(AccumCost primary, AccumCost other) {
      return real(0.5) *
             (static_cast<real>(primary) / static_cast<real>(other));
    }
    // compute the offset to be added to the integer disparity to get the final
    // result
    static real subpixelMode(AccumCost prev, AccumCost center, AccumCost next,
                             SgmSubpixelMode subpixelMode) {
      ASSERT(prev != NO_ACCUMCOST && center != NO_ACCUMCOST &&
             next != NO_ACCUMCOST);
      // use a lower quality two value interpolation if only two values are
      // available
      if (prev == center)
      ...
      real value(0);
      switch (subpixelMode) {
      case SUBPIXEL_LINEAR:
        value = linear(x);
        break;
      case SUBPIXEL_POLY4:
        value = poly4(x);
        break;
      case SUBPIXEL_PARABOLA:
        value = parabola(x);
        break;
      case SUBPIXEL_SINE:
        value = sine(x);
        break;
      case SUBPIXEL_COSINE:
        value = cosine(x);
        break;
      case SUBPIXEL_LC_BLEND:
        value = lcBlend(x);
        break;
      };
  ...
  auto pixel = [&](int idx) {
    ...
    if (d == pixel.range.minDisp)
      ...
      disparity += Fit::subpixelMode(accums[idxDisp - 1], accums[idxDisp],
                                     accums[idxDisp + 1], subpixelMode);
    ASSERT(ROUND2INT(disparity * subpixelSteps) >
           (int)std::numeric_limits<Disparity>::min());
    ASSERT(ROUND2INT(disparity * subpixelSteps) <
           (int)std::numeric_limits<Disparity>::max());
  ...
  if (!threads.empty()) {
    ...
    FOREACH(i, threads)
    threads.AddEvent(
        new EVTPixelAccumInc(disparityMap.size().area(), idxPixel, pixel));
    WaitThreadWorkers(threads.size());
  } else
    for (int r = 0; r < disparityMap.rows; ++r)
      for (int c = 0; c < disparityMap.cols; ++c)
...
void SemiGlobalMatcher::DisplayState(const cv::Size &size) const {
  ...
  for (int idx = 0; idx < size.area(); ++idx) {
    const PixelData &pixel = imagePixels[idx];
    disparity(idx) = (uint8_t)(CLAMP(pixel.range.avgDisp(), Disparity(-128),
                                     Disparity(127)) +
                               Disparity(128));
    range(idx) =
        (uint8_t)CLAMP(pixel.range.numDisp(), Disparity(0), Disparity(255));
  }
...
// Compute the disparity-map for the rectified image from the given depth-map of
// the un-rectified image; the disparity map needs to be already constructed at
// the desired size (valid size, excluding the border)
void SemiGlobalMatcher::Depth2DisparityMap(const DepthMap &depthMap,
                                           const Matrix3x3 &invH,
                                           const Matrix4x4 &invQ,
                                           Disparity subpixelSteps,
                                           DisparityMap &disparityMap) {
  auto pixel = [&](int, int r, int c) {
    const ImageRef x(c + halfWindowSizeX, r + halfWindowSizeY);
    Point2f u;
    ProjectVertex_3x3_2_2(invH.val, x.ptr(), u.ptr());
    float depth, disparity;
    if (!depthMap.sampleSafe(depth, u, [](Depth d) { return d > 0; }) ||
        !Image::Depth2Disparity(invQ, u, depth, disparity))
...
// Compute the depth-map for the un-rectified image from the given disparity-map
// of the rectified image
void SemiGlobalMatcher::Disparity2DepthMap(
    const DisparityMap &disparityMap, const AccumCostMap &costMap,
    const Matrix3x3 &H, const Matrix4x4 &Q, Disparity subpixelSteps,
    DepthMap &depthMap, ConfidenceMap &confMap) {
  ...
  if (!costMap.empty()) {
    ...
    auto pixel = [&](int, int r, int c) {
      ...
      float disparity;
      if (!disparityMap.sampleSafe(disparity, u,
                                   [](Disparity d) { return d != NO_DISP; })) {
        depthMap(x) = 0;
        confMap(x) = 0;
        return;
      }
      float cost;
      costMap.sampleSafe(cost, u,
                         [](AccumCost c) { return c != NO_ACCUMCOST; });
...
// Compute the depth-map for the un-rectified image from the given disparity-map
// of the rectified image by projecting the point-cloud to the image and setting
// the pixel depth as the average of the closest 4 points; return false if the
// disparity map is completely empty
bool SemiGlobalMatcher::ProjectDisparity2DepthMap(
    const DisparityMap &disparityMap, const AccumCostMap &costMap,
    const Matrix4x4 &Q, Disparity subpixelSteps, DepthMap &depthMap,
    DepthRangeMap &depthRangeMap, ConfidenceMap &confMap) {
  ASSERT(costMap.empty() || costMap.size() == disparityMap.size());
  ASSERT(!depthMap.empty());
  ASSERT(confMap.empty() || confMap.size() == depthMap.size());
  ...
  for (int r = 0; r < depthMap.rows; ++r) {
    for (int c = 0; c < depthMap.cols; ++c) {
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        acc.Add(Vec4f((float)depth, depthRange.x, depthRange.y,
                      depthData.confMap(r, c)),
                SQRT(depthData.distMap(r, c)));
      }
      ASSERT(!acc.IsEmpty());
...
bool SemiGlobalMatcher::ExportDisparityDataRaw(
    const String &fileName, const DisparityMap &disparityMap,
    const AccumCostMap &costMap, const cv::Size &imageSize, const Matrix3x3 &H,
    const Matrix4x4 &Q, Disparity subpixelSteps) {
  ASSERT(!disparityMap.empty());
...
bool SemiGlobalMatcher::ExportDisparityDataRawFull(
    const String &fileName, const DisparityMap &disparityMap,
    const AccumCostMap &costMap, const cv::Size &imageSize, const Matrix3x3 &H,
    const Matrix4x4 &Q, Disparity subpixelSteps) {
  ASSERT(!disparityMap.empty());
  ...
  const cv::Size size(disparityMap.width() + 2 * halfWindowSizeX,
                      disparityMap.height() + 2 * halfWindowSizeY);
  const cv::Rect ROI(halfWindowSizeX, halfWindowSizeY, disparityMap.width(),
                     disparityMap.height());
  DisparityMap disparityMapFull(size, NO_DISP);
  disparityMap.copyTo(disparityMapFull(ROI));
  if (costMap.empty())
    return ExportDisparityDataRaw(fileName, disparityMapFull, costMap,
                                  imageSize, H, Q, subpixelSteps);
  AccumCostMap costMapFull(size, NO_ACCUMCOST);
  costMap.copyTo(costMapFull(ROI));
  return ExportDisparityDataRaw(fileName, disparityMapFull, costMapFull,
                                imageSize, H, Q, subpixelSteps);
} // ExportDisparityDataRawFull

bool SemiGlobalMatcher::ImportDisparityDataRaw(
    const String &fileName, DisparityMap &disparityMap, AccumCostMap &costMap,
    cv::Size &imageSize, Matrix3x3 &H, Matrix4x4 &Q, Disparity &subpixelSteps) {
...
bool SemiGlobalMatcher::ImportDisparityDataRawFull(
    const String &fileName, DisparityMap &disparityMap, AccumCostMap &costMap,
    cv::Size &imageSize, Matrix3x3 &H, Matrix4x4 &Q, Disparity &subpixelSteps) {
  if (!ImportDisparityDataRaw(fileName, disparityMap, costMap, imageSize, H, Q,
                              subpixelSteps))
    return false;
  const cv::Size sizeValid(disparityMap.width() - 2 * halfWindowSizeX,
                           disparityMap.height() - 2 * halfWindowSizeY);
  const cv::Rect ROI(halfWindowSizeX, halfWindowSizeY, sizeValid.width,
                     sizeValid.height);
...
// export disparity-map as an image (red - maximum disparity, blue - minimum
// disparity)
Image8U3 SemiGlobalMatcher::DisparityMap2Image(const DisparityMap &disparityMap,
  ...
  if (minDisparity == NO_DISP || maxDisparity == NO_DISP) {
    ...
    if (!disparities.empty()) {
      const std::pair<float, float> th(ComputeX84Threshold<Disparity, float>(
          disparities.data(), disparities.size(), 5.2f));
  ...
  // create color image
  Image8U3 img(cv::Size(disparityMap.width() + 2 * halfWindowSizeX,
                        disparityMap.height() + 2 * halfWindowSizeY),
               Pixel8U::BLACK);
  for (int r = 0; r < disparityMap.rows; ++r) {
    for (int c = 0; c < disparityMap.cols; ++c) {
      ...
      if (disparity == NO_DISP)
        continue;
      img(r + halfWindowSizeY, c + halfWindowSizeX) = Pixel8U::gray2color(
          CLAMP((float)(maxDisparity - disparity) * sclDepth, 0.f, 1.f));
    }
  }
  return img;
} // DisparityMap2Image
bool SemiGlobalMatcher::ExportDisparityMap(const String &fileName,
                                           const DisparityMap &disparityMap,
                                           Disparity minDisparity,
                                           Disparity maxDisparity) {
  if (disparityMap.empty())
    return false;
  return DisparityMap2Image(disparityMap, minDisparity, maxDisparity)
      .Save(fileName);
} // ExportDisparityMap

// export point-cloud
bool SemiGlobalMatcher::ExportPointCloud(const String &fileName,
  ...
  for (int r = 0; r < disparityMap.rows; ++r) {
    for (int c = 0; c < disparityMap.cols; ++c) {
      ...
      if (disparity == NO_DISP)
        continue;
      Point2f u;
      Depth depth(Image::Disparity2Depth(
          Q, ImageRef(c + halfWindowSizeX, r + halfWindowSizeY),
          (float)disparity / subpixelSteps, u));
      if (depth <= 0)
        continue;
...
bool SemiGlobalMatcher::ImportPointCloud(const String &fileName,
  ...
  DisparityMap disparityMap;
  AccumCostMap costMap;
  if (!ImportDisparityDataRawFull(fileName, disparityMap, costMap, imageSize, H,
                                  Q, subpixelSteps))
    return false;
  // parse image index from the file name
  const String name(Util::getFileName(fileName));
  IIndex idxImage(NO_ID), idxImagePair(NO_ID);
  if (sscanf(name, "%u_%u", &idxImage, &idxImagePair) != 2 ||
      idxImage == NO_ID || idxImagePair == NO_ID)
    return false;
  ...
  for (int r = 0; r < disparityMap.rows; ++r) {
    for (int c = 0; c < disparityMap.cols; ++c) {
      ...
      if (!imageData.image.isInsideWithBorder<float, 1>(u))
        continue;
      pointcloud.points.emplace_back(Cast<PointCloud::Point::Type>(
          imageData.camera.TransformPointI2W(Point3(u, depth))));
      pointcloud.colors.emplace_back(
          imageData.image.empty() ? Pixel8U::WHITE : imageData.image.sample(u));
    }
  }
  return true;
} // ImportPointCloud
...
bool MVS::STEREO::ExportCamerasEngin(const Scene &scene,
                                     const String &fileName) {
  ...
  for (const Image &image : scene.images) {
    ...
    f.print("%u %u %u %s "
            ...
            image.camera.K(0, 0), image.camera.K(1, 1), image.camera.K(0, 2),
            image.camera.K(1, 2), image.camera.R(0, 0), image.camera.R(0, 1),
            image.camera.R(0, 2), image.camera.R(1, 0), image.camera.R(1, 1),
            image.camera.R(1, 2), image.camera.R(2, 0), image.camera.R(2, 1),
            image.camera.R(2, 2), t.x, t.y, t.z, 0, 0, image.neighbors.size());
