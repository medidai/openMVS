...
public:
  ...
  enum : Mask {
    INVALID = 0,
    VALID = DECLARE_NO_INDEX(Mask)
  }; // invalid/valid mask value
  enum : Disparity {
    NO_DISP = DECLARE_NO_INDEX(Disparity)
  }; // invalid disparity value
  enum : AccumCost {
    NO_ACCUMCOST = DECLARE_NO_INDEX(AccumCost)
  }; // invalid accumulated cost value
  ...
  typedef CLISTDEF0IDX(Cost,
                       Index) CostsMap; // map of pre-computed disparity costs
  typedef CLISTDEF0IDX(AccumCost, Index)
      AccumCostsMap; // map of accumulated disparity costs
...
#endif
  enum : int {
    windowSizeX = halfWindowSizeX * 2 + 1,
    windowSizeY = halfWindowSizeY * 2 + 1,
    numTexels = windowSizeX * windowSizeY
  }; // patch kernel info
...
#else
  typedef WeightedPatchFix<numTexels>
      WeightedPatch; // pre-computed patch weights
  typedef Image32F
      ImageGray; // used to store normalized float intensities image
#endif
  ...
  struct ViewData {
    ...
    ViewData GetImage(REAL scale) const {
      ...
      ASSERT(scale < 1);
      ViewData scaledView;
      cv::resize(imageColor, scaledView.imageColor, cv::Size(), scale, scale,
                 cv::INTER_AREA);
      cv::resize(imageGray, scaledView.imageGray, cv::Size(), scale, scale,
                 cv::INTER_AREA);
#if SGM_SIMILARITY == SGM_SIMILARITY_CENSUS
...
public:
  SemiGlobalMatcher(SgmSubpixelMode subpixelMode = SUBPIXEL_LC_BLEND,
                    Disparity subpixelSteps = 4, AccumCost P1 = 3,
                    AccumCost P2 = 4, float P2alpha = 14, float P2beta = 38);
  ~SemiGlobalMatcher();

  void Match(const Scene &scene, IIndex idxImage, IIndex numNeighbors,
             unsigned minResolution = 320);
  void Fuse(const Scene &scene, IIndex idxImage, IIndex numNeighbors,
            unsigned minViews, DepthMap &depthMap, ConfidenceMap &confMap);
  ...
  static bool ExportDisparityDataRaw(const String &fileName,
                                     const DisparityMap &, const AccumCostMap &,
                                     const cv::Size &imageSize,
                                     const Matrix3x3 &H, const Matrix4x4 &Q,
                                     Disparity subpixelSteps);
  static bool ExportDisparityDataRawFull(const String &fileName,
                                         const DisparityMap &,
                                         const AccumCostMap &,
                                         const cv::Size &imageSize,
                                         const Matrix3x3 &H, const Matrix4x4 &Q,
                                         Disparity subpixelSteps);
  static bool ImportDisparityDataRaw(const String &fileName, DisparityMap &,
                                     AccumCostMap &, cv::Size &imageSize,
                                     Matrix3x3 &H, Matrix4x4 &Q,
                                     Disparity &subpixelSteps);
  static bool ImportDisparityDataRawFull(const String &fileName, DisparityMap &,
                                         AccumCostMap &, cv::Size &imageSize,
                                         Matrix3x3 &H, Matrix4x4 &Q,
                                         Disparity &subpixelSteps);
  static Image8U3 DisparityMap2Image(const DisparityMap &,
                                     Disparity minDisparity = NO_DISP,
                                     Disparity maxDisparity = NO_DISP);
  static bool ExportDisparityMap(const String &fileName, const DisparityMap &,
                                 Disparity minDisparity = NO_DISP,
                                 Disparity maxDisparity = NO_DISP);
  static bool ExportPointCloud(const String &fileName, const Image &,
                               const DisparityMap &, const Matrix4x4 &Q,
                               Disparity subpixelSteps);
  static bool ImportPointCloud(const String &fileName, const ImageArr &images,
                               PointCloud &);

protected:
  void Match(const ViewData &leftImage, const ViewData &rightImage,
             DisparityMap &disparityMap, AccumCostMap &costMap);
  Index Disparity2RangeMap(const DisparityMap &disparityMap,
                           const MaskMap &maskMap, Disparity minNumDisp = 3,
                           Disparity minNumDispInvalid = 16);
#if SGM_SIMILARITY == SGM_SIMILARITY_CENSUS
  static void CensusTransform(const Image8U &imageGray, CensusMap &imageCensus);
#endif
  static void ConsistencyCrossCheck(DisparityMap &l2r, const DisparityMap &r2l,
                                    Disparity thCross = 1);
  ...
  static CLISTDEF0IDX(AccumCost, int)
      GenerateP2s(AccumCost P2, float P2alpha, float P2beta);
  static void Depth2DisparityMap(const DepthMap &, const Matrix3x3 &invH,
                                 const Matrix4x4 &invQ, Disparity subpixelSteps,
                                 DisparityMap &);
  static void Disparity2DepthMap(const DisparityMap &, const AccumCostMap &,
                                 const Matrix3x3 &H, const Matrix4x4 &Q,
                                 Disparity subpixelSteps, DepthMap &,
                                 ConfidenceMap &);
  static bool ProjectDisparity2DepthMap(const DisparityMap &,
                                        const AccumCostMap &,
                                        const Matrix4x4 &Q,
                                        Disparity subpixelSteps, DepthMap &,
                                        DepthRangeMap &, ConfidenceMap &);
