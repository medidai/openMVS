...
Planef PointCloud::EstimateGroundPlane(const ImageArr &images,
  ...
  if (GetSize() > randMinPoints) {
    ...
    SEACAVE::Random rnd;
#endif
    const REAL randPointsRatio(
        MAXF(REAL(1e-4), (REAL)randMinPoints / GetSize()));
    const SEACAVE::Random::result_type randPointsTh(
        CEIL2INT<SEACAVE::Random::result_type>(randPointsRatio *
                                               SEACAVE::Random::max()));
  ...
  const unsigned numInliers(planeThreshold > 0
                                ? EstimatePlaneTh(*pPoints, plane, threshold)
                                : EstimatePlane(*pPoints, plane, threshold));
  if (numInliers <
      MINF(ROUND2INT<unsigned>(minInliersRatio * pPoints->size()), 1000u)) {
    plane.Invalidate();
    return plane;
  }
  if (planeThreshold <= 0)
  ...
  // make sure the plane is well oriented, negate plane normal if it faces same
  // direction as cameras on average
  if (!images.empty()) {
    FloatArr cosView(0, images.size());
    for (const Image &imageData : images) {
      if (!imageData.IsValid())
        continue;
      cosView.push_back(plane.m_vN.dot(
          (const Point3f::EVecMap &)Cast<float>(imageData.camera.Direction())));
    }
...
struct Vertex {
  ...
  float scale;
  static void
  InitLoadProps(PLY &ply, int elem_count, PointCloud::PointArr &points,
                PointCloud::ColorArr &colors, PointCloud::NormalArr &normals,
                PointCloud::LabelArr &labels, PointCloud::PointViewArr &views,
                PointCloud::PointWeightArr &weights) {
    ...
    for (size_t p = 0; p < nMaxProps; ++p) {
      ...
      case 6:
        ...
        break;
      case 9:
        views.resize((IDX)elem_count);
        break;
      case 10:
        weights.resize((IDX)elem_count);
        break;
      case 11:
        labels.resize((IDX)elem_count);
        break;
      }
    }
  }
  static void InitSaveProps(PLY &ply, int elem_count, bool bColors,
                            bool bNormals, bool bViews, bool bWeights,
                            bool bLabel = false, bool bConfidence = false,
                            bool bScale = false) {
...
const PLY::PlyProperty Vertex::props[17] = {
    ...
    {"view_indices", PLY::Uint32, PLY::Uint32, offsetof(Vertex, views.pIndices),
     1, PLY::Uint8, PLY::Uint8, offsetof(Vertex, views.num)},
    {"view_weights", PLY::Float32, PLY::Float32,
     offsetof(Vertex, views.pWeights), 1, PLY::Uint8, PLY::Uint8,
     offsetof(Vertex, views.num)},
    {"label", PLY::Uint8, PLY::Uint8, offsetof(Vertex, label), 0, 0, 0, 0},
    {"confidence", PLY::Float32, PLY::Float32, offsetof(Vertex, confidence), 0,
     0, 0, 0},
    {"value", PLY::Float32, PLY::Float32, offsetof(Vertex, scale), 0, 0, 0, 0},
    // duplicates
    {"diffuse_red", PLY::Uint8, PLY::Uint8, offsetof(Vertex, c.r), 0, 0, 0, 0},
    {"diffuse_green", PLY::Uint8, PLY::Uint8, offsetof(Vertex, c.g), 0, 0, 0,
     0},
    {"diffuse_blue", PLY::Uint8, PLY::Uint8, offsetof(Vertex, c.b), 0, 0, 0,
     0}};
} // namespace BasicPLY
} // namespace PointCloudInternal
...
bool PointCloud::Load(const String &fileName) {
  ...
  for (int i = 0; i < ply.get_elements_count(); i++) {
    ...
    if (PLY::equal_strings(BasicPLY::elem_names[0], elem_name)) {
      BasicPLY::Vertex::InitLoadProps(ply, elem_count, points, colors, normals,
                                      labels, pointViews, pointWeights);
  ...
  DEBUG_EXTRA("Point-cloud '%s' loaded: %u points (%s)",
              Util::getFileNameExt(fileName).c_str(), points.size(),
              TD_TIMER_GET_FMT().c_str());
  return true;
} // Load

// save the dense point-cloud as PLY file
bool PointCloud::Save(const String &fileName, bool bViews, bool bLegacyTypes,
                      bool bBinary) const {
  if (IsEmpty())
    return false;
  TD_TIMER_STARTD();
  ...
  // write the header
  BasicPLY::Vertex::InitSaveProps(
      ply, (int)points.size(), !colors.empty(), !normals.empty(),
  ...
  DEBUG_EXTRA("Point-cloud '%s' saved: %u points (%s)",
              Util::getFileNameExt(fileName).c_str(), points.GetSize(),
              TD_TIMER_GET_FMT().c_str());
  return true;
} // Save

// save the dense point-cloud having >=N views as PLY file
bool PointCloud::SaveNViews(const String &fileName, uint32_t minViews,
                            bool bLegacyTypes, bool bBinary) const {
  if (points.IsEmpty())
    return false;
  TD_TIMER_STARTD();
  ...
  if (bLegacyTypes)
    ply.set_legacy_type_names();
  if (!ply.write(fileName, 1, BasicPLY::elem_names,
                 bBinary ? PLY::BINARY_LE : PLY::ASCII, 64 * 1024))
    return false;
  ...
  DEBUG_EXTRA("Point-cloud saved: %u points with at least %u views each (%s)",
              numPoints, minViews, TD_TIMER_GET_FMT().c_str());
  return true;
} // SaveNViews

// save the dense point-cloud + scale as PLY file
bool PointCloud::SaveWithScale(const String &fileName, const ImageArr &images,
                               float scaleMult, bool bLegacyTypes,
                               bool bBinary) const {
  if (points.empty())
    return false;
  ...
  // export the array of 3D points
  BasicPLY::Vertex::InitSaveProps(ply, (int)points.size(), !colors.empty(),
                                  !normals.empty(), false, false, false, true,
                                  true);
  if (!ply.header_complete())
    return false;
  BasicPLY::Vertex vertex;
  FOREACH(i, points) {
    ...
    if (pointWeights.empty()) {
      ...
      for (IIndex idxView : pointViews[i]) {
        const float scale((float)images[idxView].camera.GetFootprintWorld(
            Cast<REAL>(vertex.p)));
        ASSERT(scale > 0);
  ...
  DEBUG_EXTRA("Point-cloud saved: %u points with scale (%s)", points.size(),
              TD_TIMER_GET_FMT().c_str());
  return true;
} // SaveWithScale
/*----------------------------------------------------------------*/
...
void PointCloud::PrintStatistics(const Image *pImages,
                                 const OBB3f *pObb) const {
  String strPoints;
  if (pObb && pObb->IsValid()) {
    ...
    if (!pointViews.empty()) {
      strPoints += String::FormatString(
          ...
          accInside.minVal, accInside.GetMean(), accInside.GetStdDev(),
          accInside.maxVal, accOutside.minVal, accOutside.GetMean(),
          accOutside.GetStdDev(), accOutside.maxVal);
    }
  }
  String strViews;
  if (!pointViews.empty()) {
    ...
    strViews = String::FormatString(
        ...
        nPointsOpposedViews
            ? String::FormatString(" (%u (%.2f%%) points with opposed views)",
                                   nPointsOpposedViews,
                                   100.f * nPointsOpposedViews / GetSize())
                  .c_str()
            : "",
        nPoints1m, 100.f * nPoints1m / GetSize(), nPoints2,
        100.f * nPoints2 / GetSize(), nPoints3, 100.f * nPoints3 / GetSize(),
        nPoints4p, 100.f * nPoints4p / GetSize(), acc.minVal, acc.GetMean(),
        acc.GetStdDev(), acc.maxVal);
  }
  String strNormals;
  if (!normals.empty()) {
    if (!pointViews.empty() && pImages != NULL) {
      ...
      size_t nViews(0);
      size_t nPointsm(0), nPoints3(0), nPoints10(0), nPoints25(0), nPoints40(0),
          nPoints60(0), nPoints90p(0);
      ...
      strNormals = String::FormatString(
          ...
          nPointsm, 100.f * nPointsm / nViews, nPoints3,
          100.f * nPoints3 / nViews, nPoints10, 100.f * nPoints10 / nViews,
          nPoints40, 100.f * nPoints40 / nViews, nPoints60,
          100.f * nPoints60 / nViews, nPoints90p, 100.f * nPoints90p / nViews);
    } else {
      strNormals = "\n - normals info";
    }
  }
  ...
  VERBOSE("Point-cloud composed of %u points with:%s%s%s%s", GetSize(),
          strPoints.c_str(), strViews.c_str(), strNormals.c_str(),
          strWeights.c_str(), strColors.c_str());
} // PrintStatistics
/*----------------------------------------------------------------*/
