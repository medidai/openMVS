...
@brief Implements different bin packer algorithms that use the MAXRECTS, SKYLINE
and GUILLOTINE data structures.
...
// implements the MAXRECTS data structure and different bin packing algorithms
...
public:
  ...
  enum FreeRectChoiceHeuristic {
    RectBestShortSideFit =
        0, ///< -BSSF: Positions the rectangle against the short side of a free
    ...
    RectContactPointRule, ///< -CP: Chooses the placement where the rectangle
  ...
  /// rotated.
  /// @param rects [IN/OUT] The list of rectangles to insert; the rectangles
  /// will be modified with the new coordinates in the process.
  /// @param method The rectangle placement rule to use when packing.
  /// returns true if all rectangles were inserted
  RectWIdxArr Insert(RectWIdxArr &rects,
                     FreeRectChoiceHeuristic method = RectBestShortSideFit);
...
// GUILLOTINE data structure to keep track of the free space of the bin where
// rectangles may be placed
...
// implements bin packing algorithms that use the SKYLINE data structure to
// store the bin contents; uses GuillotineBinPack as the waste map
...
public:
  ...
  /// Defines the different heuristic rules that can be used to decide how to
  /// make the rectangle placements.
  ...
  /// Inserts the given list of rectangles in an offline/batch mode, possibly
  /// rotated.
  /// @param rects [in/out] The list of rectangles to insert. This vector will
  /// be update in the process.
  /// @param method The rectangle placement rule to use when packing.
  ...
  Rect ScoreRect(int width, int height, LevelChoiceHeuristic method,
                 int &score1, int &score2, int &index) const;
...
protected:
  ...
  /// Represents a single level (a horizontal line) of the
  /// skyline/horizon/envelope.
  struct SkylineNode {
  ...
  /// If true, we use the GuillotineBinPack structure to recover wasted areas
  /// into a waste map.
  bool useWasteMap;
  ...
  Rect FindPositionForNewNodeMinWaste(int width, int height, int &bestHeight,
                                      int &bestWastedArea,
                                      int &bestIndex) const;
  Rect FindPositionForNewNodeBottomLeft(int width, int height, int &bestHeight,
                                        int &bestWidth, int &bestIndex) const;

  bool RectangleFits(int skylineNodeIndex, int width, int height, int &y) const;
  bool RectangleFits(int skylineNodeIndex, int width, int height, int &y,
                     int &wastedArea) const;
  int ComputeWastedArea(int skylineNodeIndex, int width, int height,
                        int y) const;
