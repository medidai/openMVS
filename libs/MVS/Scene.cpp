...
bool Scene::IsEmpty() const { return pointcloud.IsEmpty() && mesh.IsEmpty(); }
...
bool Scene::LoadInterface(const String &fileName) {
  ...
  for (const Interface::Platform &itPlatform : obj.platforms) {
    ...
    for (const Interface::Platform::Camera &itCamera : itPlatform.cameras) {
      ...
      if (!itCamera.IsNormalized()) {
        ...
        camera.K =
            camera.GetScaledK(REAL(1) / Camera::GetNormalizationScale(
                                            itCamera.width, itCamera.height));
      }
      DEBUG_EXTRA(
          "Camera model loaded: platform %u; camera %2u; f %.3fx%.3f; poses %u",
          platforms.size() - 1, platform.cameras.size() - 1, camera.K(0, 0),
          camera.K(1, 1), itPlatform.poses.size());
    }
  ...
  for (const Interface::Image &image : obj.images) {
    ...
    // init camera
    const Interface::Platform::Camera &camera =
        obj.platforms[image.platformID].cameras[image.cameraID];
    ...
    // init neighbors
    imageData.neighbors.CopyOf(image.viewScores.data(),
                               (uint32_t)image.viewScores.size());
    imageData.avgDepth = image.avgDepth;
    ++nCalibratedImages;
    nTotalPixels += imageData.width * imageData.height;
    DEBUG_ULTIMATE("Image loaded %3u: %s", ID,
                   Util::getFileNameExt(imageData.name).c_str());
  }
  if (images.size() < 2)
    return false;
  ...
  if (!obj.vertices.empty()) {
    ...
    if (!obj.verticesNormal.empty()) {
      ...
      pointcloud.normals.CopyOf((const Point3f *)&obj.verticesNormal[0].n,
                                obj.vertices.size());
    }
    if (!obj.verticesColor.empty()) {
      ASSERT(obj.vertices.size() == obj.verticesColor.size());
      pointcloud.colors.CopyOf((const Pixel8U *)&obj.verticesColor[0].c,
                               obj.vertices.size());
    }
  }

  // import region of interest
  obb.Set(Matrix3x3f(obj.obb.rot), Point3f(obj.obb.ptMin),
          Point3f(obj.obb.ptMax));

  // import transform
  transform = obj.transform;

  DEBUG_EXTRA("Scene loaded in interface format from '%s' (%s):\n"
              "\t%u images (%u calibrated) with a total of %.2f MPixels (%.2f "
              "MPixels/image)\n"
              "\t%u points, %u vertices, %u faces",
              Util::getFileNameExt(fileName).c_str(),
              TD_TIMER_GET_FMT().c_str(), images.size(), nCalibratedImages,
              (double)nTotalPixels / (1024.0 * 1024.0),
              (double)nTotalPixels / (1024.0 * 1024.0 * nCalibratedImages),
              pointcloud.points.size(), mesh.vertices.size(),
              mesh.faces.size());
  return true;
} // LoadInterface
...
bool Scene::SaveInterface(const String &fileName, int version) const {
  ...
  FOREACH(i, images) {
    ...
    if (imageData.IsValid() && imageData.HasResolution()) {
      ...
      if (!platform.cameras[image.cameraID].HasResolution())
        platform.SetFullK(image.cameraID, imageData.camera.K, imageData.width,
                          imageData.height);
    }
    image.viewScores = std::vector<MVS::Interface::Image::ViewScore>(
        imageData.neighbors.begin(), imageData.neighbors.end());
  ...
  FOREACH(i, pointcloud.points) {
    ...
    views.ForEach([&](PointCloud::ViewArr::IDX v) {
      ...
      view.confidence =
          (pointcloud.pointWeights.IsEmpty() ? 0.f
  ...
  if (!ARCHIVE::SerializeSave(
          obj, fileName, version >= 0 ? uint32_t(version) : MVSI_PROJECT_VER))
    return false;
...
bool Scene::LoadROI(const String &fileName) {
  ...
  DEBUG_EXTRA("Region-of-interest loaded from file '%s' (%s)", fileName.c_str(),
...
bool Scene::LoadDMAP(const String &fileName) {
  ...
  platform.cameras.emplace_back(
      camera.GetScaledK(
          REAL(1) / CameraIntern::GetNormalizationScale(
                        (uint32_t)imageSize.width, (uint32_t)imageSize.height)),
      RMatrix::IDENTITY, CMatrix::ZERO);
  ...
  for (int r = 0; r < depthMap.rows; ++r) {
    for (int c = 0; c < depthMap.cols; ++c) {
      ...
      pointcloud.points.emplace_back(
          camera.TransformPointI2W(Point3(c, r, depth)));
      pointcloud.pointViews.emplace_back(PointCloud::ViewArr{0});
      pointcloud.colors.emplace_back(imageColor(r, c));
      if (!normalMap.empty())
        pointcloud.normals.emplace_back(Cast<PointCloud::Normal::Type>(
            camera.R.t() * Cast<REAL>(normalMap(r, c))));
      if (!confMap.empty())
        pointcloud.pointWeights.emplace_back(
            PointCloud::WeightArr{confMap(r, c)});
    }
  }
  ...
  if (VERBOSITY_LEVEL > 2) {
    ...
    ExportPointCloud(ComposeDepthFilePath(image.ID, "ply"), image, depthMap,
                     normalMap);
  ...
  DEBUG_EXTRA(
      "Scene loaded from depth-map format - %dx%d size, %.2f%%%% coverage "
      "(%s):\n"
      "\t1 images (%u neighbors, %.2f FOV) with a total of %.2f MPixels (%.2f "
      "MPixels/image)\n"
      "\t%u points, 0 lines",
      depthMap.width(), depthMap.height(),
      100.0 * pointcloud.GetSize() / depthMap.area(),
      TD_TIMER_GET_FMT().c_str(), IDs.size() - 1, R2D(image.ComputeFOV()),
...
// load a text list of views and their neighbors and assign them to the scene
// images; each line store the view ID followed by the 3+ closest view IDs,
// ordered in decreasing overlap:
//
...
bool Scene::LoadViewNeighbors(const String &fileName) {
  ...
  size_t argc;
  for (SML::const_iterator it = smlImages.begin(); it != smlImages.end();
       ++it) {
    // parse image element
    ...
    FOREACH(i, imageData.neighbors) {
      ...
      imageData.neighbors[i] =
          ViewScore{nID, 0, 1.f, FD2R(15.f), 0.5f, 2.f + (argc - i) * 0.5f};
    }
  }
...
Scene::SCENE_TYPE Scene::Load(const String &fileName, bool bImport) {
  ...
  DEBUG_EXTRA("Scene loaded (%s):\n"
              ...
              "MPixels/image)\n"
              "\t%u points, %u vertices, %u faces",
              TD_TIMER_GET_FMT().c_str(), images.GetSize(), nCalibratedImages,
              (double)nTotalPixels / (1024.0 * 1024.0),
              (double)nTotalPixels / (1024.0 * 1024.0 * nCalibratedImages),
              pointcloud.points.GetSize(), mesh.vertices.GetSize(),
              mesh.faces.GetSize());
  return SCENE_MVS;
#else
  if (bImport && Import(fileName))
...
#pragma omp parallel for
  ...
  FOREACH(ID, images) {
    ...
    const Image &imageData = images[ID];
    unsigned level(0);
    const unsigned nMaxResolution(Image8U::computeMaxResolution(
        imageData.width, imageData.height, level, 0, maxResolution));
    ...
    const cv::Size scaledSize(
        Image8U::computeResize(imageData.GetSize(), scale));
    ...
    FOREACH(idxVertex, mesh.vertices) {
      const Point3f xz(
          camera.TransformPointW2I3(Cast<REAL>(mesh.vertices[idxVertex])));
      if (xz.z <= 0)
        continue;
...
#pragma omp flush(bAbort)
  ...
  FOREACH(idxImage, images) {
    ...
    if (!image.IsValid())
      continue;
    const unsigned imageSize(image.RecomputeMaxResolution(
        OPTDENSE::nResolutionLevel, OPTDENSE::nMinResolution,
    ...
    const String fileName(Util::insertBeforeFileExt(
        baseName, String::FormatString("%04u", image.ID)));
    if ((nType == 2 &&
         ![&]() {
           ...
           return ExportDepthDataRaw(fileName, image.name, IDs, image.GetSize(),
                                     image.camera.K, image.camera.R,
                                     image.camera.C, 0.001f, FLT_MAX, depthMap,
                                     normalMap, ConfidenceMap(), ViewsMap());
         }()) ||
...
bool Scene::EstimateNeighborViewsPointCloud(unsigned maxResolution) {
  ...
  const auto ProjectGridToImage = [&](IIndex idI, IIndex idJ, Depth depth) {
    ...
    for (unsigned r = 0; r < maxResolution; ++r) {
      for (unsigned c = 0; c < maxResolution; ++c) {
        ...
        const Point3 X(
            imageData.camera.TransformPointI2W(Point3(x.x, x.y, depthPerturb)));
        ...
        pointcloud.pointViews.emplace_back(idI < idJ
                                               ? PointCloud::ViewArr{idI, idJ}
  ...
  FOREACH(i, images) {
    ...
    FOREACH(j, images) {
      ...
      Point3 X;
      TriangulatePoint3D(imageData.camera.K, imageData2.camera.K,
                         imageData.camera.R, imageData2.camera.R,
                         imageData.camera.C, imageData2.camera.C, Point2::ZERO,
                         Point2::ZERO, X);
...
// (inspired by: "Multi-View Stereo for Community Photo Collections", Goesele,
// 2007)
//  - nInsideROI: 0 - ignore ROI, 1 - weight more ROI points, 2 - consider only
//  ROI points
bool Scene::SelectNeighborViews(uint32_t ID, IndexArr &points,
                                unsigned nMinViews, unsigned nMinPointViews,
                                float fOptimAngle, unsigned nInsideROI) {
  ASSERT(points.empty());

  // extract the estimated 3D points and the corresponding 2D projections for
  // the reference image
  Image &imageData = images[ID];
  ...
  if (neighbors.empty()) {
    ...
    FOREACH(IDB, images) {
      ...
      if (score.points < 3)
        continue;
      ASSERT(ID != IDB);
      // compute how well the matched features are spread out (image covered
      // area)
      ...
      for (uint32_t idx : points) {
        ...
        const PointCloud::Point &point = pointcloud.points[idx];
        Point2f &ptA =
            projs.emplace_back(imageData.camera.ProjectPointP(point));
      ...
      if (projs.empty())
        continue;
      const float area(ComputeCoveredArea<float, 2, 16, false>(
          (const float *)projs.data(), projs.size(), boundsA.ptr()));
      projs.Empty();
      // store image score
    ...
    if (VERBOSITY_LEVEL > 2) {
      String msg;
      FOREACH(n, neighbors)
      msg += String::FormatString(" %3u(%upts,%.2fscl)", neighbors[n].ID,
                                  neighbors[n].points, neighbors[n].scale);
      VERBOSE("Reference image %3u sees %u views:%s (%u shared points)", ID,
              neighbors.size(), msg.c_str(), nPoints);
    }
#endif
  }
  if (points.size() <= 3 ||
      neighbors.size() < MINF(nMinViews, nCalibratedImages - 1)) {
...
void Scene::SelectNeighborViews(unsigned nMinViews, unsigned nMinPointViews,
                                float fOptimAngle, unsigned nInsideROI) {
#ifdef SCENE_USE_OPENMP
  ...
  FOREACH(idxImage, images) {
    ...
    IndexArr points;
    SelectNeighborViews(idxImage, points, nMinViews, nMinPointViews,
                        fOptimAngle, nInsideROI);
  }
} // SelectNeighborViews
/*----------------------------------------------------------------*/

// keep only the best neighbors for the reference image
bool Scene::FilterNeighborViews(ViewScoreArr &neighbors, float fMinArea,
...
bool Scene::ExportCamerasMLP(const String &fileName,
                             const String &fileNameScene) const {
  ...
  static const char mlp_raster_rot[] =
      " RotationMatrix=\"%0.6g %0.6g %0.6g 0 %0.6g %0.6g %0.6g 0 %0.6g %0.6g "
      "%0.6g 0 0 0 0 1\"/>\n"
  ...
  f.print(mlp_header, Util::getFileName(fileNameScene).c_str(),
  ...
  FOREACH(i, images) {
    ...
    f.print(mlp_raster_pos, Util::getFileName(imageData.name).c_str(),
            -camera.C.x, -camera.C.y, -camera.C.z);
    f.print(mlp_raster_cam, 0, 0, imageData.width, imageData.height,
            camera.K(1, 1) / camera.K(0, 0), camera.K(0, 0), camera.K(0, 2),
            camera.K(1, 2));
    f.print(mlp_raster_rot, camera.R(0, 0), camera.R(0, 1), camera.R(0, 2),
            -camera.R(1, 0), -camera.R(1, 1), -camera.R(1, 2), -camera.R(2, 0),
            -camera.R(2, 1), -camera.R(2, 2),
...
bool Scene::ExportLinesPLY(const String &fileName,
                           ...
                           const Pixel8U *colors, bool bBinary) {
  ...
  const size_t memBufferSize(
      2 * (8 * 3 /*pos*/ + 3 * 3 /*color*/ + 6 /*space*/ + 2 /*eol*/) +
      2048 /*extra size*/);
  PLY ply;
  if (!ply.write(fileName, 2, elem_names, bBinary ? PLY::BINARY_LE : PLY::ASCII,
                 memBufferSize))
    return false;
...
// split the scene in sub-scenes such that each sub-scene surface does not
// exceed the given maximum sampling area; the area is composed of overlapping
// samples from different cameras taking into account the footprint of each
// sample (pixels/unit-length, GSD inverse), including overlapping samples; the
// indirect goals this method tries to achieve are:
//  - limit the maximum number of images in each sub-scene such that the
//  depth-map fusion
//    can load all sub-scene's depth-maps into memory at once
//  - limit in the same time maximum accumulated images resolution (total number
//  of pixels)
//    per sub-scene in order to allow all images to be loaded and processed
//    during mesh refinement
unsigned Scene::Split(ImagesChunkArr &chunks, float maxArea,
                      int depthMapStep) const {
  TD_TIMER_STARTD();
  // gather samples from all depth-maps
  const float areaScale(0.01f);
  typedef cList<Point3f::EVec, const Point3f::EVec &, 0, 4096, uint32_t>
      Samples;
  ...
  Unsigned32Arr imageAreas(images.size());
  {
    ...
    FOREACH(idxImage, images) {
      ...
      const Camera camera(
          imageData.GetCamera(platforms, depthData.depthMap.size()));
      for (int r = (depthData.depthMap.rows % depthMapStep) / 2;
           r < depthData.depthMap.rows; r += depthMapStep) {
        for (int c = (depthData.depthMap.cols % depthMapStep) / 2;
             c < depthData.depthMap.cols; c += depthMapStep) {
          const Depth depth = depthData.depthMap(r, c);
          if (depth <= 0)
            continue;
          const Point3f X(
              Cast<float>(camera.TransformPointI2W(Point3(c, r, depth))));
...
#else
      // try to find a dominant plane, and set the bounding-box center on the
      // plane bottom
  ...
  FOREACH(c, chunks) {
    ...
    const float minImageContributionRatioChunk(maxAreaRatio *
                                               minImageContributionRatio);
  ...
  FOREACH(cSmall, chunks) {
    ...
    const Unsigned32Arr &chunkSmallImageAreas =
        chunkInserter.imagesAreas[cSmall];
    FOREACH(cLarge, chunks) {
      const ImagesChunk &chunkLarge = chunks[cLarge];
      if (chunkLarge.images.size() <= chunkSmall.images.size())
        continue;
      const Unsigned32Arr &chunkLargeImageAreas =
          chunkInserter.imagesAreas[cLarge];
      for (auto it = chunkSmall.images.begin();
           it != chunkSmall.images.end();) {
        const IIndex idxImage(*it);
  ...
  RFOREACH(cSmall, chunks) {
    ...
    if (numLargestCommonImages == 0) {
      DEBUG_ULTIMATE(
          "warning: small chunk can not be merged (%u chunk, %u images)",
    ...
    DEBUG_ULTIMATE(
        "Small chunk merged: %u chunk (%u images) -> %u chunk (%u images)",
        cSmall, chunkSmall.images.size(), idxBestChunk,
        chunkLarge.images.size());
    chunkLarge.aabb.Insert(chunkSmall.aabb);
    chunkLarge.images.insert(chunkSmall.images.begin(),
                             chunkSmall.images.end());
  ...
  DEBUG_EXTRA("Scene split (%g max-area): %u chunks (%s)", maxArea,
...
// split the scene in sub-scenes according to the given chunks array, and save
// them to disk
bool Scene::ExportChunks(const ImagesChunkArr &chunks, const String &path,
                         ARCHIVE_TYPE type) const {
  FOREACH(chunkID, chunks) {
    ...
    // set scene ROI
    subset.obb.Set(OBB3f::MATRIX::Identity(), chunk.aabb.ptMin,
                   chunk.aabb.ptMax);
    // serialize out the current state
    if (!subset.Save(String::FormatString("%s" PATH_SEPARATOR_STR
                                          "scene_%04u.mvs",
                                          path.c_str(), chunkID),
                     type))
      return false;
...
// if the scale is not given, scale it such that the bounding-box has largest
// size 1
...
bool Scene::ScaleImages(unsigned nMaxResolution, REAL scale,
  ...
  FOREACH(idx, images) {
    ...
    if (!folderName.empty()) {
      ...
      image.name = folderName +
                   String::FormatString("%05u%s", image.ID,
...
// compute translation and scale (optional) such that the scene coordinates
// center at 0 and most scene geomatry is in the unit cube ([-0.5,0.5]^3);
// return the transformation matrix that restores the scene to its original
// coordinates
...
void Scene::Transform(const Matrix3x3 &rotation, const Point3 &translation,
                      REAL scale) {
  ...
  mapTransform.topLeftCorner<3, 3>() =
      static_cast<Matrix3x3::CEMatMap>(rotationScale);
  mapTransform.topRightCorner<3, 1>() =
      static_cast<Point3::CEVecMap>(translation);
}
void Scene::Transform(const Matrix3x4 &transform) {
#if 1
  Matrix3x3 mscale, rotation;
  RQDecomp3x3<REAL>(
      cv::Mat(3, 4, cv::DataType<REAL>::type,
              const_cast<REAL *>(transform.val))(cv::Rect(0, 0, 3, 3)),
      mscale, rotation);
  const Point3 translation = transform.col(3);
#else
  Eigen::Matrix<REAL, 4, 4> transform4x4 =
      Eigen::Matrix<REAL, 4, 4>::Identity();
  transform4x4.topLeftCorner<3, 4>() =
      static_cast<const Matrix3x4::CEMatMap>(transform);
...
// transform this scene such that it best aligns with the given scene based on
// the camera positions
bool Scene::AlignTo(const Scene &scene) {
  if (images.size() < 3) {
    DEBUG("error: insufficient number of cameras to perform a similarity "
          "transform alignment");
    return false;
  }
...
// estimate ground plane, transform scene such that it is positioned at origin,
// and compute the volume of the mesh;
...
REAL Scene::ComputeLeveledVolume(float planeThreshold, float sampleMesh,
                                 unsigned upAxis, bool verbose) {
  ASSERT(!mesh.IsEmpty());
  if (planeThreshold >= 0 && !mesh.IsWatertight()) {
    // assume the mesh is opened only at the contact with the ground plane;
    // move mesh such that the ground plane is at the origin so that the volume
    // can be computed
    TD_TIMER_START();
    Planef groundPlane(mesh.EstimateGroundPlane(
        images, sampleMesh, planeThreshold,
        verbose ? MAKE_PATH("ground_plane.ply") : String()));
    if (!groundPlane.IsValid()) {
      VERBOSE("error: can not estimate the ground plane");
      return -1;
    }
    const Point3f up(upAxis == 0 ? 1.f : 0.f, upAxis == 1 ? 1.f : 0.f,
                     upAxis == 2 ? 1.f : 0.f);
    if (groundPlane.m_vN.dot(Point3f::EVec(up)) < 0.f)
      groundPlane.Negate();
    VERBOSE("Ground plane estimated at: (%.2f,%.2f,%.2f) %.2f (%s)",
            groundPlane.m_vN.x(), groundPlane.m_vN.y(), groundPlane.m_vN.z(),
            groundPlane.m_fD, TD_TIMER_GET_FMT().c_str());
    // transform the scene such that the up vector aligns with ground plane
    // normal, and the mesh center projected on the ground plane is at the
    // origin
    const Matrix3x3 rotation(
        RMatrix(Cast<REAL>(up), Cast<REAL>(Point3f(groundPlane.m_vN))).t());
...
void Scene::AddNoiseCameraPoses(float epsPosition, float epsRotation) {
  for (Platform &platform : platforms) {
    for (Platform::Pose &pose : platform.poses) {
      ...
      pose.R = RMatrix(RMatrix::Vec(
                   Point3((epsRotation * Point3::EVec::Random()).eval()))) *
               pose.R;
    }
  }
...
Scene Scene::SubScene(const IIndexArr &idxImages) const {
  ...
  for (IIndex idxImage : idxImages) {
    ...
    const auto platformIt(
        mapPlatforms.emplace(image.platformID, (uint32_t)mapPlatforms.size()));
    ...
    const auto platformCameraIt(mapPlatformCamera.emplace(
        PairIdx(image.platformID, image.cameraID),
        PairIdx(platformID, subPlatform.cameras.size())));
...
// remove all points outside the given bounding-box and keep only the cameras
// that see the remaining points
...
// estimate region-of-interest based on camera positions, directions and sparse
// points scale specifies the ratio of the ROI's diameter
bool Scene::EstimateROI(int nEstimateROI, float scale) {
  ...
  FloatArr x(nCameras), y(nCameras), z(nCameras), nx(nCameras), ny(nCameras),
      nz(nCameras);
  FOREACH(i, cameras) {
  ...
  if (camDirectMeanLen > FSQRT_2 / 2.f && nEstimateROI == 2) {
    VERBOSE("The camera directions mean is unbalanced; the scene will be "
            "considered unbounded (no ROI)");
    return false;
  }
  DEBUG_ULTIMATE("The camera positions median is (%f,%f,%f), directions mean "
                 "and norm are (%f,%f,%f), %f",
                 camCenter.x, camCenter.y, camCenter.z, camDirectMean.x,
                 camDirectMean.y, camDirectMean.z, camDirectMeanLen);
  ...
  const CMatrix sceneCenter =
      camCenter + camShiftCoeff * camDistMed * camDirectMean;
  FOREACH(i, cameras) {
    if (cameras[i].PointDepth(sceneCenter) <= 0 && nEstimateROI == 2) {
      VERBOSE("Found a camera not pointing towards the scene center; the scene "
              "will be considered unbounded (no ROI)");
      return false;
    }
...
#if TD_VERBOSE != TD_VERBOSE_OFF
  if (VERBOSITY_LEVEL > 2) {
    VERBOSE("Set the ROI with the AABB of position (%f,%f,%f) and extent "
            "(%f,%f,%f)",
            obb.m_pos[0], obb.m_pos[1], obb.m_pos[2], obb.m_ext[0],
            obb.m_ext[1], obb.m_ext[2]);
  } else {
...
// returns result of checks if the scene camera positions satisfies tower
// criteria:
//	- cameras fit a long and slim bounding box
//  - majority of cameras focus toward a middle line
bool Scene::ComputeTowerCylinder(Point2f &centerPoint, float &fRadius,
                                 float &fROIRadius, float &zMin, float &zMax,
                                 float &minCamZ, const int towerMode) {
  ...
  if (quality.y / quality.z > 0.6f || quality.x / quality.y < 0.8f) {
    ...
    if (towerMode > 0) {
      DEBUG_ULTIMATE(
          "error: does not seem to be a tower: X(%.2f), Y(%.2f), Z(%.2f)",
          quality.x, quality.y, quality.z);
      return false;
    }
  }
  ...
  centerPoint =
      ((camCenterLine.pt1 + camCenterLine.pt2) * 0.5f).topLeftCorner<2, 1>();
  ...
  cameraDistancesToMiddle[camIdx] =
      (float)norm(cameras2D[camIdx] - centerPoint);
...
size_t Scene::DrawCircle(PointCloud &pc, PointCloud::PointArr &outCircle,
                         const Point3f &circleCenter, const float circleRadius,
                         const unsigned nTargetPoints, const float fStartAngle,
...
PointCloud Scene::BuildTowerMesh(const PointCloud &origPointCloud,
                                 ...
                                 const float fRadius, const float fROIRadius,
                                 const float zMin, const float zMax,
                                 const float minCamZ, bool bFixRadius) {
  const unsigned nTargetDensity(10);
  const unsigned nTargetCircles(ROUND2INT(
      (zMax - zMin) * nTargetDensity)); // how many circles in cylinder
  const float fCircleFrequence(
      (zMax - zMin) / nTargetCircles); // the distance between neighbor circles
  PointCloud towerPC;
  PointCloud::PointArr circlePoints;
  Mesh::VertexVerticesArr meshCircles;
  if (bFixRadius) {
    const unsigned nTargetPoints(
        MAX(10, ROUND2INT(FTWO_PI * fRadius *
    ...
    for (unsigned cIdx = 0; cIdx < nTargetCircles; ++cIdx) {
      const Point3f circleCenter(centerPoint,
                                 zMin + fCircleFrequence *
                                            cIdx); // center point of the circle
      const float fStartAngle(
          fAngleBetweenPoints *
          SEACAVE::random()); // starting angle for the first point
      DrawCircle(towerPC, circlePoints, circleCenter, fRadius, nTargetPoints,
                 fStartAngle, fAngleBetweenPoints);
    ...
    for (unsigned cIdx = 0; cIdx < nTargetCircles; ++cIdx) {
      ...
      if (circleZ < minCamZ) {
        ...
        if (pDistances.size() > 2) {
          ...
          const size_t botIdx(
              MAX(1u, FLOOR2INT<unsigned>(pDistances.size() * 0.5f)));
          float avgTopDistance(0);
          for (size_t i = botIdx; i < topIdx; ++i)
    ...
    if (circleRadii.size() > 2) {
      for (size_t ri = 1; ri < circleRadii.size() - 1; ++ri) {
        ...
        // set current radius as average of the most similar values in the
        // closest 7 neighbors
        if (ri > 2 && ri < circleRadii.size() - 5) {
          ...
          circleRadius =
              ABS(medianRadius - aboveRad) < ABS(medianRadius - belowRad)
                  ? aboveRad
                  : belowRad;
        } else {
    ...
    FOREACH(rIdx, circleRadii) {
      ...
      const Point3f circleCenter(centerPoint,
                                 circleZ); // center point of the circle
      const unsigned nTargetPoints(
          MAX(10, ROUND2INT(FTWO_PI * circleRadius *
                            nTargetDensity))); // how many points on each circle
      const float fAngleBetweenPoints(
          FTWO_PI /
          nTargetPoints); // the angle between neighbor points on the circle
      const float fStartAngle(
          fAngleBetweenPoints *
          SEACAVE::random()); // starting angle for the first point
      DrawCircle(towerPC, circlePoints, circleCenter, circleRadius,
                 nTargetPoints, fStartAngle, fAngleBetweenPoints);
  ...
  if (VERBOSITY_LEVEL > 2) {
    ...
    for (Mesh::VIndex cIdx = 1; cIdx < meshCircles.size(); ++cIdx) {
      if (meshCircles[cIdx - 1].size() > 1 || meshCircles[cIdx].size() > 1) {
        ...
        for (Mesh::VIndex ti = 0, bi = 0;
             ti < topPoints.size() && bi < botPoints.size(); ++ti) {
          do {
          ...
          } while (bi < botPoints.size() &&
                   (ti + 1) * topStep > (bi + 1) * botStep);
          if (topPoints.size() > 1) {
            const Mesh::VIndex &v0(topPoints[ti]);
...
//  - towerMode:  0 - disabled, 1 - replace, 2 - append, 3 - select neighbors, 4
//  - select neighbors and append, <0 - force tower mode
void Scene::InitTowerScene(const int towerMode) {
  float fRadius;
  float fROIRadius;
  float zMax, zMin, minCamZ;
  Point2f centerPoint;
  if (!ComputeTowerCylinder(centerPoint, fRadius, fROIRadius, zMin, zMax,
                            minCamZ, towerMode))
    return;

  // add nTargetPoints points on each circle
  PointCloud towerPC(BuildTowerMesh(pointcloud, centerPoint, fRadius,
                                    fROIRadius, zMin, zMax, minCamZ, false));
  mesh.Release();
  ...
  case 1: // replace
    ...
    VERBOSE("Scene identified as tower-like; replace existing point-cloud with "
            "detected tower point-cloud");
    break;
  case 2: // append
    AppendPointCloud(towerPC);
    VERBOSE("Scene identified as tower-like; append to existing point-cloud "
            "the detected tower point-cloud");
    break;
  case 3: // select neighbors
    ...
    pointcloud.Swap(towerPC);
    VERBOSE("Scene identified as tower-like; only select view neighbors from "
            "detected tower point-cloud");
    break;
  case 4: // select neighbors and append tower points
    ...
    AppendPointCloud(towerPC);
    VERBOSE(
        "Scene identified as tower-like; select view neighbors from detected "
        "tower point-cloud and next append it to existing point-cloud");
    break;
  }
} // InitTowerScene
