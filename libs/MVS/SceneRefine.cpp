...
public:
  ...
  struct RasterMesh : TRasterMesh<RasterMesh> {
    ...
    RasterMesh(const Mesh::VertexArr &_vertices, const Camera &_camera,
               DepthMap &_depthMap, FaceMap &_faceMap, BaryMap &_baryMap)
...
public:
  MeshRefine(Scene &_scene, unsigned _nReduceMemory,
             unsigned _nAlternatePair = true, Real _weightRegularity = 1.5f,
             Real _ratioRigidityElasticity = 0.8f,
             unsigned _nResolutionLevel = 0, unsigned _nMinResolution = 640,
             unsigned nMaxViews = 8, unsigned nMaxThreads = 1);
  ~MeshRefine();
  ...
  void SubdivideMesh(uint32_t maxArea, float fDecimate = 1.f,
                     unsigned nCloseHoles = 15, unsigned nEnsureEdgeSize = 1);

  double ScoreMesh(double *gradients);

  // given a vertex position and a projection camera, compute the projected
  // position and its derivative
  template <typename TP, typename TX, typename T, typename TJ>
  static T ProjectVertex(const TP *P, const TX *X, T *x, TJ *jacobian = NULL);

  static bool IsDepthSimilar(const DepthMap &depthMap, const Point2f &pt,
                             Depth z);
  static void ProjectMesh(const Mesh::VertexArr &vertices,
                          ...
                          BaryMap &baryMap);
  static void ImageMeshWarp(const DepthMap &depthMapA, const Camera &cameraA,
                            const DepthMap &depthMapB, const Camera &cameraB,
                            const Image32F &imageB, Image32F &imageA,
                            BitMatrix &mask);
  static void ComputeLocalVariance(const Image32F &image, const BitMatrix &mask,
                                   TImage<Real> &imageMean,
                                   TImage<Real> &imageVar);
  static float
  ComputeLocalZNCC(const Image32F &imageA, const TImage<Real> &imageMeanA,
                   const TImage<Real> &imageVarA, const Image32F &imageB,
                   const TImage<Real> &imageMeanB,
                   const TImage<Real> &imageVarB, const BitMatrix &mask,
                   TImage<Real> &imageZNCC, TImage<Real> &imageDZNCC);
  static void ComputePhotometricGradient(
      const Mesh::FaceArr &faces, const Mesh::NormalArr &normals,
      const DepthMap &depthMapA, const FaceMap &faceMapA,
      const BaryMap &baryMapA, const Camera &cameraA, const Camera &cameraB,
      const View &viewB, const TImage<Real> &imageDZNCC, const BitMatrix &mask,
      GradArr &photoGrad, UnsignedArr &photoGradNorm, Real RegularizationScale);
  static float
  ComputeSmoothnessGradient1(const Mesh::VertexArr &vertices,
                             const Mesh::VertexVerticesArr &vertexVertices,
                             const BoolArr &vertexBoundary,
                             GradArr &smoothGrad1, VIndex idxStart,
                             VIndex idxEnd);
  static void ComputeSmoothnessGradient2(
      const GradArr &smoothGrad1, const Mesh::VertexVerticesArr &vertexVertices,
      const BoolArr &vertexBoundary, GradArr &smoothGrad2, VIndex idxStart,
      VIndex idxEnd);
  template <typename TYPE> static TYPE *TypePool(TYPE * = NULL);
  template <typename TYPE>
  static inline TImage<TYPE> *ImagePool(TImage<TYPE> *pImage = NULL) {
    return TypePool<TImage<TYPE>>(pImage);
  }
  static inline BitMatrix *BitMatrixPool(BitMatrix *pMask = NULL) {
    return TypePool<BitMatrix>(pMask);
  }

  static void *ThreadWorkerTmp(void *);
  void ThreadWorker();
  void WaitThreadWorkers(size_t nJobs);
  void ThSelectNeighbors(uint32_t idxImage,
...
public:
  const Real weightRegularity;  // a scalar regularity weight to balance between
                                // photo-consistency and regularization terms
  Real ratioRigidityElasticity; // a scalar ratio used to compute the regularity
                                // gradient as a combination of rigidity and
                                // elasticity
  ...
  const unsigned nMinResolution;   // how many times to scale down the images
                                   // before mesh optimization
  const unsigned nReduceMemory; // recompute image mean and variance in order to
                                // reduce memory requirements
  unsigned
      nAlternatePair; // using an image pair alternatively as reference image (0
                      // - both, 1 - alternate, 2 - only left, 3 - only right)
  ...
  Mesh::FaceArr &faces;
  Mesh::VertexVerticesArr
      &vertexVertices; // for each vertex, the list of adjacent vertices
  Mesh::VertexFacesArr
...
// call with an image pointer retrieved earlier to signal that is not needed
// anymore
template <typename TYPE> TYPE *MeshRefine::TypePool(TYPE *pObj) {
...
public:
  ...
  EVTSelectNeighbors(uint32_t _idxImage,
                     std::unordered_set<uint64_t> &_mapPairs,
                     unsigned _nMaxViews)
      : Event(EVT_JOB), idxImage(_idxImage), mapPairs(_mapPairs),
        nMaxViews(_nMaxViews) {}
};
...
public:
  ...
  EVTInitImage(uint32_t _idxImage, Real _scale, Real _sigma)
      : Event(EVT_JOB), idxImage(_idxImage), scale(_scale), sigma(_sigma) {}
};
...
public:
  ...
  EVTProjectMesh(uint32_t _idxImage, const Mesh::FaceIdxArr &_cameraFaces)
      : Event(EVT_JOB), idxImage(_idxImage), cameraFaces(_cameraFaces) {}
};
...
public:
  ...
  EVTProcessPair(uint32_t _idxImageA, uint32_t _idxImageB)
      : Event(EVT_JOB), idxImageA(_idxImageA), idxImageB(_idxImageB) {}
};
...
public:
  ...
  EVTSmoothVertices1(VIndex _idxStart, VIndex _idxEnd)
      : Event(EVT_JOB), idxStart(_idxStart), idxEnd(_idxEnd) {}
};
...
public:
  ...
  EVTSmoothVertices2(VIndex _idxStart, VIndex _idxEnd)
      : Event(EVT_JOB), idxStart(_idxStart), idxEnd(_idxEnd) {}
};
...
MeshRefine::MeshRefine(Scene &_scene, unsigned _nReduceMemory,
                       unsigned _nAlternatePair, Real _weightRegularity,
                       Real _ratioRigidityElasticity,
                       unsigned _nResolutionLevel, unsigned _nMinResolution,
                       unsigned nMaxViews, unsigned nMaxThreads)
    : weightRegularity(_weightRegularity),
      ...
      nReduceMemory(_nReduceMemory), nAlternatePair(_nAlternatePair),
      scene(_scene), faceNormals(_scene.mesh.faceNormals),
      ...
      vertexBoundary(_scene.mesh.vertexBoundary), images(_scene.images) {
...
// compute for each face the projection area as the maximum area in both images
// of a pair (make sure ListCameraFaces() was called before)
void MeshRefine::ListFaceAreas(Mesh::AreaArr &maxAreas) {
  ...
  FOREACH(idxImage, images) {
    ...
    // compute area covered by all vertices (incident faces) viewed by this
    // image
    for (int j = 0; j < faceMap.rows; ++j) {
      for (int i = 0; i < faceMap.cols; ++i) {
        const FIndex idxFace(faceMap(j, i));
        ASSERT((idxFace == NO_ID && views[idxImage].depthMap(j, i) == 0) ||
               (idxFace != NO_ID && views[idxImage].depthMap(j, i) > 0));
        if (idxFace == NO_ID)
          continue;
        ++areas[idxFace];
      }
...
// decimate or subdivide mesh such that for each face there is no image pair in
// which its projection area is bigger than the given number of pixels in both
// images
void MeshRefine::SubdivideMesh(uint32_t maxArea, float fDecimate,
                               unsigned nCloseHoles, unsigned nEnsureEdgeSize) {
  ...
  if (!bNoDecimation) {
    if (fDecimate > 0.f) {
      ...
      // estimate the faces' area that have big projection areas in both images
      // of a pair
      ...
      if (fMedianArea < fMaxArea) {
        ...
        scene.mesh.Clean(MAXF(0.1f, fMedianArea / fMaxArea), 0.f, false,
                         nCloseHoles, 0u, 0.f, false);
        scene.mesh.Clean(1.f, 0.f, false, nCloseHoles, 0u, 0.f, true);
  ...
  if (maxAreas.IsEmpty()) {
    ...
    // estimate the faces' area that have big projection areas in both images of
    // a pair
    ListFaceAreas(maxAreas);
  }

  // subdivide mesh faces if its projection area is bigger than the given number
  // of pixels
...
// make sure there are no edges too small or too long
  ...
  DEBUG_EXTRA("Mesh subdivided: %u/%u -> %u/%u vertices/faces", numVertsOld,
              numFacesOld, vertices.GetSize(), faces.GetSize());
...
double MeshRefine::ScoreMesh(double *gradients) {
  ...
  FOREACHPTR(pPair, pairs) {
    ...
    case 1:
      events.AddEvent(iteration % 2 ? new EVTProcessPair(pPair->j, pPair->i)
                                    : new EVTProcessPair(pPair->i, pPair->j));
      break;
    case 2:
    ...
    default:
      for (int ip = 0; ip < 2; ++ip)
        events.AddEvent(ip ? new EVTProcessPair(pPair->j, pPair->i)
                           : new EVTProcessPair(pPair->i, pPair->j));
    }
  }
  ...
  scoreSmooth = 0;
  const VIndex idxStep((vertices.GetSize() + (VIndex)threads.GetSize() - 1) /
                       (VIndex)threads.GetSize());
  ...
  // set the final gradient as the combination of photometric and smoothness
  // gradients
  if (ratioRigidityElasticity >= 1.f) {
    FOREACH(v, vertices)
    ((Point3d *)gradients)[v] =
        photoGradNorm[v] > 0 ? Cast<double>(photoGrad[v] / photoGradNorm[v] +
                                            smoothGrad2[v] * weightRegularity)
                             : Cast<double>(smoothGrad2[v] * weightRegularity);
  } else {
    // compute smoothing gradient as a combination of level 1 and 2 of the
    // Laplacian operator; (see page 105 of "Stereo and Silhouette Fusion for 3D
    // Object Modeling from Uncalibrated Images Under Circular Motion" C.
    // Hernandez, 2004)
    ...
    ((Point3d *)gradients)[v] =
        photoGradNorm[v] > 0 ? Cast<double>(photoGrad[v] / photoGradNorm[v] +
                                            smoothGrad2[v] * elasticity -
                                            smoothGrad1[v] * rigidity)
                             : Cast<double>(smoothGrad2[v] * elasticity -
                                            smoothGrad1[v] * rigidity);
  }
...
// given a vertex position and a projection camera, compute the projected
// position and its derivative returns the depth
template <typename TP, typename TX, typename T, typename TJ>
...
// check if any of the depths surrounding the given coordinate is similar to the
// given value
bool MeshRefine::IsDepthSimilar(const DepthMap &depthMap, const Point2f &pt,
                                Depth z) {
  ...
  for (int x = 0; x < 2; ++x) {
    for (int y = 0; y < 2; ++y) {
      ...
      if (depth <= 0 ||
          ABS(depth - z) > z * 0.01f /*!IsDepthSimilar(depth, z, 0.01f)*/)
#else
...
void MeshRefine::ProjectMesh(const Mesh::VertexArr &vertices,
                             const Mesh::FaceArr &faces,
                             const Mesh::FaceIdxArr &cameraFaces,
                             const Camera &camera, const Image8U::Size &size,
                             DepthMap &depthMap, FaceMap &faceMap,
...
void MeshRefine::ImageMeshWarp(const DepthMap &depthMapA, const Camera &cameraA,
                               const DepthMap &depthMapB, const Camera &cameraB,
                               const Image32F &imageB, Image32F &imageA,
                               BitMatrix &mask) {
  ASSERT(!imageA.empty());
...
void MeshRefine::ComputeLocalVariance(const Image32F &image,
  ...
  for (int r = HalfSize; r < RowsEnd; ++r) {
    for (int c = HalfSize; c < ColsEnd; ++c) {
      if (!mask(r, c))
        continue;
      imageMean(r, c) = (Real)((imageSum(r + HalfSize + 1, c + HalfSize + 1) -
                                imageSum(r + HalfSize + 1, c - HalfSize) -
                                imageSum(r - HalfSize, c + HalfSize + 1) +
                                imageSum(r - HalfSize, c - HalfSize)) *
                               (1.0 / (double)n));
    }
  }
  DST_Image(imageSum);
  for (int r = HalfSize; r < RowsEnd; ++r) {
    for (int c = HalfSize; c < ColsEnd; ++c) {
      if (!mask(r, c))
        continue;
      const Real sumSq =
          (Real)((imageSumSq(r + HalfSize + 1, c + HalfSize + 1) -
                  imageSumSq(r + HalfSize + 1, c - HalfSize) -
                  imageSumSq(r - HalfSize, c + HalfSize + 1) +
                  imageSumSq(r - HalfSize, c - HalfSize)) *
                 (1.0 / (double)n));
...
float MeshRefine::ComputeLocalZNCC(
    const Image32F &imageA, const TImage<Real> &imageMeanA,
    const TImage<Real> &imageVarA, const Image32F &imageB,
    const TImage<Real> &imageMeanB, const TImage<Real> &imageVarB,
    const BitMatrix &mask, TImage<Real> &imageZNCC, TImage<Real> &imageDZNCC) {
  ASSERT(imageA.size() == mask.size() && imageB.size() == mask.size() &&
         !mask.empty());
  float score(0);
  ...
  for (int r = HalfSize; r < RowsEnd; ++r) {
    for (int c = HalfSize; c < ColsEnd; ++c) {
      if (!mask(r, c))
        continue;
      const Real cv = (Real)((imageABSum(r + HalfSize + 1, c + HalfSize + 1) -
                              imageABSum(r + HalfSize + 1, c - HalfSize) -
                              imageABSum(r - HalfSize, c + HalfSize + 1) +
                              imageABSum(r - HalfSize, c - HalfSize)) *
                             (1.0 / (double)n));
      const Real invSqrtVAVB(Real(1) / SQRT(imageVarA(r, c) * imageVarB(r, c)));
      imageZNCC(r, c) =
          (cv - imageMeanA(r, c) * imageMeanB(r, c)) * invSqrtVAVB;
  ...
  for (int r = HalfSize; r < RowsEnd; ++r) {
    for (int c = HalfSize; c < ColsEnd; ++c) {
      ...
      const Real dZNCC(
          (Real)imageA(r, c) * invSqrtVAVB - (Real)imageB(r, c) * ZNCCinvVB +
          imageMeanB(r, c) * ZNCCinvVB - imageMeanA(r, c) * invSqrtVAVB);
#else
      ...
      for (int i = -HalfSize; i <= HalfSize; ++i) {
        ...
        for (int j = -HalfSize; j <= HalfSize; ++j) {
          ...
          sumB -= ZNCCinvVB;
          sumC +=
              imageMeanB(rw, cw) * ZNCCinvVB - imageMeanA(rw, cw) * invSqrtVAVB;
          ++n;
        }
      }
      if (n == 0)
        continue;
      const Real dZNCC((sumA * imageA(r, c) + sumB * imageB(r, c) + sumC) /
                       (Real)n);
      const Real ZNCC(imageZNCC(r, c));
#endif
...
void MeshRefine::ComputePhotometricGradient(
    const Mesh::FaceArr &faces, const Mesh::NormalArr &normals,
    const DepthMap &depthMapA, const FaceMap &faceMapA, const BaryMap &baryMapA,
    const Camera &cameraA, const Camera &cameraB, const View &viewB,
    const TImage<Real> &imageDZNCC, const BitMatrix &mask, GradArr &photoGrad,
    UnsignedArr &photoGradNorm, Real RegularizationScale) {
  ASSERT(faces.GetSize() == normals.GetSize() && !faces.IsEmpty());
  ASSERT(depthMapA.size() == mask.size() && faceMapA.size() == mask.size() &&
         baryMapA.size() == mask.size() && imageDZNCC.size() == mask.size() &&
         !mask.empty());
  ...
  for (int r = HalfSize; r < RowsEnd; ++r) {
    for (int c = HalfSize; c < ColsEnd; ++c) {
      ...
      // projection Jacobian matrix in the second image of the 3D point on the
      // surface
      MAYBEUNUSED const float depthB(
          ProjectVertex(cameraB.P.val, X.ptr(), xB.ptr(), xJac.val));
      ASSERT(depthB > 0);
      // compute gradient in image B
      const TMatrix<Real, 1, 2> gB(
          viewB.imageGrad.sample<Sampler, View::Grad>(sampler, xB));
      // compute gradient scale
      const Real dZNCC(imageDZNCC(r, c));
      const Real sg((gB * (xJac * (const TMatrix<Real, 3, 1> &)dA))(0) * dZNCC *
                    RegularizationScale / Nd);
...
float MeshRefine::ComputeSmoothnessGradient1(
    ...
    const Mesh::VertexVerticesArr &vertexVertices,
    const BoolArr &vertexBoundary, GradArr &smoothGrad1, VIndex idxStart,
    VIndex idxEnd) {
  ASSERT(!vertices.IsEmpty() &&
         vertices.GetSize() == vertexVertices.GetSize() &&
         vertices.GetSize() == smoothGrad1.GetSize());
  float score(0);
...
// normalized as in "Stereo and Silhouette Fusion for 3D Object Modeling from
// Uncalibrated Images Under Circular Motion" C. Hernandez, 2004
void MeshRefine::ComputeSmoothnessGradient2(
    const GradArr &smoothGrad1, const Mesh::VertexVerticesArr &vertexVertices,
    const BoolArr &vertexBoundary, GradArr &smoothGrad2, VIndex idxStart,
    VIndex idxEnd) {
  ASSERT(!smoothGrad1.IsEmpty() &&
         smoothGrad1.GetSize() == vertexVertices.GetSize() &&
         smoothGrad1.GetSize() == smoothGrad2.GetSize());
  for (VIndex idxV = idxStart; idxV < idxEnd; ++idxV) {
...
void MeshRefine::ThSelectNeighbors(uint32_t idxImage,
  ...
  Scene::FilterNeighborViews(neighbors, fMinArea, fMinScale, fMaxScale,
                             fMinAngle, fMaxAngle, nMaxViews);
  Lock l(cs);
...
void MeshRefine::ThInitImage(uint32_t idxImage, Real scale, Real sigma) {
  ...
  if (!nReduceMemory) {
    ...
    ComputeLocalVariance(img, BitMatrix(img.size(), 0xFF), view.imageMean,
                         view.imageVar);
  }
...
void MeshRefine::ThProjectMesh(uint32_t idxImage,
                               const Mesh::FaceIdxArr &cameraFaces) {
  const Image &imageData = images[idxImage];
  if (!imageData.IsValid())
    return;
...
void MeshRefine::ThProcessPair(uint32_t idxImageA, uint32_t idxImageB) {
  ...
  const float score(ComputeLocalZNCC(imageA, *imageMeanA, *imageVarA, imageAB,
                                     imageMeanAB, imageVarAB, mask, imageZNCC,
                                     imageDZNCC));
#ifdef MESHOPT_TYPEPOOL
  DST_Image(imageZNCC);
  ...
  const Real RegularizationScale(
      (Real)((REAL)(imageDataA.avgDepth * imageDataB.avgDepth) /
             (cameraA.GetFocalLength() * cameraB.GetFocalLength())));
  ComputePhotometricGradient(faces, faceNormals, depthMapA, faceMapA, baryMapA,
                             cameraA, cameraB, viewB, imageDZNCC, mask,
                             _photoGrad, _photoGradNorm, RegularizationScale);
...
void MeshRefine::ThSmoothVertices1(VIndex idxStart, VIndex idxEnd) {
  const float score(ComputeSmoothnessGradient1(
      vertices, vertexVertices, vertexBoundary, smoothGrad1, idxStart, idxEnd));
  Lock l(cs);
  scoreSmooth += score;
}
void MeshRefine::ThSmoothVertices2(VIndex idxStart, VIndex idxEnd) {
  ComputeSmoothnessGradient2(smoothGrad1, vertexVertices, vertexBoundary,
                             smoothGrad2, idxStart, idxEnd);
}
/*----------------------------------------------------------------*/
...
public:
  MeshProblem(MeshRefine &_refine)
      : refine(_refine), params(refine.vertices.GetSize() * 3) {
    // init params
  ...
  bool Evaluate(const double *const parameters, double *cost,
...
// fThPlanarVertex - threshold used to remove vertices on planar patches
// (percentage of the minimum depth, 0 - disable)
bool Scene::RefineMesh(unsigned nResolutionLevel, unsigned nMinResolution,
                       unsigned nMaxViews, float fDecimateMesh,
                       unsigned nCloseHoles, unsigned nEnsureEdgeSize,
                       unsigned nMaxFaceArea, unsigned nScales,
                       float fScaleStep, unsigned nAlternatePair,
                       float fRegularityWeight, float fRatioRigidityElasticity,
                       float fGradientStep, float fThPlanarVertex,
                       unsigned nReduceMemory) {
  if (pointcloud.IsEmpty() && !ImagesHaveNeighbors())
    SampleMeshWithVisibility();

  MeshRefine refine(*this, nReduceMemory, nAlternatePair, fRegularityWeight,
                    fRatioRigidityElasticity, nResolutionLevel, nMinResolution,
                    nMaxViews, nMaxThreads);
  if (!refine.IsValid())
    return false;
  ...
  for (unsigned nScale = 0; nScale < nScales; ++nScale) {
    ...
    refine.SubdivideMesh(nMaxFaceArea, nScale == 0 ? fDecimateMesh : 1.f,
                         nCloseHoles, nEnsureEdgeSize);
    ...
    if (VERBOSITY_LEVEL > 2)
      mesh.Save(MAKE_PATH(
          String::FormatString("MeshRefine%u.ply", nScales - nScale - 1)));
#endif
...
// minimize
...
#endif // MESHOPT_CERES
    {
      ...
      Eigen::Matrix<double, Eigen::Dynamic, 3, Eigen::RowMajor> gradients(
          refine.vertices.GetSize(), 3);
      ...
      for (int iter = 0; iter < iters; ++iter) {
        ...
        refine.ratioRigidityElasticity =
            (iter <= iterStop ? fRatioRigidityElasticity : 1.f);
        const bool bAdaptMesh(iter >= iterStart &&
                              (iter - iterStart) % 3 == 0 && iters - iter > 5);
        // evaluate residuals and gradients
        if (bAdaptMesh)
        ...
        if (bAdaptMesh) {
          // apply gradients and
          // remove planar vertices (small gradient and almost on the center of
          // their surrounding patch)
          ...
          FOREACH(v, refine.vertices) {
            ...
            if (depth < FLT_MAX) {
              ...
              if (!refine.vertexBoundary[v] && (float)gn < th &&
                  norm(refine.smoothGrad1[v]) < th)
        ...
        DEBUG_EXTRA(
            "\t%2d. f: %.5f (%.4e)\tg: %.5f (%.4e - %.4e)\ts: %.3f\tv: %5u",
            iter + 1, cost, cost / refine.vertices.GetSize(), gradients.norm(),
            gradients.norm() / refine.vertices.GetSize(),
            gv / refine.vertices.GetSize(), gstep, numVertsRemoved);
        gstep *= 0.98;
    ...
    if (VERBOSITY_LEVEL > 2)
      mesh.Save(MAKE_PATH(
          String::FormatString("MeshRefined%u.ply", nScales - nScale - 1)));
#endif
  }
