...
MaxRectsBinPack::Rect MaxRectsBinPack::Insert(int width, int height,
...
MaxRectsBinPack::RectWIdxArr
MaxRectsBinPack::Insert(RectWIdxArr &unplacedRects,
                        FreeRectChoiceHeuristic method) {
...
#pragma omp for nowait
      for (int_t i = 0; i < (int_t)unplacedRects.size(); ++i) {
        ...
        Rect newNode(ScoreRect(unplacedRects[i].rect.width,
                               unplacedRects[i].rect.height, method, score1,
                               score2));
        if (score1 < privBestScore1 ||
            (score1 == privBestScore1 && score2 < privBestScore2)) {
...
#pragma omp critical
    ...
    placedRects.Insert(
        RectWIdx{bestNode, unplacedRects[bestRectIndex].patchIdx});
...
MaxRectsBinPack::Rect MaxRectsBinPack::ScoreRect(int width, int height,
                                                 FreeRectChoiceHeuristic method,
                                                 int &score1,
                                                 int &score2) const {
  switch (method) {
  case RectBestShortSideFit:
    return FindPositionForNewNodeBestShortSideFit(width, height, score1,
                                                  score2);
  case RectBestLongSideFit:
...
MaxRectsBinPack::Rect MaxRectsBinPack::FindPositionForNewNodeBottomLeft(
    int width, int height, int &bestY, int &bestX) const {
  Rect bestNode;
...
MaxRectsBinPack::Rect MaxRectsBinPack::FindPositionForNewNodeBestShortSideFit(
...
MaxRectsBinPack::Rect MaxRectsBinPack::FindPositionForNewNodeBestLongSideFit(
    int width, int height, int &bestShortSideFit, int &bestLongSideFit) const {
  Rect bestNode;
...
MaxRectsBinPack::Rect MaxRectsBinPack::FindPositionForNewNodeBestAreaFit(
...
MaxRectsBinPack::Rect MaxRectsBinPack::FindPositionForNewNodeContactPoint(
...
void MaxRectsBinPack::PruneFreeList() {
  ...
  if (freeRectangles.size() > 0) {
          clb::sort::QuickSort(freeRectangles.Begin(), freeRectangles.GetSize(),
  NodeSortCmp); for (size_t i = 0; i < freeRectangles.GetSize()-1; ++i) if
  (freeRectangles[i].x == freeRectangles[i+1].x && freeRectangles[i].y ==
  freeRectangles[i+1].y && freeRectangles[i].width == freeRectangles[i+1].width
...
// (if mult > 0, the returned size is a multiple of that value, otherwise is a
// power of two)
...
int MaxRectsBinPack::ComputeTextureSize(const RectWIdxArr &rectsWIdx,
                                        int mult) {
...
GuillotineBinPack::RectWIdxArr
GuillotineBinPack::Insert(RectWIdxArr &unplacedRects, bool merge,
  ...
  // rectangles or there is no space. unplacedRects will get destroyed in the
  // process.
  RectWIdxArr placedRects;
  while (!unplacedRects.IsEmpty()) {
    ...
    for (size_t i = 0; i < freeRectangles.size(); ++i) {
      for (size_t j = 0; j < unplacedRects.GetSize(); ++j) {
        ...
        // Try if we can fit the rectangle upright.
                 ...
                 currentRect.height <= freeRectangles[i].height) {
          int score = ScoreByHeuristic(currentRect.width, currentRect.height,
        ...
        // If not, then perhaps flipping sideways will make it fit?
                 ...
                 currentRect.width <= freeRectangles[i].height) {
          int score = ScoreByHeuristic(currentRect.height, currentRect.width,
                                       freeRectangles[i], rectChoice);
          if (score < bestScore) {
            bestFreeRect = i;
            bestRect = j;
    ...
    placedRects.Insert(
        MaxRectsBinPack::RectWIdx{newNode, unplacedRects[bestRect].patchIdx});
...
GuillotineBinPack::Rect
GuillotineBinPack::Insert(int width, int height, bool merge,
                          FreeRectChoiceHeuristic rectChoice,
                          GuillotineSplitHeuristic splitMethod) {
...
GuillotineBinPack::Rect
GuillotineBinPack::FindPositionForNewNode(int width, int height,
                                          FreeRectChoiceHeuristic rectChoice,
                                          size_t &nodeIndex) {
  Rect bestNode;
...
SkylineBinPack::RectWIdxArr
SkylineBinPack::Insert(RectWIdxArr &unplacedRects,
...
// Perform the actual packing.
    ...
    placedRects.Insert(MaxRectsBinPack::RectWIdx{
        bestNode, unplacedRects[bestRectIndex].patchIdx});
    unplacedRects.RemoveAt(bestRectIndex);
  }
  return placedRects;
}

SkylineBinPack::Rect SkylineBinPack::Insert(int width, int height,
                                            LevelChoiceHeuristic method) {
  if (useWasteMap) {
...
SkylineBinPack::Rect SkylineBinPack::ScoreRect(int width, int height,
                                               LevelChoiceHeuristic method,
  ...
  case LevelBottomLeft:
    newNode =
        FindPositionForNewNodeBottomLeft(width, height, score1, score2, index);
    break;
  case LevelMinWasteFit:
    newNode =
        FindPositionForNewNodeMinWaste(width, height, score2, score1, index);
    break;
  default:
    ASSERT(false);
  }
...
SkylineBinPack::Rect SkylineBinPack::FindPositionForNewNodeBottomLeft(
    int width, int height, int &bestHeight, int &bestWidth,
    int &bestIndex) const {
...
SkylineBinPack::Rect SkylineBinPack::FindPositionForNewNodeMinWaste(
    int width, int height, int &bestHeight, int &bestWastedArea,
    int &bestIndex) const {
  bestHeight = std::numeric_limits<int>::max();
