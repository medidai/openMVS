...
public:
  ...
  inline bool IsEmpty() const {
    ASSERT(points.size() == pointViews.size() || pointViews.empty());
    return points.empty();
  }
  inline bool IsValid() const {
    ASSERT(points.size() == pointViews.size() || pointViews.empty());
    return !pointViews.empty();
  }
  ...
  Planef EstimateGroundPlane(const ImageArr &images, float planeThreshold = 0,
                             const String &fileExportPlane = "") const;

  bool Load(const String &fileName);
  bool Save(const String &fileName, bool bViews = false,
            bool bLegacyTypes = false, bool bBinary = true) const;
  bool SaveNViews(const String &fileName, uint32_t minViews,
                  bool bLegacyTypes = false, bool bBinary = true) const;
  bool SaveWithScale(const String &fileName, const ImageArr &images,
                     float scaleMult, bool bLegacyTypes = false,
                     bool bBinary = true) const;

  void PrintStatistics(const Image *pImages = NULL,
                       const OBB3f *pObb = NULL) const;
...
struct IntersectRayPoints {
  ...
  IntersectRayPoints(const Octree &octree, const Ray3 &_ray,
                     const PointCloud &_pointcloud, unsigned _minViews)
      : pointcloud(_pointcloud), cone(_ray, D2R(REAL(0.5))),
        coneIntersect(cone), minViews(_minViews) {
  ...
  void operator()(const IDX *idices, IDX size) {
    ...
    FOREACHRAWPTR(pIdx, idices, size) {
      ...
      if (!pointcloud.pointViews.empty() &&
          pointcloud.pointViews[idx].size() < minViews)
        continue;
