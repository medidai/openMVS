...
//   P = KR[I|-C]
// where R and C represent the camera orientation and position relative to the
// world coordinate system; R is expressed as the rotation from world to camera
// coordinates C is expressed as the explicit camera center in world coordinates
// (as opposite to standard form t which is negated and has the rotation of the
// camera already applied P = K[R|t]); the world and camera coordinates system
// is right handed, with x pointing right, y pointing down, and z pointing
// forward (see: R. Hartley, "Multiple View Geometry," 2004, pp. 156.); the
// projection in image coordinates uses the convention that the center of a
// pixel is defined at integer coordinates, i.e. the center is at (0, 0) and the
// top left corner is at (-0.5, -0.5)
...
public:
  ...
  inline CameraIntern(const Matrix3x3 &_K, const Matrix3x3 &_R,
                      const Point3 &_C)
      : K(_K), R(_R), C(_C) {}
  ...
  inline void UpdateTranslation(const Point3 &delta) { C += delta; }
  ...
  static inline TMatrix<TYPE, 3, 3> ComposeK(const TYPE &fX, const TYPE &fY,
                                             TYPER w = TYPER(1),
                                             TYPER h = TYPER(1)) {
    ASSERT(w > 0 && h > 0);
  ...
  template <typename TYPE, typename TYPER>
  static inline TMatrix<TYPE, 3, 3> ComposeInvK(const TYPE &fX, const TYPE &fY,
                                                TYPER w = TYPER(1),
                                                TYPER h = TYPER(1)) {
    ASSERT(w > 0 && h > 0);
  ...
  // scale image pixel coordinates with the given scale such that it accounts
  // for the convention that the center of a pixel is defined at integer
  // coordinates
  template <typename TYPE>
  static inline TPoint2<TYPE> ScaleImagePixel(const TPoint2<TYPE> &x, TYPE s) {
    return TPoint2<TYPE>((x.x + TYPE(0.5)) * s - TYPE(0.5),
  ...
  static inline TMatrix<TYPE, 3, 3> ScaleK(const TMatrix<TYPE, 3, 3> &K,
    ...
    return TMatrix<TYPE, 3, 3>(
        ...
        TYPE(0), K(1, 1) * s, (K(1, 2) + TYPE(0.5)) * s - TYPE(0.5), TYPE(0),
        TYPE(0), TYPE(1));
  }
  inline KMatrix GetScaledK(REAL s) const { return ScaleK(K, s); }
  // same as above, but for different scale on x and y;
  // in order to preserve the aspect ratio of the original size, scale both
  // focal lengths by the smaller of the scale factors, resulting in adding
  // pixels in the dimension that's growing;
  template <typename TYPE>
  static inline TMatrix<TYPE, 3, 3>
  ScaleK(const TMatrix<TYPE, 3, 3> &K, const cv::Size &size,
         const cv::Size &newSize, bool keepAspect = false) {
  ...
  inline KMatrix GetScaledK(const cv::Size &size, const cv::Size &newSize,
                            bool keepAspect = false) const {
  ...
  inline KMatrix GetInvK() const { return InvK(K); }
  ...
  inline TMatrix<TYPE, 3, 3> GetK(uint32_t width, uint32_t height) const {
    ...
    ASSERT(ISZERO(K(0, 1)));
    return ComposeK(TYPE(K(0, 0) * scale), TYPE(K(1, 1) * scale), width,
                    height);
  }
  template <typename TYPE>
  inline TMatrix<TYPE, 3, 3> GetInvK(uint32_t width, uint32_t height) const {
    ...
    ASSERT(ISZERO(K(0, 1)));
    return ComposeInvK(TYPE(K(0, 0) * scale), TYPE(K(1, 1) * scale), width,
                       height);
  }
  ...
  template <typename TYPE>
  static inline TMatrix<TYPE, 4, 4>
  ProjectionMatrixOpenGL(const TMatrix<TYPE, 3, 3> &K, const cv::Size &size,
                         TYPE nearZ, TYPE farZ, bool flip = true) {
    ...
    return TMatrix<TYPE, 4, 4>(fx * ihw, skew * ihw, cx * ihw - TYPE(1), 0, 0,
                               iy * fy * ihh, iy * (cy * ihh - TYPE(1)), 0, 0,
                               0, (farZ + nearZ) * ilen,
                               -TYPE(2) * farZ * nearZ * ilen, 0, 0, 1, 0);
  }
  inline Matrix4x4 GetProjectionMatrixOpenGL(const cv::Size &size, REAL nearZ,
                                             REAL farZ,
...
public:
  ...
  Camera(const Matrix3x3 &_K, const Matrix3x3 &_R, const Point3 &_C,
         bool bUpdate = true);

  Camera &operator=(const CameraIntern &camera);

  Camera GetScaled(REAL s) const; // return a camera scaled by the given factor
  Camera GetScaled(const cv::Size &size, const cv::Size &newSize)
      const; // return a camera scaled to the given resolution

  Matrix4x4
  GetP() const; // the composed projection matrix (4x4) assuming valid P
  ...
  void Transform(
      const Matrix3x3 &R, const Point3 &t,
  ...
  bool IsInFront(const Point3 &X)
      const; // test if the given 3D point is in front of the camera
  REAL DistanceSq(const Point3 &X)
      const; // compute the distance from the camera to the given 3D point
  inline REAL Distance(const Point3 &X) const { return SQRT(DistanceSq(X)); }

  static REAL StereoRectify(const cv::Size &size1, const Camera &camera1,
                            const cv::Size &size2, const Camera &camera2,
                            Matrix3x3 &R1, Matrix3x3 &R2, Matrix3x3 &K1,
                            Matrix3x3 &K2);
  static REAL StereoRectifyFusiello(const cv::Size &size1,
                                    const Camera &camera1,
                                    const cv::Size &size2,
                                    const Camera &camera2, Matrix3x3 &R1,
                                    Matrix3x3 &R2, Matrix3x3 &K1,
                                    Matrix3x3 &K2);
  static void SetStereoRectificationROI(
      const Point3fArr &points1, cv::Size &size1, const Camera &camera1,
  ...
  inline TPoint3<TYPE> ProjectPointP3(const TPoint3<TYPE> &X) const {
    ...
    return TPoint3<TYPE>((TYPE)(p[0 * 4 + 0] * X.x + p[0 * 4 + 1] * X.y +
                                p[0 * 4 + 2] * X.z + p[0 * 4 + 3]),
                         (TYPE)(p[1 * 4 + 0] * X.x + p[1 * 4 + 1] * X.y +
                                p[1 * 4 + 2] * X.z + p[1 * 4 + 3]),
                         (TYPE)(p[2 * 4 + 0] * X.x + p[2 * 4 + 1] * X.y +
                                p[2 * 4 + 2] * X.z + p[2 * 4 + 3]));
  }
  template <typename TYPE>
  ...
  inline TPoint2<TYPE> TransformPointC2I(const TPoint2<TYPE> &x) const {
    return TPoint2<TYPE>(TYPE(K(0, 2) + K(0, 0) * x.x),
  ...
  template <typename TYPE>
  inline bool IsInside(const TPoint2<TYPE> &pt,
                       const TPoint2<TYPE> &size) const {
    return pt.x >= 0 && pt.y >= 0 && pt.x < size.x && pt.y < size.y;
  }
  template <typename TYPE>
  inline bool IsInsideProjection(const TPoint3<TYPE> &X,
                                 const TPoint2<TYPE> &size) const {
    return IsInside(ProjectPoint(X), size);
  }
  template <typename TYPE>
  inline bool IsInsideProjectionP(const TPoint3<TYPE> &X,
                                  const TPoint2<TYPE> &size) const {
  ...
  template <typename TYPE> inline TYPE GetFootprintImage(TYPE depth) const {
  ...
  template <typename TYPE> inline TYPE GetFootprintWorld(TYPE depth) const {
...
MVS_API void DecomposeProjectionMatrix(const PMatrix &P, KMatrix &K, RMatrix &R,
                                       CMatrix &C);
MVS_API void DecomposeProjectionMatrix(const PMatrix &P, RMatrix &R,
                                       CMatrix &C);
MVS_API void AssembleProjectionMatrix(const KMatrix &K, const RMatrix &R,
                                      const CMatrix &C, PMatrix &P);
MVS_API void AssembleProjectionMatrix(const RMatrix &R, const CMatrix &C,
                                      PMatrix &P);
MVS_API Point3 ComputeCamerasFocusPoint(const CameraArr &cameras,
                                        const Point3 *pInitialFocus = NULL);
/*----------------------------------------------------------------*/
