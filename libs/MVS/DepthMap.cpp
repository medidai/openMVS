...
DEFVAR_OPTDENSE_uint32(
    ...
    DEFVAR_OPTDENSE_uint32(
        ...
        DEFVAR_OPTDENSE_uint32(
            ...
            "fusion in order to consider it inlier",
            "2") MDEFVAR_OPTDENSE_uint32(nMaxViewsFuse, "Max Views Fuse",
                                         "maximum number of neighbor "
                                         "depth-maps used during fusion",
                                         "32")
            DEFVAR_OPTDENSE_uint32(
                nMinViewsFilter, "Min Views Filter",
                "minimum number of images that agrees with an estimate in "
                "order to consider it inlier",
                "1") MDEFVAR_OPTDENSE_uint32(nMinViewsFilterAdjust,
                                             "Min Views Filter Adjust",
                                             "minimum number of images that "
                                             "agrees with an estimate in order "
                                             "to consider it inlier (0 - "
                                             "disabled)",
                                             "1")
                MDEFVAR_OPTDENSE_uint32(
                    nMinViewsTrustPoint, "Min Views Trust Point",
                    "min-number of views so that the point is considered for "
                    "approximating the depth-maps (<2 - random initialization)",
                    "2") MDEFVAR_OPTDENSE_uint32(nNumViews, "Num Views",
                                                 "Number of views used for "
                                                 "depth-map estimation (0 - "
                                                 "all views available)",
                                                 "0", "4", "8")
                    MDEFVAR_OPTDENSE_uint32(
                        nMinPixelsFuse, "Min Pixels Fuse",
                        "minimum number of depth-estimates that agree during "
                        "fusion in order to consider it (multiple pixels can "
                        "be from the same depth-map)",
                        "5") MDEFVAR_OPTDENSE_uint32(nMaxPointsFuse,
                                                     "Max Points Fuse",
                                                     ...
                                                     "1000")
                        MDEFVAR_OPTDENSE_uint32(
                            nMaxFuseDepth, "Max Fuse Depth",
                            "maximum depth in fusion graph traversal",
                            "100") MDEFVAR_OPTDENSE_uint32(nPointInsideROI,
                                                           "Point Inside ROI",
                                                           "consider a point "
                                                           "shared only if "
                                                           "inside ROI when "
                            ...
                            MDEFVAR_OPTDENSE_bool(
                                bAddCorners, "Add Corners",
                                "add support points at image corners with "
                                "nearest neighbor disparities",
                                "0") MDEFVAR_OPTDENSE_bool(bInitSparse,
                                                           "Init Sparse",
                                                           "init depth-map "
                                                           "only with the "
                                                           "sparse points (no "
                                                           ...
                                                           "1")
                                MDEFVAR_OPTDENSE_bool(
                                    bRemoveDmaps, "Remove Dmaps",
                                    "remove depth-maps after fusion",
                                    "0") MDEFVAR_OPTDENSE_float(fViewMinScore,
                                                                "View Min "
                                                                "Score",
                                                                "Min score to "
                                                                "consider a "
                                    ...
                                    MDEFVAR_OPTDENSE_float(
                                        fViewMinScoreRatio,
                                        "View Min Score Ratio",
                                        "Min score ratio to consider a "
                                        "neighbor images",
                                        "0.03") MDEFVAR_OPTDENSE_float(fMinArea,
                                                                       "Min "
                                                                       "Area",
                                                                       "Min "
                                                                       "shared "
                                        ...
                                        MDEFVAR_OPTDENSE_float(
                                            fMinAngle, "Min Angle",
                                            "Min angle for accepting the depth "
                                            "triangulation",
                                            "3.0")
                                            MDEFVAR_OPTDENSE_float(
                                                fOptimAngle, "Optim Angle",
                                                "Optimal angle for computing "
                                                "the depth triangulation",
                                                "12.0")
                                                MDEFVAR_OPTDENSE_float(
                                                    fMaxAngle, "Max Angle",
                                                    "Max angle for accepting "
                                                    "the depth triangulation",
                                                    "65.0")
                                                    MDEFVAR_OPTDENSE_float(
                                                        ...
                                                        "0.02") // 0.02: pixels
                                                                ...
                                                                // considered
                                                                // texture-less
    MDEFVAR_OPTDENSE_float(fDepthReprojectionErrorThreshold,
                           "Depth Reprojection Error Threshold",
                           "maximum relative difference between measured and "
                           "depth projected pixel",
                           "1.2")
        MDEFVAR_OPTDENSE_float(
            ...
            MDEFVAR_OPTDENSE_float(
                ...
                "0.3") MDEFVAR_OPTDENSE_float(fOptimizerEps, "Optimizer Eps",
                                              ...
                                              "0.001")
                MDEFVAR_OPTDENSE_int32(
                    nOptimizerMaxIters, "Optimizer Max Iters",
                    "MRF optimizer max number of iterations",
                    "80") MDEFVAR_OPTDENSE_uint32(nSpeckleSize, "Speckle Size",
                                                  "maximal size of a speckle "
                                                  "(small speckles get "
                                                  "removed)",
                                                  "100")
                    MDEFVAR_OPTDENSE_uint32(
                        ...
                        MDEFVAR_OPTDENSE_int32(nIgnoreMaskLabel,
                            ...
                            DEFVAR_OPTDENSE_uint32(
                                ...
                                "0") // see DepthFlags
    DEFVAR_OPTDENSE_uint32(
        nFuseFilter, "Fuse Filter",
        "how to fuse the depth-maps into one dense point-cloud?", "2", "0",
        "1") // see FuseMode
    MDEFVAR_OPTDENSE_uint32(
...
DepthData::DepthData(const DepthData &srcDepthData)
    : images(srcDepthData.images), neighbors(srcDepthData.neighbors),
      points(srcDepthData.points), mask(srcDepthData.mask),
      depthMap(srcDepthData.depthMap), normalMap(srcDepthData.normalMap),
      confMap(srcDepthData.confMap), dMin(srcDepthData.dMin),
      dMax(srcDepthData.dMax), size(srcDepthData.size),
...
bool DepthEstimator::ImportIgnoreMask(const Image &image0, const cv::Size &size,
                                      uint8_t nIgnoreMaskLabel,
...
//                         7 8 9
void DepthEstimator::MapMatrix2ZigzagIdx(const cv::Size &size,
...
#endif
      idxPixel(_idx), neighbors(0, 2),
#if DENSE_SMOOTHNESS != DENSE_SMOOTHNESS_NA
      neighborsClose(0, 4),
#endif
      scores(_depthData0.images.size() - 1), depthMap0(_depthData0.depthMap),
      normalMap0(_depthData0.normalMap), confMap0(_depthData0.confMap),
#if DENSE_NCC == DENSE_NCC_WEIGHTED
...
#if DENSE_REFINE == DENSE_REFINE_EXACT
DepthEstimator::PixelEstimate
DepthEstimator::PerturbEstimate(const PixelEstimate &est, float perturbation) {
...
TriangulatePointsDelaunay(const DepthData::ViewData &image,
                          const PointCloud &pointcloud, const IndexArr &points,
                          Mesh &mesh, Point2fArr &projs, bool bAddCorners) {
  ...
  if (bSparseOnly) {
    ...
    struct RasterDepth : TRasterMeshBase<RasterDepth> {
      ...
      RasterDepth(const Mesh::NormalArr &_vertexNormals, const Camera &_camera,
          ...
          : Base(_camera, _depthMap), vertexNormals(_vertexNormals),
            normalMap(_normalMap) {}
      inline void Raster(const ImageRef &pt, const Triangle &t,
                         const Point3f &bary) {
...
template <typename TYPE> class TPlaneSolverAdaptor {
public:
...
protected:
  ...
  double
      logalpha0_; // Alpha0 is used to make the error adaptive to the image size
...
void MVS::EstimatePointColors(const ImageArr &images, PointCloud &pointcloud) {
  ...
  DEBUG_ULTIMATE("Estimate dense point-cloud colors: %u colors (%s)",
                 pointcloud.colors.size(), TD_TIMER_GET_FMT().c_str());
} // EstimatePointColors
/*----------------------------------------------------------------*/

// estimate the segmentation labels of the given dense point-cloud using the
// given image masks
void MVS::EstimatePointSegmentation(const ImageArr &images,
                                    PointCloud &pointcloud, unsigned minViews) {
  TD_TIMER_START();

  ASSERT(minViews > 0 && pointcloud.IsValid());
  ASSERT(!images.empty() && !images.front().mask.empty());

  // estimate the segmentation labels for each point by projecting it into the
  // point views and setting the label by voting for the most frequent label in
  // image masks
...
#pragma omp parallel for
  ...
  DEBUG_ULTIMATE("Estimate dense point-cloud segmentation labels (%s)",
                 TD_TIMER_GET_FMT().c_str());
} // EstimatePointSegmentation

// overwrite point-cloud's colors with random colors, one for each segmentation
// label return the number of unique labels
unsigned MVS::ColorPointSegmentation(PointCloud &pointcloud) {
  ASSERT(!pointcloud.IsEmpty() && !pointcloud.labels.empty());
  ASSERT(pointcloud.colors.empty() ||
...
// as well as property maps to access each point's position and normal.
  ...
  DEBUG_ULTIMATE("Estimate dense point-cloud normals: %u normals (%s)",
                 pointcloud.normals.size(), TD_TIMER_GET_FMT().c_str());
...
// the estimated confidence is the mean of the depth differences to the 3 closer
// pixel to the central pixel
void MVS::EstimateConfidenceFromDepth(const DepthData &depthData,
                                      ConfidenceMap &confMap, int winHalfSize,
                                      int n) {
    ...
    for (int c = 0; c < depthMap.cols; c++) {
      ...
      for (int k = -winHalfSize; k <= winHalfSize; k++)
        for (int l = -winHalfSize; l <= winHalfSize; l++)
          if (r + k >= 0 && r + k < depthMap.rows && c + l >= 0 &&
              c + l < depthMap.cols && !(k == 0 && l == 0)) {
...
void MVS::EstimateConfidenceFromNormal(const DepthData &depthData,
...
#pragma omp parallel for
  ...
  for (int r = 0; r < normalMap.rows; r++) {
    for (int c = 0; c < normalMap.cols; c++) {
      ...
      for (int k = -winHalfSize; k <= winHalfSize; k++) {
        for (int l = -winHalfSize; l <= winHalfSize; l++) {
          if (r + k >= 0 && r + k < normalMap.rows && c + l >= 0 &&
              c + l < normalMap.cols && depthData.depthMap(r + k, c + l) > 0) {
...
unsigned MVS::FilterDepthMap(DepthMap &depthMap, NormalMap &normalMap,
                             const ConfidenceMap &confMap, float thConfidence) {
  ASSERT(!depthMap.empty());
...
bool MVS::ExportDepthDataRaw(const String &fileName,
  ...
  // write depth-map
  if (fwrite(depthMap.getData(), sizeof(float), depthMap.area(), f.get()) !=
      static_cast<size_t>(depthMap.area())) {
...
bool MVS::ImportDepthDataRaw(const String &fileName, String &imageFileName,
  ...
  if ((flags & HeaderDepthDataRaw::HAS_DEPTH) != 0) {
    ...
    if (fread(depthMap.getData(), sizeof(float), depthMap.area(), f.get()) !=
        static_cast<size_t>(depthMap.area())) {
      DEBUG("error: reading depth-data from file '%s'", fileName.c_str());
      return false;
    }
  } else {
    fseek(f.get(), sizeof(float) * header.depthWidth * header.depthHeight,
          SEEK_CUR);
  }
  ...
  if ((header.type & HeaderDepthDataRaw::HAS_NORMAL) != 0) {
    if ((flags & HeaderDepthDataRaw::HAS_NORMAL) != 0) {
      ...
      fseek(f.get(), sizeof(float) * 3 * header.depthWidth * header.depthHeight,
            SEEK_CUR);
    }
  }
...
#endif
  VERBOSE("Depth-maps compared for image % 3u: %.4f PSNR; %g median %g mean %g "
          "stddev error; %u (%.2f%%%%) error %u (%.2f%%%%) missing %u "
          "(%.2f%%%%) extra pixels (%s)",
...
void MVS::CompareNormalMaps(const NormalMap &normalMap,
  ...
  VERBOSE("Normal-maps compared for image % 3u: %.2f median %.2f mean %.2f "
          "stddev error (%s)",
