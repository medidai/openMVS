...
const Camera Camera::IDENTITY(Matrix3x3::eye(),
...
Camera::Camera(const Matrix3x3 &_K, const Matrix3x3 &_R, const Point3 &_C,
               bool bUpdate /*=true*/)
...
Camera Camera::GetScaled(REAL s) const { return Camera(GetScaledK(s), R, C); }
...
void Camera::ComposeP_RC() { AssembleProjectionMatrix(R, C, P); } // ComposeP_RC
void Camera::ComposeP() { AssembleProjectionMatrix(K, R, C, P); } // ComposeP
/*----------------------------------------------------------------*/
...
// decomposition of projection matrix into KR[I|-C]: internal calibration
// ([3,3]), rotation ([3,3]) and translation ([3,1]) (comparable with OpenCV:
// normalized cv::decomposeProjectionMatrix)
void MVS::DecomposeProjectionMatrix(const PMatrix &P, KMatrix &K, RMatrix &R,
                                    CMatrix &C) {
  ...
  RQDecomp3x3<REAL>(cv::Mat(3, 4, cv::DataType<REAL>::type,
...
void MVS::AssembleProjectionMatrix(const KMatrix &K, const RMatrix &R,
...
void MVS::AssembleProjectionMatrix(const RMatrix &R, const CMatrix &C,
                                   PMatrix &P) {
  Eigen::Map<Matrix3x3::EMat, 0, Eigen::Stride<4, 0>>(P.val) =
      (const Matrix3x3::EMat)R;
  Eigen::Map<Point3::EVec, 0, Eigen::Stride<0, 4>>(P.val + 3) =
      ((const Matrix3x3::EMat)R) * (-((const Point3::EVec)C));
} // AssembleProjectionMatrix
/*----------------------------------------------------------------*/

// compute the focus of attention of a set of cameras; only cameras
// that have the focus of attention in front of them are considered
Point3 MVS::ComputeCamerasFocusPoint(const CameraArr &cameras,
                                     const Point3 *pInitialFocus) {
  ...
  for (const Camera &camera : cameras) {
    ...
    const Matrix3x3::EMat m(Matrix3x3::EMat::Identity() -
                            dir * dir.transpose());
...
void GetImagePairROI(const Point3fArr &points1, const Point3fArr &points2,
                     const Matrix3x3 &K1, const Matrix3x3 &K2,
                     const Matrix3x3 &R1, const Matrix3x3 &R2,
                     const Matrix3x3 &invK1, const Matrix3x3 &invK2,
...
void SetCameraMatricesROI(const AABB2f &roi1h, const AABB2f &roi2h,
                          cv::Size &size1, cv::Size &size2, Matrix3x3 &K1,
                          Matrix3x3 &K2) {
  ...
  const int maxSize(
      MAXF(size1.width + size2.width, size1.height + size2.height) / 2);
  size1.width = size2.width =
      MINF(ROUND2INT(MAXF(size1h.x, size2h.x)), maxSize);
  size1.height = size2.height =
      MINF(ROUND2INT(MAXF(size1h.y, size2h.y)), maxSize);
...
// such that corresponding pixels in one image lie on the same scan-line in the
// other image; note: this function assumes that the two cameras are already
// undistorted
REAL Camera::StereoRectify(const cv::Size &size1, const Camera &camera1,
  ...
  ASSERT(ISEQUAL(-t.x, norm(camera2.C - camera1.C)) && ISZERO(t.y) &&
         ISZERO(t.z));
  ...
  for (int k = 0; k < 2; k++) {
    const Matrix3x3 H(k == 0 ? K1 * R1 * camera1.GetInvK()
                             : K2 * R2 * camera2.GetInvK());
    Point2f ptMin(std::numeric_limits<float>::max());
    for (int i = 0; i < 4; i++) {
      const Point2f pt((float)((i % 2) * size1.width),
...
// see: "A compact algorithm for rectification of stereo pairs", A. Fusiello, E.
// Trucco, and A. Verri, 2000
REAL Camera::StereoRectifyFusiello(const cv::Size &size1, const Camera &camera1,
                                   const cv::Size &size2, const Camera &camera2,
                                   Matrix3x3 &R1, Matrix3x3 &R2, Matrix3x3 &K1,
                                   Matrix3x3 &K2) {
  // compute relative pose
...
// adjust rectified camera matrices such that the entire area common to both
// source images is contained in the rectified images; as long as the
// focal-length on x and the skewness are equal in both cameras the point-scale
// is also equal in both images; note however that the surface-scale can still
// be different (most likely is) as it depends on its (unknown) normal too;
//  - points1 and points2: contain the pairs of corresponding pairs of image
//  projections and their depth
//  - size1 and size2: input the size of the source images, output the size of
//  the rectified images (rectified image sizes are equal)
void Camera::SetStereoRectificationROI(const Point3fArr &points1,
                                       cv::Size &size1, const Camera &camera1,
                                       const Point3fArr &points2,
                                       cv::Size &size2, const Camera &camera2,
                                       const Matrix3x3 &R1, const Matrix3x3 &R2,
                                       Matrix3x3 &K1, Matrix3x3 &K2) {
  ASSERT(!points1.empty() && points1.size() && points2.size());
  ...
  // determine the ROIs in rectified images
  AABB2f roi1h, roi2h;
  RECTIFY::GetImagePairROI(points1, points2, K1, K2, R1, R2, camera1.GetInvK(),
                           camera2.GetInvK(), roi1h, roi2h);
