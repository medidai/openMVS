...
template <int nTexels> struct WeightedPatchFix {
  struct Pixel {
    float weight;
    float tempWeight;
  };
...
struct MVS_API DepthEstimator {
  ...
  static bool ImportIgnoreMask(const Image &, const cv::Size &,
                               uint8_t nIgnoreMaskLabel, BitMatrix &,
                               Image8U * = NULL);
  static void MapMatrix2ZigzagIdx(const cv::Size &size,
...
MVS_API unsigned EstimatePlaneLockFirstPoint(const Point3Arr &, Plane &,
                                             ...
                                             size_t maxIters = 0);
MVS_API unsigned EstimatePlaneTh(const Point3Arr &, Plane &,
...
MVS_API void EstimatePointSegmentation(const ImageArr &images,
                                       PointCloud &pointcloud,
                                       unsigned minViews = 2);
MVS_API unsigned ColorPointSegmentation(PointCloud &pointcloud);
MVS_API void
EstimatePointNormals(const ImageArr &images, PointCloud &pointcloud,
                     int numNeighbors = 16 /*K-nearest neighbors*/);

MVS_API bool EstimateNormalMap(const Matrix3x3f &K, const DepthMap &,
                               NormalMap &);
MVS_API void EstimateConfidenceFromDepth(const DepthData &depthData,
                                         ConfidenceMap &confMap,
                                         int winHalfSize = 1, int n = 3);
MVS_API void EstimateConfidenceFromNormal(const DepthData &depthData,
                                          ConfidenceMap &confMap,
                                          int winHalfSize = 1);
...
MVS_API unsigned FilterDepthMap(DepthMap &depthMap, NormalMap &normalMap,
                                const ConfidenceMap &confMap,
                                float thConfidence = 0.5f);
...
MVS_API bool ExportDepthDataRaw(const String &, const String &imageFileName,
                                ...
                                const KMatrix &, const RMatrix &,
                                const CMatrix &, Depth dMin, Depth dMax,
                                const DepthMap &, const NormalMap &,
                                const ConfidenceMap &, const ViewsMap &);
