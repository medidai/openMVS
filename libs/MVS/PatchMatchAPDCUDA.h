...
PATCHMATCH_APD_HOST_DEVICE constexpr bool APDModeEnabled(unsigned mode)
{
	return mode == static_cast<unsigned>(APDMode::FULL) ||
		mode == static_cast<unsigned>(APDMode::DEFORMATION_ONLY);
}

PATCHMATCH_APD_HOST_DEVICE constexpr bool APDModeUsesFullMechanics(unsigned mode)
{
	return mode == static_cast<unsigned>(APDMode::FULL);
}
...
{
	if (config.mode > static_cast<unsigned>(APDMode::DEFORMATION_ONLY))
...
struct APDWorkingCostMinimum {
	float minimum = FLT_MAX;
	unsigned finiteCount = 0u;
};

PATCHMATCH_APD_HOST_DEVICE inline bool AccumulateAPDWorkingCostMinimum(
	APDWorkingCostMinimum& summary,
	float workingCost)
{
	if (!APDFinite(workingCost) || workingCost >= APD_BAD_COST)
		return false;
	++summary.finiteCount;
	if (workingCost < summary.minimum)
		summary.minimum = workingCost;
	return true;
}
