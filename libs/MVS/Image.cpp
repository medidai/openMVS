...
bool Image::ReadImage(IMAGEPTR pImage, Image8U3 &image) {
  ...
  if (FAILED(pImage->ReadData(image.data, PF_R8G8B8, 3,
                              (CImage::Size)image.step))) {
...
// free the image data
void Image::ReleaseImage() { image.release(); } // ReleaseImage
/*----------------------------------------------------------------*/
...
float Image::ResizeImage(unsigned nMaxResolution) {
  ...
  if (nMaxResolution == 0 || MAXF(width, height) <= nMaxResolution)
    return 1.f;
  const REAL scale(width > height ? (REAL)nMaxResolution / width
                                  : (REAL)nMaxResolution / height);
...
// compute image scale for a given max and min resolution, using the current
// image file data
unsigned Image::RecomputeMaxResolution(unsigned &level, unsigned minImageSize,
                                       unsigned maxImageSize) const {
  IMAGEPTR pImage(ReadImageHeader(name));
  if (pImage == NULL) {
    // something went wrong, use the current known size (however it will most
    // probably fail later)
    return Image8U3::computeMaxResolution(width, height, level, minImageSize,
                                          maxImageSize);
  }
  // re-compute max image size
  return Image8U3::computeMaxResolution(pImage->GetWidth(), pImage->GetHeight(),
                                        level, minImageSize, maxImageSize);
} // RecomputeMaxResolution
/*----------------------------------------------------------------*/

// resize image at desired scale
Image Image::GetImage(const PlatformArr &platforms, double scale,
                      bool bUseImage) const {
  ...
  if (!image.empty()) {
    if (bUseImage)
      cv::resize(image, scaledImage.image, scaledSize, 0, 0,
...
// compute the camera extrinsics from the platform pose and the relative camera
// pose to the platform
Camera Image::GetCamera(const PlatformArr &platforms,
                        const Image8U::Size &resolution) const {
...
REAL Image::ComputeFOV(int dir) const {
  ...
  case 2: // diagonal
    return 2 * ATAN(SQRT(REAL(SQUARE(width) + SQUARE(height))) /
...
//  - input leftPoints and rightPoints contains the pairs of corresponding image
//  projections and their depth (optional)
//  - H converts a pixel from original to rectified left-image
//  - Q converts [x' y' disparity 1] in rectified coordinates to [x*z y*z z 1]*w
//  in original image coordinates
//    where disparity=x1-x2
bool Image::StereoRectifyImages(const Image &image1, const Image &image2,
                                const Point3fArr &points1,
                                const Point3fArr &points2,
                                Image8U3 &rectifiedImage1,
                                Image8U3 &rectifiedImage2, Image8U &mask1,
                                Image8U &mask2, Matrix3x3 &H, Matrix4x4 &Q) {
  ASSERT(image1.IsValid() && image2.IsValid());
  ASSERT(image1.GetSize() == image1.image.size() &&
         image2.GetSize() == image2.image.size());
...
#elif 1
  const REAL t(Camera::StereoRectifyFusiello(image1.GetSize(), image1.camera,
                                             image2.GetSize(), image2.camera,
                                             R1, R2, K1, K2));
#else
  Pose pose;
  ComputeRelativePose(image1.camera.R, image1.camera.C, image2.camera.R,
                      image2.camera.C, pose.R, pose.C);
  cv::Mat P1, P2;
  cv::stereoRectify(image1.camera.K, cv::noArray(), image2.camera.K,
                    cv::noArray(), image1.GetSize(), pose.R,
                    Vec3(pose.GetTranslation()), R1, R2, P1, P2, Q,
                    0 /*cv::CALIB_ZERO_DISPARITY*/, -1);
  ...
  // source images is contained in the rectified images
  cv::Size size1(image1.GetSize()), size2(image2.GetSize());
  if (!points1.empty())
    Camera::SetStereoRectificationROI(points1, size1, image1.camera, points2,
                                      size2, image2.camera, R1, R2, K1, K2);
  ASSERT(size1 == size2);
  ...
  cv::warpPerspective(image1.image, rectifiedImage1, H1,
                      rectifiedImage1.size());
  rectifiedImage2.create(size2);
  cv::warpPerspective(image2.image, rectifiedImage2, H2,
                      rectifiedImage2.size());

  // mark valid regions covered by the rectified images
  struct Compute {
    static void Mask(Image8U &mask, const cv::Size &sizeh, const cv::Size &size,
                     const Matrix3x3 &H) {
      mask.create(sizeh);
      mask.memset(0);
  ...
  // from the formula that relates disparity to depth as z=B*f/d where B=-t and
  // d=x_l-x_r and the formula that converts the image projection from right to
  // left x_r=K1*K2.inv()*x_l compute the inverse projection matrix that
  // transforms image coordinates in image 1 and its corresponding disparity
  // value to the 3D point in camera 1 coordinates as:
  ...
  cv::Mat(image1.camera.K * R1.t())
      .copyTo(cv::Mat(4, 4, cv::DataType<Matrix4x4::Type>::type,
                      P.val)(cv::Rect(0, 0, 3, 3)));
  Q = P * Q;
  return true;
}

// adjust H and Q such that they correspond to rectified images scaled by the
// given factor
...
// same as above, but converts also the coordinates from rectified to original
// image
template <typename TYPE>
float TDisparity2Depth(const Matrix4x4 &Q, const TPoint2<TYPE> &u, float d,
                       Point2f &pt) {
...
float Image::Disparity2Distance(const Matrix3x3 &K, const Matrix4x4 &Q,
                                const Point2f &u, float d) {
  Point2f pt;
  const float depth(Disparity2Depth(Q, u, d, pt));
  return (float)(SQRT(SQUARE(pt.x - K(0, 2)) + SQUARE(pt.y - K(1, 2)) +
                      SQUARE(K(0, 0))) *
                 depth / K(0, 0));
}
...
bool Image::Depth2Disparity(const Matrix4x4 &Q, const Point2f &u, float d,
                            float &disparity) {
  const REAL w((Q(3, 0) * u.x + Q(3, 1) * u.y + Q(3, 2)) * d + Q(3, 3));
  if (ISZERO(w))
    return false;
