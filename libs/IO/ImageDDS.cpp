...
typedef struct DDSINFOHEADER_TYPE {
  ...
  DWORD dwPitchOrLinearSize; // distance to start of next line or formless
                             // late-allocated optimized surface size
...
#define IMAGE_DDS_DDSINFOHEADERSIZE                                            \
  (sizeof(DDSINFOHEADER) - sizeof(DWORD) /*dwHeader*/) // should be 124

#define ISBITMASK(r, g, b, a)                                                  \
  (ddsInfo.ddsPixelFormat.dwRBitMask == r &&                                   \
   ddsInfo.ddsPixelFormat.dwGBitMask == g &&                                   \
   ddsInfo.ddsPixelFormat.dwBBitMask == b &&                                   \
   ddsInfo.ddsPixelFormat.dwRGBAlphaBitMask == a)

// S T R U C T S ///////////////////////////////////////////////////

CImageDDS::CImageDDS() {} // Constructor

CImageDDS::~CImageDDS() {} // Destructor
/*----------------------------------------------------------------*/
...
HRESULT CImageDDS::ReadHeader() {
  ...
  if (ddsInfo.dwHeader != IMAGE_DDS_TYPE &&
      ...
      !(ddsInfo.dwFlags &
        (DDSDCaps | DDSDPixelFormat | DDSDWidth |
         DDSDHeight))) { // always include DDSD_CAPS, DDSD_PIXELFORMAT,
  ...
  m_numLevels =
      ddsInfo.dwFlags & DDSDMipMapCount ? (BYTE)ddsInfo.dwMipMapCount : 0;
  m_level = 0;
...
HRESULT CImageDDS::ReadData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                            Size lineWidth) {
  // read data
...
HRESULT CImageDDS::WriteHeader(PIXELFORMAT imageFormat, Size width, Size height,
                               BYTE numLevels) {
  return _FAIL;
} // WriteHeader
/*----------------------------------------------------------------*/

HRESULT CImageDDS::WriteData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                             Size lineWidth) {
  return _FAIL;
} // WriteData
/*----------------------------------------------------------------*/
