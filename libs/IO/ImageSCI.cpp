...
#define IMAGE_SCI_HEADER                                                       \
  ((DWORD)((((DWORD)IMAGE_SCI_VERSION) << 24) |                                \
           (DWORD)IMAGE_SCI_TYPE)) //"SCI"+ver = 4 bytes
...
CImageSCI::CImageSCI() {} // Constructor

CImageSCI::~CImageSCI() {} // Destructor
/*----------------------------------------------------------------*/
...
HRESULT CImageSCI::WriteHeader(PIXELFORMAT imageFormat, Size width, Size height,
                               BYTE numLevels) {
  // write header
  CImage::WriteHeader(imageFormat, width, height, numLevels);
  ((OSTREAM *)m_pStream)->setPos(0);
  const SCIINFOHEADER sciInfo = {IMAGE_SCI_HEADER,
                                 (uint16_t)m_width,
                                 (uint16_t)m_height,
                                 (uint8_t)m_format,
                                 m_numLevels,
                                 0,
                                 0};
  if (sizeof(SCIINFOHEADER) !=
      m_pStream->write(&sciInfo, sizeof(SCIINFOHEADER))) {
