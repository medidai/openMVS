...
static TIFF *_tiffStreamOpen(const char *name, const char *mode, void *fd) {
  ...
  if (strchr(mode, 'w')) {
    ...
    tif = TIFFClientOpen(name, mode, reinterpret_cast<thandle_t>(data),
                         ...
                         _tiffosCloseProc, _tiffosSizeProc, _tiffDummyMapProc,
                         _tiffDummyUnmapProc);
  } else {
...
// TIFFOpen() mode flags are different to fopen().  A 'b' in mode "rb" has no
// effect when reading.
...
CImageTIFF::CImageTIFF() : m_state(NULL) {} // Constructor
...
HRESULT CImageTIFF::ReadHeader() {
  ...
  if (TIFFGetField(tif, TIFFTAG_IMAGEWIDTH, &m_width) &&
    ...
    if (bpp > 8 && ((photometric != 2 && photometric != 1) ||
                    (ncn != 1 && ncn != 3 && ncn != 4)))
      bpp = 8;
    switch (bpp) {
    case 8:
...
HRESULT CImageTIFF::ReadData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                             Size lineWidth) {
  if (m_state && m_width && m_height) {
    ...
    if ((!is_tiled) ||
      ...
      if (!is_tiled && tile_height0 == 1 && dataFormat == m_format &&
          nStride == m_stride) {
        ...
        for (uint32_t y = 0; y < m_height;
             y += tile_height0, data += lineWidth * tile_height0) {
          ...
          for (uint32_t x = 0; x < m_width; x += tile_width0) {
            ...
            case 8: {
              ...
              for (uint32_t i = 0; i < tile_height; ++i) {
                ...
                if (!FilterFormat(dst, dataFormat, nStride, src, m_format,
                                  m_stride, tile_width)) {
                  Close();
                  return _FAIL;
                }
...
HRESULT CImageTIFF::WriteHeader(PIXELFORMAT imageFormat, Size width,
                                Size height, BYTE numLevels) {
  // TODO: to implement the TIFF encoder
  return _OK;
} // WriteHeader
/*----------------------------------------------------------------*/

HRESULT CImageTIFF::WriteData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                              Size lineWidth) {
  // TODO: to implement the TIFF encoder
  return _OK;
} // WriteData
