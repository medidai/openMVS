...
void custom_png_read_file(png_structp png_ptr, png_bytep buffer,
                          png_size_t size) {
  png_voidp const ptrIO = png_get_io_ptr(png_ptr);
  ((InputStream *)ptrIO)->read(buffer, size);
}

void custom_png_write_file(png_structp png_ptr, png_bytep buffer,
                           png_size_t size) {
  png_voidp const ptrIO = png_get_io_ptr(png_ptr);
...
CImagePNG::CImagePNG() : m_png_ptr(NULL), m_info_ptr(NULL) {} // Constructor

CImagePNG::~CImagePNG() { Close(); } // Destructor
/*----------------------------------------------------------------*/
...
HRESULT CImagePNG::ReadData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                            Size lineWidth) {
  png_structp png_ptr = (png_structp)m_png_ptr;
  ...
  if (nStride == m_stride &&
      (m_format != PF_B8G8R8A8 ||
       (dataFormat != PF_A8R8G8B8 && dataFormat != PF_A8B8G8R8))) {
...
HRESULT CImagePNG::WriteHeader(PIXELFORMAT imageFormat, Size width, Size height,
                               BYTE /*numLevels*/) {
  // initialize stuff
  ASSERT(m_png_ptr == NULL);
  ...
  png_set_write_fn(png_ptr, m_pStream, custom_png_write_file,
                   custom_png_flush_file);
...
HRESULT CImagePNG::WriteData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                             Size lineWidth) {
  png_structp png_ptr = (png_structp)m_png_ptr;
