...
// All formats are listed from left to right, most-significant bit to
// least-significant bit. For example, PF_A8R8G8B8 is ordered from the
// most-significant bit channel A (alpha), to the least-significant bit channel
// B (blue). When traversing image data, the data is stored in memory on a
// little-endian machine from least-significant bit to most-significant bit,
// which means that the channel order in memory is from least-significant bit
// (blue) to most-significant bit (alpha).
...
public:
  enum IMCREATE { READ, WRITE };
  typedef uint32_t Size;

  CImage() {}
  virtual ~CImage() {}

  virtual HRESULT Reset(Size width, Size height, PIXELFORMAT pixFormat,
                        Size levels = 1, bool bAllocate = false);
  ...
  virtual HRESULT WriteHeader(PIXELFORMAT, Size width, Size height,
                              BYTE numLevels);
  ...
  static bool FilterFormat(void *, PIXELFORMAT, Size, const void *, PIXELFORMAT,
                           Size, Size nSzize);
  static void FlipRB24(uint8_t *data, Size size, Size stride);
  static void CopyFlipRB24(uint8_t *pDst, const uint8_t *pSrc, Size size,
