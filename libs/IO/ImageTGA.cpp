...
typedef struct TGAINFOHEADER_TYPE {
  ...
  struct {              // Image specifications
    uint16_t shXOrigin; // X-origin, absolute coordinate of lower-left corner
    ...
    union {
      BYTE byAlphaInfo; // Image descriptor, bits 3-0 give the alpha channel
                        // depth, bits 4-5 give direction
      struct {
...
CImageTGA::CImageTGA() {} // Constructor

CImageTGA::~CImageTGA() {} // Destructor
/*----------------------------------------------------------------*/
...
HRESULT CImageTGA::ReadData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                            Size lineWidth) {
  // read data
  if (dataFormat == m_format && nStride == m_stride) {
    ...
    for (Size j = 0; j < m_height; ++j) {
      if (m_lineWidth != m_pStream->read(buffer, m_lineWidth))
        return _INVALIDFILE;
      if (!FilterFormat((BYTE *)pData + (m_height - j - 1) * lineWidth,
...
HRESULT CImageTGA::WriteHeader(PIXELFORMAT imageFormat, Size width, Size height,
                               BYTE numLevels) {
  return _FAIL;
} // WriteHeader
/*----------------------------------------------------------------*/

HRESULT CImageTGA::WriteData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                             Size lineWidth) {
  return _FAIL;
} // WriteData
/*----------------------------------------------------------------*/
