...
CImageBMP::CImageBMP() {} // Constructor

CImageBMP::~CImageBMP() {} // Destructor
/*----------------------------------------------------------------*/
...
HRESULT CImageBMP::ReadHeader() {
  ...
  if (sizeof(BITMAPFILEHEADER) !=
          m_pStream->read(&bmp_fileheader, sizeof(BITMAPFILEHEADER)) ||
  ...
  if (sizeof(BITMAPINFOHEADER) !=
          m_pStream->read(&bmp_infoheader, sizeof(BITMAPINFOHEADER)) ||
  ...
  if (bmp_infoheader.biPlanes != 1 ||
      (bmp_infoheader.biCompression != BI_RGB &&
       bmp_infoheader.biCompression != BI_BITFIELDS)) {
  ...
  m_numLevels = 0;
  m_level = 0;
  m_lineWidth = (bmp_fileheader.bfSize - bmp_fileheader.bfOffBits) /
                bmp_infoheader.biHeight; // bmp_infoheader.biSizeImage may be
...
HRESULT CImageBMP::ReadData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                            Size lineWidth) {
  // read data
  ...
  if (dataFormat == m_format && nStride == m_stride) {
    ...
    for (Size j = 0; j < m_height; ++j) {
      if (m_lineWidth != m_pStream->read(buffer, m_lineWidth))
        return _INVALIDFILE;
      if (!FilterFormat((uint8_t *)pData + (m_height - j - 1) * lineWidth,
                        dataFormat, nStride, buffer, m_format, m_stride,
                        m_width))
        return _FAIL;
    }
  }
...
HRESULT CImageBMP::WriteHeader(PIXELFORMAT imageFormat, Size width, Size height,
                               BYTE /*numLevels*/) {
  // write header
  m_numLevels = 0;
  m_level = 0;
  ...
  const BITMAPFILEHEADER bmp_fileheader = {
      ...
      (DWORD)sizeof(BITMAPFILEHEADER) + (DWORD)sizeof(BITMAPINFOHEADER) +
          m_lineWidth * m_height, // total file size
      0, 0,                       // reserved
      (DWORD)sizeof(BITMAPFILEHEADER) +
          (DWORD)sizeof(BITMAPINFOHEADER) // offset to the bitmap bits
  };
  ...
  if (sizeof(BITMAPFILEHEADER) !=
          m_pStream->write(&bmp_fileheader, sizeof(BITMAPFILEHEADER)) ||
      sizeof(BITMAPINFOHEADER) !=
          m_pStream->write(&bmp_infoheader, sizeof(BITMAPINFOHEADER))) {
...
HRESULT CImageBMP::WriteData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                             Size lineWidth) {
  // write data
  const size_t nSize = m_width * m_stride;
  const size_t nPad = m_lineWidth - nSize;
  CAutoPtrArr<uint8_t> const bufferPad(nPad ? new uint8_t[nPad] : NULL);
  if (nPad)
    memset(bufferPad, 0, nPad);
  if (dataFormat == m_format && nStride == m_stride) {
    // write data buffer directly to the image
    for (Size j = 0; j < m_height; ++j)
      if (nSize !=
              m_pStream->write(
                  (uint8_t *)pData + (m_height - j - 1) * lineWidth, nSize) ||
    ...
    CAutoPtrArr<uint8_t> const buffer(new uint8_t[m_lineWidth]);
    for (Size j = 0; j < m_height; ++j) {
      if (!FilterFormat(buffer, m_format, m_stride,
                        (uint8_t *)pData + (m_height - j - 1) * lineWidth,
                        dataFormat, nStride, m_width))
        return _FAIL;
