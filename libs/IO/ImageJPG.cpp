...
skip_input_data(j_decompress_ptr cinfo, long num_bytes) {
  ...
  if (num_bytes > (long)source->pub.bytes_in_buffer) {
    ...
    source->pStream->setPos(source->pStream->getPos() +
                            (num_bytes - source->pub.bytes_in_buffer));
...
CImageJPG::CImageJPG() : m_state(NULL) {} // Constructor
...
HRESULT CImageJPG::ReadHeader() {
  ...
  if (setjmp(state->jerr.setjmp_buffer) == 0) {
    ...
    state->source.buffer = (JOCTET *)(*state->cinfo.mem->alloc_small)(
        (j_common_ptr)&state->cinfo, JPOOL_PERMANENT,
        JPG_BUFFER_SIZE * sizeof(JOCTET));
...
HRESULT CImageJPG::ReadData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                            Size lineWidth) {
  JpegState *state = (JpegState *)m_state;
    ...
    if (setjmp(jerr->setjmp_buffer) == 0) {
      ...
      if (dataFormat == m_format && nStride == m_stride) {
        ...
        JSAMPARRAY buffer = (*cinfo->mem->alloc_sarray)(
            (j_common_ptr)cinfo, JPOOL_IMAGE, m_lineWidth, 1);
...
HRESULT CImageJPG::WriteHeader(PIXELFORMAT imageFormat, Size width, Size height,
                               BYTE numLevels) {
...
HRESULT CImageJPG::WriteData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                             Size lineWidth) {
  // TODO: to implement the JPG encoder
  ...
  //	if (flip)
  //		row_pointer[0] = (JSAMPROW)& buffer[(cinfo.image_height - 1 -
  //cinfo.next_scanline) * row_stride]; 	else 		row_pointer[0] = (JSAMPROW)&
  //buffer[cinfo.next_scanline * row_stride]; 	(void)
