...
#pragma omp critical
    ...
    if (mat.diffuse_name.IsEmpty())
      const_cast<String &>(mat.diffuse_name) =
          name + "_" + mat.name + "_map_Kd." + (texLossless ? "png" : "jpg");
...
bool ObjModel::Save(const String &fileName, unsigned precision,
                    bool texLossless) const {
  if (vertices.empty())
    return false;
...
bool ObjModel::Load(const String &fileName) {
  ...
  while (fin.good()) {
    ...
    if (keyword == "v") {
      ...
      for (size_t k = 0; k < 3; ++k) {
        in >> keyword;
        switch (sscanf(keyword, "%u/%u/%u", f.vertices + k, f.texcoords + k,
                       f.normals + k)) {
        case 1:
...
ObjModel::MaterialLib::Material *ObjModel::GetMaterial(const String &name) {
  MaterialLib::Materials::iterator it(std::find_if(
      material_lib.materials.begin(), material_lib.materials.end(),
