...
HRESULT CImage::Reset(Size width, Size height, PIXELFORMAT pixFormat,
                      Size levels, bool bAllocate) {
...
HRESULT CImage::ReadHeader() { return _OK; } // ReadHeader
/*----------------------------------------------------------------*/

HRESULT CImage::ReadData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                         Size lineWidth) {
  // read data
  if (dataFormat == m_format && nStride == m_stride) {
    ...
    for (Size j = 0; j < m_dataHeight; ++j, (uint8_t *&)pData += lineWidth) {
      if (m_lineWidth != m_pStream->read(buffer, m_lineWidth))
        return _INVALIDFILE;
      if (!FilterFormat(pData, dataFormat, nStride, buffer, m_format, m_stride,
                        m_dataWidth))
        return _FAIL;
    }
  }
...
HRESULT CImage::WriteHeader(PIXELFORMAT imageFormat, Size width, Size height,
                            BYTE numLevels) {
  // write header
...
HRESULT CImage::WriteData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                          Size lineWidth) {
  // write data
  if (dataFormat == m_format && nStride == m_stride) {
    ...
    CAutoPtrArr<uint8_t> const buffer(new uint8_t[m_lineWidth]);
    for (Size j = 0; j < m_dataHeight; ++j, (uint8_t *&)pData += lineWidth) {
      if (!FilterFormat(buffer, m_format, m_stride, pData, dataFormat, nStride,
                        m_dataWidth))
        return _FAIL;
...
CImage::Size CImage::GetDataSizes(Size mipLevel, Size &width,
                                  Size &height) const {
...
bool CImage::FilterFormat(void *pDst, PIXELFORMAT formatDst, Size strideDst,
                          const void *pSrc, PIXELFORMAT formatSrc,
                          Size strideSrc, Size nSzize) {
  ...
  case PF_GRAY8:
    ...
    case PF_R5G6B5:
      ...
      for (Size i = 0; i < nSzize;
        ...
        ((uint8_t *)pDst)[0] =
            RGB24TO8(RGB16TOR(clr), RGB16TOG(clr), RGB16TOB(clr));
      }
      return true;
    ...
    case PF_R8G8B8:
      ...
      for (Size i = 0; i < nSzize;
           ++i, (uint8_t *&)pDst += strideDst, (uint8_t *&)pSrc += strideSrc)
        ((uint8_t *)pDst)[0] = RGB24TO8(
            ((uint8_t *)pSrc)[2], ((uint8_t *)pSrc)[1], ((uint8_t *)pSrc)[0]);
      return true;
    }
    break;
  ...
  case PF_R5G6B5:
    ...
    case PF_GRAY8:
      ...
      for (Size i = 0; i < nSzize;
           ++i, (uint8_t *&)pDst += strideDst, (uint8_t *&)pSrc += strideSrc)
        *((uint16_t *)pDst) = RGB24TO16(
            ((uint8_t *)pSrc)[0], ((uint8_t *)pSrc)[0], ((uint8_t *)pSrc)[0]);
      return true;
    ...
    case PF_A8R8G8B8:
      ...
      for (Size i = 0; i < nSzize;
           ++i, (uint8_t *&)pDst += strideDst, (uint8_t *&)pSrc += strideSrc)
        *((uint16_t *)pDst) = RGB24TO16(
            ((uint8_t *)pSrc)[3], ((uint8_t *)pSrc)[2], ((uint8_t *)pSrc)[1]);
      return true;
    ...
    case PF_B8G8R8:
      ...
      for (Size i = 0; i < nSzize;
           ++i, (uint8_t *&)pDst += strideDst, (uint8_t *&)pSrc += strideSrc)
        *((uint16_t *)pDst) = RGB24TO16(
            ((uint8_t *)pSrc)[0], ((uint8_t *)pSrc)[1], ((uint8_t *)pSrc)[2]);
      return true;
    ...
    case PF_A8B8G8R8:
      ...
      for (Size i = 0; i < nSzize;
           ++i, (uint8_t *&)pDst += strideDst, (uint8_t *&)pSrc += strideSrc)
        *((uint16_t *)pDst) = RGB24TO16(
            ((uint8_t *)pSrc)[1], ((uint8_t *)pSrc)[2], ((uint8_t *)pSrc)[3]);
      return true;
    }
    break;
  ...
  case PF_R8G8B8:
    ...
    case PF_GRAY8:
      ...
      for (Size i = 0; i < nSzize;
           ++i, (uint8_t *&)pDst += strideDst, (uint8_t *&)pSrc += strideSrc)
        ((uint8_t *)pDst)[0] = ((uint8_t *)pDst)[1] = ((uint8_t *)pDst)[2] =
            *((uint8_t *)pSrc);
      return true;
  ...
  case PF_A8R8G8B8:
    ...
    case PF_GRAY8:
      ...
      for (Size i = 0; i < nSzize;
           ++i, (uint8_t *&)pDst += strideDst, (uint8_t *&)pSrc += strideSrc) {
        ((uint8_t *)pDst)[0] = 0xFF;
        ((uint8_t *)pDst)[1] = ((uint8_t *)pDst)[2] = ((uint8_t *)pDst)[3] =
            *((uint8_t *)pSrc);
      }
      return true;
...
// size is the number of pixels to process.
// stride is the number of bytes used for one pixel for dest and source
// respectively.
void CImage::CopyFlipRB24(uint8_t *pDst, const uint8_t *pSrc, Size size,
                          Size strideDst, Size strideSrc) {
...
#endif
#ifdef _IMAGE_TIFF
  else if (_tcsncicmp(fext, _T(".tif"), 4) == 0 ||
           _tcsncicmp(fext, _T(".tiff"), 5) == 0)
...
void CImage::Dump(LPCTSTR szFileName) {
  const String strName =
      Util::getFileName(szFileName) + String::ToString(m_width) + _T("x") +
