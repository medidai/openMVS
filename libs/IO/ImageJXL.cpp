...
struct JxlState {
  ...
  bool ReadStreamChunk(IOSTREAMPTR &stream, size_t chunk_size = 1024 * 64) {
    ...
    size_t read = in->read(compressed.data() + compressed.size() - chunk_size,
                           chunk_size);
  ...
  bool HandleNeedMoreInput(IOSTREAMPTR &stream) {
    ...
    if (remaining_size > 0)
      memmove(compressed.data(),
              compressed.data() + compressed.size() - remaining_size,
              remaining_size);
...
HRESULT CImageJXL::ReadHeader() {
  ...
  if (!state->decoder)
    return _FAIL;
  JxlDecoderSubscribeEvents(state->decoder, JXL_DEC_BASIC_INFO |
  ...
  if (!state->ReadStreamChunk(m_pStream))
    return _FAIL;
  JxlDecoderSetInput(state->decoder, state->compressed.data(),
  ...
  for (;;) {
    ...
    if (status == JXL_DEC_BASIC_INFO) {
      if (JxlDecoderGetBasicInfo(state->decoder, &state->info) ==
          JXL_DEC_SUCCESS) {
    ...
    if (status == JXL_DEC_COLOR_ENCODING || status == JXL_DEC_FRAME ||
...
HRESULT CImageJXL::ReadData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                            Size lineWidth) {
  ...
  for (;;) {
    ...
    if (status == JXL_DEC_NEED_IMAGE_OUT_BUFFER) {
      if (JxlDecoderSetImageOutBuffer(state->decoder, &format, buffer,
    ...
    if (status == JXL_DEC_FULL_IMAGE) {
      if (buffer == pixels_u8.data()) {
        ...
        uint8_t *src = buffer;
        for (Size j = 0; j < m_height;
             ++j, dst += lineWidth, src += m_width * m_stride)
          if (!FilterFormat(dst, dataFormat, nStride, src, m_format, m_stride,
                            m_width))
            return _FAIL;
      }
      continue;
...
HRESULT CImageJXL::WriteHeader(PIXELFORMAT imageFormat, Size width, Size height,
                               BYTE numLevels) {
  m_width = width;
  m_height = height;
  m_dataWidth = width;
...
HRESULT CImageJXL::WriteData(void *pData, PIXELFORMAT dataFormat, Size nStride,
                             Size lineWidth) {
  if (!m_pStream)
    return _FAIL;
  ...
  format.data_type = (dataFormat == PF_GRAY32F) ? (JxlDataType)JXL_TYPE_FLOAT
                                                : (JxlDataType)JXL_TYPE_UINT8;
  format.endianness = (JxlEndianness)JXL_NATIVE_ENDIAN;
  format.align = 0u;
  if (JxlEncoderAddImageFrame(frame_settings, &format, pData,
                              m_height * lineWidth) != JXL_ENC_SUCCESS) {
  ...
  for (;;) {
    JxlEncoderStatus status =
        JxlEncoderProcessOutput(enc, &next_out, &avail_out);
