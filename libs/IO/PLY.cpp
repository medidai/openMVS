...
const int PLY::ply_type_size[] = {0, 1, 2, 4, 1, 2, 4, 4, 8};

const PLY::RuleName PLY::rule_name_list[] = {
    {AVERAGE_RULE, "avg"}, {RANDOM_RULE, "rnd"},     {MINIMUM_RULE, "max"},
    {MAXIMUM_RULE, "min"}, {MAJORITY_RULE, "major"}, {SAME_RULE, "same"},
    {-1, "end_marker"}};
...
void PLY::release() {
  if (mfp) {
    ...
    if (static_cast<Stream *>(istream)->getInputStream(ISTREAM::LAYER_ID_IN)) {
      IOSTREAM *const iostream(static_cast<Stream *>(istream)->getIOStream(
          ISTREAM::LAYER_ID_IN, OSTREAM::LAYER_ID_OUT));
      if (iostream)
        delete iostream;
      else
        delete istream;
    } else {
      ASSERT(static_cast<Stream *>(ostream)->getOutputStream(
          OSTREAM::LAYER_ID_OUT));
      IOSTREAM *const iostream(static_cast<Stream *>(ostream)->getIOStream(
          ISTREAM::LAYER_ID_IN, OSTREAM::LAYER_ID_OUT));
      if (iostream)
        delete iostream;
      else
...
bool PLY::write(LPCSTR _filename, int nelems, LPCSTR *elem_names,
                int _file_type, size_t memBufferSize) {
  filename = _filename;
  if (memBufferSize == 0) {
    // create output file now
    File *const pf(
        new File(filename.c_str(), File::WRITE, File::CREATE | File::TRUNCATE));
    if (!pf->isOpen())
      return false;
    return write(new BufferedOutputStream<true>(pf, 64 * 1024), nelems,
                 elem_names, _file_type, memBufferSize);
  }
  return write((OSTREAM *)NULL, nelems, elem_names, _file_type, memBufferSize);
}

bool PLY::write(OSTREAM *fp, int nelems, LPCSTR *elem_names, int _file_type,
...
void PLY::element_layout(const char *elem_name, int nelems, int nprops,
...
void PLY::describe_property(const char *elem_name, int nprops,
                            const PlyProperty *props) {
  // look for appropriate element
  put_element_setup(elem_name);
...
int PLY::get_element_description(const char *elem_name,
                                 std::vector<PlyProperty *> &prop_list) const {
...
void PLY::get_element_setup(const char *elem_name, int nprops,
                            PlyProperty *prop_list) {
  // find information about the element
  PlyElement *elem = find_element(elem_name);
  which_elem = elem;
  ...
  for (int i = 0; i < nprops; ++i) {
    ...
    if (index == -1) {
      DEBUG("warning: Can't find property '%s' in element '%s'",
...
std::vector<std::string> &PLY::get_comments() { return comments; }
...
std::vector<std::string> &PLY::get_obj_info() { return obj_info; }
...
PLY::PlyOtherProp *PLY::get_other_properties(const char *elem_name,
                                             int offset) {
  // find information about the element
  PlyElement *elem = find_element(elem_name);
  if (elem == NULL) {
...
PLY::PlyOtherElems *PLY::get_other_element() {
  ...
  other.other_props = get_other_properties(elem->name.c_str(),
...
void PLY::set_legacy_type_names() { write_type_names = old_type_names; }
...
void PLY::write_binary_item(const ValueType &val, int from_type, int to_type) {
  switch (to_type) {
  case Int8: {
...
void PLY::write_ascii_item(const ValueType &val, int from_type, int to_type) {
  switch (to_type) {
  case Int8:
  case Int16:
  case Int32:
...
void PLY::get_stored_item(const void *ptr, int type, ValueType &val) {
  switch (type) {
  case Int8:
...
void PLY::store_item(void *ptr, int to_type, const ValueType &val,
                     int from_type) {
  switch (to_type) {
  case Int8:
...
void PLY::setup_property(const PlyProperty &prop) {
  ...
  if (index == -1) {
    DEBUG("warning: Can't find property '%s' in element '%s'",
          prop.name.c_str(), elem->name.c_str());
    return;
  }
  PlyProperty *prop_ptr = elem->props[index];
  prop_ptr->internal_type = prop.internal_type;
...
void PLY::describe_other_properties(PlyOtherProp *other, int offset) {
  // look for appropriate element
  PlyElement *elem = find_element(other->name.c_str());
  if (elem == NULL) {
    DEBUG("warning: describe_other_properties: can't find element '%s'",
          other->name.c_str());
    return;
  }
...
PLY::PlyPropRules *PLY::init_rule(const char *elem_name) {
  ...
  for (PlyRuleList *list = rule_list; list != NULL; list = list->next) {
    ...
    if (!found_prop) {
      DEBUG("warning: Can't find property '%s' for rule '%s'", list->property,
            list->name);
      continue;
    }
  }
...
void PLY::modify_rule(PlyPropRules *rules, const char *prop_name,
                      int rule_type) {
...
void *PLY::get_new_props() {
  ...
  for (size_t i = 0; i < elem->props.size(); ++i) {
    ...
    case SAME_RULE: {
      ...
      for (size_t j = 1; j < rules->props.size(); ++j)
        if (val.d != vals[j])
          abort_ply("error: get_new_props: Error combining properties that "
                    "should be the same");
      break;
    }
    default:
...
void PLY::set_prop_rules(PlyRuleList *prop_rules) { rule_list = prop_rules; }
...
PLY::PlyRuleList *PLY::append_prop_rule(PlyRuleList *rule_list,
                                        const char *name,
                                        const char *property) {
  char *str2;
  char *ptr;
