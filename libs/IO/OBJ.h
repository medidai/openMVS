...
// defined as a list of vertices, and texture vertices. Vertices are stored in a
// counter-clockwise order by default, making explicit declaration of face
// normals unnecessary.
...
public:
  ...
  struct MaterialLib {
    ...
    // Saves the material lib to a .mtl file and all textures of its materials
    // with the given prefix name
    bool Save(const String &prefix, bool texLossless = false) const;
    // Loads the material lib from a .mtl file and all textures of its materials
...
public:
  ...
  // Saves the obj model to an .obj file, its material lib and the materials
  // with the given file name
  bool Save(const String &fileName, unsigned precision = 6,
            bool texLossless = false) const;
  // Loads the obj model from an .obj file, its material lib and the materials
  // with the given file name
  bool Load(const String &fileName);
