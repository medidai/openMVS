...
3. Algorithm features such as APD and DVP remain independently disabled by
   default. Enabling DVP requires geometric-consistency iterations.
4. An observer binary with an empty instrumentation directory performs no
   capture and emits no instrumentation artifacts.
5. Production DMAPs remain bit-exact when validating disabled, summary, and
   prefilter paths on the same host and build lineage.
6. Deep and trace `Process<true>` evidence is diagnostic-only unless separately
...
7. Unavailable is distinct from zero and invalid. Preserve reason codes.
8. Mechanics use initialization and complete logical iterations. Checkerboard
   phases appear only in timing evidence.
9. Existing complete captures and reports are immutable.
10. The current experiment-lock schema is v4. Capture closure is mandatory for
   ...
   pass.
11. Generated evidence remains outside Git.
...
For DVP changes, read
[`12_dvp_epipolar_debugging.md`](12_dvp_epipolar_debugging.md). Preserve the
four family identities, the scoped paper claim, side-independent Eq. 11 support,
native fallback, and the global-v5/nested-DVP-v1 schema boundary. Treat all DVP
summary/maps/trace evidence as diagnostic `Process<true>` mechanics; rank
quality only with fresh production endpoints.
