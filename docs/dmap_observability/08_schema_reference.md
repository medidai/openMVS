...
## DVP epipolar mechanics

Global frame-summary schema v5 extends the candidate namespace to 24 slots and
adds an independent `dvp` namespace. `openmvs.dmap.dvp_observability` and
`openmvs.dmap.dvp_pixel_mechanics` use nested schema v1.

Nested APD schema v5 preserves the APD v4 state, update, and trace binary
record sizes, but versions the combined-path meaning of the existing candidate
masks, winner/runner-up slots, and update source. Slots `22` and `23` are DVP
epipolar proposals and source code `12` is the DVP final winner. The identical
`openmvs.dmap.apd_candidate_accounting` object in the APD manifest and summary
declares whether those slots were active. DVP proposal details remain in the
independent DVP namespace; the APD record supplies complete active-objective
ordering and closure. APD schemas v1-v4 remain readable under their original
slot and source domains.

APD v5 also separates an evaluated fitted-plane score from the historical
support-usable count. Every IEEE-finite evaluated score is retained in the map;
`fitted_plane.finite_pixels` still means nonnegative and strictly below the
OpenMVS `fBadCost` sentinel. A candidate may strictly improve an even worse
incumbent while remaining non-usable, so accepted events are not required to
be a subset of usable events. The validator reports this population explicitly.

Summary rows contain one exact aggregate record per complete logical iteration.
Maps capture contains exactly 56 DVP artifacts per logical iteration. Targeted
`openmvs.dmap.dvp_trace` schema-v1 rows add per-view endpoint depths, view
identities, source offsets, proposals, costs, and final decisions. Endpoint
arrays remain targeted-trace-only; full-frame maps retain endpoint counts and
view masks rather than all samples.

The contract distinguishes generated, tested, finite, sequentially accepted,
final winner, and final depth retained. It also distinguishes an unavailable
interval from a numeric zero. For Eq. 11, left and right endpoint-support gates
are validated independently. See
[DVP epipolar proposals](12_dvp_epipolar_debugging.md).
