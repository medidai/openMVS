...
| `summary` | observer | `Process<false>` normally; APD/DVP exact aggregates use `Process<true>` | counters, availability, metadata, logical summaries, timings | ordinary summary only after endpoint parity; APD/DVP summary is diagnostic-only |
...
with the endpoint and require parity before using an ordinary non-APD/non-DVP
summary for quality gates. When APD or DVP exact aggregate mechanics are
requested, summary uses `Process<true>` so the hot kernel can emit exact
mechanism counters. The report marks that cohort diagnostic-only; its DMAP must
not be substituted for the production endpoint.
...
See [DVP epipolar proposals](12_dvp_epipolar_debugging.md) for the DVP-specific
summary, 56-map logical-iteration bundle, and targeted endpoint trace contract.
