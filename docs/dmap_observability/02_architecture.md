...
- `libs/MVS/PatchMatchDVPCUDA.h` owns the shared host/device DVP family,
  validation, Eq. 11 order-statistic, interval, and decision contract.
...
succeeds. Ordinary non-APD/non-DVP summary capture also uses `Process<false>`
and has the same parity requirement. APD and DVP exact aggregate summary
captures instead use `Process<true>` and are always diagnostic-only; the
production endpoint remains the quality authority.
